...
function emulate() {
  ...
  if (avd_list.length == 1) {
    shell.Run(
      "emulator -cpu-delay 0 -no-boot-anim -cache %Temp%cache -avd " +
        avd_list[0]
    );
  }
  ...
  if (avd_list.length > 1) {
    while (!avd_list[avd_id]) {
      WScript.Echo(
        "Choose from one of the following Android Virtual Devices [0 to " +
          (avd_list.length - 1) +
          "]:"
      );
...
function debug() {
  if (emulator_running()) {
    ...
    WScript.Echo(
      "# Plug in your device or launch an emulator with cordova/emulate #"
    );
    WScript.Echo(
      "##################################################################"
    );
