...
function downloadCommonsCodec() {
  if (!fso.FileExists(ROOT + "\\framework\\libs\\commons-codec-1.6.jar")) {
    // We need the .jar
    var url =
      "http://mirror.symnds.com/software/Apache//commons/codec/binaries/commons-codec-1.6-bin.zip";
    ...
    if (!fso.FileExists(savePath)) {
      ...
      if (xhr.status == 200) {
        ...
        WScript.Echo(
          "Could not retrieve the commons-codec. Please download it yourself and put into the framework/libs directory. This process may fail now. Sorry."
        );
      }
    }
    ...
    fso.MoveFile(
      ROOT + "\\framework\\libs\\commons-codec-1.6\\commons-codec-1.6.jar",
      ROOT + "\\framework\\libs\\commons-codec-1.6.jar"
    );
...
var ACTIVITY_PATH =
  PROJECT_PATH + "\\src\\" + PACKAGE_AS_PATH + "\\" + ACTIVITY + ".java";
...
// create the project
exec(
  "android.bat create project --target " +
    TARGET +
    " --path " +
    PROJECT_PATH +
    " --package " +
...
if (
  !fso.FileExists(ROOT + "\\cordova-" + VERSION + ".jar") &&
  !fso.FileExists(ROOT + "\\cordova-" + VERSION + ".js")
) {
  // update the cordova framework project to a target that exists on this machine
  exec(
    "android.bat update project --target " +
      TARGET +
      " --path " +
      ROOT +
      "\\framework"
...
exec(
  "%comspec% /c copy " +
    ROOT +
    "\\bin\\templates\\project\\AndroidManifest.xml " +
    PROJECT_PATH +
...
exec(
  "%comspec% /c copy " +
    ROOT +
    "\\bin\\templates\\project\\Activity.java " +
    ACTIVITY_PATH +
    " /Y"
);
...
exec(
  "%comspec% /c copy " +
    ROOT +
    "\\bin\\templates\\cordova\\appinfo.jar " +
    PROJECT_PATH +
    "\\cordova\\appinfo.jar /Y"
);
exec(
  "%comspec% /c copy " +
    ROOT +
    "\\bin\\templates\\cordova\\cordova.js " +
    PROJECT_PATH +
    "\\cordova\\cordova.js /Y"
);
exec(
  "%comspec% /c copy " +
    ROOT +
    "\\bin\\templates\\cordova\\cordova.bat " +
    PROJECT_PATH +
    "\\cordova\\cordova.bat /Y"
);
exec(
  "%comspec% /c copy " +
    ROOT +
    "\\bin\\templates\\cordova\\clean.bat " +
    PROJECT_PATH +
    "\\cordova\\clean.bat /Y"
);
exec(
  "%comspec% /c copy " +
    ROOT +
    "\\bin\\templates\\cordova\\debug.bat " +
    PROJECT_PATH +
    "\\cordova\\debug.bat /Y"
);
exec(
...
exec(
  "%comspec% /c copy " +
    ROOT +
    "\\bin\\templates\\cordova\\emulate.bat " +
    PROJECT_PATH +
    "\\cordova\\emulate.bat /Y"
);
exec(
  "%comspec% /c copy " +
    ROOT +
    "\\bin\\templates\\cordova\\BOOM.bat " +
    PROJECT_PATH +
    "\\cordova\\BOOM.bat /Y"
);
