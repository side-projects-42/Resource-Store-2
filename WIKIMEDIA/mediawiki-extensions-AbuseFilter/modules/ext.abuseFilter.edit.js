...
(function () {
  ...
  var $filterBox,
    ...
    toggleWarnPreviewButton,
    warnMessageExisting,
    warnMessageOther,
    toggleDisallowPreviewButton,
    disallowMessageExisting,
  ...
  function getCurrentMessage(action) {
    var existing =
        action === "warn" ? warnMessageExisting : disallowMessageExisting,
  ...
  function addText() {
    ...
    if (useAce) {
      ...
      $plainTextBox.textSelection("encapsulateSelection", {
    ...
    if ($element.css("display") !== "none") {
      ...
      api
        ...
        .done(function parseMessage(data) {
          api
            .parse(data.query.allmessages[0]["*"], {
              disablelimitreport: "",
              preview: "",
              prop: "text",
            ...
            .done(function showMessage(html) {
              ...
              previewButton.setFlags({ destructive: true, progressive: false });
            });
        });
    }
  }
  ...
  function onFilterGroupChange() {
    ...
    if (
      ...
      $afWarnMessageExisting = $(
        "#mw-abusefilter-warn-message-existing select"
      );
      newVal = mw.config.get("wgAbuseFilterDefaultWarningMessage")[
        $(this).val()
      ];

      if (
        $afWarnMessageExisting.find("option[value='" + newVal + "']").length
      ) {
    ...
    if (
      ...
      $afDisallowMessageExisting = $(
        "#mw-abusefilter-disallow-message-existing select"
      );
      newVal = mw.config.get("wgAbuseFilterDefaultDisallowMessage")[
        $(this).val()
      ];

      if (
        $afDisallowMessageExisting.find("option[value='" + newVal + "']").length
      ) {
  ...
  function buildSelector(action, config) {
    ...
    hiddenField = OO.ui.infuse(
      $("#mw-abusefilter-hidden-" + action + "-field")
    );
  ...
  $(function () {
    ...
    if (isFilterEditor) {
      ...
      $("#mw-abusefilter-warn-preview-button").on("click", function () {
        previewMessage("warn");
      });
      $("#mw-abusefilter-disallow-preview-button").on("click", function () {
        previewMessage("disallow");
      });
      $("#mw-abusefilter-warn-edit-button").on("click", function () {
        editMessage("warn");
      });
      $("#mw-abusefilter-disallow-edit-button").on("click", function () {
        editMessage("disallow");
      });
      $(".mw-abusefilter-action-checkbox input").on(
        "click",
      ...
      cbDeleted.on("change", function () {
        if (cbDeleted.isSelected()) {
          cbEnabled.setSelected(false);
        }
      });

      $("#mw-abusefilter-edit-group-input select").on(
        "change",
        onFilterGroupChange
      );

      $("#mw-abusefilter-export-link").on("click", function (e) {
        e.preventDefault();
        $exportBox.toggle();
      });
    }
