mw.loader.using("ext.visualEditor.targetLoader").then(function () {
  mw.libs.ve.targetLoader.addPlugin(function () {
    ...
    OO.inheritClass(
      ve.init.mw.AbuseFilterSaveErrorHandler,
      ve.init.mw.SaveErrorHandler
    );

    ve.init.mw.AbuseFilterSaveErrorHandler.static.name = "abuseFilter";

    ve.init.mw.AbuseFilterSaveErrorHandler.static.matchFunction = function (
      data
    ) {
      return (
        data.errors &&
        data.errors.some(function (err) {
          return (
            err.code === "abusefilter-disallowed" ||
            err.code === "abusefilter-warning"
          );
        })
      );
    };

    ve.init.mw.AbuseFilterSaveErrorHandler.static.process = function (
      data,
      target
    ) {
