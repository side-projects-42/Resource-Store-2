	...
	$.client = {
		...
		profile: function (nav) {
			...
			var versionNumber,
				...
				versionPrefixes =
					"(?:chrome|crios|firefox|fxios|opera|version|konqueror|msie|safari|android)",
				...
				rName =
					/(chrome|crios|firefox|fxios|konqueror|msie|opera|safari|rekonq|android)/,
			...
			if (
				(match = new RegExp(versionPrefixes + versionSuffix).exec(ua))
			) {
				version = match[1];
			}
			...
			profileCache[key] = {
				...
				version: version,
				versionBase:
					version !== x ? Math.floor(versionNumber).toString() : x,
