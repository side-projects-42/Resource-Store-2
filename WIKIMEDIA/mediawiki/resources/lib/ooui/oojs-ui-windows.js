...
(function (OO) {
			...
			for (i = 0, len = matches.length; i < len; i++) {
				...
				if (
					(filters.visible !== undefined &&
						match.isVisible() !== filters.visible) ||
					(filters.disabled !== undefined &&
						match.isDisabled() !== filters.disabled)
				) {
					matches.splice(i, 1);
					len--;
					i--;
	...
	OO.ui.Error = function OoUiError(message, config) {
		...
		this.recoverable =
			config.recoverable === undefined || !!config.recoverable;
		...
		function proceed(step) {
			return function () {
				...
				if (typeof result === "number") {
					if (result < 0) {
						throw new Error(
							"Cannot go back in time: flux capacitor is out of service"
						);
					}
				...
				if (
					...
					result.length &&
					result[0] instanceof OO.ui.Error
				) {
	...
	OO.ui.Process.prototype.createStep = function (step, context) {
		...
		throw new Error(
			"Cannot create process step: number, promise or function expected"
		);
	};
	...
	OO.ui.WindowInstance.prototype.isOpened = function () {
		return (
			this.deferreds.opened.state() === "resolved" &&
	...
	OO.ui.WindowInstance.prototype.isClosing = function () {
		return (
			this.deferreds.closing.state() === "resolved" &&
	...
	OO.ui.WindowManager.prototype.onWindowResize = function () {
		...
		this.onWindowResizeTimeout = setTimeout(
			this.afterWindowResizeHandler,
			200
		);
	};
	...
	OO.ui.WindowManager.prototype.openWindow = function (
		...
		if (!this.hasWindow(win)) {
			...
			(this.lifecycle && this.lifecycle.isOpening())
		) {
	...
	OO.ui.WindowManager.prototype.removeWindows = function (names) {
		...
		promises = names.map(function (name) {
			...
			return manager
				.closeWindow(name)
				.closed.then(cleanupWindow, cleanupWindow);
		});
	...
	OO.ui.WindowManager.prototype.updateWindowSize = function (win) {
		...
		this.$element.toggleClass(
			"oo-ui-windowManager-fullscreen",
			isFullscreen
		);
		this.$element.toggleClass(
	...
	OO.ui.WindowManager.prototype.togglePreventIosScrolling = function (on) {
		var isIos = /ipad|iphone|ipod/i.test(navigator.userAgent),
			...
			scrollableRoot = OO.ui.Element.static.getRootScrollableElement(
				$body[0]
			),
		...
		if (on) {
			...
			if (this.getCurrentWindow().getSize() === "full") {
				$body
					.add($body.parent())
					.addClass("oo-ui-windowManager-ios-modal-ready");
			}
		} else {
	...
	OO.ui.Window = function OoUiWindow(config) {
		...
		this.$content.addClass("oo-ui-window-content").attr("tabindex", -1);
		this.$frame
	...
	OO.ui.Window.prototype.getSize = function () {
		...
		if (
			size !== "full" &&
			viewport.rect.right - viewport.rect.left < sizes[size].width
		) {
			size = "full";
		}
	...
	OO.ui.Window.prototype.withoutSizeTransitions = function (callback) {
		...
		var oldTransition =
			this.$frame.css("transition-property") +
			" " +
	...
	OO.ui.Window.prototype.getContentHeight = function () {
		...
		return (
			...
			this.$frame.outerHeight() -
			this.$frame.innerHeight() +
			// Use combined heights of children
			(this.$head.outerHeight(true) +
				bodyHeight +
	...
	OO.ui.Window.prototype.setManager = function (manager) {
		if (this.manager) {
			throw new Error(
				"Cannot set window manager, window already has a manager"
			);
		}
	...
	OO.ui.Window.prototype.updateSize = function () {
		if (!this.manager) {
			throw new Error(
				"Cannot update window size, must be attached to a manager"
			);
		}
	...
	OO.ui.Window.prototype.initialize = function () {
		if (!this.manager) {
			throw new Error(
				"Cannot initialize window, must be attached to a manager"
			);
		}
	...
	OO.ui.Window.prototype.open = function (data) {
		if (!this.manager) {
			throw new Error(
				"Cannot open window, must be attached to a manager"
			);
		}
	...
	OO.ui.Window.prototype.close = function (data) {
		if (!this.manager) {
			throw new Error(
				"Cannot close window, must be attached to a manager"
			);
		}
	...
	OO.ui.Window.prototype.setup = function (data) {
		...
		return this.getSetupProcess(data)
			.execute()
			.then(function () {
				win.updateSize();
				// Force redraw by asking the browser to measure the elements' widths
				win.$element
					.addClass("oo-ui-window-active oo-ui-window-setup")
					.width();
	...
	OO.ui.Window.prototype.ready = function (data) {
		...
		this.$content.trigger("focus");
		return this.getReadyProcess(data)
			.execute()
			.then(function () {
				// Force redraw by asking the browser to measure the elements' widths
	...
	OO.ui.Window.prototype.hold = function (data) {
		...
		return this.getHoldProcess(data)
			.execute()
			.then(function () {
				...
				// Force redraw by asking the browser to measure the elements' widths
				win.$element
					.removeClass("oo-ui-window-ready oo-ui-window-setup")
					.width();
				win.$content
					.removeClass(
						"oo-ui-window-content-ready oo-ui-window-content-setup"
					)
					.width();
			});
	};
	...
	OO.ui.Window.prototype.teardown = function (data) {
		...
		return this.getTeardownProcess(data)
			.execute()
			.then(function () {
				// Force redraw by asking the browser to measure the elements' widths
	...
	OO.ui.Dialog.prototype.getSetupProcess = function (data) {
		...
		// Parent method
		return OO.ui.Dialog.super.prototype.getSetupProcess
			.call(this, data)
			.next(function () {
				var config = this.constructor.static,
					actions =
						data.actions !== undefined
							...
							: config.actions,
					title =
						data.title !== undefined ? data.title : config.title;
	...
	OO.ui.Dialog.prototype.getTeardownProcess = function (data) {
		// Parent method
		return OO.ui.Dialog.super.prototype.getTeardownProcess
			.call(this, data)
			.first(function () {
	...
	OO.ui.Dialog.prototype.executeAction = function (action) {
		...
		return this.getActionProcess(action)
			.execute()
	 ...
	 */
	OO.ui.MessageDialog.prototype.toggleVerticalActionLayout = function (
		value
	) {
	...
	OO.ui.MessageDialog.prototype.getActionProcess = function (action) {
		...
		return OO.ui.MessageDialog.super.prototype.getActionProcess.call(
			this,
			action
		);
	};
	...
	OO.ui.MessageDialog.prototype.getSetupProcess = function (data) {
		...
		// Parent method
		return OO.ui.MessageDialog.super.prototype.getSetupProcess
			.call(this, data)
			.next(function () {
				this.title.setLabel(
	...
	OO.ui.MessageDialog.prototype.getReadyProcess = function (data) {
		...
		// Parent method
		return OO.ui.MessageDialog.super.prototype.getReadyProcess
			.call(this, data)
			.next(function () {
	...
	OO.ui.MessageDialog.prototype.initialize = function () {
		...
		this.container = new OO.ui.PanelLayout({
			scrollable: true,
			classes: ["oo-ui-messageDialog-container"],
		});
		this.text = new OO.ui.PanelLayout({
			padded: true,
			expanded: false,
			classes: ["oo-ui-messageDialog-text"],
		});
	...
	OO.ui.MessageDialog.prototype.fitActions = function () {
		...
		for (i = 0, len = actions.length; i < len; i++) {
			action = actions[i];
			if (
				action.$element[0].scrollWidth > action.$element[0].clientWidth
			) {
	...
	OO.ui.ProcessDialog.prototype.initialize = function () {
		...
		this.$errors
			...
			.append(
				this.$errorsTitle,
				$("<div>")
					.addClass("oo-ui-processDialog-errors-actions")
					.append(
	...
	OO.ui.ProcessDialog.prototype.executeAction = function (action) {
		var dialog = this;
		return OO.ui.ProcessDialog.super.prototype.executeAction
			.call(this, action)
	...
	OO.ui.ProcessDialog.prototype.setDimensions = function () {
		...
		// Parent method
		OO.ui.ProcessDialog.super.prototype.setDimensions.apply(
			this,
			arguments
		);
	...
	OO.ui.ProcessDialog.prototype.fitLabel = function () {
		...
		if (
			!OO.ui.isMobile() &&
			2 * biggerWidth + labelWidth < navigationWidth
		) {
	...
	OO.ui.ProcessDialog.prototype.showErrors = function (errors) {
		...
		if (warning) {
			this.retryButton.setLabel(
				OO.ui.msg("ooui-dialog-process-continue")
			);
		} else {
	...
	OO.ui.ProcessDialog.prototype.getTeardownProcess = function (data) {
		// Parent method
		return OO.ui.ProcessDialog.super.prototype.getTeardownProcess
			.call(this, data)
			.first(function () {
