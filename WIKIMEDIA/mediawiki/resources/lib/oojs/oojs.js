...
(function (global) {
	...
	OO.inheritClass = function (targetFn, originFn) {
		...
		if (!originFn) {
			throw new Error(
				"inheritClass: Origin is not a function (actually " +
					originFn +
					")"
			);
		}
		if (targetFn.prototype instanceof originFn) {
			throw new Error(
				"inheritClass: Target already inherits from origin"
			);
		}
	...
	OO.mixinClass = function (targetFn, originFn) {
		...
		if (!originFn) {
			throw new Error(
				"mixinClass: Origin is not a function (actually " +
					originFn +
					")"
			);
		}
	...
	(function () {
		...
		function validateMethod(method, context) {
			...
			if (typeof method === "string") {
				...
				if (context === undefined || context === null) {
					throw new Error(
						'Method name "' + method + '" has no context.'
					);
				}
				...
				throw new Error(
					"Invalid callback. Function or method name expected."
				);
			}
		}
		...
		OO.EventEmitter.prototype.off = function (event, method, context) {
			...
			if (
				!hasOwn.call(this.bindings, event) ||
	...
	(function () {
		...
		OO.EmitterList.prototype.aggregate = function (events) {
			...
			for (itemEvent in events) {
				...
				if (
					...
					if (groupEvent) {
						throw new Error(
							"Duplicate item event aggregation for " + itemEvent
						);
					}
		...
		OO.EmitterList.prototype.moveItem = function (item, newIndex) {
			...
			if (existingIndex === -1) {
				throw new Error(
					"Item cannot be moved, because it is not in the list."
				);
			}
		...
		OO.EmitterList.prototype.insertItem = function (item, index) {
			...
			if (item.connect && item.disconnect) {
				...
				for (event in this.aggregateItemEvents) {
					events[event] = [
						"emit",
						this.aggregateItemEvents[event],
						item,
					];
				}
	 ...
	 */
	OO.SortedEmitterList.prototype.setSortingCallback = function (
		sortingCallback
	) {
	...
	OO.Factory.prototype.register = function (constructor, name) {
		if (typeof constructor !== "function") {
			throw new Error(
				"constructor must be a function, got " + typeof constructor
			);
		}
		if (arguments.length <= 1) {
