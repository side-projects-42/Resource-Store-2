...
(function () {
	...
	function _init() {
		...
		/* Check if browser supports userData behavior */
			...
			if (_storage_elm.addBehavior) {
				...
				try {
					_observer_update =
						_storage_elm.getAttribute("jStorage_update");
				} catch (E6) {}
	...
	function _handlePubSub() {
		...
		for (
			i = len = _storage.__jstorage_meta.PubSub.length - 1;
			i >= 0;
			i--
		) {
	...
	function _fireSubscribers(channel, payload) {
		if (_pubsub_observers[channel]) {
			for (
				var i = 0, len = _pubsub_observers[channel].length;
				i < len;
				i++
			) {
