...
(function () {
	...
	mw.widgets.SizeFilterWidget = function MwWidgetsSizeFilterWidget(config) {
		...
		// Properties
		this.radioselectinput = new OO.ui.RadioSelectInputWidget(
			config.radioselectinput
		);
	 ...
	 * @inheritdoc
	 */
	mw.widgets.SizeFilterWidget.static.reusePreInfuseDOM = function (
		node,
		config
	) {
		config = mw.widgets.SizeFilterWidget.parent.static.reusePreInfuseDOM(
			node,
			config
		);
		config.radioselectinput =
			OO.ui.RadioSelectInputWidget.static.reusePreInfuseDOM(
	 ...
	 * @inheritdoc
	 */
	mw.widgets.SizeFilterWidget.static.gatherPreInfuseState = function (
		node,
		config
	) {
		var state =
			mw.widgets.SizeFilterWidget.parent.static.gatherPreInfuseState(
				node,
				config
			);
		state.radioselectinput =
			OO.ui.RadioSelectInputWidget.static.gatherPreInfuseState(
				$(node).find(".oo-ui-radioSelectInputWidget"),
				config.radioselectinput
			);
	 ...
	 * @inheritdoc
	 */
	mw.widgets.SizeFilterWidget.prototype.restorePreInfuseState = function (
		state
	) {
		mw.widgets.SizeFilterWidget.parent.prototype.restorePreInfuseState.call(
			this,
			state
		);
