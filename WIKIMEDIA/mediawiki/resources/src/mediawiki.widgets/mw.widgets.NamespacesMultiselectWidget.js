...
(function () {
	...
	mw.widgets.NamespacesMultiselectWidget =
		function MwWidgetsNamespacesMultiselectWidget(config) {
			var i,
				...
				namespaces = {},
				options =
					mw.widgets.NamespaceInputWidget.static.getNamespaceDropdownOptions(
						{}
					);
			...
			// Parent constructor
			mw.widgets.NamespacesMultiselectWidget.parent.call(
				this,
				$.extend(
					true,
					{
					...
					config,
					{
						selected:
							config && config.selected
								? config.selected.map(function (id) {
										return {
											data: id,
											label: namespaces[id],
										};
			...
			// Initialization
			this.$element.addClass("mw-widgets-namespacesMultiselectWidget");
	 ...
	 * @inheritdoc
	 */
	mw.widgets.NamespacesMultiselectWidget.prototype.createMenuOptionWidget =
		function (data, label, icon) {
