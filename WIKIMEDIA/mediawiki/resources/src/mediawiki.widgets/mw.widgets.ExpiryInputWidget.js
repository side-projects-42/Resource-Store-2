...
(function () {
	...
	mw.widgets.ExpiryWidget = function (config) {
		...
		this.relativeField = new config.RelativeInputClass(
		...
		this.dateTimeField.on(
			...
			function (value) {
				...
				this.relativeField.setValue(
					datetime.utc().locale("en").format(RFC2822)
				);
			}.bind(this)
		);
	...
	mw.widgets.ExpiryWidget.static.reusePreInfuseDOM = function (node, config) {
		...
		config = mw.widgets.ExpiryWidget.parent.static.reusePreInfuseDOM(
			node,
			config
		);
		...
		if ($relativeElement.hasClass("oo-ui-textInputWidget")) {
			...
			$relativeElement.hasClass("mw-widget-selectWithInputWidget")
		) {
			config.RelativeInputClass = mw.widgets.SelectWithInputWidget;
		}

		config.relativeInput =
			config.RelativeInputClass.static.reusePreInfuseDOM(
				$relativeElement,
				config.relativeInput
			);
	 ...
	 * @inheritdoc
	 */
	mw.widgets.ExpiryWidget.static.gatherPreInfuseState = function (
		node,
		config
	) {
		var state = mw.widgets.ExpiryWidget.parent.static.gatherPreInfuseState(
			node,
			config
		);

		state.relativeInput =
			config.RelativeInputClass.static.gatherPreInfuseState(
	...
	mw.widgets.ExpiryWidget.prototype.restorePreInfuseState = function (state) {
		mw.widgets.ExpiryWidget.parent.prototype.restorePreInfuseState.call(
			this,
			state
		);
	...
	mw.widgets.ExpiryWidget.prototype.setDisabled = function (disabled) {
		mw.widgets.ExpiryWidget.parent.prototype.setDisabled.call(
			this,
			disabled
		);
