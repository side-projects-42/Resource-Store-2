...
(function () {
	 ...
	 */
	mw.widgets.CategoryMultiselectWidget = function MWCategoryMultiselectWidget(
		config
	) {
		...
		// Parent constructor
		mw.widgets.CategoryMultiselectWidget.parent.call(
			this,
			$.extend(true, {}, config, {
				menu: {
	 ...
	 * @method
	 */
	mw.widgets.CategoryMultiselectWidget.prototype.updateMenuItems =
		function () {
	...
	mw.widgets.CategoryMultiselectWidget.prototype.clearInput = function () {
		mw.widgets.CategoryMultiselectWidget.parent.prototype.clearInput.call(
			this
		);
	 ...
	 */
	mw.widgets.CategoryMultiselectWidget.prototype.getNewMenuItems = function (
		input
	) {
		var i,
			promises = [],
	 ...
	 * @inheritdoc
	 */
	mw.widgets.CategoryMultiselectWidget.prototype.isAllowedData = function (
		data
	) {
		...
		return mw.widgets.CategoryMultiselectWidget.parent.prototype.isAllowedData.call(
			this,
			data
		);
	};
	 ...
	 * @inheritdoc
	 */
	mw.widgets.CategoryMultiselectWidget.prototype.createTagItemWidget =
		function (data) {
			var title = mw.Title.makeTitle(NS_CATEGORY, data);
	 ...
	 * @inheritdoc
	 */
	mw.widgets.CategoryMultiselectWidget.prototype.findItemFromData = function (
		data
	) {
		...
		return OO.ui.mixin.GroupElement.prototype.findItemFromData.call(
			this,
			title.getMainText()
		);
	};
	 ...
	 * @return {boolean}
	 */
	mw.widgets.CategoryMultiselectWidget.prototype.validateSearchTypes =
		function () {
			...
			if (
				...
				throw new Error(
					"Can't have additional search types with mw.widgets.CategoryMultiselectWidget.SearchType.SubCategories"
				);
			}
			...
			if (
				...
				throw new Error(
					"Can't have additional search types with mw.widgets.CategoryMultiselectWidget.SearchType.ParentCategories"
				);
			}
	 ...
	 */
	mw.widgets.CategoryMultiselectWidget.prototype.setSearchTypes = function (
		searchTypes
	) {
	 ...
	 * @return {jQuery.Promise} Resolves with an array of categories
	 */
	mw.widgets.CategoryMultiselectWidget.prototype.searchCategories = function (
		input,
		searchType
	) {
		...
		switch (searchType) {
			...
			case mw.widgets.CategoryMultiselectWidget.SearchType.InternalSearch:
				this.api
					...
					.done(function (res) {
						var categories = res.query.allpages.map(function (
							page
						) {
							return page.title;
						});
			...
			case mw.widgets.CategoryMultiselectWidget.SearchType.SubCategories:
				...
				this.api
					...
					.done(function (res) {
						var categories = res.query.categorymembers.map(
							function (category) {
			...
			case mw.widgets.CategoryMultiselectWidget.SearchType
				.ParentCategories:
				if (input.indexOf("|") > -1) {
					deferred.resolve([]);
					break;
