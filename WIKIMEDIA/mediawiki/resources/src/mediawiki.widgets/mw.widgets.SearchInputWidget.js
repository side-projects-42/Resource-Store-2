...
(function () {
		...
		$form.on(
			...
			function () {
				mw.track("mw.widgets.SearchInputWidget", {
					...
					index: this.lastLookupItems.indexOf(this.$input.val()),
				});
			}.bind(this)
		);
	...
	mw.widgets.SearchInputWidget.prototype.getSuggestionsPromise = function () {
		...
		promise = mw.searchSuggest.request(
			api,
			this.getQueryValue(),
			function () {},
			this.limit,
	 ...
	 * @inheritdoc mw.widgets.TitleWidget
	 */
	mw.widgets.SearchInputWidget.prototype.getOptionsFromData = function (
		data
	) {
		var items = [],
			titles = data.data[1],
		...
		$.each(titles, function (i, result) {
			items.push(
				new mw.widgets.TitleOptionWidget(
	...
	mw.widgets.SearchInputWidget.prototype.onLookupMenuChoose = function () {
		mw.widgets.SearchInputWidget.parent.prototype.onLookupMenuChoose.apply(
			this,
			arguments
		);
