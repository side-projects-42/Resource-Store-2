...
function appendWithoutParsing($parent, children) {
	...
	for (i = 0, len = children.length; i < len; i++) {
		...
		if (
			...
			children[i].hasClass("mediaWiki_htmlEmitter")
		) {
...
mw.jqueryMsg.Parser = function (options) {
	...
	this.settings.onlyCurlyBraceTransform =
		this.settings.format === "text" || this.settings.format === "escaped";
	this.astCache = {};
...
mw.jqueryMsg.Parser.prototype = {
	...
	getAst: function (key, replacements) {
		...
		if (!Object.prototype.hasOwnProperty.call(this.astCache, key)) {
			...
			wikiText = mw.internalDoTransformFormatForQqx(
				wikiText,
				replacements
			);
	...
	wikiTextToAst: function (input) {
		...
		function unpipedWikilink() {
			...
			return result === null ? null : [["CONCAT"].concat(result[0])];
		}
		...
		function isAllowedHtml(startTagName, endTagName, attributes) {
			...
			for (i = 0, len = attributes.length; i < len; i += 2) {
				...
				if (
					settings.allowedHtmlCommonAttributes.indexOf(
						attributeName
					) === -1 &&
					(
						settings.allowedHtmlAttributesByElement[startTagName] ||
						[]
				...
				if (
					attributeName === "style" &&
					attributes[i + 1].search(badStyle) !== -1
				) {
					mw.log(
						"HTML tag not parsed due to dangerous style attribute"
					);
					return false;
				}
			}
		...
		function html() {
			...
			if (parsedCloseTagResult === null) {
				...
				return [
					"CONCAT",
					input.slice(startOpenTagPos, endOpenTagPos),
			...
			if (isAllowedHtml(startTagName, endTagName, attributes)) {
				...
				result = [
					...
					parsedHtmlContents,
					input.slice(startCloseTagPos, endCloseTagPos)
				);
			}
...
mw.jqueryMsg.HtmlEmitter.prototype = {
		...
		if (arg instanceof $ && !arg.hasClass("mediaWiki_htmlEmitter")) {
			...
			if (typeof arg === "function") {
				...
				if (
					...
					mw.log(
						"External link in message had illegal target " + target
					);
	...
	plural: function (nodes) {
		...
		count = parseFloat(
			this.language.convertNumber(textify(nodes[0]), true)
		);
		forms = nodes.slice(1);
			...
			if (form instanceof $ && form.hasClass("mediaWiki_htmlEmitter")) {
				...
				if (firstChild && firstChild.nodeType === Node.TEXT_NODE) {
					...
					if (/^\d+=/.test(firstChildText)) {
						explicitPluralFormNumber = parseInt(
							firstChildText.split(/=/)[0],
							10
						);
						// Use the digit part as key and rest of first text node and
						// rest of child nodes as value.
						firstChild.textContent = firstChildText.slice(
							firstChildText.indexOf("=") + 1
						);
				...
				explicitPluralForms[explicitPluralFormNumber] = form.slice(
					form.indexOf("=") + 1
				);
				forms[formIndex] = undefined;
			}
	...
	int: function (nodes) {
		...
		return mw.jqueryMsg.getMessageFunction()(
	...
	if (
		...
		(!/\{\{|[<>[&]/.test(this.map.get(this.key)) &&
			...
			!this.parameters.some(function (param) {
				return (
					param instanceof $ ||
					(param && param.nodeType !== undefined)
				);
			}))
	) {
