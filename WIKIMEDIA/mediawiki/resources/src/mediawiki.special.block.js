	...
	$(function () {
		...
		function updateBlockOptions() {
			var blocktarget = blockTargetWidget.getValue().trim(),
				...
				isSitewide =
					editingIsSelected && editingRestrictionValue === "sitewide";
			...
			if (hideUserWidget) {
				hideUserWidget.setDisabled(
					isNonEmptyIp || !isIndefinite || !isSitewide
				);
			}
			...
			pageRestrictionsWidget.setDisabled(
				!editingIsSelected || isSitewide
			);
			namespaceRestrictionsWidget.setDisabled(
				!editingIsSelected || isSitewide
			);
		...
		if (blockTargetWidget) {
			...
			editingRestrictionWidget = OO.ui.infuse(
				$("#mw-input-wpEditingRestriction")
			);
			pageRestrictionsWidget = OO.ui.infuse(
				$("#mw-input-wpPageRestrictions")
			);
			namespaceRestrictionsWidget = OO.ui.infuse(
				$("#mw-input-wpNamespaceRestrictions")
			);
			if (mw.config.get("wgEnablePartialActionBlocks")) {
				// TODO: Use an ID after T280837 is fixed
				actionRestrictionsWidget = OO.ui.infuse(
					$(
			...
			if (blockAllowsUTEdit) {
				preventTalkPageEditWidget = infuseIfExists(
