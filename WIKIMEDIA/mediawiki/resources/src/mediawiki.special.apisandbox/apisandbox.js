(function () {
	...
	WidgetMethods = {
		...
		tokenWidget: {
			...
			fetchToken: function () {
				this.pushPending();
				return api
					.getToken(this.paramInfo.tokentype)
		...
		dropdownWidget: {
			...
			apiCheckValid: function (shouldSuppressErrors) {
				var ok =
					this.getApiValue() !== undefined || shouldSuppressErrors;
		...
		tagWidget: {
			...
			getApiValueForTemplates: function () {
				return this.isDisabled()
					? this.parseApiValue(this.paramInfo.default)
					: this.getValue();
			},
			getApiValue: function () {
		...
		submoduleWidget: {
			single: function () {
				var v = this.isDisabled()
					? this.paramInfo.default
					: this.getApiValue();
				return v === undefined
					? []
					: [{ value: v, path: this.paramInfo.submodules[v] }];
			},
			multi: function () {
	...
	Validators = {
		generic: function () {
			return (
				!Util.apiBool(this.paramInfo.required) ||
				this.getApiValue() !== ""
			);
		},
	};
	...
	Util = {
		...
		fetchModuleInfo: function (module) {
			...
			if (Object.prototype.hasOwnProperty.call(moduleInfoCache, module)) {
				...
				return deferred.promise({ abort: apiPromise.abort });
			}
		},
		...
		createWidgetForParameter: function (pi, opts) {
			...
			switch (pi.type) {
				...
				default:
					...
					items = pi.type.map(function (v) {
						...
						if (
							pi.deprecatedvalues &&
							pi.deprecatedvalues.indexOf(v) >= 0
						) {
							config.classes.push("apihelp-deprecated-value");
						}
						if (
							pi.internalvalues &&
							pi.internalvalues.indexOf(v) >= 0
						) {
							...
							widget.getMenu().on(
								...
								function (item) {
									this.$element.toggleClass(
										...
										pi.internalvalues.indexOf(item.data) >=
											0
									);
								},
								[],
			...
			if (Util.apiBool(pi.required) || opts.nooptional) {
				...
				if (widget.getApiValueForTemplates) {
					finalWidget.getApiValueForTemplates =
						widget.getApiValueForTemplates.bind(widget);
				}
		...
		formatRequest: function (displayParams, rawParams) {
			...
			mw.hook("apisandbox.formatRequest").fire(
				items,
				displayParams,
				rawParams
			);
		...
		onFormatDropdownChange: function () {
			var i,
				...
				items = menu.getItems(),
				selectedField = menu.findSelectedItem()
					? menu.findSelectedItem().getData()
					: null;
	...
	ApiSandbox = {
		...
		init: function () {
			...
			pages.main = new ApiSandbox.PageLayout({
				key: "main",
				path: "main",
			});
		...
		updateUI: function (params) {
			...
			try {
						...
						for (j = 0; j < subpages.length; j++) {
							if (
								...
								pages[subpages[j].key] =
									new ApiSandbox.PageLayout(subpages[j]);
							}
							if (params !== undefined) {
				...
				if (bookletPages.length > i) {
					removePages = bookletPages.splice(
						i,
		...
		sendRequest: function (params) {
			...
			$.when.apply($, deferreds).done(function () {
						...
						deferred.done(function () {
							...
							if (tokenErrorCount === errorCount) {
								...
								actions.push({
									action: "fix",
									label: mw
										.message("apisandbox-results-fixtoken")
										.text(),
									flags: "primary",
								});
							}
				...
				$progressText = $("<span>").text(
					mw.message("apisandbox-sending-request").text()
				);
				progress = new OO.ui.ProgressBarWidget({
					progress: false,
				});

				$result = $("<div>").append($progressText, progress.$element);
				...
				page.setupOutlineItem = function () {
					this.outlineItem.setLabel(
						mw.message("apisandbox-results").text()
					);
				};
				...
				if (!formatDropdown) {
					...
					formatDropdown
						.getMenu()
						.on("select", Util.onFormatDropdownChange);
				}

				menu = formatDropdown.getMenu();
				selectedLabel = menu.findSelectedItem()
					? menu.findSelectedItem().getLabel()
					: "";
				...
				menu.chooseItem(
					menu.getItemFromLabel(selectedLabel) ||
						menu.findFirstSelectableItem()
				);
				...
				page.$element.empty().append(
					new OO.ui.FieldLayout(formatDropdown, {
						label: Util.parseMsg(
				...
				api.post(params, {
					...
					xhr: function () {
						...
						xhr.addEventListener("progress", function (e) {
							if (!progressLoading) {
								...
								$progressText.text(
									mw
										.message("apisandbox-loading-results")
										.text()
								);
							}
							if (e.lengthComputable) {
					...
					.then(
						function (data, jqXHR) {
							...
							if (
								/^text\/mediawiki-api-prettyprint-wrapped(?:;|$)/.test(
									ct
								)
							) {
							...
							if (
								...
								button
									.on("click", ApiSandbox.fixTokenAndResend)
									.on(
										"click",
										button.setDisabled,
										[true],
	...
	ApiSandbox.PageLayout.prototype.setupOutlineItem = function () {
		...
		this.outlineItem.setIcon(
			this.apiIsValid || suppressErrors ? null : "alert"
		);
		this.outlineItem.setTitle(
			this.apiIsValid || suppressErrors
				? ""
				: mw.message("apisandbox-alert-page").plain()
		);
	};
	 ...
	 */
	ApiSandbox.PageLayout.prototype.makeWidgetFieldLayouts = function (
		ppi,
		name
	) {
		var j,
		...
		$tmp.filter("dl")
			...
			.children(".mw-collapsible-toggle")
			.each(function () {
				var $this = $(this);
			...
			case "limit":
				helpLabel.addInfo(
					Util.parseMsg(
						...
						1,
						ppi.min,
						ppi.highmax !== undefined ? ppi.highmax : ppi.max
					),
					" ",
		...
		if (Util.apiBool(ppi.multi)) {
			...
			if (
				flag &&
				!(widget instanceof OO.ui.TagMultiselectWidget) &&
				!(
		...
		helpField = new OO.ui.FieldLayout(helpLabel, {
			align: "top",
		...
		if (ppi.tokentype) {
			...
			widgetField = new OO.ui.ActionFieldLayout(
				widget,
				button,
				layoutConfig
			);
	 ...
	 */
	ApiSandbox.PageLayout.prototype.updateTemplatedParameters = function (
		params
	) {
		var p,
			toProcess,
		...
		doProcess = function (placeholder, target) {
			...
			if (done) {
				container = Util.apiBool(p.info.deprecated)
					? that.deprecatedItemsFieldset
					: that.itemsFieldset;
				items = container.getItems();
				index = undefined;
				for (i = 0; i < items.length; i++) {
					if (
						items[i].apiParamIndex !== undefined &&
						items[i].apiParamIndex > p.info.index
					) {
						index = i;
						break;
					}
	...
	ApiSandbox.PageLayout.prototype.loadParamInfo = function () {
		var dynamicFieldset,
			...
			addDynamicParamWidget = function () {
				...
				layout = new OO.ui.ActionFieldLayout(widget, button, {
					label: name,
					align: "left",
				});
		...
		Util.fetchModuleInfo(this.apiModule)
			.done(function (pi) {
				...
				items.push(
					new OO.ui.FieldLayout(new OO.ui.Widget({}).toggle(false), {
						align: "top",
					...
					buttons.push(
						new OO.ui.PopupButtonWidget({
							...
							popup: {
								...
								classes: ["mw-apisandbox-popup-help"],
								$content: $("<ul>").append(
									pi.examples.map(function (example) {
										var $a = $("<a>")
				...
				if (pi.parameters.length) {
					...
					for (i = 0; i < pi.parameters.length; i++) {
						tmp = that.makeWidgetFieldLayouts(
							pi.parameters[i],
							prefix + pi.parameters[i].name
						);
						that.widgets[prefix + pi.parameters[i].name] =
							tmp.widget;
				...
				if (
					...
					!Util.apiBool(pi.dynamicparameters)
				) {
					items.push(
						new OO.ui.FieldLayout(
				...
				if (Util.apiBool(pi.dynamicparameters)) {
					...
					$("<fieldset>")
						.append(
							$("<legend>").text(
								mw
									.message("apisandbox-dynamic-parameters")
									.text()
							),
				...
				that.deprecatedItemsFieldset = new OO.ui.FieldsetLayout()
					.addItems(deprecatedItems)
					.toggle(false);
				$tmp = $("<fieldset>")
					...
					.append(
						$("<legend>").append(
							new OO.ui.ToggleButtonWidget({
								label: mw
									.message("apisandbox-deprecated-parameters")
									.text(),
							}).on(
								"change",
								that.deprecatedItemsFieldset.toggle,
								[],
								that.deprecatedItemsFieldset
							).$element
						),
					...
					.appendTo(that.$element);
				that.deprecatedItemsFieldset.on(
					"add",
					function () {
			...
			$.when.apply($, promises).then(function () {
				...
				if (that.getOutlineItem()) {
					...
					that.getOutlineItem().setTitle(
						that.apiIsValid || suppressErrors
							? ""
							: mw.message("apisandbox-alert-page").plain()
					);
				}
	...
	ApiSandbox.PageLayout.prototype.loadQueryParams = function (params) {
		if (this.paramInfo === null) {
			...
			$.each(this.widgets, function (name, widget) {
				var v = Object.prototype.hasOwnProperty.call(params, name)
					? params[name]
					: undefined;
	 ...
	 */
	ApiSandbox.PageLayout.prototype.getQueryParams = function (
		params,
		displayParams
	) {
		// eslint-disable-next-line no-jquery/no-each-util
