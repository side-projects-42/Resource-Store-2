...
(function () {
	...
	mw.ForeignStructuredUpload.BookletLayout.prototype.initialize =
		function () {
			var booklet = this;
			return mw.ForeignStructuredUpload.BookletLayout.parent.prototype.initialize
				.call(this)
				.then(function () {
					return $.when(
						...
						booklet.upload.getApi().then(function (api) {
							...
							if (api.apiUrl) {
								...
								booklet.categoriesWidget.api =
									new mw.ForeignApi(api.apiUrl);
							}
						...
						booklet.upload.loadConfig().then(
							function (config) {
								var msgPromise,
									...
									msgs =
										config.licensemessages[
											isLocal ? "local" : "foreign"
										];

								// Hide disabled fields
								booklet.descriptionField.toggle(
									!!fields.description
								);
								booklet.categoriesField.toggle(
									!!fields.categories
								);
								...
								if (
									mw
										.message(
											"upload-form-label-own-work-message-" +
												msgs
										)
										.exists()
								) {
									...
									msgPromise = booklet.upload.apiPromise.then(
										function (api) {
											return api.loadMessages([
												// These messages are documented where msgPromise resolves
												"upload-form-label-own-work-message-" +
													msgs,
												"upload-form-label-not-own-work-message-" +
													msgs,
												"upload-form-label-not-own-work-local-" +
													msgs,
											]);
										}
									);
	 ...
	 * @return {mw.Upload}
	 */
	mw.ForeignStructuredUpload.BookletLayout.prototype.createUpload =
		function () {
			...
			this.selectFileWidget.on("change", function () {
				...
				if (layout.getDateFromLastModified(file) !== undefined) {
					layout.dateWidget.setValue(
						layout.getDateFromLastModified(file)
					);
				}
	...
	mw.ForeignStructuredUpload.BookletLayout.prototype.renderInfoForm =
		function () {
			...
			this.descriptionField = new OO.ui.FieldLayout(
			...
			this.categoriesField = new OO.ui.FieldLayout(
	 ...
	 * @inheritdoc
	 */
	mw.ForeignStructuredUpload.BookletLayout.prototype.onInfoFormChange =
		function () {
			var layout = this,
				validityPromises = [];
	 ...
	 */
	mw.ForeignStructuredUpload.BookletLayout.prototype.validateFilename =
		function (filename) {
			return new mw.Api()
				.get({
					action: "query",
	...
	mw.ForeignStructuredUpload.BookletLayout.prototype.saveFile = function () {
		...
		return this.uploadPromise
			...
			.then(
				mw.ForeignStructuredUpload.BookletLayout.parent.prototype.saveFile.bind(
					this
				)
			);
	};
	 ...
	 */
	mw.ForeignStructuredUpload.BookletLayout.prototype.getDateFromExif =
		function (file) {
			var fileReader,
				deferred = $.Deferred();
	 ...
	 */
	mw.ForeignStructuredUpload.BookletLayout.prototype.getDateFromLastModified =
		function (file) {
	...
	mw.ForeignStructuredUpload.BookletLayout.prototype.clear = function () {
		mw.ForeignStructuredUpload.BookletLayout.parent.prototype.clear.call(
			this
		);
