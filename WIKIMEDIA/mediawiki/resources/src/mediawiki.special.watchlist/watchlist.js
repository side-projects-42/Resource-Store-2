...
(function () {
	$(function () {
		...
		$resetForm.on("submit", function (event) {
			...
			if (!$progressBar) {
				$progressBar = new OO.ui.ProgressBarWidget({ progress: false })
					.$element;
				$progressBar.css({
					position: "absolute",
					width: "100%",
				...
				function (event) {
					...
					function forEachMatchingTitle(title, callback) {
						var titleObj = mw.Title.newFromText(title),
							associatedTitleObj = titleObj.isTalkPage()
								? titleObj.getSubjectPage()
								: titleObj.getTalkPage(),
							associatedTitle =
								associatedTitleObj.getPrefixedText();
						$(".mw-changeslist-line").each(function () {
							...
							$line.find("[data-target-page]").each(function () {
								var $this = $(this),
									rowTitle = String($this.data("targetPage"));
								if (
									rowTitle === title ||
									rowTitle === associatedTitle
								) {
					...
					if ($unwatchLink.hasClass("mw-unwatch-link")) {
						api.unwatch(pageTitle).done(function () {
							forEachMatchingTitle(
								pageTitle,
								function (rowPageTitle, $row, $rowUnwatchLink) {
									$rowUnwatchLink
										...
										.attr(
											"title",
											mw.msg("tooltip-ca-watch")
										)
										.attr(
											"href",
											mw.util.getUrl(rowPageTitle, {
												action: "watch",
											})
										)
						...
						api.watch(pageTitle).then(function () {
							forEachMatchingTitle(
								pageTitle,
								function (rowPageTitle, $row, $rowUnwatchLink) {
									$rowUnwatchLink
