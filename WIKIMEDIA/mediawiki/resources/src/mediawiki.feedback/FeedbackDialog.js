...
FeedbackDialog.prototype.initialize = function () {
	var feedbackSubjectFieldLayout,
...
FeedbackDialog.prototype.validateFeedbackForm = function () {
	var isValid =
		(!this.useragentMandatory || this.useragentCheckbox.isSelected()) &&
...
FeedbackDialog.prototype.getSetupProcess = function (data) {
	return FeedbackDialog.parent.prototype.getSetupProcess
		.call(this, data)
		.next(function () {
			...
			if (data.foreignApi) {
				return data.foreignApi
					...
					.then(function (response) {
						dialog.feedbackPageUrl = OO.getProp(
							response,
							"query",
							"pages",
							0,
...
FeedbackDialog.prototype.getReadyProcess = function (data) {
	return FeedbackDialog.parent.prototype.getReadyProcess
		.call(this, data)
		.next(function () {
...
FeedbackDialog.prototype.getActionProcess = function (action) {
	if (action === "cancel") {
		...
		return new OO.ui.Process(function () {
			...
			// Post the message
			return this.messagePosterPromise
				.then(
					function (poster) {
					...
					function () {
						fb.status = "error4";
						mw.log.warn(
							"Feedback report failed because MessagePoster could not be fetched"
						);
					}
				)
				.then(
...
FeedbackDialog.prototype.postMessage = function (poster, subject, message) {
	...
	return poster.post(subject, message).then(
		function () {
			fb.status = "submitted";
		},
		function (mainCode, secondaryCode, details) {
			if (mainCode === "api-fail") {
				if (secondaryCode === "http") {
					...
					// ajax request failed
					mw.log.warn(
						"Feedback report failed with HTTP error: " +
							details.textStatus
					);
				} else {
					fb.status = "error2";
					mw.log.warn(
						"Feedback report failed with API error: " +
							secondaryCode
					);
				}
...
FeedbackDialog.prototype.getTeardownProcess = function (data) {
	return FeedbackDialog.parent.prototype.getTeardownProcess
		.call(this, data)
		.first(function () {
			this.emit(
				"submit",
