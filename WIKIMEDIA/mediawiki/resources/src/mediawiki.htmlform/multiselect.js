...
(function () {
	function addMulti($oldContainer, $container) {
		var name = $oldContainer.find("input:first-child").attr("name"),
			oldClass = (" " + $oldContainer.attr("class") + " ").replace(
				/(mw-htmlform-field-HTMLMultiSelectField|mw-htmlform-dropdown)/g,
				""
			),
			$select = $("<select>"),
				...
				if ($el.is("[data-ooui]")) {
					...
					mw.loader.using(modules, function () {
						convertCheckboxesWidgetToTags(
							OO.ui.FieldLayout.static.infuse($el)
						);
					});
				} else {
