(function () {
	 ...
	 */
	mw.widgets.datetime.ProlepticGregorianDateTimeFormatter =
		function MwWidgetsDatetimeProlepticGregorianDateTimeFormatter(config) {
			...
			// Parent constructor
			mw.widgets.datetime.ProlepticGregorianDateTimeFormatter.super.call(
				this,
				config
			);
	...
	mw.widgets.datetime.ProlepticGregorianDateTimeFormatter.static.formats = {
		...
		"@datetime":
			"$!{dow|short} ${day|#} ${month|short} ${year|#} ${hour|0}:${minute|0}:${second|0} $!{zone|short}",
		"@default":
			"$!{dow|short} ${day|#} ${month|short} ${year|#} ${hour|0}:${minute|0}:${second|0} $!{zone|short}",
	};
	 ...
	 * @property {Object}
	 */
	mw.widgets.datetime.ProlepticGregorianDateTimeFormatter.static.fullMonthNames =
		null;
	 ...
	 * @property {Object}
	 */
	mw.widgets.datetime.ProlepticGregorianDateTimeFormatter.static.shortMonthNames =
		null;
	 ...
	 * @property {Object}
	 */
	mw.widgets.datetime.ProlepticGregorianDateTimeFormatter.static.fullDayNames =
		null;
	 ...
	 * @property {Object}
	 */
	mw.widgets.datetime.ProlepticGregorianDateTimeFormatter.static.shortDayNames =
		null;
	 ...
	 * @property {string[]}
	 */
	mw.widgets.datetime.ProlepticGregorianDateTimeFormatter.static.dayLetters =
		null;
	 ...
	 * @property {string[]}
	 */
	mw.widgets.datetime.ProlepticGregorianDateTimeFormatter.static.hour12Periods =
		null;

	mw.widgets.datetime.ProlepticGregorianDateTimeFormatter.static.setupDefaults =
		function () {
			mw.widgets.datetime.DateTimeFormatter.static.setupDefaults.call(
				this
			);
			...
			if (!this.hour12Periods) {
				this.hour12Periods = [mw.msg("period-am"), mw.msg("period-pm")];
			}
		};
	 ...
	 */
	mw.widgets.datetime.ProlepticGregorianDateTimeFormatter.prototype.getFieldForTag =
		function (tag, params) {
			var spec = null;
			...
			switch (tag + "|" + params[0]) {
				...
				default:
					return mw.widgets.datetime.ProlepticGregorianDateTimeFormatter.super.prototype.getFieldForTag.call(
						this,
						tag,
						params
					);
	 ...
	 */
	mw.widgets.datetime.ProlepticGregorianDateTimeFormatter.prototype.getComponentsFromDate =
		function (date) {
			var ret;
	 ...
	 * @inheritdoc
	 */
	mw.widgets.datetime.ProlepticGregorianDateTimeFormatter.prototype.getDateFromComponents =
		function (components) {
			var date = new Date();
			...
			if (
				...
				components.hour12 !== undefined &&
				components.hour12period !== undefined
			) {
				components.hour =
					(components.hour12 % 12) +
					(components.hour12period ? 12 : 0);
			}
			components = $.extend(
				{},
				this.getComponentsFromDate(null),
				components
			);
	 ...
	 * @inheritdoc
	 */
	mw.widgets.datetime.ProlepticGregorianDateTimeFormatter.prototype.adjustComponent =
		function (date, component, delta, mode) {
			...
			if (component === "month" || component === "year") {
				components.day = Math.min(
					components.day,
					this.getDaysInMonth(components.month, components.year)
				);
			}
	 ...
	 * @return {number}
	 */
	mw.widgets.datetime.ProlepticGregorianDateTimeFormatter.prototype.getDaysInMonth =
		function (month, year) {
			switch (month) {
				case 4:
				case 6:
	 ...
	 * @inheritdoc
	 */
	mw.widgets.datetime.ProlepticGregorianDateTimeFormatter.prototype.getCalendarHeadings =
		function () {
			...
			if (this.weekStartsOn) {
				return a
					...
					.concat(a.slice(0, this.weekStartsOn));
			} else {
	 ...
	 * @inheritdoc
	 */
	mw.widgets.datetime.ProlepticGregorianDateTimeFormatter.prototype.sameCalendarGrid =
	 ...
	 * @inheritdoc
	 */
	mw.widgets.datetime.ProlepticGregorianDateTimeFormatter.prototype.getCalendarData =
		function (date) {
			var dt,
				t,
				d,
			...
			if (this.local) {
				ret.header =
					this.fullMonthNames[dt.getMonth() + 1] +
					" " +
					dt.getFullYear();
				d = dt.getDay() % 7;
				...
				ret.header =
					this.fullMonthNames[dt.getUTCMonth() + 1] +
					" " +
					dt.getUTCFullYear();
				d = dt.getUTCDay() % 7;
