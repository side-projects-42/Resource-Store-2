(function () {
	...
	mw.widgets.datetime.DateTimeInputWidget =
		function MwWidgetsDatetimeDateTimeInputWidget(config) {
			...
			if (
				$.isPlainObject(config.formatter) &&
				config.formatter.format === undefined
			) {
			...
			if (
				...
				config.min.getTime() >= -62167219200000
			) {
				this.min = config.min;
			} else {
			...
			if (
				...
				config.max.getTime() <= 253402300799999
			) {
				this.max = config.max;
			} else {
			...
			if (
				config.formatter instanceof
				mw.widgets.datetime.DateTimeFormatter
			) {
				this.formatter = config.formatter;
			} else if ($.isPlainObject(config.formatter)) {
				this.formatter =
					new mw.widgets.datetime.ProlepticGregorianDateTimeFormatter(
						config.formatter
					);
			} else {
				throw new Error(
					'"formatter" must be an mw.widgets.datetime.DateTimeFormatter or a plain object'
				);
			}
			...
			if (this.type === "time" || config.calendar === null) {
				...
				this.calendar = new mw.widgets.datetime.CalendarWidget(
					config.calendar
				);
			}
	 ...
	 * @return {Date|null}
	 */
	mw.widgets.datetime.DateTimeInputWidget.prototype.parseDateValue =
		function (value) {
			var date, m;
	 ...
	 * @inheritdoc
	 */
	mw.widgets.datetime.DateTimeInputWidget.prototype.cleanUpValue = function (
		value
	) {
		var date, pad;
	 ...
	 * @return {Date|null}
	 */
	mw.widgets.datetime.DateTimeInputWidget.prototype.getValueAsDate =
		function () {
	 ...
	 * @private
	 */
	mw.widgets.datetime.DateTimeInputWidget.prototype.setupFields =
		function () {
			var i,
				$field,
				spec,
				...
				spanValFunc = function (v) {
					if (v === undefined) {
						return this.data(
							"mw-widgets-datetime-dateTimeInputWidget-value"
						);
					} else {
						v = String(v);
						this.data(
						...
						if (v === "") {
							v = this.data(
								"mw-widgets-datetime-dateTimeInputWidget-placeholder"
							);
						}
						this.text(v);
						return this;
			...
			for (i = 0; i < specs.length; i++) {
				...
				if (spec.editable && spec.type !== "static") {
					...
					if (spec.type !== "static") {
						...
						$field.on(
							"focus",
							this.onFieldFocus.bind(this, $field)
						);
					}
					if (spec.type === "static") {
	 ...
	 * @private
	 */
	mw.widgets.datetime.DateTimeInputWidget.prototype.updateFieldsFromValue =
		function () {
			var i,
				$field,
				spec,
			...
			if (date === null) {
				...
				for (i = 0; i < this.fields.length; i++) {
					...
					$field
						.removeClass(
							"mw-widgets-datetime-dateTimeInputWidget-invalid oo-ui-element-hidden"
						)
						.val("");
				...
				for (i = 0; i < this.fields.length; i++) {
					...
					if (spec.type !== "static") {
						$field.val(
							spec.formatValue(this.components[spec.component])
						);
					}
					if (spec.intercalarySize) {
						if (
			...
			this.$element.toggleClass(
				"mw-widgets-datetime-dateTimeInputWidget-empty",
				date === null
			);
		};
	 ...
	 * @private
	 */
	mw.widgets.datetime.DateTimeInputWidget.prototype.updateValueFromFields =
		function () {
			var i,
				v,
				$field,
			...
			if (!anyInvalid) {
				...
				if (
					!curDate ||
					!newDate ||
					curDate.getTime() !== newDate.getTime()
				) {
	 ...
	 * @private
	 */
	mw.widgets.datetime.DateTimeInputWidget.prototype.onClearClick =
		function () {
			this.blur();
			this.setValue("");
		};
	 ...
	 */
	mw.widgets.datetime.DateTimeInputWidget.prototype.onHandleClick =
		function () {
			this.focus();
		};
	 ...
	 */
	mw.widgets.datetime.DateTimeInputWidget.prototype.onFieldFocus = function (
		$field
	) {
		var spec = $field.data(
			"mw-widgets-datetime-dateTimeInputWidget-fieldSpec"
	 ...
	 * @param {jQuery.Event} e Click event
	 */
	mw.widgets.datetime.DateTimeInputWidget.prototype.onFieldClick = function (
		$field
	) {
		var spec = $field.data(
			"mw-widgets-datetime-dateTimeInputWidget-fieldSpec"
	 ...
	 */
	mw.widgets.datetime.DateTimeInputWidget.prototype.onFieldBlur = function (
		$field
	) {
		var v,
			date,
		...
		if (!date) {
			...
			v = spec.formatValue(
				this.formatter.getComponentsFromDate(date)[spec.component]
			);
			if (v !== $field.val()) {
				$field.val(v);
			}
	 ...
	 */
	mw.widgets.datetime.DateTimeInputWidget.prototype.onFieldChange =
		function () {
			this.updateValueFromFields();
		};
	 ...
	 * @return {boolean} False to cancel the default event
	 */
	mw.widgets.datetime.DateTimeInputWidget.prototype.onFieldWheel = function (
		$field,
		e
	) {
		var delta = 0,
	 ...
	 * @private
	 */
	mw.widgets.datetime.DateTimeInputWidget.prototype.onCalendarChange =
		function () {
	...
	mw.widgets.datetime.DateTimeInputWidget.prototype.setDisabled = function (
		...
		mw.widgets.datetime.DateTimeInputWidget.super.prototype.setDisabled.call(
			this,
			disabled
		);
	...
	mw.widgets.datetime.DateTimeInputWidget.prototype.focus = function () {
		if (!this.getFocusedField().length) {
			this.$fields
				.find(".mw-widgets-datetime-dateTimeInputWidget-editField")
				.first()
				.trigger("focus");
		}
		return this;
	 ...
	 * @inheritdoc
	 */
	mw.widgets.datetime.DateTimeInputWidget.prototype.simulateLabelClick =
		function () {
			this.focus();
		};
})();
