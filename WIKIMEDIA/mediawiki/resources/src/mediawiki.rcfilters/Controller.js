 ...
 *  parameter
 */
Controller = function MwRcfiltersController(
	filtersModel,
	changesListModel,
	savedQueriesModel,
	config
...
Controller.prototype.initialize = function (
		...
		$.each(namespaceStructure, function (namespaceID, label) {
			...
			var isTalk = mw.Title.isTalkNamespace(namespaceID),
				nsFilter = {
					...
					description: "",
					identifiers: [isTalk ? "talk" : "subject"],
					cssClass: "mw-changeslist-ns-" + namespaceID,
				};
			items.push(nsFilter);
			(isTalk ? nsAllDiscussions : nsAllContents).subset.push({
				filter: namespaceID,
			});
		});
		...
		groups: [
			{
				...
				default: mw.user.options.get(
					this.limitPreferenceName,
					displayConfig.limitDefault
				),
				sticky: true,
			...
			},
			{
				...
				default: mw.user.options.get(
					this.daysPreferenceName,
					displayConfig.daysDefault
				),
				sticky: true,
				filters: [
					// Hours (1, 2, 6, 12)
	...
	this.uriProcessor = new UriProcessor(this.filtersModel, {
 ...
 */
Controller.prototype._createFilterDataFromNumber = function (
	num,
	numForDisplay
) {
	return {
 ...
 */
Controller.prototype.addNumberValuesToGroup = function (
	groupData,
	arbitraryValues
) {
	var controller = this,
...
Controller.prototype.emptyFilters = function () {
	var highlightedFilterNames = this.filtersModel
		...
		.map(function (filterItem) {
			return { name: filterItem.getName() };
		});
...
Controller.prototype.toggleFilterSelect = function (filterName, isSelected) {
	...
	isSelected =
		isSelected === undefined ? !filterItem.isSelected() : isSelected;
...
Controller.prototype.setShowLinkedTo = function (value) {
	var targetItem = this.filtersModel
			.getGroup("page")
			.getItemByParamName("target"),
		showLinkedToItem = this.filtersModel
			.getGroup("toOrFrom")
...
Controller.prototype._checkForNewChanges = function () {
	...
	return this._queryChangesList("liveUpdate", params).then(function (data) {
		return data.status;
	});
};
...
Controller.prototype.applySavedQuery = function (queryID) {
	...
	if (currentMatchingQuery && currentMatchingQuery.getID() === queryID) {
...
Controller.prototype.updateStickyPreferences = function () {
	...
	this.updateDaysDefault(
		this.filtersModel.getGroup("days").findSelectedItems()[0].getParamName()
	);
	this.updateLimitDefault(
		this.filtersModel
			.getGroup("limit")
			.findSelectedItems()[0]
			.getParamName()
...
Controller.prototype.updateCollapsedState = function (isCollapsed) {
	this.updateNumericPreference(
		this.collapsedPreferenceName,
		Number(isCollapsed)
	);
};
...
Controller.prototype.updateStateFromUrl = function (fetchChangesList) {
	fetchChangesList =
		fetchChangesList === undefined ? true : !!fetchChangesList;
...
Controller.prototype._queryChangesList = function (counterId, params) {
	...
	return $.ajax(uri.toString(), { contentType: "html" }).then(
...
Controller.prototype._fetchChangesList = function () {
	return this._queryChangesList("updateChangesList").then(
		function (data) {
			var $parsed;
...
Controller.prototype.trackFilterGroupings = function (action) {
	...
	if (
		...
		filters.forEach(function (filterName) {
			mw.track("event.ChangesListFilterGrouping", {
				action: action,
...
Controller.prototype.resetSearchForView = function (view) {
	...
	this.setSearch(this.filtersModel.getViewTrigger(view));
};
