...
ChangesListWrapperWidget.prototype.onModelUpdate = function (
	...
	var conflictItem,
		...
		loaderPromise =
			mw.user.options.get("usenewrc") && !OO.ui.isMobile()
				? mw.loader.using([
	...
	if (isEmpty) {
		...
		if (this.filtersViewModel.hasConflict()) {
			...
			$message.append(
				$("<div>")
					.addClass(
						"mw-rcfilters-ui-changesListWrapperWidget-results-conflict"
					)
					.text(mw.message("rcfilters-noresults-conflict").text()),
				$("<div>")
					.addClass(
						"mw-rcfilters-ui-changesListWrapperWidget-results-message"
					)
					...
					.text(
						mw
							.message(
								conflictItem.getCurrentConflictResultMessage()
							)
							.text()
					)
			);
...
ChangesListWrapperWidget.prototype.toggleOverlay = function (isVisible) {
	this.$element.toggleClass(
		"mw-rcfilters-ui-changesListWrapperWidget--overlaid",
		isVisible
	);
};
...
ChangesListWrapperWidget.prototype.updateEnhancedParentHighlight = function () {
	var activeHighlightClasses,
		$enhancedTopPageCell = this.$element.find(
			"table.mw-enhanced-rc.mw-collapsible"
		);

	activeHighlightClasses = this.filtersViewModel
		.getCurrentlyUsedHighlightColors()
		.map(function (color) {
	...
	$enhancedTopPageCell.each(function () {
		...
		// to the first row
		$rowsWithDifferentHighlights = $table
			.find("tr:not(:first-child)")
			.filter(function () {
				var classesInThisRow,
					$this = $(this);

				classesInThisRow = activeHighlightClasses.filter(function (
					className
				) {
					// eslint-disable-next-line no-jquery/no-class-state
		...
		$table
			...
			.toggleClass(
				...
				$rowsWithDifferentHighlights.length > 0
			);
	});
};
...
ChangesListWrapperWidget.prototype.inEnhancedMode = function () {
	...
	return (
		...
		(uri.query.enhanced === undefined &&
			Number(mw.user.options.get("usenewrc")))
	);
};
...
ChangesListWrapperWidget.prototype.applyHighlight = function () {
	...
	this.filtersViewModel.getHighlightedItems().forEach(
		function (filterItem) {
...
ChangesListWrapperWidget.prototype.clearHighlight = function () {
	...
	mw.rcfilters.HighlightColors.forEach(
		function (color) {
			// The following classes are used here:
			// * mw-rcfilters-highlight-color-c1
			// * mw-rcfilters-highlight-color-c2
