...
FiltersViewModel.prototype.reassessFilterInteractions = function (item) {
	...
	iterationItems.forEach(function (checkedItem) {
		var allCheckedItems = checkedItem
				.getSubset()
				.concat([checkedItem.getName()]),
	...
	this.getItems().forEach(function (filterItem) {
		...
		$.each(model.groups, function (groupName, groupModel) {
			if (filterItem.getGroupName() === groupName) {
				...
				inConflict =
					groupModel.areAnySelectedInConflictWith(filterItem);
			} else {
				...
				inConflict =
					...
					filterItemGroup
						.findSelectedItems()
						.every(function (otherGroupItem) {
							return groupModel.areAllSelectedInConflictWith(
								otherGroupItem
							);
						});
			}
...
FiltersViewModel.prototype.initializeFilters = function (filterGroups, views) {
		...
		expandConflictDefinitions = function (obj) {
			...
			$.each(obj, function (key, conflicts) {
				...
				conflicts.forEach(function (conflict) {
					...
					if (conflict.filter) {
						filterName = model.groups[
							conflict.group
						].getPrefixedName(conflict.filter);
						filter = model.getItemByName(filterName);

						// Rename
						adjustedConflicts[filterName] = $.extend({}, conflict, {
							filter: filterName,
							item: filter,
						});
					} else {
						...
						model.groups[conflict.group]
							.getItems()
							.forEach(function (groupItem) {
								// Rebuild the conflict
								adjustedConflicts[groupItem.getName()] =
									$.extend({}, conflict, {
	...
	filterGroups.forEach(function (data) {
		...
		for (i = 0; i < data.filters.length; i++) {
			// eslint-disable-next-line mediawiki/msg-doc
			data.filters[i].label = data.filters[i].label
				? mw.msg(data.filters[i].label)
				: data.filters[i].name;
			// eslint-disable-next-line mediawiki/msg-doc
			data.filters[i].description = data.filters[i].description
				? mw.msg(data.filters[i].description)
				: "";
		}
	});
	...
	$.each(allViews, function (viewName, viewData) {
		...
		viewData.groups.forEach(function (groupData) {
			...
			model.groups[group].initializeFilters(
				groupData.filters,
				groupData.default
			);
			...
			groupData.filters.forEach(function (itemData) {
				var filterItem = model.groups[group].getItemByParamName(
					itemData.name
				);
				// Filter conflicts
...
FiltersViewModel.prototype.updateStateFromParams = function (params) {
	...
	$.each(this.getFilterGroups(), function (groupName, groupModel) {
		params[groupName] = groupModel.normalizeArbitraryValue(
			params[groupName]
		);
	});
	...
	Object.keys(filtersValue).forEach(
		function (filterName) {
 ...
 * @return {Object} Empty parameter state
 */
FiltersViewModel.prototype.getMinimizedParamRepresentation = function (
	parameters
) {
	var result = {};
 ...
 */
FiltersViewModel.prototype.getCurrentParameterState = function (
	removeStickyParams
) {
	var state = this.getMinimizedParamRepresentation(
		$.extend(
			true,
			{},
		...
		if (model.isSticky()) {
			if (model.isPerGroupRequestParameter()) {
				...
				result = result.concat(
					model.getItems().map(function (filterItem) {
 ...
 */
FiltersViewModel.prototype.getParametersFromFilters = function (
	filterDefinition
) {
	...
	if (filterDefinition) {
		...
		this.getItems().forEach(function (filterItem) {
			...
			groupItemDefinition[filterItem.getGroupName()][
				filterItem.getName()
			] = filterItem.coerceValue(filterDefinition[filterItem.getName()]);
		});
	}
	...
	$.each(groupItems, function (group, model) {
		$.extend(
			...
			model.getParamRepresentation(
				groupItemDefinition ? groupItemDefinition[group] : null
			)
		);
	});
...
FiltersViewModel.prototype.getFiltersFromParameters = function (params) {
	...
	$.each(params, function (paramName, paramValue) {
		...
		if (itemOrGroup) {
			groupName =
				itemOrGroup instanceof FilterItem
	...
	this.getItems().forEach(function (filterItem) {
		if (filterItem.isHighlightSupported()) {
			result[filterItem.getName() + "_color"] =
				highlightEnabled && filterItem.isHighlighted()
 ...
 */
FiltersViewModel.prototype.sanitizeStringOptionGroup = function (
	groupName,
	valueArray
) {
...
FiltersViewModel.prototype.areVisibleFiltersEmpty = function () {
	...
	return !this.getItems().some(function (filterItem) {
		var visible =
				!filterItem.getGroupModel().isSticky() &&
				!filterItem.getGroupModel().isHidden(),
...
FiltersViewModel.prototype.areNamespacesEffectivelyInverted = function () {
	return (
		this.getInvertModel().isSelected() &&
...
FiltersViewModel.prototype.getGroupFilters = function (groupName) {
	return (
		(this.getGroup(groupName) && this.getGroup(groupName).getItems()) || []
	);
};
...
FiltersViewModel.prototype.getHighlightedItems = function () {
	return this.getItems().filter(function (filterItem) {
		return (
			filterItem.isHighlightSupported() && filterItem.getHighlightColor()
		);
	});
};
