...
(function () {
	...
	Tipsy.prototype = {
		...
		tip: function () {
			if (!this.$tip) {
				this.$tip = $('<div class="tipsy" role="tooltip"></div>').html(
	...
	$.fn.tipsy = function (options) {
		...
		if (options.trigger != "manual") {
			var eventIn =
					options.trigger == "hover" ? "mouseenter focus" : "focus",
				eventOut =
					options.trigger == "hover" ? "mouseleave blur" : "blur";
			if (options.live) {
				mw.track("mw.deprecate", "tipsy-live");
				mw.log.warn(
					'Use of the "live" option of jquery.tipsy is no longer supported.'
				);
			}
