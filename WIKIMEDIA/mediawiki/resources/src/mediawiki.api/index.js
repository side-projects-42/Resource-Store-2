	...
	mw.Api.prototype = {
			...
			xhr = $.ajax(ajaxOptions)
				...
				.done(function (result, textStatus, jqXHR) {
					...
					if (
						...
						// errorformat!=bc
						code =
							result.errors[0].code === undefined
								? "unknown"
		...
		postWithToken: function (tokenType, params, ajaxOptions) {
			var api = this,
				...
				abortedPromise = $.Deferred()
					.reject("http", { textStatus: "abort", exception: "abort" })
					.promise(),
				abortable,
				aborted;

			return api
				.getToken(tokenType, assertParams)
				.then(function (token) {
					params.token = token;
		...
		getErrorMessage: function (data) {
			if (
				...
				return $("<div>").append(
					mw
						.message("api-clientside-error-invalidresponse")
						.parseDom()
				);
			} else if (data.xhr) {
				if (data.textStatus === "timeout") {
					...
					return $("<div>").append(
						mw.message("api-clientside-error-timeout").parseDom()
					);
				} else if (data.textStatus === "abort") {
					// Request cancelled by calling the abort() method on the promise
					return $("<div>").append(
						mw.message("api-clientside-error-aborted").parseDom()
					);
					...
					return $("<div>").append(
						mw.message("api-clientside-error-noconnect").parseDom()
					);
				}
			} else if (data.error) {
				...
				mw.log.warn(
					"mw.Api#getErrorMessage could not handle the response:",
					data
				);
				return $("<div>").append(
					mw
