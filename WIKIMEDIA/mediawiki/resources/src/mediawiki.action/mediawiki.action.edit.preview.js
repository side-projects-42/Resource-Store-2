...
(function () {
	...
	function doLivePreview(e) {
		...
		if (isDiff) {
			...
			$.when(diffRequest, parseRequest).done(function (response) {
				...
				try {
					diffHtml =
						response[0].query.pages[0].revisions[0].diff.body;
			...
			parseRequest.done(function (response) {
				...
				if (response.parse.templates) {
					...
					if (newList.length) {
						...
						$(".mw-templatesUsedExplanation > p").msg(
				...
				if (response.parse.limitreporthtml) {
					$(".limitreport")
						.html(response.parse.limitreporthtml)
						.find(".mw-collapsible")
						.makeCollapsible();
				}
				if (
				...
				if (response.parse.text) {
					...
					$content.detach().html(response.parse.text);
		...
		$.when(parseRequest, diffRequest)
			.done(function (parseResp) {
				var parse = parseResp && parseResp[0].parse,
					isSubject = section === "new",
					$summaryPreview = $editform
						.find(".mw-summary-preview")
						.empty();
			...
			.fail(function (code, result) {
				...
				$errorBox = $("<div>")
					...
					.append(
						$("<strong>").text(
							mw.message("previewerrortext").text()
						)
					)
					.append($errorMsg);
				$wikiDiff.hide();
	...
	$(function () {
		...
		if (
			$(
				"#mw-userjsyoucanpreview, #mw-usercssyoucanpreview, #mw-userjspreview, #mw-usercsspreview"
			).length
		) {
			selector = "#wpDiff";
		} else {
