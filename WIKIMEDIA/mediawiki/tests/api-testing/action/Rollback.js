...
describe("The rollback action", function testEditRollback() {
	...
	it("rolls back consecutive edits", async () => {
		...
		edits.alice1 = await alice.edit(title, {
			text: "One",
			summary: "first",
		});
		edits.bob2 = await bob.edit(title, { text: "Two", summary: "second" });
		edits.alice3 = await alice.edit(title, {
			text: "Three",
			summary: "third",
		});
	...
	it("doesn't roll back edits by another user", async () => {
		...
		const error = await mindy.actionError(
			"rollback",
			{
				title,
				user: bob.username,
	...
	it("should undo the last edit on the page using the pageid parameter", async () => {
		...
		const firstEdit = await alice.edit(title, {
			text: "One",
			summary: "first",
		});
		const secondEdit = await bob.edit(title, {
			text: "Two",
			summary: "second",
		});
