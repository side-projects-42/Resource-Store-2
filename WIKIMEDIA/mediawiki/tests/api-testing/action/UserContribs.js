...
describe("The usercontribs list query", function testUserContribsListQuery() {
	...
	before(async () => {
		...
		edits.fiona1 = await fiona.edit(titleX, {
			text: "Fiona X ONE",
			summary: "fiona x one",
		});
		edits.fiona2 = await fiona.edit(titleX, {
			text: "Fiona X TWO",
			summary: "fiona x two",
			minor: true,
		});
		...
		edits.franky1 = await franky.edit(titleY, {
			text: "Franky Y ONE",
			summary: "franky y one",
		});
		edits.franky2 = await franky.edit(titleY, {
			text: "Franky Y TWO",
			summary: "franky y two",
			minor: true,
		});

		// Fiona edits Y, Franky edits X
		edits.fiona3 = await fiona.edit(titleY, {
			text: "Fiona Y TREE",
			summary: "fiona y three",
		});
		edits.franky3 = await fiona.edit(titleX, {
			text: "Franky X TREE",
			summary: "franky x three",
		});
	});
	...
	it("can filter by namespace", async () => {
		...
		const expected = [edits.franky1, edits.franky2, edits.fiona3];
	...
	it("can list top only", async () => {
		...
		const expected = [edits.fiona3, edits.franky3];
	...
	it("can list creation only", async () => {
		...
		const expected = [edits.fiona1, edits.franky1];
