...
describe("Revision", () => {
	...
	describe("GET /revision/{from}/compare/{to}", () => {
		...
		it("should return 400 if revision id is not an integer", async () => {
			const { status } = await client.get(
				`/revision/${validRevId}/compare/${invalidRevId}`
			);
		...
		it("should successfully get diff between 2 valid revisions", async () => {
			...
			const { newrevid: revId1 } = await mindy.edit(page, {
				text: "Mindy Edit 1",
			});
			const { newrevid: revId2 } = await mindy.edit(page, {
				text: "Mindy Edit 2",
			});
		...
		it("should return 404 for revision that does not exist", async () => {
			const { status } = await client.get(
				`/revision/${validRevId}/compare/${nonExistentRevId}`
			);
			assert.strictEqual(status, 404);
		});

		it("should return 400 if revision ids belong to different pages", async () => {
			const { newrevid: pageOneRev } = await mindy.edit(page, {
				text: "Page 1 edit",
			});
			const { newrevid: pageTwoRev } = await mindy.edit(pageTwo, {
				text: "Page 2 edit",
			});
			const { status } = await client.get(
				`/revision/${pageOneRev}/compare/${pageTwoRev}`
			);
	...
	describe("GET /revision/{id}", () => {
		it("should successfully get source and metadata for revision", async () => {
			...
			const { status, body, headers } = await clientV0.get(
				`/revision/${newrevid}`
			);
	...
	describe("GET /revision/{id}/bare", () => {
		it("should successfully get information about revision", async () => {
			...
			const { status, body, headers } = await client.get(
				`/revision/${newrevid}/bare`
			);
