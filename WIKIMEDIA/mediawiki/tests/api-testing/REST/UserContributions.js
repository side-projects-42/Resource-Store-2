...
describe("GET contributions", () => {
	...
	before(async () => {
		...
		editToDelete = await bethAction.edit(pageToDelete, [
			{ text: "Beth edit 1" },
		]);
		...
		for (let i = 1; i <= 5; i++) {
			...
			arnoldsTags[i] = oddEdit
				? [{ name: tag, description: `<i>${tagDisplay}</i>` }]
				: [];
			const revData = await arnoldAction.edit(page, {
				text: revisionText[i],
				tags,
			});
			await utils.sleep();
	...
	const testGetEdits = async (client, endpoint) => {
		...
		assert.equal(
			contributions[0].page.key,
			utils.dbkey(lastRevision.title)
		);
		...
		assert.isAbove(
			Date.parse(contributions[0].timestamp),
	...
	const testPagingForward = async (client, endpoint) => {
		...
		// assert body.contributions has the correct content
		assert.equal(
			latestSegment.contributions[0].id,
			arnoldsEdits[5].newrevid
		);
		assert.equal(
			latestSegment.contributions[1].id,
			arnoldsEdits[4].newrevid
		);
		...
		const latestSegmentTag = latestSegment.contributions[0].tags.find(
			(tag) => tag.name === "user-contribs-api-test"
		);
		assert.propertyVal(
			latestSegmentTag,
			"description",
			arnoldsTags[5][0].description
		);

		const latestSegmentTag2 = latestSegment.contributions[1].tags.find(
			(tag) => tag.name === "user-contribs-api-test"
		);
		...
		// assert body.contributions has the correct content
		assert.equal(
			olderSegment.contributions[0].id,
			arnoldsEdits[3].newrevid
		);
		assert.equal(
			olderSegment.contributions[1].id,
			arnoldsEdits[2].newrevid
		);

		// ensure first edit has tags and correct property values
		const olderSegmentTag = olderSegment.contributions[0].tags.find(
			(tag) => tag.name === "user-contribs-api-test"
		);
		assert.propertyVal(
			olderSegmentTag,
			"description",
			arnoldsTags[3][0].description
		);

		// ensure second edit does not have tags
		const olderSegmentTag2 = latestSegment.contributions[1].tags.find(
			(tag) => tag.name === "user-contribs-api-test"
		);
		...
		// assert body.contributions has the correct content
		assert.equal(
			finalSegment.contributions[0].id,
			arnoldsEdits[1].newrevid
		);

		const finalSegmentTags = olderSegment.contributions[0].tags.find(
			(tag) => tag.name === "user-contribs-api-test"
		);
		assert.propertyVal(
			finalSegmentTags,
			"description",
			arnoldsTags[1][0].description
		);
	};
	...
	const testPreserveTagFilter = async (client, endpoint) => {
		...
		// get latest segment
		const { body: latestSegment } = await client.get(endpoint, {
			limit: 2,
			tag: "user-contribs-api-test",
		});
	...
	describe("GET /me/contributions", () => {
		...
		it("Returns 400 if segment size is out of bounds", async () => {
			const { status: minLimitStatus } = await arnold.get(endpoint, {
				limit: 0,
			});
			assert.equal(minLimitStatus, 400);

			const { status: maxLimitStatus } = await arnold.get(endpoint, {
				limit: 30,
			});
