...
describe("Page History", () => {
	...
	async function setupDeletedPage() {
		const editOne = await bot.edit(titleToDelete, {
			text: "Delete Me 1",
			summary: "edit 1",
		});
		const editTwo = await mindy.edit(titleToDelete, {
			text: "Counting 1",
			summary: "edit 2",
		});
	...
	before(async () => {
		...
		const { rollback } = await mindy.action(
			"rollback",
			{
				title,
				user: bot.username,
		...
		// Undo last edit
		addEditInfo(await mindy.edit(title, { undo: edits.all[0].id }), [
			edits.reverts,
		]);
	});
	...
	describe("Revision deletion and un-deletion", () => {
		...
		it("Should get total number of edits and editors when edits are hidden and shown", async () => {
			...
			const revHideEdits = await client.get(
				`/page/${titleToDelete}/history/counts/edits`
			);
			...
			assert.deepEqual(
				revHideEdits.body,
				{ count: 1, limit: false },
				"Edit count of 1 after hiding a revision"
			);

			const revHideEditors = await client.get(
				`/page/${titleToDelete}/history/counts/editors`
			);
			assert.equal(revHideEditors.status, 200);
			assert.deepEqual(
				revHideEditors.body,
				{ count: 1, limit: false },
				"Editor count of 1 after hiding a revision"
			);
			...
			const revShowEdits = await client.get(
				`/page/${titleToDelete}/history/counts/edits`
			);
			...
			assert.deepEqual(
				revShowEdits.body,
				{ count: 2, limit: false },
				"Edit count of 2 after un-hiding the hidden revision"
			);

			const revShowEditors = await client.get(
				`/page/${titleToDelete}/history/counts/editors`
			);
			assert.deepEqual(
				revShowEditors.body,
				{ count: 2, limit: false },
				"Editor count of 2 after un-hiding the hidden revision"
			);
	...
	describe("GET /page/{title}/history/counts/edits", () => {
		...
		it("Should return 400 for invalid parameter", async () => {
			const res = await client.get(
				`/page/${title}/history/counts/editts`
			);
		...
		it("Should return 404 for title that does not exist", async () => {
			...
			const res = await client.get(
				`/page/${title2}/history/counts/edits`
			);
		...
		it("Should get total number of edits between revisions, normal order", async () => {
			...
			const res = await client.get(
				`/page/${title}/history/counts/edits?from=${fromRev}&to=${toRev}`
			);
		...
		it("Should get total number of edits between revisions, reverse order", async () => {
			...
			const toRev = edits.all[edits.all.length - 2].id;
			const res = await client.get(
				`/page/${title}/history/counts/edits?from=${toRev}&to=${fromRev}`
			);
		...
		it("Should return 404 for deleted page", async () => {
			await mindy.action(
				"delete",
				{ title: titleToDelete, token: await mindy.token() },
				"POST"
			);
			const { status: editsStatus } = await client.get(
				`/page/${titleToDelete}/history/counts/edits`
			);
	...
	describe("GET /page/{title}/history/counts/anonymous", () => {
		it("Should get total number of anonymous edits", async () => {
			const res = await client.get(
				`/page/${title}/history/counts/anonymous`
			);
	...
	describe("GET /page/{title}/history/counts/reverted", () => {
		it("Should get total number of reverted edits", async () => {
			const res = await client.get(
				`/page/${title}/history/counts/reverted`
			);
	...
	describe("GET /page/{title}/history/counts/editors", () => {
		it("Should return 404 for deleted page", async () => {
			const { status: editorsStatus } = await client.get(
				`/page/${titleToDelete}/history/counts/editors`
			);
		...
		it("Should get total number of unique editors between revisions, normal order", async () => {
			...
			const toRev = edits.all[edits.all.length - 2].id;
			const res = await client.get(
				`/page/${title}/history/counts/editors?from=${fromRev}&to=${toRev}`
			);
		...
		it("Should get total number of unique editors between revisions, reverse order", async () => {
			...
			const toRev = edits.all[edits.all.length - 2].id;
			const res = await client.get(
				`/page/${title}/history/counts/editors?from=${toRev}&to=${fromRev}`
			);
	...
	describe("GET page/{title}/history?filter={tag}", () => {
		it("Should get all revisions", async () => {
			...
			res.body.revisions.forEach((rev, i) =>
				assert.deepNestedInclude(rev, edits.all[i])
			);
		...
		it("Should get revisions by anonymous users", async () => {
			...
			res.body.revisions.forEach((rev, i) =>
				assert.deepNestedInclude(rev, edits.anon[i])
			);
			assert.include(
				res.body.latest,
				`page/${title}/history?filter=anonymous`
			);
			assert.equal(res.status, 200);
		...
		it("Should get revisions by bots", async () => {
			...
			res.body.revisions.forEach((rev, i) =>
				assert.deepNestedInclude(rev, edits.bot[i])
			);
		...
		it("Should get reverted revisions", async () => {
			...
			res.body.revisions.forEach((rev, i) =>
				assert.deepNestedInclude(rev, edits.reverts[i])
			);
			assert.include(
				res.body.latest,
				`page/${title}/history?filter=reverted`
			);
			assert.equal(res.status, 200);
		});
		...
		it("Should return 404 for title that does not exist", async () => {
			const title2 = utils.title("Random_");
			const res = await client.get(`/page/${title2}/history`, {
				filter: "bot",
			});
		...
		it("Should update cache control headers", async () => {
			const title2 = utils.title("Random_");
			const edit1 = await alice.edit(title2, {
				text: "Old Content",
				summary: "make page",
			});
			const res1a = await client.get(`/page/${title2}/history`, {
				filter: "bot",
			});
			const res1b = await client.get(`/page/${title2}/history`, {
				filter: "bot",
			});

			assert.equal(
				res1a.headers["last-modified"],
				res1b.headers["last-modified"]
			);
			assert.equal(res1a.headers.etag, res1b.headers.etag);

			const edit2 = await alice.edit(title2, {
				text: "New Content",
				summary: "poke page",
			});
			const res2 = await client.get(`/page/${title2}/history`, {
				filter: "bot",
			});

			assert.equal(
				Date.parse(res1a.headers["last-modified"]),
				Date.parse(edit1.newtimestamp)
			);

			assert.equal(
				Date.parse(res2.headers["last-modified"]),
	...
	describe("GET /page/{title}/history?{older_than|newer_than={id}}", () => {
		it("Should get revisions newer than specified id for a page", async () => {
			...
			res.body.revisions.forEach((rev, i) =>
				assert.deepNestedInclude(rev, expected[i])
			);
			assert.include(res.body.latest, `page/${title}/history`);
			assert.include(
				res.body.older,
				`page/${title}/history?older_than=${
					expected[expected.length - 1].id
				}`
			);
			assert.equal(res.status, 200);
		});
		...
		it("Should get revisions older than specified id for a page", async () => {
			...
			assert.include(
				res.body.newer,
				`page/${title}/history?newer_than=${expected[0].id}`
			);
			assert.equal(res.status, 200);
		...
		it("Should get revisions using both filter and newer_than|older_than parameters", async () => {
			...
			assert.include(
				res.body.older,
				`page/${title}/history?filter=bot&older_than=${edits.bot[1].id}`
			);
			assert.equal(res.status, 200);
