...
(function () {
	...
	QUnit.test("TableWidgetModel initialization", function (assert) {
		assert.deepEqual(
			widgetA.model.data,
			widgetAexpectedInitialData,
			"Table data is initialized properly"
		);
	});

	QUnit.test("TableWidgetModel#getRowProperties", function (assert) {
		assert.deepEqual(
			widgetA.model.getRowProperties("bar"),
			widgetAexpectedRowProps,
			"Row props are returned successfully"
		);
	});
	...
	QUnit.test("TableWidget#setValue", function (assert) {
		...
		assert.deepEqual(
			widgetA.model.data,
			widgetAexpectedDataAfterValue,
			"Table data is modified successfully"
		);
	});
	...
	QUnit.skip("TableWidget#insertColumn/insertRow", function (assert) {
		...
		assert.deepEqual(
			widgetA.model.data,
			widgetAexpectedDataAfterRowColumnInsertions,
			"Row and column are added successfully"
		);
	});
	...
	QUnit.skip("TableWidget#removeRow by index", function (assert) {
		...
		assert.deepEqual(
			widgetA.model.data,
			widgetAexpectedDataAfterColumnRemoval,
			"Invalid row removal by index does not change table data"
		);
	});
	...
	QUnit.skip("TableWidget#removeRow by key", function (assert) {
		...
		assert.deepEqual(
			widgetA.model.data,
			widgetAexpectedDataAfterColumnRemoval,
			"Invalid row removal by key does not change table data"
		);
	});

	QUnit.test("TableWidget populate text inputs", function (assert) {
		assert.deepEqual(
			widgetB.getItems()[0].getItems()[2].getValue(),
			"13",
			"Initial data is populated in text inputs properly"
		);
	});
})();
