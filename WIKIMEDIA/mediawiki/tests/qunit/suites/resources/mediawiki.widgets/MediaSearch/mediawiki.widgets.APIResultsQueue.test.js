...
(function () {
		...
		queue
			...
			.then(function (data) {
				...
				assert.strictEqual(
					queue.getQueueSize(),
					3,
					"Query 1: Remaining queue size."
				);

				// Check if sources are depleted
				assert.strictEqual(
					providers[0].isDepleted(),
					false,
					"Query 1: Full provider not depleted."
				);
				assert.strictEqual(
					providers[1].isDepleted(),
					true,
					"Query 1: Empty provider is depleted."
				);
				assert.strictEqual(
					providers[2].isDepleted(),
					true,
			...
			.then(function (data1) {
				...
				assert.strictEqual(
					data1.length,
					10,
					"Query 1: Second set of results received."
				);
				assert.strictEqual(
					queue.getQueueSize(),
					5,
				...
				assert.strictEqual(
					providers[0].isDepleted(),
					false,
					"Query 2: Full provider not depleted."
				);
				assert.strictEqual(
					providers[1].isDepleted(),
					false,
					"Query 2: Empty provider not depleted."
				);
				assert.strictEqual(
					providers[2].isDepleted(),
					false,
					"Query 2: Single result provider not depleted."
				);
			...
			.then(function (data2) {
				...
				assert.strictEqual(
					data2.length,
					10,
					"Query 2: Results received."
				);
				assert.strictEqual(
					queue.getQueueSize(),
					3,
					"Query 2: Remaining queue size."
				);
				// Check if sources are depleted
				assert.strictEqual(
					providers[0].isDepleted(),
				...
				assert.strictEqual(
					providers[1].isDepleted(),
					true,
					"Query 2: Empty provider is not depleted."
				);
				assert.strictEqual(
					providers[2].isDepleted(),
					true,
					"Query 2: Single result provider is not depleted."
				);
			})
	...
	QUnit.test("Abort providers", function (assert) {
		...
		biggerQueue.get(100).always(function () {
