(function () {
	var planetHeader = ["Planet", "Radius (km)"],
		...
		numbers = [["12"], ["7"], ["13,000"], ["9"], ["14"], ["8.0"]],
		numbersAsc = [["7"], ["8.0"], ["9"], ["12"], ["14"], ["13,000"]],
		correctDateSorting1 = [
			["01 January 2010"],
			["05 February 2010"],
			["16 January 2010"],
	...
	tableTest(
		...
		simpleAsc,
		function ($table) {
			$table.tablesorter({ sortList: [{ 0: "asc" }, { 1: "asc" }] });
		}
	);
	tableTest(
		...
		function ($table) {
			$table.tablesorter();
			$table.data("tablesorter").sort([{ 0: "desc" }, { 1: "asc" }]);
		}
	);
	tableTest(
	...
	tableTest(
		...
		function ($table) {
			var event;
			$table.tablesorter({ sortList: [{ 0: "desc" }, { 1: "desc" }] });
	...
	QUnit.test("Reset sorting making table appear unsorted", function (assert) {
		...
		assert.strictEqual(
			$table.find("th.headerSortUp").length +
				$table.find("th.headerSortDown").length,
			0,
	...
	QUnit.test("Basic planet table: one unsortable column", function (assert) {
		...
		assert.deepEqual(tableExtract($table), planets, "table not sorted");
	...
	QUnit.test("Rowspan invalid value (T265503)", function (assert) {
		var rowspanText =
				"Row 1 col 3, Row 2 col 3, row 3 col 3 (but there is no row 3)",
			$table = $(
	...
	QUnit.test(
		...
		function (assert) {
			...
			$table.tablesorter();
			assert.strictEqual(
				$table.find("#A2").data("headerIndex"),
				undefined,
				"A2 should not be a sort header"
			);
			assert.strictEqual(
				$table.find("#C1").data("headerIndex"),
				2,
	...
	QUnit.test(
		...
		function (assert) {
			...
			$table.tablesorter();
			assert.strictEqual(
				$table.find("#C2").data("headerIndex"),
				2,
	...
	QUnit.test(
		...
		function (assert) {
			...
			$table.tablesorter();
			assert.strictEqual(
				$table.find("tr").eq(1).find("th").eq(1).data("headerIndex"),
				2,
