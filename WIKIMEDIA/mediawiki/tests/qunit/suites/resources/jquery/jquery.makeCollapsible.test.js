(function () {
	...
	QUnit.test("testing hooks/triggers", function (assert) {
		...
		$collapsible.on("beforeCollapse.mw-collapsible", function () {
			assert.assertTrue(
				...
				"first beforeCollapseExpand: content is visible"
			);
		});
		$collapsible.on("afterCollapse.mw-collapsible", function () {
			assert.assertTrue(
				$content.css("display") === "none",
				"first afterCollapseExpand: content is hidden"
			);
			...
			$collapsible.on("beforeExpand.mw-collapsible", function () {
				assert.assertTrue(
					$content.css("display") === "none",
					"second beforeCollapseExpand: content is hidden"
				);
			});
			$collapsible.on("afterExpand.mw-collapsible", function () {
				assert.assertTrue(
					$content.css("display") !== "none",
					"second afterCollapseExpand: content is visible"
				);
			});
	...
	QUnit.test("basic operation (<table>)", function (assert) {
		...
		assert.assertTrue(
			...
			"headerRow is visible"
		);
		assert.assertTrue(
			$contentRow.css("display") !== "none",
			"contentRow is visible"
		);

		$collapsible.on("afterCollapse.mw-collapsible", function () {
			assert.assertTrue(
				$headerRow.css("display") !== "none",
				"after collapsing: headerRow is still visible"
			);
			assert.assertTrue(
				$contentRow.css("display") === "none",
				"after collapsing: contentRow is hidden"
			);

			$collapsible.on("afterExpand.mw-collapsible", function () {
				assert.assertTrue(
					$headerRow.css("display") !== "none",
					"after expanding: headerRow is still visible"
				);
				assert.assertTrue(
					$contentRow.css("display") !== "none",
	...
	function tableWithCaptionTest($collapsible, test, assert) {
		...
		$collapsible.on("afterCollapse.mw-collapsible", function () {
			assert.assertTrue(
				$caption.css("display") !== "none",
				"after collapsing: caption is still visible"
			);
			assert.assertTrue(
				$headerRow.css("display") === "none",
				"after collapsing: headerRow is hidden"
			);
			assert.assertTrue(
				$contentRow.css("display") === "none",
				"after collapsing: contentRow is hidden"
	...
	function listTest(listType, test, assert) {
		...
		assert.assertTrue(
			...
			"toggleItem is visible"
		);
		assert.assertTrue(
			$contentItem.css("display") !== "none",
			"contentItem is visible"
		);

		$collapsible.on("afterCollapse.mw-collapsible", function () {
			assert.assertTrue(
				$toggleItem.css("display") !== "none",
				"after collapsing: toggleItem is still visible"
			);
			assert.assertTrue(
				$contentItem.css("display") === "none",
				"after collapsing: contentItem is hidden"
			);

			$collapsible.on("afterExpand.mw-collapsible", function () {
				assert.assertTrue(
					$toggleItem.css("display") !== "none",
					"after expanding: toggleItem is still visible"
				);
				assert.assertTrue(
					$contentItem.css("display") !== "none",
					"after expanding: contentItem is visible"
				);
			});
	...
	QUnit.test("mw-made-collapsible data added", function (assert) {
		var $collapsible = prepareCollapsible("<div>" + loremIpsum + "</div>");

		assert.strictEqual(
			$collapsible.data("mw-made-collapsible"),
			true,
	...
	QUnit.test("mw-collapsible added when missing", function (assert) {
		...
		assert.assertTrue(
			$collapsible.hasClass("mw-collapsible"),
		...
		function (assert) {
			...
			$collapsible1
				.on("afterCollapse.mw-collapsible", function () {
					// eslint-disable-next-line no-jquery/no-class-state
					assert.assertTrue(
						$collapsible1.hasClass("mw-collapsed"),
						"after collapsing: parent is collapsed"
					);
					// eslint-disable-next-line no-jquery/no-class-state
					assert.assertFalse(
						$collapsible2.hasClass("mw-collapsed"),
