(function () {
	...
	QUnit.test("getAccessKeyPrefix", function (assert) {
		...
		for (i = 0; i < getAccessKeyPrefixTestData.length; i++) {
			assert.strictEqual(
				...
				getAccessKeyPrefixTestData[i][2],
				"Correct prefix for " + getAccessKeyPrefixTestData[i][0]
			);
		}
	});
	...
	QUnit.test("updateTooltipAccessKeys - current browser", function (assert) {
		...
		assert.notEqual(
			result[1],
			"test-",
			"Prefix used for testing shouldn't be used in production."
		);
	});
	...
	QUnit.test("updateTooltipAccessKeys with label element", function (assert) {
		...
		assert.strictEqual(
			$input.prop("title"),
			"",
			"No title attribute added to input element."
		);
		assert.strictEqual(
			$label.prop("title"),
			"Title [test-a]",
			"title updated for associated label element."
		);
		$.fn.updateTooltipAccessKeys.setTestMode(false);
	});
