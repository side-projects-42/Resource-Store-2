(function () {
	...
	QUnit.test("FormData support", function (assert) {
		...
		this.server.respond(function (request) {
			if (window.FormData) {
				...
				assert.notOk(
					request.url.match(/action=test/),
					"Request has no query string"
				);
				assert.strictEqual(
					request.requestBody,
					"action=test&format=json",
					"Request uses query string body"
				);
			}
	...
	QUnit.test(
		...
		function (assert) {
			...
			return api.get({
				test: [new mw.Title("Foo"), new mw.Title("Bar")],
			});
		}
	);
	...
	QUnit.test("Omitting false booleans", function (assert) {
		...
		this.server.respond(function (request) {
			assert.notOk(
				request.url.match(/foo/),
				"foo query parameter is not present"
			);
			assert.ok(
				request.url.match(/bar=true/),
				"bar query parameter is present with value true"
			);
			request.respond(200, { "Content-Type": "application/json" }, "[]");
		});
	...
	QUnit.test("getToken() - uncached", function (assert) {
		...
		this.server.respondWith(/type=testuncached/, [
			200,
			{ "Content-Type": "application/json" },
	...
	QUnit.test("getToken() - no query", function (assert) {
		...
		this.server.respondWith(/type=testnoquery/, [
			200,
			{ "Content-Type": "application/json" },
	...
	QUnit.test("getToken() - deprecated", function (assert) {
		...
		this.server.respondWith(/type=csrf/, [
			200,
			{ "Content-Type": "application/json" },
	...
	QUnit.test("postWithToken( tokenType, params )", function (assert) {
		...
		this.server.respondWith("GET", /type=testpost/, [
			200,
			{ "Content-Type": "application/json" },
		...
		return api
			.postWithToken("testpost", { action: "example", key: "foo" })
			.then(function (data) {
	...
	QUnit.test(
		...
		function (assert) {
			...
			this.server.respondWith(/assert=user/, [
				200,
				{ "Content-Type": "application/json" },
	...
	QUnit.test(
		...
		function (assert) {
			...
			return api
				...
				.then(function () {
					assert.strictEqual(
						test.server.requests[0].requestHeaders["X-Foo"],
						"Bar",
						"Header sent"
					);
					...
					return api.postWithToken(
						...
						function () {
							assert.ok(
								false,
								"This parameter cannot be a callback"
							);
						}
					);
				})
	...
	QUnit.test("postWithToken() - badtoken", function (assert) {
		...
		return api
			.postWithToken("testbadtoken", { action: "example", key: "foo" })
			.then(function (data) {
				assert.deepEqual(data, { example: { foo: "quux" } });
			});
	});
	...
	QUnit.test("postWithToken() - badtoken-cached", function (assert) {
		...
		return api
			.postWithToken("testonce", { action: "example", key: "foo" })
			.then(function (data) {
				...
				return api.postWithToken("testonce", {
					action: "example",
					key: "bar",
				});
			})
