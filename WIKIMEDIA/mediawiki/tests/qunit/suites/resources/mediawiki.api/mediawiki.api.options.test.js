(function () {
	...
	QUnit.test("saveOptions without Unit Separator", function (assert) {
		...
		this.server.respond(/meta=tokens&type=csrf/, [
			200,
		...
		return QUnit.whenPromisesComplete(
			...
			api
				.saveOptions({ foo: "bar|quux", bar: "a|b|c", baz: "quux" })
				.then(function () {
			...
			api.saveOptions({ "foo|bar=quux": null }).then(function () {
				assert.ok(
					true,
					"Request completed: reset an option, not bundleable"
				);
			})
		);
	});
		...
		return QUnit.whenPromisesComplete(
			...
			api
				...
				.done(function () {
					assert.ok(
						true,
						"Request completed: bundleable with unit separator"
					);
				}),
			api
				.saveOptions({
				...
				.done(function () {
					assert.ok(
						true,
						"Request completed: not bundleable with unit separator"
					);
				}),
