(function () {
	...
	QUnit.test("edit( title, transform String )", function (assert) {
		this.server.respond(function (req) {
			...
			if (
				/edit.+basetimestamp=2016-01-01.+starttimestamp=2016-01-02.+text=Box\./.test(
					req.requestBody
				)
			) {
				req.respond(
	...
	QUnit.test("edit( title, transform Promise )", function (assert) {
		this.server.respond(function (req) {
			...
			if (
				/edit.+basetimestamp=2016-02-01.+starttimestamp=2016-02-02.+text=Promise\./.test(
					req.requestBody
				)
			) {
				req.respond(
		...
		return new mw.Api()
			.edit("Async", function (revision) {
				return $.Deferred().resolve(
					revision.content.replace("Async", "Promise")
				);
			})
			.then(function (edit) {
	...
	QUnit.test("edit( title, transform Object )", function (assert) {
		this.server.respond(function (req) {
			...
			if (
				/edit.+basetimestamp=2016-03-01.+starttimestamp=2016-03-02.+text=Content&summary=Sum/.test(
					req.requestBody
				)
			) {
				req.respond(
