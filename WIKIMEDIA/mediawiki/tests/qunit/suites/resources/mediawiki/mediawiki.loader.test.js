(function () {
	...
	mw.loader.addSource({
		testloader:
			mw.config.get("wgScriptPath") + "/tests/qunit/data/load.mock.php",
	});
	...
	QUnit.test(".using( .., Function callback ) Promise", function (assert) {
		...
		mw.loader.implement("test.promise", [
			QUnit.fixurl(
				mw.config.get("wgScriptPath") +
	...
	QUnit.test(
		...
		function (assert) {
			...
			mw.loader
				.using("test.set.circleC")
				.then(
					function () {
						assert.ok(
							false,
							"Unexpected resolution, expected error."
						);
					},
					function (e) {
						assert.ok(
							/Circular/.test(String(e)),
	...
	QUnit.test(".load() - Error: Unregistered", function (assert) {
		...
		assert.deepEqual(capture, [
			"Skipped unresolvable module test.load.unreg",
		]);
	});
	...
	QUnit.test('.implement( styles={ "css": [text, ..] } )', function (assert) {
		var $element = $('<div class="mw-test-implement-a"></div>').appendTo(
			"#qunit-fixture"
		);

		assert.notEqual($element.css("float"), "right", "style is clear");
	...
	QUnit.test(
		...
		function (assert) {
			var $element1 = $(
					'<div class="mw-test-implement-b1"></div>'
				).appendTo("#qunit-fixture"),
				$element2 = $(
					'<div class="mw-test-implement-b2"></div>'
				).appendTo("#qunit-fixture"),
				$element3 = $(
					'<div class="mw-test-implement-b3"></div>'
				).appendTo("#qunit-fixture"),
				done = assert.async();
			...
			assert.notEqual($element2.css("float"), "left", "style is clear");
			assert.notEqual(
				...
				},
				{
					url: {
						...
						screen: [
							...
							urlStyleTest(
								".mw-test-implement-b2",
								"float",
								"left"
							),
							urlStyleTest(
								".mw-test-implement-b3",
								"float",
								"right"
							),
						],
	...
	QUnit.test(
		".implement( styles={ <media>: text } ) (back-compat)",
		function (assert) {
			var $element = $(
				'<div class="mw-test-implement-c"></div>'
			).appendTo("#qunit-fixture");
	...
	// Backwards compatibility
	QUnit.test(
		".implement( styles={ <media>: [url, ..] } ) (back-compat)",
		function (assert) {
			var $element = $(
					'<div class="mw-test-implement-d"></div>'
				).appendTo("#qunit-fixture"),
				$element2 = $(
					'<div class="mw-test-implement-d2"></div>'
				).appendTo("#qunit-fixture"),
				done = assert.async();
					...
					assertStyleAsync(
						...
						function () {
							assert.notEqual(
								$element2.css("text-align"),
								"center",
								"print style is not applied (T42500)"
							);
							done();
						}
					);
	...
	QUnit.test(".implement( messages before script )", function (assert) {
		mw.loader.implement(
			...
			function () {
				assert.strictEqual(
					mw.loader.getState("test.implement.order"),
					"executing",
					"state during script execution"
				);
				assert.strictEqual(
					mw.msg("test-foobar"),
					"Hello Foobar, $1!",
					"messages load before script execution"
				);
			},
			{},
			{
	...
	QUnit.test(".implement( styles with @import )", function (assert) {
		...
		mw.loader.implement(
			...
			function () {
				$element = $(
					'<div class="mw-test-implement-import">Foo bar</div>'
				).appendTo("#qunit-fixture");
	...
	QUnit.test(".implement( dependency with styles )", function (assert) {
		var $element = $('<div class="mw-test-implement-e"></div>').appendTo(
				"#qunit-fixture"
			),
			$element2 = $('<div class="mw-test-implement-e2"></div>').appendTo(
				"#qunit-fixture"
			);
	...
	QUnit.test(".implement( only messages )", function (assert) {
		assert.assertFalse(
			mw.messages.exists("T31107"),
			"Verify that the test message doesn't exist yet"
		);
		...
		return mw.loader.using("test.implement.msgs", function () {
			assert.ok(
				mw.messages.exists("T31107"),
				"T31107: messages-only module should implement ok"
			);
		});
	});
			...
			"test.implement.packageFiles",
			{
				...
				files: {
					...
					"resources/src/foo/init.js": function (require) {
						initJsRan = true;
						assert.deepEqual(
							require("./data/hello.json"),
							{ hello: "world" },
							"require() with .json file"
						);
						assert.deepEqual(
							require("./foo.js"),
							{ answer: 42 },
							"require() with .js file in same directory"
						);
						assert.deepEqual(
							require("../bar/bar.js"),
							{ data: "Hello Alice" },
							"require() with ../ of a file that uses same-directory require()"
						);
						assert.deepEqual(
							require("./foo.js"),
							{ answer: 42 },
							"require()ing the same script twice only runs it once"
						);
					},
				},
			},
	...
	QUnit.test(".addSource()", function (assert) {
		...
		assert.throws(
			...
			/already registered/,
			"duplicate pair from addSource(Object)"
		);
		...
		assert.throws(
			...
			/already registered/,
			"duplicate ID from addSource(Object)"
		);
	});
	...
	QUnit.test(
		"Url composition (modules considered for version)",
		function (assert) {
			mw.loader.register([
			...
			return mw.loader
				.using(["testUrlIncDump", "testUrlInc"])
				.then(function (require) {
					...
					assert.strictEqual(
						mw.loader.getState("testUrlInc"),
						"ready",
						"testUrlInc also loaded"
					);
				});
		}
	);
	...
	QUnit.test(
		"Url composition (order of modules for version) – T188076",
		function (assert) {
			mw.loader.register([
				// [module, version, dependencies, group, source]
	...
	QUnit.test("Broken indirect dependency", function (assert) {
		...
		assert.strictEqual(
			mw.loader.getState("test.module1"),
			"error",
			"State of test.module1"
		);
		assert.strictEqual(
			mw.loader.getState("test.module2"),
			"error",
			"State of test.module2"
		);
		assert.strictEqual(
			mw.loader.getState("test.module3"),
			"error",
			"State of test.module3"
		);
	...
	QUnit.test("Out-of-order implementation", function (assert) {
		...
		this.tick();
		assert.strictEqual(
			mw.loader.getState("test.module4"),
			"ready",
			"State of test.module4"
		);
		assert.strictEqual(
			mw.loader.getState("test.module5"),
			"registered",
			"State of test.module5"
		);
		assert.strictEqual(
			mw.loader.getState("test.module6"),
			"registered",
			"State of test.module6"
		);
	...
	QUnit.test("Missing dependency", function (assert) {
		...
		this.tick();
		assert.strictEqual(
			mw.loader.getState("test.module7"),
			"registered",
			'Expected "registered" state for test.module7'
		);
		assert.strictEqual(
			mw.loader.getState("test.module8"),
			"loaded",
			'Expected "loaded" state for test.module8'
		);
		assert.strictEqual(
			mw.loader.getState("test.module9"),
			"registered",
			'Expected "registered" state for test.module9'
		);
		...
		assert.strictEqual(
			mw.loader.getState("test.module7"),
			"missing",
			'Expected "missing" state for test.module7'
		);
		assert.strictEqual(
			mw.loader.getState("test.module8"),
			"error",
			'Expected "error" state for test.module8'
		);
		assert.strictEqual(
			mw.loader.getState("test.module9"),
			"error",
			'Expected "error" state for test.module9'
		);
	...
	QUnit.test("Dependency handling", function (assert) {
		...
		function verifyModuleStates() {
			assert.strictEqual(
				mw.loader.getState("testMissing"),
				"missing",
				'Module "testMissing" state'
			);
			assert.strictEqual(
				mw.loader.getState("testUsesMissing"),
				"error",
				'Module "testUsesMissing" state'
			);
			assert.strictEqual(
				mw.loader.getState("testUsesNestedMissing"),
				"error",
				'Module "testUsesNestedMissing" state'
			);
		}
		...
		return mw.loader.using(["testUsesNestedMissing"]).then(
			...
			function (e, modules) {
				...
				assert.ok(
					modules.indexOf("testMissing") !== -1,
					"Triggered by testMissing."
				);
	...
	QUnit.test("Skip-function handling", function (assert) {
		...
		return mw.loader.using(["testUsesSkippable"]).then(
			function () {
				assert.strictEqual(
					mw.loader.getState("testSkipped"),
					"ready",
					"Skipped module"
				);
				assert.strictEqual(
					mw.loader.getState("testNotSkipped"),
					"ready",
					"Regular module"
				);
				assert.strictEqual(
					mw.loader.getState("testUsesSkippable"),
					"ready",
					"Regular module with skippable dependency"
				);
			},
	...
	QUnit.test('.load( "/absolute-path" )', function (assert) {
		...
		assert.strictEqual(
			target.slice(0, 1),
			"/",
			"URL is relative to document root"
		);
	...
	QUnit.test("Empty string module name - T28804", function (assert) {
		...
		assert.strictEqual(
			mw.loader.moduleRegistry[""].state,
			"registered",
			"State before"
		);
		assert.strictEqual(
			mw.loader.moduleRegistry[""].version,
			"v1",
			"Version"
		);
	...
	QUnit.test("Executing race - T112232", function (assert) {
		...
		mw.loader.implement("testRaceRedHerring", function () {}, {
		...
		return mw.loader.using("testRaceLoadMe", function () {
			assert.strictEqual(done, true, "script ran");
			assert.strictEqual(
				mw.loader.getState("testRaceLoadMe"),
				"ready",
				"state"
			);
		});
	...
	QUnit.test("Stale response caching - T117587", function (assert) {
		...
		assert.strictEqual(
			mw.loader.store.get("test.stale"),
			false,
			"Not in store"
		);
		...
		return mw.loader
			...
			.then(function () {
				...
				assert.strictEqual(
					mw.loader.moduleRegistry["test.stale"].version,
					"v1",
					"Override version"
				);
				assert.strictEqual(
					mw.loader.moduleRegistry["test.stale"].state,
					"ready"
				);
				assert.strictEqual(
					typeof mw.loader.store.get("test.stale"),
					"string",
					"In store"
				);
			})
	...
	QUnit.test("Stale response caching - backcompat", function (assert) {
		...
		assert.strictEqual(
			mw.loader.store.get("test.stalebc"),
			false,
			"Not in store"
		);
		...
		return mw.loader
			...
			.then(function () {
				...
				assert.strictEqual(
					typeof mw.loader.store.get("test.stalebc"),
					"string",
					"In store"
				);
			})
	...
	QUnit[mw.loader.store.enabled ? "test" : "skip"](
		"mw.loader.store.init - Invalid JSON",
		function (assert) {
			// Reset
	...
	QUnit[mw.loader.store.enabled ? "test" : "skip"](
		"mw.loader.store.init - Wrong JSON",
		function (assert) {
			// Reset
			this.sandbox.stub(mw.loader.store, "enabled", null);
			this.sandbox.stub(mw.loader.store, "items", {});
	...
	QUnit[mw.loader.store.enabled ? "test" : "skip"](
		"mw.loader.store.init - Expired JSON",
		function (assert) {
			// Reset
			this.sandbox.stub(mw.loader.store, "enabled", null);
			this.sandbox.stub(mw.loader.store, "items", {});
	...
	QUnit[mw.loader.store.enabled ? "test" : "skip"](
		"mw.loader.store.init - Good JSON",
		function (assert) {
			// Reset
			this.sandbox.stub(mw.loader.store, "enabled", null);
			this.sandbox.stub(mw.loader.store, "items", {});
		...
		return mw.loader
			...
			.then(function (require) {
				...
				assert.strictEqual(
					typeof module1,
					"object",
					"export of module with no export"
				);
				...
				assert.strictEqual(
					module4.pizza(),
					"hello world",
					"module can require other modules"
				);
				...
				assert.throws(
					...
					/is not loaded/,
					"Requesting non-existent modules throws error."
				);
			});
	});
	...
	QUnit.test("require() in debug mode", function (assert) {
		...
		mw.loader.implement("test.require.callback", [
			QUnit.fixurl(
				path + "/tests/qunit/data/requireCallMwLoaderTestCallback.js"
			),
		]);
		mw.loader.implement("test.require.define", [
			QUnit.fixurl(
	...
	QUnit.test("Implicit dependencies", function (assert) {
		...
		mw.loader.implement("site", function () {
			site++;
		});
		mw.loader.implement("user", function () {
			user++;
			siteFromUser = site;
		});
	...
	QUnit.test(".getScript() - success", function (assert) {
		var scriptUrl = QUnit.fixurl(
			mw.config.get("wgScriptPath") +
				"/tests/qunit/data/mediawiki.loader.getScript.example.js"
		);

		return mw.loader.getScript(scriptUrl).then(function () {
			assert.strictEqual(
				mw.getScriptExampleScriptLoaded,
				true,
				"Data attached to a global object is available"
			);
		});
	});
