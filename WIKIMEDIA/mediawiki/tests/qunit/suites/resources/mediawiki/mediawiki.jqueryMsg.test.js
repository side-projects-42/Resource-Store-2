(function () {
	...
	function assertBothModes(
		...
		assert.strictEqual(
			formatText.apply(null, parserArguments),
			expectedResult,
			assertMessage + " when format is 'text'"
		);
		assert.strictEqual(
			formatParse.apply(null, parserArguments),
			expectedResult,
			assertMessage + " when format is 'parse'"
		);
	}
		...
		QUnit.newMwEnvironment({
			setup: function () {
				...
				expectedListUsersSitename =
					...
					"Wiki</a>";
				expectedLinkPagenamee =
					'<a href="https://example.org/wiki/Foo?bar=baz#val/2_%2B_2" class="external">Test</a>';
			...
			messages: {
				...
				"plural-msg-explicit-forms-nested":
					"Found {{PLURAL:$1|$1 results|0=no results in {{SITENAME}}|1=$1 result}}",
				...
				"see-portal-url":
					"{{Int:portal-url}} is an important community page.",
				...
				"external-link-plural":
					"Foo {{PLURAL:$1|is [$2 one]|are [$2 some]|2=[$2 two]|3=three|4=a=b}} things.",
				"plural-only-explicit-forms":
					"It is a {{PLURAL:$1|1=single|2=double}} room.",
				"plural-empty-explicit-form":
					"There is me{{PLURAL:$1|0=| and other people}}.",
			},
		})
	);
	...
	QUnit.test("Replace", function (assert) {
		...
		assert.strictEqual(
			formatParse("simple", "bar"),
			"Foo bar baz $2",
			"Replacements with less substitutes"
		);
		assert.strictEqual(
			formatParse("simple", "bar", "quux"),
			"Foo bar baz quux",
			"Replacements with all substitutes"
		);
	...
	QUnit.test("Plural", function (assert) {
		assert.strictEqual(
			formatParse("plural-msg", 0),
			"Found 0 items",
			"Plural test for english with zero as count"
		);
		assert.strictEqual(
			formatParse("plural-msg", 1),
			"Found 1 item",
			"Singular test for english"
		);
		assert.strictEqual(
			formatParse("plural-msg", 2),
			"Found 2 items",
			"Plural test for english"
		);
		assert.strictEqual(
			formatParse("plural-msg-explicit-forms-nested", 6),
			"Found 6 results",
			"Plural message with explicit plural forms"
		);
		assert.strictEqual(
			formatParse("plural-msg-explicit-forms-nested", 0),
			"Found no results in Wiki",
			"Plural message with explicit plural forms, with nested {{SITENAME}}"
		);
		assert.strictEqual(
			formatParse("plural-msg-explicit-forms-nested", 1),
			"Found 1 result",
			"Plural message with explicit plural forms with placeholder nested"
		);
		assert.strictEqual(
			formatParse("plural-empty-explicit-form", 0),
			"There is me."
		);
		assert.strictEqual(
			formatParse("plural-empty-explicit-form", 1),
			"There is me and other people."
		);
		assert.strictEqual(
			formatParse("plural-empty-explicit-form", 2),
			"There is me and other people."
		);
	});
	...
	QUnit.test("Gender", function (assert) {
		...
		mw.messages.set(
			"gender-msg-one-form",
			"{{GENDER:$1|User}}: $2 {{PLURAL:$2|edit|edits}}"
		);
		...
		mw.messages.set(
			"gender-msg-lowercase",
			"{{gender:$1|he|she}} is awesome"
		);
		assert.strictEqual(
	...
	QUnit.test("Case changing", function (assert) {
		mw.messages.set(
			"to-lowercase",
			"{{lc:thIS hAS MEsSed uP CapItaliZatiON}}"
		);
		assert.strictEqual(
			formatParse("to-lowercase"),
			"this has messed up capitalization",
			"To lowercase"
		);
		...
		mw.messages.set(
			"uc-to-lcfirst",
			"{{lcfirst:THis hAS MEsSed uP CapItaliZatiON}}"
		);
		mw.messages.set(
			"lc-to-lcfirst",
			"{{lcfirst:thIS hAS MEsSed uP CapItaliZatiON}}"
		...
		assert.strictEqual(
			formatParse("uc-to-lcfirst"),
			"tHis hAS MEsSed uP CapItaliZatiON",
			"Lcfirst caps"
		);
		assert.strictEqual(
			formatParse("lc-to-lcfirst"),
			"thIS hAS MEsSed uP CapItaliZatiON",
			"Lcfirst lowercase"
		);
		...
		mw.messages.set(
			"uc-to-ucfirst",
			"{{ucfirst:THis hAS MEsSed uP CapItaliZatiON}}"
		);
		mw.messages.set(
			"lc-to-ucfirst",
			"{{ucfirst:thIS hAS MEsSed uP CapItaliZatiON}}"
		...
		assert.strictEqual(
			formatParse("uc-to-ucfirst"),
			"THis hAS MEsSed uP CapItaliZatiON",
			"Ucfirst caps"
		);
		assert.strictEqual(
			formatParse("lc-to-ucfirst"),
			"ThIS hAS MEsSed uP CapItaliZatiON",
			"Ucfirst lowercase"
		);
		...
		mw.messages.set(
			"mixed-to-sentence",
			"{{ucfirst:{{lc:thIS hAS MEsSed uP CapItaliZatiON}}}}"
		);
		assert.strictEqual(
			formatParse("mixed-to-sentence"),
			"This has messed up capitalization",
			"To sentence case"
		);
		mw.messages.set(
			"all-caps-except-first",
			"{{lcfirst:{{uc:thIS hAS MEsSed uP CapItaliZatiON}}}}"
		);
		assert.strictEqual(
			formatParse("all-caps-except-first"),
			"tHIS HAS MESSED UP CAPITALIZATION",
			"To opposite sentence case"
		);
	});

	QUnit.test("Grammar", function (assert) {
		assert.strictEqual(
			formatParse("grammar-msg"),
			"Przeszukaj Wiki",
			"Grammar Test with sitename"
		);

		mw.messages.set(
			"grammar-msg-wrong-syntax",
			"Przeszukaj {{GRAMMAR:grammar_case_xyz}}"
		);
		assert.strictEqual(
			formatParse("grammar-msg-wrong-syntax"),
			"Przeszukaj ",
			"Grammar Test with wrong grammar template syntax"
		);
	});

	QUnit.test("Bi-di", function (assert) {
		assert.strictEqual(
			formatParse("bidi-msg", "Bob"),
			"Welcome \u202ABob\u202C!",
			"Bidi test (LTR)"
		);
		assert.strictEqual(
			formatParse("bidi-msg", "בוב"),
			"Welcome \u202Bבוב\u202C!",
			"Bidi test (RTL)"
		);
	});
	...
	QUnit.test("Links", function (assert) {
		...
		testCases.forEach(function (testCase) {
			...
			assert.htmlEqual(formatParse(key), output, "HTML in links: " + key);
		});
	});
	...
	QUnit.test("mw.Message.prototype.parser monkey-patch", function (assert) {
		...
		verifyGetMessageFunction(
			"jquerymsg-test-pagetriage-del-talk-page-notify-summary",
			"plain",
			false
		);
		verifyGetMessageFunction(
			"jquerymsg-test-categorytree-collapse-bullet",
			"plain",
			false
		);
		verifyGetMessageFunction(
			"jquerymsg-test-wikieditor-toolbar-help-content-signature-result",
			"plain",
			false
		);
		...
		function (assert) {
			...
			assert.htmlEqual(
				mw
					.message(
						"1x-wikitext",
						'<script>alert( "1x-wikitext test" )</script>'
					)
					.parse(),
	...
	QUnit.test("formatnum", function (assert) {
		...
		formatnumTests.forEach(function (test) {
			...
			assert.strictEqual(
				parser
					.parse(
						test.integer ? "formatnum-msg-int" : "formatnum-msg",
						[test.number]
					)
					.html(),
				test.result,
	...
	QUnit.test("HTML", function (assert) {
		...
		assertBothModes(
			assert,
			["jquerymsg-italics-msg"],
			mw.messages.get("jquerymsg-italics-msg"),
		...
		assertBothModes(
			assert,
			["jquerymsg-bold-msg"],
			mw.messages.get("jquerymsg-bold-msg"),
			"Simple bold unchanged"
		);

		mw.messages.set(
			"jquerymsg-bold-italics-msg",
			"It is <b><i>key</i></b>"
		);
		assertBothModes(
			assert,
			["jquerymsg-bold-italics-msg"],
			mw.messages.get("jquerymsg-bold-italics-msg"),
			"Bold and italics nesting order preserved"
		);

		mw.messages.set(
			"jquerymsg-italics-bold-msg",
			"It is <i><b>vital</b></i>"
		);
		assertBothModes(
			assert,
			["jquerymsg-italics-bold-msg"],
			mw.messages.get("jquerymsg-italics-bold-msg"),
			"Italics and bold nesting order preserved"
		);

		mw.messages.set(
			"jquerymsg-italics-with-link",
		...
		assert.htmlEqual(
			...
			'An <i>italicized <a title="link" href="' +
				mw.html.escape(mw.util.getUrl("link")) +
				'">wiki-link</i>',
		...
		mw.messages.set(
			"jquerymsg-italics-id-class",
			'<i id="foo" class="bar">Foo</i>'
		);
		assert.htmlEqual(
		...
		mw.messages.set(
			"jquerymsg-script-and-external-link",
			'<script>alert( "jquerymsg-script-and-external-link test" );</script> [http://example.com <i>Foo</i> bar]'
		);
		assert.htmlEqual(
		...
		mw.messages.set(
			"jquerymsg-link-script",
			'[http://example.com <script>alert( "jquerymsg-link-script test" );</script>]'
		);
		assert.htmlEqual(
		...
		mw.messages.set(
			"jquerymsg-double-quotes-preserved",
			'<i id="double">Double</i>'
		);
		assert.strictEqual(
		...
		mw.messages.set(
			"jquerymsg-single-quotes-normalized-to-double",
		...
		mw.messages.set(
			"jquerymsg-escaped-double-quotes-attribute",
			'<i style="font-family:&quot;Arial&quot;">Styled</i>'
		);
		assert.htmlEqual(
		...
		mw.messages.set(
			"jquerymsg-escaped-single-quotes-attribute",
			"<i style='font-family:&#039;Arial&#039;'>Styled</i>"
		);
		assert.htmlEqual(
		...
		mw.messages.set(
			"jquerymsg-wikitext-contents-parsed",
			"<i>[http://example.com Example]</i>"
		);
		assert.htmlEqual(
		...
		mw.messages.set(
			"jquerymsg-wikitext-contents-script",
		...
		mw.messages.set(
			"jquerymsg-asciialphabetliteral-regression",
			'<b >>>="dir">asd</b>'
		);
		assert.htmlEqual(
	...
	QUnit.test("Nowiki", function (assert) {
		...
		mw.messages.set(
			"jquerymsg-nowiki-htmltag",
			"Foo <nowiki><b>bar</b></nowiki> baz."
		);
		assert.strictEqual(
		...
		cases = [
			{
				...
				msg: "[[$1]] [[$1|a]]",
				expected:
					'<a title="x" href="/wiki/x">x</a> <a title="x" href="/wiki/x">a</a>',
			},
			{
	...
	QUnit.test("Integration", function (assert) {
		...
		assert.strictEqual(
			mw.message("missing-message").parse(),
			"⧼missing-message⧽",
			"qqx message (missing)"
		);
		assert.strictEqual(
			mw.message("missing-message", $bar, "baz").parse(),
			"⧼missing-message⧽",
			"qqx message (missing) with parameters"
		);
		assert.strictEqual(
			mw.message("qqx-message").parse(),
			"(qqx-message)",
			"qqx message (defined)"
		);
		assert.strictEqual(
			mw.message("qqx-message", $bar, "baz").parse(),
			"(qqx-message: <b>bar</b>, baz)",
			"qqx message (defined) with parameters"
		);
		assert.strictEqual(
			mw.message("non-qqx-message").parse(),
			"<b>hello world</b>",
