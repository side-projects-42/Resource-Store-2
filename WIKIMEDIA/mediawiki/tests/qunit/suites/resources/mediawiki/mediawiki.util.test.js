(function () {
	var util = require("mediawiki.util"),
		...
		IPV4_CASES = [
			...
			[
				false,
				"74.24.52.13/20",
				"IPv4 ranges are not recognized as valid IPs",
			],
		],
	...
	QUnit.test("rawurlencode", function (assert) {
		assert.strictEqual(
			util.rawurlencode("Test:A & B/Here"),
	...
	QUnit.test("escapeIdForAttribute", function (assert) {
		...
		[
			...
			assert.strictEqual(
				util.escapeIdForAttribute(testCase[1]),
				testCase[2]
			);
		});
	});
	...
	QUnit.test("wikiUrlencode", function (assert) {
		assert.strictEqual(
			util.wikiUrlencode("Test:A & B/Here"),
			"Test:A_%26_B/Here"
		);
	...
	QUnit.test("getUrl", function (assert) {
		...
		assert.strictEqual(
			href,
			"/wiki/Foo:Sandbox%3F_5%2B5%3D10!_(test)/sub_",
			"complex title"
		);
		...
		assert.strictEqual(
			href,
			"/wiki/My$$test$$$$$title",
			"title with multiple consecutive dollar signs"
		);
		...
		assert.strictEqual(
			href,
			"/w/index.php?title=Foobar&action=edit",
			"default title with query string"
		);

		href = util.getUrl("Sandbox", { action: "edit" });
		assert.strictEqual(
			href,
			"/w/index.php?title=Sandbox&action=edit",
			"simple title with query string"
		);
		...
		assert.strictEqual(
			href,
			"/w/index.php?title=Foo:Sandbox&action=edit#Fragment",
			"namespaced title with query string and fragment"
		);
		...
		assert.strictEqual(
			href,
			"/w/index.php?title=Sandbox&action=edit",
			"title with query string and empty fragment"
		);
		...
		assert.strictEqual(
			href,
			"/wiki/#Fragment",
			"empty title with fragment"
		);

		href = util.getUrl("#Fragment", { action: "edit" });
		assert.strictEqual(
			href,
			"/w/index.php?action=edit#Fragment",
			"empty title with query string and fragment"
		);

		mw.util.setOptionsForTest({ FragmentMode: ["legacy"] });
		href = util.getUrl("Foo:Sandbox \xC4#Fragment \xC4", {
			action: "edit",
		});
		assert.strictEqual(
			href,
			"/w/index.php?title=Foo:Sandbox_%C3%84&action=edit#Fragment_.C3.84",
			"title with query string, fragment, and special characters"
		);
		...
		assert.strictEqual(
			href,
			"/w/index.php?title=Foo:Sandbox_%C3%84&action=edit#Fragment_Ä",
			"title with query string, fragment, and special characters"
		);
		...
		assert.strictEqual(
			href,
			"/w/index.php?title=Foo:%2523&action=edit#Fragment",
	...
	QUnit.test("wikiScript", function (assert) {
		...
		assert.strictEqual(
			util.wikiScript("index"),
			mw.config.get("wgScript"),
			"wikiScript( index ) returns wgScript"
		);
		assert.strictEqual(
			util.wikiScript("load"),
			"/w/l.php",
	...
	QUnit.test("addCSS", function (assert) {
		...
		assert.strictEqual(
			style.disabled,
			false,
			'property "disabled" is available and set to false'
		);

		assert.strictEqual(
			$el.css("visibility"),
			"hidden",
			"Added style properties are in effect"
		);
	...
	QUnit.test("getParamValue", function (assert) {
		...
		assert.strictEqual(
			util.getParamValue("foo", url),
			"right",
			"Use latest one, ignore hash"
		);
		assert.strictEqual(
			util.getParamValue("bar", url),
			null,
		...
		assert.strictEqual(
			util.getParamValue("foo", url),
			null,
			"Ignore hash if param is not in querystring but in hash (T29427)"
		);

		url = "example.org?" + $.param({ TEST: "a b+c" });
		assert.strictEqual(
			util.getParamValue("TEST", url),
			"a b+c",
			'T32441: getParamValue must understand "+" encoding of space'
		);
		...
		assert.strictEqual(
			util.getParamValue("TEST", url),
			"a b+c d",
			'T32441: getParamValue must understand "+" encoding of space (multiple spaces)'
		);

		url = "example.org?title=Autom%F3vil";
		assert.strictEqual(
			util.getParamValue("title", url),
			null,
			"T268058: getParamValue can return null on input it cannot decode."
		);
	});

	function getParents(link) {
		return $(link)
			.parents("#qunit-fixture *")
			.toArray()
			.map(function (el) {
				return (
					el.tagName +
	...
	QUnit.test("addPortletLink (Minerva list)", function (assert) {
		...
		assert.propEqual(getParents(link), ["UL#p-list"], "List structure");
	});
	...
	QUnit.test("validateEmail", function (assert) {
		...
		assert.strictEqual(
			util.validateEmail("user@localhost"),
			true,
			"Return true for a valid e-mail address"
		);

		// testEmailWithCommasAreInvalids
		assert.strictEqual(
			util.validateEmail("user,foo@example.org"),
			false,
			"Emails with commas are invalid"
		);
		assert.strictEqual(
			util.validateEmail("userfoo@ex,ample.org"),
			false,
			"Emails with commas are invalid"
		);
	...
	QUnit.module("parseImageUrl", function (hooks) {
		...
		[
			{
				...
				width: 939,
				resizedUrl:
					"//upload.wikimedia.org/wikipedia/commons/thumb/b/bf/Princess_Alexandra_of_Denmark_%28later_Queen_Alexandra%2C_wife_of_Edward_VII%29_with_her_two_eldest_sons%2C_Prince_Albert_Victor_%28Eddy%29_and_George_Frederick_Ernest_Albert_%28later_George_V%29.jpg/1000px-thumbnail.jpg",
			},

			{
				...
				width: 939,
				resizedUrl:
					"//upload.wikimedia.org/wikipedia/commons/thumb/b/bf/Princess_Alexandra_of_Denmark_%28later_Queen_Alexandra%2C_wife_of_Edward_VII%29_with_her_two_eldest_sons%2C_Prince_Albert_Victor_%28Eddy%29_and_George_Frederick_Ernest_Albert_%28later_George_V%29.jpg/1000px-ki708pr1r6g2dl5lbhvwdqxenhait13.jpg",
			},

			{
				...
				width: 99,
				resizedUrl:
					"/wiki/images/thumb/9/91/Anticlockwise_heliotrope%27s.jpg/1000px-Anticlockwise_heliotrope%27s.jpg",
			},

			{
				...
				width: 150,
				resizedUrl:
					"/wiki/images/thumb/8/80/Wikipedia-logo-v2.svg/langde-1000px-Wikipedia-logo-v2.svg.png",
			},

			{
				...
				width: 100,
				resizedUrl:
					"/wiki/images/thumb/1/10/Little_Bobby_Tables-100px-file.jpg/qlow-1000px-Little_Bobby_Tables-100px-file.jpg",
			},

			{
				...
				width: 100,
				resizedUrl:
					"/wiki/images/thumb/1/10/Little_Bobby%22%3B_Tables-100px-file.jpg/qlow-1000px-Little_Bobby%22%3B_Tables-100px-file.jpg",
			},

			{
				...
				width: 150,
				resizedUrl:
					"//upload.wikimedia.org/wikipedia/commons/thumb/8/80/Wikipedia-logo-v2.svg/1000px-Wikipedia-logo-v2.svg.png",
			},
...

			{
				...
				width: 180,
				resizedUrl:
					"http://localhost/thumb.php?f=Stuffless_Figaro%27s.jpg&width=1000",
			},
...

			{
				...
				width: 150,
				resizedUrl:
					"/wikipedia/commons/thumb/Wikipedia-logo-v2.svg/1000px-Wikipedia-logo-v2.svg.png",
			},

			{
				...
				width: 150,
				resizedUrl:
					"/wikipedia/commons/thumb/Wikipedia-logo-v2.svg/langde-1000px-Wikipedia-logo-v2.svg.png",
			},
			...
			QUnit.test(
				"parseImageUrl: " + thisCase.typeOfUrl,
				function (assert) {
					var data;
					...
					if (thisCase.name !== undefined) {
						...
						if (thisCase.resizedUrl) {
							...
							assert.strictEqual(
								data.resizeUrl(1000),
								thisCase.resizedUrl,
								"Resized URL is correct"
							);
						} else {
							assert.notOk(
								data.resizeUrl,
								"resizeUrl is not set"
							);
						}
					} else {
						assert.strictEqual(
		...
		QUnit.test(
			...
			function (assert) {
				...
				resizeUrl = mw.util.parseImageUrl(
				...
				assert.strictEqual(
					resizeUrl(500),
					"/w?title=Special:Redirect/file/Princess_Alexandra_of_Denmark_(later_Queen_Alexandra,_wife_of_Edward_VII)_with_her_two_eldest_sons,_Prince_Albert_Victor_(Eddy)_and_George_Frederick_Ernest_Albert_(later_George_V).jpg&width=500",
					"Resized URL is correct"
				);
			}
		);
	...
	QUnit.test("debounce", function (assert) {
		...
		setTimeout(function () {
			assert.deepEqual(q, [3], "Last one ran");
			done();
		});
	});
