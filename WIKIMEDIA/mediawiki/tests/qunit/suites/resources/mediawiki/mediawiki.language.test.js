(function () {
	...
	QUnit.test("mw.language getData and setData", function (assert) {
		...
		assert.strictEqual(
			mw.language.getData("en", "testkey"),
			"testvalue",
			"Getter setter test for mw.language"
		);
		assert.strictEqual(
			mw.language.getData("en", "invalidkey"),
			null,
			"Getter setter test for mw.language with invalid key"
		);
		assert.strictEqual(
			mw.language.getData("xx", "invalidLang"),
			undefined,
			"Getter setter test for mw.language with invalid lang"
		);
		mw.language.setData("en-us", "testkey", "testvalue");
		assert.strictEqual(
			mw.language.getData("en-US", "testkey"),
			"testvalue",
			"Case insensitive test for mw.language"
		);
	});
	...
	QUnit.test("mw.language.commafy test", function (assert) {
		...
		assert.strictEqual(
			...
			"1234.567",
			"Pattern with no digit grouping separator defined"
		);
		assert.strictEqual(
			mw.language.commafy(123456789.567, "###0.#####"),
			"123456789.567",
			"Pattern with no digit grouping separator defined, bigger decimal part"
		);
		assert.strictEqual(
			mw.language.commafy(0.567, "###0.#####"),
			"0.567",
			"Decimal part 0"
		);
		assert.strictEqual(
			mw.language.commafy(".567", "###0.#####"),
			"0.567",
			"Decimal part missing. replace with zero"
		);
		assert.strictEqual(
			mw.language.commafy(1234, "##,#0.#####"),
			"12,34",
			"Pattern with no fractional part"
		);
		assert.strictEqual(
			mw.language.commafy(-1234.567, "###0.#####"),
			"-1234.567",
		...
		assert.strictEqual(
			mw.language.commafy(-1234.567, "#,###.00"),
			"-1,234.56",
			"Fractional part bigger than pattern."
		);
		assert.strictEqual(
			mw.language.commafy(123456789.567, "###,##0.00"),
			"123,456,789.56",
			"Decimal part as group of 3"
		);
		assert.strictEqual(
			mw.language.commafy(123456789.567, "###,###,#0.00"),
			"1,234,567,89.56",
			"Decimal part as group of 3 and last one 2"
		);
	});
	...
	QUnit.test("mw.language.convertNumber", function (assert) {
		...
		assert.strictEqual(
			mw.language.convertNumber(180),
			"180",
			"formatting 3-digit"
		);
		assert.strictEqual(
			mw.language.convertNumber(1800),
			"1.800",
			"formatting 4-digit"
		);
		assert.strictEqual(
			mw.language.convertNumber(18000),
			"18.000",
			"formatting 5-digit"
		);

		assert.strictEqual(
			mw.language.convertNumber("1.800", true),
			1800,
			"unformatting"
		);
		...
		assert.strictEqual(
			mw.language.convertNumber(180),
			"180",
			"formatting 3-digit with minimumGroupingDigits=2"
		);
		assert.strictEqual(
			mw.language.convertNumber(1800),
			"1800",
			"formatting 4-digit with minimumGroupingDigits=2"
		);
		assert.strictEqual(
			mw.language.convertNumber(18000),
			"18.000",
			"formatting 5-digit with minimumGroupingDigits=2"
		);
		...
		assert.strictEqual(
			mw.language.convertNumber(180),
			"180",
			"formatting 3-digit with minimumGroupingDigits=3"
		);
		assert.strictEqual(
			mw.language.convertNumber(1800),
			"1800",
			"formatting 4-digit with minimumGroupingDigits=3"
		);
		assert.strictEqual(
			mw.language.convertNumber(18000),
			"18000",
			"formatting 5-digit with minimumGroupingDigits=3"
		);
		assert.strictEqual(
			mw.language.convertNumber(180000),
			"180.000",
			"formatting 6-digit with minimumGroupingDigits=3"
		);
	});
	...
	QUnit.test(
		...
		function (assert) {
			...
			assert.strictEqual(
				mw.language.convertNumber(1200),
				"१.२००",
				"format"
			);
			assert.strictEqual(
				mw.language.convertNumber("१.२००", true),
				1200,
				"unformat from digit transform"
			);
			assert.strictEqual(
				mw.language.convertNumber("1.200", true),
				1200,
				"unformat plain"
			);
			...
			assert.strictEqual(
				mw.language.convertNumber("१.२००", true),
				1200,
				"unformat from digit transform (when disabled)"
			);
			assert.strictEqual(
				mw.language.convertNumber("1.200", true),
				1200,
				"unformat plain (digit transform disabled)"
			);
		}
	);
	...
	grammarTests = {
		...
		ru: [
			...
			},
			{
				...
				expected: "Википедию",
				description:
					"Grammar test for accusative case, Википедия -> Вики",
			},
			{
				word: "Викисловарь",
				grammarForm: "accusative",
				...
				description:
					"Grammar test for accusative case, Викисловарь -> Викисловарь",
			},
			{
				word: "Викицитатник",
				grammarForm: "accusative",
				...
				description:
					"Grammar test for accusative case, Викицитатник -> Викицитатник",
			},
			{
				word: "Викиучебник",
				grammarForm: "accusative",
				...
				description:
					"Grammar test for accusative case, Викиучебник -> Викиучебник",
			},
			{
				word: "Викитека",
				grammarForm: "accusative",
				expected: "Викитека",
				description:
					"Grammar test for accusative case, Викитека -> Викитеку",
			},
			{
				word: "Викиновости",
				grammarForm: "accusative",
				...
				description:
					"Grammar test for accusative case, Викиновости -> Викиновости",
			},
			{
				word: "Викиверситет",
				grammarForm: "accusative",
				...
				description:
					"Grammar test for accusative case, Викиверситет -> Викиверситет",
			},
			{
				word: "Викивиды",
				grammarForm: "accusative",
				expected: "Викивиды",
				description:
					"Grammar test for accusative case, Викивиды -> Викивиды",
			},
			{
				word: "Викиданные",
				grammarForm: "accusative",
				expected: "Викиданные",
				description:
					"Grammar test for accusative case, Викиданные -> Викиданные",
			},
			{
				word: "Викисклад",
				grammarForm: "accusative",
				expected: "Викисклад",
				description:
					"Grammar test for accusative case, Викисклад -> Викисклад",
			},
			{
				word: "Викигид",
				grammarForm: "accusative",
				expected: "Викигид",
				description:
					"Grammar test for accusative case, Викигид -> Викигид",
			},
			{
				word: "Мета",
				grammarForm: "accusative",
			...
			},
			{
				...
				expected: "Инкубатор",
				description:
					"Grammar test for accusative case, Инкубатор -> Инкубатор",
			},
			{
				word: "песчаник",
				grammarForm: "prepositional",
	...
	QUnit.test("List to text test", function (assert) {
		...
		assert.strictEqual(
			mw.language.listToText(["a", "b"]),
			"a and b",
			"Two items"
		);
		assert.strictEqual(
			mw.language.listToText(["a", "b", "c"]),
			"a, b and c",
			"More than two items"
		);
	});
	...
	QUnit.test("mw.language.bcp47", function (assert) {
		...
		bcp47Tests.forEach(function (data) {
			...
			assert.strictEqual(
				mw.language.bcp47(input.toLowerCase()),
				expected
			);
			assert.strictEqual(
				mw.language.bcp47(input.toUpperCase()),
				expected
			);
		});
	});
