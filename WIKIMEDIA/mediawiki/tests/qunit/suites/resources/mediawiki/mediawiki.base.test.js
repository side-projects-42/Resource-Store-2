(function () {
	...
	QUnit.test(
		...
		function (assert) {
			...
			assert.verifySteps(["early x", "early y", "early z", "late z"]);
		}
	);
	...
	QUnit.test(
		...
		function (assert) {
			...
			mw.hook("test.memory").add(function (data) {
				assert.step(data + "2");
			});

			mw.hook("test.memory").remove(handler).add(handler);
			assert.verifySteps(
				["A1", "A2", "A1"],
	...
	QUnit.test("mw.hook - Unregistering handler.", function (assert) {
		...
		mw.hook("test.remove").add(handler).remove(handler).fire("A");

		assert.verifySteps(
			[],
			"The handler was unregistered before the fired event."
		);

		mw.hook("test.remove").add(handler).remove(handler);

		assert.verifySteps(
			["A"],
			"The handler runs with memory event before it is unregistered."
		);
	});
