...
(function () {
	...
	QUnit.test("Initializing filter item", function (assert) {
		...
		item = new mw.rcfilters.dm.FilterItem("filter1", group1, {
			conflicts: {
				group2__conflict1: {
					group: "group2",
					filter: "group2__conflict1",
				},
				group2__conflict2: {
					group: "group2",
					filter: "group2__conflict2",
				},
				group2__conflict3: {
					group: "group2",
					filter: "group2__conflict3",
				},
			},
		});
		assert.deepEqual(
		...
		assert.strictEqual(
			item.existsInConflicts(
				new mw.rcfilters.dm.FilterItem("conflict1", group2)
			),
			true,
		...
		assert.strictEqual(
			item.existsInConflicts(
				new mw.rcfilters.dm.FilterItem("conflict10", group1)
			),
			false,
	...
	QUnit.test("get/set boolean value", function (assert) {
		var group = new mw.rcfilters.dm.FilterGroup("group1", {
				type: "boolean",
			}),
