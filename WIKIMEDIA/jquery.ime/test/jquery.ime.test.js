(function ($) {
  ...
  QUnit.test("Initialization tests", function (assert) {
    ...
    assert.strictEqual(
      typeof $input.data("ime"),
      "undefined",
      "ime not initialized before calling ime()"
    );
    ...
    assert.strictEqual(
      typeof inputIME,
      "object",
      "ime is defined for a regular <input>"
    );
    ...
    assert.strictEqual(
      $specialPath.data("ime").options.imePath,
      specialPath,
    ...
    assert.strictEqual(
      $readonlyTextarea.data("ime"),
      undefined,
      "ime is not defined for a readonly <textarea>"
    );
    assert.strictEqual(
      $disabledTextarea.data("ime"),
      undefined,
      "ime is not defined for a disabled <textarea>"
    );
    assert.strictEqual(
      $noimeTextarea.data("ime"),
      undefined,
      'ime is not defined for a <textarea> with class "noime"'
    );
  });
  ...
  QUnit.test("Selector tests", function (assert) {
    ...
    assert.strictEqual(
      typeof selector,
      "object",
      "selector on textarea is defined"
    );

    assert.strictEqual(
      textareaIME.isActive(),
      false,
      "selector is not active initially"
    );
    assert.strictEqual(
      textareaIME.getIM(),
      null,
      "inputmethod is not enabled initially"
    );
    ...
    assert.strictEqual(
      textareaIME.isActive(),
      true,
      "selector is active after enabling"
    );
  ...
  QUnit.test("Preferences tests", function (assert) {
    ...
    assert.strictEqual(
      $.ime.preferences.getPreviousLanguages().length,
      1,
      "Hindi added to previous languages"
    );
    // set it again
    $.ime.preferences.setLanguage("hi");
    assert.strictEqual(
      $.ime.preferences.getPreviousLanguages().length,
      1,
      "Hindi not duplicated in previous languages"
    );
    ...
    assert.strictEqual(
      $.ime.preferences.getPreviousLanguages().length,
      2,
      "Kannada added to previous languages"
    );
    $.ime.preferences.setLanguage("hi");
    $.ime.preferences.setIM("hi-inscript");
    assert.strictEqual(
      $.ime.preferences.getIM("hi"),
      "hi-inscript",
      "Hindi Inscript is the preferred IM for Hindi"
    );
    assert.strictEqual(
      $.ime.preferences.getIM("kn"),
      "kn-inscript",
      "Kannada Inscript is the preferred IM for Kannada"
    );
  });
  ...
  QUnit.test("Utility functions tests", function (assert) {
    ...
    assert.strictEqual(
      textareaIME.getLanguage(),
      null,
      "ime language is initially null"
    );
    setLanguageResult = textareaIME.setLanguage("noSuchLanguage");
    assert.strictEqual(
      setLanguageResult,
      false,
      "Setting an invalid language returns false"
    );
    assert.strictEqual(
      textareaIME.getLanguage(),
      null,
      "Language does not change after an invalid setting"
    );
    setLanguageResult = textareaIME.setLanguage("ru");
    assert.strictEqual(
      setLanguageResult,
      true,
      "Setting a valid language returns true"
    );
    assert.strictEqual(
      textareaIME.getLanguage(),
      "ru",
      "Language changed after setting a valid value"
    );
  });

  QUnit.test(
    "Default input method for language without input methods test",
    function (assert) {
      $.ime.preferences.setLanguage("es");
      assert.strictEqual(
        $.ime.preferences.getIM(),
        "system",
        "Use native keyboard is selected by default"
      );
    }
  );
  ...
  $.each($.ime.sources, function (inputmethodId) {
    ...
    testDescription =
      "Input method rules file test for input method " +
  ...
  $.each($.ime.languages, function (languageCode) {
    ...
    QUnit.test(
      "Input method rules test for language " + language.autonym,
      function (assert) {
        var i,
          inputmethod,
  ...
  imeTest = function (options) {
    ...
    QUnit.test(opt.description, function (assert) {
      ...
      ime.load(opt.inputmethod).done(function () {
        ...
        assert.strictEqual(
          imesettingLabel,
          $.ime.sources[opt.inputmethod].name,
