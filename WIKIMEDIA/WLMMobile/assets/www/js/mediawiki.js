    ...
    loader: (function () {
      ...
      function getMarker() {
        ...
        if ($marker) {
          ...
          mw.log(
            'getMarker> No <meta name="ResourceLoaderDynamicStyles"> found, inserting dynamically.'
          );
          $marker = $("<meta>")
            .attr("name", "ResourceLoaderDynamicStyles")
            .appendTo("head");
          return $marker;
        }
      }
        ...
        for (n = 0; n < len; n += 1) {
          if ($.inArray(deps[n], resolved) === -1) {
            if ($.inArray(deps[n], unresolved) !== -1) {
              throw new Error(
                "Circular reference detected: " + module + " -> " + deps[n]
              );
            }
      ...
      function addScript(src, callback) {
        ...
        if (ready || !blocking) {
          ...
          document.write(
            mw.html.element("script", { type: "text/javascript", src: src }, "")
          );
          if ($.isFunction(callback)) {
      ...
      function execute(module, callback) {
        ...
        if (registry[module] === undefined) {
          ...
          throw new Error(
            "Module has not been requested from the server yet: " + module
          );
        ...
        try {
          ...
          if (window.console && typeof window.console.log === "function") {
            console.log(
              "mw.loader::execute> Exception thrown by " +
                module +
                ": " +
                e.message
            );
      ...
      return {
        ...
        implement: function (module, script, style, msgs) {
          ...
          if (!$.isFunction(script) && !$.isArray(script)) {
            throw new Error(
              "script must be a function or an array, not a " + typeof script
            );
          }
          ...
          if (
            ...
            registry[module].script !== undefined
          ) {
        ...
        using: function (dependencies, ready, error) {
          ...
          if (tod !== "object" && tod !== "string") {
            throw new Error(
              "dependencies must be a string or an array, not a " + tod
            );
          }
          ...
          // If any dependencies have errors execute error immediately
            ...
            if ($.isFunction(error)) {
              error(
                new Error('one or more dependencies have state "error"'),
                dependencies
              );
            }
          }
        ...
        load: function (modules, type) {
          ...
          if (typeof modules !== "object" && typeof modules !== "string") {
            throw new Error(
              "modules must be a string or an array, not a " + typeof modules
            );
          }
          ...
          if (typeof modules === "string") {
            ...
            if (/^(https?:)?\/\//.test(modules)) {
              ...
              // Unknown type
              throw new Error(
                "invalid type for external url, must be text/css or text/javascript. not " +
                  type
              );
            }
