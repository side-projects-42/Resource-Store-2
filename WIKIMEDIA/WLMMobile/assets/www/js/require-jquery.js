...
(function (undefined) {
  ...
  function configurePackageDir(pkgs, currentPackages, dir) {
    ...
    for (i = 0; (pkgObj = currentPackages[i]); i++) {
      ...
      if (
        dir &&
        (!location ||
          (location.indexOf("/") !== 0 && location.indexOf(":") === -1))
      ) {
  ...
  function newContext(contextName) {
    ...
    function callPlugin(pluginName, depManager) {
      var map = depManager.map,
        ...
        name = map.name,
        plugin =
          plugins[pluginName] || (plugins[pluginName] = defined[pluginName]),
        load;
      ...
      if (fullName in defined) {
        ...
        plugin.load(
          name,
          makeRequire(map.parentMap, true, function (deps, cb) {
            var moduleDeps = [],
              i,
              dep,
              depMap;
            ...
            depManager.moduleDeps = (depManager.moduleDeps || []).concat(
              moduleDeps
            );
    ...
    function getManager(map, shouldQueue) {
      var fullName = map.fullName,
        prefix = map.prefix,
        plugin = prefix
          ? plugins[prefix] || (plugins[prefix] = defined[prefix])
          : null,
        manager,
        created,
        pluginManager,
      ...
      if (!manager) {
        ...
        manager = {
          ...
          id:
            (prefix && !plugin ? managerCounter++ + "__p@:" : "") +
              ...
              deps[i] =
                {
                  id: name,
                  uri: name
                    ? context.nameToUrl(name, null, relModuleMap)
                    : undefined,
          ...
          if (!loaded[prop]) {
            if (expired) {
              ...
              if (prop.indexOf("!") === -1) {
                ...
                moduleDeps =
                  managerCallbacks[prop] && managerCallbacks[prop].moduleDeps;
                if (moduleDeps) {
  ...
  req.attach = function (
    ...
    if (isBrowser) {
      ...
      node.type =
        type || (context && context.config.scriptType) || "text/javascript";
      node.charset = "utf-8";
  ...
  if (isBrowser) {
    ...
    for (
      globalI = scripts.length - 1;
      globalI > -1 && (script = scripts[globalI]);
      globalI--
    ) {
