...
L.Util = {
  extend: function (/*Object*/ dest) /*-> Object*/ {
...
L.Mixin.Events = {
  ...
  removeEventListener: function (
    ...
    for (
      var i = 0, events = this._leaflet_events, len = events[type].length;
      i < len;
      i++
    ) {
      if (
...
L.LatLngBounds = L.Class.extend({
  initialize: function (southWest, northEast) {
    ...
    var latlngs =
      southWest instanceof Array ? southWest : [southWest, northEast];
...
L.Map = L.Class.extend({
  ...
  getBoundsZoom: function (bounds, inside) {
    // (LatLngBounds)
  ...
  _initPanes: function () {
    ...
    this._objectsPane = panes.objectsPane = this._createPane(
...
L.Projection.Mercator = {
  ...
  unproject: function (/*Point*/ point, /*Boolean*/ unbounded) /*-> LatLng*/ {
    ...
    while (Math.abs(dphi) > tol && --i > 0) {
      ...
      dphi =
        Math.PI / 2 -
        2 * Math.atan(ts * Math.pow((1.0 - con) / (1.0 + con), 0.5 * eccent)) -
        phi;
      phi += dphi;
    }
    ...
    var subdomains = this.options.subdomains,
      s =
        this.options.subdomains[
          (tilePoint.x + tilePoint.y) % subdomains.length
        ];
...
L.TileLayer.WMS = L.TileLayer.extend({
  ...
  onAdd: function (map) {
    var projectionKey =
      parseFloat(this.wmsParams.version) >= 1.3 ? "crs" : "srs";
...
L.Popup = L.Class.extend({
  ...
  _updatePosition: function () {
    ...
    this._containerLeft =
      pos.x - Math.round(this._containerWidth / 2) + this.options.offset.x;
  ...
  _adjustPan: function () {
    ...
    if (containerPos.x + this._containerWidth > size.x) {
      adjustOffset.x =
        containerPos.x + this._containerWidth - size.x + padding.x;
    }
    if (containerPos.y < 0) {
...
L.Circle = L.Path.extend({
  ...
  projectLatlngs: function () {
    var equatorLength = 40075017,
      hLength =
        equatorLength * Math.cos(L.LatLng.DEG_TO_RAD * this._latlng.lat);

    var lngSpan = (this._mRadius / hLength) * 360,
      latlng2 = new L.LatLng(
        this._latlng.lat,
        this._latlng.lng - lngSpan,
        true
      ),
...
L.DomEvent = {
  ...
  getMousePosition: function (e, container) {
    var x = e.pageX
      ...
      pos = new L.Point(x, y);
    return container
      ? pos.subtract(L.DomUtil.getViewportOffset(container))
      : pos;
  },
...
L.Map.ScrollWheelZoom = L.Handler.extend({
  ...
  _performZoom: function () {
    ...
    var newCenter = this._getCenterForScrollWheelZoom(
        this._lastMousePos,
        delta
      ),
      newZoom = zoom + delta;
  ...
  _getCenterForScrollWheelZoom: function (mousePos, delta) {
    var map = this._map,
      ...
      centerOffset = mousePos
        .subtract(viewHalf)
        .multiplyBy(1 - Math.pow(2, -delta)),
...
L.Map.TouchZoom = L.Handler.extend({
  ...
  _onTouchStart: function (e) {
    ...
    var p1 = this._map.mouseEventToLayerPoint(e.touches[0]),
      ...
      viewCenter = this._map.containerPointToLayerPoint(
  ...
  _onTouchEnd: function (e) {
    ...
    var oldZoom = this._map.getZoom(),
      ...
      centerPoint = this._map
        .getPixelOrigin()
        .add(this._startCenter)
        .add(centerOffset),
    ...
    this._map._runAnimation(
      center,
      zoom,
      finalScale / this._scale,
      this._startCenter.add(centerOffset)
    );
  },
});
...
L.Map.include({
  ...
  _initControlPos: function () {
    var corners = (this._controlCorners = {}),
      ...
      controlContainer = L.DomUtil.create(
        "div",
        classPart + "control-container",
        this._container
      );
    ...
    corners.topLeft = L.DomUtil.create(
      "div",
      top + " " + left,
      controlContainer
    );
    corners.topRight = L.DomUtil.create(
      "div",
      top + " " + right,
      controlContainer
    );
    corners.bottomLeft = L.DomUtil.create(
      "div",
      bottom + " " + left,
      controlContainer
    );
    ...
    : {
        ...
        _offsetIsWithinView: function (offset, multiplyFactor) {
          var m = multiplyFactor || 1,
            size = this.getSize();
          return (
            Math.abs(offset.x) <= size.x * m && Math.abs(offset.y) <= size.y * m
          );
        },
      }
);
  ...
  !L.DomUtil.TRANSITION
    ...
    : {
        _zoomToIfCenterInView: function (center, zoom, centerOffset) {
          ...
          var centerPoint = this.containerPointToLayerPoint(
        ...
        _runAnimation: function (center, zoom, scale, origin) {
          ...
          if (L.Browser.android) {
            this._tileBg.style[transform + "Origin"] =
