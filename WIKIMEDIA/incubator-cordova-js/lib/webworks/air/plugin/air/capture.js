...
function capture(action, win, fail) {
  var onCaptured = blackberry.events.registerEventHandler(
      "onCaptured",
      function (path) {
    ...
    request = new blackberry.transport.RemoteFunctionCall(
      "blackberry/media/camera/" + action
    );
...
module.exports = {
  ...
  captureAudio: function (args, win, fail) {
    ...
    if (args[0].limit > 0 && args[0].duration) {
      ...
      id =
        id.getDay().toString() +
        id.getHours().toString() +
        id.getSeconds().toString() +
        id.getMilliseconds().toString() +
        id.getYear().toString();

      var fileName =
        blackberry.io.dir.appDirs.shared.music.path + "/audio" + id + ".wav";
