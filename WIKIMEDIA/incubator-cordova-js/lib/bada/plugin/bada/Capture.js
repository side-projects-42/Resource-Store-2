module.exports = {
  ...
  captureImage: function (success, fail, options) {
    ...
    var appcontrolobject = Osp.App.AppManager.findAppControl(
      "osp.appcontrol.provider.camera",
      "osp.appcontrol.operation.capture"
    );

    if (appcontrolobject) {
      appcontrolobject.start(
        dataList,
        function (cbtype, appControlId, operationId, resultList) {
          var i;
          var pluginResult = [];
          if (cbtype === "onAppControlCompleted") {
            ...
            var error = {
              message: "An error occured while capturing image",
              code: 0,
            };
            fail(error);
          }
  ...
  captureVideo: function (success, fail, options) {
    ...
    var appcontrolobject = Osp.App.AppManager.findAppControl(
      "osp.appcontrol.provider.camera",
      "osp.appcontrol.operation.record"
    );
