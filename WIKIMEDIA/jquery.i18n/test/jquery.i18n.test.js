(function ($) {
  ...
  QUnit.test("Message parse HTML", function (assert) {
    ...
    assert.strictEqual(
      $fixture.i18n().html(),
      "X&lt;i&gt;Y&lt;/i&gt;",
      "Content of fixture localized with HTML encoded"
    );
    $fixture.data("i18n", "[html]x");
    assert.strictEqual(
      $fixture.i18n().html(),
      "X<i>Y</i>",
      "Content of fixture localized with HTML as is"
    );
  });
  ...
  QUnit.test("Message parse attributes", function (assert) {
    ...
    assert.strictEqual(
      $fixture.i18n().attr("title"),
      "title X",
      "Content of title attribute localized"
    );
  });
  ...
  QUnit.test("Message parse tests (en)", function (assert) {
    ...
    $.when(i18n.load("i18n/test-en.json", "en")).always(function () {
    ...
    pluralAndGenderMessage =
      "$1 has $2 {{plural:$2|kitten|kittens}}. " +
      "{{gender:$3|He|She}} loves to play with {{plural:$2|it|them}}.";
    pluralAndGenderMessageWithLessParaMS =
      "$1 has $2 {{plural:$2|kitten}}. " +
  ...
  $.when($.i18n().load("i18n/test-ml.json", "ml")).then(function () {
    QUnit.test("Message parse tests (ml, fr)", function (assert) {
      ...
      pluralAndGenderMessage =
        ...
        "{{gender:$3|He|She}} loves to play with {{plural:$2|it|them}}.";
      assert.strictEqual(
        $.i18n(pluralAndGenderMessage, "മീര", 1, "female"),
        "മീരയ്ക്കു് ഒരു പൂച്ചക്കുട്ടി ഉണ്ടു്. അവൾ അതുമായി കളിക്കാൻ ഇഷ്ടപ്പെടുന്നു.",
        "Plural and gender test - female, singular"
      );
      assert.strictEqual(
        $.i18n(pluralAndGenderMessage, "ഹാരി", 2, "male"),
        "ഹാരിയ്ക്കു് 2 പൂച്ചക്കുട്ടികൾ ഉണ്ടു്. അവൻ അവറ്റകളുമായി കളിക്കാൻ ഇഷ്ടപ്പെടുന്നു.",
        "Plural and gender test - male, plural"
      );
      i18n.locale = "fr";
      assert.strictEqual(
        $.i18n("Restaurer $1 modification{{PLURAL:$1||s}}", 1),
        "Restaurer 1 modification",
        "Plural rule parsed correctly for French"
      );
      assert.strictEqual(
        $.i18n("Restaurer $1 modification{{PLURAL:$1||s}}", 2),
        "Restaurer 2 modifications",
        "Plural rule parsed correctly for French"
      );
    });
  });
  ...
  QUnit.test("Message load tests", function (assert) {
    ...
    assert.strictEqual(
      $.i18n("message_3"),
      "THREE",
      "Messages loaded for locale test-en"
    );
  ...
  QUnit.test("Locale Fallback test", function (assert) {
    ...
    assert.strictEqual(
      $.i18n("hindi"),
      "हिन्दी",
      "Message got from fallback locale - Hindi"
    );
    assert.strictEqual(
      $.i18n("this-does-not-exist"),
      "This does not exist",
      "Message got from fallback locale - English"
    );
    i18n.locale = "tt-cyrl";
    i18n.load(
      {
  ...
  QUnit.test("Message parse plural tests for Arabic", function (assert) {
    ...
    assert.strictEqual(
      $.i18n("{{plural:$1|zero|one|two|few|many|other}}", 1),
      "one",
      "Arabic plural test for one"
    );
    assert.strictEqual(
      $.i18n("{{plural:$1|zero|one|two|few|many|other}}", "٠"),
      "zero",
      "Arabic plural test for arabic digit zero"
    );
    assert.strictEqual(
      $.i18n("{{plural:$1|zero|one|two|few|many|other}}", 2),
      "two",
      "Arabic plural test for two"
    );
    assert.strictEqual(
      $.i18n("{{plural:$1|zero|one|two|few|many|other}}", 3),
      "few",
      "Arabic plural test for few"
    );
    assert.strictEqual(
      $.i18n("{{plural:$1|zero|one|two|few|many|other}}", "٨"),
      "few",
      "Arabic plural test for few"
    );
    assert.strictEqual(
      $.i18n("{{plural:$1|zero|one|two|few|many|other}}", 9),
      "few",
      "Arabic plural test for few"
    );
    assert.strictEqual(
      $.i18n("{{plural:$1|zero|one|two|few|many|other}}", 110),
      "few",
      "Arabic plural test for few"
    );
    assert.strictEqual(
      $.i18n("{{plural:$1|zero|one|two|few|many|other}}", 11),
      "many",
      "Arabic plural test for many"
    );
    assert.strictEqual(
      $.i18n("{{plural:$1|zero|one|two|few|many|other}}", 15),
      "many",
      "Arabic plural test for many"
    );
    assert.strictEqual(
      $.i18n("{{plural:$1|zero|one|two|few|many|other}}", 99),
      "many",
      "Arabic plural test for many"
    );
    assert.strictEqual(
      $.i18n("{{plural:$1|zero|one|two|few|many|other}}", 9999),
      "many",
      "Arabic plural test for many"
    );
    assert.strictEqual(
      $.i18n("{{plural:$1|zero|one|two|few|many|other}}", 100),
      "other",
      "Arabic plural test for other"
    );
    assert.strictEqual(
      $.i18n("{{plural:$1|zero|one|two|few|many|other}}", 102),
      "other",
      "Arabic plural test for other"
    );
    assert.strictEqual(
      $.i18n("{{plural:$1|zero|one|two|few|many|other}}", 1000),
      "other",
      "Arabic plural test for other"
    );
    assert.strictEqual(
      $.i18n("{{plural:$1|zero|one|two|few|many|other}}", 1.7),
      "other",
      "Arabic decimal plural test for one"
    );
    assert.strictEqual(
      $.i18n("{{plural:$1|zero|one|two|few|many|other}}", "٠١٢٣٤٥٦٧٨٩"),
      "many",
  ...
  QUnit.test("Message parse plural tests for German", function (assert) {
    var i18n,
      key = "$1 {{plural:$1|Meldung|Meldungen}}";
    $.i18n();
    i18n = $(document).data("i18n");
    // Switch to locale locally
    i18n.locale = "de";
    ...
    assert.strictEqual(
      $.i18n(key, 1),
      "1 Meldung",
      "German plural test for one"
    );
    assert.strictEqual(
      $.i18n(key, 2),
      "2 Meldungen",
      "German plural test for few"
    );
    assert.strictEqual(
      $.i18n(key, 5),
      "5 Meldungen",
      "German plural test for many"
    );
    assert.strictEqual(
      $.i18n(key, 21),
      "21 Meldungen",
  ...
  QUnit.test("Message parse plural tests for Polish", function (assert) {
    var i18n,
      key = "$1 {{plural:$1|błąd|błędy|błędów}}";
    $.i18n();
    i18n = $(document).data("i18n");
    // Switch to locale locally
    i18n.locale = "pl";
    ...
    assert.strictEqual(
      $.i18n(key, 5),
      "5 błędów",
      "Polish plural test for many"
    );
    assert.strictEqual(
      $.i18n(key, 21),
      "21 błędów",
      "Polish plural test for 21 (many)"
    );
    assert.strictEqual(
      $.i18n(key, 22),
      "22 błędy",
      "Polish plural test for 22 (few)"
    );
  });
  ...
  QUnit.test("Test explicit plural forms", function (assert) {
    $.i18n();
    assert.strictEqual(
      $.i18n.languages["default"].convertPlural(0, [
        "0=Explicit Zero",
        "Singular",
        "Plural",
      ]),
    ...
    assert.strictEqual(
      $.i18n.languages["default"].convertPlural(1, [
        "0=Explicit Zero",
        "Singular",
        "Plural",
        "1=Explicit One",
    ...
    assert.strictEqual(
      $.i18n.languages["default"].convertPlural(3, [
        "0=Explicit Zero",
        "1=Explicit One",
        "Singular",
        "Plural",
    ...
    assert.strictEqual(
      $.i18n("Found {{PLURAL:$1|$1 results|1=$1 result}}", 1),
      "Found 1 result",
      "Plural message with explicit plural forms, plural form contains placeholder."
    );
  });
  ...
  QUnit.test("Digit transform table tests (Persian)", function (assert) {
    ...
    i18n.locale = "fa";
    assert.strictEqual(
      $.i18n.parser.language.convertNumber("8"),
      "۸",
      "Persian transform of 8"
    );
    assert.strictEqual(
      $.i18n.parser.language.convertNumber("8", true),
      8,
      "Persian transform of 8"
    );
    assert.strictEqual(
      $.i18n.parser.language.convertNumber("0123456789"),
      "۰۱۲۳۴۵۶۷۸۹",
      "Persian transform of 0123456789"
    );
    assert.strictEqual(
      $.i18n.parser.language.convertNumber("۰۱۲۳۴۵۶۷۸۹", true),
      123456789,
      "Persian transform of 0123456789"
    );
  });
  ...
  QUnit.test("Digit transform table tests (Arabic)", function (assert) {
    ...
    assert.strictEqual(
      $.i18n.parser.language.convertNumber("٠١٢٣٤٥٦٧٨٩", true),
      123456789,
      "Arabic transform of 0123456789"
    );
  });
  ...
  QUnit.test("Support fallback loading from folder tests", function (assert) {
    ...
    $.when(i18n.load("i18n/fallback", "uk")).then(function () {
      i18n.locale = "uk";
  ...
  QUnit.test("Support combined file loading from URL", function (assert) {
    ...
    $.when(i18n.load("i18n/test-combined.json")).then(function () {
      i18n.locale = "de";
  ...
  function grammarTest(langCode, test) {
    QUnit.test("Grammar test for language " + langCode, function (assert) {
      ...
      for (i = 0; i < test.length; i++) {
        grammarMessage =
          "{{GRAMMAR:" + test[i].grammarForm + "|" + test[i].word + "}}";
        assert.strictEqual(
