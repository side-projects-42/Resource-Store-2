...
(function (global, WinJS, undefined) {
  ...
  function drainQueue(queue) {
    ...
    return dispatchEvent(queue[0]).then(drainNext, drainError);
  }
  ...
  var ListenerType = WinJS.Class.mix(
    WinJS.Class.define(null),
  ...
  if (window.Windows && Windows.UI) {
    ...
    function suspendingHandler(e) {
      WinJS.Application.queueEvent({
        type: checkpointET,
        _deferral: e.suspendingOperation.getDeferral(),
      });
    }
...
(function (WinJS, undefined) {
  ...
  var go = function (distance, fromStack, toStack, delta) {
    ...
    if (distance > 0) {
      return raiseBeforeNavigate(fromStack[fromStack.length - distance]).then(
        function goBeforeCompleted(cancel) {
          if (!cancel) {
            ...
            return raiseNavigating(delta)
              .then(raiseNavigated, function (err) {
  ...
  WinJS.Namespace.defineWithParent(WinJS, "Navigation", {
    ...
    navigate: function (location, initialState) {
      ...
      return raiseBeforeNavigate(proposed).then(function navBeforeCompleted(
        cancel
      ) {
        if (!cancel) {
    ...
    var IOHelper = WinJS.Class.define(
      ...
      },
      {
        exists: function (fileName) {
          ...
          /// </returns>
          return this.folder.getFileAsync(fileName).then(
            function () {
              return true;
            },
            function () {
        ...
        remove: function (fileName) {
          ...
          /// </returns>
          var that = this;
          return that.folder.getFileAsync(fileName).then(
            function (fileItem) {
        ...
        writeText: function (fileName, str) {
          ...
          return that.folder
            .createFileAsync(
              fileName,
              sto.CreationCollisionOption.replaceExisting
            )
        ...
        readText: function (fileName, def) {
          ...
          return that.folder.getFileAsync(fileName).then(function (fileItem) {
            return fileItem
              .openAsync(sto.FileAccessMode.read)
              .then(function (randomAccessStream) {
                var reader = new Windows.Storage.Streams.DataReader(
    ...
    _loadState: function (e) {
      ...
      if (
        e.previousExecutionState !==
          0 /* ApplicationExecutionState.NotRunning */ &&
        e.previousExecutionState !==
          1 /* ApplicationExecutionState.CurrentlyRunning */
      ) {
        return app.local
          .readText("_sessionState.json", "{}")
          .then(function (str) {
