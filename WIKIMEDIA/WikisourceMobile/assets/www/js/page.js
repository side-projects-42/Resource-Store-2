(function () {
        ...
        this.lang,
        {
          dataFilter: function (text) {
            ...
            $.each(newPage.sections, function (index, section) {
              if (
                section.id !== 0 ||
                typeof section.references !== "undefined"
              ) {
  ...
  Page.prototype.getCanonicalUrl = function () {
    return (
      app.baseUrlForLanguage(this.lang) +
      "/wiki/" +
      encodeURIComponent(this.title.replace(/ /g, "_"))
    );
  };
  ...
  Page.prototype.getAPIUrl = function () {
    return (
      app.baseUrlForLanguage(this.lang) +
      "/w/api.php?format=json&action=mobileview&page=" +
      encodeURIComponent(this.title) +
      "&redirects=1&prop=sections|text&sections=all&sectionprop=level|line&noheadings=true"
    );
  };
