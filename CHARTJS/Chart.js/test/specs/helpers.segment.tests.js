...
describe("helpers.segments", function () {
  describe("_boundSegment", function () {
    ...
    it("should find segment when starting before line", function () {
      expect(
        _boundSegment(segment, points, { property: "x", start: 5, end: 15 })
      ).toEqual([{ start: 0, end: 1, loop: false, style: undefined }]);
    });

    it("should find segment directly on point", function () {
      expect(
        _boundSegment(segment, points, { property: "x", start: 10, end: 10 })
      ).toEqual([{ start: 0, end: 0, loop: false, style: undefined }]);
    });

    it("should find segment from range between points", function () {
      expect(
        _boundSegment(segment, points, { property: "x", start: 11, end: 14 })
      ).toEqual([{ start: 0, end: 1, loop: false, style: undefined }]);
    });

    it("should find segment from point between points", function () {
      expect(
        _boundSegment(segment, points, { property: "x", start: 22, end: 22 })
      ).toEqual([{ start: 1, end: 2, loop: false, style: undefined }]);
    });

    it("should find whole segment", function () {
      expect(
        _boundSegment(segment, points, { property: "x", start: 0, end: 50 })
      ).toEqual([{ start: 0, end: 2, loop: false, style: undefined }]);
    });
    ...
    it("should find segment from after the line", function () {
      expect(
        _boundSegment(segment, points, { property: "x", start: 25, end: 35 })
      ).toEqual([{ start: 1, end: 2, loop: false, style: undefined }]);
    });
    ...
    it("should find multiple segments", function () {
      ...
      expect(
        _boundSegment({ start: 0, end: 4, loop: false }, points2, {
          property: "y",
          start: 60,
          end: 60,
        })
    ...
    it("should find correct segments when there are multiple points with same property value", function () {
      ...
      expect(
        _boundSegment({ start: 0, end: 6, loop: false }, repeatedPoints, {
          property: "x",
          start: 1,
          end: 1.1,
        })
      ...
      ).toEqual([{ start: 2, end: 4, loop: false, style: undefined }]);
      expect(
        _boundSegment({ start: 0, end: 6, loop: false }, repeatedPoints, {
          property: "x",
          start: 2,
      ...
      ).toEqual([{ start: 2, end: 6, loop: false, style: undefined }]);
      expect(
        _boundSegment({ start: 0, end: 6, loop: false }, repeatedPoints, {
          property: "x",
          start: 0,
      ...
      ).toEqual([{ start: 0, end: 6, loop: false, style: undefined }]);
    });
  });
});
