...
import {
  ...
  a1 as _angleBetween,
  a2 as _updateBezierControlPoints,
  a3 as _computeSegments,
  a4 as _boundSegments,
  a5 as _coordsAnimated,
  a6 as _steppedInterpolation,
  a7 as _bezierInterpolation,
  a8 as _pointInLine,
  a9 as _steppedLineTo,
  aa as _bezierCurveTo,
  ab as drawPoint,
  ac as toTRBL,
  ad as toTRBLCorners,
  ae as _normalizeAngle,
  af as _boundSegment,
  ag as getRtlAdapter,
  ah as _alignStartEnd,
  ai as overrideTextDirection,
  aj as restoreTextDirection,
  ak as _toLeftRightCenter,
  al as distanceBetweenPoints,
  am as toFontString,
  an as _setMinAndMaxByKey,
  ao as _decimalPlaces,
  ap as almostEquals,
  aq as almostWhole,
  ar as _longestText,
  as as _filterBetween,
  at as _lookup,
...
const optionKeys = (optionNames) =>
  isArray(optionNames) ? optionNames : Object.keys(optionNames);
  ...
  parse(start, count) {
    ...
    if (me._parsing === false) {
      ...
      const isNotInOrderComparedToPrev = () =>
        isNaN(cur[iAxis]) || (prev && cur[iAxis] < prev[iAxis]);
      for (i = 0; i < count; ++i) {
...
class BarController extends DatasetController {
  ...
  _calculateBarIndexPixels(index, ruler, options) {
    const me = this;
    const stackCount = me.chart.options.skipNull
...
class PolarAreaController extends DatasetController {
  ...
  updateElements(arcs, start, count, mode) {
    ...
    for (i = start; i < start + count; i++) {
      ...
      let outerRadius = this.chart.getDataVisibility(i)
...
class Scale extends Element {
  ...
  _computeGridLineItems(chartArea) {
    ...
    for (i = 0; i < ticksLength; ++i) {
      ...
      const tickBorderDashOffset = resolve(
        [gridLines.tickBorderDashOffset, borderDashOffset],
        context,
        i
      );
  ...
  _computeLabelItems(chartArea) {
    ...
    for (i = 0, ilen = ticks.length; i < ilen; ++i) {
      ...
      const strokeColor = resolve(
        [optionTicks.textStrokeColor],
        me.getContext(i),
        i
      );
      const strokeWidth = resolve(
        [optionTicks.textStrokeWidth],
        me.getContext(i),
        i
      );
      if (isHorizontal) {
...
class PluginService {
  ...
  _createDescriptors(chart, all) {
    ...
    return options === false && !all
      ? []
      : createDescriptors(plugins, options, all);
  }
...
function includePluginDefaults(options) {
  ...
  options.plugins.title =
    options.plugins.title !== false &&
    merge(Object.create(null), [defaults.plugins.title, options.plugins.title]);
...
class Chart {
  ...
  update(mode) {
    ...
    if (
      me.notifyPlugins("beforeUpdate", { mode, cancellable: true }) === false
    ) {
      return;
    }
  ...
  _updateDatasets(mode) {
    ...
    if (
      me.notifyPlugins("beforeDatasetsUpdate", { mode, cancellable: true }) ===
      false
    ) {
      return;
    }
  ...
  _drawDatasets() {
    const me = this;
    if (
      me.notifyPlugins("beforeDatasetsDraw", { cancellable: true }) === false
    ) {
      return;
    }
...
function clipArc(ctx, element) {
  const { startAngle, endAngle, pixelMargin, x, y, outerRadius, innerRadius } =
    element;
...
class LineElement extends Element {
  ...
  draw(ctx, chartArea, start, count) {
    ...
    ctx.restore();
    if (
      _coordsAnimated(points[0]) ||
      _coordsAnimated(points[points.length - 1])
    ) {
      this._pointsUpdated = false;
      this._path = undefined;
    }
