...
describe("URLRewriteStream", function () {
  ...
  describe("pipe()", function () {
    it("should replace URLs when there are more than one URL in the file", function (done) {
      ...
      var resultStream = fs
        .createReadStream("./tests/fixtures/before.css")
        .pipe(stream);
      ...
      streamString(resultStream, function (err, result) {
        ...
        assert.equal(
          result,
          fs.readFileSync("./tests/fixtures/after.css", "utf8")
        );
        done();
      });
    });
    ...
    it("should replace URLs when there are two URLs on the same line", function (done) {
      ...
      var resultStream = fs
        .createReadStream("./tests/fixtures/minified-before.css")
        .pipe(stream);
      ...
      streamString(resultStream, function (err, result) {
        ...
        assert.equal(
          result,
          fs.readFileSync("./tests/fixtures/minified-after.css", "utf8")
        );
        done();
      });
    });
    ...
    it("should emit an error event if there's a transform function error", function (done) {
      ...
      fs.createReadStream("./tests/fixtures/minified-before.css").pipe(stream);
