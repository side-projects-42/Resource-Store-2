  ...
  function edit(options) {
    ...
    if (!parent) {
      if (options.className) {
        var parents = (options.document || document).getElementsByClassName(
          options.className
        );
        if (parents) {
    ...
    if (!options.readonly && options.contentassist) {
      contentAssistFactory = {
        createContentAssistMode: function (editor) {
          contentAssist = new mContentAssist.ContentAssist(
            editor.getTextView()
          );
          var contentAssistWidget = new mContentAssist.ContentAssistWidget(
            contentAssist
          );
          return new mContentAssist.ContentAssistMode(
            contentAssist,
            contentAssistWidget
          );
        },
    ...
    var syntaxHighlighter = {
      ...
      highlight: function (lang, editor) {
        ...
        if (lang) {
          ...
          this.styler = new mTextStyler.TextStyler(
            textView,
            lang,
            annotationModel
          );
          editor.setFoldingRulerVisible(
            options.showFoldingRuler === undefined || options.showFoldingRuler
          );
        }
      },
    };

    var keyBindingFactory = function (
      editor,
      keyModeStack,
      undoStack,
      contentAssist
      ...
      var codeBindings = new mEditorFeatures.SourceCodeActions(
        editor,
        undoStack,
        contentAssist
      );
    ...
    editor.setLineNumberRulerVisible(
      options.showLinesRuler === undefined || options.showLinesRuler
    );
    editor.setAnnotationRulerVisible(
      options.showAnnotationRuler === undefined || options.showFoldingRuler
    );
    editor.setOverviewRulerVisible(
      options.showOverviewRuler === undefined || options.showOverviewRuler
    );
    editor.setFoldingRulerVisible(
      options.showFoldingRuler === undefined || options.showFoldingRuler
    );
    ...
    if (contentAssist) {
      ...
      var jsTemplateContentAssistProvider =
        new mJSContentAssist.EsprimaJavaScriptContentAssistProvider();
    ...
    editor.removeAllErrorMarkers = function () {
      var annotationModel = editor.getAnnotationModel();
      annotationModel.removeAnnotations(
        mAnnotations.AnnotationType.ANNOTATION_WARNING
      );
    };
