...
define("plugins/esprima/esprimaJsContentAssist", [
  "plugins/esprima/esprimaVisitor",
  "plugins/esprima/types",
  "plugins/esprima/proposalUtils",
  ...
  function calculateFunctionProposal(name, type, offset) {
    var paramsOffset = mTypes.findReturnTypeEnd(type),
      paramsStr,
      params;
    ...
    for (var p = 0; p < plen; p++) {
      ...
      positions.push({
        offset: offset + completion.length + 1,
        length: argName.length,
      });
      completion += argName;
    }
    completion += ")";
  ...
  function inRange(offset, range, includeEdge) {
    return (
      range[0] <= offset &&
      (includeEdge ? range[1] >= offset : range[1] > offset)
    );
  }
  /**
  ...
  function afterDot(offset, memberExpr, contents) {
    ...
    if (
      !inRange(offset - 1, memberExpr.range) ||
  ...
  function shouldVisit(root, offset, prefix, contents) {
    ...
    var findParent = function (node, parents, isInitialVisit) {
      ...
      if (!isInitialVisit) {
        ...
        if (
          node.type === "Program" ||
          (node.type === "BlockStatement" && inRange(offset, node.range))
        ) {
          throw "done";
        }
      ...
      if (
        (node.range && inRange(offset - 1, node.range)) ||
        node.type === "Program"
      ) {
    ...
    if (parents && parents.length) {
      ...
      for (var i = 0; i < parents.length; i++) {
        if (
          (parents[i].type === "FunctionDeclaration" ||
            parents[i].type === "FunctionExpression") &&
      ...
      if (parent.type === "MemberExpression") {
        if (parent.property && inRange(offset - 1, parent.property.range)) {
          ...
          inRange(offset - 1, parent.range) &&
          afterDot(offset, parent, contents)
        ) {
        ...
        (!parent.init || isBefore(offset, parent.init.range))
      ) {
  ...
  function inferencer(node, env) {
    ...
    switch (type) {
      ...
      case "ObjectExpression":
        ...
        for (i = 0; i < node.properties.length; i++) {
          ...
          if (property.key && property.key.name) {
            ...
            var docComment = findAssociatedCommentBlock(
              property.key,
              env.comments
            );
            ...
            if (
              ...
              property.value.extras.cname =
                env.getQualifiedName() + property.key.name;
      ...
      case "FunctionExpression":
        ...
        var functionTypeName =
          (isConstuctor ? "*" : "?") + newTypeName + ":" + paramSigs.join(",");
        if (isConstuctor) {
      ...
      case "AssignmentExpression":
        ...
        if (
          rightMost &&
          (rightMost.type === "Identifier" || rightMost.type === "Literal")
        ) {
          if (!rightMost.extras) {
            rightMost.extras = {};
          }
      ...
      case "MemberExpression":
        if (node.property) {
          if (
            !node.computed || // like this: foo.at
            (node.computed &&
              node.property.type === "Literal" &&
              typeof node.property.value === "string")
          ) {
    ...
    if (node === env.defer) {
      node.extras.associatedComment = findAssociatedCommentBlock(
        node,
        env.comments
      );
  ...
  function inferencerPostOp(node, env) {
    ...
    switch (type) {
      ...
      case "MemberExpression":
        ...
        if (
          mTypes.isArrayType(node.object.extras.inferredType) &&
          node.computed
        ) {
          // inferred type of expression is the type of the dereferenced array
          node.extras.inferredType = mTypes.extractArrayParameterType(
            node.object.extras.inferredType
          );
        } else if (
          node.computed &&
          node.property &&
          ...
          node.extras.inferredType = node.property
            ? node.property.extras.inferredType
            : node.object.extras.inferredType;
        }
        break;
      case "CallExpression":
      ...
      case "NewExpression":
        ...
        node.extras.inferredType = mTypes.extractReturnType(
          node.callee.extras.inferredType
        );
        break;
      case "ObjectExpression":
            ...
            if (name) {
              ...
              if (name === kvps[i].value.name) {
                ...
                if (
                  def &&
                  def.range &&
                  mTypes.isFunctionOrConstructor(def.typeName)
                ) {
                  range = def.range;
                }
              }
      ...
      case "FunctionExpression":
        ...
        if (node.body) {
          ...
          if (node.body.extras.isConstructor) {
            ...
            env.addOrSetVariable(
              mTypes.extractReturnType(node.extras.inferredType),
              node.extras.target,
              node.extras.inferredType,
              fnameRange
            );
          } else {
            ...
            if (!node.extras.jsdocReturn) {
              ...
              if (
                returnStatement &&
                returnStatement.extras &&
                returnStatement.extras.inferredType
              ) {
                returnType = returnStatement.extras.inferredType;
              } else {
                returnType = "undefined";
              }
              node.extras.inferredType = updateReturnType(
                node.extras.inferredType,
                returnType
              );
            }
      ...
      case "Property":
        node.extras.inferredType = node.key.extras.inferredType =
          node.value.extras.inferredType;
        break;
        ...
        if (
          ...
          if (node.left.computed) {
            ...
            if (
              rightMost &&
              !(
                rightMost.type === "Identifier" &&
                rightMost.name === "prototype"
              )
            ) {
      ...
      case "Identifier":
        ...
        if (newTypeName && !node.extras.isDecl) {
          ...
          if (
            !inRange(env.offset, node.range, true) &&
            !isReserverdWord(name)
          ) {
            ...
            if (node.extras.target) {
              ...
              env.addVariable(
                name,
                node.extras.target,
                env.newFleetingObject(),
                node.range
              );
            } else {
              // add as a global variable
              node.extras.inferredType = env.addOrSetGlobalVariable(
                name,
                null,
                node.range
              ).typeName;
      ...
      case "Literal":
        if (node.extras.target && typeof node.value === "string") {
          ...
          node.extras.inferredType =
            oftype[0].toUpperCase() + oftype.substring(1, oftype.length);
        }
        break;
      ...
      case "ArrayExpression":
        ...
        if (node.elements) {
          for (i = 0; i < node.elements.length; i++) {
            if (node.elements[i]) {
              node.extras.inferredType = mTypes.parameterizeArray(
                node.elements[i].extras.inferredType
              );
            }
          }
        }
  ...
  function addLintGlobals(env, lintOptions) {
    ...
    if (comments) {
      for (i = 0; i < comments.length; i++) {
        ...
        if (
          comments[i].type === "Block" &&
          comments[i].value.substring(0, "global".length) === "global"
        ) {
  ...
  function leftTypeIsMoreGeneral(leftTypeName, rightTypeName, env) {
    function isEmpty(generatedTypeName) {
      if (generatedTypeName === "Object" || generatedTypeName === "undefined") {
        ...
        leftTypeName.substring(0, mTypes.GEN_NAME.length) !== mTypes.GEN_NAME
      ) {
        return false;
      }
    ...
    if (leftNum === 0) {
      ...
      return (
        rightTypeName !== "undefined" &&
        rightTypeName !== "Object" &&
        !isEmpty(rightTypeName)
      );
    } else {
      return false;
    }
  ...
  function createEnvironment(options) {
    var buffer = options.buffer,
      ...
      globalObjName = options.globalObjName;
    if (!offset) {
    ...
    return {
      ...
      scope: function (target) {
        ...
        if (target && target.extras.inferredType) {
          ...
          if (
            mTypes.isFunctionOrConstructor(inferredType) &&
            !mTypes.isPrototype(inferredType)
          ) {
      ...
      addVariable: function (name, target, typeName, range, docRange) {
        ...
        if (!type.$$isBuiltin) {
          ...
          if (
            typeContainsProperty(type, name) &&
            leftTypeIsMoreGeneral(typeName, type[name].typeName, this)
          ) {
            // do nuthin
          } else {
            type[name] = new mTypes.Definition(
      ...
      addOrSetGlobalVariable: function (name, typeName, range, docRange) {
        ...
        return this.addOrSetVariable(
          name,
          // mock an ast node with a global type
          { extras: { inferredType: this.globalTypeName() } },
          typeName,
          range,
          docRange
        );
      ...
      addOrSetVariable: function (name, target, typeName, range, docRange) {
        ...
        var current = this._allTypes[targetType],
          found = false;
      ...
      removeVariable: function (name, target) {
        ...
        if (
          !type.$$isBuiltin &&
          type[name] &&
          !(type[name] && !type.hasOwnProperty(name))
        ) {
          delete type[name];
        }
      },
      ...
      createConstructor: function (constructorName, rawTypeName) {
        ...
        this._allTypes[constructorName].$$proto = new mTypes.Definition(
          flobj,
          null,
          this.uidj
        );
        this._allTypes[rawTypeName].$$proto = new mTypes.Definition(
          constructorName,
          null,
          this.uid
        );
  ...
  function createInferredProposals(
    targetTypeName,
    env,
    completionKind,
    prefix,
    ...
    var prop,
      ...
      propType,
      res,
      type = env.findType(targetTypeName),
      proto = type.$$proto;
    if (!relevance) {
      relevance = 100;
    }
    ...
    for (prop in type) {
      if (
        type.hasOwnProperty(prop) ||
        (!protoIsObject && realProto.hasOwnProperty(prop))
      ) {
        ...
        if (proposalUtils.looselyMatches(prefix, propName)) {
          ...
          if (first === "?" || first === "*") {
            // we have a function
            res = calculateFunctionProposal(
              propName,
              propType,
              replaceStart - 1
            );
            var funcDesc =
              res.completion + " : " + mTypes.createReadableType(propType, env);
            proposals["$" + propName] = {
  ...
  function createNoninferredProposals(
    environment,
    prefix,
    replaceStart,
    proposals
    ...
    function forType(type) {
      for (var prop in type) {
        if (isInterestingProperty(type, prop)) {
          ...
          var first = propType.charAt(0);
          if (first === "?" || first === "*") {
            var res = calculateFunctionProposal(
              prop,
              propType,
              replaceStart - 1
            );
    ...
    if (currentType) {
      for (var prop in currentType) {
        if (currentType.hasOwnProperty(prop) && prop !== "$$isBuiltin") {
          ...
          while (
            mTypes.isFunctionOrConstructor(propType) ||
            mTypes.isArrayType(propType)
          ) {
  ...
  function filterTypes(environment, kind, moduleTypeName) {
    ...
    if (
      mTypes.isFunctionOrConstructor(moduleTypeName) ||
      mTypes.isArrayType(moduleTypeName)
    ) {
      var retType = moduleTypeName;
      while (
        mTypes.isFunctionOrConstructor(retType) ||
        mTypes.isArrayType(retType)
      ) {
  ...
  function findGlobalObject(comments, lintOptions) {
    for (var i = 0; i < comments.length; i++) {
      ...
      if (
        comment.type === "Block" &&
        (comment.value.substring(0, "jslint".length) === "jslint" ||
  ...
  function filterAndSortProposals(proposalsObj) {
    ...
    while (
      i < proposals.length &&
      proposals[i].description.indexOf("---") === 0
    ) {
      toRemove[i] = true;
      i++;
    }
    ...
    while (i < proposals.length) {
      if (
        proposals[i].description.indexOf("---") === 0 &&
        proposals[i - 1].description.indexOf("---") === 0
      ) {
        toRemove[i] = true;
      }
      i++;
      ...
      try {
        ...
        if (completionKind) {
          var environment = createEnvironment({
            ...
            globalObjName: findGlobalObject(root.comments, this.lintOptions),
          ...
          if (environment.defer) {
            ...
            environment.deferredComments = extractDocComments(
              environment.comments,
              environment.defer.range
            );
          }
    ...
    _internalFindDefinition: function (buffer, offset, findName) {
      ...
      var findIdentifier = function (node) {
        if (
          ...
          inRange(offset, node.range, true)
        ) {
          toLookFor = node;
          // cut visit short
          throw "done";
      ...
      var target = this._doVisit(root, environment);
      var lookupName =
        toLookFor.type === "Identifier" ? toLookFor.name : "this";
      var maybeType = environment.lookupName(
        lookupName,
        toLookFor.extras.target || target,
        false,
        true
      );
      if (maybeType) {
        var hover =
          mTypes.styleAsProperty(lookupName, findName) +
          " : " +
          mTypes.createReadableType(
            maybeType.typeName,
            environment,
            true,
            0,
    ...
    computeSummary: function (buffer, fileName) {
      ...
      if (environment.amdModule) {
        ...
        if (args && args.length > 0) {
          modType = mTypes.extractReturnType(
            args[args.length - 1].extras.inferredType
          );
        } else {
          modType = "Object";
        }
      ...
      if (
        mTypes.isFunctionOrConstructor(modType) ||
        environment.findType(modType).$$isBuiltin
      ) {
  ...
  return {
    EsprimaJavaScriptContentAssistProvider:
      EsprimaJavaScriptContentAssistProvider,
  };
});
