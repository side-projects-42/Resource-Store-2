...
define("plugins/esprima/types", ["plugins/esprima/proposalUtils"], function (
  proposalUtils,
  scriptedLogger
) {
  /**
  ...
  Window.prototype = {
    ...
    openDialog: new Definition(
      "?Window:strUrl,strWindowName,strWindowFeatures,[args]"
    ),
    ...
    removeEventListener: new Definition(
      "?undefined:type,listener,[useCapture]"
    ),
    ...
    Option: new Definition(
      "*HTMLOptionElement:[text].[value],[defaultSelected],[selected]"
    ),
  ...
  Types.prototype = {
    ...
    Document: {
      ...
      getElementsByTagNameNS: new Definition(
        "?HTMLCollection:namespace,localName"
      ),
      ...
      createProcessingInstruction: new Definition(
      ...
      createNodeIterator: new Definition(
        "?NodeIterator:root,[whatToShow],[filter]"
      ),
      createTreeWalker: new Definition(
    ...
    DOMImplementation: {
      ...
      createDocumentType: new Definition(
        "?DocumentType:qualifiedName,publicId,systemId"
      ),
      createDocument: new Definition(
...
*/
  ...
  return {
    ...
    isPrototype: function (typeName) {
      return (
        typeName.charAt(0) === "*" &&
        typeName.substr(-protoLength, protoLength) === "~proto"
      );
    },
    ...
    extractArrayParameterType: function (arrayType) {
      if (
        arrayType.substr(0, "Array.<".length) === "Array.<" &&
        arrayType.substr(-1, 1) === ">"
      ) {
    ...
    parseJSDocComment: function (docComment) {
      ...
      if (docComment) {
        ...
        try {
          var rawresult = doctrine.parse("/*" + commentText + "*/", {
            unwrap: true,
            tags: ["param", "type", "return"],
          });
    ...
    convertJsDocType: function (jsdocType, env) {
      ...
      switch (jsdocType.type) {
        ...
        case "RestType":
          return (
            "Array.<" + this.convertJsDocType(jsdocType.expression, env) + ">"
          );
        case "ArrayType":
          if (jsdocType.elements && jsdocType.elements.length > 0) {
            ...
            return (
              "Array.<" +
              this.convertJsDocType(jsdocType.elements[0], env) +
              ">"
            );
          }
          return "Array";
          ...
          if (jsdocType.params) {
            for (i = 0; i < jsdocType.params.length; i++) {
              ...
              if (jsdocType.params[i].expression) {
                paramType =
                  "/" +
                  this.convertJsDocType(jsdocType.params[i].expression, env);
              }
        ...
        case "TypeApplication":
          ...
          if (
            expr === "Array" &&
            jsdocType.applications &&
            jsdocType.applications.length > 0
          ) {
            // only parameterize arrays not handling objects yet
            return (
              "Array.<" +
              this.convertJsDocType(jsdocType.applications[0], env) +
              ">"
            );
          }
          return expr;
    ...
    styleAsProperty: function (prop, useHtml) {
      return useHtml
        ? '<span style="color: blue;font-weight:bold;">' + prop + "</span>"
        : prop;
    },
    styleAsType: function (type, useHtml) {
      return useHtml
        ? '<span style="color: green;font-weight:bold;">' + type + "</span>"
        : type;
    },
      ...
      if (first === "?" || first === "*") {
        ...
        if (useFunctionSig) {
          ...
          args.split(",").forEach(function (arg) {
            ...
            if (argSig) {
              var sig = self.createReadableType(
                argSig,
                env,
                true,
                depth + 1,
          ...
          return (
            ...
            (useHtml
              ? ")<br/>" +
                proposalUtils.repeatChar("&nbsp;&nbsp;", depth + 1) +
                "⇒ "
              : ") ⇒ ") +
          ...
          if (type.hasOwnProperty(val) && val !== "$$proto") {
            ...
            props.push(
              (useHtml
                ...
                : "") +
                this.styleAsProperty(val, useHtml) +
                ":" +
                name
            );
          }
