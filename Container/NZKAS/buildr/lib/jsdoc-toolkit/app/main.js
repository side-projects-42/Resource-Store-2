...
function main() {
  ...
  if (JSDOC.opt.T) {
    ...
    if (JSDOC.opt.Z) {
      LOG.warn(
        "So you want to see the data structure, eh? This might hang if you have circular refs..."
      );
      ...
      if (typeof JSDOC.opt.t != "undefined") {
        try {
          ...
          if (!publish) {
            LOG.warn(
              "No publish() function is defined in that template so nothing to do."
            );
          } else {
          ...
          LOG.warn(
            "Sorry, that doesn't seem to be a valid template: " +
              JSDOC.opt.t +
              "publish.js : " +
              e
          );
