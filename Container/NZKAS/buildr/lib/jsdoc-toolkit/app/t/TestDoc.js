...
TestDoc.run = function (src) {
  ...
  for (
    var start = -1, end = 0;
    (start = src.indexOf("/*t:", end)) > end;
    start = end
  ) {
    run(src.substring(start + 4, (end = src.indexOf("*/", start))));
  }
};
...
TestDoc.Result.prototype.toString = function () {
  if (typeof this.verdict == "boolean") {
    return (
      (this.verdict ? "ok" : "not ok") +
      " " +
      ++TestDoc.report.counter +
      " - " +
      this.message
    );
  }
    ...
    print(
      "Failed " +
        ...
        (TestDoc.plans == 0
          ? 0
          : Math.round(
              (TestDoc.passes / (TestDoc.passes + TestDoc.fails)) * 10000
            ) / 100) +
        "% okay. Planned to run " +
        TestDoc.plans +
        ", did run " +
    ...
    return function (url) {
      ...
      if (httpRequest.readyState == 4) {
        if (httpRequest.status >= 400) {
          throw (
            "The HTTP Request returned an error code: " + httpRequest.status
          );
        }
      }
