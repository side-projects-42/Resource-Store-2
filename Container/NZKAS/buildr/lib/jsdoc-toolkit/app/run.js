...
if (arguments[arguments.length - 1].match(/^-j=(.+)/)) {
  if (RegExp.$1.charAt(0) == SYS.slash || RegExp.$1.charAt(1) == ":") {
    // absolute path to here
    SYS.pwd = new FilePath(RegExp.$1).toDir().toString();
  } else {
    // relative path to here
    SYS.pwd = new FilePath(SYS.userDir + SYS.slash + RegExp.$1)
      .toDir()
      .toString();
  }
  arguments.pop();
...
IO = {
  ...
  copyFile: function (
    ...
    var bis = new Packages.java.io.BufferedInputStream(
      new Packages.java.io.FileInputStream(inFile),
      4096
    );
    var bos = new Packages.java.io.BufferedOutputStream(
      new Packages.java.io.FileOutputStream(outFile),
      4096
    );
    var theChar;
  ...
  ls: function (/**string*/ dir, /**number*/ recurse, _allFiles, _path) {
    ...
    for (var f = 0; f < files.length; f++) {
      ...
      if (new File(_path.join(SYS.slash) + SYS.slash + file).list()) {
        // it's a directory
        _path.push(file);
        ...
        _allFiles.push(
          (_path.join(SYS.slash) + SYS.slash + file).replace(
            SYS.slash + SYS.slash,
            SYS.slash
          )
        );
