  ...
  for (var p = this._index.first(); p; p = this._index.next()) {
    ...
    for (var i = 0; i < borrows.length; i++) {
      if (/#$/.test(borrows[i].alias)) {
        LOG.warn(
          "Attempted to borrow entire instance of " +
            borrows[i].alias +
      ...
      if (
        borrowAsName.length > symbol.alias.length &&
        borrowAsName.indexOf(symbol.alias) == 0
      ) {
    ...
    if (symbol.memberOf) {
      switch (symbol.memberOf.charAt(symbol.memberOf.length - 1)) {
        ...
        case "-":
          ...
          break;
        default:
          // memberOf ends in none of the above
          symbol.isStatic = true;
          break;
      }
    }
      ...
      if (!container) {
        if (JSDOC.Lang.isBuiltin(symbol.memberOf))
          ...
          LOG.warn(
            "Trying to document " +
              symbol.name +
              " as a member of undocumented symbol " +
              symbol.memberOf +
              "."
          );
        }
...
JSDOC.SymbolSet.prototype.walk = function (symbol) {
  ...
  for (var i = 0; i < augments.length; i++) {
    ...
    if (!contributer && JSDOC.Lang.isBuiltin("" + augments[i])) {
      contributer = new JSDOC.Symbol(
        "_global_." + augments[i],
        [],
        augments[i],
        new JSDOC.DocComment("Built in.")
      );
