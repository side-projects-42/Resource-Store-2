...
JSDOC.Walker.prototype.init = function () {
  ...
  var globalSymbol = new JSDOC.Symbol(
    "_global_",
    [],
    "GLOBAL",
    ...
    if (this.token.popNamescope) {
      ...
      if (symbol.is("FUNCTION")) {
        if (
          ...
          symbol.comment.getTag("function").length == 0
        ) {
          symbol.isa = "OBJECT";
        }
      }
...
JSDOC.Walker.prototype.step = function () {
  if (this.token.is("JSDOC")) {
    ...
    if (doc.getTag("lends").length > 0) {
      ...
      if (matching) matching.popNamescope = name;
        ...
        LOG.warn(
          "Mismatched } character. Can't parse code in file " +
            symbol.srcFile +
            "."
        );
      ...
      if (doc.getTag("memberOf").length > 0) {
        virtualName = (doc.getTag("memberOf")[0] + "." + virtualName).replace(
          /([#.])\./,
          "$1"
        );
      ...
      this.lastDoc = null;
      return true;
    } else if (doc.getTag("overview").length > 0) {
      // it's a file overview
  ...
  } else if (!JSDOC.Parser.conf.ignoreCode) {
    // it's code
    if (this.token.is("NAME")) {
      ...
      // foo = function() {}
        ...
        if (
          doc &&
          (constructs = doc.getTag("constructs")) &&
          constructs.length
        ) {
      ...
      // foo = new function() {} or foo = (function() {}
        ...
        (this.ts.look(2).is("NEW") || this.ts.look(2).is("LEFT_PAREN")) &&
      ...
      // foo(...)
        ...
        if (typeof JSDOC.PluginManager != "undefined") {
          ...
          if (functionCall.doc) {
            this.ts.insertAhead(
              new JSDOC.Token(functionCall.doc, "COMM", "JSDOC")
            );
          }
        }
      }
      this.lastDoc = null;
    } else if (this.token.is("FUNCTION")) {
    ...
    if (symbol.alias.match(/(^.*)[#.-][^#.-]+/)) {
      ...
      if (!parent) {
        if (JSDOC.Lang.isBuiltin(parentName))
          ...
          if (symbol.alias.indexOf("$anonymous") < 0)
            // these will be ignored eventually
            LOG.warn(
              "Trying to document " +
                symbol.alias +
      ...
      if (parent)
        name =
          parentName + (parent.is("CONSTRUCTOR") ? "#" : ".") + nameFragment;
    } else {
      parent = this.namescope.last(1);
      name =
        parent.alias + (parent.is("CONSTRUCTOR") ? "#" : ".") + nameFragment;
    }
  }
