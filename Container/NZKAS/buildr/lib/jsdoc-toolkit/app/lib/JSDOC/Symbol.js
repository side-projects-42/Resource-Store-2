...
JSDOC.Symbol.prototype.__defineGetter__("name", function () {
  return this._name;
});
JSDOC.Symbol.prototype.__defineSetter__("params", function (v) {
  for (var i = 0, l = v.length; i < l; i++) {
    if (v[i].constructor != JSDOC.DocTag) {
      ...
      this._params[i] = new JSDOC.DocTag(
        "param" + (v[i].type ? " {" + v[i].type + "}" : "") + " " + v[i].name
      );
    } else {
      this._params[i] = v[i];
    }
...
JSDOC.Symbol.prototype.__defineGetter__("params", function () {
  return this._params;
});
...
JSDOC.Symbol.prototype.setTags = function () {
  ...
  if (params.length) {
    ...
    if (thisParams.length == 0) {
      // none exist yet, so just bung all these user-defined params straight in
      this.params = params;
    } else {
      // need to overlay these user-defined params on to existing parser-defined params
  ...
  if (properties.length) {
    ...
    for (var i = 0; i < properties.length; i++) {
      var property = new JSDOC.Symbol(
        this.alias + "#" + properties[i].name,
        [],
        "OBJECT",
        new JSDOC.DocComment("/**" + properties[i].desc + "*/")
      );
  ...
  if (inherits.length) {
    for (var i = 0; i < inherits.length; i++) {
      if (
        /^\s*([a-z$0-9_.#:-]+)(?:\s+as\s+([a-z$0-9_.#:-]+))?/i.test(
          inherits[i].desc
        )
      ) {
        ...
        if (inAs.indexOf(inAlias) != 0) {
          ...
          var joiner = ".";
          if (
            this.alias.charAt(this.alias.length - 1) == "#" ||
            inAs.charAt(0) == "#"
          ) {
            joiner = "";
          }
...
JSDOC.Symbol.prototype.setType = function (
  /**String*/ comment,
  /**Boolean*/ overwrite
) {
...
JSDOC.Symbol.prototype.inherit = function (symbol) {
  if (!this.hasMember(symbol.name) && !symbol.isInner) {
    if (symbol.is("FUNCTION")) this.methods.push(symbol);
    else if (symbol.is("OBJECT")) this.properties.push(symbol);
  }
};
