...
JSDOC.Parser = {
  ...
  addSymbol: function (symbol) {
    ...
    if (JSDOC.opt.S) {
      ...
      if (/^exports\./.test(symbol.alias)) {
        ...
        if (!JSDOC.Parser.secureModules[fileNS]) {
          ...
          nsSymbol.desc = (
            JSDOC.Parser.symbols.getSymbol(symbol.srcFile) || { desc: "" }
          ).desc;
    ...
    if (JSDOC.Parser.symbols.hasSymbol(symbol.alias)) {
      ...
      if (oldSymbol.comment.isUserComment) {
        ...
        if (symbol.comment.isUserComment) {
          // old and new are both documented
          LOG.warn(
            "The symbol '" + symbol.alias + "' is documented more than once."
          );
          return;
        } else {
          // old is documented but new isn't
          return;
        }
      }
    }
    ...
    if (
      JSDOC.Parser.conf.treatUnderscoredAsPrivate &&
      symbol.name.match(/[.#-]_[^.#-]+$/)
    ) {
...
JSDOC.Parser.parse = function (/**JSDOC.TokenStream*/ ts, /**String*/ srcFile) {
  ...
  for (
    var p = JSDOC.Parser.symbols._index.first();
    p;
    p = JSDOC.Parser.symbols._index.next()
  ) {
    var symbol = p.value;
