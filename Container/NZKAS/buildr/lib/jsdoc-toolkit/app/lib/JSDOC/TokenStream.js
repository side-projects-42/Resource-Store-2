...
/**
	@type JSDOC.Token
*/
JSDOC.TokenStream.prototype.look = function (
  /**Number*/ n,
  ...
  if (considerWhitespace == true) {
    ...
    while (true) {
      ...
      if (
        i != this.cursor &&
        (this.tokens[i] === undefined || this.tokens[i].is("WHIT"))
      ) {
        if (n < 0) i--;
        else i++;
        continue;
...
/**
	@type JSDOC.Token[]
*/
JSDOC.TokenStream.prototype.balance = function (
  /**String*/ start,
  /**String*/ stop
) {
...
JSDOC.TokenStream.prototype.getMatchingToken = function (
  /**String*/ start,
  /**String*/ stop
) {
  var depth = 0;
