  ...
  describe("Parsing", function () {
    describe("Replacement", function () {
      it("should accept string only input", function () {
        var mf = new MessageFormat("en");
        expect(mf.parse("This is a string").program.statements[0].val).to.be(
          "This is a string"
        );
        expect(mf.parse("☺☺☺☺").program.statements[0].val).to.be("☺☺☺☺");
        expect(mf.parse("This is \n a string").program.statements[0].val).to.be(
          "This is \n a string"
        );
        expect(mf.parse("中国话不用彁字。").program.statements[0].val).to.be(
          "中国话不用彁字。"
        );
      });
      ...
      it("should allow you to escape { and } characters", function () {
        ...
        expect(mf.parse("test\\}").program.statements[0].val).to.eql("test}");
        expect(mf.parse("\\{test\\}").program.statements[0].val).to.eql(
          "{test}"
        );
      });
      ...
      it("should gracefully handle quotes (since it ends up in a JS String)", function () {
        var mf = new MessageFormat("en");
        expect(
          mf.parse('This is a dbl quote: "').program.statements[0].val
        ).to.eql('This is a dbl quote: "');
        expect(
          mf.parse("This is a single quote: '").program.statements[0].val
        ).to.eql("This is a single quote: '");
      });
      ...
      it("should maintain exact strings - not affected by variables", function () {
        ...
        expect(mf.parse("x { test}").program.statements[0].val).to.be("x ");
        expect(
          mf.parse("x{test} x ").program.statements[1].statements[1].val
        ).to.be(" x ");
        expect(mf.parse("x\n{test}\n").program.statements[0].val).to.be("x\n");
        expect(
          mf.parse("x\n{test}\n").program.statements[1].statements[1].val
        ).to.be("\n");
      });
      ...
      it("should handle extended character literals", function () {
        ...
        expect(mf.parse("☺{test}").program.statements[0].val).to.be("☺");
        expect(
          mf.parse("中{test}中国话不用彁字。").program.statements[1]
            .statements[1].val
        ).to.be("中国话不用彁字。");
      });
      ...
      it("shouldn't matter if it has html or something in it", function () {
        var mf = new MessageFormat("en");
        expect(
          mf.parse('<div class="test">content: {TEST}</div>').program
            .statements[0].val
        ).to.be('<div class="test">content: ');
        expect(
          mf.parse('<div class="test">content: {TEST}</div>').program
            .statements[1].statements[0].argumentIndex
        ).to.be("TEST");
        expect(
          mf.parse('<div class="test">content: {TEST}</div>').program
            .statements[1].statements[1].val
        ).to.be("</div>");
      });

      it("should allow you to use extension keywords for plural formats everywhere except where they go", function () {
        var mf = new MessageFormat("en");
        expect(mf.parse("select select, ").program.statements[0].val).to.eql(
          "select select, "
        );
        expect(
          mf.parse("select offset, offset:1 ").program.statements[0].val
        ).to.eql("select offset, offset:1 ");
        expect(mf.parse("one other, =1 ").program.statements[0].val).to.eql(
          "one other, =1 "
        );
        expect(
          mf.parse("one {select} ").program.statements[1].statements[0]
            .argumentIndex
        ).to.eql("select");
        expect(
          mf.parse("one {plural} ").program.statements[1].statements[0]
            .argumentIndex
        ).to.eql("plural");
      });
    });
    ...
    describe("Selects", function () {
      ...
      it("should be very whitespace agnostic", function () {
        var mf = new MessageFormat("en");
        var firstRes = JSON.stringify(
          mf.parse("{VAR, select, key{a} other{b}}")
        );
        expect(JSON.stringify(mf.parse("{VAR,select,key{a}other{b}}"))).to.eql(
          firstRes
        );
        expect(
          JSON.stringify(
      ...
      it("should allow you to use MessageFormat extension keywords other places, including in select keys", function () {
        ...
        expect(
          mf.parse("x {TEST, select, select{a} other{b} }").program
            .statements[1].statements[0].elementFormat.val.pluralForms[0].key
        ).to.eql("select");
        // use `offset` as a key (since it goes here in a `plural` case)
        expect(
          mf.parse("x {TEST, select, offset{a} other{b} }").program
            .statements[1].statements[0].elementFormat.val.pluralForms[0].key
        ).to.eql("offset");
        // use the exact variable name as a key name
        expect(
          mf.parse("x {TEST, select, TEST{a} other{b} }").program.statements[1]
            .statements[0].elementFormat.val.pluralForms[0].key
        ).to.eql("TEST");
      });
    ...
    describe("Plurals", function () {
      ...
      it("should accept exact values with `=` prefixes", function () {
        var mf = new MessageFormat("en");
        expect(
          mf.parse("{NUM, plural, =0{e1} other{2}}").program.statements[0]
            .statements[0].elementFormat.val.pluralForms[0].key
        ).to.eql(0);
        expect(
          mf.parse("{NUM, plural, =1{e1} other{2}}").program.statements[0]
            .statements[0].elementFormat.val.pluralForms[0].key
        ).to.eql(1);
        expect(
          mf.parse("{NUM, plural, =2{e1} other{2}}").program.statements[0]
            .statements[0].elementFormat.val.pluralForms[0].key
        ).to.eql(2);
        expect(
          mf.parse("{NUM, plural, =1{e1} other{2}}").program.statements[0]
            .statements[0].elementFormat.val.pluralForms[1].key
        ).to.eql("other");
      });
      ...
      it("should accept the 6 official keywords", function () {
        ...
        expect(
          ...
          ).program.statements[0].statements[0].elementFormat.val.pluralForms[0]
            .key
        ).to.eql("zero");
        expect(
          mf.parse(
        ...
        expect(
          mf.parse(
            "{NUM, plural, zero{0} one{1} two{2} few{5} many{100} other{101}}"
          ).program.statements[0].statements[0].elementFormat.val.pluralForms[1]
            .key
        ).to.eql("one");
        expect(
          mf.parse(
            "{NUM, plural, zero{0} one{1} two{2} few{5} many{100} other{101}}"
          ).program.statements[0].statements[0].elementFormat.val.pluralForms[2]
            .key
        ).to.eql("two");
        expect(
          mf.parse(
            "{NUM, plural, zero{0} one{1} two{2} few{5} many{100} other{101}}"
          ).program.statements[0].statements[0].elementFormat.val.pluralForms[3]
            .key
        ).to.eql("few");
        expect(
          mf.parse(
            "{NUM, plural, zero{0} one{1} two{2} few{5} many{100} other{101}}"
          ).program.statements[0].statements[0].elementFormat.val.pluralForms[4]
            .key
        ).to.eql("many");
        expect(
          mf.parse(
            "{NUM, plural, zero{0} one{1} two{2} few{5} many{100} other{101}}"
          ).program.statements[0].statements[0].elementFormat.val.pluralForms[5]
            .key
        ).to.eql("other");
      });
      ...
      it("should be gracious with whitespace", function () {
        var mf = new MessageFormat("en");
        var firstRes = JSON.stringify(
          mf.parse("{NUM, plural, one{1} other{2}}")
        );
        expect(
          JSON.stringify(mf.parse("{ NUM, plural, one{1} other{2} }"))
        ).to.eql(firstRes);
        expect(JSON.stringify(mf.parse("{NUM,plural,one{1}other{2}}"))).to.eql(
          firstRes
        );
        expect(
          JSON.stringify(
      ...
      it("should take an offset", function () {
        ...
        expect(
          mf.parse("{NUM, plural, offset:4 other{a}}").program.statements[0]
            .statements[0].elementFormat.val.offset
        ).to.eql(4);
        expect(
          mf.parse("{NUM,plural,offset:4other{a}}").program.statements[0]
            .statements[0].elementFormat.val.offset
        ).to.eql(4);
      });
    });
    ...
    describe("Nested/Recursive blocks", function () {
      it("should allow a select statement inside of a select statement", function () {
        ...
        expect(
          mf.parse("{NUM1, select, other{{NUM2, select, other{a}}}}").program
        ...
        expect(
          mf.parse(
            ...
            .val.statements[0].statements[0].elementFormat.val.pluralForms[0]
            .val.statements[0].statements[0].elementFormat.val.pluralForms[0]
            .val.statements[0].val
        ).to.eql("b");
      ...
      it("should allow nested plural statements - with and without offsets", function () {
        ...
        expect(function () {
          var a = mf.parse(
            "{NUM1, plural, offset:1 other{{NUM2, plural, other{a}}}}"
          );
        }).to.not.throwError();
        expect(function () {
          var a = mf.parse(
            "{NUM1, plural, other{{NUM2, plural, offset:1 other{a}}}}"
          );
        }).to.not.throwError();
        expect(function () {
          var a = mf.parse(
      ...
      it("should allow nested plural and select statements - with and without offsets", function () {
        ...
        expect(function () {
          var a = mf.parse(
            "{NUM1, select, other{{NUM2, plural, offset:1 other{a}}}}"
          );
        }).to.not.throwError();
        expect(function () {
          var a = mf.parse(
        ...
        expect(
          mf.parse(
            "{NUM1, select, other{{NUM2, plural, offset:1 other{{NUM3, select, other{{NUM4, plural, offset:1 other{c}}}}}}}}"
          ).program.statements[0].statements[0].elementFormat.val.pluralForms[0]
            .val.statements[0].statements[0].elementFormat.val.pluralForms[0]
            .val.statements[0].statements[0].elementFormat.val.pluralForms[0]
            .val.statements[0].statements[0].elementFormat.val.pluralForms[0]
  ...
  describe("Message Formatting", function () {
    describe("Basic API", function () {
      ...
      it("should get escaped brackets all the way out the other end", function () {
        ...
        expect(mf.compile("\\}\\}\\}")()).to.eql("}}}");
        expect(mf.compile("\\{\\{\\{{test}\\}\\}\\}")({ test: 4 })).to.eql(
          "{{{4}}}"
        );
        expect(
          mf.compile("\\{\\{\\{{test, select, other{#}}\\}\\}\\}")({ test: 4 })
        ).to.eql("{{{4}}}");
        expect(
          mf.compile("\\{\\{\\{{test, plural, other{#}}\\}\\}\\}")({ test: 4 })
        ).to.eql("{{{4}}}");
        expect(
          mf.compile("\\{\\{\\{{test, plural, offset:1 other{#}}\\}\\}\\}")({
            test: 4,
          })
        ).to.eql("{{{3}}}");
      });
      ...
      it("can substitute named variables", function () {
        ...
        expect(mf.compile("The var is {VAR}.")({ VAR: 5 })).to.eql(
          "The var is 5."
        );
      });
      ...
      it("can substitute shorthand variables", function () {
        ...
        expect(
          mf.compile("{VAR, select, other{The var is #.}}")({ VAR: 5 })
        ).to.eql("The var is 5.");
        expect(mf.compile("{0, select, other{The var is #.}}")([5])).to.eql(
          "The var is 5."
        );
      });
      ...
      it("should reject number injections of numbers that don't exist", function () {
        ...
        expect(mfunc({ FRIENDS: 0, ENEMIES: 0 })).to.eql(
          "I have 0 friends but 0 enemies."
        );
        expect(function () {
