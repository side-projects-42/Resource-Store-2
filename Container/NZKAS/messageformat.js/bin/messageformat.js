...
(fs = require("fs")),
  ...
  (description = {
    ...
    combine:
      "combines multiple input files to the provided namespace array element",
...
function build(inputdir, options, callback) {
  ...
  fs.readFile(inclFile, function (err, inclStr) {
    ...
    fs.readFile(localeFile, function (err, localeStr) {
      if (err)
        handleError(new Error("locale " + options.locale + " not supported."));
          ...
          fs.stat(path, function (err, stat) {
            ...
            fs.readFile(path, "utf8", function (err, text) {
              ...
              if (options.combine !== undefined) {
                ...
                if (options.verbose)
                  console.log(
                    "Adding to " + options.namespace + '["' + nm + '"]'
                  );
              } else {
                if (options.verbose)
                  console.log(
                    "Building " + options.namespace + '["' + nm + '"]'
                  );
              }

              compiledMessageFormat.push(
                compiler(options, nm, JSON.parse(text))
              );
              cb();
            });
          });
