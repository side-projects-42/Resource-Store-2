...
var commentTemplate = handlebars.compile(
  fs.readFileSync(__dirname + "/comment.hbs", "utf-8")
);
...
function Audit(options) {
  ...
  this.debug = createDebugger(
    "audit:" + options.owner + "/" + options.repo + "-" + options.pr
  );
}

Audit.prototype.ensureCommitData = function () {
  if (
    this.options.headRemote &&
  ...
  return this.repo.getPrCommitData(this.options.pr).then(function (data) {
...
Audit.prototype.audit = function () {
  return this.ensureCommitData().then(this._audit.bind(this));
};
...
Audit.prototype.applyLabels = function (labels, state) {
  ...
  return this.repo.getLabels({ pr: pr }).then(function (labels) {
    var ops = [],
...
Audit.prototype.finishAudit = function (values) {
  var result = values[0],
    ...
    status = {
      ...
      state: result.state,
      url:
        "http://contribute.jquery.org/CLA/status/?" +
