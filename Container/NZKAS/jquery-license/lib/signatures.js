  ...
  result = result
    ...
    .filter(function (entry, index) {
      ...
      if (duplicates.length) {
        duplicates[0].names = unique(
          duplicates[0].names.concat(entry.names)
        ).reverse();
        duplicates[0].errors = unique(
          duplicates[0].errors.concat(entry.errors)
        ).reverse();
        return false;
      }
      return true;
