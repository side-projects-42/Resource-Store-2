...
exports.get = {
  "returns singleton": function (test) {
    ...
    test.equal(
      instance1.name,
      "test-repo",
      "Instance #1 has correct repo name"
    );
    test.equal(
      instance1.owner,
      "testuser",
      "Instance #1 has correct owner name"
    );
    test.equal(
      instance3.name,
      "test-repo-2",
      "Instance #3 has correct repo name"
    );
    test.equal(
      instance3.owner,
      "testuser-2",
      "Instance #3 has correct owner name"
    );
...
exports.auditBranch = {
  ...
  success: function (test) {
    ...
    this.repo.audit = function (options) {
      test.equal(
        options.committish,
        providedOptions.branch,
        "Should audit with provided branch"
      );
      test.strictEqual(
        options.signatures,
        providedOptions.signatures,
        "Should audit with provided signatures"
      );
      test.strictEqual(
        options.exceptions,
        providedOptions.exceptions,
    ...
    this.repo.auditBranch(providedOptions).then(function (result) {
      test.strictEqual(
        result,
        providedResult,
        "Should resolve to provided result"
      );
      test.done();
    });
  },
...
exports.auditPr = {
  ...
  success: function (test) {
    ...
    this.repo.fetch = function (remote, branch) {
      test.equal(
        remote,
        expectedFetches[fetchCalls].remote,
        "Should fetch from correct remote"
      );
      test.equal(
        branch,
        expectedFetches[fetchCalls].branch,
