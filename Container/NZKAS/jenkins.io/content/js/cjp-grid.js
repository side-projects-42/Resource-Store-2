$.fn.cjpGrid = function (data, config) {
  ...
  var $this = this.addClass("bootstrap3").addClass("cjp-grid-box smart-grid");
  var elem = $(this)[0];
  ...
  function _generateCols(row, reset) {
    ...
    if (typeof row === "string") {
      ...
      colIdMap.push(row);
      cols.push(
        $.extend(_makeObj(row), { lostNFound: true, className: "lost-n-found" })
      );
    } else
  ...
  function _makeCellObj(cell, col, key) {
    ...
    if (typeof newVal === "string") {
      ...
      //check for url...
        ...
        newVal.indexOf("www.") === 0 ||
        newVal.indexOf("http://") === 0 ||
        newVal.indexOf("https://") === 0
      ) {
        newCell.type = "url";
        newCell.displayValue = [
          '<a href="',
          newVal,
          '" target="_blank">',
          newVal.replace("http://", "").replace("https://", ""),
          "</a>",
        ].join("");
      }
    }
  ...
  function _$getDomType(childElemName) {
    ...
    if (viewType === "table") {
      if (childElemName === "root")
        return $("<table/>").addClass("grid table cjp-inner-grid");
      else if (childElemName === "group")
        return $("<tbody/>").addClass("group");
      else if (childElemName === "groupTitle")
        return $("<tr/>").addClass("groupTitle group-header");
      else if (childElemName === "row")
        return $("<tr/>").addClass("grid-row item");
      else return $("<td/>").addClass("cell");
    } else if (viewType === "list") {
      if (childElemName === "root")
        return $("<ul/>").addClass("cjp-inner-grid");
      else if (childElemName === "group") return $("<li/>").addClass("group");
      else if (childElemName === "groupTitle")
        return $("<div/>").addClass("groupTitle group-header");
      else if (childElemName === "row")
        return $("<ul/>").addClass("grid-row tile");
      else return $("<li/>").addClass("cell");
    } else if (viewType === "count") {
      if (childElemName === "root")
        return $("<ol/>").addClass("cjp-inner-grid");
      ...
      if (childElemName === "root")
        return $("<div/>").addClass("cjp-inner-grid");
      else if (childElemName === "group") return $("<div/>").addClass("group");
      else if (childElemName === "groupTitle")
        return $("<div/>").addClass("groupTitle group-header");
      else if (childElemName === "row")
        return $("<div/>").addClass("grid-row tile");
  ...
  function _baseRenderer(cell, row, col, customRenderer) {
    ...
    else $cell.html(cell.displayValue);
  ...
  function _drawCell(key, cell, row) {
    cell = cell || {};
    var path = cell.path;
    if (typeof path === "string" && row) cell.value = row.items[path];
  ...
  function _drawRow(key, row) {
    ...
    if (colIdMap.length === 0) _generateCols(row);
    ...
    if (cells.length > 0 || $.isPlainObject(cells)) {
      $.each(cells, function (key, cell) {
        ...
        else unfoundRows.splice(unfoundRows.indexOf(key), 1);
    ...
    var $newRow =
      elem.methods.getViewType() === "count" ? $("<li/>").append($row) : $row;
  ...
  function _drawGroups(groups) {
    ...
    function drawGroup(i, groupObj) {
      ...
      var $tbodyInner =
        elem.methods.getViewType() === "count"
          ? $("<ol/>").appendTo($tbody)
          : $tbody;
  ...
  function _drawToolbar(localConfig) {
    ...
    if ($("#hover-space").length < 1)
      $("body").append('<div id="cbp-hover-space" class="bootstrap3" />');
    ...
    $toolbar = $(
      [
        '<div class="toolbar-box"><div class="toolbar">',
  ...
  function _drawPulldowns(options, config_title) {
    options = options || [];
    var id =
      typeof config_title === "string"
        ? config_title.toLowerCase().replace(/\ /g, "-")
        : config_title.id;
    var title =
      typeof config_title === "string" ? config_title : config_title.title;
    ...
    var $pullDownWrapper = $('<li class="dropdown" />')
      ...
      .append(
        $(
          [
            '<a href="#" class="dropdown btn-sm" data-toggle="dropdown" role="button" aria-expanded="false">',
            title,
    ...
    var $ul = $('<ul class="dropdown-menu" role="menu"/>')
      .append(
        $(
          [
            '<li data-effect="none"><a class="btn-sm" href="#',
            id,
            '=__none__">None</a></li>',
          ].join("")
        )
  ...
  var hashOps = {
    group: function (group) {
      ...
      function makeGrouping(i, item) {
        ...
        } else groupObj = groupedRows[groupKey];
