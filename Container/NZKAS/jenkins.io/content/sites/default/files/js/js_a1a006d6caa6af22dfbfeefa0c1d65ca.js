...
Drupal.behaviors.blockDrag = function (context) {
  ...
  tableDrag.onDrop = function () {
    ...
    if ($(dragObject.rowObject.element).prev("tr").is(".region-message")) {
      ...
      var regionName = regionRow.className.replace(
        /([^ ]+[ ]+)*region-([^ ]+)-message([ ]+[^ ]+)*/,
        "$2"
      );
      var regionField = $(
        "select.block-region-select",
        dragObject.rowObject.element
      );
      var weightField = $("select.block-weight", dragObject.rowObject.element);
      var oldRegionName = weightField[0].className.replace(
        /([^ ]+[ ]+)*block-weight-([^ ]+)([ ]+[^ ]+)*/,
        "$2"
      );
  ...
  $(
    "select.block-region-select:not(.blockregionselect-processed)",
    context
  ).each(function () {
