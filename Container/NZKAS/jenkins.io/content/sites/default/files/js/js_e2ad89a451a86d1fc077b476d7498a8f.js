...
Drupal.behaviors.tableDrag = function (context) {
  for (var base in Drupal.settings.tableDrag) {
    if (!$("#" + base + ".tabledrag-processed", context).size()) {
      ...
      $("#" + base)
        .filter(":not(.tabledrag-processed)")
        .each(function () {
...
Drupal.tableDrag = function (table, tableSettings) {
  ...
  if (this.indentEnabled) {
    ...
    var testCell = $(
      "> tbody > tr.draggable:first td:first, > tr.draggable:first td:first",
      table
    )
      .prepend(indent)
      .prepend(indent);
    this.indentAmount =
      $(".indentation", testCell).get(1).offsetLeft -
      $(".indentation", testCell).get(0).offsetLeft;
...
Drupal.tableDrag.prototype.hideColumns = function () {
  for (var group in this.tableSettings) {
    ...
    if (hidden && cell[0] && cell.css("display") != "none") {
      ...
      var headerIndex =
        $("> td:not(:hidden)", cell.parent()).index(cell.get(0)) + 1;
...
Drupal.tableDrag.prototype.dragRow = function (event, self) {
  if (self.dragObject) {
    ...
    if (self.indentEnabled) {
      ...
      self.dragObject.indentMousePos.x +=
...
Drupal.tableDrag.prototype.updateField = function (changedRow, group) {
  ...
  // Siblings are easy, check previous and next rows.
    ...
    if ($(previousRow).is(".draggable") && $("." + group, previousRow).length) {
      ...
      if (this.indentEnabled) {
        if (
          $(".indentations", nextRow).size() == $(".indentations", changedRow)
        ) {
          sourceRow = nextRow;
        }
      } else {
  ...
  // Go up as many parents as indentations in the changed row.
    ...
    while (
      previousRow.length &&
      $(".indentation", previousRow).length >= this.rowObject.indents
    ) {
  ...
  if (targetElement) {
    ...
    switch (rowSettings.action) {
      case "depth":
        ...
        targetElement.value = $(
          ".indentation",
          $(sourceElement).parents("tr:first")
        ).size();
        break;
      case "match":
        // Update the value.
...
Drupal.tableDrag.prototype.checkScroll = function (cursorY) {
  ...
  var windowHeight = (this.windowHeight =
    ...
    (de.clientHeight && de.clientWidth != 0
      ? de.clientHeight
      : b.offsetHeight));
  var scrollY = (this.scrollY = document.all
    ? !de.scrollTop
      ? b.scrollTop
      : de.scrollTop
    : window.pageYOffset
...
Drupal.tableDrag.prototype.row.prototype.findChildren = function (addClasses) {
  ...
  if (addClasses && rows.length) {
    $(
      ".indentation:nth-child(" + (parentIndentation + 1) + ")",
      rows[rows.length - 1]
...
$(document).ready(function () {
  ...
  if (Drupal && Drupal.settings && Drupal.settings.admin_menu) {
    ...
    if (Drupal.settings.admin_menu.tweak_modules) {
      $(
        "#system-modules fieldset:not(.collapsed), #system-modules-1 fieldset:not(.collapsed)"
      ).addClass("collapsed");
    }
  }
...
Drupal.ahah.prototype.beforeSubmit = function (form_values, element, options) {
  ...
  if (this.progress.type == "bar") {
    ...
    this.progress.element = $(progressBar.element).addClass(
      "ahah-progress ahah-progress-bar"
    );
...
Drupal.ahah.prototype.success = function (response, status) {
  ...
  this.form_target
    ? form.attr("target", this.form_target)
    : form.removeAttr("target");
  this.form_encattr
    ? form.attr("target", this.form_encattr)
    : form.removeAttr("encattr");
...
Drupal.ahah.prototype.error = function (response, uri) {
  ...
  $(this.element)
    .parent("form")
    .attr({ action: this.form_action, target: this.form_target });
  // Remove the progress element.
  if (this.progress.element) {
    $(this.progress.element).remove();
  }
    ...
    $("#" + id).bind("focus", Drupal.settings.datePopup[id], function (e) {
      if (!$(this).hasClass("date-popup-init")) {
        ...
        switch (datePopup.func) {
          case "datepicker":
            $(this).datepicker(datePopup.settings).addClass("date-popup-init");
            $(this).click(function () {
              $(this).focus();
            });
            break;
          ...
          case "timeEntry":
            $(this).timeEntry(datePopup.settings).addClass("date-popup-init");
            $(this).click(function () {
              $(this).focus();
            });
            break;
...
Drupal.behaviors.tableHeader = function (context) {
  ...
  $("table.sticky-enabled thead:not(.tableHeader-processed)", context).each(
    function () {
      // Clone thead so it inherits original jQuery properties.
      var headerClone = $(this)
        .clone(true)
        .insertBefore(this.parentNode)
        .wrap('<table class="sticky-header"></table>')
        .parent()
        .css({
          position: "fixed",
          top: "0px",
...
if (Drupal.jsEnabled) {
  $(document).ready(function () {
    if (
      $("#edit-pathauto-perform-alias").size() &&
...
Drupal.behaviors.autocomplete = function (context) {
  var acdb = [];
  $("input.autocomplete:not(.autocomplete-processed)", context).each(
    function () {
      var uri = this.value;
      if (!acdb[uri]) {
        acdb[uri] = new Drupal.ACDB(uri);
      }
      var input = $("#" + this.id.substr(0, this.id.length - 13)).attr(
        "autocomplete",
        "OFF"
      )[0];
...
Drupal.jsAC.prototype.onkeyup = function (input, e) {
  ...
  switch (e.keyCode) {
    ...
    default:
      // all other keys
      if (input.value.length > 0) this.populatePopup();
...
Drupal.toggleFieldset = function (fieldset) {
  if ($(fieldset).is(".collapsed")) {
    ...
    var content = $("> div:not(.action)", fieldset).slideUp(
      "fast",
      function () {
  ...
  $("fieldset.collapsible > legend:not(.collapse-processed)", context).each(
    function () {
      ...
      $(this)
        ...
        .after(
          $('<div class="fieldset-wrapper"></div>').append(
            fieldset.children(":not(legend):not(.action)")
          )
        )
...
Drupal.behaviors.teaser = function (context) {
  ...
  $("textarea.teaser:not(.teaser-processed)", context).each(function () {
    ...
    var button = $(
      '<div class="teaser-button-wrapper"><input type="button" class="teaser-button" /></div>'
    );
    ...
    if (text.length >= 2) {
      ...
      $(teaser).attr("disabled", "");
      $("input", button)
        .val(Drupal.t("Join summary"))
        .toggle(join_teaser, split_teaser);
    } else {
      $("input", button)
        .val(Drupal.t("Split summary at cursor"))
        .toggle(split_teaser, join_teaser);
    ...
    if (
      ...
      teaser.is(".form-textarea:not(.textarea-processed)")
    ) {
...
Drupal.behaviors.textarea = function (context) {
  $("textarea.resizable:not(.textarea-processed)", context).each(function () {
    ...
    $(this)
      ...
      .parent()
      .append($('<div class="grippie"></div>').mousedown(startDrag));

    var grippie = $("div.grippie", $(this).parent())[0];
    grippie.style.marginRight =
      grippie.offsetWidth - $(this)[0].offsetWidth + "px";
