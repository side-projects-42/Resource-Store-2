...
FBL.ns(function () {
  with (FBL) {
    ...
    var cache = Cc["@mozilla.org/network/cache-service;1"].getService(
        Ci.nsICacheService
      ),
      prefs = Cc["@mozilla.org/preferences-service;1"].getService(
        Ci.nsIPrefBranch2
      ),
      PrefService = Cc["@mozilla.org/preferences-service;1"].getService(
        Ci.nsIPrefService
      );
    ...
    function chromeToPath(aPath) {
      if (!aPath || !/^chrome:/.test(aPath)) return urlToPath(aPath);
      ...
      if (/^file:/.test(rv)) rv = urlToPath(rv);
      else rv = urlToPath("file://" + rv);
    ...
    Firebug.FireUnitModule = extend(Firebug.Module, {
      ...
      initialize: function () {
        ...
        testTimeout = Firebug.getPref(
          Firebug.prefDomain,
          "fireunit.testTimeout"
        );
      },
      ...
      watchWindow: function (context, win) {
        ...
        if (FBTrace.DBG_FIREUNIT)
          FBTrace.sysout(
            "fireunit.FireUnitModule.watchWindow: fireunit initialized for: " +
              win.location.href
          );
            ...
            function () {
              if (win.fireunit.testTimeout) {
                testTimeoutID = setTimeout(function () {
                  if (FBTrace.DBG_FIREUNIT)
                    FBTrace.sysout(
                      "fireunit.testTimeout TEST FAILED: " + win.location
                    );

                  win.fireunit.ok(
                    false,
                    $FU_STR("fireunit.label.Timeout") +
                      " (" +
                      formatTime(win.fireunit.testTimeout) +
                      "): " +
                      win.location
                  );
      ...
      addStyleSheets: function (panel) {
        this.addStyleSheet(
          panel.document,
          "chrome://fireunit/skin/tabView.css",
          "tabViewCss"
        );
        this.addStyleSheet(
          panel.document,
      ...
      var fireunit = {
        ...
        forceHttp: function () {
          ...
          if (canServer(win) && win.location.protocol !== "http:") {
            ...
            if (FBTrace.DBG_FIREUNIT)
              FBTrace.sysout(
                "fireunit.forceHttp server directory registered: " +
                  dir.path +
                  " => " +
                  basePath
              );
        ...
        getProfile: function () {
          ...
          var table = context
            .getPanel("console")
            .panelNode.getElementsByClassName("profileSizer");

          if (!table.length) {
            if (win.console) {
              win.console.error(
                "Firebug Script panel must be enabled in order for FireUnit profiling to work."
              );
            }
            return null;
          }
          ...
          var time = table
            .getElementsByClassName("profileTime")[0]
        ...
        ok: function (pass, msg, expected, actual) {
          var result =
            arguments.length > 2
              ? new Firebug.FireUnitModule.TestResult(
                  win,
                  pass,
                  msg,
                  expected,
         ...
         */
        log: function (msg) {
          var result = new Firebug.FireUnitModule.TestResult(
            win,
            undefined,
            msg,
            undefined,
        ...
        group: function (name) {
          ...
          panel.appendGroup(
            name,
            win.location.href.substr(win.location.href.lastIndexOf("/") + 1)
          );
        },
        ...
        panel: function (name) {
          ...
          if (canChrome(win)) return context.getPanel(name).panelNode;
        },
        // HTTP Server
        registerPathHandler: function (path, handler) {
          if (!canServer(win)) return;

          return getServer().registerPathHandler(
            path,
        ...
        privilege: function (enable) {
          if (enable) Firebug.FireUnitModule.Privilege.enable();
          else Firebug.FireUnitModule.Privilege.disable();
        },
      };
    ...
    function $FU_STR(name) {
      try {
        return document
          .getElementById("strings_fireUnit")
          .getString(name.replace(" ", "_", "g"));
      } catch (err) {
        if (FBTrace.DBG_FIREUNIT) {
    ...
    FireUnitPanel.prototype = extend(Firebug.Panel, {
      ...
      updateOption: function (name, value) {
        if (FBTrace.DBG_FIREUNIT)
          FBTrace.sysout(
            "fireunit.FireUnitPanel.updateOption: " + name + ": " + value
          );

        if (
          name == "fireunit.showPass" ||
          name == "fireunit.showFail" ||
          name == "fireunit.showLog"
        )
          this.updatePanelFilter();
      },
      ...
      getOptionsMenuItems: function (context) {
        ...
        items.push(
          this.optionMenu(
            $FU_STR("fireunit.option.Passing Tests"),
            "fireunit.showPass"
          )
        );
        items.push(
          this.optionMenu(
            $FU_STR("fireunit.option.Failing Tests"),
            "fireunit.showFail"
          )
        );
        items.push(
          this.optionMenu($FU_STR("fireunit.option.Log"), "fireunit.showLog")
        );
        items.push("-");
        items.push({
      ...
      appendGroup: function (name, fileName) {
        ...
        var row =
          Firebug.FireUnitModule.TestResultRep.groupContainerTag.insertRows(
            {},
            tbody.lastChild ? tbody.lastChild : tbody
          )[0];
        if (!this.groups) {
      ...
      appendSummary: function () {
        ...
        // Append summary row.
        var summaryRow =
          Firebug.FireUnitModule.TestResultRep.summaryTag.insertRows(
            { summary: summary },
            tbody.lastChild ? tbody.lastChild : tbody
          )[0];
    ...
    Firebug.FireUnitModule.TestResultRep = domplate(Firebug.Rep, {
      ...
      resultTag: FOR(
        ...
        TR(
          ...
          TD(
            { class: "testResultCol", width: "100%" },
            DIV(
              { class: "testResultMessage testResultLabel" },
              "$result|getMessage"
            ),
            DIV(
              {
                class:
                  "testResultFullMessage testResultMessage testResultLabel",
              },
              "$result.msg"
            )
          ),
          TD(
            { class: "testResultCol" },
            DIV(
              { class: "testResultFileName testResultLabel" },
              "$result.fileName"
            )
          )
        )
      ...
      groupStartTag: TR(
        ...
        TD(
          { class: "testResultCol" },
          DIV({ class: "testResultFileName testResultLabel" }, "$fileName")
        )
      ),

      //container for group contents
      groupContainerTag: TR(
        {},
        TD(
          { class: "testGroupContainer", colspan: 2 },
          TABLE({ class: "testTable", cellpadding: 0, cellspacing: 0 }, TBODY())
        )
      ),
      ...
      summaryTag: TR(
        { class: "testResultSummaryRow testResultRow" },
        TD(
          { class: "testResultCol", colspan: 2 },
          SPAN(
            {
              class: "testResultSummaryLabel",
      ...
      toggleResultRow: function (row) {
        ...
        if (hasClass(row, "opened")) {
          var infoBodyRow = this.resultInfoTag.insertRows(
            { result: result },
            row
          )[0];
      ...
      initInfoBody: function (infoBodyRow) {
        ...
        var tabViewNode = TabView.viewTag.replace(
          { result: result },
          infoBodyRow.firstChild,
          TabView
        );
      ...
      onViewSource: function (testResult) {
        ...
        FirebugContext.chrome.select(
          new SourceLink(stackFrame.fileName, stackFrame.lineNumber, "js")
        );
      },
    ...
    Firebug.FireUnitModule.TestResultTabView = domplate(Firebug.Rep, {
      ...
      viewTag: TABLE(
        { class: "tabView", cellpadding: 0, cellspacing: 0 },
        TBODY(
          TR(
            { class: "tabViewRow" },
            TD(
              { class: "tabViewCol", valign: "top" },
        ...
        A(
          {
            ...
            onclick: "$onClickTab",
            view: "Compare",
            $collapsed: "$result|hideCompareTab",
          },
      ...
      stackTag: TABLE(
        { class: "testResultStackInfoBody", cellpadding: 0, cellspacing: 0 },
        TBODY(
          FOR(
            "stack",
            "$result.stack",
      ...
      compareTag: TABLE(
        { class: "testResultCompareInfoBody", cellpadding: 0, cellspacing: 0 },
        TBODY(
          TR(
            { class: "testResultCompareTitle expected" },
            TD($FU_STR("fireunit.title.Expected")),
            TD(
              {
                class: "testResultCompareSwitch expected",
                onclick: "$onSwitchView",
              },
              $FU_STR("fireunit.switch.view_source")
            )
          ),
          TR(TD({ class: "testResultExpected", colspan: 2 })),
          TR(
            { class: "testResultCompareTitle result" },
            TD($FU_STR("fireunit.title.Result")),
            TD(
              {
                class: "testResultCompareSwitch result",
                onclick: "$onSwitchView",
              },
              $FU_STR("fireunit.switch.view_source")
            )
          ),
          TR(TD({ class: "testResultResult", colspan: 2 })),
          TR(
            {
              class: "testResultCompareTitle diff",
              $collapsed: "$result|hideDiffGroup",
            },
            TD({ colspan: 2 }, $FU_STR("fireunit.title.Difference"))
          ),
          TR(TD({ class: "testResultDiff", colspan: 2 }))
        )
      ),
      ...
      updateTabBody: function (viewBody, tabName) {
        ...
        if (tabName == "Compare" && !tabCompareBody.updated) {
          ...
          this.insertXml(
            result.expected + " (" + typeof result.expected + ")",
            getElementByClass(viewBody, "testResultExpected")
          );
          this.insertXml(
            result.result + " (" + typeof result.result + ")",
            getElementByClass(viewBody, "testResultResult")
          );
      ...
      onSwitchView: function (event) {
        ...
        var result = infoRow.repObject;
        var sourceBody = getElementByClass(
          infoRow,
          expected ? "testResultExpected" : "testResultResult"
        );

        clearNode(sourceBody);

        if (target.sourceView) this.insertXml(result.expected, sourceBody);
        else
          insertWrappedText(
        ...
        target.innerHTML = $FU_STR(
          "fireunit.switch." +
            (target.sourceView ? "view_source" : "pretty_print")
        );
      ...
      onClickStackFrame: function (event) {
        FirebugContext.chrome.select(
          new SourceLink(
            ...
            event.target.getAttribute("lineNumber"),
            "js"
          )
        );
      },
      ...
      insertXml: function (xml, parentNode) {
        ...
        if (
          docElem.namespaceURI == nsURI &&
          docElem.nodeName == "parsererror"
        ) {
          var errorNode = Firebug.FireUnitModule.ParseErrorRep.tag.replace(
            {
              error: {
        ...
        for (var i = 0; i < docElem.childNodes.length; i++)
          Firebug.HTMLPanel.CompleteElement.getNodeTag(
            docElem.childNodes[i]
     ...
     */
    Firebug.FireUnitModule.TestResult = function (
      win,
      pass,
      msg,
      expected,
      ...
      this.stack = [];
      for (
        var frame = Components.stack, i = 0;
        frame;
        frame = frame.caller, i++
      ) {
        var fileName = unescape(frame.filename ? frame.filename : "");
        if (fileName == "chrome://fireunit/content/fireunit.js") continue;
    ...
    Firebug.FireUnitModule.Privilege = {
      ...
      privileges:
        "UniversalPreferencesWrite UniversalXPConnect UniversalBrowserWrite " +
        "UniversalPreferencesRead UniversalBrowserRead",
      rePref: /p[0-9]+.id/,

      enable: function () {
        if (FBTrace.DBG_FIREUNIT) FBTrace.sysout("fireunit.Privilege.enable");
      ...
      disable: function () {
        if (FBTrace.DBG_FIREUNIT) FBTrace.sysout("fireunit.Privilege.disable");
      ...
      isEnabled: function () {
        ...
        if (!this.isEnabledForUri("http://localhost:7080")) return false;

        if (!this.isEnabledForUri("file://")) return false;
