...
vows
  .describe("Multiprocess Appender")
  .addBatch({
    worker: {
      topic: function () {
        var fakeNet = makeFakeNet(),
          appender = sandbox
            .require("../lib/appenders/multiprocess", {
              requires: {
                net: fakeNet,
              },
            })
      ...
      "should buffer messages written before socket is connected": function (
        net
      ) {
      ...
      "should attempt to re-open the socket on error": function (net) {
        assert.equal(
          net.data[4],
          JSON.stringify("after error, before connect")
        );
    ...
    "worker with timeout": {
      ...
      "should attempt to re-open the socket": function (net) {
        ...
        assert.equal(
          net.data[4],
          JSON.stringify("after timeout, before close")
        );
        assert.equal(
          net.data[6],
          JSON.stringify("after close, before connect")
        );
    ...
    master: {
      ...
      "should listen for log messages on loggerPort and loggerHost": function (
        net
      ) {
        assert.equal(net.port, 1234);
