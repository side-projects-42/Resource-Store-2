...
vows
  .describe("../lib/appenders/dateFile")
  .addBatch({
    appender: {
  ...
  .addBatch({
    configure: {
      ...
      "with options.alwaysIncludePattern": {
        topic: function () {
          var self = this,
            ...
            thisTime = format.asString(
              options.appenders[0].pattern,
              new Date()
            );
          fs.writeFileSync(
      ...
      "with cwd option": {
        topic: function () {
          var fileOpened,
            appender = sandbox.require("../lib/appenders/dateFile", {
              requires: {
                "../streams": {
        ...
        "should prepend options.cwd to config.filename": function (fileOpened) {
          var expected =
            path.sep + path.join("absolute", "path", "to", "whatever.log");
