...
vows
  .describe("log4js cluster appender")
  .addBatch({
    "when in master mode": {
      topic: function () {
        ...
        masterAppender(
          new LoggingEvent("wovs", "Info", ["masterAppender test"])
        );
    ...
    "when in worker mode": {
      topic: function () {
        ...
        // Actual test - log message using masterAppender
        workerAppender(
          new LoggingEvent("wovs", "Info", ["workerAppender test"])
        );
        workerAppender(
          new LoggingEvent("wovs", "Info", [new Error("Error test")])
        );
      ...
      "worker should serialize an Error correctly": function (topic) {
        ...
        assert(
          JSON.parse(topic.registeredProcessEvents[1].event).data[0].stack
        );
        var actual = JSON.parse(topic.registeredProcessEvents[1].event).data[0]
          .stack;
