...
vows
  ...
  .addBatch({
    colouredLayout: {
      ...
      "should support the console.log format for the message": function (
        layout
      ) {
        var output = layout({
          data: ["thing %d", 2],
        ...
        assert.equal(
          output,
          "\x1B[31m[2010-12-05 14:18:30.045] [ERROR] cheese - \x1B[39mthing 2"
        );
      },
    },
    ...
    messagePassThroughLayout: {
      ...
      "should output the first item even if it is not a string": function (
        layout
      ) {
        assert.equal(
          layout({
      ...
      "with passed augmented errors": {
        ...
        "should print error the contained error message": function (
          layoutOutput
        ) {
          var m = layoutOutput.match(/\{ \[Error: My Unique Error Message\]/);
          assert.isArray(m);
        },
        "should print error augmented string attributes": function (
          layoutOutput
        ) {
          var m = layoutOutput.match(/augmented:\s'My Unique attribute value'/);
          assert.isArray(m);
        },
        "should print error augmented object attributes": function (
          layoutOutput
        ) {
    ...
    patternLayout: {
      ...
      'should default to "time logLevel loggerName - message"': function (
        args
      ) {
        test(
          args,
      ...
      "%x{doesNotExist} should output the string stored in tokens": function (
        args
      ) {
        test(args, "%x{doesNotExist}", "null");
      },
      "%x{fnThatUsesLogEvent} should be able to use the logEvent": function (
        args
      ) {
