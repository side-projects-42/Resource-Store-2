...
vows
  ...
  .addBatch({
    "with config file changing": {
      ...
      "should configure log4js from first log4js.json found": function (
        logEvents
      ) {
    ...
    "with config file staying the same": {
      topic: function () {
        var pathsChecked = [],
          ...
          logEvents = [],
          logger,
          modulePath = require("path").normalize(
            __dirname + "/../lib/log4js.json"
          ),
          mtime = new Date(),
          fakeFS = {
            config: {
    ...
    "when config file is removed": {
      ...
      "should not clear configuration when config file not found": function (
        args
      ) {
        var logEvents = args[1];
        assert.equal(logEvents.length, 3);
    ...
    "when called twice with reload options": {
      topic: function () {
        var modulePath = require("path").normalize(
            __dirname + "/../lib/log4js.json"
          ),
          fakeFS = {
            readFileSync: function (file, encoding) {
