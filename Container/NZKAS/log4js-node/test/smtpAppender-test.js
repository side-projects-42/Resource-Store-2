...
function checkMessages(result, sender, subject) {
  for (var i = 0; i < result.results.length; ++i) {
    ...
    assert.equal(
      result.results[i].subject,
      subject ? subject : "Log event #" + (i + 1)
    );
    assert.ok(
      new RegExp(".+Log event #" + (i + 1) + "\n$").test(result.results[i].text)
    );
  }
}
  ...
  .addBatch({
    ...
    "multiple events in one email": {
      ...
      "messages should contain proper data": function (result) {
        ...
        assert.equal(
          result.results[0].text.match(new RegExp(".+Log event #[1-2]$", "gm"))
            .length,
          2
        );
    ...
    "error when sending email": {
      ...
      "should be logged to console": function (cons) {
        ...
        assert.equal(
          cons.errors[0].msg,
          "log4js.smtpAppender - Error happened"
        );
