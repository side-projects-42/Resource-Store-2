...
vows
  ...
  .addBatch({
    arguments: {
      ...
      "should take a filename and a pattern and return a WritableStream":
        function (stream) {
          assert.equal(
            stream.filename,
            __dirname + "/test-date-rolling-file-stream-1"
    ...
    "default arguments": {
      topic: new DateRollingFileStream(
        __dirname + "/test-date-rolling-file-stream-2"
      ),
    ...
    "with a pattern of .yyyy-MM-dd": {
      topic: function () {
        var that = this,
          stream = new DateRollingFileStream(
            __dirname + "/test-date-rolling-file-stream-5",
            ".yyyy-MM-dd",
            null,
            now
          );
      ...
      "when the day changes": {
        ...
        teardown: cleanUp(
          __dirname + "/test-date-rolling-file-stream-5.2012-09-12"
        ),
