...
function checkForMismatch(topic) {
  var er = topic.log4js.levels
    .toLevel(topic.baseLevel)
    .isLessThanOrEqualTo(topic.log4js.levels.toLevel(topic.comparisonLevel));

  assert.equal(
    er,
    topic.expectedResult,
    "Mismatch: for setLevel(" +
      topic.baseLevel +
      ") was expecting a comparison with " +
      topic.comparisonLevel +
      " to be " +
      topic.expectedResult
  );
}
...
function setupBaseLevelAndCompareToOtherLevels(baseLevel) {
  var baseLevelSubContext =
    "and checking the logger whose level was set to " + baseLevel;
...
function compareToOtherLevels(subContext) {
  ...
  return function (comparisonLevel) {
    var comparisonLevelSubContext =
      "with isLevelEnabled(" + comparisonLevel + ")";

    // calculate this independently of log4js, but we'll add a vow
    // later on to check that we're not mismatched with log4js
    var expectedResult =
      strLevels.indexOf(baseLevel) <= strLevels.indexOf(comparisonLevel);
    ...
    subSubContext["finally checking for comparison mismatch with log4js"] =
      checkForMismatch;
  };
}
...
for (var cfg in configs) {
  ...
  if (nop) {
    context =
      "Setting up loggers with initial levels, then NOT setting a configuration,";
  } else {
    context =
      "Setting up loggers with initial levels, then setting a configuration which " +
      cfg +
      ",";
  }
