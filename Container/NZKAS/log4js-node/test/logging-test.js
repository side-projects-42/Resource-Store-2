  ...
  .addBatch({
    getBufferedLogger: {
      ...
      "cache events": {
        ...
        "should not emit log events if .flush() is not called.": function (
          events
        ) {
    ...
    "configuration that causes an error": {
      topic: function () {
        var log4js = sandbox.require("../lib/log4js", {
            requires: {
              "./appenders/file": {
                name: "file",
                appender: function () {},
              ...
              fs: {
                ...
                readFileSync: function (filename) {
                  ...
                  return JSON.stringify({
                    appenders: [{ type: "file", filename: "whatever.log" }],
                  });
                },
    ...
    addAppender: {
      ...
      "without a category": {
        "should register the function as a listener for all loggers": function (
          log4js
        ) {
          var appenderEvent,
        ...
        "if an appender for a category is defined": {
          "should register for that category": function (log4js) {
            ...
            log4js
              .getLogger("pants")
              .debug("this should not be propagated to otherEvent");
            assert.isUndefined(otherEvent);
            assert.equal(
              appenderEvent.data[0],
              "this should not be propagated to otherEvent"
            );
          },
        },
      },
    ...
    "default setup": {
      ...
      "should not replace console.log with log4js version": function (
        appenderEvents
      ) {
    ...
    console: {
      ...
      "when replaceConsole called": {
        ...
        "should replace console.log methods with log4js ones": function (
          logEvents
        ) {
          assert.equal(logEvents.length, 9);
          assert.equal(
            logEvents[0].data[0],
            "Some debug message someone put in a module"
          );
      ...
      "when turned off": {
        topic: function (test) {
          ...
          try {
            test.fakeConsole.log(
              "This should cause the error described in the setup"
            );
          } catch (e) {
            return e;
          }
    ...
    "console configuration": {
      ...
      "when enabled": {
        ...
        "should allow for turning on console replacement": function (
          logEvents
        ) {
