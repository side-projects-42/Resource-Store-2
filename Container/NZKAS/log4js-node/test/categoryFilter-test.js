...
vows
  .describe("log4js categoryFilter")
  .addBatch({
    appender: {
      topic: function () {
        ...
        var appender = require("../lib/appenders/categoryFilter").appender(
          ["app"],
          function (evt) {
            logEvents.push(evt);
          }
        ...
        log4js
          .getLogger("db")
          .debug("This shouldn't be included by the appender anyway");
    ...
    configure: {
      ...
      "tmp-tests.log": {
        topic: function () {
          fs.readFile(
            __dirname + "/categoryFilter-noweb.log",
            "utf8",
            this.callback
          );
        },
      ...
      "tmp-tests-web.log": {
        topic: function () {
          fs.readFile(
            __dirname + "/categoryFilter-web.log",
            "utf8",
            this.callback
          );
        },
        "should contain only error and warning log messages": function (
          contents
        ) {
          var messages = contents.trim().split(EOL);
