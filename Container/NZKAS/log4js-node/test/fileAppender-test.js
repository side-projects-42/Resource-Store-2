...
vows
  ...
  .addBatch({
    ...
    "with default fileAppender settings": {
      ...
      "log messages should be in the basic layout format": function (
        err,
        fileContents
      ) {
        assert.match(
      ...
      topic: function () {
        ...
        function addAppender(cat) {
          var testFile = path.join(
            __dirname,
            "/fa-subcategories-test-" +
              cat.join("-").replace(/\./g, "_") +
              ".log"
          );
          remove(testFile);
          log4js.addAppender(
        ...
        var file_sub1_sub12$sub1_sub13 = addAppender([
          "sub1.sub12",
          "sub1.sub13",
        ]);
        ...
        setTimeout(function () {
          that.callback(null, {
            ...
            file_sub1_sub12$sub1_sub13: fs
              .readFileSync(file_sub1_sub12$sub1_sub13)
              .toString(),
    ...
    "with a max file size and no backups": {
      ...
      "log file should only contain the second message": function (
        err,
        fileContents
      ) {
      ...
      "the number of files": {
        ...
        "starting with the test file name should be two": function (
          err,
          files
        ) {
    ...
    "with a max file size and 2 compressed backups": {
      ...
      "the log files": {
        topic: function (files) {
          var logFiles = files.filter(function (file) {
            return (
              file.indexOf("fa-maxFileSize-with-backups-compressed-test.log") >
              -1
            );
          });
          return logFiles;
  ...
  .addBatch({
    configure: {
      "with fileAppender": {
        ...
        "should load appender configuration from a json file": function (
          err,
          contents
        ) {
