    ...
    var _count = {},
      ...
      template = {
        ...
        test_drop_setup: function () {
          ...
          Y.Assert.isTrue(
            drop.get("node").hasClass("yui3-dd-drop"),
            "drop: Drop Instance ClassName"
          );
        },
        ...
        test_drag_setup: function () {
          ...
          Y.Assert.isTrue(
            dd.get("node").hasClass("yui3-dd-draggable"),
            "dd: Drag Instance ClassName"
          );
        },
        ...
        test_drag_drop_setup: function () {
          ...
          Y.Assert.isInstanceOf(
            Y.DD.Drop,
            dd.target,
            "drag.target: Drop Instance"
          );
          Y.each(dd._yuievt.targets, function (v, k) {
            Y.Assert.areSame(
              v,
              dd.target._yuievt.targets[k],
              "bubbleTargets are not the same"
            );
          });
        },
        test_drag_drop_group_setup: function () {
          ...
          Y.Assert.areSame(
            dd.get("groups").length,
            dd.target.get("groups").length,
            "Groups failed to pass from Drag to Drop"
          );
        },
        ...
        test_drag_add_handle: function () {
          ...
          Y.Assert.isNotUndefined(
            dd._handles[wrap._yuid],
            "dd: Handles " + wrap._yuid + " not there (Node Based Handle)"
          );
          dd.set("handles", false);
          Y.Assert.isNull(dd._handles, "dd: Handles NOT Null");
        },
        ...
        test_drag_move: function () {
          dd.on("drag:end", function () {
            Y.Assert.areSame(
              moveCount,
              _count["drag:drag"],
              "drag:drag should fire " + moveCount + " times"
            );
            //Y.Assert.areSame(1, _count['drag:drophit'], 'drag:drophit should fire 1 time');
            Y.Assert.areSame(
              1,
              _count["drag:end"],
              "drag:end should fire 1 time"
            );
            Y.Assert.areSame(
              1,
              _count["drag:start"],
              "drag:start should fire 1 time"
            );
        ...
        test_drag_destroy: function () {
          ...
          Y.Assert.isFalse(
            dd.get("node").hasClass("yui3-dd-draggable"),
            "drag: Drag Instance NO ClassName"
          );
        ...
        test_proxy: function () {
          ...
          Y.Assert.isTrue(
            p.hasClass("yui3-dd-proxy"),
            "proxy: Proxy Node Instance ClassName"
          );
        },
        test_proxy_setup_events: function () {
          Y.each(dd_events, function (v) {
            ...
            Y.Assert.isInstanceOf(
              Y.EventHandle,
              handle,
              "proxy:handle [" + v + "]: Handle Instance"
            );
          });
        },
        ...
        test_proxy_destroy: function () {
          proxy.destroy();
          Y.Assert.isFalse(
            proxy.get("node").hasClass("yui3-dd-draggable"),
            "proxy: Drag Instance NO ClassName"
          );
        ...
        test_drop_destroy: function () {
          ...
          Y.Assert.isFalse(
            drop.get("node").hasClass("yui3-dd-drop"),
            "Drop: Drop Instance NO ClassName"
          );
        ...
        */
        ...
        test_delegate_disabled: function () {
          ...
          Y.Assert.isFalse(
            mDown,
            "Delegate mouseDown fired on a disabled item"
          );
        },
      };
