...
exports.configure = function (c) {
  ...
  YUI.prototype.use = function () {
    ...
    if (this.config.loadDir) {
      this.log(
        "loadDir config found, searching for modules..",
        "info",
        "loadDir"
      );
        ...
        self.config.loadDir.dirs.forEach(function (p) {
          ...
          var list = fs.readdir(
            ...
            files.add(function (err, mods) {
              ...
              self.log(
                "Found (" + mods.length + ") files under: " + p,
                "info",
                "loadDir"
              );
              mods.forEach(function (f) {
                if (path.extname(f) === ".js") {
                  ...
                  YUI.include(
                    f,
                    files.add(function (err, fn) {
                      self.log("Autoloaded file: " + f, "info", "loadDir");
                      fn({
                        add: function (name, func, version, meta) {
                          self.Env._loader.moduleInfo[name].fullpath = f;
                          self.Env._loader.moduleInfo[name].loadDir =
                            path.dirname(f);
                        },
                      });
                    })
