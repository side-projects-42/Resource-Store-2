...
*/
  ...
  this.regexNonAssignmentBinaryExpressionOperators =
...
ZeParser.prototype = {
  ...
  parse: function (match) {
    if (match)
      match = this.tokenizer.storeCurrentAndFetchNextToken(
        false,
        match,
        this.stack
      );
    // meh
      ...
      match = this.tokenizer.storeCurrentAndFetchNextToken(
        ...
        this.stack,
        true
      ); // initialization step, dont store the match (there isnt any!)
    ...
    do {
      if (match && match.name != 12 /*eof*/) {
        ...
        if (match.name != 14 /*error*/)
          this.failignore("UnexpectedToken", match, this.stack);
  ...
  eatSemiColon: function (match, stack) {
    ...
    if (match.value == ";")
      ...
      if (
        match.name != 12 /*EOF*/ &&
        (match.semi || (!match.newline && match.value != "}")) &&
        !(match.newline && (match.value == "++" || match.value == "--"))
      ) {
   ...
   * @return {Object}
   */
  eatExpressions: function (
    mayParseLabeledStatementInstead,
    match,
    stack,
    onlyOne,
    forHeader,
    isBreakOrContinueArg
  ) {
    if (this.ast) {
      //#ifdef FULL_AST
      var pstack = stack;
    ...
    do {
      ...
      if (!first) {
        ...
        if (
          ...
          match = this.failsafe("ExpectedAnotherExpressionComma", match);
      }
      ...
      while (parseAnotherExpression) {
        ...
        while (
          ...
          (isUnary =
            (match.value &&
              this.hashUnaryKeywordStart[match.value[0]] &&
              this.regexUnaryKeywords.test(match.value)) || // (match.value == 'delete' || match.value == 'void' || match.value == 'typeof' || match.value == 'new') ||
            (match.name == 11 /*PUNCTUATOR*/ &&
          ...
          if (
            ...
            match = this.failsafe("ExpectedAnotherExpressionRhs", match);
        ...
        if (match.value == "(") {
          ...
          if (
            ...
            match = this.failsafe("GroupingShouldStartWithExpression", match);
          // keep parsing expressions as long as they are followed by a comma
          match = this.eatExpressions(false, match, stack);

          if (match.value != ")")
            match = this.failsafe("UnclosedGroupingOperator", match);
          if (this.ast) {
            //#ifdef FULL_AST
            match.twin = lhp;
            lhp.twin = match;
          ...
          while (match.value != "}" && match.name != 14 /*error*/) {
            ...
            if (getset == "get") {
              ...
              if (match.value == ":") {
                ...
                if (match.value != ")")
                  match = this.failsafe("GetterHasNoArguments", match);
                if (this.ast) {
                  //#ifdef FULL_AST
                  match.twin = lhp;
                  lhp.twin = match;
              ...
              if (match.value == ":") {
                ...
                if (match.name != 2 /*IDENTIFIER*/) {
                  if (match.value == ")")
                    match = this.failsafe("SettersMustHaveArgument", match);
                  else
                    match = this.failsafe(
                ...
                if (match.value != ")") {
                  if (match.value == ",")
                    match = this.failsafe("SettersOnlyGetOneArgument", match);
                  else
                    match = this.failsafe(
            ...
            if (match.value == ",") {
              ...
              match = this.failsafe("UnclosedObjectLiteral", match);
          ...
          if (mayParseLabeledStatementInstead && match.value == ":")
            match = this.failsafe("LabelsMayNotBeReserved", match);
          if (match.name == 2 /*IDENTIFIER*/) {
            ...
            if (
              ...
              match = this.failsafe("FunctionNameMustNotBeReserved", match);
            match = this.tokenizer.storeCurrentAndFetchNextToken(
              false,
              match,
              stack
          ...
          match = this.eatFunctionParametersAndBody(
            match,
            stack,
            true,
            funcExprToken
          ); // first token after func-expr is div
          ...
          if (mayParseLabeledStatementInstead && match.value == ":") {
            ...
            if (
              match.error &&
              match.error.msg == ZeParser.Errors.UnableToParseStatement.msg
            ) {
        ...
        while (match.value == "." || match.value == "[" || match.value == "(") {
          ...
          if (match.value == ".") {
            ...
            if (
              ...
              if (match.value == "]")
                match = this.failsafe("SquareBracketsMayNotBeEmpty", match);
              else
                match = this.failsafe("SquareBracketExpectsExpression", match);
            }
            match = this.eatExpressions(false, match, stack);
            if (match.value != "]")
              match = this.failsafe("UnclosedSquareBrackets", match);
            if (this.ast) {
              //#ifdef FULL_AST
              match.twin = lhsb;
            ...
            if (
              /*is left hand side start?*/ match.name <= 6 ||
              this.regexLhsStart.test(match.value)
            )
              match = this.eatExpressions(false, match, stack); // arguments are optional
            if (match.value != ")")
              match = this.failsafe("UnclosedCallParens", match);
            if (this.ast) {
              //#ifdef FULL_AST
              match.twin = lhp;
              lhp.twin = match;
        ...
        do {
          ...
          if (
            ...
            if (match.isAssignment) {
              if (!acceptAssignment)
                this.failignore("IllegalLhsForAssignment", match, stack);
              else if (parsedNonAssignmentOperator)
                this.failignore(
            ...
            if (ternary) {
              ...
              if (
                ...
                this.failignore("InvalidCenterTernaryExpression", match, stack);
  ...
  eatFunctionDeclaration: function (match, stack) {
    ...
    if (
      ...
      this.failignore("FunctionNameMayNotBeReserved", match, stack);
    if (this.ast) {
      //#ifdef FULL_AST
    ...
    match = this.eatFunctionParametersAndBody(
      ...
      false,
      functionKeyword
    ); // first token after func-decl is regex
    if (this.ast) {
      //#ifdef FULL_AST
  ...
  eatObjectLiteralColonAndBody: function (match, stack) {
    ...
    if (match == prevMatch)
      match = this.failsafe("ObjectLiteralMissingPropertyValue", match);
  ...
  eatFunctionParametersAndBody: function (match, stack, div, funcToken) {
    ...
    if (match.value != "(")
      match = this.failsafe("ExpectingFunctionHeaderStart", match);
    else if (this.ast) {
      //#ifdef FULL_AST
      var lhp = match;
      funcToken.lhp = match;
    ...
    if (match.value != ")")
      match = this.failsafe("ExpectedFunctionHeaderClose", match); // TOFIX: can be various things here...
    match = this.tokenizer.storeCurrentAndFetchNextToken(false, match, stack);
  ...
  eatFunctionBody: function (match, stack, div, funcToken) {
    ...
    if (match.value != "{")
      match = this.failsafe("ExpectedFunctionBodyCurlyOpen", match);
    if (this.ast) {
      //#ifdef FULL_AST
      var lhc = match;
    ...
    if (match.value != "}")
      match = this.failsafe("ExpectedFunctionBodyCurlyClose", match);
    if (this.ast) {
      //#ifdef FULL_AST
      match.twin = lhc;
      lhc.twin = match;
    ...
    do {
      ...
      if (match.value == "=") {
        ...
        if (
          ...
          match = this.failsafe("VarInitialiserExpressionExpected", match);
  ...
  eatIf: function (match, stack) {
    ...
    match = this.tokenizer.storeCurrentAndFetchNextToken(false, match, stack);
    if (match.value != "(")
      match = this.failsafe("ExpectedStatementHeaderOpen", match);
    if (this.ast) {
      //#ifdef FULL_AST
      var lhp = match;
    ...
    if (
      ...
      match = this.failsafe("StatementHeaderIsNotOptional", match);
    match = this.eatExpressions(false, match, stack);
    if (match.value != ")")
      match = this.failsafe("ExpectedStatementHeaderClose", match);
    if (this.ast) {
      //#ifdef FULL_AST
      match.twin = lhp;
  ...
  eatDo: function (match, stack) {
    ...
    match = this.tokenizer.storeCurrentAndFetchNextToken(false, match, stack);
    match = this.eatStatement(false, match, stack);
    if (match.value != "while")
      match = this.failsafe("DoShouldBeFollowedByWhile", match);
    if (this.ast) {
      //#ifdef FULL_AST
      match.hasDo = doToken;
    } //#endif
  ...
  eatFor: function (match, stack) {
    ...
    if (match.value == "in") {
      ...
      if (match.value != ";")
        match = this.failsafe("ForHeaderShouldHaveSemisOrIn", match);
      ...
      if (
        ...
        match = this.eatExpressions(false, match, stack); // in is ok here
      if (match.value != ";")
        match = this.failsafe("ExpectedSecondSemiOfForHeader", match);
      if (this.ast) {
        //#ifdef FULL_AST
    ...
    if (this.ast) {
      ...
      if (
        match.forType == "in" &&
        stack[stack.length - 1].desc == "expressions"
      ) {
        // create ref to this expression group to the opening bracket
        lhp.expressionArg = stack[stack.length - 1];
      }
  ...
  eatThrow: function (match, stack) {
    ...
    if (match.value == ";")
      match = this.failsafe("ThrowMustHaveArgument", match);
    match = this.eatExpressions(false, match, stack);
    match = this.eatSemiColon(match, stack);
  ...
  eatSwitch: function (match, stack) {
    ...
    match = this.tokenizer.storeCurrentAndFetchNextToken(false, match, stack);
    if (match.value != "{")
      match = this.failsafe("SwitchBodyStartsWithCurly", match);
    ...
    while (
      match.value == "case" ||
      (!stack.parsedSwitchDefault && match.value == "default")
    ) {
      parsedAnything = true;
    ...
    if (match.value != "}" && match.name != 14 /*error*/)
  ...
  eatTry: function (match, stack) {
    ...
    match = this.tokenizer.storeCurrentAndFetchNextToken(false, match, stack);
    if (match.value != "{")
      match = this.failsafe("MissingTryBlockCurlyOpen", match);
    ...
    if (match.value != "}")
      match = this.failsafe("MissingTryBlockCurlyClose", match);
  ...
  eatCatch: function (match, stack) {
    ...
    if (this.ast) {
      ...
      if (catchScopeBackup.global) this.scope.push(catchScopeBackup[0]);
      // global (has no `arguments` but always a `this`)
        ...
        if (catchScopeBackup[2] && catchScopeBackup[2].value == "arguments")
          this.scope.push(catchScopeBackup[2]);
      } else this.scope.push(catchScopeBackup[1], catchScopeBackup[2]); // function scope, copy this and arguments
    } //#endif

    match = this.tokenizer.storeCurrentAndFetchNextToken(false, match, stack);
    if (match.value != "(")
      match = this.failsafe("CatchHeaderMissingOpen", match);
    if (this.ast) {
      //#ifdef FULL_AST
      var lhp = match;
    } //#endif
    ...
    match = this.tokenizer.storeCurrentAndFetchNextToken(false, match, stack);
    if (match.value != ")")
      match = this.failsafe("CatchHeaderMissingClose", match);
    if (this.ast) {
      //#ifdef FULL_AST
      match.twin = lhp;
      lhp.twin = match;
    ...
    match = this.tokenizer.storeCurrentAndFetchNextToken(false, match, stack);
    if (match.value != "{")
      match = this.failsafe("MissingCatchBlockCurlyOpen", match);
    if (this.ast) {
      //#ifdef FULL_AST
      var lhc = match;
    } //#endif
    ...
    if (match.value != "}")
      match = this.failsafe("MissingCatchBlockCurlyClose", match);
    if (this.ast) {
      //#ifdef FULL_AST
      match.twin = lhc;
      lhc.twin = match;
  ...
  eatFinally: function (match, stack) {
    ...
    match = this.tokenizer.storeCurrentAndFetchNextToken(false, match, stack);
    if (match.value != "{")
      match = this.failsafe("MissingFinallyBlockCurlyOpen", match);
    if (this.ast) {
      //#ifdef FULL_AST
      var lhc = match;
    } //#endif
    ...
    if (match.value != "}") match = this.eatStatements(match, stack);
    if (match.value != "}")
      match = this.failsafe("MissingFinallyBlockCurlyClose", match);
    if (this.ast) {
      //#ifdef FULL_AST
      match.twin = lhc;
      lhc.twin = match;
...
ZeParser.testSuite = function (tests) {
  ...
  for (var i = 0; i < tests.length; ++i) {
    var test = tests[i],
      input = test[0],
      outputLen = test[1].length ? test[1][1] : test[1],
...
ZeParser.hashStartKeyOrReserved = Object.create
  ? Object.create(null, {
      b: { value: 1 },
      c: { value: 1 },
      d: { value: 1 },
  ...
  : { b: 1, c: 1, d: 1, e: 1, f: 1, i: 1, n: 1, r: 1, s: 1, t: 1, v: 1, w: 1 };
ZeParser.regexIsKeywordOrReserved =
...
ZeParser.hashUnaryKeywordStart = Object.create
  ? Object.create(null, {
      d: { value: 1 },
      v: { value: 1 },
      t: { value: 1 },
