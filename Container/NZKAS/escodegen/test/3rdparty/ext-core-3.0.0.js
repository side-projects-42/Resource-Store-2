  ...
  (function () {
    ...
    Ext.lib.Dom = {
      ...
      getDocumentHeight: function () {
        return MAX(
          !isCSS1 ? doc.body.scrollHeight : doc.documentElement.scrollHeight,
      ...
      getDocumentWidth: function () {
        return MAX(
          !isCSS1 ? doc.body.scrollWidth : doc.documentElement.scrollWidth,
        ...
        if (el != bd) {
          if (el.getBoundingClientRect) {
            ...
            while (p && p != bd) {
              if (
                !Ext.isOpera ||
                (p.tagName != "TR" && !fly(p).isStyle("display", "inline"))
              ) {
                x -= p.scrollLeft;
                y -= p.scrollTop;
              }
      ...
      request: function (method, uri, cb, data, options) {
        if (options) {
          ...
          if (xmlData || jsonData) {
            ...
            data =
              xmlData ||
              (Ext.isObject(jsonData) ? Ext.encode(jsonData) : jsonData);
          }
        }
        ...
        Ext.each(fElements, function (element) {
          ...
          if (!element.disabled && name) {
            if (/select-(one|multiple)/i.test(type)) {
              ...
              if (
                !(/radio|checkbox/i.test(type) && !element.checked) &&
    ...
    EXTLIB.AnimBase.prototype = {
      ...
      getAttr: function (attr) {
        ...
        return !!a[2] || (el.getStyle("position") == "absolute" && !!a[3])
          ? el.dom["offset" + a[0].charAt(0).toUpperCase() + a[0].substr(1)]
          : 0;
      },
...
(function () {
  ...
  Ext.Element = function (element, forceNew) {
    var dom =
        typeof element == "string" ? DOC.getElementById(element) : element,
      id;
  ...
  (function () {
    ...
    function chkCache(prop) {
      return (
        propCache[prop] ||
        (propCache[prop] =
          prop == "float" ? propFloat : prop.replace(camelRe, camelFn))
      );
    }
      ...
      getStyle: (function () {
        return view && view.getComputedStyle
          ...
          : function (prop) {
              ...
              prop = chkCache(prop);
              return (
                el.style[prop] || ((cs = el.currentStyle) ? cs[prop] : null)
              );
            };
      })(),
  ...
  function doFormUpload(o, ps, url) {
    ...
    function cb() {
      var me = this,
        ...
        r = { responseText: "", responseXML: null, argument: o.argument },
        doc,
        firstChild;
    ...
    request: function (o) {
      ...
      if (me.fireEvent(BEFOREREQUEST, me, o)) {
        ...
        if ((form = Ext.getDom(o.form))) {
          ...
          if (
            o.isUpload ||
            /multipart\/form-data/i.test(form.getAttribute("enctype"))
          ) {
        ...
        method =
          o.method || me.method || (p || o.xmlData || o.jsonData ? POST : GET);
