...
(function (window) {
  ...
  var oldWin = window,
    ...
    reOpera = /Opera/,
    /** Opera [[Class]] */
    operaClass = reOpera.test((operaClass = getClassOf(opera)))
      ? operaClass
      : (opera = null);
  ...
  function parse(ua) {
      ...
      return reduce(guesses, function (result, value, key) {
        ...
        return (
          ...
          ((value[product] ||
            ...
            RegExp(
              "\\b" + (key.pattern || qualify(key)) + "(?:\\b|\\w*\\d)",
              "i"
            ).exec(ua)) &&
            (key.label || key))
        );
    ...
    function getProduct(guesses) {
      return reduce(guesses, function (result, guess) {
        ...
        if (
          ...
          if (
            (result = String(guess.label || result).split("/"))[1] &&
            !/[\d.]+/.test(result[0])
          ) {
            result[0] += " " + result[1];
          }
    ...
    // detect false positives for Firefox/Safari
      ...
      !name ||
      (data = !/\bMinefield\b/i.test(ua) && /Firefox|Safari/.exec(name))
    ) {
      ...
      if (
        ...
        (product ||
          manufacturer ||
          /Android|Symbian OS|Tablet OS|webOS/.test(os))
      ) {
        name =
          /[a-z]+(?: Hat)?/i.exec(/Android/.test(os) ? os : data) + " Browser";
      }
    }
    ...
    if (useFeatures) {
      ...
      // detect Adobe AIR
        ...
        getClassOf((data = window.runtime)) == "ScriptBridgingProxyObject"
      ) {
        name = "Adobe AIR";
    ...
    // add mobile postfix
      ...
      (name == "IE" || (name && !product && !/Browser/.test(name))) &&
    ...
    if (
      layout &&
      !/Avant|Nook/.test(name) &&
      (/Browser|Lunascape|Maxthon/.test(name) ||
        (/^(?:Adobe|Arora|Midori|Phantom|Rekonq|Rock|Sleipnir|Web)/.test(
          name
        ) &&
          layout[1]))
    ) {
    ...
    if (
      (data = /\b(?:AMD|IA|Win|WOW|x86_|x)64\b/i).test(arch) &&
      !/\bi686\b/i.test(arch)
    ) {
