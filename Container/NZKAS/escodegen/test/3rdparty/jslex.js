...
Narcissus.lexer = (function () {
  ...
  function isValidIdentifierChar(ch, first) {
    ...
    try {
      valid =
        Function("x", "return (x." + (first ? "" : "x") + ch + ");")(x) ===
        true;
    } catch (ex) {}
  ...
  Tokenizer.prototype = {
    ...
    skip: function () {
      ...
      for (;;) {
        ...
        if (ch === "\n" && !this.scanNewlines) {
          ...
          (ch === "/" && next === "/") ||
          (Narcissus.options.allowHTMLComments &&
            ch === "<" &&
            next === "!" &&
            input[this.cursor + 1] === "-" &&
            input[this.cursor + 2] === "-" &&
    ...
    lexIdent: function (ch, keywordIsName) {
      ...
      if (keywordIsName) return;

      var kw = definitions.keywords[id];
      if (kw && !(kw in this.blackList)) token.type = kw;
    },
      ...
      if (ch === "\\" && input[this.cursor + 1] === "u") {
        ...
        try {
          ch = String.fromCharCode(
            parseInt(input.substring(this.cursor + 2, this.cursor + 6), 16)
          );
        } catch (ex) {
          return null;
        }
