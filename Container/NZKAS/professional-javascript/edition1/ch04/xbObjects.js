...
function _Classes() {
  ...
  function registerClass(className, parentClassName) {
    ...
    if (!parentClassName) parentClassName = null;
    else if (!(parentClassName in _classes))
      throw (
        "xbObjects.js:_Classes::registerClass: parentClassName " +
        parentClassName +
        " not defined"
      );
  ...
  function defineClass(className, prototype_func) {
    ...
    if (!classRef)
      throw (
        "xbObjects.js:_Classes::defineClass: className " +
        className +
        " not registered"
      );
    ...
    if (parentClassName) {
      ...
      if (!parentClassRef)
        throw (
          "xbObjects.js:_Classes::defineClass: parentClassName " +
          parentClassName +
          " not registered"
        );
      ...
      for (p in parentPrototype) {
        switch (p) {
          ...
          default:
            if (
              childPrototype[p] == parentPrototype[p] &&
              parentPrototype.inheritedFrom[p]
            ) {
              childPrototype.inheritedFrom[p] =
                parentPrototype.inheritedFrom[p];
            } else {
...
function xbObject() {
  ...
  function _prototype_func() {
    ...
    function parentMethod(
      ...
      var parentClassName =
        _classes[className].classConstructor.prototype.inheritedFrom[method];
      var tempMethod =
        _classes[parentClassName].classConstructor.prototype[method];
    ...
    function isInstanceOf(otherClassConstructor) {
      ...
      while (className) {
        if (className == otherClassName) return true;
