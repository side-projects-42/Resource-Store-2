    ...
    find: function (selector) {
      if (this.length === 1 && !/,/.test(selector)) {
        ...
        return this.pushStack(
          /[^+>] [^+>]/.test(selector) ? jQuery.unique(elems) : elems,
          "find",
          selector
        );
      }
    ...
    closest: function (selector) {
      ...
      return this.map(function () {
        ...
        while (cur && cur.ownerDocument) {
          if (pos ? pos.index(cur) > -1 : jQuery(cur).is(selector)) return cur;
          cur = cur.parentNode;
        }
      });
    },
      ...
      if (this[0]) {
        ...
        if (first)
          for (var i = 0, l = this.length; i < l; i++)
            callback.call(
              root(this[i], first),
              i > 0 ? extra.cloneNode(true) : fragment
            );
    ...
    var Sizzle = function (selector, context, results, seed) {
      ...
      if (parts.length > 1 && Expr.match.POS.exec(selector)) {
        if (parts.length === 2 && Expr.relative[parts[0]]) {
          ...
          set = Sizzle.filter(
            later,
            Sizzle(/\s$/.test(selector) ? selector + "*" : selector, context)
          );
        } else {
        ...
        "+": function (checkSet, part) {
          for (var i = 0, l = checkSet.length; i < l; i++) {
            ...
            if (elem) {
              ...
              checkSet[i] =
                typeof part === "string" ? cur || false : cur === part;
            }
          }
      ...
      preFilter: {
        CLASS: function (match, curLoop, inplace, result, not) {
          ...
          for (var i = 0; curLoop[i]; i++) {
            if (
              not ^
              ((" " + curLoop[i].className + " ").indexOf(match) >= 0)
            ) {
