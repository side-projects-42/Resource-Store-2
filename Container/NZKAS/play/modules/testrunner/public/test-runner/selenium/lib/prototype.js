...
Ajax.Responders = {
  ...
  register: function (responderToAdd) {
    if (!this.include(responderToAdd)) this.responders.push(responderToAdd);
  },
  ...
  buildMatchExpression: function () {
    ...
    if ((clause = params.classNames).length > 0)
      for (var i = 0; i < clause.length; i++)
        conditions.push(
          "Element.hasClassName(element, " + clause[i].inspect() + ")"
        );
    if ((clause = params.attributes)) {
      clause.each(function (attribute) {
        ...
        switch (attribute.operator) {
          ...
          case "~=":
            conditions.push(
              splitValueBy(" ") + ".include(" + attribute.value.inspect() + ")"
            );
            break;
          case "|=":
            conditions.push(
  ...
  findElements: function (scope) {
    ...
    if ((element = $(this.params.id)))
      if (this.match(element))
        if (!scope || Element.childOf(element, scope)) return [element];
  ...
  return $A(arguments)
    .map(function (expression) {
      return expression
        .strip()
        .split(/\s+/)
        .inject([null], function (results, expr) {
...
Form.Element.Serializers = {
  ...
  select: function (element) {
    return Form.Element.Serializers[
