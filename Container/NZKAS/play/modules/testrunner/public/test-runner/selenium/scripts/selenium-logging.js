...
Logger.prototype = {
  ...
  openLogWindow: function () {
    this.logWindow = window.open(
      getDocumentBase(document) +
        "SeleniumLog.html?startingThreshold=" +
        this.threshold,
      "SeleniumLog",
      "width=600,height=1000,bottom=0,right=0,status,scrollbars,resizable"
    );
    this.logWindow.moveTo(
      window.screenX + 1210,
      window.screenY + window.outerHeight - 1400
    );
    if (browserVersion.appearsToBeBrokenInitialIE6) {
      ...
      var pendingMessage = new LogMessage(
        "warn",
        "You appear to be running an unpatched IE 6, which is not stable and can crash due to memory problems.  We recommend you run Windows update to install a more stable version of IE."
      );
  ...
  show: function () {
    ...
    setTimeout(function () {
      LOG.error(
        "Log window displayed.  Logging events will now be recorded to this window."
      );
    }, 500);
  },
      ...
      if (logWindow.append) {
        ...
        if (this.pendingMessages.length > 0) {
          ...
          while (this.pendingMessages.length > 0) {
            ...
            logWindow.append(
              msg.type + "(" + msg.timestamp + "): " + msg.msg,
              msg.type
            );
          }
          logWindow.append(
            "info(" +
              new Date().getTime() +
              "): Done appending missed logging messages",
            "info"
          );
        }
        logWindow.append(
          logLevel + "(" + new Date().getTime() + "): " + message,
          logLevel
        );
      }
    } else {
