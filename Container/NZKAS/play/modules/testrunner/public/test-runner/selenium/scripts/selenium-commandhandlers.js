...
objectExtend(CommandHandlerFactory.prototype, {
  ...
  registerAction: function (
    ...
    this.handlers[name] = new ActionHandler(
      actionBlock,
      wait,
      dontCheckAlertsAndConfirms
    );
  ...
  _registerAllAccessors: function (seleniumApi) {
    ...
    for (var functionName in seleniumApi) {
      ...
      if (match) {
        ...
        this._registerStoreCommandForAccessor(
          baseName,
          accessBlock,
          requiresTarget
        );

        var predicateBlock = this._predicateForAccessor(
          accessBlock,
          requiresTarget,
          isBoolean
        );
        this._registerAssertionsForPredicate(baseName, predicateBlock);
        this._registerWaitForCommandsForPredicate(
          seleniumApi,
          baseName,
          predicateBlock
        );
    ...
    return function (target, value) {
      ...
      if (PatternMatcher.matches(value, accessorResult)) {
        return new PredicateResult(
          true,
          "Actual value '" + accessorResult + "' did match '" + value + "'"
        );
      } else {
        return new PredicateResult(
          false,
          "Actual value '" + accessorResult + "' did not match '" + value + "'"
        );
      }
    };
  },
  ...
  _predicateForBooleanAccessor: function (accessBlock) {
    ...
    return function () {
      ...
      if (arguments.length > 2)
        throw new SeleniumError("Too many arguments! " + arguments.length);
      if (arguments.length == 2) {
  ...
  _registerAssertionsForPredicate: function (baseName, predicateBlock) {
    ...
    var negativeassertBlock = this.createAssertionFromPredicate(
      invertedPredicateBlock
    );
    this.registerAssert(
      "assert" + this._invertPredicateName(baseName),
      negativeassertBlock,
      true
    );
    this.registerAssert(
      "verify" + this._invertPredicateName(baseName),
      negativeassertBlock,
      false
    );
  },
  ...
  _registerWaitForCommandsForPredicate: function (
    seleniumApi,
    baseName,
    predicateBlock
  ) {
    ...
    var invertedPredicateBlock = this._invertPredicate(predicateBlock);
    var waitForNotActionMethod = this._waitForActionForPredicate(
      invertedPredicateBlock
    );
    ...
    this.registerAction(
      "waitFor" + this._invertPredicateName(baseName),
      waitForNotActionBlock,
      false,
      true
    );
  ...
  _registerStoreCommandForAccessor: function (
    baseName,
    accessBlock,
    requiresTarget
  ) {
...
ActionHandler.prototype.execute = function (seleniumApi, command) {
  if (
    this.checkAlerts &&
    null == /(Alert|Confirmation)(Not)?Present/.exec(command.command)
  ) {
  ...
  var terminationCondition = this.wait
    ? seleniumApi.makePageLoadCondition()
    : handlerCondition;
