  ...
  this.recordPageLoad = function (elementOrWindow) {
    ...
    try {
      if (elementOrWindow.location && elementOrWindow.location.href) {
        ...
        elementOrWindow.contentWindow &&
        elementOrWindow.contentWindow.location &&
        elementOrWindow.contentWindow.location.href
      ) {
        LOG.debug(
          "Page load location=" + elementOrWindow.contentWindow.location.href
        );
      } else {
        LOG.debug(
          "Page load location unknown, current window location=" +
            this.getCurrentWindow(true).location
        );
      }
    } catch (e) {
      LOG.error(
...
BrowserBot.prototype.triggerMouseEvent = function (
  ...
  if (
    ...
    element.ownerDocument.createEventObject
  ) {
    //IE
    ...
    if (evt.initMouseEvent) {
      ...
      LOG.warn(
        "element doesn't have initMouseEvent; firing an event which should -- but doesn't -- have other mouse-event related attributes here, as well as controlKeyDown, altKeyDown, shiftKeyDown, metaKeyDown"
      );
...
BrowserBot.prototype.modifyWindowToRecordPopUpDialogs = function (
  ...
  var newOpen = function (url, windowName, windowFeatures, replaceFlag) {
    ...
    if (windowName == "" || windowName == "_blank") {
      ...
      LOG.warn(
        "Opening window '_blank', which is not a real window name.  Randomizing target to be: " +
          windowName
      );
    }
    var openedWindow = myOriginalOpen(
      url,
      windowName,
      windowFeatures,
      replaceFlag
...
BrowserBot.prototype.modifySeparateTestWindowToDetectPageLoads = function (
  ...
  if (oldMarker) {
    LOG.debug(
      "modifySeparateTestWindowToDetectPageLoads: already polling this window: " +
        oldMarker
    );
    return;
  }
  ...
  if (frameElement && !htaSubFrame) {
    LOG.debug(
      "modifySeparateTestWindowToDetectPageLoads: this window is a frame; attaching a load listener"
    );
    ...
    LOG.debug(
      "dgf frameElement['frame'+this.uniqueId]=" +
        frameElement["frame" + this.uniqueId]
    );
...
BrowserBot.prototype._getFrameElementByName = function (name, doc, win) {
  ...
  LOG.warn(
    "_getFrameElementByName couldn't find a frame or iframe; checking every element for the name " +
      name
  );
  return BrowserBot.prototype.locateElementByName(
    win.name,
    win.parent.document
  );
};
...
BrowserBot.prototype.pollForLoad = function (
  ...
  try {
    ...
    var isSamePage = this._isSamePage(
      windowObject,
      originalDocument,
      originalLocation,
      originalHref,
    ...
    if (!isSamePage && rs == "complete") {
      ...
      if (/(TestRunner-splash|Blank)\.html\?start=true$/.test(currentHref)) {
        ...
        LOG.debug(
          "pollForLoad page load detected in non-current window; ignoring (currentlySelected=" +
    ...
    LOG.debug(
      "Exception during pollForLoad; this should get noticed soon (" +
        e.message +
        ")!"
    );
    ...
    if (
      ...
      if (
        ...
        if (
          ...
          LOG.info(
            "getReadyState not loaded, frame location was about:blank, but frame src = " +
              windowObject.frameElement.src
          );
          return null;
        }
        LOG.debug(
          "getReadyState = windowObject.frames.length = " +
        ...
        for (var i = 0; i < windowObject.frames.length; i++) {
          ...
          if (
            ...
            LOG.debug(
              "getReadyState aha! the nested frame " +
                windowObject.frames[i].name +
                " wasn't ready!"
            );
            return null;
          }
        ...
        LOG.debug(
          "pollForLoad readyState was null and DOM appeared to not be ready yet"
        );
      }
    }
...
BrowserBot.prototype.isPollingForLoad = function (win) {
  ...
  if (!marker) {
    LOG.debug(
      "isPollingForLoad false, missing uniqueId " +
        this.uniqueId +
        ": " +
        marker
    );
  ...
  if (!this.pollingForLoad[marker]) {
    LOG.debug(
      "isPollingForLoad false, this.pollingForLoad[" +
        marker +
        "]: " +
...
BrowserBot.prototype._handleClosedSubFrame = function (
  testWindow,
  doNotModify
) {
  if (this.proxyInjectionMode) {
...
BrowserBot.prototype._recursivelyDeleteCookie = function (
  ...
  if (slashIndex != -1) {
    deleted = this._recursivelyDeleteCookie(
      cookieName,
      domain,
...
BrowserBot.prototype.recursivelyDeleteCookie = function (
  ...
  var deleted = this._recursivelyDeleteCookie(
    cookieName,
    "." + domain,
    path,
    doc
 ...
 */
BrowserBot.prototype.findElementRecursive = function (
  locatorType,
  locatorString,
  inDocument,
  inWindow
) {
  var element = this.findElementBy(
    locatorType,
    locatorString,
    inDocument,
    inWindow
  ...
  for (var i = 0; i < inWindow.frames.length; i++) {
    ...
    if (inWindow.frames[i].document) {
      element = this.findElementRecursive(
        locatorType,
        locatorString,
        inWindow.frames[i].document,
        inWindow.frames[i]
      );
...
BrowserBot.prototype.findElementOrNull = function (locator, win) {
  ...
  var element = this.findElementRecursive(
    locator.type,
    locator.string,
    win.document,
    win
...
BrowserBot.prototype.locateElementByIdentifier = function (
  ...
  return (
    BrowserBot.prototype.locateElementById(identifier, inDocument, inWindow) ||
 ...
 */
BrowserBot.prototype.locateElementById = function (
  identifier,
  inDocument,
  inWindow
) {
  ...
  if (element && element.getAttribute("id") === identifier) {
    ...
    return BrowserBot.prototype.locateElementByXPath(
      xpath,
      inDocument,
      inWindow
    );
 ...
 * expressions.
 */
BrowserBot.prototype.locateElementByName = function (
  locator,
  document,
  inWindow
) {
 ...
 */
BrowserBot.prototype.locateElementByDomTraversal = function (
  domTraversal,
  document,
  window
) {
 ...
 * begin with "//".
 */
BrowserBot.prototype.locateElementByXPath = function (
  xpath,
  inDocument,
  inWindow
) {
 ...
 * begin with "link:".
 */
BrowserBot.prototype.locateElementByLinkText = function (
  linkText,
  inDocument,
  inWindow
) {
  ...
  if (element.target) {
    if (element.target == "_blank" || /^selenium_blank/.test(element.target)) {
      ...
      if (tagName == "a" || tagName == "form") {
        ...
        LOG.warn(
          "Link has target '_blank', which is not supported in Selenium!  Randomizing target to be: " +
            newTarget
        );
...
BrowserBot.prototype._getFrameFromGlobal = function (target) {
  ...
  var frameElement = this.findElementBy(
    "implicit",
    target,
 ...
 */
BrowserBot.prototype.selectElementsBy = function (
  filterType,
  filter,
  elements
) {
...
BrowserBot.prototype.locateElementByClass = function (locator, document) {
  return elementFindFirstMatchingChild(document, function (element) {
...
BrowserBot.prototype.locateElementByUIElement = function (locator, inDocument) {
  ...
  var pageElements = UIMap.getInstance().getPageElements(
    locators[0],
    inDocument
  );
  ...
  if (locators.length > 1) {
    for (var i = 0; i < pageElements.length; ++i) {
      var locatedElements = eval_locator(
        locators[1],
        inDocument,
        pageElements[i]
      );
 ...
 *                False otherwise.
 */
BrowserBot.prototype.locateElementByUIElement.is_fuzzy_match = function (
  node,
  target
) {
  try {
...
IEBrowserBot.prototype.pollForLoad = function (
  ...
  if (this.pageLoadError) {
    if (this.pageUnloading) {
      var self = this;
      LOG.debug(
        "pollForLoad UNLOADING (" +
          marker +
          "): caught exception while firing events on unloading page: " +
          this.pageLoadError.message
      );
      this.reschedulePoller(
        loadFunction,
        windowObject,
...
IEBrowserBot.prototype._windowClosed = function (win) {
  try {
    ...
    if (!c) {
      try {
        ...
        if (de.message == "Permission denied") {
          ...
          LOG.debug(
            "IEBrowserBot.windowClosed: got " +
              de.message +
              " (this.pageUnloading=" +
              this.pageUnloading +
              "); assuming window is unloading, probably not closed yet"
          );
          return false;
        } else {
          // this is probably one of those frame window situations
          LOG.debug(
            "IEBrowserBot.windowClosed: couldn't read win.document, assume closed: " +
              de.message +
              " (this.pageUnloading=" +
              this.pageUnloading +
              ")"
    ...
    if (c == null) {
      LOG.debug(
        "IEBrowserBot.windowClosed: win.closed was null, assuming closed"
      );
      return true;
    }
    return c;
    ...
    LOG.debug(
      "IEBrowserBot._windowClosed: Got an exception trying to read win.closed; we'll have to take a guess!"
    );
...
IEBrowserBot.prototype._fireEventOnElement = function (
  ...
  try {
    ...
    if (pageUnloading) {
      ...
      LOG.warn(
        "Caught exception when firing events on unloading page: " + e.message
      );
      return;
    }
    throw e;
