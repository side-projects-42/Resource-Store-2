...
Selenium.prototype.makePageLoadCondition = function (timeout) {
  ...
  return Selenium.decorateFunctionWithTimeout(
    fnBind(this._isNewPageLoaded, this),
    timeout
  );
};
...
Selenium.prototype.getEval = function (script) {
  ...
  try {
    ...
    throw new SeleniumError(
      "Threw an exception: " + extractExceptionMessage(e)
    );
  }
};
...
Selenium.prototype.findWindow = function (soughtAfterWindowPropertyValue) {
  ...
  if (soughtAfterWindowPropertyValue.match("^title=")) {
    ...
    soughtAfterWindowPropertyValue = soughtAfterWindowPropertyValue.replace(
      /^title=/,
      ""
    );
  } else {
  ...
  throw new SeleniumError(
    "could not find window with property " +
      targetPropertyName +
      " matching " +
      soughtAfterWindowPropertyValue
  );
...
Selenium.prototype.doDragAndDropToObject = function (
  ...
  var destinationLeftX = this.getElementPositionLeft(
    locatorOfDragDestinationObject
  );
  var destinationTopY = this.getElementPositionTop(
    locatorOfDragDestinationObject
  );
...
Selenium.prototype.getHtmlSource = function () {
   ...
   */
  return this.browserbot.getDocument().getElementsByTagName("html")[0]
    .innerHTML;
};
...
Selenium.prototype.getXpathCount = function (xpath) {
   ...
   */
  var result = this.browserbot.evaluateXPathCount(
    xpath,
...
Selenium.prototype.doDeleteCookie = function (name, optionsString) {
  ...
  if (optionsString && !matched) {
    LOG.warn(
      "Using entire optionsString as a path; please change the argument to deleteCookie to use path=" +
        optionsString
    );
    path = optionsString;
  }
...
Selenium.prototype.doAddLocationStrategy = function (
  strategyName,
  functionDefinition
) {
  /**
  ...
  try {
    ...
    throw new SeleniumError(
      "Error evaluating function definition: " + extractExceptionMessage(ex)
    );
  }
  var safeStrategyFunction = function () {
    try {
      ...
      throw new SeleniumError(
        "Error executing strategy function " +
          strategyName +
          ": " +
...
Selenium.prototype.doCaptureEntirePageScreenshot = function (filename, kwargs) {
  ...
  if (
    ...
    throw new SeleniumError(
      "captureEntirePageScreenshot is only " +
  ...
  if (browserVersion.isIE) {
    ...
    if (
      ...
      if (!this.snapsieSrc) {
        ...
        var snapsieUrl = this.browserbot.buttonWindow.location.href.replace(
  ...
  var SGNsUtils = {
    dataUrlToBinaryInputStream: function (dataUrl) {
      var nsIoService = Components.classes[
        "@mozilla.org/network/io-service;1"
      ].getService(Components.interfaces.nsIIOService);
      var channel = nsIoService.newChannelFromURI(
    ...
    newFileOutputStream: function (nsFile) {
      ...
      var fileOutputStream = Components.classes[
    ...
    writeBinaryInputStreamToFileOutputStream: function (
      binaryInputStream,
      fileOutputStream
    ) {
  ...
  // save to file
  var nsFile = Components.classes["@mozilla.org/file/local;1"].createInstance(
  ...
  try {
    ...
    loop.commandComplete = function (result) {
      ...
      if (this.pendingRollupCommands.length == 0) {
        ...
        LOG.info(
          "Rollup execution complete: " +
            (result.failed ? "failed! (see error messages below)" : "ok")
        );
