...
(function () {
  ...
  BaseExprHasPredicates.prototype.evaluatePredicates = function (
    nodeset,
    start
  ) {
  ...
  BinaryExpr = function (op, left, right, datatype) {
    ...
    this.needContextPosition =
      left.needContextPosition || right.needContextPosition;
    this.needContextNode = left.needContextNode || right.needContextNode;

    // Optimize [@id="foo"] and [@name="bar"]
    if (this.op == "=") {
      if (
        !right.needContextNode &&
        !right.needContextPosition &&
        right.datatype != "nodeset" &&
        right.datatype != "void" &&
        left.quickAttr
      ) {
        this.quickAttr = true;
        ...
        !left.needContextNode &&
        !left.needContextPosition &&
        left.datatype != "nodeset" &&
        left.datatype != "void" &&
        right.quickAttr
      ) {
        this.quickAttr = true;
  ...
  BinaryExpr.compare = function (op, comp, left, right, ctx) {
    ...
    if (left.isNodeSet && right.isNodeSet) {
      ...
      for (var i = 0, l0 = lnodes.length; i < l0; i++)
        for (var j = 0, l1 = rnodes.length; j < l1; j++)
          if (
            comp(
              NodeUtil.to("string", lnodes[i]),
              NodeUtil.to("string", rnodes[j])
            )
          )
            return true;
      return false;
    ...
    if (left.isNodeSet || right.isNodeSet) {
      if (left.isNodeSet) (nodeset = left), (primitive = right);
      else (nodeset = right), (primitive = left);

      nodes = nodeset.list();
      type = typeof primitive;
      for (var i = 0, l = nodes.length; i < l; i++) {
        if (comp(NodeUtil.to(type, nodes[i]), primitive)) return true;
      }
      return false;
    }
  ...
  BinaryExpr.parse = function (lexer) {
    ...
    while (true) {
      ...
      while (
        stack.length &&
        precedence <= this.ops[stack[stack.length - 1]][0]
      ) {
  ...
  UnaryExpr.parse = function (lexer) {
    ...
    else return UnionExpr.parse(lexer);
  };
  ...
  UnionExpr.parse = function (lexer) {
    ...
    if (!this.ops[lexer.peek()]) return expr;
  ...
  PathExpr.parse = function (lexer) {
    ...
    if (this.ops[lexer.peek()]) {
      ...
      if (!expr) {
        ...
        path.step("/", expr);
      } else if (!this.ops[lexer.peek()]) return expr;
      else path = new PathExpr(expr);
    }
...
@*/
      if (attrValue && attrName == "id" && node.getElementById) {
        ...
        for (var i = 0, l = nodes.length; i < l; i++) {
          ...
          if (
            uai.opera
              ? node.name == attrValue && test.match(node)
              : test.match(node)
          ) {
            nodeset.push(node);
          }
...
@*/
  ...
  Step.axises = {
    ancestor: [
      true,
      function (test, node, nodeset, _, __, prevNodeset, prevIndex) {
...
@else @*/
    ...
    "descendant-or-self": [
      ...
      function (
        ...
        return NodeUtil.getDescendantNodes(
          test,
          node,
          nodeset,
          attrName,
    ...
    following: [
      false,
      function (test, node, nodeset, attrName, attrValue) {
        do {
          var child = node;
          while ((child = child.nextSibling)) {
            ...
            nodeset = NodeUtil.getDescendantNodes(
              test,
              child,
              nodeset,
              attrName,
  ...
  Step.prototype.evaluate = function (ctx, special, prevNodeset, prevIndex) {
    ...
    if (!special && this.op == "//") {
      if (!this.needContextPosition && this.axis == "child") {
        if (this.quickAttr) {
          var attrValue = this.attrValueExpr
            ? this.attrValueExpr.string(ctx)
            : null;
          var nodeset = NodeUtil.getDescendantNodes(
            this.test,
            node,
            new NodeSet(),
            this.attrName,
...
@else @*/
  ...
  FunctionCall = function (name) {
    ...
    if (!info) throw Error(name + " is not a function");
...
@else @*/
  ...
  FunctionCall.parse = function (lexer) {
    var expr,
      func = new FunctionCall(lexer.next());
    lexer.next();
    while (lexer.peek() != ")") {
      if (lexer.empty()) {
  ...
  FunctionCall.prototype.arg = function (arg) {
    ...
    this.needContextNode =
      args.needContextNode || this.needContextNodeInfo[args.length];
  };
...
@*/
      ...
      nodes.sort(function (a, b) {
        ...
        if (a.compareDocumentPosition) {
          ...
          while (
            (ancestor1 = node1.parentNode) != (ancestor2 = node2.parentNode)
          ) {
            node1 = ancestor1;
            node2 = ancestor2;
          }
...
@else @*/
  NodeSet.prototype.reserveDelByNodeID = function (id, offset, reverse) {
    ...
    if ((index = map[id])) {
      if (
        (reverse && this.length - offset - 1 > index) ||
...
@else @*/
    ...
    this.length++;
    if (reverse) this.nodes.unshift(node);
