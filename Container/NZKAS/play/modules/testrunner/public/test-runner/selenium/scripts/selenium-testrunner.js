...
objectExtend(HtmlTestRunner.prototype, {
  ...
  loadSuiteFrame: function () {
    ...
    if (testSuiteName) {
      ...
      selenium.browserbot.baseUrl = absolutify(
        testSuiteName,
  ...
  _getSeparateApplicationWindow: function () {
    if (this.appWindow == null) {
      this.appWindow = openSeparateApplicationWindow(
  ...
  _onloadTestSuite: function () {
    ...
    if (this.controlPanel.isAutomatedRun()) {
      ...
      addLoadListener(
        this._getApplicationWindow(),
        fnBind(this._startSingleTest, this)
      );
      this._getApplicationWindow().src = this.controlPanel.getAutoUrl();
    } else {
      var testCaseLoaded = fnBind(function () {
      ...
      this.getTestSuite()
        .getSuiteRows()
        [testNumber].loadTestCase(testCaseLoaded);
    }
  },
  ...
  startTest: function () {
    ...
    storedVars.space = " ";
    this.currentTest = new HtmlRunnerTestLoop(
      testFrame.getCurrentTestCase(),
      this.metrics,
      this.commandFactory
    );
...
objectExtend(HtmlTestRunnerControlPanel.prototype, {
  initialize: function () {
    ...
    this.pauseButton.onclick = fnBindAsEventListener(
...
objectExtend(AbstractResultAwareRow.prototype, {
  ...
  setStatus: function (status) {
    this.unselect();
    this.trElement.className = this.trElement.className.replace(
      /status_[a-z]+/,
      ""
    );
    if (status) {
...
objectExtend(HtmlTestCaseRow.prototype, {
  getCommand: function () {
    return new SeleniumCommand(
      getText(this.trElement.cells[0]),
  ...
  isBreakpoint: function () {
    if (
      this.trElement.isBreakpoint == undefined ||
      this.trElement.isBreakpoint == null
    ) {
      return false;
    }
...
objectExtend(HtmlTestSuite.prototype, {
  ...
  _collectSuiteRows: function () {
    ...
    for (
      var tableNum = 1;
      tableNum < sel$A(this.suiteDocument.getElementsByTagName("table")).length;
      tableNum++
    ) {
  ...
  _startCurrentTestCase: function () {
    this.getCurrentRow().loadTestCase(
      fnBind(htmlTestRunner.startTest, htmlTestRunner)
    );
  },
...
objectExtend(SeleniumTestResult.prototype, {
  ...
  _saveToFile: function (fileName, form) {
    ...
    scriptFile.WriteLine(
      "<html><head><title>Test suite results</title><style>"
    );
...
objectExtend(HtmlTestCase.prototype, {
  initialize: function (testWindow, htmlTestSuiteRow) {
    ...
    this.headerRow = new TitleRow(
      this.testDocument.getElementsByTagName("tr")[0]
    );
...
var get_new_rows = function () {
  ...
  for (var i = 0; i < new_block.length; i++) {
    var new_source = new_block[i][0].tokenizer.source.slice(
      new_block[i][0].start,
      new_block[i][0].end
    );

    var row =
      '<td style="display:none;" class="js">getEval</td>' +
...
objectExtend(HtmlRunnerTestLoop.prototype, {
  initialize: function (htmlTestCase, metrics, seleniumCommandFactory) {
    this.commandFactory = new HtmlRunnerCommandFactory(
    ...
    if (this.expectedFailure != null) {
      ...
      if (!result.failed) {
        ...
        result.failureMessage =
          "Expected " + this.expectedFailureType + " did not occur.";
      } else {
        if (
          PatternMatcher.matches(this.expectedFailure, result.failureMessage)
        ) {
