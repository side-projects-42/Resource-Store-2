...
TestLoop.prototype = {
  ...
  _handleCommandError: function (e) {
    if (!e.isSeleniumError) {
      LOG.exception(e);
      var msg =
        "Command execution failure. Please search the forum at http://clearspace.openqa.org for error details from the log window.";
  ...
  continueTestWhenConditionIsTrue: function () {
    ...
    try {
      if (this.waitForCondition == null) {
        ...
        window.setTimeout(
          fnBind(this.continueTestWhenConditionIsTrue, this),
          10
        );
      }
    } catch (e) {
