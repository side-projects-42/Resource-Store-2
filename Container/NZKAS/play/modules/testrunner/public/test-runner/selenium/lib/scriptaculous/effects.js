...
Element.getOpacity = function (element) {
  ...
  if (
    (opacity = (Element.getStyle(element, "filter") || "").match(
      /alpha\(opacity=(.*)\)/
    ))
  )
...
Element.childrenWithClassName = function (element, className, findFirst) {
  ...
  var results = $A($(element).getElementsByTagName("*"))[
    findFirst ? "detect" : "select"
  ](function (c) {
...
Effect.Transitions.pulse = function (pos) {
  return Math.floor(pos * 10) % 2 == 0
    ? pos * 10 - Math.floor(pos * 10)
    : 1 - (pos * 10 - Math.floor(pos * 10));
};
