...
function buildDriverUrl() {
  ...
  return (
    s.substring(0, slashPairOffset + pathSlashOffset) +
...
function logToRc(logLevel, message) {
  if (debugMode) {
    ...
    sendToRCAndForget(
      "logLevel=" + logLevel + ":" + message.replace(/[\n\r\015]/g, " ") + "\n",
      "logging=true"
    );
  }
}
...
objectExtend(RemoteRunner.prototype, {
  ...
  nextCommand: function () {
    ...
    sendToRC(
      postResult,
      urlParms,
      fnBind(this._HandleHttpResponse, this),
...
function addUrlParams(url) {
  return (
    url +
    "&localFrameAddress=" +
    (proxyInjectionMode ? makeAddressToAUTFrame() : "top") +
...
function sendToRCAndForget(dataToBePosted, urlParams) {
  ...
  if (!(browserVersion.isChrome || browserVersion.isHTA)) {
    ...
    url =
      ...
      ".selenium.doesnotexist/selenium-server/driver/?" +
      urlParams;
  } else {
...
function getSeleniumWindowNameURLparameters() {
  ...
  if (w["seleniumWindowName"] == null) {
    if (w.name) {
      ...
      w["seleniumWindowName"] =
        "generatedSeleniumWindowName_" + Math.round(100000 * Math.random());
    }
  }
...
function makeAddressToAUTFrame(w, frameNavigationalJSexpression) {
  ...
  for (var j = 0; j < w.frames.length; j++) {
    var t = makeAddressToAUTFrame(
      w.frames[j],
      frameNavigationalJSexpression + ".frames[" + j + "]"
    );
    if (t != null) {
      return t;
    }
