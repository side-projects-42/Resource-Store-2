...
test("glob - stream", async (t) => {
	t.deepEqual((await getStream.array(globby.stream("*.tmp"))).sort(), [
		"a.tmp",
		"b.tmp",
		"c.tmp",
		"d.tmp",
...
test("glob with multiple patterns - stream", async (t) => {
	t.deepEqual(
		await getStream.array(globby.stream(["a.tmp", "*.tmp", "!{c,d,e}.tmp"])),
		["a.tmp", "b.tmp"]
	);
});

test("respect patterns order - stream", async (t) => {
	t.deepEqual(await getStream.array(globby.stream(["!*.tmp", "a.tmp"])), [
		"a.tmp",
	]);
});
...
test("gitignore option defaults to false - stream", async (t) => {
	const actual = await getStream.array(
		globby.stream("*", { onlyFiles: false })
	);
	t.true(actual.includes("node_modules"));
});
...
test("respects gitignore option true - stream", async (t) => {
	const actual = await getStream.array(
		globby.stream("*", { gitignore: true, onlyFiles: false })
	);
	t.false(actual.includes("node_modules"));
});
...
test("respects gitignore option false - stream", async (t) => {
	const actual = await getStream.array(
		globby.stream("*", { gitignore: false, onlyFiles: false })
	);
	t.true(actual.includes("node_modules"));
});
