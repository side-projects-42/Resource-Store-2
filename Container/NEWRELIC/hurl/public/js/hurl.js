  ...
  removeEmptyData: function (data) {
    ...
    for (key in data) {
      if ((value = data[key].value)) {
        if (
          $(
            "input[name=" + data[key].name + "].defaulted:not(.focused)"
          ).val() != value
        ) {
    ...
    for (key in keepers) data.push(keepers[key]);
  ...
  $("#hurl-form").submit(function () {
    ...
    $(this).hurlAjaxSubmit(function (res) {
      ...
      if (data.error) {
        ...
        if (data.prev_hurl) {
          $("#page-prev")
            .attr("href", "/hurls/" + data.prev_hurl)
            .show();
          $("#page-next").attr("href", "/").show();
        }
        $(".permalink").attr(
          "href",
          "/hurls/" + data.hurl_id + "/" + data.view_id
        );
