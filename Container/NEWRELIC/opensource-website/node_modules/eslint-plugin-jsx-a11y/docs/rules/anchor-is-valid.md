...
To properly function as a hyperlink, the `href` attribute should be present and also contain a valid *URL*. *JavaScript* strings, empty values or using only **\#** are not considered valid `href` values.
