...
stack-trace works great with [long-stack-traces](https://github.com/tlrobinson/long-stack-traces), when parsing an `err.stack` that has crossed the event loop boundary, a `CallSite` object returning `'----------------------------------------'` for `getFileName()` is created. All other methods of the event loop boundary call site return `null`.
...
The official v8 CallSite object API can be found [here](http://code.google.com/p/v8/wiki/JavaScriptStackTraceApi). A quick excerpt:
