...
<body>
<h1 id="jsdom-changelog">jsdom Changelog</h1>
<!-- Style guide:
...
<li>Fixed the <code>new File()</code> constructor to no longer convert <code>/</code> to <code>:</code>, per <a href="https://github.com/w3c/FileAPI/issues/41">a pending spec update</a>.</li>
...
<li>Fixed <code>&lt;input type=checkbox&gt;</code> and <code>&lt;input type=radio&gt;</code> to be mutable even when disabled, per <a href="https://github.com/whatwg/html/pull/5805">a spec update</a>.</li>
...
<h2 id="section-9">16.2.0</h2>
<ul>
<li>Added support for custom elements! Congratulations and thanks to <span class="citation" data-cites="pmdartus">[@pmdartus]</span>(https://github.com/jsdom/jsdom/commits?author=pmdartus) for making this happen, after ten months of hard work and lots of effort poured into the complex architectural prerequisites in jsdom and supporting packages.</li>
...
<h2 id="section-12">16.0.0</h2>
<p>For this release we’d like to welcome <span class="citation" data-cites="pmdartus">[@pmdartus]</span>(https://github.com/jsdom/jsdom/commits?author=pmdartus) to the core team. Among other work, he’s driven the heroic effort of constructor prototype and reform in jsdom and its dependencies over the last few months, to allow us to move away from shared constructors and prototypes, and set the groundwork for custom elements support (<a href="https://github.com/jsdom/jsdom/pull/2548">coming soon</a>!).</p>
...
<li>Set <code>canvas</code> as an optional `<code>peerDependency</code>, which apparently helps with Yarn PnP support.</li>
</ul>
<h2 id="section-15">15.1.1</h2>
<ul>
<li>Moved the <code>nonce</code> property from <code>HTMLScriptElement</code> and <code>HTMLStyleElement</code> to <code>HTMLElement</code>. Note that it is still just a simple reflection of the attribute, and has not been updated for the rest of the changes in <a href="https://github.com/whatwg/html/pull/2373">whatwg/html#2373</a>.</li>
...
<li>Fixed <code>XMLHttpRequest</code> reserializing <code>content-type</code> request headers unnecessarily. See <a href="https://github.com/whatwg/mimesniff/issues/84">whatwg/mimesniff#84</a> for more details. (thiagohirata)</li>
...
<li>Fixed event path iteration in shadow DOM cases, following spec fixes at <a href="https://github.com/whatwg/dom/pull/686">whatwg/dom#686</a> and <a href="https://github.com/whatwg/dom/pull/750">whatwg/dom#750</a>.</li>
...
<li>Updated the minimum <a href="https://www.npmjs.com/package/nwsapi"><code>nwsapi</code></a> version from v2.1.1 to v2.1.3, bringing along a few fixes in our selector engine.</li>
</ul>
<h2 id="section-19">14.0.0</h2>
<p>Breaking changes:</p>
<ul>
<li><code>JSDOM.fragment()</code> now creates fragments whose document has no <a href="https://html.spec.whatwg.org/multipage/#concept-document-bc">browsing context</a>, i.e. no associated <code>Window</code>. This means the <code>defaultView</code> property will be null, resources will not load, etc.</li>
...
<li>Upgraded our <a href="https://github.com/lddubeau/saxes"><code>saxes</code></a> dependency, so that it now correctly errors on XML fragments like <code>&lt;foo bar:="1"/&gt;</code>.</li>
</ul>
<h2 id="section-21">13.1.0</h2>
<ul>
...
<li>Fixed the performance of parsing large text nodes, particularly noticeable for large inline <code>&lt;style&gt;</code> or <code>&lt;script&gt;</code> elements. This regressed in v11.6.0. To learn more, see <a href="https://bugs.chromium.org/p/v8/issues/detail?id=6730#c4">V8 issue #6730</a>.</li>
...
<li>Fixed radio button group name matching to be case-sensitive, per <a href="https://github.com/whatwg/html/commit/6acdb2122298d2bb7bb839c0a61b4e1f9b0f9bc9">a spec update</a>.</li>
...
<h2 id="section-22">13.0.0</h2>
<p>Breaking change:</p>
<ul>
<li>Removed support for v1.x of the <a href="https://github.com/Automattic/node-canvas"><code>canvas</code></a> package, in favor of v2.x. This also removes support for <code>canvas-prebuilt</code>, since <code>canvas</code> v2.x has a built-in prebuilt version.</li>
</ul>
<p>Other changes:</p>
<ul>
<li>Added proper XML serialization, based on the <a href="https://github.com/jsdom/w3c-xmlserializer"><code>w3c-xmlserializer</code></a> package. Previously we were just using the HTML serialization, even in XML documents.</li>
...
<li>Fixed our implementation of the node tree modification constraints (for example the <a href="https://dom.spec.whatwg.org/#concept-node-ensure-pre-insertion-validity">ensure pre-insertion validity</a> algorithm). It is no longer possible to add, remove, or move nodes to create impossible DOM trees. (pmdartus)</li>
</ul>
<h2 id="section-23">12.2.0</h2>
<ul>
...
<p>This major release brings along our <a href="https://github.com/jsdom/jsdom#loading-subresources">new resource loader API</a>, finally bringing all the capabilities from jsdom v9 to the new (jsdom v10+) API. Thanks very much to <span class="citation" data-cites="sarvaje">[@sarvaje]</span>(https://github.com/sarvaje) for his work to make this possible!</p>
<p>Breaking changes:</p>
<ul>
<li>jsdom now requires Node.js v8.</li>
<li>Removed the old jsdom API, as the new API now has all the capabilities you need.</li>
<li>Updated our <a href="https://github.com/inikulin/parse5/"><code>parse5</code></a> dependency to v5, which changes the format of the node locations returned by <code>dom.nodeLocation()</code>.</li>
<li>Updated our <a href="https://github.com/jsdom/whatwg-url"><code>whatwg-url</code></a> dependency to v7, which changes the origin of <code>file:</code> URLs to be an opaque origin (and thus <code>file:</code> URLs are no longer same origin to each other).</li>
</ul>
<p>Other changes:</p>
<ul>
...
<li>Improved MIME type and <code>data:</code> URL parsing throughout jsdom, by using the new <a href="https://www.npmjs.com/package/whatwg-mimetype"><code>whatwg-mimetype</code></a> and <a href="https://www.npmjs.com/package/data-urls"><code>data-urls</code></a> packages.</li>
<li>Removed some unnecessary <code>.webidl</code> files that were included in the npm package.</li>
</ul>
<h2 id="section-32">11.6.2</h2>
<ul>
...
<p>Another regression remains where we are emitting spurious CSS-parsing <code>jsdomError</code> events; see <a href="https://github.com/jsdom/jsdom/issues/2123">#2123</a>. We also discovered a large amount of preexisting brokenness around <code>&lt;style&gt;</code>, <code>&lt;link&gt;</code>, and <code>@import</code>; see <a href="https://github.com/jsdom/jsdom/issues/2124">#2124</a> for more details.</p>
<p>We’ll try to fix these soon, especially the regression.</p>
<h2 id="section-34">11.6.0</h2>
<ul>
...
<li>Fixed many other small edge-case conformance issues in the API surface of various web APIs; see <a href="https://github.com/jsdom/jsdom/pull/2053">#2053</a> and <a href="https://github.com/jsdom/jsdom/pull/2081">#2081</a> for more information.</li>
...
<h2 id="section-36">11.4.0</h2>
<p>For this release we’d like to welcome <span class="citation" data-cites="Zirro">[@Zirro]</span>(https://github.com/jsdom/jsdom/commits?author=Zirro) to the core team; his contributions over the course of this year have enhanced jsdom immensely.</p>
<ul>
...
<h2 id="section-37">11.3.0</h2>
<p>For this release we’d like to formally welcome <span class="citation" data-cites="TimothyGu">[@TimothyGu]</span>(https://github.com/jsdom/jsdom/commits?author=TimothyGu) to the core team, as a prolific contributor. He will join the illustrious ranks of those who do so much work on jsdom that we no longer note their names in the changelog.</p>
<ul>
...
<p>This infrastructure will allow us to improve and implement many other similar behaviors; that work is being tracked in <a href="https://github.com/jsdom/jsdom/issues/1129">#1129</a>.</p>
...
<h2 id="section-48">9.8.3</h2>
<ul>
<li>Fixed syntax errors in Node.js v4.</li>
</ul>
<h2 id="section-49">9.8.2</h2>
<ul>
...
<p>Mutation observer support is <a href="https://github.com/jsdom/jsdom/issues/639">in progress</a>; please use the GitHub reactions feature to vote on that issue if you are impacted by this removal and are hoping for mutation observer support to replace it.</p>
...
<li>Added a basic implementation of the <a href="https://w3c.github.io/page-visibility/">Page Visibility API</a>, in particular a <code>document.hidden</code> property that always returns <code>true</code>, and a <code>document.visibilityState</code> property that always returns <code>"prerender"</code>. This is a more standard alternative to our proprietary <code>navigator.noUI</code>, which will be removed whenever we release v9.0.0. (kapouer)</li>
</ul>
<h2 id="section-72">8.2.0</h2>
<ul>
...
<h2 id="section-82">7.2.0</h2>
<ul>
<li>Added support for text selection APIs on <code>&lt;input&gt;</code> and <code>&lt;textarea&gt;</code>! (sjelin and yaycmyk)</li>
<li>Replaced our default XML parser with <a href="https://www.npmjs.com/package/sax">sax</a>, thus fixing many (but not all) issues with XML and XHTML parsing. To get a flavor of the issues fixed, check out these now-closed bugs: <a href="https://github.com/jsdom/jsdom/issues/393">#393</a>, <a href="https://github.com/jsdom/jsdom/issues/651">#651</a>, <a href="https://github.com/jsdom/jsdom/issues/415">#415</a>, <a href="https://github.com/jsdom/jsdom/issues/1276">#1276</a>.</li>
...
<p>Finally, if you’re a loyal jsdom fan whose made it this far into the changelog, I’d urge you to come join us in <a href="https://github.com/jsdom/jsdom/issues/1139">#1139</a>, where we are brainstorming a modernized jsdom API that could get rid of many of the warts in the current one.</p>
<h2 id="section-96">5.6.1</h2>
<ul>
...
<h2 id="section-97">5.6.0</h2>
<ul>
<li><code>virtualConsole.sendTo</code> now returns <code>this</code>, allowing for <a href="https://github.com/jsdom/jsdom/tree/60ccb9b318d0bae8fe37e19af5af444b9c98ddac#forward-a-windows-console-output-to-the-iojs-console">a nice shorthand</a>. (jeffcarp)</li>
</ul>
<h2 id="section-98">5.5.0</h2>
<ul>
...
<ul>
<li>Elements with weird tag names, of the type that only the parser can normally create, can now be cloned (<a href="https://github.com/jsdom/jsdom/issues/1142">#1142</a>)</li>
...
<ul>
<li>Notably, this makes jQuery’s <code>show()</code> and <code>hide()</code> methods now work correctly; see <a href="https://github.com/jsdom/jsdom/issues/994">#994</a>.</li>
</ul></li>
...
<li>Some weirdnesses around the <code>style</code> attribute were fixed along the way; see e.g. <a href="https://github.com/jsdom/jsdom/issues/1109">#1109</a>.</li>
...
<h2 id="section-103">5.3.0</h2>
<ul>
<li>Added a <code>virtualConsole</code> option to the document creation methods, along with the <code>jsdom.createVirtualConsole</code> factory. (See <a href="https://github.com/jsdom/jsdom/blob/dbf88666d1152576237ed1c741263f5516bb4005/README.md#capturing-console-output">examples in the readme</a>.) With this option you can install a virtual console before the document is even created, thus allowing you to catch any virtual console events that occur during initialization. (jeffcarp)</li>
</ul>
<h2 id="section-104">5.2.0</h2>
<ul>
...
<p>Finally, it’s worth noting that we now delegate our cookie handling in general to the <a href="https://www.npmjs.com/package/tough-cookie">tough-cookie</a> package, which should hopefully mean that it now captures many of the behaviors that were previously missing (for example <a href="https://github.com/jsdom/jsdom/issues/1027">#1027</a>). <span class="citation" data-cites="inikulin">@inikulin</span> is working on <a href="https://github.com/goinstant/tough-cookie/pull/30">a large pull request to fix tough-cookie to be more spec compliant</a>, which should automatically be picked up by jsdom installs once it is merged.</p>
<h2 id="section-108">4.5.1</h2>
<ul>
<li>Removed unnecessary browserify dependency that was erroneously included in 4.5.0.</li>
</ul>
<h2 id="section-109">4.5.0</h2>
<ul>
<li>Added <code>document.currentScript</code>. (jeffcarp)</li>
</ul>
<h2 id="section-110">4.4.0</h2>
<ul>
...
<p>Note that as of this release, jsdom no longer works with Node.js™, and instead requires io.js. You are still welcome to install a release in <a href="https://github.com/jsdom/jsdom/tree/3.x">the 3.x series</a> if you are stuck on legacy technology like Node.js™.</p>
...
<li>Fixed a couple small issues while browserifying, mainly around <code>jsdom.env</code>. However, while doing so discovered that <code>&lt;script&gt;</code>s in general don’t work too well in a browserified jsdom; see <a href="https://github.com/jsdom/jsdom/issues/1023">#1023</a>.</li>
</ul>
<h2 id="section-122">3.1.0</h2>
<ul>
<li>Added support for <a href="https://github.com/jsdom/jsdom#custom-external-resource-loader">custom external resource loading</a>. (tobie)</li>
</ul>
<h2 id="section-123">3.0.3</h2>
<ul>
...
<li>Introduced a new “virtual console” abstraction for capturing console output from inside the page. <a href="https://github.com/jsdom/jsdom#capturing-console-output">See the readme for more information.</a> Note that <code>console.error</code> will no longer contribute to the (non-standard, and likely dying in the future) <code>window.errors</code> array. (jeffcarp)</li>
...
<h2 id="section-128">1.5.0</h2>
<ul>
<li>Add: missing <code>window.console</code> methods, viz. <code>assert</code>, <code>clear</code>, <code>count</code>, <code>debug</code>, <code>group</code>, <code>groupCollapse</code>, <code>groupEnd</code>, <code>table</code>, <code>time</code>, <code>timeEnd</code>, and <code>trace</code>. All except <code>assert</code> do nothing for now, but see <a href="https://github.com/jsdom/jsdom/issues/979">#979</a> for future plans. (jeffcarp)</li>
<li>Tweak: make <code>childNodes</code>, and the many places in jsdom that use it, much faster. (Joris-van-der-Wel)</li>
</ul>
<h2 id="section-129">1.4.1</h2>
<ul>
...
<h2 id="section-142">1.0.0</h2>
<p>For a consolidated list of changes from 0.11.1 to 1.0.0, see <a href="https://github.com/jsdom/jsdom/wiki/Changes-from-0.11.1-to-1.0.0">this wiki page</a>.</p>
<ul>
...
<li>An overhaul of the <a href="https://github.com/jsdom/jsdom#initialization-lifecycle">initialization lifecycle</a>, to bring more control and address common use cases. (Sebmaster)</li>
...
<li>As part of the new parser story, we are not supporting XML for now. It might work if you switch to a different parser (e.g. htmlparser2), but in the end, HTML and XML are very different, and we are not attempting to be an XML DOM. That said, we eventually want to support XML to the same extent browsers do (i.e., support XHTML and SVG, with an appropriate MIME type switch); this is being planned in <a href="https://github.com/jsdom/jsdom/issues/820">#820</a>.</li>
</ul>
