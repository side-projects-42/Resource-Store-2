...
async function installDependencies(kind, dependencyList) {
  ...
  await exec.exec(
    `sudo apt-get install -y --no-install-recommends ${dependencyList}`
  );
...
async function installBuildDependencies() {
  const dependencyList =
    "libyaml-dev libgdbm-dev libreadline-dev libncurses5-dev zlib1g-dev libffi-dev";
...
async function installRubyBuild(rubyVersion) {
  ...
  if (usesOldOpenSsl(rubyVersion)) {
    ...
    repoPath =
      "--branch ruby23-openssl-linux https://github.com/eregon/ruby-build.git";
...
async function downgradeMySQL() {
  ...
  const mirrorUrl =
    "https://security.debian.org/debian-security/pool/updates/main/m/mysql-5.5/";
  ...
  const promise2 = exec.exec("wget", [
    pkgOption,
    `${mirrorUrl}/libmysqlclient18_5.5.62-0%2Bdeb8u1_amd64.deb`,
  ]);
  const promise3 = exec.exec("wget", [
    pkgOption,
    `${mirrorUrl}/libmysqlclient-dev_5.5.62-0%2Bdeb8u1_amd64.deb`,
  ]);
  const promise4 = exec.exec("wget", [
    pkgOption,
    `${ubuntuUrl}/g/glibc/multiarch-support_2.27-3ubuntu1.2_amd64.deb`,
  ]);
  ...
  await exec.exec("sudo", [
    "dpkg",
    "-i",
    `${pkgDir}/multiarch-support_2.27-3ubuntu1.2_amd64.deb`,
  ]);
  await exec.exec("sudo", [
    "dpkg",
    "-i",
    `${pkgDir}/libmysqlclient18_5.5.62-0+deb8u1_amd64.deb`,
  ]);
  await exec.exec("sudo", [
    "dpkg",
    "-i",
    `${pkgDir}/libmysqlclient-dev_5.5.62-0+deb8u1_amd64.deb`,
  ]);
...
async function buildRuby(rubyVersion) {
  ...
  await exec.exec(
    `ruby-build --verbose ${rubyVersion} ${rubyPath(rubyVersion)}`
  );
  core.endGroup();
}
...
async function upgradeRubyGems(rubyVersion) {
  ...
  await execute("gem --version").then((res) => {
    gemVersionStr = res;
  });
  ...
  if (parseFloat(rubyVersion) < 2.7) {
    if (parseFloat(gemVersionStr) < 3.0) {
      ...
      await exec
        .exec("gem", ["update", "--system", "3.0.6", "--force"])
        .then((res) => {
          exitCode = res;
        });
      if (exitCode != 0) {
  ...
  await execute("which gem").then((res) => {
    core.info("which gem: " + res);
  });
  await execute("gem --version").then((res) => {
    core.info("New RubyGems is: " + res);
  });
...
async function installBundler(rubyVersion) {
  ...
  if (!fs.existsSync(`${rubyBinPath}/bundle`)) {
    ...
    await execute("bundle --version").then((res) => {
      bundleVersionStr = res;
    });
...
async function setupEnvironment(rubyVersion, dependencyList) {
  const systemDependencyList =
    "libcurl4-nss-dev build-essential libsasl2-dev libxslt1-dev libxml2-dev";
...
function bundleCacheKey(rubyVersion) {
  const keyHash = fileHash(
    `${process.env.GITHUB_WORKSPACE}/newrelic_rpm.gemspec`
  );
...
async function main() {
  ...
  try {
    ...
    if (isRubyBuilt(rubyVersion) && isBuildJob()) {
      return;
    }
