...
CodeHighlighter.init = function () {
  ...
  if (
    ...
    return; // throw out Safari versions that don't support replace function
  ...
  function parse(text, ignoreCase) {
    ...
    return text.replace(
      new RegExp(rules, ignoreCase ? "gi" : "g"),
      function () {
        var i = 0,
          j = 1,
          rule;
  ...
  function highlightCode(styleSet) {
    ...
    var stylableEls = codeEls.filter(function (item) {
    ...
    for (var i = 0; i < stylableEls.length; i++) {
      ...
      if (
        ...
        stylableEls[i].parentNode.nodeName == "PRE"
      ) {
        stylableEls[i] = stylableEls[i].parentNode;

        parsed = stylableEls[i].innerHTML.replace(
          /(<code[^>]*>)([^<]*)<\/code>/i,
          function () {
            return (
              arguments[1] +
              parse(arguments[2], styleSet.ignoreCase) +
              "</code>"
            );
          }
        );
