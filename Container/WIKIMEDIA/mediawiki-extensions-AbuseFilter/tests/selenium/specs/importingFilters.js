...
describe("When importing a filter", function () {
  ...
  describe("Data on the editing interface is correct", function () {
    it("filter specs are copied", function () {
      ...
      assert.strictEqual(
        ViewEditPage.comments.getValue(),
        filterSpecs.comments + "\n"
      );
      assert.strictEqual(
        ViewEditPage.rules.getValue(),
        filterSpecs.rules + "\n"
      );
    });
    it("filter flags are copied", function () {
      assert.strictEqual(
        ViewEditPage.enabled.isSelected(),
        !!filterSpecs.enabled
      );
      assert.strictEqual(
        ViewEditPage.hidden.isSelected(),
        !!filterSpecs.hidden
      );
      assert.strictEqual(
        ViewEditPage.deleted.isSelected(),
        !!filterSpecs.deleted
      );
    });
    it("filter actions are copied", function () {
      ...
      assert.strictEqual(
        ViewEditPage.warnOtherMessage.getValue(),
