...
function _exec(win, fail, clazz, action, args) {
  ...
  try {
    ...
    origResult =
      "" +
      org.apache.cordova.JavaPluginManager.exec(
        clazz,
        action,
        callbackId,
        JSON.stringify(args),
    ...
    if (origResult.length > 0) {
      ...
      execResult = { status: cordova.callbackStatus.NO_RESULT, message: "" };
    }
  } catch (e) {
    console.log("BlackBerryPluginManager Error: " + e);
    execResult = { status: cordova.callbackStatus.ERROR, message: e.message };
  }
