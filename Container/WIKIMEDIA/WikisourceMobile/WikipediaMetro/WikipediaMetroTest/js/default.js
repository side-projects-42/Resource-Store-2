...
(function () {
  ...
  (function () {
    ...
    HubContents.groupedList = HubContents.itemList.createGrouped(
      getGroupKey,
      getGroupData,
      compareGroups
    );
  ...
  function setupApp() {
    return new WinJS.Promise(function (complete, error, prog) {
      ...
      function doStuff() {
        ...
        if ($("#appbar").length) {
          $("#readInCmd")[0].winControl.label = mediaWiki
            .message("menu-language")
            .plain();
          $("#pinCmd")[0].winControl.label = mediaWiki
            .message("menu-win8-pin")
            .plain();
          $("#unpinCmd")[0].winControl.label = mediaWiki
            .message("menu-win8-unpin")
            .plain();
          $("#browserCmd")[0].winControl.label = mediaWiki
            .message("menu-open-browser")
            .plain();
        }
        $("#offline").localize();
        ...
        $("#appbar").bind("beforeshow", function () {
          ...
          if (state.current().type == "article") {
            if (
              Windows.UI.StartScreen.SecondaryTile.exists(tileId(lang, title))
            ) {
              $("#pinCmd").hide();
              $("#unpinCmd").show();
            } else {
        ...
        $("#pinCmd").click(function () {
          var lang = state.current().lang,
            ...
            uriLogo = new Windows.Foundation.Uri(
              "ms-appx:///images/secondary-tile.png"
            ),
            tileActivationArguments =
              "lang=" + lang + "&" + "title=" + encodeURIComponent(title),
          ...
          tile
            .requestCreateAsync({ x: selectionRect.left, y: selectionRect.top })
    ...
    $.ajax({
      ...
      success: function (data, textstatus, request) {
        if (data && data instanceof Array) {
          ...
          if (suggestions) {
            suggestionRequest.searchSuggestionCollection.appendQuerySuggestions(
              suggestions
            );
  ...
  function updateLiveTile(title, content) {
    ...
    tileNotification.expirationTime = new Date(
      currentTime.getTime() + 600 * 1000
    );
    Notifications.TileUpdateManager.createTileUpdaterForApplication().update(
      tileNotification
    );
  }
  ...
  function initHub(lang) {
    ...
    fetchFeed(lang, "featured", function (htmlList, err) {
      ...
      if (htmlList.length) {
        ...
        updateLiveTile(
          mediaWiki.message("win8-tile-featured-article").plain(),
          txt
        );
      }
  ...
  function showHistoryMenu(anchor) {
    ...
    state.stack
      ...
      .forEach(function (item, n) {
        ...
        if (item.type == "hub") {
          label =
            mediaWiki.message("sitename").plain() + " (" + item.lang + ")";
