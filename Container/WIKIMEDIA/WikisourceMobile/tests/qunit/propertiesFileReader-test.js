...
test("Can parse very simple input?", function () {
  parseEqual("Single line, no newline", "key=value", {
    key: "value",
  });
  parseEqual(
...
test("Can ignore blank/empty lines?", function () {
  parseEqual("Single line with newline", "key=value\n", {
    key: "value",
  });
  parseEqual(
...
test("Key format", function () {
  ...
  keyEqual("Simple key", "key=value");
  keyEqual("Simple key with preceding whitespace", "   key=value");
  keyEqual("Simple key with preceding whitespace (tab)", "\tkey=value");
  keyEqual("Simple key with following whitespace", "key   =value");
  keyEqual(
    "Simple key with preceding and following whitespace",
    "  key   =value"
  );
  keyEqual("Key with : as sep", "key:value");
  keyEqual("Key with space as sep", "key value");
});
...
test("Empty value with no separator; doc example", function () {
  parseEqual("Line with no value & no separator", "cheeses", {
    cheeses: "",
  });
});
...
test("Escapes", function () {
  ...
  parseEqual("\\t", "key=Foo\\tbar", {
    key: "Foo\tbar",
  });
  parseEqual("\\\\", "key=Foo\\\\bar", {
    key: "Foo\\bar",
  });
  parseEqual("\\b (b, not backspace)", "key=Foo\\bbar", {
    key: "Foobbar",
  });
  parseEqual("\\z (z, not ctrl+z)", "key=Foo\\zbar", {
    key: "Foozbar",
  });
  parseEqual("\\n (multiple)", "key=Foo\\nbar\\nbaz\\nbing", {
    key: "Foo\nbar\nbaz\nbing",
