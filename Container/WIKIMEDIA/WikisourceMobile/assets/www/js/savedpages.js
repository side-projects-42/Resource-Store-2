window.savedPages = (function () {
                ...
                if (index === 1) {
                  ...
                  function saveNextPage(curPage) {
                    $("#migration-status").html(
                      mw.msg(
                        "migrating-saved-page-status",
                        pages[curPage].title
                      )
                    );
                    app
                      .navigateToPage(pages[curPage].key)
                      .done(function () {
                        savedPages
                          .saveCurrentPage({ silent: true })
                          .done(function () {
                  ...
                  navigator.notification.alert(
                    mw.msg("migrating-saved-pages-confirm-cancel")
                  );
                  d.resolve(); // Not failure.
                }
              },
              mw.msg("migrating-saved-pages-confirm-title"),
              mw.msg("confirm-button-yes") +
                "," +
                mw.msg("confirm-button-not-now")
            );
          } else {
            d.resolve();
          }
      ...
      this.keys(function (records) {
        if (records != null) {
          if (records.length > MAX_LIMIT) {
            ...
            savedPagesDB.save({
              key: app.curPage.getAPIUrl(),
              title: title,
              lang: app.curPage.lang,
  ...
  function deleteSavedPage(title, url) {
    chrome
      .confirm(mw.message("saved-page-remove-prompt", title).plain())
      .done(function (answer) {
        if (answer) {
          var savedPagesDB = new Lawnchair(
            { name: "savedPagesDB" },
            function () {
              this.remove(url, function () {
                chrome.showNotification(
                  mw.message("saved-page-removed", title).plain()
                );
  ...
  function onClearSavedPages() {
    chrome
      .confirm(mw.message("clear-all-saved-pages-prompt").plain())
      .done(function (answer) {
        if (answer) {
          var savedPagesDB = new Lawnchair(
            { name: "savedPagesDB" },
    ...
    var savedPagesDB = new Lawnchair({ name: "savedPagesDB" }, function () {
      this.all(function (savedpages) {
        $.each(savedpages, function (i, page) {
          ...
          if (
            page.version !== SAVED_PAGES_VERSION &&
            typeof page.lang === "undefined"
          ) {
            page.disabled = true;
          }
        });
        ...
        $("#savedPages .cleanButton")
          .unbind("click", onClearSavedPages)
          .bind("click", onClearSavedPages);
