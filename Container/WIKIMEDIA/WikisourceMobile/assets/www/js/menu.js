...
function updateMenuState() {
  ...
  $("#menu").remove();
  var $menu = $("<div>");
  $menu.attr("id", "menu").appendTo("body");

  $.each(menu_items, function (i, item) {
    var $button = $("<button>");
    $button.attr("id", item.id).attr("title", mw.msg(item.id));
    if (item.disabled) {
    ...
    $button.append("<span>").appendTo($menu);
  });
...
function popupMenu(items, callback, options) {
  options = $.extend(
    { destructiveButtonIndex: null, cancelButtonIndex: null },
    options || {}
  );
...
function showListActions(origin) {
  popupMenu(
    [mw.msg("menu-savedPages"), mw.msg("menu-history"), mw.msg("menu-cancel")],
    function (val, index) {
      if (index == 0) {
