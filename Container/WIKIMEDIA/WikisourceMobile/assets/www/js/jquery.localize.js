...
(function ($) {
  ...
  $.fn.localize = function (options) {
    ...
    function msg(key) {
      ...
      args.unshift(
        options.prefix + (key in options.keys ? options.keys[key] : key)
      );
    ...
    return (
      $(this)
        ...
        .each(function () {
          var $el = $(this);
          $el.text(msg($el.attr("key"))).replaceWith($el.html());
        })
        .end()
        .find("[title-msg]")
        .each(function () {
          var $el = $(this);
          $el.attr("title", msg($el.attr("title-msg"))).removeAttr("title-msg");
        })
        .end()
        .find("[alt-msg]")
        .each(function () {
          var $el = $(this);
          $el.attr("alt", msg($el.attr("alt-msg"))).removeAttr("alt-msg");
        })
        .end()
