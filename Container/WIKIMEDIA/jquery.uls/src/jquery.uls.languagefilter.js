...
(function ($) {
  ...
  LanguageFilter.prototype = {
    ...
    searchAPI: function (query) {
      return $.get(this.options.searchAPI, { search: query }).then(
        function (result) {
          var autofillLabel,
            results = [];
          ...
          $.each(
            result.languagesearch,
            function (apiCode, name) {
              var code, redirect;
    ...
    resultHandler: function (query, results, autofillLabel) {
      if (results.length === 0) {
        this.$suggestion.val("");
        this.$element.trigger("noresults.uls", {
          query: query,
    ...
    autofill: function (langCode, languageName) {
      ...
      if (suggestion.toLowerCase() !== languageName.toLowerCase()) {
        ...
        suggestion =
          userInput + autonym.substring(userInput.length, autonym.length);
  ...
  $.fn.languagefilter = function (option) {
    return this.each(function () {
      ...
      if (!data) {
        $this.data(
          "languagefilter",
          (data = new LanguageFilter(this, options))
        );
      }
