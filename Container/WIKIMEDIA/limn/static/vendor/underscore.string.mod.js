require.define(
  "/node_modules/underscore.string.js",
  function (require, module, exports, __dirname, __filename, undefined) {
    // Underscore.string
      ...
      var _s = {
        ...
        humanize: function (str) {
          return _s.capitalize(
            this.underscored(str).replace(/_id$/, "").replace(/_/g, " ")
          );
        },
        ...
        prune: sArgs(function (str, length, pruneStr) {
          ...
          if (template.substring(length - 1, length + 1).search(/^\w\w$/) === 0)
            pruned = _s.rtrim(
              template.slice(0, length).replace(/([\W][\w]*)$/, "")
            );
          else pruned = _s.rtrim(template.slice(0, length));

          pruned = pruned.replace(/\W+$/, "");

          return pruned.length + pruneStr.length > str.length
            ? str
            : str.substring(0, pruned.length) + pruneStr;
        }),
        ...
        toNumber: function (str, decimals) {
          var num = parseNumber(
            parseNumber(str).toFixed(parseNumber(decimals))
          );
        ...
        strRight: sArgs(function (sourceStr, sep) {
          ...
          return pos != -1
            ? sourceStr.slice(pos + sep.length, sourceStr.length)
            : sourceStr;
        }),
