...
function builder(saw, xs) {
  ...
  function action(step, key, start, finish) {
    ...
    Hash(context).forEach(function (v, k) {
      cb[k] = v;
    });
  ...
  ["seq", "par"].forEach(function (name) {
    this[name + "_"] = function (key) {
      ...
      var cb = typeof key === "function" ? args[0] : args[1];
  ...
  this.forEach = function (limit, cb) {
    ...
    this.seq(function () {
      if (context.stack.length === 0) return this(null);
      ...
      if (!limit || limit <= 0)
        ...
        visitor = function eachCall(x, i) {
          if (active >= limit) return queue.push(eachCall.bind(this, x, i));
  ...
  this.seqMap = function (cb) {
    ...
    this.seqEach(function (x, i) {
      ...
      var next = function () {
        res[i] = arguments[1];
        if (i === lastIdx) context.stack = res;
        self.apply(self, arguments);
      };
  ...
  this.parFilter = function (limit, cb) {
    ...
    Seq()
      ...
      .seq(function () {
        context.stack = res.sort().map(function (pair) {
          return pair[1];
        });
        saw.next();
      });
  ...
  ["forEach", "Each", "Map", "Filter"].forEach(function (name) {
  ...
  ["push", "pop", "shift", "unshift", "splice", "reverse"].forEach(function (
    name
  ) {
    this[name] = function () {
      context.stack[name].apply(context.stack, slice.call(arguments));
      saw.next();
      return this;
    };
  },
  ...
  ["map", "filter", "reduce"].forEach(function (name) {
    this[name] = function () {
      var res = context.stack[name].apply(context.stack, slice.call(arguments));
