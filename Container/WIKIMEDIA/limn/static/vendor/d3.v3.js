(function () {
  ...
  d3.xhr = function (url, mimeType, callback) {
    var xhr = {},
      ...
      request = new (
        window.XDomainRequest && /^(http(s)?:)?\/\//.test(url)
          ? XDomainRequest
          : XMLHttpRequest
      )();
    "onload" in request
  ...
  d3_transitionPrototype.styleTween = function (name, tween, priority) {
    ...
    return this.tween("style." + name, function (d, i) {
      var f = tween.call(
        ...
        d,
        i,
        getComputedStyle(this, null).getPropertyValue(name)
      );
      return (
        f &&
        function (t) {
  ...
  d3.geo.greatArc = function () {
    ...
    greatArc.source = function (_) {
      ...
      interpolate =
        source_ && target_ ? d3.geo.interpolate(source_, target_) : null;
      return greatArc;
    };
  ...
  var d3_geo_area = {
    ...
    polygonEnd: function () {
      d3_geo_areaSum +=
        d3_geo_areaRing < 0 ? 4 * π + d3_geo_areaRing : d3_geo_areaRing;
  ...
  function d3_geo_areaRingStart() {
    ...
    function nextPoint(λ, φ) {
      ...
      if (
        Math.abs(Math.abs(φ0) - π / 2) < ε &&
        Math.abs(Math.abs(φ) - π / 2) < ε
      )
        return;
      var cosφ = Math.cos(φ),
        sinφ = Math.sin(φ);
      if (Math.abs(φ0 - π / 2) < ε) d3_geo_areaRing += (λ - λ1) * 2;
        ...
        var dλ = λ - λ0,
          ...
          d = Math.atan2(
            Math.sqrt(
              (d = cosφ * Math.sin(dλ)) * d +
                (d = cosφ0 * sinφ - sinφ0 * cosφ * cosdλ) * d
            ),
