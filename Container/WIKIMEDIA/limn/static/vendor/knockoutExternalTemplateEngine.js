...
*/
  ...
  var errorHtml =
      "<div class='infuser-error'>The template <a href='{TEMPLATEURL}'>{TEMPLATEID}</a> could not be loaded. {STATUS}</div>",
    returnErrorTemplate = function (status, templateId, templatePath) {
      return errorHtml
        .replace("{STATUS}", status)
        .replace("{TEMPLATEID}", templateId)
        .replace("{TEMPLATEURL}", templatePath);
    },
    errors = [];
  var helpers = {
      getTemplatePath: function (templateOptions) {
        var templateFile =
          templateOptions.templatePrefix +
          templateOptions.templateId +
          templateOptions.templateSuffix;
        return templateOptions.templateUrl === undefined ||
          templateOptions.templateUrl === ""
          ? templateFile
      ...
      templateGetError: function (templateId, templatePath, callback) {
        return function (exception) {
          ...
          var templateHtml = returnErrorTemplate(
            "HTTP Status code: " + exception.status,
            templateId,
            templatePath
          );
  ...
  var infuser = {
    ...
    get: function (options, callback) {
      var templateOptions = $.extend(
          {},
          infuser.defaults,
          typeof options === "object" ? options : { templateId: options }
        ),
        template;
      ...
      if (!template || $.inArray(templateOptions.ajax.url, errors) !== -1) {
        templateOptions.ajax.success = helpers.templateGetSuccess(
          templateOptions.ajax.url,
          callback
        );
        templateOptions.ajax.error = helpers.templateGetError(
          templateOptions.templateId,
          templateOptions.ajax.url,
          callback
        );
    ...
    getSync: function (options) {
      ...
      if (!template || $.inArray(templateOptions.ajax.url, errors) !== -1) {
        ...
        templateOptions.ajax.error = function (exception) {
          ...
          templateHtml = returnErrorTemplate(
            "HTTP Status code: exception.status",
            templateOptions.templateId,
            templateOptions.ajax.url
          );
    ...
    infuse: function (templateId, renderOptions) {
      var templateOptions = $.extend(
          {},
          infuser.defaults,
          typeof templateId === "object" ? templateId : renderOptions,
          typeof templateId === "string"
            ? { templateId: templateId }
            : undefined
        ),
        targetElement =
          typeof templateOptions.target === "function"
            ? templateOptions.target(templateId)
            : templateOptions.target;
      if (templateOptions.useLoadingTemplate) {
        templateOptions.loadingTemplate.transitionIn(
          targetElement,
          templateOptions.loadingTemplate.content
        );
      }
      infuser.get(templateOptions, function (template) {
        var _template = template;
        templateOptions.preRender(targetElement, _template);
        _template = templateOptions.bindingInstruction(
          _template,
...
(function (global, ko, jQuery, infuser, undefined) {
  var ExternalTemplateSource = function (templateId, options) {
    ...
    self.template = ko.observable(
      infuser.defaults.useLoadingTemplate
  ...
  var KoExternalTemplateEngine = function (koEngineType) {
      ...
      if (typeof template == "string") {
        ...
        if (elem) return new ko.templateSources.domElement(elem);
          ...
          if (!engine.templates[template]) {
            engine.templates[template] = new ExternalTemplateSource(
              template,
              options
            );
          }
    ...
    engine.renderTemplate = function (template, bindingContext, options) {
      var templateSource = engine.makeTemplateSource(
        template,
        bindingContext,
        options
      );
      return engine.renderTemplateSource(
        templateSource,
        bindingContext,
        options
      );
  ...
  if (
    ...
    ko.setTemplateEngine(
      new KoExternalTemplateEngine(ko.jqueryTmplTemplateEngine)
    );
  } else {
