(function ($, undefined) {
  ...
  $.trafficCop = function (url, options) {
    var reqOptions = url,
      key;
    arguments.length === 2 &&
      (reqOptions = $.extend(!0, options, { url: url })),
  ...
  (function ($, global, undefined) {
    var hashStorage = {
        ...
        purge: function () {
          for (var i = 0; i < this.templateIds.length; i++)
            document.body.removeChild(
              document.getElementById(this.templateIds[i])
            );
          this.templateIds = [];
        },
      },
      ...
      helpers = {
        ...
        templateGetError: function (templateId, templatePath, callback) {
          return function (exception) {
            $.inArray(templateId, errors) === -1 && errors.push(templateId);
      ...
      infuser = {
        storageOptions: { hash: hashStorage, script: scriptStorage },
        store: hashStorage,
        defaults: {
          templateUrl: "",
          templateSuffix: ".html",
          templatePrefix: "",
          ajax: { async: !0, dataType: "html", type: "GET" },
          ...
          loadingTemplate: {
            ...
            transitionIn: function (target, content) {
              var tgt = $(target);
              tgt.hide(), tgt.html(content), tgt.fadeIn();
            },
        ...
        get: function (options, callback) {
          ...
          (templateOptions.ajax.url = helpers.getTemplatePath(templateOptions)),
            ...
            !template || $.inArray(templateOptions.ajax.url, errors) !== -1
          ...
          if (!template || $.inArray(templateOptions.ajax.url, errors) !== -1)
            (templateHtml = null),
              ...
              (templateOptions.ajax.error = function (exception) {
                $.inArray(templateOptions.ajax.url) === -1 &&
              ...
              templateHtml === null
                ...
                : (infuser.store.storeTemplate(
                    templateOptions.ajax.url,
                    templateHtml
                  ),
                  (template = infuser.store.getTemplate(
                    templateOptions.ajax.url
                  )));
          return template;
        },
        infuse: function (templateId, renderOptions) {
          ...
          templateOptions.useLoadingTemplate &&
  ...
  (function (global, ko, jQuery, infuser, undefined) {
      ...
      (self.templateId = templateId),
        ...
        self.options &&
          ...
          (self.options.afterRender = function () {
            self.loaded && origAfterRender.apply(self.options, arguments);
          }));
    };
    ko.utils.extend(ExternalTemplateSource.prototype, {
      data: function (key, value) {
        if (arguments.length === 1)
          return (
            key === "precompiled" && this.template(), this.template.data[key]
          );
        this.template.data[key] = value;
      },
      text: function (value) {
        this.loaded || this.getTemplate();
    ...
    var KoExternalTemplateEngine = function (koEngineType) {
      ...
      return (
        (engine.templates = {}),
        (engine.makeTemplateSource = function (
          template,
          bindingContext,
          options
        ) {
    ...
    (ko.KoExternalTemplateEngine = KoExternalTemplateEngine),
      jQuery.tmpl && jQuery.tmpl.tag.tmpl.open.toString().indexOf("__") >= 0
