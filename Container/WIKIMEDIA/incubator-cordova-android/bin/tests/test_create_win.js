...
var version = fs
  .readFileSync(build_path + "/VERSION")
  .toString()
  .replace("\r\n", "");
...
create_project.on("exit", function (code) {
  ...
  // });
  path.exists(
    build_path + util.format("/framework/assets/cordova-%s.js", version),
    function (exists) {
      assert(!exists, "javascript file did not get cleaned up");
    }
  );
  path.exists(
    build_path + util.format("/framework/cordova-%s.jar", version),
    function (exists) {
  ...
  path.exists(
    util.format("%s/AndroidManifest.xml", project_path),
    function (exists) {
  ...
  path.exists(
    util.format("%s/res/xml/plugins.xml", project_path),
    function (exists) {
  ...
  path.exists(
    util.format("%s/res/xml/cordova.xml", project_path),
    function (exists) {
      assert(exists, "plugins.xml did not get created");
    }
  );
  ...
  path.exists(
    util.format("%s/libs/cordova-%s.jar", project_path, version),
    function (exists) {
  ...
  path.exists(
    util.format("%s/assets/www/cordova-%s.js", project_path, version),
    function (exists) {
  ...
  path.exists(
    util.format("%s/cordova/cordova.bat", project_path),
    function (exists) {
  ...
  path.exists(
    util.format("%s/cordova/debug.bat", project_path),
    function (exists) {
  ...
  path.exists(
    util.format("%s/cordova/BOOM.bat", project_path),
    function (exists) {
  ...
  path.exists(
    util.format("%s/cordova/log.bat", project_path),
    function (exists) {
  ...
  path.exists(
    util.format("%s/cordova/clean.bat", project_path),
    function (exists) {
  ...
  path.exists(
    util.format("%s/cordova/emulate.bat", project_path),
    function (exists) {
  ...
  path.exists(
    util.format("%s/cordova/appinfo.jar", project_path),
    function (exists) {
  ...
  exec(
    ...
    function (error, stdout, stderr) {
      ...
      path.exists(
        util.format("%s/bin/%s-debug.apk", project_path, project_name),
        function (exists) {
