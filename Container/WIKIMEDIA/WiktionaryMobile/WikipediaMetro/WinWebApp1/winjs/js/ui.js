  ...
  function processAllImpl(rootElement) {
    return new WinJS.Promise(function (complete, error) {
      ...
      for (var i = 0, len = elements.length; i < len; i++) {
        ...
        if (ctl) {
          pending++;
          activate(elements[i], ctl).then(checkAllComplete, function (err) {
            error(err);
          });
        }
      }
...
(function (WinJS, undefined) {
  var QueryCollection = WinJS.Class.derive(
    Array,
    function (items) {
      /// <summary locid="25">
    ...
    },
    {
      ...
      include: function (items) {
        ...
        if (typeof items.length === "number") {
          ...
          items.DOCUMENT_FRAGMENT_NODE &&
          items.nodeType === items.DOCUMENT_FRAGMENT_NODE
        ) {
  ...
  WinJS.Namespace.defineWithParent(WinJS, "Utilities", {
    ...
    query: function (query, element) {
      ...
      /// </returns>
      return new WinJS.Utilities.QueryCollection(
...
(function (WinJS, undefined) {
  ...
  WinJS.Namespace.defineWithParent(WinJS, "Utilities", {
    ...
    getContentWidth: function (element) {
      ...
      /// </returns>
      var border =
          getDimension(element, "borderLeftWidth") +
          getDimension(element, "borderRightWidth"),
        padding =
          getDimension(element, "paddingLeft") +
          getDimension(element, "paddingRight");
    ...
    getTotalWidth: function (element) {
      ...
      /// </returns>
      var margin =
        getDimension(element, "marginLeft") +
        getDimension(element, "marginRight");
    ...
    getContentHeight: function (element) {
      ...
      /// </returns>
      var border =
          getDimension(element, "borderTopWidth") +
          getDimension(element, "borderBottomWidth"),
        padding =
          getDimension(element, "paddingTop") +
          getDimension(element, "paddingBottom");
    ...
    getTotalHeight: function (element) {
      ...
      /// </returns>
      var margin =
        getDimension(element, "marginTop") +
        getDimension(element, "marginBottom");
...
(function (WinJS, globalObj, undefined) {
    ...
    return new WinJS.Promise(function (c, e, p) {
      ...
      if (typeof href === "string") {
        ...
        var complete = function (load) {
          ...
          temp.contentDocument.removeEventListener(
            "DOMContentLoaded",
            domContentLoaded,
            false
          );
        ...
        temp.contentDocument.addEventListener(
          "DOMContentLoaded",
          domContentLoaded,
          false
        );
        ...
        state.loadScript =
          href.getAttribute("data-win-fragmentLoad") || state.loadScript;
  ...
  function controlStaticStateLoaded(href, temp, state) {
    ...
    return new WinJS.Promise(function (c, e, p) {
      ...
      //
      forEach(cd.querySelectorAll("style"), function (e, i) {
        addStyle(e, href, i);
      });
  ...
  function initialize() {
    ...
    var csss = head.querySelectorAll(
      'link[rel="stylesheet"], link[type="text/css"]'
    );
  ...
  function prepareFragment(href) {
    ...
    /// </returns>
    initialize();
    return controlStaticState(href).then(function () {
      return;
    });
  }
  ...
  function selfhost(load) {
    ...
    if (globalObj.parent) {
      if (globalObj.parent.document[loaderStateProp] != "loading") {
        forEach(
          globalObj.document.querySelectorAll(
            'head > script[type="ms-deferred/javascript"]'
          ),
          function (e) {
            addScript(e);
          }
  ...
  var lexer = (function () {
      ...
      while (offset < limit) {
        ...
        switch (code) {
          ...
          case /*\*/ 92:
            if (offset + 5 < limit) {
              if (
                text.charCodeAt(offset + 1) === /*u*/ 117 &&
                isHexDigit(text.charCodeAt(offset + 2)) &&
                isHexDigit(text.charCodeAt(offset + 3)) &&
                isHexDigit(text.charCodeAt(offset + 4)) &&
    ...
    function readDecimalLiteral(text, offset, limit) {
      ...
      if (
        ...
        offset + 1 < limit &&
        isDecimalDigit(text.charCodeAt(offset + 1))
      ) {
    ...
    function readStringLiteralToken(text, offset, limit) {
        ...
        if (offset + 1 < limit && text.charCodeAt(offset) === /*\*/ 92) {
          ...
          switch (text.charCodeAt(offset + 1)) {
            ...
            case 0x000d: // carriage return
              if (
                offset + 2 < limit &&
                text.charCodeAt(offset + 2) === 0x000a
              ) {
                // Skip \r\n
                offset += 3;
              } else {
      ...
      hasEscape =
        hasEscape ||
        length === 1 ||
        text.charCodeAt(offset - 1) !== quoteCharCode;
      return {
          ...
          case /*-*/ 45:
            if (offset < limit) {
              ...
              if (afterSign === /*.*/ 46) {
                ...
                if (
                  signOffset < limit &&
                  isDecimalDigit(text.charCodeAt(signOffset))
                ) {
                  token = readDecimalLiteralToken(
                    text,
                    startOffset,
                    signOffset,
                    limit
...
(function (global, undefined) {
  ...
  var OptionsInterpreter = WinJS.Class.derive(
    BaseInterpreter,
    ...
    },
    {
      _error: function (message) {
        var error = new Error(
          "Invalid options record, expected to be in the format of an object literal. " +
            message
        );
        error.name = "WinJS.UI.ParseError";
        throw error;
      ...
      _readArrayElements: function (a) {
        ...
        while (
          ...
          this._current.type !== tokenType.rightBracket
        ) {
          if (!first) {
            this._read(tokenType.comma);
          }
      ...
      _readObjectProperties: function (o) {
        ...
        while (
          this._current.type !== tokenType.eof &&
          this._current.type !== tokenType.rightBrace
        ) {
          if (!first) {
            this._read(tokenType.comma);
          }
...
(function (WinJS, undefined) {
  function isSelectable(elem) {
    ...
    while (elem) {
      if (
        elem.hasAttribute("data-win-selectable") ||
...
(function (WinJS, undefined) {
  ...
  WinJS.Namespace.defineWithParent(WinJS, "UI", {
    TabContainer: WinJS.Class.define(
          ...
          set: function (e) {
            if (e != this._focusElement) {
              if (this._focusElement) {
                ...
                this._focusElement.removeEventListener(
                  "onTabExit",
                  this._tabExitHandler
                );
              }
...
(function (WinJS, undefined) {
  ...
  function getAnimationStaggerOffset(element, animArray, iElem) {
    for (var i = 0; i < animArray.length; i++) {
      ...
      if (typeof animArray[i].stagger === "function") {
        animArray[i].delay = animArray[i].stagger(
          iElem,
          animArray[i].initialDelay
        );
      }
  ...
  function getTransitionStaggerTransform(transArray, iElem) {
    for (var i = 0; i < transArray.length; i++) {
      ...
      if (typeof transArray[i].stagger === "function") {
        transArray[i].delay = transArray[i].stagger(
          iElem,
          transArray[i].initialDelay
        );
      }
  ...
  function execElementTransition(elem, transitionArray) {
    elem.style.msTransitionProperty = transitionArray
      .map(function (t) {
        return t.name;
      })
      .join(",");
    elem.style.msTransitionDelay = transitionArray
      .map(function (t) {
        return t.delay + "ms";
      })
      .join(",");
    elem.style.msTransitionDuration = transitionArray
      .map(function (t) {
        return t.duration + "ms";
      })
      .join(",");
    elem.style.msTransitionTimingFunction = transitionArray
      .map(function (t) {
        return t.timing;
      })
      .join(",");
  ...
  function executeElementAnimation(elem, animArray, styleelm) {
    ...
    elem.style.msAnimationDelay = animArray
      .map(function (a) {
        return a.delay + "ms";
      })
      .join(",");
    elem.style.msAnimationDuration = animArray
      .map(function (a) {
        return a.duration + "ms";
      })
      .join(",");
    elem.style.msAnimationTimingFunction = animArray
      .map(function (a) {
        return a.timing;
      })
      .join(",");
    elem.style.msAnimationName = animArray
      .map(function (a) {
        return a.keyframe;
      })
      .join(",");
    ...
    var promise = new WinJS.Promise(function (c, e, p) {
      var executeAnimationEnd = function (event) {
        if (event.srcElement === elem) {
          if (
            releasePVLAnimation(styleelm, styleRef) === 0 &&
            styleelm.parentElement
          ) {
          ...
          for (var i = 0; i < elemArray.length; i++) {
            if (elemArray[i] instanceof Array) {
              for (var j = 0; j < elemArray[i].length; j++) {
                ...
                promiseArray.push(
                  executeElementAnimation(elemArray[i][j], animArray, styleelm)
                );
              }
            } else {
              getAnimationStaggerOffset(elemArray[i], animArray, i);
              promiseArray.push(
                executeElementAnimation(elemArray[i], animArray, styleelm)
              );
            }
          }
        } else {
          ...
          for (var i = 0; i < elemArray.length; i++) {
            if (elemArray[i] instanceof Array) {
              for (var j = 0; j < elemArray[i].length; j++) {
                ...
                promiseArray.push(
                  execElementTransition(elemArray[i][j], transArray)
                );
              }
            } else {
              getTransitionStaggerTransform(transArray, i);
              promiseArray.push(
                execElementTransition(elemArray[i], transArray)
              );
            }
          }
          return WinJS.Promise.join(promiseArray);
