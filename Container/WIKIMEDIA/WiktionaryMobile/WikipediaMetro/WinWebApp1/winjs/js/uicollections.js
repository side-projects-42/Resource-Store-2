...
(function (WinJS) {
    ...
    FlipView: WinJS.Class.define(
      function (element, options) {
        ...
        if (options) {
          ...
          placeholderRenderer = options.placeholderRenderer
            ? options.placeholderRenderer
            : trivialPlaceholderRenderer;
        ...
        if (!dataSource) {
          ...
          if (childElements.length > 0) {
            dataSource = new thisWinUI.ArrayDataSource(childElements.slice(0), {
              compareByIdentity: true,
            });
            keepInMemory = true;
            countToLoad = childElements.length;
          } else {
            dataSource = new thisWinUI.ArrayDataSource([], {
              compareByIdentity: true,
            });
          }
        }
      ...
      },
      {
        ...
        next: function () {
          ...
          var animation = this._nextAnimation
            ? this._nextAnimation
            : this._defaultNextAnimation.bind(this);
        ...
        previous: function () {
          ...
          var animation = this._prevAnimation
            ? this._prevAnimation
            : this._defaultPrevAnimation.bind(this);
          ...
          if (elements) {
            ...
            animation(currElement, prevElement).then(function () {
              ...
              that._pageManager.endAnimatedPrevious(
                elements.curr,
                elements.prev
              );
              that._animationsFinished();
        ...
        currentPage: {
          ...
          set: function (index) {
            ...
            var jumpAnimation = this._jumpAnimation
              ? this._jumpAnimation
              : this._defaultJumpAnimation.bind(this);
            ...
            if (elements) {
              ...
              jumpAnimation(currElement, newCurrElement).then(function () {
                ...
                that._pageManager.endAnimatedJump(
                  elements.oldPage,
                  elements.newPage
                );
                that._animationsFinished();
        ...
        placeholderRenderer: {
          ...
          set: function (placeholderRenderer) {
            this._setDatasource(
              this._dataSource,
              this._itemRenderer,
              typeof placeholderRenderer === "object"
                ? placeholderRenderer.renderItem
                : placeholderRenderer,
              0
            );
          },
        ...
        count: function () {
          ...
          return new WinJS.Promise(function (complete, error) {
            if (that._itemsManager) {
              if (
                that._cachedSize === WinJS.UI.CountResult.unknown ||
                that._cachedSize >= 0
              ) {
        ...
        addEventListener: function (eventName, eventHandler, useCapture) {
          ...
          return this._flipviewDiv.addEventListener(
            eventName,
            eventHandler,
            useCapture
          );
        ...
        removeEventListener: function (eventName, eventHandler, useCapture) {
          ...
          return this._flipviewDiv.removeEventListener(
            eventName,
            eventHandler,
            useCapture
          );
        ...
        _initializeFlipView: function (
          ...
          this._pageManager = new thisWinUI._FlipPageManager(
            this._flipviewDiv,
            this._panningDiv,
            this._panningDivContainer,
            this._itemsManager,
            keepInMemory,
            itemSpacing,
            {
          ...
          this._contentDiv.addEventListener(
            ...
            function () {
              ...
              that._buttonFadePromise = WinJS.Promise.timeout(
                buttonFadeDelay
              ).then(function () {
          ...
          this._panningDivContainer.addEventListener(
            "scroll",
            function () {
          ...
          this._flipviewDiv.addEventListener(
            "DOMNodeInserted",
            function (event) {
        ...
        _setDatasource: function (
          ...
          this._pageManager.setNewItemsManager(
            this._itemsManager,
            initialIndex
          );
        },
        ...
        _setupOrientation: function () {
          if (this._horizontal) {
            ...
            this._flipviewDiv.setAttribute(
              "aria-label",
              horizontalFlipViewLabel
            );
            var rtl =
              window.getComputedStyle(this._flipviewDiv, null).direction ===
              "rtl";
            this._rtl = rtl;
            if (rtl) {
              this._prevButton.className =
                navButtonClass + " " + navButtonRightClass;
              this._nextButton.className =
                navButtonClass + " " + navButtonLeftClass;
            } else {
              this._prevButton.className =
                navButtonClass + " " + navButtonLeftClass;
              this._nextButton.className =
            ...
            this._prevButton.className =
              navButtonClass + " " + navButtonTopClass;
            this._nextButton.className =
              navButtonClass + " " + navButtonBottomClass;
        ...
        _defaultJumpAnimation: function (curr, next) {
          ...
          incomingPageMove.left = this._horizontal
            ? (this._rtl ? -jumpAnimationMoveDelta : jumpAnimationMoveDelta) +
              "px"
            : "0px";
          incomingPageMove.top = this._horizontal
            ? "0px"
            : jumpAnimationMoveDelta + "px";
        ...
        _customMoveTransition: function (elements, offset) {
          ...
          for (var i = 0, len = elements.length; i < len; i++) {
            elements[i].style.msTransform = this._horizontal
  ...
  // Errors
  thisWinUI.FlipView.noElement =
    "Invalid argument: A FlipView requires a DOM element passed in as its first parameter"; // TODO: it seems like noElement should be a UI namespace error instead of a dedicated flipview one
  thisWinUI.FlipView.badAxis =
    "Invalid argument: orientation must be a string, either 'horizontal' or 'vertical'";
  thisWinUI.FlipView.badCurrentPage =
    "Invalid argument: currentPage must be a number greater than or equal to zero and be within the bounds of the datasource";
  thisWinUI.FlipView.noitemsManagerForCount =
    "Invalid operation: can't get count if no dataSource has been set";
  thisWinUI.FlipView.badItemSpacingAmount =
    "Invalid argument: itemSpacing must be a number greater than or equal to zero";
})(WinJS);
  ...
  WinJS.Namespace.defineWithParent(WinJS, "UI", {
    ...
    _FlipPageManager: WinJS.Class.define(
      function (
        ...
        this._rtl =
          window.getComputedStyle(this._flipperDiv, null).direction === "rtl";
        ...
        var that = this;
        this._panningDiv.addEventListener(
          "keydown",
          function (event) {
        ...
        this._flipperDiv.addEventListener(
          "focus",
          function (event) {
      ...
      },
      {
        ...
        initialize: function (initialIndex, countToLoad, horizontal) {
          ...
          if (!this._currentPage) {
            ...
            var pagesToInit =
              (countToLoad > 0 ? countToLoad - 1 : 0) + 2 * this._bufferSize;
        ...
        currentIndex: function () {
          ...
          var element = this._navigationAnimationRecord
            ? this._navigationAnimationRecord.newCurrentElement
        ...
        scrollPosChanged: function () {
          ...
          var newPos = this._viewportStart(),
            bufferEnd =
              this._lastScrollPos > newPos
                ? this._getTailOfBuffer()
                : this._getHeadOfBuffer();
          ...
          while (
            this._currentPage.element &&
            this._itemStart(this._currentPage) > newPos
          ) {
            if (this._currentPage.prev.element) {
              ...
              if (this._currentPage.element) {
                ...
                msSetImmediate(function () {
                  containerStyle["overflow-x"] = that._horizontal
                    ? "scroll"
                    : "hidden";
                  containerStyle["overflow-y"] = that._horizontal
                    ? "hidden"
                    : "scroll";
                });
                return;
          ...
          while (
            this._currentPage.element &&
            this._itemEnd(this._currentPage) <= newPos
          ) {
        ...
        itemRetrieved: function (real, placeholder) {
          ...
          this._forEachPage(function (curr) {
            if (curr.element === placeholder) {
              if (
                curr === that._currentPage ||
                curr === that._currentPage.next
              ) {
          ...
          if (this._navigationAnimationRecord) {
            var animatingElements =
              this._navigationAnimationRecord.elementContainers;
            ...
            if (index > currIndex) {
              for (i = 0; i < distance && newCurrent.element; i++) {
                ...
                if (!newCurrent.element) {
                  newCurrent.setElement(
                    this._itemsManager.nextItem(newCurrent.prev.element)
                  );
                }
              }
            } else {
              for (i = 0; i < distance && newCurrent.element; i++) {
                ...
                if (!newCurrent.element) {
                  newCurrent.setElement(
                    this._itemsManager.previousItem(newCurrent.next.element)
                  );
                }
              }
            }
        ...
        endAnimatedNext: function (curr, next) {
          this._navigationAnimationRecord.oldCurrentPage.setElement(
            curr.element,
            true
          );
          this._navigationAnimationRecord.newCurrentPage.setElement(
            next.element,
            true
          );
        ...
        inserted: function (element, prev, next, animateInsertion) {
          ...
          this._getAnimationRecord(element).successfullyMoved =
            elementSuccessfullyPlaced;
          this._setButtonStates();
        },
        ...
        notificationsEnded: function () {
          ...
          function animateOldViewportItemMoved(record, item) {
            ...
            if (!record.successfullyMoved) {
              ...
              var newCurrentIndex = that._currentPage.element
                ? that._itemsManager.itemIndex(that._currentPage.element)
                : 0;
              newLocation +=
                newCurrentIndex > indexMovedTo
                  ? -100 * that._bufferSize
                  : 100 * that._bufferSize;
            } else {
            ...
            animationPromises.push(
              that._moveFlipPage(movedPage, function () {
          ...
          if (
            oldCurrent !== this._currentPage.element ||
            oldNext !== this._currentPage.next.element
          ) {
          ...
          joinAnimationPromises().then(function () {
            ...
            var newCurrRecord = that._getAnimationRecord(
                that._currentPage.element
              ),
              newNextRecord = that._getAnimationRecord(
                that._currentPage.next.element
              );
            that._forEachPage(function (curr) {
              var record = that._getAnimationRecord(curr.element);
              if (record) {
                if (!record.inserted) {
                  if (record.originalLocation !== record.newLocation) {
                    if (
                      (record !== oldCurrentRecord &&
                        record !== oldNextRecord) ||
                      (record === oldCurrentRecord &&
                        !oldCurrentRecord.moved) ||
                      (record === oldNextRecord && !oldNextRecord.moved)
                    ) {
                      animationPromises.push(
                        that._moveFlipPage(curr, function () {
                  ...
                  record !== newCurrRecord &&
                  record !== newNextRecord
                ) {
            ...
            joinAnimationPromises().then(function () {
              ...
              if (newNextRecord && newNextRecord.inserted) {
                animationPromises.push(
                  that._insertFlipPage(that._currentPage.next)
                );
              }
              joinAnimationPromises().then(function () {
        ...
        _timeoutPageSelection: function () {
          ...
          this._lastTimeoutRequest = WinJS.Promise.timeout(
            itemSelectedEventDelay
          );
        ...
        _getElementKey: function (element) {
          return element.msDataItem
            ? element.msDataItem.key
            : this._getTemporaryKey(element);
        },

        _getAnimationRecord: function (element) {
          return element
            ? this._animationRecords[this._getElementKey(element)]
            : null;
        },
        ...
        _fetchPreviousItems: function (setPrevMarker) {
          ...
          for (var i = 0; i < this._bufferSize; i++) {
            if (curr.element) {
              curr.prev.setElement(
                this._itemsManager.previousItem(curr.element)
              );
            } else {
        ...
        _announceElementVisible: function (element) {
          if (element && !element.visible) {
            ...
            event.initCustomEvent(
              ...
              true,
              true,
              { source: this._flipperDiv, visible: true }
            );
        ...
        _announceElementInvisible: function (element) {
          if (element && element.visible) {
            ...
            event.initCustomEvent(
              ...
              true,
              true,
              { source: this._flipperDiv, visible: false }
            );
        ...
        _createDiscardablePage: function (content) {
          ...
          page.centerElement = function () {
            page.element.style.left =
              Math.max(
                (page.div.offsetWidth - page.element.offsetWidth) / 2,
                0
              ) + "px";
            page.element.style.top =
              Math.max(
                (page.div.offsetHeight - page.element.offsetHeight) / 2,
                0
              ) + "px";
          };
          page.centerElement();
        ...
        _itemInView: function (flipPage) {
          return (
            this._itemEnd(flipPage) > this._viewportStart() &&
            this._itemStart(flipPage) < this._viewportEnd()
          );
        },
        ...
        _viewportEnd: function () {
          ...
          if (this._horizontal) {
            if (this._rtl) {
              return (
                this._viewportStart() + this._panningDivContainer.offsetWidth
              );
            } else {
        ...
        _itemEnd: function (flipPage) {
          ...
          return (
            (this._horizontal
              ? flipPage.location + div.offsetWidth
              : flipPage.location + div.offsetHeight) + this._itemSpacing
          );
        },

        _itemSize: function (flipPage) {
          return this._horizontal
            ? flipPage.div.offsetWidth
            : flipPage.div.offsetHeight;
        },

        _panningDivEnd: function () {
          return this._horizontal
            ? this._panningDiv.offsetWidth
            : this._panningDiv.offsetHeight;
        },
        ...
        _movePageAhead: function (referencePage, pageToPlace) {
          ...
          if (pageToPlace.element) {
            ...
            this._itemStart(
              pageToPlace,
              this._itemStart(referencePage) + delta
            );
          }
        },
        ...
        _setupSnapPoints: function () {
          ...
          containerStyle[
            this._horizontal ? propertyName + "-x" : propertyName + "-y"
          ] = "snapInterval(0px, " + snapInterval + "px)";
        },
        ...
        _itemSettledOn: function () {
          ...
          if (this._viewportOnItemStart()) {
            ...
            if (this._lastSelectedElement !== this._currentPage.element) {
              if (
                this._lastSelectedPage &&
                this._lastSelectedPage.element &&
                !isFlipper(this._lastSelectedPage.element)
              ) {
                this._lastSelectedPage.element.setAttribute(
                  "aria-selected",
                  false
                );
              }
...
(function (global) {
  ...
  function errorDoesNotExist() {
    return Promise.wrapError(
      new WinJS.ErrorFromName(UI.FetchError.doesNotExist)
    );
  }

  function errorNoLongerMeaningful() {
    return Promise.wrapError(
      new WinJS.ErrorFromName(UI.EditError.noLongerMeaningful)
    );
  }

  var keySearchRange = 5;

  var ArrayDataAdaptor = WinJS.Class.define(
    ...
    },
    {
      ...
      insertBefore: function (key, data, nextKey, nextIndexHint) {
        // key parameter is ignored, as keys are part of data or are generated
        return this._insert(
          this._indexFromKeyAndHint(nextKey, nextIndexHint),
          data
        );
      },
      ...
      insertAfter: function (key, data, previousKey, previousIndexHint) {
        // key parameter is ignored, as keys are part of data or are generated
        return this._insert(
          this._indexFromKeyAndHint(previousKey, previousIndexHint) + 1,
          data
        );
      },
      ...
      moveBefore: function (key, nextKey, indexHint, nextIndexHint) {
        return this._move(
          this._indexFromKeyAndHint(key, indexHint),
          this._indexFromKeyAndHint(nextKey, nextIndexHint)
        );
      },
      ...
      _indexFromKeyAndHint: function (key, indexHint) {
        ...
        for (
          i = indexHint,
            max = Math.min(i + keySearchRange, this._array.length - 1);
          i <= max;
          i++
        ) {
        ...
        for (
          i = indexHint - 1, min = Math.max(indexHint - keySearchRange, 0);
          i >= min;
          i--
        ) {
          if (this._itemKey(i) === key) {
  ...
  var IteratorDataAdaptor = WinJS.Class.define(
    ...
    },
    {
      ...
      itemsFromIndex: function (index, countBefore, countAfter) {
        ...
        return this._arrayDataAdaptor.itemsFromIndex(
          index,
          countBefore,
          countAfter
        );
...
(function () {
  ...
  var ListNotificationHandler = WinJS.Class.define(
    function (groupDataAdaptor) {
      // Constructor
  ...
  var GroupDataAdaptor = WinJS.Class.define(
    function (listDataSource, groupOf) {
      // Constructor
      ...
      this._listBinding = listDataSource.createListBinding(
        new ListNotificationHandler(this)
      );
    ...
    },
    {
      ...
      _processBatch: function () {
        ...
        for (var i = 0; i < fetchBatchSize; i++) {
          ...
          if (
            previousGroup &&
            groupInfo &&
            groupInfo.key === previousGroup.key
          ) {
            ...
            if (previousGroup.lastItem === previousItem) {
              if (
                previousGroup.lastItem.handle !== previousGroup.firstItem.handle
              ) {
              ...
              if (
                previousGroup.firstItem.handle !== previousGroup.lastItem.handle
              ) {
      ...
      _fetchBatch: function (itemPromise, countBefore, countAfter) {
        ...
        this._listBinding.jumpToItem(itemPromise);
        for (
          batchIndex = countBefore + 1;
          batchIndex < fetchBatchSize;
          batchIndex++
        ) {
      ...
      _fetchAdjacent: function (item, after) {
        this._fetchBatch(
          this._listBinding.fromKey(item.key),
          after ? 0 : fetchBatchSize - 1,
          after ? fetchBatchSize - 1 : 0
        );
      },
      ...
      _fetchNextBatch: function (group, countBefore, countAfter) {
        if (group) {
          ...
          if (!group.firstReached) {
            ...
            countBefore > 0 &&
            !groupReady((groupPrev = this._previousGroup(group)))
          ) {
            this._fetchAdjacent(
              groupPrev.lastReached ? groupPrev.firstItem : group.firstItem,
              false
            );
          } else {
            groupNext = this._nextGroup(group);
            this._fetchAdjacent(
              groupNext.firstReached ? groupNext.lastItem : group.lastItem,
              true
            );
          }
        } else {
      ...
      _processInsertion: function (item, previousHandle, nextHandle) {
        ...
        if (groupPrev) {
          ...
          if (
            !groupPrev.lastReached ||
            previousHandle !== groupPrev.lastItem.handle ||
            (groupInfo = this._groupOf(item)).key === groupPrev.key
          ) {
        ...
        if (groupNext) {
          ...
          if (
            !groupNext.firstReached ||
            nextHandle !== groupNext.firstItem.handle ||
            (groupInfo || this._groupOf(item)).key === groupNext.key
          ) {
            this._releaseGroup(groupPrev);
          }
          this._beginRefresh();
  ...
  WinJS.Namespace.define("WinJS.UI", {
    ...
    GroupDataSource: function (listDataSource, groupOf) {
      return new UI.ListDataSource(
        new GroupDataAdaptor(listDataSource, groupOf)
      );
    },
  });
})();
    ...
    GroupedItemDataSource: function (listDataSource, groupOf) {
      ...
      groupedItemDataSource.createListBinding = function (notificationHandler) {
        ...
        if (listBinding.fromKey) {
          groupedItemListBinding.fromKey = function (
            key,
            prefetchBefore,
            prefetchAfter
          ) {
            return createGroupedItemPromise(
              listBinding.fromKey(key, prefetchBefore, prefetchAfter)
            );
          };
        }

        if (listBinding.fromIndex) {
          groupedItemListBinding.fromIndex = function (
            index,
            prefetchBefore,
            prefetchAfter
          ) {
            return createGroupedItemPromise(
              listBinding.fromIndex(index, prefetchBefore, prefetchAfter)
            );
          };
        }

        if (listBinding.fromDescription) {
          groupedItemListBinding.fromDescription = function (
            description,
            prefetchBefore,
            prefetchAfter
          ) {
            return createGroupedItemPromise(
              listBinding.fromDescription(
                description,
                prefetchBefore,
                prefetchAfter
              )
...
(function (global) {
  ...
  var listDataSourceIsInvalid =
    "Invalid argument: dataSource must be an object.";
  var itemRendererIsInvalid =
    "Invalid argument: itemRenderer must be a function.";
  var callbackIsInvalid1 = "Invalid argument: ";
  var callbackIsInvalid2 = " must be a function.";
  var priorityIsInvalid =
    "Invalid argument: priority must be one of following values: Priority.high or Priority.medium.";
  var itemIsInvalid =
    "Invalid argument: item must be a DOM element that was returned by the Items Manager, and has not been replaced or released.";
  ...
  var ItemsManager = WinJS.Class.define(
    function (
      listDataSource,
      itemRenderer,
      elementNotificationHandler,
      options
    ) {
      // Constructor
      ...
      this._listBinding = this._listDataSource.createListBinding(
        new ListNotificationHandler(this)
      );
      ...
      this._renderManager = new UI.RenderManager(itemRenderer, function (item) {
        return item.key;
      });
    ...
    },
    {
      ...
      itemFromDescription: function (description) {
        ...
        return this._elementForItem(
          this._listBinding.fromDescription(description)
        );
      },
      ...
      prioritize: function (first, last, priority) {
        ...
        for (
          var itemPromise = this._listBinding.jumpToItem(itemFirst);
          true;
          itemPromise = this._listBinding.next()
        ) {
      ...
      _defineIDProperty: function (itemForRenderer) {
        ...
        Object.defineProperty(itemForRenderer, "id", {
          get: function () {
            return UI.RenderManager.itemID(
              that._ownerElement,
              itemForRenderer.key
            );
          },
      ...
      _changeElement: function (record, elementNew, elementNewIsPlaceholder) {
        ...
        if (
          record.item &&
          record.elementIsPlaceholder &&
          !elementNewIsPlaceholder
        ) {
      ...
      _restoreElementState: function (record) {
        if (this._itemNotificationHandler.restoreState) {
          this._itemNotificationHandler.restoreState(
            record.item.key,
            record.element
          );
        }
      ...
      _changed: function (newItem, oldItem) {
        ...
        var that = this;
        record.renderPromise = this._renderItem(
          newItem,
          Priority.immediate
        ).then(function (elementNew) {
      ...
      _indexChanged: function (handle, newIndex, oldIndex) {
        ...
        if (this._elementNotificationHandler.indexChanged) {
          this._handlerToNotify().indexChanged(
            record.element,
            newIndex,
            oldIndex
          );
        ...
        if (elementDelayed) {
          ...
          if (this._getRecordPriority(record) === Priority.high) {
            ...
            if (typeof index !== "number" || index > 0) {
              ...
              var recordPrev =
                this._handleMap[this._listBinding.previous().handle];
              if (
                recordPrev &&
                this._getRecordPriority(record) === Priority.high &&
                recordPrev.renderPromise
              ) {
                return;
              }
            }
...
(function (global) {
  ...
  var listDataAdaptorIsInvalid =
    "Invalid argument: listDataAdaptor must be an object or an array.";
  var indexIsInvalid =
    "Invalid argument: index must be a non-negative integer.";
  ...
  var invalidKeyReturned =
    "Error: data adaptor returned item with undefined or null key.";
  var invalidIndexReturned =
    "Error: data adaptor should return undefined, null or a non-negative integer for the index.";
  var invalidCountReturned =
    "Error: data adaptor should return undefined, null, CountResult.unknown, or a non-negative integer for the count.";
  var invalidRequestedCountReturned =
    "Error: data adaptor should return CountResult.unknown, CountResult.failure, or a non-negative integer for the count.";
  ...
  function ListDataSource(listDataAdaptor) {
    ...
    function changeCount(count) {
      ...
      forEachBindingRecord(function (bindingRecord) {
        if (
          bindingRecord.notificationHandler &&
          bindingRecord.notificationHandler.countChanged
        ) {
      ...
      if (slotNext !== undefined) {
        ...
        if (slotNext === undefined) {
          ...
          if (slotNext === listEnd && !listEnd.firstInSequence) {
            ...
            slotNext =
              lastSequenceStart && lastSequenceStart.index === undefined
                ? lastSequenceStart
                : undefined;
          }
        }
    ...
    function sendInsertedNotification(slot) {
      ...
      for (property in bindingMapUnion) {
        ...
        if (bindingRecord.notificationHandler) {
          handlerToNotify(bindingRecord).inserted(
            slot.item,
            slotPrev.lastInSequence || slotPrev === slotsStart
              ? null
              : slotPrev.handle,
            slotNext.firstInSequence || slotNext === slotsEnd
              ? null
              : slotNext.handle
          );
        }
    ...
    function moveSlot(slot, slotMoveBefore, mergeWithPrev, mergeWithNext) {
      ...
      for (property in bindingMapUnion) {
        var bindingRecord = bindingMapUnion[property];
        handlerToNotify(bindingRecord).moved(
          slot.item,
          (slotMoveAfter.lastInSequence && !mergeWithPrev) ||
            slotMoveAfter === slotsStart
            ? null
            : slotMoveAfter.handle,
          (slotMoveBefore.firstInSequence && !mergeWithNext) ||
            slotMoveBefore === slotsEnd
            ? null
            : slotMoveBefore.handle
        );
      }
    ...
    function createPlaceholder(slot) {
      ...
      if (
        slot.prev === slotsStart &&
        !slot.firstInSequence &&
        !indexUpdateDeferred
      ) {
    ...
    function defineCommonItemProperties(item, slot) {
      ...
      Object.defineProperty(item, "requestID", {
        // DEPRECATED: requestID has been replaced by handle
        value: slot.handle,
        writable: false,
        enumerable: false,
        configurable: true,
      ...
      if (!refreshInProgress && !slotFetchInProgress(slot)) {
        ...
        if (listDataAdaptor.itemsFromStart) {
          ...
          fetchItems(
            slot,
            fetchID,
            listDataAdaptor.itemsFromIndex(0, 0, count - 1)
          );
        }
      }
    }
      ...
      if (!refreshInProgress && !slotFetchInProgress(slot)) {
        ...
        if (listDataAdaptor.itemsFromKey && slot.key !== undefined) {
          fetchItems(
            slot,
            fetchID,
            listDataAdaptor.itemsFromKey(slot.key, countBefore, countAfter)
          );
        } else {
          ...
          fetchItems(
            slot,
            fetchID,
            listDataAdaptor.itemsFromIndex(
              index,
              Math.min(countBefore, index),
              countAfter
            )
          );
        }
      ...
      if (!refreshInProgress && !slotFetchInProgress(slot)) {
        ...
        if (listDataAdaptor.itemsFromIndex) {
          ...
          fetchItems(
            slot,
            fetchID,
            listDataAdaptor.itemsFromIndex(index, countBefore, countAfter)
          );
        } else {
          ...
          if (slot.key !== undefined) {
            ...
            for (
              slotSearch = slot.prev;
              slotSearch !== slotsStart;
              slotSearch = slotSearch.prev
            ) {
              if (
                slotSearch.index !== undefined &&
                slotSearch.key !== undefined
              ) {
            ...
            for (
              slotSearch = slot.next;
              slotSearch !== slotsEnd;
              slotSearch = slotSearch.next
            ) {
              if (
                slotSearch.index !== undefined &&
                slotSearch.key !== undefined
            ...
            if (slotClosest === slotsStart) {
              fetchItemsForIndex(
                0,
                slot,
                listDataAdaptor.itemsFromStart(index + 1)
              );
            } else if (
              slotSearch.index !== undefined &&
              slotSearch.key !== undefined
    ...
    function fetchItemsFromDescription(
      slot,
      description,
      countBefore,
      countAfter
      ...
      if (!refreshInProgress && !slotFetchInProgress(slot)) {
        ...
        fetchItems(
          slot,
          fetchID,
          listDataAdaptor.itemsFromDescription(
            description,
            countBefore,
            countAfter
          )
    ...
    function refreshRange(slot, fetchID, countBefore, countAfter) {
      ...
      if (listDataAdaptor.itemsFromKey) {
        ...
        fetchItemsForRefresh(
          ...
          listDataAdaptor.itemsFromIndex(
            index,
            Math.min(countBefore + searchDelta, index),
            countAfter + searchDelta
          )
        );
      }
    }
    ...
    function refreshFirstItem(fetchID) {
      ...
      if (listDataAdaptor.itemsFromStart) {
        ...
        fetchItemsForRefresh(
          null,
          fetchID,
          listDataAdaptor.itemsFromIndex(0, 0, 0)
        );
      }
    }
    ...
    function refreshRanges(slotFirst, allRanges) {
      ...
      for (var slot = slotFirst; slot !== slotsEnd; slot = slot.next) {
        if (
          ...
          slot.kind === "item" &&
          !deletedKeys[slot.key] &&
          !keyFetchInProgress(slot.key)
        ) {
          ...
          if (
            !slotRefresh ||
            slotRefresh.firstInSequence ||
            slotRefresh.lastInSequence
          ) {
            slotFetchFirst = slot;
            fetchID = newFetchID();
          }
        ...
        if (slotFetchFirst === undefined) {
            ...
            if (
              ...
              if (!refreshKeyMap[slot.key]) {
                ...
                fetchItemsForRefresh(
                  slot.key,
                  newFetchID(),
                  listDataAdaptor.itemsFromKey(slot.key, 1, 1)
                );
              }
            }
          }
          ...
          if (
            ...
            refreshRange(
              ...
              fetchID,
              0,
              fetchCount - 1 + refreshFetchExtra
            );
    ...
    function processRefreshResults(
      ...
      if (results === UI.FetchError.noResponse) {
        ...
        if (slot === undefined) {
          ...
          if (
            index !== undefined &&
            (slotPrev = refreshIndexMap[indexFirst - 1])
          ) {
            if (!slotPrev.lastInSequence) {
              // Something has changed since the refresh began; start again
              beginRefresh();
        ...
        for (var i = 1; i < resultsCount; i++) {
          ...
          if (slotNext === undefined) {
            ...
            if (
              ...
              !processRefreshSlotIndex(slotNext, slot.index + 1)
            ) {
              return;
            }
    ...
    function concludeRefresh() {
      ...
      for (slot = slotsStart; slot; slot = slot.next) {
        ...
        if (slot.lastInSequence) {
          ...
          sequenceOld = {
            ...
            last: slot,
            sequenceNew:
              bestMatchCount > 0 ? sequencesNew[bestMatch] : undefined,
      ...
      if (
        sequencesOld[sequenceCountOld - 1].sequenceNew !==
        sequencesNew[sequenceCountNew - 1]
      ) {
      ...
      // be merged below).
      sequencesNew[sequenceCountNew - 1].sequenceOld =
        sequencesOld[sequenceCountOld - 1];
      ...
      for (i = 0; i < sequenceCountOld; i++) {
        ...
        if (
          ...
          (sequenceOldBestMatch = sequenceOld.sequenceNew.sequenceOld) ===
            sequenceOldPrev &&
          (sequenceOld.last.next !== slotsEnd || !refreshEnd.firstInSequence)
        ) {
          mergeSequencesForRefresh(
            sequenceOldBestMatch.last,
            sequenceOld.first
          );
      ...
      for (i = 0; i < sequenceCountOld; i++) {
        ...
        if (
          sequenceOld &&
          !sequenceOld.indexRequested &&
          (!sequenceOld.sequenceNew ||
            sequenceOld.sequenceNew.sequenceOld !== sequenceOld)
        ) {
      ...
      for (i = 0; i < sequenceCountNew; i++) {
        ...
        for (
          slotRefresh = sequenceNew.first;
          !slotFromSlotRefresh(slotRefresh) && !slotRefresh.lastInSequence;
          slotRefresh = slotRefresh.next
        ) {
          /*@empty*/
        }
        if (slotRefresh.lastInSequence && !slotFromSlotRefresh(slotRefresh)) {
          ...
          for (
            slotRefresh = sequenceNew.last;
            !slotFromSlotRefresh(slotRefresh);
      ...
      for (i = 0; i < sequenceCountOld; i++) {
        ...
        if (sequenceOld) {
          ...
          if (
            ...
            sequenceNew.firstInner !== undefined
          ) {
            ...
            for (slot = sequenceOld.first; true; slot = slot.next, ordinal++) {
              ...
              if (
                slotRefresh &&
                slotRefresh.sequenceNumber ===
                  sequenceNew.firstInner.sequenceNumber
              ) {
            ...
            var piles = [];
            for (
              slotRefresh = sequenceNew.firstInner;
              true;
              slotRefresh = slotRefresh.next
            ) {
              ...
              if (ordinal !== undefined) {
                ...
                while (searchFirst <= searchLast) {
                  var searchMidpoint = Math.floor(
                    0.5 * (searchFirst + searchLast)
                  );
            ...
            sequenceOld.stationarySlot = slotFromSlotRefresh(
              stationaryItems[0]
            );
            ...
            for (j = 0; j < stationaryItemCount - 1; j++) {
              ...
              for (
                slotRefresh = slotRefresh.next;
                slotRefresh !== slotRefreshStop && slot !== slotStop;
                slotRefresh = slotRefresh.next
              ) {
                if (!slotFromSlotRefresh(slotRefresh)) {
              ...
              if (!slotRefresh.stationary) {
                ...
                if (slotPrev) {
                  ...
                  for (
                    ...
                    !slotRefreshStationary.stationary &&
                    slotRefreshStationary !== sequenceNew.lastInner;
                    slotRefreshStationary = slotRefreshStationary.next
                  ) {
                    /*@empty*/
                  }
          ...
          for (
            ...
            if (!slot) {
              ...
              if (slotPrev) {
                ...
                var slotRefreshOld;
                for (
                  slotRefreshOld = sequenceNew.firstInner;
                  !slotFromSlotRefresh(slotRefreshOld);
                  slotRefreshOld = slotRefreshOld.next
                ) {
                  /*@empty*/
                }
          ...
          if (sequenceNew) {
            ...
            while (!slotBefore.firstInSequence) {
              ...
              if (indexBefore !== undefined) {
                ...
                if (
                  slotCollisionBefore &&
                  slotCollisionBefore !== slotBefore.prev
                ) {
            ...
            while (!slotAfter.lastInSequence) {
              ...
              if (indexAfter !== undefined) {
                ...
                if (
                  slotCollisionAfter &&
                  slotCollisionAfter !== slotAfter.next
                ) {
              ...
              if (firstPlaceholder !== undefined) {
                if (lastItem !== undefined) {
                  ...
                  queueFetchFromIndex(
                    fetches,
                    slotRequestedByIndex,
                    requestedIndexOffset + 1,
                    placeholderCount - requestedIndexOffset
                  );
                }
              }
    ...
    function potentialMirage(slot) {
      return (
        (slot.kind === "placeholder" && !slot.indexRequested) ||
        !slotRequested(slot)
      );
    }
    ...
    function mergePossible(
      ...
      if (
        slotBeforeEnd.next !== slotAfterStart &&
        sequenceStart(slotBefore) === slotsStart &&
        sequenceEnd(slotAfter) === slotsEnd
      ) {
        return false;
      }
    ...
    function removeMiragesAndMerge(slotBefore, slotAfter) {
      // If slotBefore and slotAfter aren't in adjacent sequences, ensure that at least one of them can be moved
      if (
        sequenceEnd(slotBefore).next !== sequenceStart(slotAfter) &&
        sequenceStart(slotBefore) === slotsStart &&
        sequenceEnd(slotAfter) === slotsEnd
      ) {
        return false;
        ...
        if (indexChanged) {
          forEachBindingRecordOfSlot(slot, function (bindingRecord) {
            if (bindingRecord.notificationHandler.indexChanged) {
              handlerToNotify(bindingRecord).indexChanged(
                slot.handle,
                slot.index,
                indexOld
              );
    ...
    function updateIndices() {
      ...
      var slotFirstInSequence, indexNew;
    ...
    function processResultsAsynchronously(
      slot,
      refreshID,
      fetchID,
      results,
      ...
      (function () {
        ...
        while (true) {
          if (offsetBefore > 0) {
            ...
            var slotExpectedBefore = slotFromResult(
              results[offsetBefore - 1],
              candidateKeyMap
            );
            if (slotExpectedBefore) {
              if (
                slotBefore.firstInSequence ||
                slotExpectedBefore !== slotBefore.prev
              ) {
                if (!mergePossible(slotExpectedBefore, slotBefore)) {
                  ...
                  synchronousCallback &&
                  mergeRequiresNotifications(slotExpectedBefore, slotBefore)
                ) {
                  // Process these results from an asynchronous call
                  processResultsAsynchronously(
                    slot,
                    refreshID,
                    fetchID,
                    results,
                  ...
                  while (
                    offsetMirageBefore > 0 &&
                    !slotMirageBefore.firstInSequence
                  ) {
        ...
        while (true) {
          if (offsetAfter < resultsCount - 1) {
            // There are still results to process
            var slotExpectedAfter = slotFromResult(
              results[offsetAfter + 1],
              candidateKeyMap
            );
            if (slotExpectedAfter) {
              if (
                slotAfter.lastInSequence ||
                slotExpectedAfter !== slotAfter.next
              ) {
                if (!mergePossible(slotAfter, slotExpectedAfter)) {
                  ...
                  synchronousCallback &&
                  mergeRequiresNotifications(slotAfter, slotExpectedAfter)
                ) {
                  // Process these results from an asynchronous call
                  processResultsAsynchronously(
                    slot,
                  ...
                  while (
                    offsetMirageAfter < resultsCount - 1 &&
                    !slotMirageAfter.lastInSequence
                  ) {
        ...
        updateItemRange(
          slotBefore,
          slotAfter,
          index - offset + offsetBefore,
          slot.released ? null : slot,
          slotFirst,
          slotLast
        );
      })();
    ...
    function processResultsForIndex(
      indexRequested,
      slot,
      refreshID,
      results,
      ...
      if (results === UI.FetchError.noResponse) {
        ...
        var resultsCount = results.length;
        if (
          slot.index >= indexFirst &&
          slot.index < indexFirst + resultsCount
        ) {
    ...
    function reduceReleasedSlotCount() {
      ...
      var releasedSlotsTarget = Math.max(
        releasedSlotsMax / 2,
        Math.min(releasedSlotsMax * 0.9, releasedSlotsMax - 10)
      );
    ...
    function slotFromIndex(index) {
      ...
      if (slot === slotsEnd) {
        ...
        var slotNext = successorFromIndex(
          index,
          indexMap,
          slotsStart,
          slotsEnd
        ...
        if (
          (slot.firstInSequence || slot.prev.kind !== "placeholder") &&
          (slot.lastInSequence || slotNext.kind !== "placeholder")
        ) {
    ...
    function releaseSlot(slot) {
      ...
      if (!refreshInProgress) {
        ...
        if (releasedSlotsMax === 0) {
          ...
          if (
            !releasedSlotReductionInProgress &&
            releasedSlots > releasedSlotsMax
          ) {
    ...
    function adjustedIndex(slot) {
      ...
      return typeof slot.indexNew === "number"
        ? slot.indexNew + delta
    ...
    function updateNewIndicesAfterSlot(slot, indexDelta) {
      ...
      for (slot = slot.next; slot; slot = slot.next) {
        if (slot.firstInSequence) {
          var indexNew =
            slot.indexNew !== undefined ? slot.indexNew : slot.index;
          if (indexNew !== undefined) {
    ...
    function moveItem(slot, slotMoveBefore, append, applyEdit) {
      return new Promise(function (complete, error) {
        var slotNext, firstInSequence, lastInSequence;
      ...
      this.inserted = function (newItem, previousKey, nextKey, index) {
        ...
        if (editsQueued) {
          ...
          if (
            ...
            (slotPrev &&
              slotNext &&
              (slotPrev.next !== slotNext ||
                slotPrev.lastInSequence ||
                slotNext.firstInSequence))
          ) {
            ...
            insertNewSlot(
              key,
              newItem,
              slotNext ? slotNext : slotPrev.next,
              !!slotPrev,
              !!slotNext
            );
    ...
    this.createListBinding = function (notificationHandler) {
      ...
      function adjustCurrentSlot() {
        moveCursor(
          !slotCurrent ||
            slotCurrent.lastInSequence ||
            slotCurrent.next === slotsEnd
            ? null
            : slotCurrent.next
        );
      }
      ...
      var listBinding = {
        ...
        previous: function () {
          return itemPromiseFromSlot(
            slotCurrent ? previousSlot(slotCurrent) : null
          );
        },

        next: function () {
          return itemPromiseFromSlot(
            slotCurrent ? nextSlot(slotCurrent) : null
          );
        },
      ...
      if (listDataAdaptor.itemsFromKey || listDataAdaptor.itemsFromIndex) {
        listBinding.fromKey = function (key, prefetchBefore, prefetchAfter) {
          return itemPromiseFromSlot(
            slotFromKey(key, prefetchBefore, prefetchAfter)
          );
        };
      }
      ...
      if (
        listDataAdaptor.itemsFromIndex ||
        (listDataAdaptor.itemsFromStart && listDataAdaptor.itemsFromKey)
      ) {
        listBinding.fromIndex = function (
          index,
          prefetchBefore,
          prefetchAfter
        ) {
          return itemPromiseFromSlot(
            slotFromIndex(index, prefetchBefore, prefetchAfter)
          );
        };
      }

      if (listDataAdaptor.itemsFromDescription) {
        listBinding.fromDescription = function (
          description,
          prefetchBefore,
          prefetchAfter
        ) {
          return itemPromiseFromSlot(
            slotFromDescription(description, prefetchBefore, prefetchAfter)
          );
        };
      }
    ...
    if (listDataAdaptor.insertAtStart) {
      this.insertAtStart = function (key, data) {
        ...
        return insertItem(
          ...
          slotsStart.lastInSequence ? null : slotsStart.next,
          true,
    ...
    if (listDataAdaptor.insertBefore) {
      this.insertBefore = function (key, data, nextKey) {
        ...
        return insertItem(
          ...
          // applyEdit
          function () {
            return listDataAdaptor.insertBefore(
              key,
              data,
              nextKey,
              adjustedIndex(slotNext)
            );
          }
        );
      };
    ...
    if (listDataAdaptor.insertAfter) {
      this.insertAfter = function (key, data, previousKey) {
        ...
        return insertItem(
          ...
          // slotInsertBefore, append
          slotPrev ? slotPrev.next : null,
          true,

          // applyEdit
          function () {
            return listDataAdaptor.insertAfter(
              key,
              data,
              previousKey,
              adjustedIndex(slotPrev)
            );
          }
        );
      };
    ...
    if (listDataAdaptor.insertAtEnd) {
      this.insertAtEnd = function (key, data) {
        ...
        return insertItem(
          ...
          // slotInsertBefore, append
          slotsEnd.firstInSequence ? null : slotsEnd,
          false,
    ...
    if (listDataAdaptor.moveBefore) {
      this.moveBefore = function (key, nextKey) {
        ...
        return moveItem(
          ...
          // applyEdit
          function () {
            return listDataAdaptor.moveBefore(
              key,
              nextKey,
              adjustedIndex(slot),
              adjustedIndex(slotNext)
    ...
    if (listDataAdaptor.moveAfter) {
      this.moveAfter = function (key, previousKey) {
        ...
        return moveItem(
          ...
          // applyEdit
          function () {
            return listDataAdaptor.moveAfter(
              key,
              previousKey,
              adjustedIndex(slot),
              adjustedIndex(slotPrev)
...
(function (global) {
  ...
  var invalidElement =
    "Error: expected a DOM element or an HTML string (with a single root element).";
  ...
  function loadItemResources(subtree, complete) {
    ...
    for (
      var element = subtree, elementPrev = null;
      elementPrev !== subtree || element !== elementPrev.nextSibling;
  ...
  function createTask(
    ...
    if (
      outstandingResourceFetches < simultaneousResourceFetches ||
  ...
  WinJS.Namespace.define("WinJS.UI", {
    ...
    RenderManager: WinJS.Class.define(
      function (renderer, keyOf) {
        // Constructor
      ...
      },
      {
        ...
        render: function (item, priority) {
          ...
          return new WinJS._PerfMeasurement_Promise(
  ...
  UI.RenderManager.itemID = function (ownerElement, key) {
    ...
    return (
      ownerID +
      "_" +
      key.replace(/[`'"<>&\\\/]/g, function (character) {
...
(function (global) {
  ...
  var StorageDataAdaptor = WinJS.Class.define(
    function (query, options) {
      ...
      this._query = query;
      this._loader = new Windows.Storage.BulkAccess.FileInformationFactory(
        this._query,
        mode,
        size,
        flags,
    ...
    },
    {
      ...
      itemsFromEnd: function (count) {
        ...
        return this.getCount().then(function (totalCount) {
          ...
          return that.itemsFromIndex(
            totalCount - 1,
            Math.min(totalCount - 1, count - 1),
            1
          );
        });
      },
      ...
      itemsFromIndex: function (index, countBefore, countAfter) {
        ...
        return this._loader
          .getItemsAsync(first, count)
          .then(function (itemsVector) {
      ...
      itemsFromDescription: function (description, countBefore, countAfter) {
        var that = this;
        return this._query
          .findStartIndexAsync(description)
          .then(function (index) {
  ...
  WinJS.Namespace.define("WinJS.UI", {
    ...
    StorageDataSource: function (query, options) {
      return new WinJS.UI.ListDataSource(
        new StorageDataAdaptor(query, options)
      );
    },
  });
})();
...
(function (global, WinJS, undefined) {
  ...
  function _ListviewAnimationRemoveStage(itemsRemoved, canvas) {
    ...
    for (var i = 0, len = itemIDs.length; i < len; i++) {
      ...
      if (itemAnimationStage) {
        ...
        if (
          itemAnimationStage instanceof _ListviewAnimationAddStage &&
          !itemAnimationStage.running()
        ) {
  ...
  _ListviewAnimationRemoveStage.prototype.animateStage = function () {
    ...
    return Promise.any([
      Animation.createDeleteFromListAnimation(items).execute(),
      Promise.timeout(expectedRemoveDuration),
    ]).then(done, done);
  };
  ...
  // it too.
  function _ListviewAnimationMoveStage(
    itemsMoved,
    gridLayout,
    useFadeAnimation,
    canvas
      ...
      if (itemAnimationStage) {
        ...
        if (!itemAnimationStage.running()) {
          if (itemAnimationStage instanceof _ListviewAnimationMoveStage) {
            ...
            itemAnimationStage.removeItemFromStage(itemID);
            this._useFadeAnimation =
              this._useFadeAnimation || itemAnimationStage._useFadeAnimation;
  ...
  _ListviewAnimationMoveStage.prototype.animateStage = function () {
    ...
    var animation = this._gridLayout
      ? this._useFadeAnimation
        ? this.createFadeReflowAnimation(itemIDs)
        : this.createGridReflowAnimation(itemIDs)
      : this.createListReflowAnimation(itemIDs);
    return animation.then(done, done);
  };

  _ListviewAnimationMoveStage.prototype.createGridReflowAnimation = function (
    itemIDs
  ) {
    var clones = [],
      movedItems = [],
    ...
    for (var i = 0, len = itemIDs.length; i < len; i++) {
      ...
      if (!itemData.isHeader) {
        if (
          itemData.oldColumn !== undefined &&
          itemData.oldColumn !== itemData.column
        ) {
          var cloneEnd, actualStart;
          if (itemData.oldColumn > itemData.column) {
            ...
            actualStart = -(canvasHeight + itemData.oldTop);
            cloneEnd =
              canvasHeight - itemData.oldTop + itemData.top + itemData.oldTop;
          }
    ...
    var animation = Animation.createDeleteFromListAnimation(
      [junkDiv],
      movedItems
    );
    ...
    return Promise.any([
      animation.execute(),
      Promise.timeout(expectedMoveDurationReflow),
    ]).then(done, done);
  };

  _ListviewAnimationMoveStage.prototype.createListReflowAnimation = function (
    itemIDs
  ) {
  ...
  _ListviewAnimationMoveStage.prototype.createFadeReflowAnimation = function (
    itemIDs
  ) {
    ...
    for (i = 0, len = itemIDs.length; i < len; i++) {
      ...
      var itemData = this._affectedItems[itemIDs[i]];
      if (
        itemData.oldLeft !== itemData.left ||
        itemData.oldTop !== itemData.top
      ) {
        var elementStyle = itemData.element.style;
    ...
    return Promise.any([
      WinJS.UI._ListViewAnimationHelper._slowCrossFade(
        fadedInLayer,
        fadedOutLayer
      ),
      Promise.timeout(expectedMoveDurationFade),
    ]).then(done, done);
  };
  ...
  _ListviewAnimationAddStage.prototype.animateStage = function () {
    ...
    return Promise.any([
      Animation.createAddToListAnimation(items).execute(),
      Promise.timeout(expectedAddDuration),
    ]).then(done, done);
  };
  ...
  _ListviewAnimationAddStage.prototype.updateItemLocation = function (
    itemData
  ) {
    itemData.element.style.left = itemData.left + "px";
    itemData.element.style.top = itemData.top + "px";
  ...
  function _ListViewAnimationTracker(
    removeStage,
    moveStage,
    addStage,
    oldTracker
  ...
  _ListViewAnimationTracker.prototype = {
    ...
    nextStage: function () {
      ...
      var targetStage =
        this._currentStage === AnimationStage.move
          ? this._moveStage
          : this._currentStage === AnimationStage.add
          ? this._addStage
          : null;
  ...
  WinJS.Namespace.define("WinJS.UI", {
    _ListViewAnimationHelper: {
      ...
      _slowCrossFade: function (incoming, outgoing) {
        ...
        var promise1 = WinJS.UI.executeTransition(incoming, {
          name: "opacity",
          delay: 0,
          duration: 500,
          timing: "linear",
        ...
        var promise2 = WinJS.UI.executeTransition(outgoing, {
          name: "opacity",
          delay: 0,
          duration: 500,
          timing: "linear",
      ...
      animateListFadeBetween: function (
        oldAnimationTracker,
        canvas,
        affectedItems,
        inserted,
        ...
        return new _ListViewAnimationTracker(
          new _ListviewAnimationRemoveStage(removed, canvas),
          new _ListviewAnimationMoveStage(affectedItems, true, true, canvas),
          new _ListviewAnimationAddStage(inserted, canvas),
          oldAnimationTracker
        );
      },

      animateEntrance: function (canvas, firstEntrance) {
        Animation.transitionContent(canvas, [
          { left: firstEntrance ? "100px" : "40px", top: "0px" },
        ]);
      },
...
(function (global, WinJS, undefined) {
  ...
  WinJS.UI._SelectionMode.prototype = {
    initialize: function (modeSite) {
      ...
      function createArrowHandler(direction) {
        return function (oldFocus) {
          ...
          return modeSite._layout.getAdjacent(
            oldFocus,
            items.rootElement(items.itemAt(oldFocus)),
            direction
          );
        };
      }
      ...
      this.keyboardNavigationHandlers[Key.upArrow] = createArrowHandler(
        WinJS.UI._UP
      );
      this.keyboardNavigationHandlers[Key.downArrow] = createArrowHandler(
        WinJS.UI._DOWN
      );
      this.keyboardNavigationHandlers[Key.leftArrow] = createArrowHandler(
        WinJS.UI._LEFT
      );
      this.keyboardNavigationHandlers[Key.rightArrow] = createArrowHandler(
        WinJS.UI._RIGHT
      );
      this.keyboardNavigationHandlers[Key.pageUp] = createArrowHandler(
        WinJS.UI._PAGEUP
      );
      this.keyboardNavigationHandlers[Key.pageDown] = createArrowHandler(
        WinJS.UI._PAGEDOWN
      );
      ...
      this.keyboardAcceleratorHandlers[Key.a] = function () {
        if (that.site._multiSelection()) {
          view.finalItem(function (maxIndex) {
            that.site._selection.set(
              new WinJS.UI.ListViewItems([
                {
                  begin: 0,
                  end: maxIndex,
                },
    ...
    staticMode: function () {
      return (
        this.site._tap === WinJS.UI.Tap.none &&
        this.site._selectionMode === WinJS.UI.SelectionMode.none
      );
    },

    togglePressed: function (add) {
      if (!this.staticMode()) {
        utilities[add ? "addClass" : "removeClass"](
          this.pressedItem,
          WinJS.UI._pressedClass
        );
    ...
    handleTap: function (itemIndex) {
      ...
      if (site._selectionAllowed() && site._selectOnTap()) {
        if (
          site._selectionMode === WinJS.UI.SelectionMode.single ||
          site._selectionMode === WinJS.UI.SelectionMode.extended
        ) {
          if (!selected) {
    ...
    handleCrossSlide: function (itemIndex) {
      ...
      if (
        site._selectionAllowed() &&
        site._crossSlide === WinJS.UI.CrossSlide.select
      ) {
    ...
    startDragAndDrop: function (pressedPosition, cancelCapture) {
      ...
      if (dragInfo.dragData.count() > 0) {
        ...
        var site = this.site;
        site._pushMode(
          new WinJS.UI._DragSourceMode(
            site,
            dragInfo.dragData,
            dragInfo.selectedItems,
            dragInfo.thumbnailIndex,
            dragInfo.thumbnail,
            dragInfo.offset,
            cancelCapture
          )
    ...
    selectionAllowed: function () {
      ...
      if (site._selectionAllowed()) {
        ...
        return (
          site._element.dispatchEvent(eventObject) &&
          (selected ||
            newSelection.getAllIndices().indexOf(this.pressedIndex) !== -1)
        );
      } else {
        return false;
      }
      ...
      if (!selected) {
        (this.animations[pressedIndex] || Promise.wrap()).then(function () {
          ...
          if (pressedElement === root) {
            site
              ._renderSelection(pressedIndex, pressedElement, true)
              .then(function (root) {
                var style = root.style,
                  selectionCheckmark = root.querySelector(
                    "." + WinJS.UI._selectionCheckmarkClass
                  ),
                  selectionBackground = root.querySelector(
                    "." + WinJS.UI._selectionBackgroundClass
                  );
        ...
        site._view.items.rootElement(pressedElement).style.zIndex =
          WinJS.UI._THUMBNAIL_ZINDEX;
      }
    },
    ...
    isInteractive: function (element) {
      var matches = element.parentNode.querySelectorAll(
        ".win-interactive, .win-interactive *"
      );
    ...
    onMSPointerDown: function (eventObject) {
      ...
      if (
        (touchInput || leftButton) &&
        this.pressedIndex === WinJS.UI._INVALID_INDEX &&
        !this.isInteractive(eventObject.srcElement)
      ) {
        ...
        if (this.pressedIndex !== WinJS.UI._INVALID_INDEX) {
          ...
          var pressedElement = items.itemAt(this.pressedIndex),
            ...
            crossSlide =
              touchInput && site._crossSlide === WinJS.UI.CrossSlide.select;

          this.canDrag =
            this.prepareDragAndDrop(this.pressedPosition).dragData.count() > 0;
          ...
          if (this.gestureRecognizer) {
            this.gestureRecognizer.processDownEvent(
    ...
    onMSPointerMove: function (eventObject) {
      if (this.gestureRecognizer) {
        this.gestureRecognizer.processMoveEvents(
      ...
      if (this.crossSlideState !== null) {
        ...
        if (
          Math.abs(this.pressedPosition.left - clicked.left) >
            WinJS.UI._DRAG_START_THRESHOLD ||
          Math.abs(this.pressedPosition.top - clicked.top) >
            WinJS.UI._DRAG_START_THRESHOLD
        ) {
          var that = this;
    ...
    onMSPointerUp: function (eventObject) {
      if (this.gestureRecognizer) {
        this.gestureRecognizer.processUpEvent(
          Windows.UI.Input.PointerPoint.getCurrentPoint(eventObject.pointerId)
        );
      }
      ...
      if (!touchInput && this.pressedIndex !== WinJS.UI._INVALID_INDEX) {
        if (eventObject.ctrlKey) {
          ...
          currentFocus !== WinJS.UI._INVALID_INDEX
        ) {
      ...
      if (this.pointerId === eventObject.pointerId) {
        ...
        if (this.pressedItem) {
          if (this.crossSlideState !== CrossSlideState.completed) {
            ...
            this.clearItem(
              this.pressedIndex,
              this.site._selection.isSelected(this.pressedIndex) ||
                (this.crossSlideSelected && this.crossSlideSelectionChanged)
            );

            if (!eventObject.ctrlKey && !eventObject.shiftKey) {
              var items = site._view.items;
              this.pressedItem = items.rootElement(
                items.itemAt(this.pressedIndex)
              );
    ...
    startCrossSlide: function () {
      if (window.Windows) {
        if (!this.cachedRecognizer || this.cachedRecognizer.isActive) {
          ...
          this.cachedRecognizer.gestureSettings =
            settings.crossSlide |
            settings.hold |
            settings.manipulationTranslateX |
            settings.manipulationTranslateY;

          var that = this;
          this.cachedRecognizer.addEventListener(
            "crosssliding",
        ...
        this.gestureRecognizer.crossSlideHorizontally =
          !this.site._layout.horizontal;
      }
    },
    ...
    selfReveal: function () {
      ...
      if (!this.crossSlideSelfReveal) {
        ...
        AnimationHelper.selfReveal(this.pressedItem, offset).then(function () {
          that.pressedItem.style.msTransform =
            that.crossSlideTransform +
            " translate(" +
            offset.left +
            "px, " +
            offset.top +
    ...
    animateSelectionChange: function (select) {
      ...
      var toAnimate = [],
        selectionCheckmark = this.pressedItem.querySelector(
          "." + WinJS.UI._selectionCheckmarkClass
        ),
        selectionBackground = this.pressedItem.querySelector(
          "." + WinJS.UI._selectionBackgroundClass
        );
      ...
      if (this.selectionHint) {
        utilities[classOperation](
          this.selectionHint.querySelector("." + WinJS.UI._selectionHintClass),
          WinJS.UI._revealedClass
        );
      }
    },
    ...
    endCrossSlide: function (animateBack) {
      ...
      return new Promise(function (complete) {
        ...
        function cleanUp() {
          var wasSelected =
            that.site._selection.isSelected(pressedIndex) ||
            (that.crossSlideSelected && that.crossSlideSelectionChanged);
        ...
        if (!that.pressedItem) {
          ...
          that.animations[pressedIndex] = Animation.crossSlideSelect(
            that.pressedItem,
            []
          );
      ...
      if (this.pressedItem) {
        if (this.crossSlideState !== eventObject.crossSlidingState) {
          if (eventObject.crossSlidingState === CrossSlideState.started) {
            this.crossSlideStart = Math.floor(
              eventObject.position[this.site._layout.horizontal ? "y" : "x"]
            );
            ...
            eventObject.crossSlidingState === CrossSlideState.completed
          ) {
            var that = this,
              site = this.site,
              selection = site._selection,
            ...
            eventObject.crossSlidingState === CrossSlideState.rearranging
          ) {
            if (this.canDrag) {
            ...
            CrossSlideState.selected(eventObject.crossSlidingState) &&
            !CrossSlideState.selected(this.crossSlideState) &&
            this.canSelect
          ) {
            this.animateSelectionChange(
              !this.site._selection.isSelected(this.pressedIndex)
            );
          } else if (
            !CrossSlideState.selected(eventObject.crossSlidingState) &&
        ...
        var lastState = this.canDrag
          ? CrossSlideState.rearranging
          : CrossSlideState.selectSpeedBumping;
        if (eventObject.crossSlidingState < lastState) {
          var offset =
            Math.floor(
              eventObject.position[this.site._layout.horizontal ? "y" : "x"]
            ) - this.crossSlideStart;
          if (this.crossSlideSelfReveal) {
            offset = Math.max(offset, WinJS.UI._SELFREVEAL_OFFSET);
          }
          var transform =
            "translate" +
            (this.site._layout.horizontal ? "Y" : "X") +
            "(" +
            offset +
            "px)";
          this.pressedItem.style.msTransform =
            this.crossSlideTransform + " " + transform;
        }
      }
    },
    ...
    addSelectionHint: function () {
      if (!this.site._selection.isSelected(this.pressedIndex)) {
        ...
        var that = this;
        this.site._layout
          .getItemPosition(this.pressedIndex)
          .then(function (pos) {
    ...
    onKeyDown: function (eventObject) {
      ...
      if (!this.isInteractive(eventObject.srcElement)) {
        ...
        if (this.keyboardNavigationHandlers[keyCode]) {
          this.keyboardNavigationHandlers[keyCode](oldFocus).then(function (
            index
          ) {
            newFocus = index;
            setNewFocus();
          ...
          keyCode === Key.enter ||
          (keyCode === Key.space && !eventObject.ctrlKey)
        ) {
    ...
    reorderSupported: function () {
      return (
        this.site.reorder &&
        !this.site._groups.groupByFunction &&
        this.site._dragSupported()
      );
    },
...
(function (global, WinJS, undefined) {
  ...
  WinJS.UI._DragSourceMode.prototype = {
    ...
    onMSPointerMove: function (eventObject) {
      if (
        this.previousPosition.x !== eventObject.x ||
        this.previousPosition.y !== eventObject.y
      ) {
        ...
        if (this.target !== newTarget) {
          ...
          this.dragData.dropEffect =
            this.target !== WinJS.UI._INVALID_INDEX ? "copy" : "none";
        ...
        if (
          (eventObject.reorder || this.dragData.dropEffect === "move") &&
          !this.hidden
        ) {
          this.hideDraggedItems();
        } else if (
          !eventObject.reorder &&
          this.dragData.dropEffect !== "move" &&
          this.hidden
        ) {
          this.showDraggedItems();
        }
        ...
        style.cursor =
          this.dragData.dropEffect === "none" ? "no-drop" : "default";
    ...
    showDraggedItems: function () {
      ...
      Animation.createAddToListAnimation(animated, [])
        .execute()
        .then(done, done);
    },
    ...
    hideDraggedItems: function () {
      ...
      if (animated.length) {
        Animation.createDeleteFromListAnimation(animated, [])
          .execute()
          .then(done, done);
      } else {
        done();
    ...
    dragStartAnimations: function () {
      ...
      var toFade = [],
        selectionCheckmark = this.thumbnail.querySelector(
          "." + WinJS.UI._selectionCheckmarkClass
        ),
        selectionBackground = this.thumbnail.querySelector(
          "." + WinJS.UI._selectionBackgroundClass
        );
      ...
      var overlay = this.thumbnail.querySelector(
        "." + WinJS.UI._draggedOverlayClass
      );
      Promise.timeout().then(function () {
        AnimationHelper.fadeInDragOverlay(overlay);
      });

      var numberElement = this.thumbnail.querySelector(
        "." + WinJS.UI._draggedNumberClass
      );
      if (numberElement) {
    ...
    dragEndAnimations: function () {
      ...
      if (count > 1 || currentTarget.element !== this.site._element) {
        Animation.createDeleteFromListAnimation(thumbnail, [])
          .execute()
          .then(done, done);
      } else {
    ...
    callDragHandler: function (method, eventObject) {
      if (this.target !== WinJS.UI._INVALID_INDEX) {
        ...
        if (targetObject.element[WinJS.UI._DRAG_TARGET_EXPANDO][method]) {
          var cursorPosition = WinJS.UI._getCursorPos(eventObject);
          targetObject.element[WinJS.UI._DRAG_TARGET_EXPANDO][method](
            eventObject,
            {
              cursorPosition: {
    ...
    targetFromPosition: function (cursor) {
      for (var i = 0, len = this.targets.length; i < len; i++) {
        ...
        if (
          cursor.left >= target.left &&
          cursor.left < target.left + target.width &&
          cursor.top >= target.top &&
          cursor.top < target.top + target.height
        ) {
          return i;
        }
      }
    ...
    getTargets: function () {
      ...
      for (
        var element = document.body, elementPrev = null;
...
(function (global, WinJS, undefined) {
  ...
  WinJS.UI._DragTargetHandler.prototype = {
    onDragEnter: function (eventObject, detail) {
      var that = this,
        dragEnterEvent = this.fireDragEnterEvent(
      ...
      if (dragEnterEvent.detail.count && this.site._dragSupported()) {
        ...
        reorderData &&
        reorderData.uniqueID === this.site._element.uniqueID
      ) {
    ...
    onDragOver: function (eventObject, detail) {
      if (this.dragHandler) {
        ...
        this.site._layout.dragOver(
          detail.cursorPosition.x,
          detail.cursorPosition.y,
          this.count
        );
    ...
    translateCursorPosition: function (detail) {
      ...
      if (site._rtl() && site._horizontal()) {
        scrollPos =
          site._element[site._scrollLength] -
          scrollPos -
          site._viewport.offsetWidth;
      }
    ...
    inScrollZone: function (position) {
      ...
      if (this.site._horizontal()) {
        return (
          position.x < WinJS.UI._AUTOSCROLL_THRESHOLD ||
          position.x > viewportSize.width - WinJS.UI._AUTOSCROLL_THRESHOLD
        );
      } else {
        return (
          position.y < WinJS.UI._AUTOSCROLL_THRESHOLD ||
          position.y > viewportSize.height - WinJS.UI._AUTOSCROLL_THRESHOLD
        );
      }
    },
    ...
    autoScroll: function () {
      if (this.inScrollZone(this.cursorPosition)) {
        var site = this.site,
          scrollDelta =
            WinJS.UI._AUTOSCROLL_DELTA *
            (this.cursorPosition[site._horizontal() ? "x" : "y"] <
            WinJS.UI._AUTOSCROLL_THRESHOLD
              ? -1
              : 1);
    ...
    reorderItems: function (insertIndex, draggedItems) {
      ...
      if (site._element.dispatchEvent(eventObject)) {
        ...
        if (insertIndex < 0 || insertIndex >= site._cachedCount) {
          ...
          site._itemsManager.simplerItemAtIndex(
            insertIndex,
            function (nextElement) {
              move(
                dataSource.moveBefore.bind(dataSource),
                nextElement.msDataItem.key
              );
            }
          );
        }
...
(function (global, WinJS, undefined) {
  WinJS.UI._elementIsInvalid =
    "Invalid argument: ListView expects valid DOM element as the first argument.";
  WinJS.UI._layoutIsInvalid =
    "Invalid argument: layout must be one of following values: 'verticalgrid', " +
    "'horizontalgrid' or 'list'.";
  WinJS.UI._modeIsInvalid =
    "Invalid argument: mode must be one of following values: 'static', 'browse', " +
    "'singleselection' or 'multiselection'.";
  WinJS.UI._loadingBehaviorIsInvalid =
    "Invalid argument: loadingBehavior must be 'incremental' or 'randomaccess'.";
  WinJS.UI._itemIndexIsInvalid = "Invalid argument: index is invalid.";
  WinJS.UI._pagesToLoadIsInvalid =
    "Invalid argument: pagesToLoad must be a positive number.";
  WinJS.UI._pageLoadThresholdIsInvalid =
    "Invalid argument: pageLoadThreshold must be a positive number.";
  WinJS.UI._automaticallyLoadItemsIsInvalid =
    "Invalid argument: automaticallyLoadItems must be a boolean.";
...
(function (global, WinJS, undefined) {
  ...
  WinJS.UI._GroupsContainer.prototype = {
    addItem: function GroupsContainer_addItem(
      itemsManager,
      itemIndex,
      element,
      groupAddedCallback
      ...
      if (currentData && newGroupData.key === currentData.key) {
        ...
        nextGroup &&
        nextGroup.waitingList &&
        nextGroup.userData.key === this.groupByFunction(item).key
      ) {
    ...
    addGroup: function GroupsContainer_addGroup(
      currentGroup,
      currentIndex,
      toInsert
    ) {
    ...
    findStart: function GroupsContainer_findStart(
      itemsManager,
      group,
      itemIndex,
      counter
    ...
    groupFromImpl: function GroupsContainer_groupFromImpl(
      fromGroup,
      toGroup,
      comp
    ) {
      ...
      if (comp(centerGroup, center)) {
        ...
        center < toGroup &&
        !comp(this.groups[center + 1], center + 1)
      ) {
    ...
    renderGroup: function GroupsContainer_renderGroup(index) {
      var group = this.groups[index],
        element = this.site.groupRenderer
          ? this.site._headersPool.renderItem(group.userData)
          : null;
      return element;
    },
    ...
    rebuildGroups: function GroupsContainer_rebuildGroups(
      itemsManager,
      itemIndex,
      end,
...
(function (global, WinJS, undefined) {
  ...
  function getDimension(element, property) {
    return WinJS.Utilities.convertToPixels(
      element,
      window.getComputedStyle(element, null)[property]
    );
  }
  ...
  WinJS.Namespace.define("WinJS.UI", {
    ...
    _LayoutCommon: WinJS.Class.derive(WinJS.UI.Layout, function (options) {}, {
      ...
      reset: function () {
        ...
        this._leadingMargins =
          this._leadingGroupMargins =
          this._headerMargins =
            {
              left: 0,
              right: 0,
      ...
      _getSize: function () {
        if (
          ...
          var size =
            this._itemSize && typeof this._itemSize === "function"
              ? this._itemSize()
              : this._itemSize;
          this._totalItemWidth =
            this._itemWidth =
            this._offsetWidth =
              size.width;
          ...
          this._measuring = new Promise(function (complete) {
            function computeSizeOfRendered(element) {
              ...
              function measure() {
                if (
                  groupInfo &&
                  groupInfo(newGroup ? newGroup : that._dummyGroup.userData)
                    .multiSize
                ) {
                  that._rowHeight = groupInfo(
                    newGroup ? newGroup : that._dummyGroup.userData
                  ).slotHeight;
                } else {
      ...
      getAdjacent: function (index, element, direction) {
        var scrollbarPos = this._site.scrollbarPos,
          viewportLength =
            this._site.viewportSize[this.horizontal ? "width" : "height"],
        ...
        if (direction === WinJS.UI._PAGEUP) {
          return new Promise(function (complete) {
            that
              .calcFirstDisplayedItem(scrollbarPos, true)
              .then(function (firstElementOnPage) {
                if (index !== firstElementOnPage) {
                  complete(firstElementOnPage);
                } else {
                  that
                    .calcFirstDisplayedItem(
                      Math.max(
                        0,
                        scrollbarPos - viewportLength + currentItemLength
                      ),
                      false
                    )
                    .then(function (newFocus) {
          ...
          return new Promise(function (complete) {
            that
              .calcLastDisplayedItem(scrollbarPos, viewportLength, true)
              .then(function (lastElementOnPage) {
                if (index !== lastElementOnPage) {
                  complete(lastElementOnPage);
                } else {
                  that
                    .calcLastDisplayedItem(
                      scrollbarPos + viewportLength - currentItemLength,
                      viewportLength,
                      false
                    )
  ...
  FixedSizeDecorator.prototype = {
    getGroupSize: function (layout, group, groupIndex, itemsCount) {
      return (
        Math.ceil(itemsCount / layout._itemsPerColumn) *
          layout._getTotalItemWidth() +
        layout._headerSlot.cx +
        (groupIndex ? 0 : layout._leadingGroupMargin)
      );
    },

    calcItemPosition: function (layout, group, groupIndex, index) {
      var coordinates = layout._indexToCoordinate(
          group ? index - group.startIndex : index
        ),
        pos = {
          left:
            (group ? group.offset : 0) +
            layout._headerSlot.cx +
            (groupIndex ? 0 : layout._leadingGroupMargin) +
            coordinates.column * layout._getTotalItemWidth() +
            layout._leadingMargin,
          top:
            layout._headerSlot.cy +
            coordinates.row * layout._getTotalItemHeight(),
      ...
      if (layout._rtl) {
        pos.left =
          layout._getCanvasWidth(layout._count) -
          pos.left -
          layout._getTotalItemWidth();
      }
    ...
    getAdjacent: function (
      ...
      return new Promise(function (complete) {
        var currentColumn = Math.floor(
          (index - group.startIndex) / layout._itemsPerColumn
        );
        ...
        switch (direction) {
          ...
          case WinJS.UI._LEFT:
            complete(
              currentColumn > 0
                ? { index: index - layout._itemsPerColumn }
                : { group: -1 }
            );
            break;
          case WinJS.UI._RIGHT:
            var lastColumnOfGroup = Math.floor(
              groupSize / layout._itemsPerColumn
            );
            complete(
              currentColumn < lastColumnOfGroup
                ? {
                    index: Math.min(
                      index + layout._itemsPerColumn,
                      group.startIndex + groupSize - 1
                    ),
                  }
                : { group: 1 }
            );
            break;
          default:
            layout._super.getAdjacent
              .call(layout, index, element, direction)
  ...
  VariableSizeDecorator.prototype = {
    getGroupSize: function (layout, group, groupIndex, itemsCount) {
      ...
      if (this.occupancyMap.length > 0) {
        measuredItems =
          Math.ceil((this.occupancyMap.length - 1) / layout._itemsPerColumn) *
          groupInfo.slotWidth;
      ...
      var otherItems =
        Math.ceil(itemsCount / layout._itemsPerColumn) * groupInfo.slotWidth +
        layout._headerSlot.cx +
        (groupIndex ? 0 : layout._leadingGroupMargin);
    ...
    markSlotAsFull: function (layout, index, itemEntry) {
      ...
      for (
        var r = coordinates.row, toRow = coordinates.row + itemEntry.rows;
        r < toRow;
        r++
      ) {
        for (
          var c = coordinates.column,
            toColumn = coordinates.column + itemEntry.columns;
          c < toColumn;
          c++
        ) {
    ...
    isSlotEmpty: function (layout, itemSize, row, column) {
      for (var r = row, toRow = row + itemSize.rows; r < toRow; r++) {
        for (
          var c = column, toColumn = column + itemSize.columns;
          c < toColumn;
          c++
        ) {
          if (
            r >= layout._itemsPerColumn ||
            this.occupancyMap[this.coordinateToIndex(layout, c, r)] !==
              undefined
          ) {
            return false;
          }
    ...
    findEmptySlot: function (layout, startIndex, itemSize, newColumn) {
      var coordinates = layout._indexToCoordinate(startIndex),
        ...
        lastColumn = Math.floor(
          (this.occupancyMap.length - 1) / layout._itemsPerColumn
        );
    ...
    findItem: function (index) {
      for (
        var inMapIndex = index, len = this.occupancyMap.length;
        inMapIndex < len;
        inMapIndex++
      ) {
    ...
    getItemSize: function (layout, group, element, index) {
      ...
      return {
        ...
        rows: Math.min(
          layout._itemsPerColumn,
          Math.max(1, Math.ceil(totalItemHeight / groupInfo.slotHeight))
        ),
      };
    },
    ...
    addItemToMap: function (layout, group, index) {
      ...
      function add(mapEntry, newColumn) {
        var inMapIndex = that.findEmptySlot(
          layout,
          that.lastAdded,
          mapEntry,
          newColumn
      ...
      return new Promise(function (complete) {
        ...
        if (layout._itemSize && typeof layout._itemSize === "function") {
          ...
          if (size.width && size.height) {
            ...
            mapEntry = {
              ...
              rows: Math.min(
                layout._itemsPerColumn,
                Math.max(1, Math.ceil(size.height / groupInfo.slotHeight))
              ),
            };
          }
        ...
        if (mapEntry) {
          ...
          function processElement(element) {
            inMapIndex = add(
              that.getItemSize(layout, group, element, index),
              newColumn
            );
            complete(inMapIndex);
          }

          layout._site.itemsManager.simplerItemAtIndex(
            group.startIndex + index,
            whenElementRendered(that, processElement)
          );
        }
      });
    },
        ...
        this.adding[index] = new Promise(function (complete) {
          if (index > 0) {
            that.ensureInMap(layout, group, index - 1).then(function () {
              that
                .addItemToMap(layout, group, index)
                .then(function (inMapIndex) {
                  complete(inMapIndex);
                });
            });
          } else {
    ...
    calcItemPosition: function (layout, group, groupIndex, index) {
      ...
      return new Promise(function (complete) {
        that
          .ensureInMap(layout, group, index - group.startIndex)
          .then(function (inMapIndex) {
            var groupInfo = layout._getGroupInfo(group),
              ...
              pos = {
                left:
                  ...
                  layout._headerSlot.cx +
                  (groupIndex ? 0 : layout._leadingGroupMargin) +
                  coordinates.column * groupInfo.slotWidth +
                  layout._leadingMargin,
                top:
                  layout._headerSlot.cy +
                  coordinates.row * groupInfo.slotHeight,
    ...
    itemFromOffset: function (
      ...
      if (this.occupancyMap.length > 0) {
        ...
        if (offset < measuredWidth) {
          var counter = layout._itemsPerColumn,
            index =
              (Math.max(0, Math.floor(offset / groupInfo.slotWidth)) + last) *
                layout._itemsPerColumn -
              last;
      ...
      return (
        ...
        (Math.max(
          0,
          Math.floor((offset - measuredWidth) / groupInfo.slotWidth)
        ) +
          last) *
          layout._itemsPerColumn -
        last
    ...
    getAdjacent: function (
        ...
        switch (direction) {
          case WinJS.UI._UP:
            if (row > 0) {
              ...
              for (c = column - 1; c >= 0; c--) {
                entry =
                  this.occupancyMap[this.coordinateToIndex(layout, c, row)];
                if (!entry || entry.index !== index) {
          ...
          case WinJS.UI._RIGHT:
            inMapIndex =
              column < lastColumn
                ? Math.min(
                    inMapIndex + layout._itemsPerColumn,
                    this.occupancyMap.length - 1
                  )
                : this.occupancyMap.length;
            break;
        }

        inMap = inMapIndex >= 0 && inMapIndex < this.occupancyMap.length;
        if (inMap) {
          newIndex = that.occupancyMap[inMapIndex]
            ? that.occupancyMap[inMapIndex].index
            : undefined;
        }
      ...
      return Promise.wrap(
        inMap
          ? { index: group.startIndex + newIndex }
          : { group: inMapIndex < 0 ? -1 : 1 }
      );
    },
  };
    ...
    GridLayout: WinJS.Class.derive(
      ...
      },
      {
            ...
            if (count) {
              that._measureItems(that._groupInfo).then(function (initalized) {
                if (initalized) {
                  ...
                  that._leadingMargin =
                    that._leadingMargins[that._rtl ? "right" : "left"];
                  that._headerMargin =
                    that._headerMargins[that._rtl ? "right" : "left"];
                  that._leadingGroupMargin =
                    that._leadingGroupMargins[that._rtl ? "right" : "left"];
                  ...
                  that._itemsPerColumn = Math.floor(
                    (that._site.viewportSize.height - that._headerSlot.cy) /
                      (that._rowHeight
                        ? that._rowHeight
                        : that._getTotalItemHeight())
                  );
                  if (that._maxRows) {
                    that._itemsPerColumn = Math.min(
                      that._itemsPerColumn,
                      that._maxRows
                    );
                  }
                  ...
                  that
                    .calcFirstDisplayedItem(from, false)
                    .then(function (begin) {
                      that
                        .calcLastDisplayedItem(from, to - from, false)
                        .then(function (last) {
                          var end = last + 1;
        ...
        _getCanvasWidth: function (count) {
          ...
          return (
            lastGroup.offset +
            this._decorateGroup(lastGroup).getGroupSize(
              this,
              lastGroup,
              groupIndex,
        ...
        getAdjacent: function (index, element, direction) {
          ...
          return new Promise(function (complete) {
            ...
            that
              ._decorateGroup(group)
              .getAdjacent(
                that,
                group,
                that._getItemsCount(group, groupIndex),
                index,
                element,
                that._adjustDirection(direction)
              )
              .then(function (newPosition) {
                if (newPosition.group) {
                  ...
                  if (newGroupIndex < 0) {
                    ...
                    complete(
                      newGroup.startIndex +
                        that._getItemsCount(newGroup, newGroupIndex) -
                        1
                    );
                  } else {
        ...
        _calcItemPosition: function (index, groupIndex) {
          ...
          return this._decorateGroup(group).calcItemPosition(
            this,
            group,
            groupIndex,
            index
        ...
        layoutItem: function (itemIndex, element) {
          ...
          this._calcItemPosition(itemIndex, groupIndex).then(function (
            itemPos
          ) {
            ...
            if (
              ...
              itemData.left = itemPos.left;
              itemData.top = itemPos.top;
              var coordinates = that._indexToCoordinate(
                groupIndex >= 0
                  ? itemIndex - that._site.groups.group(groupIndex).startIndex
                  : itemIndex
              );
        ...
        layoutHeader: function (groupIndex, element) {
          ...
          if (this._rtl) {
            pos.left =
              this._getCanvasWidth(this._count) -
              pos.left -
              this._totalHeaderWidth -
              (groupIndex ? 0 : this._leadingGroupMargin);
          }

          if (group.left !== pos.left || group.top !== pos.top) {
            if (this._animateEndLayout) {
              this._headersMoved.push({
                element: element,
                left: pos.left,
                top: pos.top,
              });
        ...
        endLayout: function () {
          ...
          var affectedItems = {},
            ...
            firstPromise = this.calcFirstDisplayedItem(
              Math.max(0, scrollbarPos - viewportSize),
              false
            ),
            lastPromise = this.calcLastDisplayedItem(
              scrollbarPos + viewportSize,
              viewportSize,
              false
            ),
          ...
          WinJS.Promise.join([firstPromise, lastPromise]).then(function () {
            var variablySizedItemsFound = false,
              ...
              len;
            for (
              i = firstNearbyItem - 1, len = lastNearbyItem + 1;
              i < len;
              i++
            ) {
              var itemData = that._getItemInfo(i);
              if (itemData && itemData.element) {
                variablySizedItemsFound =
                  variablySizedItemsFound ||
                  that._multiSize(groups.group(groups.groupFromItem(i)));
                var itemRecord =
                  that._cachedItemRecords[itemData.element.uniqueID];
                if (
                  itemRecord &&
                  (itemRecord.oldRow !== itemData.row ||
                    itemRecord.oldColumn !== itemData.column ||
                    itemRecord.oldLeft !== itemData.left ||
                    itemRecord.oldTop !== itemData.top)
                ) {
            ...
            if (variablySizedItemsFound) {
              that._trackedAnimation = AnimationHelper.animateListFadeBetween(
                that._trackedAnimation,
                that._site.scrollableElement,
                affectedItems,
                insertedMap,
                removedMap,
                that._headersMoved
              ...
              that._trackedAnimation = AnimationHelper.animateReflow(
                that._trackedAnimation,
                that._site.scrollableElement,
                affectedItems,
                insertedMap,
        ...
        _getItemsCount: function (group, groupIndex) {
          if (groupIndex + 1 < this._site.groups.length()) {
            return (
              this._site.groups.group(groupIndex + 1).startIndex -
              group.startIndex
            );
          } else {
        ...
        calcLastDisplayedItem: function (
          scrollbarPos,
          viewportLength,
          wholeItem
        ) {
            ...
            if (that._initialized()) {
              ...
              if (that._count && typeof groupIndex === "number") {
                ...
                if (offset - groupOffset >= that._headerSlot.cx) {
                  ...
                  complete(
                    Math.min(startIndex + index, startIndex + groupSize)
                  );
                } else {
                ...
                complete(
                  Math.ceil(
                    Math.max(
                      0,
                      offset - that._leadingGroupMargin - that._leadingMargin
                    ) / that._getTotalItemWidth()
                  ) *
                    that._itemsPerColumn -
                    1
                );
        ...
        hitTest: function (x, y) {
          if (this._count) {
            ...
            var groupIndex = i - 1,
              ...
              startIndex = group.startIndex,
              index = this._decorateGroup(group).itemFromOffset(
                this,
                group,
                groupIndex,
                x,
              ...
              row = Math.min(
                this._itemsPerColumn - 1,
                Math.floor(
                  (y - this._headerSlot.cy) / this._getTotalItemHeight()
                )
              );
        ...
        dragOver: function (x, y) {
          ...
          return new Promise(function (complete) {
            function firstRow(index) {
              return (
                that._itemsPerColumn > 1 &&
                that._indexToCoordinate(index).row === 0
              );
            }
            ...
            function lastRow(index) {
              return (
                that._itemsPerColumn > 1 &&
                that._indexToCoordinate(index).row === that._itemsPerColumn - 1
              );
            }
            ...
            if (itemIndex !== -1) {
              that.getItemPosition(itemIndex).then(function (itemPos) {
                ...
                var beforeItem =
                    lastRow(beforeIndex) && !secondHalf
                      ? null
                      : that._getItemInfo(beforeIndex).element,
                  afterItem =
                    firstRow(afterIndex) && secondHalf
                      ? null
                      : that._getItemInfo(afterIndex).element;
                ...
                if (that._itemsPerColumn > 1) {
                  ...
                  offsets = [
                    { left: -WinJS.UI._DRAG_BETWEEN_OFFSET },
                    { left: WinJS.UI._DRAG_BETWEEN_OFFSET },
                  ];
                }

                that._dragBetweenEnter(
                  [that._beforeItem, that._afterItem],
                  offsets
                );
        ...
        _decorateGroup: function (group) {
          if (!group.decorator) {
            group.decorator = this._multiSize(group)
              ? new VariableSizeDecorator()
              : new FixedSizeDecorator();
          }
        ...
        dataModified: function (inserted, removed, itemsMoved) {
          if (
            this._initialized() &&
            (inserted.length > 0 || removed.length > 0 || itemsMoved)
          ) {
            ...
            var firstNearbyItem,
              ...
              viewportSize = this._site.viewportSize.width,
              firstPromise = this.calcFirstDisplayedItem(
                Math.max(0, this._site.scrollbarPos - viewportSize),
                false
              ),
              lastPromise = this.calcLastDisplayedItem(
                this._site.scrollbarPos + viewportSize,
                viewportSize,
                false
              );
            ...
            if (count) {
              ...
              for (var i = 0; i < count; i++) {
                ...
                if (previousGroup) {
                  ...
                  group.offset =
                    previousGroup.offset +
                    this._decorateGroup(previousGroup).getGroupSize(
                      this,
                      previousGroup,
                      i - 1,
                      itemsCount
              ...
              if (this._site.groups.pinnedItem !== undefined) {
                var pinnedGroupIndex = this._site.groups.groupFromItem(
                    this._site.groups.pinnedItem
                  ),
                  pinnedGroup = this._site.groups.group(pinnedGroupIndex),
                  pinnedCoordinates = this._indexToCoordinate(
                    this._site.groups.pinnedItem - pinnedGroup.startIndex
                  ),
                  pinnedGroupOffset =
                    this._site.groups.pinnedOffset.left -
                    this._headerSlot.cx -
                    (pinnedGroupIndex ? 0 : this._leadingGroupMargin) -
                    this._leadingMargin -
                    this._decorateGroup(pinnedGroup).itemOffset(
                      this,
                      pinnedGroup,
                      this._site.groups.pinnedItem
                    ),
...
(function (global, WinJS, undefined) {
  ...
  WinJS.UI._IncrementalView.prototype = {
    addItem: function IncrementalView_addItem(
      fragment,
      itemIndex,
      currentPass,
      finishCallback
      ...
      this.site._itemsManager.simplerItemAtIndex(
        itemIndex,
        function (item) {
          ...
          function configure() {
            if (that.site._isSelected(itemIndex)) {
              that.site
                ._renderSelection(itemIndex, item, true, true)
                .then(completeItemLayout);
            } else {
          ...
          if (item.parentNode !== fragment) {
            if (itemIndex === 0) {
              ...
              that.items
                .requestItem(itemIndex - 1)
                .then(function (previousItem) {
                  insert(
                    item,
                    nextItemSibling(that.items.rootElement(previousItem))
                  );
                });
            }
    ...
    realizeItems: function IncrementalView_realizeItem(
      fragment,
      begin,
      end,
      count,
      ...
      if (counter !== 0) {
        for (var itemIndex = begin; itemIndex < end; itemIndex++) {
          ...
          if (!item) {
            ...
            this.site._layout.layoutItem(
              itemIndex,
              this.items.rootElement(item)
            );
            callCallback();
          }
        }
        ...
        this.site._itemsCount(function (count) {
          if (!that.destroyed && that.realizePass === currentPass) {
            if (count > that.lastItem + 1) {
              ...
              that.site._layout
                .startLayout(0, that.pagesToLoad * viewportLength, count)
                .then(function (range) {
                  if (range) {
                    var end = Math.min(
                        count,
                        range.end - range.begin + that.lastItem + 1
                      ),
    ...
    updateItems: function IncrementalView_updateItems(callback) {
      ...
      this.site._itemsCount(function (count) {
        if (!that.destroyed && that.realizePass === currentPass) {
          if (count !== 0) {
            that.site._layout
              .startLayout(0, scrollbarPos + viewportLength, that.lastItem + 1)
              .then(function (range) {
                if (range) {
                  that.realizeItems(
                    that.site._canvas,
    ...
    download: function IncrementalView_download(action, callback) {
      ...
      if (
        this.site._cachedCount === WinJS.UI._UNINITIALIZED ||
        this.lastItem === WinJS.UI._UNINITIALIZED
      ) {
    ...
    checkProgressBarVisibility:
      function IncrementalView_checkProgressBarVisibility() {
        var viewportLength = this.site._getViewportLength(),
          scrollBarPos = this.site.scrollPosition;

        if (
          this.site._cachedCount > this.lastItem + 1 &&
          this.loadingInProgress &&
          scrollBarPos >=
            this.site._canvas[
              this.site._layout.horizontal ? "offsetWidth" : "offsetHeight"
            ] -
              viewportLength
        ) {
          this.showProgressBar();
    ...
    scrollbarAtEnd: function IncrementalView_scrollbarAtEnd(
      scrollbarPos,
      scrollLength,
      viewportSize
    ) {
      ...
      if (last) {
        lastOffset =
          last[this.site._layout.horizontal ? "offsetLeft" : "offsetTop"];
      }

      return (
        scrollbarPos + viewportLength >
        lastOffset - viewportLength * this.pageLoadThreshold
      );
    },
    ...
    onScroll: function IncrementalView_onScroll(
      scrollbarPos,
      scrollLength,
      viewportSize
    ) {
      this.checkProgressBarVisibility();
      if (
        this.scrollbarAtEnd(scrollbarPos, scrollLength, viewportSize) &&
        this.automaticallyLoadItems
      ) {
        this.download(this.loadNextChunk.bind(this));
      } else {
    ...
    refresh: function IncrementalView_refresh(
      scrollbarPos,
      scrollLength,
      viewportSize,
      newCount
      ...
      this.download(function (callback) {
        that.updateItems(function () {
          if (
            that.scrollbarAtEnd(scrollbarPos, scrollLength, viewportSize) &&
            newCount > end &&
            that.automaticallyLoadItems
          ) {
    ...
    updateScrollbar: function IncrementalView_updateScrollbar() {
      ...
      if (this.lastItem !== -1) {
        this.site._layout
          .getScrollbarRange(this.lastItem + 1)
          .then(function (range) {
            that.site._setCanvasLength(
              range.end - range.begin + WinJS.UI._INCREMENTAL_CANVAS_PADDING
            );
          });
      } else {
...
(function (global, WinJS, undefined) {
  ...
  WinJS.UI._ItemsContainer.prototype = {
    ...
    setPlaceholderAt: function ItemsContainer_setPlaceholderAt(
      itemIndex,
      placeholder
    ) {
    ...
    rootElement: function ItemsContainer_rootElement(element) {
      if (
        element &&
        element.parentNode &&
        element.parentNode !== this.site._canvas
      ) {
...
(function (global, WinJS, undefined) {
  ...
  ListLayout.prototype = {
    ...
    getItemPosition: function (itemIndex) {
      ...
      pos.offsetWidth =
        pos.totalWidth - (this._totalItemWidth - this._offsetWidth);
    ...
    endLayout: function () {
      ...
      WinJS.Promise.join([firstPromise, lastPromise]).then(function () {
        ...
        that._trackedAnimation = AnimationHelper.animateListReflow(
          that._trackedAnimation,
          that._site.scrollableElement,
          affectedItems,
          insertedMap,
    ...
    calcFirstDisplayedItem: function (scrollbarPos, wholeItem) {
      ...
      return new Promise(function (complete, error) {
        if (that._initialized()) {
          complete(
            Math[wholeItem ? "ceil" : "floor"](
              scrollbarPos / that._totalItemHeight
            )
          );
        } else {
          error(WinJS.UI._layoutNotInitialized);
    ...
    calcLastDisplayedItem: function (scrollbarPos, viewportLength, wholeItem) {
      ...
      return new Promise(function (complete, error) {
        if (that._initialized()) {
          complete(
            Math[wholeItem ? "floor" : "ceil"](
              (scrollbarPos + viewportLength) / that._totalItemHeight
            ) - 1
          );
        } else {
          error(WinJS.UI._layoutNotInitialized);
  ...
  FlowLayout.prototype = {
    ...
    endLayout: function () {
      if (this._animateEndLayout) {
        ...
        AnimationHelper.animateListReflow(
          this._site.scrollableElement,
          [],
          this._cachedInserted,
          []
        );
      ...
      if (bottom < threshold) {
        return (
          this.lastIndex + Math.ceil((threshold - bottom) / this.average) + 1
        );
      } else {
        return null;
      }
    ...
    ListLayout: WinJS.Class.derive(
      ...
      },
      {
        ...
        getAdjacent: function (index, element, direction) {
          ...
          switch (direction) {
            ...
            case WinJS.UI._RIGHT:
              ...
              break;
            default:
              return this._super.getAdjacent.call(
                this,
                index,
                element,
                direction
        ...
        setSite: function (layoutSite) {
          ...
          var worker =
            this._site.loadingBehavior === "incremental"
              ? new FlowLayout()
              : new ListLayout();
...
(function (global, WinJS, undefined) {
  ...
  var ZoomableView = WinJS.Class.define(
    function (listView) {
      // Constructor
  ...
  WinJS.Namespace.defineWithParent(WinJS, "UI", {
    ...
    ListView: WinJS.Class.define(
      function (element, options) {
        ...
        this._itemsPool = new WinJS.UI._ItemsPool(
          this,
          WinJS.UI._itemClass,
          WinJS.UI._selectedClass
        );
        this._headersPool = new WinJS.UI._ItemsPool(
          this,
          WinJS.UI._headerClass
        );
        if (!options.dataSource) {
          this._dataSource = new WinJS.UI.ArrayDataSource(childElements, {
            compareByIdentity: true,
          });
      ...
      },
      {
        ...
        groupDataSource: {
          ...
          set: function (newValue) {
            ...
            if (newValue && newValue._groupOf && newValue._groupOf()) {
              this._groups = new WinJS.UI._GroupsContainer(
                this,
                newValue._groupOf()
              );
            } else {
        ...
        loadingBehavior: {
          get: function () {
            return this._view instanceof WinJS.UI._IncrementalView
              ? "incremental"
              : "randomaccess";
          },
          set: function (newValue) {
        ...
        selection: {
          ...
          set: function (newSelection) {
            ...
            if (ranges.length > 0) {
              var multi =
                ranges.length > 1 || ranges[0].begin !== ranges[0].end;
              if (
                !this._selectionAllowed() ||
                (multi && !this._multiSelection())
              ) {
                throw new Error(WinJS.UI._itemIndexIsInvalid);
              }
            }
        ...
        lastVisible: function ListView_lastVisible() {
          ...
          return this._layout
            .calcLastDisplayedItem(
              this.scrollPosition,
              this._getViewportLength(),
              false
            )
            .then(function (lastVisible) {
              return Promise.wrap(Math.min(that._cachedCount - 1, lastVisible));
            });
        },

        addEventListener: function ListView_addEventListener(
          eventName,
          eventCallback,
          capture
        ) {
        ...
        removeEventListener: function ListView_removeEventListener(
          eventName,
          eventCallback,
          capture
        ) {
        ...
        dataObject: function ListView_dataObject(itemIndex, callback) {
          if (itemIndex >= 0 && itemIndex < this._cachedCount) {
            var something = this._itemsManager.simplerItemAtIndex(
              itemIndex,
              function (element) {
        ...
        _setupInternalTree: function ListView_setupInternalTree() {
          ...
          utilities[this._rtl() ? "addClass" : "removeClass"](
            this._element,
            WinJS.UI._rtlListViewClass
          );
        ...
        _unsetFocusOnItem: function ListView_unsetFocusOnItem(newFocusExists) {
          if (!newFocusExists) {
            ...
            this._keyboardEventsHelper._shouldHaveFocus =
        ...
        _setFocusOnItem: function ListView_setFocusOnItem(index) {
          ...
          var that = this;
          this._focusRequest = this._view.items
            .requestItem(index)
            .then(function (item) {
        ...
        _attachEvents: function ListView_attachEvents() {
          ...
          elementEvents.forEach(function (elementEvent) {
            that._element.attachEvent(
              "on" + elementEvent.name,
          ...
          elementEvents.forEach(function (eventHandler) {
            that._element.addEventListener(
              eventHandler.name,
              eventHandler.handler,
          ...
          var viewportEvents = [listViewHandler("Scroll")];

          viewportEvents.forEach(function (viewportEvent) {
            that._viewport.addEventListener(
              viewportEvent.name,
              viewportEvent.handler,
              false
            );
          });
        },
        ...
        _updateItemsManager: function ListView_updateItemsManager() {
            ...
            notificationHandler = {
              ...
              changed: function ListView_changed(newItem, oldItem) {
                ...
                if (elementInfo) {
                  ...
                  var placeholder =
                    that._updater.placeholders[oldItem.uniqueID];
                  if (placeholder) {
                    that._updater.placeholders[oldItem.uniqueID].element =
                      newItem;
                    that._updater.changed = true;
                  }
                }
                for (
                  var i = 0, len = that._notificationHandlers.length;
                  i < len;
                  i++
                ) {
              ...
              indexChanged: function ListView_indexChanged(
                item,
                newIndex,
                oldIndex
              ) {
              ...
              endNotifications: function ListView_endNotifications() {
                ...
                if (updater && updater.changed) {
                  ...
                  for (var i = 0, len = keys.length; i < len; i++) {
                    if (
                      !that._itemsManager.isPlaceholder(
                        that._insertedElements[keys[i]]
                      )
                    ) {
                  ...
                  that._layout.dataModified(
                    inserted,
                    updater.removedElements,
                    updater.itemsMoved
                  );
                  ...
                  for (i in updater.placeholders) {
                    if (updater.placeholders.hasOwnProperty(i)) {
                      ...
                      newPlaceholders[placeholder.newIndex] =
                        placeholder.element;
                    }
                  }
              ...
              itemAvailable: function ListView_itemAvailable(
                item,
                placeholder
              ) {
                var callbacksMap = that._itemsManager.callbacksMap,
                ...
                if (that._insertedElements[placeholder.uniqueID]) {
                  ...
                  if (that._updater && that._updater.changed) {
                    ...
                    item.style.opacity = 0.0;
                    AnimationHelper.animateItemAdded(
                      item,
                      index !== that._cachedCount - 1
                    );
                  }
                }
                ...
                if (
                  that._updater &&
                  that._updater.placeholders[placeholder.uniqueID]
                ) {
          ...
          this._itemsManager = thisWinUI.createItemsManager(
            this._dataSource,
            function (item) {
              return that._itemsPool.renderItem(item);
            },
            notificationHandler,
            {
          ...
          this._itemsManager.simplerItemAtIndex = function (
            index,
            itemAvailable,
            itemUnavailable,
        ...
        _unregisterNotificationHandler: function (handler) {
          for (
            var i = 0, len = this._notificationHandlers.length;
            i < len;
            i++
          ) {
        ...
        _onPropertyChange: function ListView_onPropertyChange() {
          if (
            event.propertyName === "dir" ||
            event.propertyName === "style.direction"
          ) {
            utilities[this._rtl() ? "addClass" : "removeClass"](
              this._element,
              WinJS.UI._rtlListViewClass
          ...
          if (!this._marginsPromise) {
            ...
            this._marginsPromise = new Promise(function (complete) {
              that._itemsManager.simplerItemAtIndex(0, function (element) {
                ...
                for (var i = 0, len = margins.length; i < len; i++) {
                  var margin = margins[i];
                  computed[margin] = window.getComputedStyle(element, null)[
                    margin
                  ];
        ...
        _renderSelection: function ListView_renderSelection(
          index,
          element,
          selected,
          aria
          ...
          return new Promise(function (complete) {
            that._layout.getItemPosition(index).then(function (pos) {
              that._getItemMargins().then(function (margins) {
                var root, next, elementStyle, rootStyle;
                ...
                utilities[selected ? "addClass" : "removeClass"](
                  root,
                  WinJS.UI._selectedClass
                );
                if (aria) {
        ...
        _flowLayout: function ListView__flowLayout() {
          return (
            this._view instanceof WinJS.UI._IncrementalView &&
            this._layout instanceof WinJS.UI.ListLayout
          );
        },

        _getViewportLength: function ListView_getViewportLength() {
          return this._getViewportSize()[
            this._horizontal() ? "width" : "height"
          ];
        },
        ...
        _setDragHandler: function ListView_setDragHandler() {
          this._element[WinJS.UI._DRAG_TARGET_EXPANDO] =
            new WinJS.UI._DragTargetHandler(this);
        },
        ...
        _rtl: function ListView_rtl() {
          return (
            window.getComputedStyle(this._element, null).direction === "rtl"
          );
        },
        ...
        _showProgressBar: function ListView_showProgressBar(parent, x, y) {
          ...
          if (!progressBar.parentNode) {
            ...
            this._progressIndicatorDelayTimer = Promise.timeout(
              WinJS.UI._LISTVIEW_PROGRESS_DELAY
            ).then(function () {
          ...
          if (selection.getIndicesCount() > 0 && this.editable) {
            ...
            if (handled) {
              ...
              selection.forEachIndex(function (index) {
                that._itemsManager.simplerItemAtIndex(
                  index,
                  function (element) {
                    that._itemsManager.dataSource.remove(
                      element.msDataItem.key
                    );
                  }
                );
        ...
        _executePaste: function ListView_executePaste() {
          ...
          if (clipboardContent && this.editable) {
            var e = document.createEvent("CustomEvent");
            e.initCustomEvent("itemspaste", false, true, {
              dataPackage: clipboardContent,
            });
        ...
        _configureForZoom: function (
          ...
          if (WinJS.validation) {
            if (
              !this._view.realizePage ||
              typeof this._view.begin !== "number"
            ) {
              // TODO: Replace with proper validation pattern
              throw new Error(
                "ListView can only be used with SemanticZoom if randomaccess loading behavior is specified."
              );
            }
          }
        ...
        _getCurrentItem: function () {
          ...
          if (typeof indexFocused !== "number") {
            ...
            this._setCurrentItem(
              0.5 * this._viewportWidth,
              0.5 * this._viewportHeight
            );
          ...
          var listBinding = this._dataSource.createListBinding(),
            promiseItem = listBinding
              .fromIndex(indexFocused)
              .then(function (item) {
                listBinding.release();
                return item;
              });
          ...
          var promisePosition = this._getItemOffsetPosition(indexFocused).then(
            function (posCanvas) {
              var scrollOffset = parseInt(
                that._canvas.style[that._startProperty],
                10
              );
        ...
        _beginZoom: function () {
          ...
          var horizontal = this._horizontal(),
            scrollOffset =
              horizontal && this._rtl()
                ? this.scrollPosition
                : -this.scrollPosition;

          this._viewport.style[horizontal ? "overflowX" : "overflowY"] =
            "visible";
          this._canvas.style[this._startProperty] = scrollOffset + "px";

          this._viewport.style[horizontal ? "overflowY" : "overflowX"] =
            "visible";
        },
        ...
        _positionItem: function (item, position) {
            ...
            return that
              ...
              .then(function (posCanvas) {
                ...
                if (rtl) {
                  ...
                  right = Math.max(
                    posCanvas.width,
                    Math.min(that._viewportWidth, right)
                  );

                  scrollPosition =
                    canvasSize -
                    (posCanvas.left + posCanvas.width) -
                    (that._viewportWidth - right);
                } else {
                  ...
                  var start = position[that._startProperty],
                    startMax =
                      viewportSize -
                      (horizontal ? posCanvas.width : posCanvas.height);
                ...
                var adjustedScrollPosition = Math.max(
                    0,
                    Math.min(canvasSize - viewportSize, scrollPosition)
                  ),
                  scrollAdjustment =
                    (rtl ? -1 : 1) * (adjustedScrollPosition - scrollPosition);
          ...
          if (typeof itemIndex === "number") {
            ...
            if (typeof key === "string" && listBinding.fromKey) {
              ...
              var description = this._isZoomedOut
              ...
              if (WinJS.validation) {
                if (description === undefined) {
                  // TODO: Replace with proper validation pattern
                  throw new Error(
                    "Item must provide index, key or description of corresponding item."
                  );
                }
              }
        ...
        _endZoom: function (isCurrentView) {
          ...
          var horizontal = this._horizontal(),
            scrollOffset = parseInt(
              this._canvas.style[this._startProperty],
              10
            );
          ...
          this.scrollPosition =
            horizontal && this._rtl() ? scrollOffset : -scrollOffset;
          this._disableEntranceAnimation = !isCurrentView;
        ...
        _getItemOffsetPosition: function (index) {
          ...
          function elementOffsetPosition(element) {
            ...
            return {
              left:
                elementRoot.offsetLeft +
                (element === elementRoot ? 0 : element.offsetLeft),
              top:
                elementRoot.offsetTop +
                (element === elementRoot ? 0 : element.offsetTop),
          ...
          return new Promise(function (complete) {
            ...
            if (elementAtIndex) {
              ...
              that._itemsCount(function (count) {
                that._layout.startLayout(0, 0, count).then(function () {
                  var promisePosition = that._layout
                      .getItemOffset(index)
                      .then(function () {
                  ...
                  if (elementRealized) {
                    ...
                    var offsetPosRealized =
                      elementOffsetPosition(elementRealized);

                    promiseResult = Promise.join([
                      promisePosition,
                      that._layout.getItemPosition(indexRealized),
                      ...
                      return {
                        left:
                          pos.left +
                          (offsetPosRealized.left - posRealized.left),
                        top:
                          pos.top + (offsetPosRealized.top - posRealized.top),
        ...
        _multiSelection: function ListView_multiSelection() {
          ...
          return (
            this._selectionMode === SelectionMode.multi ||
...
(function (global, WinJS, undefined) {
  ...
  WinJS.UI._ScrollView.prototype = {
    createItem: function ScrollView_createItem(
      itemIndex,
      available,
      unavailable
    ) {
      ...
      var something = this.site._itemsManager.simplerItemAtIndex(
        itemIndex,
        function (element) {
          that.site._groups.addItem(
            that.site._itemsManager,
            itemIndex,
            element,
            function () {
    ...
    addItem: function ScrollView_addItem(
      fragment,
      itemIndex,
      element,
      currentPass
      ...
      if (this.realizePass === currentPass) {
        ...
        if (layoutIndex !== WinJS.UI._INVALID_INDEX) {
          ...
          if (this.site._isSelected(itemIndex)) {
            this.site
              ._renderSelection(itemIndex, element, true, true)
              .then(layoutItem);
          } else {
            layoutItem(element);
          }
    ...
    updateItem: function ScrollView_updateItem(
      itemData,
      itemIndex,
      itemIsReadyCallback
    ) {
    ...
    realizeItems: function ScrollView_realizeItems(
      fragment,
      begin,
      end,
      count,
      ...
      var waitFor =
          (this.site._groups.groupByFunction && this.site._rtl()) ||
          this.firstLayoutPass
            ? end
            : -1,
        firstItem,
        lastItem,
      ...
      if (!this.firstLayoutPass) {
        var scrollbarPos = this.site.scrollPosition,
          firstPromise = this.site.layout.calcFirstDisplayedItem(
            scrollbarPos,
            false
          ),
          lastPromise = this.site.layout.calcLastDisplayedItem(
            scrollbarPos,
            this.site._getViewportLength(),
            false
          );
        ...
        Promise.join([firstPromise, lastPromise]).then(function () {
          ...
          for (
            var i = that._firstIndexDisplayed;
            i <= that._lastIndexDisplayed;
            i++
          ) {
      ...
      function newItemIsReady(itemIndex, element) {
        if (itemIndex < waitFor) {
          ...
          if (
            that._lastIndexDisplayed &&
            itemIndex >= that._firstIndexDisplayed &&
            itemIndex <= that._lastIndexDisplayed
          ) {
            that.site._hideProgressBar();
          }
        }
    ...
    deferAriaSetup: function ScrollView_deferAriaSetup(count) {
      var that = this,
        elementId = that.site._element.id
          ? that.site._element.id
          : that.site._element.uniqueID;
    ...
    realizePage: function ScrollView_realizePage(
      scrollbarPos,
      forceRelayout,
          ...
          if (count !== 0) {
            ...
            that.site._layout
              .startLayout(
                Math.max(
                  0,
                  scrollbarPos - that.pagesToPrefetch * viewportLength
                ),
                    ...
                    that.realizeItems(
                      ...
                      function (firstItem, lastItem) {
                        if (that.realizePass === currentPass) {
                          ...
                          that.site._layout
                            .getItemPosition(that.begin)
                            .then(function (position) {
                              that.site._groups.pinItem(that.begin, {
                                left: position.left,
                              });
                            });
    ...
    onScroll: function ScrollView_onScroll(
      scrollbarPos,
      scrollLength,
      viewportSize
    ) {
    ...
    refresh: function ScrollView_refresh(
      scrollbarPos,
      scrollLength,
      viewportSize,
      newCount
    ...
    updateScrollbar: function ScrollView_updateScrollbar(absolute) {
      ...
      if (this.site._cachedCount > 0) {
        this.site._layout
          .getScrollbarRange(this.site._cachedCount)
          .then(function (range) {
            that.site._setCanvasLength(
              absolute ? range.end - range.begin : range.end
            );
          });
      } else {
        this.site._setCanvasLength(0);
    ...
    fixScrollbarRange: function ScrollView_fixScrollbarRange(scrollbarPos) {
      ...
      if (this.site._groups.length() && this.site._groups.group(0).offset) {
        ...
        this.site._layout
          .calcFirstDisplayedItem(scrollbarPos)
          .then(function (firstDisplayed) {
            that.site._layout
              .getItemPosition(firstDisplayed)
              .then(function (position) {
                that.site._layout
                  .getScrollbarRange()
                  .then(function (scrollbarRange) {
                    if (scrollbarRange.begin < 0) {
                      var start = position[that.site._startProperty],
                        itemPos = start - scrollbarRange.begin;

                      that.site._groups.pinItem(firstDisplayed, {
                        left: itemPos,
                      });
...
(function (global, WinJS, undefined) {
  ...
  WinJS.Namespace.define("WinJS.UI", {
    ListViewItems: WinJS.Class.derive(
      Array,
      function (elements) {
      ...
      },
      {
        ...
        _set: function (elements) {
          ...
          if (
            ...
            !(elements instanceof WinJS.UI.ListViewItems)
          ) {
            elements = [elements];
          }
