...
(function (global) {
  if (global.msSetImmediate) {
    ...
    //
    var userFrameTime = frameLen * 0.7;
    ...
    dispatchEvent: function (type, details) {
      ...
      if (listeners) {
        ...
        for (
          ...
          i < len && !eventValue._stopImmediatePropagationCalled;
          i++
        ) {
...
(function (global, WinJS) {
  ...
  function isPromise(value) {
    return (
      typeof value === "object" && value && typeof value.then === "function"
    );
  }
  ...
  var PromiseBase = WinJS.Class.define(null, {
    _listeners: null,
  ...
  //
  var ThenPromise = WinJS.Class.derive(
    PromiseBase,
    function (creator) {
  ...
  var CompletePromise = WinJS.Class.derive(
    PromiseBase,
    function (value) {
  ...
  var ErrorPromise = WinJS.Class.derive(
    PromiseBase,
    function (error) {
      this._error(error);
    },
  ...
  //
  var Promise = WinJS.Class.derive(
    PromiseBase,
    ...
    },
    {
          ...
          function (c, e, p) {
            ...
            keys.forEach(function (key) {
              Promise.as(values[key]).then(
                function () {
                  c({ key: key, value: values[key] });
                },
                function () {
                  e({ key: key, value: values[key] });
                }
              );
            });
          },
      ...
      timeout: function (timeout) {
        ...
        /// </returns>
        if (!timeout) {
          return new WinJS.Promise(function timeoutComplete(c) {
            msQueueCallback(c);
          });
        } else {
          var id = 0;
  ...
  Object.defineProperties(
    Promise,
    WinJS.Utilities.createEventProperties(errorET)
  );
...
(function (global, WinJS) {
  WinJS.Namespace.define("WinJS", {
    ...
    //
    ErrorFromName: WinJS.Class.derive(
      Error,
...
(function (global, undefined) {
  ...
  var insertAdjacentHTML = (insertAdjacentHTMLUnsafe = function (
    element,
    position,
    text
  ) {
