...
*/
  ...
  var BindingInterpreter = WinJS.Class.derive(
    WinJS.UI._optionsParser._BaseInterpreter,
    ...
    },
    {
      _error: function (message) {
        var error = new Error(
          "Invalid binding, expected to be <destProp>:<sourceProp>;. " + message
        );
...
(function (WinJS, undefined) {
  var mixin = {
    ...
    _notifyListeners: function (name, newValue, oldValue) {
      ...
      if (listeners) {
        ...
        //
        var cap = (that._updatedValues[name] = {
          newValue: newValue,
          oldValue: oldValue,
          started: false,
        });
        ...
        //
        cap.promise = WinJS.Promise.timeout().then(function () {
          cap.started = true;
          var join;
          ...
          function cleanup() {
            cap.finished = true;
            if (cap === that._updatedValues[name]) {
              delete that._updatedValues[name];
            }
          }
          if (join) {
    ...
    unbind: function (name, action) {
      ...
      if (name && action) {
        ...
        if (that._updatedValues) {
          Object.keys(that._updatedValues).forEach(function (n) {
            that._cancel(n);
          });
        }
        this._listeners = {};
  ...
  var bind = function (observable, bindingDescriptor) {
    ...
    return {
      cancel: function () {
        cancelSimple();
        Object.keys(complexLast).forEach(function (k) {
          cancelComplex(k);
        });
      },
    };
  ...
  var unwrap = function (data) {
    ...
    /// If data is an observable proxy, the original object is returned, otherwise data is returned.
    /// </returns>
    if (data && data.backingData) return data.backingData;
    else return data;
  };
    ...
    Template: WinJS.Class.define(
          ...
          element.renderItem = function (item, recycled) {
            ...
            if (
              ...
              if (okToReuse) {
                ...
                // templates' output.
                //
                recycled.msRendererPromise = WinJS.Binding.processAll(
                  recycled,
                  item.data,
                  true,
                  that.bindingCache
      ...
      },
      {
        ...
        render: function (dataContext, container) {
          ...
          return WinJS.UI.Fragments.cloneTo(
            that.href || that.element,
            undefined,
            d
          )
            .then(function () {
              ...
              function complete() {
                return work(WinJS.UI.processAll).then(function () {
  ...
  Object.defineProperties(WinJS.Binding.Template, {
    ...
    render: {
      value: function (href, dataContext, container) {
        ...
        /// either container or the created DIV.
        /// </returns>
        return new WinJS.Binding.Template(null, { href: href }).render(
          dataContext,
          container
        );
      },
...
(function (WinJS, undefined) {
  ...
  function buildContext(element, baseElement, skipRoot) {
    ...
    while (element && !context) {
      ...
      if (
        element === baseElement ||
        (skipRoot && element.parentNode === baseElement)
      ) {
        break;
      }
  ...
  function inContainer(baseElement, control, start) {
    if (control && control.constructor.isDeclarativeControlContainer) {
      return true;
    }
    if (
      start !== baseElement &&
      start.parentNode &&
      start.parentNode !== baseElement
    ) {
    ...
    if (action instanceof Function) {
      ...
      if (cacheEntry) {
        if (result && result.cancel) {
          cacheEntry.bindings.push(function () {
            result.cancel();
          });
        } else {
      ...
      //
      action
        .render(getValue(source, bind.source), e)
        .then(function () {
  ...
  function declarativeBindImpl(
    ...
    for (var i = neg ? -1 : 0, l = elements.length; i < l; i++) {
      ...
      if (!declBind.implemented) {
        for (
          var bindIndex = 0, bindLen = declBind.length;
          bindIndex < bindLen;
          bindIndex++
        ) {
  ...
  function oneTime(source, sourceProperties, dest, destProperties) {
    ...
    dest[destProperties[destProperties.length - 1]] = getValue(
      source,
      sourceProperties
    );
    return { cancel: noop };
...
(function (global, undefined) {
  ...
  function cleanup() {
    if (U._DOMWeakRefTable_sweepPeriod === 0) {
      // If we're using post
  ...
  function scheduleCleanupIfNeeded() {
    if (
      (global.Debug && U._DOMWeakRefTable_noTimeoutUnderDebugger) ||
      cleanupToken
    ) {
      return;
    }
  ...
  function unscheduleCleanupIfNeeded() {
    ...
    if (period === 0) {
      // if we're using post
      if (!cleanupToken) {
        // and there isn't already one scheduled
        if (Object.keys(table).length !== 0) {
          // and there are items in the table
