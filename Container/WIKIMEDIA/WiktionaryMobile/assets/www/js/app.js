window.app = (function () {
  ...
  function loadPage(title, language) {
    ...
    function doRequest() {
      Page.requestFromTitle(title, language)
        .done(function (page) {
          if (page === null) {
            setErrorPage(404);
          }
  ...
  function mobileUrlForUrl(url) {
    ...
    if (url.indexOf(".m." + PROJECTNAME) < 0) {
      return (
        url.substring(0, url.indexOf(PROJECTNAME)) +
        "m." +
        url.substring(url.indexOf(PROJECTNAME))
      );
    }
    return url;
  }
  ...
  function navigateTo(title, lang, options) {
    var d = $.Deferred();
    var options = $.extend(
      { cache: false, updateHistory: true },
      options || {}
    );
