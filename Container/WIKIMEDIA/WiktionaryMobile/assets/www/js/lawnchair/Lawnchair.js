...
var Lawnchair = function () {
  ...
  if (arguments.length <= 2 && arguments.length > 0) {
    var callback =
        typeof arguments[0] === "function" ? arguments[0] : arguments[1],
  ...
  if (options.adapter) {
    adapter =
      Lawnchair.adapters[self.indexOf(Lawnchair.adapters, options.adapter)];
    adapter = adapter.valid() ? adapter : undefined;
    // otherwise find the first valid adapter for this env
  } else {
    for (var i = 0, l = Lawnchair.adapters.length; i < l; i++) {
      adapter = Lawnchair.adapters[i].valid()
        ? Lawnchair.adapters[i]
        : undefined;
      if (adapter) break;
    }
  }
...
Lawnchair.adapter = function (id, obj) {
  ...
  var implementing =
      "adapter valid init keys save batch get exists all remove nuke".split(
        " "
      ),
