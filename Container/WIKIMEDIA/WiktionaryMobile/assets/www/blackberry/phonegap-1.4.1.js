  ...
  Battery.prototype._stateChange = function (state) {
    ...
    if (state === 2 || state === 3) {
      ...
      if (state === 2 && (me._isPlugged === false || me._isPlugged === null)) {
        ...
        state === 3 &&
        (me._isPlugged === true || me._isPlugged === null)
      ) {
...
phonegap.PluginManager = (function (webworksPluginManager) {
      ...
      execute: function (webWorksResult, action, args, win, fail) {
        ...
        switch (action) {
          ...
          case "captureAudio":
            return {
              status: PhoneGap.callbackStatus.INVALID_ACTION,
              message: "captureAudio is not currently supported",
            };
        }
      ...
      execute: function (webWorksResult, action, args, win, fail) {
        ...
        switch (action) {
          case "getConnectionInfo":
            ...
            callbackID = blackberry.events.registerEventHandler(
              "networkChange",
              win
            );

            //pass our callback id down to our network extension
            request = new blackberry.transport.RemoteFunctionCall(
              "com/phonegap/getConnectionInfo"
            );
            ...
            return {
              status: PhoneGap.callbackStatus.OK,
              message: { type: connectionType, event: eventType },
            };
        }
        return retInvalidAction;
      },
    ...
    notificationAPI = {
      execute: function (webWorksResult, action, args, win, fail) {
        if (args.length !== 3) {
          return {
            status: 9,
            message: "Notification action - " + action + " arguments not found",
          };
        }
        ...
        switch (action) {
          case "alert":
            blackberry.ui.dialog.customAskAsync.apply(this, [
              msg,
              [btnLabel],
              win,
              { title: title },
  ...
  phonegap.PlayBookPluginManager.prototype.exec = function (
    win,
    fail,
    clazz,
    action,
    ...
    if (
      (wwResult.status === PhoneGap.callbackStatus.OK ||
        wwResult.status ===
          PhoneGap.callbackStatus.CLASS_NOT_FOUND_EXCEPTION) &&
      plugins[clazz]
    ) {
