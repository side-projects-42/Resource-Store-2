...
function setMenuItemState(action, state) {
  window.plugins.SimpleMenu.setMenuState(
    action,
    state,
    function () {},
    function () {}
...
function updateMenuState() {
  ...
  var menu_handlers = {
    "read-in": function () {
      languageLinks.showLangLinks(app.curPage);
    },
  ...
  window.plugins.SimpleMenu.loadMenu(
    $("#appMenu")[0],
    menu_handlers,
    function (success) {
      console.log(success);
...
window.preferencesDB.addOnSet(function (id, value) {
  window.plugins.preferences.set(
    id,
    value,
    function () {},
    function () {}
...
savedPages.doSave = function () {
  ...
  var gotPath = function (cachedPage) {
    $("#main img").each(function () {
      ...
      window.plugins.urlCache.getCachedPathForURI(
        target,
        function (imageFile) {
    ...
    chrome.showNotification(
      mw.message("page-saved", app.curPage.title).plain()
    );
    chrome.hideSpinner();
    d.resolve();
  };
