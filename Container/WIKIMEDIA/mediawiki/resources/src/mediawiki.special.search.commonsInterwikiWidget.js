(function () {
	var api = new mw.Api(),
		...
		moreResultsText = new mw.Message(
			mw.messages,
	...
	function itemTemplate(results) {
		...
		for (i = 0; i < results.length; i++) {
			result = results[i];
			imageCaption = mw.html.element(
				"span",
				{ class: "iw-result__mini-gallery__caption" },
				result.title
			);
	...
	function itemWrapperTemplate(pageQuery, itemTemplateOutput) {
		return (
			'<li class="iw-resultset iw-resultset--image" data-iw-resultset-pos="0">' +
			...
			'<a href="/w/index.php?title=Special:Search&search=' +
	...
	api.get({
		...
		multimediaWidgetTemplate = itemWrapperTemplate(
			pageUrl.query.search,
