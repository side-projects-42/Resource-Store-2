...
(function () {
	var cascadeableLevels = require("./config.json").CascadingRestrictionLevels,
		...
		levelSelectors = $("[id ^= mwProtect-level-]")
			.map(function () {
				return OO.ui.infuse(this);
			})
			.toArray(),
		expiryInputs = $("[id ^= mwProtect-][id $= -expires]")
			.map(function () {
				return OO.ui.infuse(this);
			})
			.toArray(),
		expirySelectors = $("[id ^= mwProtectExpirySelection-]")
			.map(function () {
				return OO.ui.infuse(this);
			})
			.toArray(),
	...
	function isUnchained() {
		return mwProtectUnchained.isElementAttached()
			? mwProtectUnchained.isSelected()
			: true;
	}
	...
	function getMaxLevel() {
		return Math.max.apply(
			Math,
			levelSelectors.map(function (widget) {
	...
	function setAllSelectors(val) {
		levelSelectors.forEach(function (widget) {
			if (typeof val === "number") {
				widget.setValue(
					widget.dropdownWidget.getMenu().getItems()[val].getData()
				);
			} else {
				widget.setValue(val);
			}
	...
	expiryInputs.forEach(function (widget) {
		widget.on("change", function (val) {
			if (isUnchained()) {
				if (val) {
					expirySelectors[expiryInputs.indexOf(widget)].setValue(
						"othertime"
					);
				}
			} else {
	...
	$("#mwProtect-reason input").codePointLimit(
