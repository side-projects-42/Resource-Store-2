...
(function () {
	...
	function justify() {
		...
		$gallery.children("li.gallerybox").each(function () {
			...
			captionWidth = $this
				.children()
				.children("div.gallerytextwrapper")
				.width();
			...
			for (i = 0; i < rows.length; i++) {
				...
				if (preferredHeight > curRowHeight * 1.5) {
					...
					if (i === rows.length - 1) {
						...
						avgZoom =
							(totalZoom / (rows.length - 1)) * curRowHeight;
						if (
							isFinite(avgZoom) &&
							avgZoom >= 1 &&
							avgZoom <= 1.5
				...
				for (j = 0; j < curRow.length; j++) {
					...
					if (newWidth < 60 || !isFinite(newWidth)) {
						...
						$caption.width(
							curRow[j].captionWidth +
								(newWidth - curRow[j].imgWidth)
						);
					}
			...
			$content
				...
				.on("focus blur", "a", function (e) {
					...
					$(this)
						...
						.toggleClass("mw-gallery-focused", gettingFocus);
				});
		}

		$galleries = $content.find(
