	...
	$(function () {
			...
			if (shouldTestSearch) {
				...
				requestAnimationFrame(function () {
					requestAnimationFrame(function () {
						...
						if (
							performance.getEntriesByName(loadStartMark)
								.length &&
							!performance.getEntriesByName(
								loadStartToFirstRenderMeasure
							).length
		...
		function renderFunction(text, context) {
			var formData = getFormData(context),
				textboxConfig =
					context.data.$textbox.data("mw-searchsuggest") || {};
		...
		function specialRenderFunction(query, context) {
			...
			if (
				mw.user.options.get("search-match-redirect") &&
				...
				$el.append(
					...
					$("<div>").addClass("special-query").text(query)
				).show();
			} else {
