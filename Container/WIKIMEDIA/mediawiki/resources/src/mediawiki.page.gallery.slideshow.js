...
(function () {
	...
	mw.GallerySlideshow = function (gallery) {
		...
		$(window).on(
			"resize",
			OO.ui.debounce(this.setSizeRequirement.bind(this), 100)
		);
	...
	mw.GallerySlideshow.prototype.drawCarousel = function () {
		...
		this.$imgCaption = $("<p>").attr(
			"class",
			"mw-gallery-slideshow-caption"
		);
		this.$imgContainer = $("<div>").attr(
			"class",
	...
	mw.GallerySlideshow.prototype.showCurrentImage = function (init) {
		...
		// 3. Copy caption
		this.$imgCaption.empty().append($caption.clone());
		...
		this.loadImage(this.$thumbnail)
			.done(
				function (info) {
					// Show this image to the user only if its still the current one
					if (
						this.$thumbnail.attr("src") === $thumbnail.attr("src")
					) {
	...
	mw.GallerySlideshow.prototype.getImageInfo = function ($img) {
		...
		if (this.imageInfoCache[imageSrc] === undefined) {
			...
			this.imageInfoCache[imageSrc] = api
				.get(params)
				.then(function (data) {
					if (
						OO.getProp(
