...
(function () {
	...
	function detectParserForColumn(table, rows, column) {
		...
		while (i < l) {
			...
			if (
				rows[rowIndex] &&
				!$(rows[rowIndex]).hasClass(config.cssChildRow)
			) {
				if (rowIndex !== lastRowIndex) {
					lastRowIndex = rowIndex;
					cellIndex = $(rows[rowIndex]).data("columnToCell")[column];
					nodeValue = getElementSortKey(
		...
		for (i = 0; i < totalRows; i++) {
			...
			if ($row.hasClass(config.cssChildRow)) {
				cache.row[cache.row.length - 1] =
					cache.row[cache.row.length - 1].add($row);
		...
		if ($tableRows.length <= 1) {
			...
			$tableRows.each(function (rowIndex, row) {
				...
				$.each(row.cells, function (columnIndex, cell) {
					var matrixRowIndex, matrixColumnIndex;
					...
					while (
						exploded[rowIndex] &&
						exploded[rowIndex][columnIndex] !== undefined
					) {
						++columnIndex;
					}
					...
					for (
						...
						matrixColumnIndex < columnIndex + colspan;
						++matrixColumnIndex
					) {
						for (
							matrixRowIndex = rowIndex;
							matrixRowIndex < rowIndex + rowspan;
							++matrixRowIndex
						) {
	...
	function multisort(table, sortList, cache) {
		...
		for (i = 0; i < sortList.length; i++) {
			...
			if (
				window.Intl &&
				Intl.Collator &&
				cachedParsers[sortList[i][0]].type === "text"
			) {
				sortFn[i] = sortText;
			} else {
	...
	function buildTransformTable() {
		var ascii,
			...
			separatorTransformTable = mw.config.get(
		...
		if (
			...
			(separatorTransformTable[0] === "" && digitTransformTable[2] === "")
		) {
			...
			ascii = separatorTransformTable[0]
				.split("\t")
				.concat(digitTransformTable[0].split("\t"));
			localised = separatorTransformTable[1]
				.split("\t")
	...
	function explodeRowspans($table) {
		...
		function filterfunc() {
			return (
				$.data(this, "tablesorter").realCellIndex >=
						...
						if (totalRows > 0) {
							...
							if (!e[config.sortMultiSortKey]) {
								...
								if (isValueInArray(col, config.sortList)) {
									...
									config.sortList =
										config.sortList.concat(newSortList);
								}
							}
							...
							appendToTable(
								$table[0],
								multisort($table[0], config.sortList, cache)
							);
	...
	ts.addParser({
		...
		is: function (s) {
			return (
				ts.dateRegex[0].test(s) ||
				ts.dateRegex[1].test(s) ||
				ts.dateRegex[2].test(s)
			);
		},
		format: function (s) {
			var match, y;
			...
			if ((match = s.match(ts.dateRegex[0])) !== null) {
				if (
					mw.config.get("wgDefaultDateFormat") === "mdy" ||
