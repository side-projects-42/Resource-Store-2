...
(function () {
	...
	function toggleElement($collapsible, action, $defaultToggle, options) {
		...
		if (!options.plainMode && $collapsible.is("table")) {
			...
			!options.plainMode &&
			($collapsible.is("ul") || $collapsible.is("ol"))
		) {
			// Lists
	...
	$.fn.makeCollapsible = function (options) {
		...
		this.each(function () {
			...
			collapseText =
				...
				$collapsible.attr("data-collapsetext") ||
				mw.msg("collapsible-collapse");
			expandText =
				options.expandText ||
			...
			buildDefaultToggleLink = function () {
				return $("<a>")
					...
					.text(collapseText)
					.wrap(
						'<span class="mw-collapsible-toggle mw-collapsible-toggle-default"></span>'
					)
					.parent()
					.attr({
						role: "button",
			...
			if ($customTogglers && $customTogglers.length) {
				...
				if ($collapsible.is("table")) {
					...
					if ($caption.length) {
						...
						if (!$toggle.length) {
							$toggle = buildDefaultToggleLink().prependTo(
								$firstItem.eq(-1)
							);
						}
					}
					...
					$collapsible.parent().children(".mw-collapsible").length ===
						1 &&
					...
					if (!$toggle.length) {
						...
						$toggle = buildDefaultToggleLink();
						$toggle
							.wrap('<li class="mw-collapsible-toggle-li"></li>')
							.parent()
					...
					if (
						!$collapsible.find("> .mw-collapsible-content").length
					) {
			...
			$toggle
				.on(
					"click.mw-collapsible keypress.mw-collapsible",
					actionHandler
				)
