...
(function () {
	...
	inspect.getModuleSize = function (moduleName) {
		...
		while (i--) {
			if (
				args[i] === null ||
				($.isPlainObject(args[i]) && $.isEmptyObject(args[i]))
			) {
				args.splice(i, 1);
			} else {
				break;
	...
	inspect.grep = function (pattern) {
		...
		return inspect.getLoadedModules().filter(function (moduleName) {
			...
			if (
				...
				pattern.test(module.script.toString())
			) {
				return true;
			}
	...
	inspect.reports = {
		...
		css: function () {
			...
			inspect.getLoadedModules().forEach(function (name) {
				var css,
					stats,
					module = mw.loader.moduleRegistry[name];
		...
		store: function () {
			var raw,
				stats = { enabled: mw.loader.store.enabled };
			if (stats.enabled) {
		...
		time: function () {
			...
			if (!mw.loader.profiler) {
				mw.log.warn(
					"mw.inspect: The time report requires $wgResourceLoaderEnableJSProfiler."
				);
				return [];
			}
