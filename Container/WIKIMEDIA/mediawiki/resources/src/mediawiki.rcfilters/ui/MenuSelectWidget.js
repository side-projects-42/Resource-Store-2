 ...
 *  ]
 */
MenuSelectWidget = function MwRcfiltersUiMenuSelectWidget(
	controller,
	model,
	config
) {
	...
	this.setGroupElement(
		$("<div>").addClass("mw-rcfilters-ui-menuSelectWidget-group")
	);
	...
	if (!config.isMobile) {
		...
		header = new FilterMenuHeaderWidget(this.controller, this.model, {
	...
	this.$element
		...
		.append(this.$body.append(this.$group, this.noResults.$element));

	// Append all footers; we will control their visibility
	// based on view
	config.footers = config.isMobile ? [] : config.footers || [];
	config.footers.forEach(
		function (footerData) {
			var isSticky =
					footerData.sticky === undefined
						? true
						: !!footerData.sticky,
				adjustedData = {
	...
	$.each(groups, function (groupName, groupModel) {
		if (!groupModel.isHidden()) {
			viewGroupCount[groupModel.getView()] =
				viewGroupCount[groupModel.getView()] || 0;
			viewGroupCount[groupModel.getView()]++;
		}
	});
...
MenuSelectWidget.prototype.updateView = function () {
	...
	if (this.views[viewName] && this.currentView !== viewName) {
		...
		this.$element
			.data("view", viewName)
			.removeClass(
				"mw-rcfilters-ui-menuSelectWidget-view-" + this.currentView
			)
...
MenuSelectWidget.prototype.updateFooterVisibility = function (currentView) {
	...
	this.footers.forEach(function (data) {
		data.$element.toggle(
			...
			!data.views ||
				data.views.length === 0 ||
				data.views.indexOf(currentView) > -1
		);
	});
};
...
MenuSelectWidget.prototype.onDocumentKeyDown = function (e) {
	...
	if (!this.isDisabled() && this.isVisible()) {
		...
		nextItem =
			nextItem && nextItem.constructor.static.selectable
				? nextItem
				: null;
