...
(function () {
	...
	mw.widgets.CalendarWidget.prototype.getDisplayLayers = function () {
		return ["month", "year", "duodecade"].slice(
			this.precision === "month" ? 1 : 0
		);
	};
	...
	mw.widgets.CalendarWidget.prototype.updateUI = function (fade) {
		...
		if (
			...
			this.previousMoment &&
			this.previousMoment.isSame(
				this.moment,
				this.precision === "month" ? "month" : "day"
			)
		) {
			// Already displayed
			return;
		...
		this.$oldBody = this.$body.addClass(
		...
		switch (this.displayLayer) {
					...
					items.push(
						$("<div>")
							...
							.toggleClass(
								...
								!currentDay.isSame(this.moment, "month")
							)
							.toggleClass(
				...
				for (i = 0; i < 12; i++) {
					items.push(
						$("<div>")
							...
							.toggleClass(
								"mw-widget-calendarWidget-item-selected",
								currentMonth.isSame(selected, "month")
							)
			...
			case "duodecade":
				...
				currentYear = moment({
					year: Math.floor(this.moment.year() / 20) * 20,
				});
				for (i = 0; i < 20; i++) {
					items.push(
						$("<div>")
							...
							.toggleClass(
								"mw-widget-calendarWidget-item-selected",
								currentYear.isSame(selected, "year")
							)
		...
		if (this.displayLayer === "month") {
			needsFade =
				needsFade || !this.moment.isSame(this.previousMoment, "month");
		} else if (this.displayLayer === "year") {
			needsFade =
				needsFade || !this.moment.isSame(this.previousMoment, "year");
		...
		if (fade && needsFade) {
			...
			setTimeout(function () {
				...
				$bodyWrapper.addClass(
					"mw-widget-calendarWidget-body-wrapper-fade-" + fade
				);
			});
		} else {
	...
	mw.widgets.CalendarWidget.prototype.onUpButtonClick = function () {
		var layers = this.getDisplayLayers(),
	...
	mw.widgets.CalendarWidget.prototype.setDateFromMoment = function () {
		...
		var newDate = moment(this.moment)
			.locale("en")
	...
	mw.widgets.CalendarWidget.prototype.onKeyDown = function (e) {
		var dir = OO.ui.Element.static.getDir(this.$element),
			nextDirectionKey =
				dir === "ltr" ? OO.ui.Keys.RIGHT : OO.ui.Keys.LEFT,
			prevDirectionKey =
				dir === "ltr" ? OO.ui.Keys.LEFT : OO.ui.Keys.RIGHT,
			changed = true;
		...
		if (!this.isDisabled()) {
			switch (e.which) {
				...
				case OO.ui.Keys.UP:
					this.moment.subtract(
						1,
						this.precision === "month" ? "month" : "week"
					);
					break;
				...
				case OO.ui.Keys.PAGEUP:
					this.moment.subtract(
						1,
						this.precision === "month" ? "year" : "month"
					);
					break;
