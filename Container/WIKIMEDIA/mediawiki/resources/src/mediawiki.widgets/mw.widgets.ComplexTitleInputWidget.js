...
(function () {
	...
	mw.widgets.ComplexTitleInputWidget =
		function MwWidgetsComplexTitleInputWidget(config) {
			...
			this.title = new mw.widgets.TitleInputWidget(
				$.extend({}, config.title, {
					relative: true,
			...
			this.$element
				...
				.append(this.namespace.$element, this.title.$element);
	 ...
	 * @inheritdoc
	 */
	mw.widgets.ComplexTitleInputWidget.static.reusePreInfuseDOM = function (
		node,
		config
	) {
		config =
			mw.widgets.ComplexTitleInputWidget.parent.static.reusePreInfuseDOM(
				node,
				config
			);
		config.namespace =
			mw.widgets.NamespaceInputWidget.static.reusePreInfuseDOM(
	 ...
	 * @inheritdoc
	 */
	mw.widgets.ComplexTitleInputWidget.static.gatherPreInfuseState = function (
		node,
		config
	) {
		var state =
			mw.widgets.ComplexTitleInputWidget.parent.static.gatherPreInfuseState(
				node,
				config
			);
		state.namespace =
			mw.widgets.NamespaceInputWidget.static.gatherPreInfuseState(
				$(node).find(".mw-widget-namespaceInputWidget"),
				config.namespace
			);
	 ...
	 * dropdown changes.
	 */
	mw.widgets.ComplexTitleInputWidget.prototype.updateTitleNamespace =
		function () {
	...
	mw.widgets.ComplexTitleInputWidget.prototype.setDisabled = function (
		...
		mw.widgets.ComplexTitleInputWidget.parent.prototype.setDisabled.call(
			this,
			disabled
		);
		if (this.namespace) {
