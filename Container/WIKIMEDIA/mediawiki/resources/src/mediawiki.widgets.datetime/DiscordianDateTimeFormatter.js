(function () {
	...
	mw.widgets.datetime.DiscordianDateTimeFormatter =
		function MwWidgetsDatetimeDiscordianDateTimeFormatter(config) {
			...
			// Parent constructor
			mw.widgets.datetime.DiscordianDateTimeFormatter.super.call(
				this,
				config
			);
		};
	...
	mw.widgets.datetime.DiscordianDateTimeFormatter.static.formats = {
		...
		"@date":
			"$!{dow|full}${not-intercalary|1|, }${season|full}${not-intercalary|1| }${day|#}, ${year|#}",
		"@datetime":
			"$!{dow|full}${not-intercalary|1|, }${season|full}${not-intercalary|1| }${day|#}, ${year|#} ${hour|0}:${minute|0}:${second|0} $!{zone|short}",
		"@default":
			"$!{dow|full}${not-intercalary|1|, }${season|full}${not-intercalary|1| }${day|#}, ${year|#} ${hour|0}:${minute|0}:${second|0} $!{zone|short}",
	};
	 ...
	 */
	mw.widgets.datetime.DiscordianDateTimeFormatter.prototype.getFieldForTag =
		function (tag, params) {
			var spec = null;
			...
			switch (tag + "|" + params[0]) {
				...
				case "day|0":
					spec = {
						...
						formatValue: function (v) {
							...
							return mw.widgets.datetime.DateTimeFormatter.prototype.formatSpecValue.call(
								this,
								v
							);
						},
						parseValue: function (v) {
							...
							return mw.widgets.datetime.DateTimeFormatter.prototype.parseSpecValue.call(
								this,
								v
							);
						},
				...
				default:
					return mw.widgets.datetime.DiscordianDateTimeFormatter.super.prototype.getFieldForTag.call(
						this,
						tag,
						params
					);
	 ...
	 */
	mw.widgets.datetime.DiscordianDateTimeFormatter.prototype.getComponentsFromDate =
		function (date) {
			var ret,
				day,
				month,
	 ...
	 * @inheritdoc
	 */
	mw.widgets.datetime.DiscordianDateTimeFormatter.prototype.adjustComponent =
	 ...
	 * @return {Object} components
	 */
	mw.widgets.datetime.DiscordianDateTimeFormatter.prototype.adjustComponentInternal =
		function (components, component, delta, mode) {
			...
			if (component === "Day") {
				...
				while (i-- > 0) {
					if (
						components.Day === preTib &&
						components.Season === 1 &&
						this.isLeapYear(components.Year)
					) {
						components.Day = "tib";
				...
				switch (mode) {
					case "overflow":
						if (
							components.Day === "tib" &&
							(component === "Season" || component === "Year")
						) {
							components.Day = 59; // Could choose either one...
							wasTib = true;
						} else {
						...
						while (i-- > 0) {
							...
							if (components[component] < min) {
								...
								components = this.adjustComponentInternal(
									components,
									next,
									-1,
									mode
	 ...
	 * @inheritdoc
	 */
	mw.widgets.datetime.DiscordianDateTimeFormatter.prototype.getDateFromComponents =
		function (components) {
			var month,
				day,
				days,
	 ...
	 * @return {boolean}
	 */
	mw.widgets.datetime.DiscordianDateTimeFormatter.prototype.isLeapYear =
		function (year) {
			year -= 1166;
			if (year % 4) {
				return false;
	 ...
	 * @inheritdoc
	 */
	mw.widgets.datetime.DiscordianDateTimeFormatter.prototype.getCalendarHeadings =
		function () {
	 ...
	 * @inheritdoc
	 */
	mw.widgets.datetime.DiscordianDateTimeFormatter.prototype.sameCalendarGrid =
		function (date1, date2) {
			...
			return (
				...
				components1.Season === components2.Season
			);
		};
	 ...
	 * @inheritdoc
	 */
	mw.widgets.datetime.DiscordianDateTimeFormatter.prototype.getCalendarData =
		function (date) {
			var dt,
				components,
				season,
