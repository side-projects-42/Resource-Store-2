(function () {
	...
	mw.widgets.datetime.CalendarWidget =
		function MwWidgetsDatetimeCalendarWidget(config) {
			...
			OO.ui.mixin.TabIndexedElement.call(
				this,
				$.extend({}, config, { $tabIndexed: this.$element })
			);
	 ...
	 * @chainable
	 */
	mw.widgets.datetime.CalendarWidget.prototype.setSelected = function (
		dates
	) {
		var i,
			changed = false;
	 ...
	 * @chainable
	 */
	mw.widgets.datetime.CalendarWidget.prototype.setFocusedDate = function (
		date
	) {
	 ...
	 * @return {Date}
	 */
	mw.widgets.datetime.CalendarWidget.prototype.adjustDate = function (
		date,
		component,
		delta
	) {
	...
	mw.widgets.datetime.CalendarWidget.prototype.updateUI = function () {
		var r,
			...
			datePartIsEqual = this.formatter.datePartIsEqual.bind(
				this.formatter
			),
			...
			for (c = 0; c < row.length; c++) {
				...
				if (day === null) {
					...
					if (!this.buttons[k]) {
						...
						this.buttons[k].connect(this, {
							click: ["onDayClick", this.buttons[k]],
						});
					}
					this.buttons[k]
						.setData(day.date)
						.setDisabled(
							day.date < this.min || day.date > this.max
						);
					$cell = this.buttons[k].$element;
					$cell.toggleClass(
						"mw-widgets-datetime-calendarWidget-focused",
						this.formatter.datePartIsEqual(focusedDate, day.date)
					);
					$cell.toggleClass(
						"mw-widgets-datetime-calendarWidget-selected",
	...
	mw.widgets.datetime.CalendarWidget.prototype.onPrevClick = function () {
		this.setFocusedDate(
			this.adjustDate(this.getFocusedDate(), "month", -1)
		);
		if (
			!this.$widget ||
			OO.ui.contains(this.$element[0], document.activeElement, true)
		) {
	 ...
	 */
	mw.widgets.datetime.CalendarWidget.prototype.onDayClick = function (
		$button
	) {
	 ...
	 */
	mw.widgets.datetime.CalendarWidget.prototype.onDocumentMouseDown =
		function (e) {
			if (
				this.$widget &&
		...
		if (!this.isDisabled()) {
			switch (e.which) {
				...
				case OO.ui.Keys.LEFT:
					this.setFocusedDate(
						this.adjustDate(focusedDate, "day", -1)
					);
					return false;
				...
				case OO.ui.Keys.UP:
					this.setFocusedDate(
						this.adjustDate(focusedDate, "week", -1)
					);
					return false;
				...
				case OO.ui.Keys.PAGEUP:
					this.setFocusedDate(
						this.adjustDate(focusedDate, "month", -1)
					);
					return false;
	...
	mw.widgets.datetime.CalendarWidget.prototype.checkFocus = function () {
		...
		if (
			!activeElement ||
			!OO.ui.contains(containers, activeElement, true)
		) {
			this.toggle(false);
		}
	};
	...
	mw.widgets.datetime.CalendarWidget.prototype.toggle = function (visible) {
		...
		// Parent method
		mw.widgets.datetime.CalendarWidget.super.prototype.toggle.call(
			this,
			visible
		);
