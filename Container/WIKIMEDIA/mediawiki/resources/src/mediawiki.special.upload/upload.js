		...
		timeout: function () {
			...
			new mw.Api()
				.get({
					...
					titles: [
						title ? title.getPrefixedText() : this.nameToCheck,
					],
					prop: "imageinfo",
		...
		getPreview: function ($element, $previewContainer) {
			...
			new mw.Api()
				...
				.done(function (result) {
					uploadTemplatePreview.processResult(
						result,
						template,
						$previewContainer
					);
	...
	$(function () {
		...
		if (mw.config.get("wgAjaxLicensePreview") && $license.length) {
			...
			$license.on("change", function () {
				...
				uploadTemplatePreview.getPreview(
					$license,
					$("#mw-license-preview")
				);
			});
	...
	$(function () {
		...
		function showPreview(file) {
			var $canvas,
				...
				thumb = mw.template
					.get("mediawiki.special.upload", "thumbnail.html")
					.render();
			...
			fetchPreview(
				...
				function (dataURL) {
					...
					img.onload = function () {
						...
						// Image size
						info =
							mw.msg("widthheight", logicalWidth, logicalHeight) +
							", " +
			...
			if (file.size > maxSize) {
				$error = $(
					'<p class="error mw-upload-source-error" id="wpSourceTypeFile-error">' +
						mw
							.message("largefileserver", file.size, maxSize)
							.escaped() +
						"</p>"
				);
	...
	$(function () {
		...
		allowCloseWindow = mw.confirmCloseWindow({
			test: function () {
				...
				return (
					($wpUploadFile.length &&
						$wpUploadFile.get(0).files.length !== 0) ||
					$uploadForm.data("origtext") !== $uploadForm.serialize()
				);
			},
	...
	$(function () {
		...
		$(".mw-editTools").on("focusout", function (e) {
			...
			if (
				...
				$(e.relatedTarget).closest(".mw-editTools").length > 0
			) {
				return;
			}
