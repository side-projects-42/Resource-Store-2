...
(function (OO) {
	...
	OO.ui.mixin.DraggableElement = function OoUiMixinDraggableElement(config) {
		...
		this.$element.addClass("oo-ui-draggableElement").on({
		...
		this.toggleDraggable(
			config.draggable === undefined ? true : !!config.draggable
		);
	};
	...
	OO.ui.mixin.DraggableElement.prototype.toggleDraggable = function (
		...
		isDraggable =
			isDraggable !== undefined ? !!isDraggable : !this.draggable;

		if (this.draggable !== isDraggable) {
			this.draggable = isDraggable;

			this.$handle.toggleClass(
	 ...
	 * @param {OO.ui.mixin.DraggableElement} item Dragged item
	 */
	OO.ui.mixin.DraggableGroupElement.prototype.onItemDragStart = function (
		item
	) {
		if (!this.isDraggable()) {
			return;
	 ...
	 */
	OO.ui.mixin.DraggableGroupElement.prototype.onItemDropOrDragEnd =
		function () {
			var targetIndex,
				originalIndex,
			...
			if (item) {
				...
				targetIndex =
					item.getIndex() + (item.getIndex() > originalIndex ? 1 : 0);
	 ...
	 */
	OO.ui.mixin.DraggableGroupElement.prototype.reorder = function (
		item,
		newIndex
	) {
	...
	OO.ui.mixin.RequestManager = function OoUiMixinRequestManager(config) {
		...
		this.showPendingRequest =
			!!this.pushPending && config.showPendingRequest !== false;
	};
	...
	OO.ui.mixin.LookupElement = function OoUiMixinLookupElement(config) {
		...
		this.allowSuggestionsWhenEmpty =
			config.allowSuggestionsWhenEmpty || false;
	 ...
	 */
	OO.ui.mixin.LookupElement.prototype.onLookupMenuToggle = function (
		visible
	) {
		if (!visible) {
	 ...
	 */
	OO.ui.mixin.LookupElement.prototype.setLookupsDisabled = function (
		disabled
	) {
		...
		if (!this.allowSuggestionsWhenEmpty && value === "") {
			...
			this.getLookupMenuItems()
				.done(function (items) {
					...
					if (items.length) {
						widget.lookupMenu.addItems(items).toggle(true);
	 ...
	 * @chainable
	 */
	OO.ui.mixin.LookupElement.prototype.initializeLookupMenuSelection =
		function () {
			if (
				this.lookupHighlightFirstItem &&
	 ...
	 * @inheritdoc OO.ui.mixin.RequestManager
	 */
	OO.ui.mixin.LookupElement.prototype.getRequestCacheDataFromResponse =
		function (response) {
	...
	OO.ui.TabPanelLayout = function OoUiTabPanelLayout(name, config) {
		...
		// Initialization
		this.$element.addClass("oo-ui-tabPanelLayout").attr("role", "tabpanel");
	};
	...
	OO.ui.StackLayout = function OoUiStackLayout(config) {
		...
		config = $.extend(
			{ scrollable: !!(config && config.continuous) },
			config
		);
		...
		if (this.continuous) {
			...
			this.$element.on(
				"scroll",
				OO.ui.debounce(this.onScroll.bind(this), 250)
			);
		}
		if (Array.isArray(config.items)) {
			this.addItems(config.items);
	 ...
	 */
	OO.ui.StackLayout.prototype.updateHiddenState = function (
		items,
		selectedItem
	) {
		var i, len;
	...
	OO.ui.BookletLayout.prototype.onStackLayoutFocus = function (e) {
		...
		for (name in this.pages) {
			...
			if (
				this.pages[name].$element[0] === $target[0] &&
	 ...
	 */
	OO.ui.BookletLayout.prototype.onStackLayoutVisibleItemChange = function (
		page
	) {
	 ...
	 */
	OO.ui.BookletLayout.prototype.onOutlineSelectWidgetSelect = function (
		item
	) {
		if (item) {
	...
	OO.ui.BookletLayout.prototype.findClosestPage = function (page) {
		...
		if (index !== -1) {
			...
			if (this.outlined) {
				level = this.outlineSelectWidget
					.findItemFromData(page.getName())
					.getLevel();
				if (
					prev &&
					level ===
						this.outlineSelectWidget
							.findItemFromData(prev.getName())
							.getLevel()
				) {
					return prev;
				}
	...
	OO.ui.BookletLayout.prototype.setPage = function (name) {
		var selectedItem,
			...
			previousPage =
				this.currentPageName && this.pages[this.currentPageName];
	...
	OO.ui.IndexLayout.prototype.onStackLayoutFocus = function (e) {
		...
		for (name in this.tabPanels) {
			...
			if (
				this.tabPanels[name].$element[0] === $target[0] &&
	...
	OO.ui.IndexLayout.prototype.getClosestTabPanel = function (tabPanel) {
		...
		if (index !== -1) {
			...
			level = this.tabSelectWidget
				.findItemFromData(tabPanel.getName())
				.getLevel();
			if (
				prev &&
				level ===
	...
	OO.ui.IndexLayout.prototype.setTabPanel = function (name) {
		var selectedItem, $focused, previousTabPanel, tabPanel;
	...
	OO.ui.ToggleButtonWidget = function OoUiToggleButtonWidget(config) {
		...
		this.$element.addClass("oo-ui-toggleButtonWidget").append(this.$button);
	...
	OO.ui.ToggleButtonWidget.prototype.setButtonElement = function ($button) {
		...
		OO.ui.mixin.ButtonElement.prototype.setButtonElement.call(
			this,
			$button
		);
	 ...
	 */
	OO.ui.OutlineControlsWidget = function OoUiOutlineControlsWidget(
		outline,
		config
	) {
		// Allow passing positional parameters inside the config object
	...
	OO.ui.TabOptionWidget = function OoUiTabOptionWidget(config) {
		...
		// Initialization
		this.$element.addClass("oo-ui-tabOptionWidget").attr("role", "tab");
	};
	...
	OO.ui.TabOptionWidget.prototype.scrollElementIntoView = function (config) {
		...
		if (!OO.ui.isMobile() || !this.getElementGroup()) {
			// Parent method
			return OO.ui.TabOptionWidget.super.prototype.scrollElementIntoView.call(
				this
			);
		} else {
			padding = Math.max(
				(this.getElementGroup().$element[0].clientWidth -
	...
	OO.ui.TabSelectWidget = function OoUiTabSelectWidget(config) {
		...
		// Initialization
		this.$element.addClass("oo-ui-tabSelectWidget").attr("role", "tablist");
	...
	OO.ui.TagItemWidget.prototype.onKeyDown = function (e) {
		...
		if (
			...
			this.emit(
				"navigate",
				e.keyCode === OO.ui.Keys.LEFT ? movement.left : movement.right
			);
			return false;
		}
	};
	...
	OO.ui.TagMultiselectWidget = function OoUiTagMultiselectWidget(config) {
		...
		this.inputPosition =
			this.constructor.static.allowedInputPositions.indexOf(
				...
				: "inline";
		this.allowEditTags =
			config.allowEditTags === undefined ? true : !!config.allowEditTags;
		...
		this.setTabIndexedElement(
			this.hasInput ? this.input.$input : $tabFocus
		);
	...
	OO.ui.TagMultiselectWidget.prototype.onInputBlur = function () {
		...
		this.toggleValid(
			this.checkValidity() && (!this.hasInput || !this.input.getValue())
		);
	};
	 ...
	 *  a meta key like Control
	 */
	OO.ui.TagMultiselectWidget.prototype.doInputArrow = function (
		e,
		direction
	) {
		if (
		...
		if (this.hasInput) {
			...
			if (this.inputPosition === "outline") {
				...
				this.input.$input.attr(
					"placeholder",
					isUnderLimit ? this.inputPlaceholder : ""
				);
	...
	OO.ui.TagMultiselectWidget.prototype.setDisabled = function (isDisabled) {
		// Parent method
		OO.ui.TagMultiselectWidget.super.prototype.setDisabled.call(
			this,
			isDisabled
		);
	 ...
	 */
	OO.ui.TagMultiselectWidget.prototype.onTagNavigate = function (
		item,
		direction
	) {
	...
	OO.ui.TagMultiselectWidget.prototype.isAllowedData = function (data) {
		if (!this.allowDuplicates && this.isDuplicateData(data)) {
			return false;
		}
	...
	OO.ui.TagMultiselectWidget.prototype.isUnderLimit = function () {
		return !this.tagLimit || this.getItemCount() < this.tagLimit;
	};
	 ...
	 */
	OO.ui.TagMultiselectWidget.prototype.createTagItemWidget = function (
		data,
		label
	) {
		label = label || data;
	...
	OO.ui.TagMultiselectWidget.prototype.updateInputSize = function () {
		...
		if (this.inputPosition === "inline" && !this.isDisabled()) {
			...
			if (
				...
				this.contentWidthWithPlaceholder =
	...
	OO.ui.PopupTagMultiselectWidget = function OoUiPopupTagMultiselectWidget(
		...
		// Parent constructor
		OO.ui.PopupTagMultiselectWidget.super.call(
			this,
			$.extend(
				{
					inputPosition: "none",
		...
		if (this.hasInput) {
			this.input.$input.on(
				"focus",
				this.popup.toggle.bind(this.popup, true)
			);
		}
		...
		OO.ui.warnDeprecation(
			"PopupTagMultiselectWidget: Deprecated widget. Use MenuTagMultiselectWidget instead. See T208821."
		);
	};
	 ...
	 */
	OO.ui.PopupTagMultiselectWidget.prototype.onPopupToggle = function (
		isVisible
	) {
	...
	OO.ui.PopupTagMultiselectWidget.prototype.onTagSelect = function (item) {
		if (this.popupInput && this.allowEditTags) {
			...
			// Parent
			OO.ui.PopupTagMultiselectWidget.super.prototype.onTagSelect.call(
				this,
				item
			);
		}
	 ...
	 */
	OO.ui.MenuTagMultiselectWidget = function OoUiMenuTagMultiselectWidget(
		config
	) {
		var options,
			selected,
		...
		this.clearInputOnChoose =
			config.clearInputOnChoose === undefined ||
			!!config.clearInputOnChoose;
		this.menu = this.createMenuWidget(
			$.extend(
				{
					widget: this,
					hideOnChoose: false,
					...
					$floatableContainer:
						this.hasInput && this.inputPosition === "outline"
							? this.input.$element
							: this.$element,
	 ...
	 */
	OO.ui.MenuTagMultiselectWidget.prototype.onMenuChoose = function (
		menuItem,
		selected
	) {
	 ...
	 */
	OO.ui.MenuTagMultiselectWidget.prototype.onMenuToggle = function (
		isVisible
	) {
		if (!isVisible) {
	...
	OO.ui.MenuTagMultiselectWidget.prototype.onTagSelect = function (tagItem) {
		...
		if (!this.allowArbitrary) {
			...
			// Use the default
			OO.ui.MenuTagMultiselectWidget.super.prototype.onTagSelect.call(
				this,
				tagItem
			);
		}
	...
	OO.ui.MenuTagMultiselectWidget.prototype.removeItems = function (items) {
		...
		// Parent
		OO.ui.MenuTagMultiselectWidget.super.prototype.removeItems.call(
			this,
			items
		);
	...
	OO.ui.MenuTagMultiselectWidget.prototype.setValue = function (valueObject) {
		...
		valueObject.forEach(
			function (obj) {
				...
				menuItem =
					this.menu.getItemFromLabel(label) ||
					this.menu.findItemFromData(data);
				if (menuItem) {
	 ...
	 * @inheritdoc
	 */
	OO.ui.MenuTagMultiselectWidget.prototype.setDisabled = function (
		isDisabled
	) {
		// Parent method
		OO.ui.MenuTagMultiselectWidget.super.prototype.setDisabled.call(
			this,
			isDisabled
		);
	 ...
	 * @chainable
	 */
	OO.ui.MenuTagMultiselectWidget.prototype.initializeMenuSelection =
		function () {
			var highlightedItem;
			this.menu.highlightItem(
				this.allowArbitrary ? null : this.menu.findFirstSelectableItem()
			);
	...
	OO.ui.MenuTagMultiselectWidget.prototype.getTagInfoFromInput = function () {
		var val = this.input.getValue(),
			...
			item =
				this.menu.findHighlightedItem() ||
				this.menu.findItemFromData(val),
	 ...
	 */
	OO.ui.MenuTagMultiselectWidget.prototype.addOptions = function (
		menuOptions
	) {
		var widget = this,
			optionsData = [],
	...
	OO.ui.SelectFileWidget.prototype.updateUI = function () {
		...
		if (this.currentFiles.length) {
			...
			if (this.showDropTarget) {
				if (!this.multiple) {
					this.pushPending();
					this.loadAndGetImageUrl(this.currentFiles[0])
						.done(
							function (url) {
								this.$thumbnail.css(
									"background-image",
