...
(function (OO) {
	 ...
	 */
	OO.ui.Toolbar = function OoUiToolbar(
		toolFactory,
		toolGroupFactory,
		config
	) {
	...
	OO.ui.Toolbar.prototype.onPointerDown = function (e) {
		...
		if (
			...
			$closestWidgetToEvent[0] === $closestWidgetToToolbar[0]
		) {
			return false;
		}
	};
	...
	OO.ui.Toolbar.prototype.getNarrowThreshold = function () {
		if (this.narrowThreshold === null) {
			this.narrowThreshold =
				this.$group[0].offsetWidth + this.$actions[0].offsetWidth;
		}
	...
	OO.ui.Toolbar.prototype.setup = function (groups) {
		...
		for (i = 0, len = groups.length; i < len; i++) {
			...
			type = this.getToolGroupFactory().lookup(groupConfig.type)
				? groupConfig.type
				: defaultType;
			toolGroup = this.getToolGroupFactory().create(
				type,
				this,
				groupConfig
			);
	...
	OO.ui.Tool = function OoUiTool(toolGroup, config) {
		...
		this.$accel.addClass("oo-ui-tool-accel").prop({
	...
	OO.ui.ToolGroup = function OoUiToolGroup(toolbar, config) {
		...
		this.$element.addClass("oo-ui-toolGroup").append(this.$group);
	 ...
	 */
	OO.ui.ToolFactory.prototype.getTools = function (
		include,
		exclude,
		promote,
		demote
		...
		included = OO.simpleArrayDifference(
			this.extract(include),
	...
	OO.ui.ToolGroupFactory.static.getDefaultClasses = function () {
		return [OO.ui.BarToolGroup, OO.ui.ListToolGroup, OO.ui.MenuToolGroup];
	};
	...
	OO.ui.PopupTool = function OoUiPopupTool(toolGroup, config) {
		...
		this.popup.setPosition(
			toolGroup.getToolbar().position === "bottom" ? "above" : "below"
		);
	...
	OO.ui.ToolGroupTool = function OoUiToolGroupTool(toolGroup, config) {
		...
		// Properties
		this.innerToolGroup = this.createGroup(
			this.constructor.static.groupConfig
		);
	...
	OO.ui.ToolGroupTool.prototype.createGroup = function (group) {
		...
		return this.toolbar
			.getToolGroupFactory()
			.create("list", this.toolbar, group);
	};
	...
	OO.ui.PopupToolGroup = function OoUiPopupToolGroup(toolbar, config) {
		...
		this.onPopupDocumentMouseKeyUpHandler =
			this.onPopupDocumentMouseKeyUp.bind(this);
		this.$handle = $("<span>");
		...
		OO.ui.mixin.FloatableElement.call(
			this,
			$.extend(
				{
					...
					verticalPosition:
						this.toolbar.position === "bottom" ? "above" : "below",
				},
				config
			)
		);
		...
		this.$element.addClass("oo-ui-popupToolGroup").prepend(this.$handle);
	...
	OO.ui.PopupToolGroup.prototype.onPopupDocumentMouseKeyUp = function (e) {
		...
		if (
			$target.closest(".oo-ui-popupToolGroup-tools")[0] === this.$group[0]
		) {
			return;
		}
		this.setActive(false);
	...
	OO.ui.PopupToolGroup.prototype.onMouseKeyDown = function (e) {
		...
		return OO.ui.PopupToolGroup.super.prototype.onMouseKeyDown.call(
			this,
			e
		);
	};
		...
		if (this.active !== value) {
			...
			if (value) {
				...
				if (
					...
					containerLeft =
						this.$clippableScrollableContainer[0] ===
					...
					this.$clippable.css({
						"margin-left": -(
							this.$element.offset().left - containerLeft
						),
	...
	OO.ui.ListToolGroup.prototype.populate = function () {
		...
		if (this.allowCollapse !== undefined) {
			...
			allowCollapse = OO.simpleArrayDifference(
	...
	OO.ui.ListToolGroup.prototype.onMouseKeyUp = function (e) {
		...
		if (
			...
			// directly.
			return OO.ui.ListToolGroup.super.super.prototype.onMouseKeyUp.call(
				this,
				e
			);
		} else {
