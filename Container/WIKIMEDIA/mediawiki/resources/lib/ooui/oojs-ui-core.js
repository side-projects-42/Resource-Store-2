...
(function (OO) {
	...
	OO.ui.isFocusableElement = function ($element) {
		...
		if (
			["input", "select", "textarea", "button", "object"].indexOf(
				nodeName
			) !== -1
		) {
			return true;
	...
	OO.ui.findFocusable = function ($container, backwards) {
		var $focusable = $([]),
			...
			$focusableCandidates = $container.find(
				"input, select, textarea, button, object, a, area, [contenteditable], [tabindex]"
			);

		if (backwards) {
			$focusableCandidates =
				Array.prototype.reverse.call($focusableCandidates);
		}
	...
	*/
	...
	OO.ui.Element.static.unsafeInfuse = function (elem, config, domPromise) {
		...
		if (data) {
			...
			if (domPromise) {
				...
				state = data.constructor.static.gatherPreInfuseState(
					$elem,
					data
				);
				...
				if (infusedChildren && infusedChildren.length) {
					infusedChildren.forEach(function (childData) {
						var childState =
							childData.constructor.static.gatherPreInfuseState(
								$elem,
								childData
							);
						domPromise.done(
							childData.restorePreInfuseState.bind(
								childData,
								childState
							)
						);
		...
		if (
			!(
				cls &&
				(cls === OO.ui.Element ||
					cls.prototype instanceof OO.ui.Element)
			)
		) {
			throw new Error(
				"Unknown widget type: id: " + id + ", class: " + data._
			);
		}
	...
	OO.ui.Element.static.getRelativePosition = function ($element, $anchor) {
		...
		while (elementDocument !== anchorDocument) {
			...
			if (!iframe) {
				throw new Error(
					"$element frame is not contained in $anchor frame"
				);
			}
	...
	OO.ui.Element.static.getBorders = function (el) {
		var doc = this.getDocument(el),
			...
			top =
				parseFloat(
					style ? style.borderTopWidth : $el.css("borderTopWidth")
				) || 0,
			left =
				parseFloat(
					style ? style.borderLeftWidth : $el.css("borderLeftWidth")
				) || 0,
			bottom =
				parseFloat(
					style
						? style.borderBottomWidth
						: $el.css("borderBottomWidth")
				) || 0,
			right =
				parseFloat(
					style ? style.borderRightWidth : $el.css("borderRightWidth")
				) || 0;
	...
	(function () {
		 ...
		 * @return {number}
		 */
		OO.ui.Element.static.computeNormalizedScrollLeft = function (
			nativeOffset,
			el
		) {
		 ...
		 * @return {number}
		 */
		OO.ui.Element.static.computeNativeScrollLeft = function (
			normalizedOffset,
			el
		) {
			// All browsers use the correct scroll type ('negative') on the root, so don't
		...
		OO.ui.Element.static.setScrollLeft = function (el, scrollLeft) {
			scrollLeft = OO.ui.Element.static.computeNativeScrollLeft(
				scrollLeft,
				el
			);
			if (isRoot(el)) {
	 ...
	 */
	OO.ui.Element.static.getClosestScrollableContainer = function (
		el,
		dimension
	) {
		var i,
	...
	OO.ui.Element.static.scrollIntoView = function (elOrPosition, config) {
		...
		container =
			...
			(elOrPosition instanceof HTMLElement
				? this.getClosestScrollableContainer(
						elOrPosition,
						config.direction
				  )
		...
		if ($container.is("html, body")) {
			...
			position = {
				top:
					elementPosition.top -
					(containerDimensions.rect.top +
						containerDimensions.borders.top),
				bottom:
					containerDimensions.rect.bottom -
					containerDimensions.borders.bottom -
					containerDimensions.scrollbar.bottom -
					elementPosition.bottom,
				left:
					elementPosition.left -
					(containerDimensions.rect.left +
						containerDimensions.borders.left),
				right:
					containerDimensions.rect.right -
					containerDimensions.borders.right -
		...
		if (!config.direction || config.direction === "x") {
			if (position.left < padding.left) {
				animations.scrollLeft =
					containerDimensions.scroll.left +
					position.left -
					padding.left;
				...
				animations.scrollLeft =
					...
					Math.min(
						position.left - padding.left,
						-position.right + padding.right
					);
			}
			if (animations.scrollLeft !== undefined) {
				animations.scrollLeft =
					OO.ui.Element.static.computeNativeScrollLeft(
						animations.scrollLeft,
						container
					);
			}
	...
	OO.ui.Theme.prototype.updateElementClasses = function (element) {
		...
		$elements.removeClass(classes.off).addClass(classes.on);
	};
	 ...
	 */
	OO.ui.mixin.TabIndexedElement = function OoUiMixinTabIndexedElement(
		config
	) {
		// Configuration initialization
	 ...
	 * @return {OO.ui.Element} The element, for chaining
	 */
	OO.ui.mixin.TabIndexedElement.prototype.setTabIndexedElement = function (
		$tabIndexed
	) {
	 ...
	 */
	OO.ui.mixin.TabIndexedElement.prototype.onTabIndexedElementDisable =
		function () {
			this.updateTabIndex();
		};
	...
	OO.ui.mixin.GroupElement.prototype.moveItem = function (items, newIndex) {
		...
		// Mixin method
		newIndex = OO.EmitterList.prototype.moveItem.call(
			this,
			items,
			newIndex
		);
	 ...
	 */
	OO.ui.mixin.GroupElement.prototype.insertItemElements = function (
		itemWidget,
		index
	) {
	 ...
	 */
	OO.ui.mixin.LabelElement.static.highlightQuery = function (
		text,
		query,
		compare,
		combineMarks
		...
		if (!query.length || offset === -1) {
			...
			$result.append(
				...
				$("<span>")
					...
					.text(
						text.slice(offset, offset + query.length + comboLength)
					),
				document.createTextNode(
					text.slice(offset + query.length + comboLength)
				)
			);
		}
	...
	OO.ui.mixin.LabelElement.prototype.setLabel = function (label) {
		...
		label =
			((typeof label === "string" || label instanceof $) &&
				label.length) ||
			(label instanceof OO.ui.HtmlSnippet && label.toString().length)
				? label
				: null;
		...
		this.$element.toggleClass(
			"oo-ui-labelElement",
			!!this.label && !this.invisibleLabel
		);
	 ...
	 * @return {OO.ui.Element} The element, for chaining
	 */
	OO.ui.mixin.LabelElement.prototype.setInvisibleLabel = function (
		invisibleLabel
	) {
	...
	OO.ui.mixin.LabelElement.prototype.setHighlightedQuery = function (
		...
		return this.setLabel(
			this.constructor.static.highlightQuery(
				text,
				query,
				compare,
				combineMarks
	...
	OO.ui.mixin.IconElement.prototype.setIcon = function (icon) {
		icon = OO.isPlainObject(icon)
			? OO.ui.getLocalValue(icon, null, "default")
			: icon;
		icon =
			typeof icon === "string" && icon.trim().length ? icon.trim() : null;
	...
	OO.ui.mixin.IndicatorElement = function OoUiMixinIndicatorElement(config) {
		...
		// Initialization
		this.setIndicator(
			config.indicator || this.constructor.static.indicator
		);
	 ...
	 */
	OO.ui.mixin.IndicatorElement.prototype.setIndicatorElement = function (
		$indicator
	) {
		if (this.$indicator) {
			this.$indicator
				.removeClass(
					"oo-ui-indicatorElement-indicator oo-ui-indicator-" +
						this.indicator
				)
				.removeAttr("title");
		}
	...
	OO.ui.mixin.IndicatorElement.prototype.setIndicator = function (indicator) {
		indicator =
			typeof indicator === "string" && indicator.length
				? indicator.trim()
				: null;
	 ...
	 */
	OO.ui.mixin.FlaggedElement.prototype.setFlaggedElement = function (
		$flagged
	) {
	 ...
	 */
	OO.ui.mixin.AccessKeyedElement = function OoUiMixinAccessKeyedElement(
		config
	) {
		// Configuration initialization
		config = config || {};
	 ...
	 * @return {OO.ui.Element} The element, for chaining
	 */
	OO.ui.mixin.AccessKeyedElement.prototype.setAccessKey = function (
		accessKey
	) {
	...
	OO.ui.mixin.RequiredElement = function OoUiMixinRequiredElement(config) {
		...
		this.indicatorElement =
			config.indicatorElement !== undefined
				? config.indicatorElement
				: this;
		if (this.indicatorElement && !this.indicatorElement.getIndicator) {
			throw new Error(
				"config.indicatorElement must mixin OO.ui.mixin.IndicatorElement."
			);
		}

		// Initialization
		this.setRequiredElement(
			config.$required || this.$input || this.$element
		);
	 ...
	 */
	OO.ui.mixin.RequiredElement.prototype.setRequiredElement = function (
		$required
	) {
		if (this.$required) {
	...
	OO.ui.mixin.RequiredElement.prototype.setRequired = function (state) {
		this.required = !!state;
		if (this.required) {
			this.$required.prop("required", true).attr("aria-required", "true");
			if (
				this.indicatorElement &&
				this.indicatorElement.getIndicator() === null
			) {
				this.indicatorElement.setIndicator("required");
			}
		} else {
			this.$required.prop("required", false).removeAttr("aria-required");
			if (
				this.indicatorElement &&
				this.indicatorElement.getIndicator() === "required"
			) {
	...
	OO.ui.ButtonWidget = function OoUiButtonWidget(config) {
		...
		OO.ui.mixin.TitledElement.call(
			this,
			$.extend(
				{
					$titled: this.$button,
		...
		OO.ui.mixin.AccessKeyedElement.call(
			this,
			$.extend(
				{
	...
	OO.ui.ButtonWidget.prototype.setNoFollow = function (noFollow) {
		...
		if (noFollow !== this.noFollow) {
			if (!noFollow) {
				rel = this.rel.concat();
				rel.splice(this.rel.indexOf("nofollow"), 1);
				this.setRel(rel);
			} else {
	...
	OO.ui.IndicatorWidget = function OoUiIndicatorWidget(config) {
		...
		// Mixin constructors
		OO.ui.mixin.IndicatorElement.call(
			this,
			$.extend(
				{
	 ...
	 */
	OO.ui.mixin.PendingElement.prototype.setPendingElement = function (
		$pending
	) {
		if (this.$pending) {
	...
	OO.ui.mixin.FloatableElement = function OoUiMixinFloatableElement(config) {
		...
		this.hideWhenOutOfView =
			config.hideWhenOutOfView === undefined
				? true
				: !!config.hideWhenOutOfView;
	};
	...
	OO.ui.mixin.FloatableElement.prototype.setVerticalPosition = function (
		...
		if (
			["below", "above", "top", "bottom", "center"].indexOf(position) ===
			-1
		) {
	...
	OO.ui.mixin.FloatableElement.prototype.setHorizontalPosition = function (
		...
		if (
			["before", "after", "start", "end", "center"].indexOf(position) ===
			-1
		) {
			throw new Error(
				"Invalid value for horizontal position: " + position
			);
		}
	...
	OO.ui.mixin.FloatableElement.prototype.togglePositioning = function (
		...
		positioning =
			positioning === undefined ? !this.positioning : !!positioning;
		...
		if (
			...
			OO.ui.warnDeprecation(
				"FloatableElement#togglePositioning: Before calling this method, the element must be attached to the DOM."
			);
		...
		if (this.positioning !== positioning) {
			...
			if (positioning) {
				...
				this.$floatableClosestScrollable.on(
					"scroll",
				...
				if (this.$floatableClosestScrollable) {
					this.$floatableClosestScrollable.off(
						"scroll",
						this.onFloatableScrollHandler
					);
					this.$floatableClosestScrollable = null;
	...
	OO.ui.mixin.FloatableElement.prototype.isElementInViewport = function (
		...
		bottomEdgeInBounds =
			elemRect.bottom >= contRect.top &&
			elemRect.bottom <= contRect.bottom;
		leftEdgeInBounds =
			elemRect.left >= contRect.left && elemRect.left <= contRect.right;
		rightEdgeInBounds =
			elemRect.right >= contRect.left && elemRect.right <= contRect.right;
		if (direction === "rtl") {
		...
		if (
			!(
				...
				(
					...
					(this.$floatableClosestScrollable[0] ===
	...
	OO.ui.mixin.FloatableElement.prototype.computePosition = function () {
		...
		scrollableX =
			$offsetParent.css("overflow-x") === "scroll" ||
			$offsetParent.css("overflow-x") === "auto";
		scrollableY =
			$offsetParent.css("overflow-y") === "scroll" ||
			$offsetParent.css("overflow-y") === "auto";

		vertScrollbarWidth =
			$offsetParent.innerWidth() - $offsetParent.prop("clientWidth");
		horizScrollbarHeight =
			$offsetParent.innerHeight() - $offsetParent.prop("clientHeight");
		...
		scrollLeft =
			scrollableX && !isBody
				? OO.ui.Element.static.getScrollLeft($offsetParent[0])
				: 0;
		...
		containerPos.bottom =
			containerPos.top + this.$floatableContainer.outerHeight();
		containerPos.right =
			containerPos.left + this.$floatableContainer.outerWidth();
		containerPos.start =
			direction === "rtl" ? containerPos.right : containerPos.left;
		containerPos.end =
			direction === "rtl" ? containerPos.left : containerPos.right;
		...
		if (this.verticalPosition === "below") {
			...
			newPos.top =
				containerPos.top +
				(this.$floatableContainer.height() - this.$floatable.height()) /
					2;
		}
		...
		if (this.horizontalPosition === "before") {
			...
			newPos.left =
				containerPos.left +
				(this.$floatableContainer.width() - this.$floatable.width()) /
					2;
		}
		...
		if (newPos.start !== undefined) {
			if (direction === "rtl") {
				newPos.right =
					(isBody
						? $($offsetParent[0].ownerDocument.documentElement)
						: $offsetParent
					).outerWidth() - newPos.start;
			} else {
	 ...
	 */
	OO.ui.mixin.ClippableElement.prototype.setClippableElement = function (
		$clippable
	) {
		if (this.$clippable) {
		...
		this.$clippable = $clippable.addClass(
	 ...
	 */
	OO.ui.mixin.ClippableElement.prototype.setClippableContainer = function (
		$clippableContainer
	) {
	 ...
	 * @return {OO.ui.Element} The element, for chaining
	 */
	OO.ui.mixin.ClippableElement.prototype.toggleClipping = function (
		clipping
	) {
		...
		if (clipping && !this.warnedUnattached && !this.isElementAttached()) {
			OO.ui.warnDeprecation(
				"ClippableElement#toggleClipping: Before calling this method, the element must be attached to the DOM."
			);
			this.warnedUnattached = true;
		}
		...
		if (this.clipping !== clipping) {
			...
			if (clipping) {
				...
				this.$clippableScroller = this.$clippableScrollableContainer.is(
					"html, body"
				)
					? $(
					...
					: this.$clippableScrollableContainer;
				this.$clippableScroller.on(
					"scroll",
					this.onClippableScrollHandler
				);
				this.$clippableWindow = $(this.getElementWindow()).on(
					"resize",
				...
				this.$clippableScrollableContainer = null;
				this.$clippableScroller.off(
					"scroll",
					this.onClippableScrollHandler
				);
				this.$clippableScroller = null;
				this.$clippableWindow.off(
					"resize",
					this.onClippableWindowResizeHandler
				);
				this.$clippableWindow = null;
	 ...
	 */
	OO.ui.mixin.ClippableElement.prototype.setIdealSize = function (
		width,
		height
	) {
	 ...
	 */
	OO.ui.mixin.ClippableElement.prototype.getHorizontalAnchorEdge =
		function () {
			if (
	...
	OO.ui.mixin.ClippableElement.prototype.clip = function () {
		...
		if (this.$clippableScrollableContainer.is("html, body")) {
			$viewport = $(
				this.$clippableScrollableContainer[0].ownerDocument.body
			);
		...
		vertScrollbarWidth =
			$viewport.innerWidth() - $viewport.prop("clientWidth");
		horizScrollbarHeight =
			$viewport.innerHeight() - $viewport.prop("clientHeight");
	...
	OO.ui.PopupWidget = function OoUiPopupWidget(config) {
		...
		OO.ui.mixin.IconElement.call(this, config);
		OO.ui.mixin.LabelElement.call(this, config);
		OO.ui.mixin.ClippableElement.call(
			this,
			$.extend(
				{
					$clippable: this.$body,
		...
		if (config.head) {
			...
			this.$head = $("<div>")
				...
				.append(
					this.$icon,
					this.$label,
					(this.closeButton && this.closeButton.$element) || ""
				);
	...
	OO.ui.PopupWidget.prototype.onDocumentMouseDown = function (e) {
		if (
			this.isVisible() &&
			!OO.ui.contains(
				this.$element.add(this.$autoCloseIgnore).get(),
				e.target,
				true
			)
		) {
		...
		if (this.anchored !== show) {
			if (show) {
				...
				this.$element.addClass(
					"oo-ui-popupWidget-anchored-" + this.anchorEdge
				);
			} else {
		...
		if (change) {
			...
			if (show) {
				if (this.autoClose) {
					...
					$lastFocusableElement.on(
						"keydown",
						this.onTabKeyDownHandler
					);
					$firstFocusableElement.on(
						"keydown",
						this.onShiftTabKeyDownHandler
					);
				}
				...
				if (this.autoFlip) {
					if (
						this.popupPosition === "above" ||
						this.popupPosition === "below"
					) {
						if (
							this.isClippedVertically() ||
				...
				if (this.autoClose) {
					...
					// when popup closes
					$lastFocusableElement.off(
						"keydown",
						this.onTabKeyDownHandler
					);
					$firstFocusableElement.off(
						"keydown",
						this.onShiftTabKeyDownHandler
					);
	...
	OO.ui.PopupWidget.prototype.computePosition = function () {
		...
		floatablePos[far] =
			floatablePos[near] + this.$floatableContainer["outer" + sizeProp]();
		// Measure where the offsetParent is and compute our position based on that and parentPosition
		offsetParentPos =
			this.$element.offsetParent()[0] === document.documentElement
				? { top: 0, left: 0 }
		...
		if (positionProp === near) {
			...
			popupPos[far] =
				offsetParentPos[near] +
				this.$element.offsetParent()["inner" + sizeProp]() -
				parentPosition[far];
		...
		if (this.anchored) {
			...
			anchorOffset =
				(start === far ? -1 : 1) * (anchorPos - popupPos[start]);
		...
		containerPos =
			this.$container[0] === document.documentElement
				? { top: 0, left: 0 }
				: this.$container.offset();
		containerPos[far] =
			containerPos[near] + this.$container["inner" + sizeProp]();
		...
		if (this.anchored) {
			...
			anchorOffset +=
				(positionProp === start ? -1 : 1) * positionAdjustment;
	 ...
	 */
	OO.ui.PopupWidget.prototype.setAutoCloseIgnore = function (
		$autoCloseIgnore
	) {
			...
			$.extend(
				...
				config.popup,
				{
					$autoCloseIgnore: this.$element.add(
						config.popup && config.popup.$autoCloseIgnore
					),
				}
			)
		);
	...
	OO.ui.SelectWidget.prototype.onMouseDown = function (e) {
		...
		if (!this.isDisabled() && e.which === OO.ui.MouseButtons.LEFT) {
			...
			if (item && item.isSelectable()) {
				...
				this.getElementDocument().addEventListener(
					"mousemove",
					this.onDocumentMouseMoveHandler,
					true
				);
			}
		}
	...
	OO.ui.SelectWidget.prototype.onDocumentKeyPress = function (e) {
		...
		if (!e.charCode) {
			if (
				...
				this.keyPressBuffer = this.keyPressBuffer.substr(
					0,
		...
		this.keyPressBufferTimer = setTimeout(
			this.clearKeyPressBuffer.bind(this),
			1500
		);
	...
	OO.ui.SelectWidget.prototype.bindDocumentKeyPressListener = function () {
		this.getElementDocument().addEventListener(
			"keypress",
			this.onDocumentKeyPressHandler,
			true
		);
	};
	...
	OO.ui.SelectWidget.prototype.findSelectedItems = function () {
		...
		return this.multiselect ? selected : selected[0] || null;
	};
	...
	OO.ui.SelectWidget.prototype.getItemFromLabel = function (label, prefix) {
		...
		for (i = 0; i < len; i++) {
			item = this.items[i];
			if (
				item instanceof OO.ui.OptionWidget &&
				item.isSelectable() &&
				filter(item)
			) {
				return item;
	 ...
	 */
	OO.ui.SelectWidget.prototype.findRelativeSelectableItem = function (
		item,
		direction,
		filter
	) {
	 ...
	 * @param {Object} [config] Configuration options
	 */
	OO.ui.MenuSectionOptionWidget = function OoUiMenuSectionOptionWidget(
		config
	) {
		// Parent constructor
	...
	OO.ui.MenuSelectWidget = function OoUiMenuSelectWidget(config) {
		...
		OO.ui.mixin.ClippableElement.call(
			this,
			$.extend({ $clippable: this.$group }, config)
		);
		OO.ui.mixin.FloatableElement.call(this, config);
		...
		this.hideOnChoose =
			config.hideOnChoose === undefined || !!config.hideOnChoose;
		...
		this.onInputEditHandler = OO.ui.debounce(
			this.updateItemVisibility.bind(this),
			100
		);
		...
		if (
			...
			switch (e.keyCode) {
				...
				case OO.ui.Keys.RIGHT:
					...
					if (!this.$input) {
						OO.ui.MenuSelectWidget.super.prototype.onDocumentKeyDown.call(
							this,
							e
						);
					}
				...
				case OO.ui.Keys.ESCAPE:
					...
					break;
				default:
					return OO.ui.MenuSelectWidget.super.prototype.onDocumentKeyDown.call(
						this,
						e
					);
			}
	...
	OO.ui.MenuSelectWidget.prototype.updateItemVisibility = function () {
		...
		if (this.$input && this.filterFromInput) {
			filter = showAll
				? null
				: this.getItemMatcher(this.$input.val(), this.filterMode);
			...
			if (
				...
				!(
					...
					this.lastHighlightedItem.isVisible()
				) &&
				this.isVisible()
			) {
	...
	OO.ui.MenuSelectWidget.prototype.bindDocumentKeyDownListener = function () {
		if (this.$input) {
			...
			OO.ui.MenuSelectWidget.super.prototype.bindDocumentKeyDownListener.call(
				this
			);
		}
	};
	 ...
	 * @inheritdoc
	 */
	OO.ui.MenuSelectWidget.prototype.unbindDocumentKeyDownListener =
		function () {
			if (this.$input) {
				this.$input.off("keydown", this.onDocumentKeyDownHandler);
			} else {
				OO.ui.MenuSelectWidget.super.prototype.unbindDocumentKeyDownListener.call(
					this
				);
			}
		};
	 ...
	 * @inheritdoc
	 */
	OO.ui.MenuSelectWidget.prototype.bindDocumentKeyPressListener =
		function () {
			if (this.$input) {
				...
				OO.ui.MenuSelectWidget.super.prototype.bindDocumentKeyPressListener.call(
					this
				);
			}
		};
	 ...
	 * @inheritdoc
	 */
	OO.ui.MenuSelectWidget.prototype.unbindDocumentKeyPressListener =
		function () {
			if (this.$input) {
				if (this.filterFromInput) {
					this.$input.off(
				...
				OO.ui.MenuSelectWidget.super.prototype.unbindDocumentKeyPressListener.call(
					this
				);
			}
		};
	...
	OO.ui.MenuSelectWidget.prototype.addItems = function (items, index) {
		// Parent method
		OO.ui.MenuSelectWidget.super.prototype.addItems.call(
			this,
			items,
			index
		);
	...
	OO.ui.MenuSelectWidget.prototype.toggle = function (visible) {
		...
		if (visible && !this.warnedUnattached && !this.isElementAttached()) {
			OO.ui.warnDeprecation(
				"MenuSelectWidget#toggle: Before calling this method, the menu must be attached to the DOM."
			);
			this.warnedUnattached = true;
		}
			...
			if (visible) {
				...
				if (
					...
					this.setVerticalPosition(
						this.constructor.static.flippedPositions[
	...
	OO.ui.RadioOptionWidget.prototype.setDisabled = function (disabled) {
		OO.ui.RadioOptionWidget.super.prototype.setDisabled.call(
			this,
			disabled
		);
	...
	OO.ui.CheckboxMultioptionWidget.prototype.setSelected = function (state) {
		OO.ui.CheckboxMultioptionWidget.super.prototype.setSelected.call(
			this,
			state
		);
	 ...
	 * @inheritdoc
	 */
	OO.ui.CheckboxMultioptionWidget.prototype.setDisabled = function (
		disabled
	) {
		OO.ui.CheckboxMultioptionWidget.super.prototype.setDisabled.call(
			this,
			disabled
		);
	 ...
	 *  in the select.
	 */
	OO.ui.CheckboxMultiselectWidget.prototype.getRelativeFocusableItem =
	...
	OO.ui.ProgressBarWidget = function OoUiProgressBarWidget(config) {
		...
		// Initialization
		this.setProgress(
			config.progress !== undefined ? config.progress : false
		);
	...
	OO.ui.ProgressBarWidget.prototype.setProgress = function (progress) {
		...
		this.$element.toggleClass(
			"oo-ui-progressBarWidget-indeterminate",
			progress === false
		);
	};
	...
	OO.ui.InputWidget.static.gatherPreInfuseState = function (node, config) {
		var state = OO.ui.InputWidget.super.static.gatherPreInfuseState(
			node,
			config
		);
		if (config.$input) {
	...
	OO.ui.InputWidget.prototype.restorePreInfuseState = function (state) {
		OO.ui.InputWidget.super.prototype.restorePreInfuseState.call(
			this,
			state
		);
	...
	OO.ui.ButtonInputWidget = function OoUiButtonInputWidget(config) {
		...
		// Mixin constructors
		OO.ui.mixin.ButtonElement.call(
			this,
			$.extend(
				{
					$button: this.$input,
	...
	OO.ui.ButtonInputWidget.prototype.getInputElement = function (config) {
		var type;
		type =
			["button", "submit", "reset"].indexOf(config.type) !== -1
				? config.type
				: "button";
		return $(
			"<" +
				(config.useInputTag ? "input" : "button") +
				' type="' +
				type +
				'">'
		);
	...
	OO.ui.CheckboxInputWidget = function OoUiCheckboxInputWidget(config) {
		...
		// Mixin constructors
		OO.ui.mixin.RequiredElement.call(
			this,
			$.extend(
				{},
				{
		...
		this.setSelected(
			config.selected !== undefined ? config.selected : false
		);
		this.setIndeterminate(
			config.indeterminate !== undefined ? config.indeterminate : false
		);
	};
	 ...
	 * @inheritdoc
	 */
	OO.ui.CheckboxInputWidget.static.gatherPreInfuseState = function (
		node,
		config
	) {
		var state = OO.ui.CheckboxInputWidget.super.static.gatherPreInfuseState(
			node,
			config
		);
		if (config.$input) {
	 ...
	 */
	OO.ui.CheckboxInputWidget.prototype.setSelected = function (
		state,
		internal
	) {
		state = !!state;
	 ...
	 * @return {OO.ui.CheckboxInputWidget} The widget, for chaining
	 */
	OO.ui.CheckboxInputWidget.prototype.setIndeterminate = function (
		state,
		internal
	) {
		state = !!state;
	 ...
	 * @inheritdoc
	 */
	OO.ui.CheckboxInputWidget.prototype.restorePreInfuseState = function (
		state
	) {
		OO.ui.CheckboxInputWidget.super.prototype.restorePreInfuseState.call(
			this,
			state
		);
		if (
			state.checked !== undefined &&
			state.checked !== this.isSelected()
		) {
	...
	OO.ui.DropdownInputWidget.prototype.setValue = function (value) {
		...
		selected =
			this.dropdownWidget.getMenu().findItemFromData(value) ||
	...
	OO.ui.DropdownInputWidget.prototype.setOptionsData = function (options) {
		...
		for (optIndex = 0; optIndex < options.length; optIndex++) {
			...
			if (opt.optgroup !== undefined) {
				...
				optionWidget = widget.createMenuSectionOptionWidget(
					opt.optgroup
				);
	 ...
	 */
	OO.ui.DropdownInputWidget.prototype.createMenuOptionWidget = function (
		data,
		label
	) {
	 ...
	 */
	OO.ui.DropdownInputWidget.prototype.createMenuSectionOptionWidget =
		function (label) {
	...
	OO.ui.DropdownInputWidget.prototype.updateOptionsInterface = function () {
		var $optionsContainer = this.$input,
		...
		this.dropdownWidget
			...
			.forEach(function (optionWidget) {
				...
				if (!(optionWidget instanceof OO.ui.MenuSectionOptionWidget)) {
					...
					// the selection state of the input has been changed since it was created.
					$optionNode[0].defaultSelected =
						optionWidget.getData() === defaultValue;

					$optionsContainer.append($optionNode);
				} else {
					$optionNode = $("<optgroup>").attr(
						"label",
						optionWidget.getLabel()
					);
	 ...
	 * @inheritdoc
	 */
	OO.ui.RadioInputWidget.static.gatherPreInfuseState = function (
		node,
		config
	) {
		var state = OO.ui.RadioInputWidget.super.static.gatherPreInfuseState(
			node,
			config
		);
		if (config.$input) {
	...
	OO.ui.RadioInputWidget.prototype.restorePreInfuseState = function (state) {
		OO.ui.RadioInputWidget.super.prototype.restorePreInfuseState.call(
			this,
			state
		);
		if (
	 ...
	 * @inheritdoc
	 */
	OO.ui.RadioSelectInputWidget.static.gatherPreInfuseState = function (
		node,
		config
	) {
		var state =
			OO.ui.RadioSelectInputWidget.super.static.gatherPreInfuseState(
				node,
				config
			);
		state.value = $(node)
			.find(".oo-ui-radioInputWidget .oo-ui-inputWidget-input:checked")
			.val();
		return state;
	};
	 ...
	 * @inheritdoc
	 */
	OO.ui.RadioSelectInputWidget.static.reusePreInfuseDOM = function (
		node,
		config
	) {
		config = OO.ui.RadioSelectInputWidget.super.static.reusePreInfuseDOM(
			node,
			config
		);
	...
	OO.ui.RadioSelectInputWidget.prototype.setValue = function (value) {
		...
		// Only allow setting values that are actually present in the dropdown
		selected =
			this.radioSelectWidget.findItemFromData(value) ||
	...
	OO.ui.RadioSelectInputWidget.prototype.setDisabled = function (state) {
		...
		OO.ui.RadioSelectInputWidget.super.prototype.setDisabled.call(
			this,
			state
		);
		return this;
	...
	OO.ui.RadioSelectInputWidget.prototype.setOptionsData = function (options) {
		...
		this.radioSelectWidget.clearItems().addItems(
	 ...
	 * @inheritdoc
	 */
	OO.ui.CheckboxMultiselectInputWidget.static.gatherPreInfuseState =
		function (node, config) {
			var state =
				OO.ui.CheckboxMultiselectInputWidget.super.static.gatherPreInfuseState(
					node,
					config
				);
			state.value = $(node)
				.find(
					".oo-ui-checkboxInputWidget .oo-ui-inputWidget-input:checked"
				)
				.toArray()
				.map(function (el) {
					return el.value;
	...
	OO.ui.CheckboxMultiselectInputWidget.static.reusePreInfuseDOM = function (
		...
		config =
			OO.ui.CheckboxMultiselectInputWidget.super.static.reusePreInfuseDOM(
				node,
				config
			);
	 ...
	 * @protected
	 */
	OO.ui.CheckboxMultiselectInputWidget.prototype.getInputElement =
		function () {
			// Actually unused
			return $("<unused>");
		};
	 ...
	 * @private
	 */
	OO.ui.CheckboxMultiselectInputWidget.prototype.onCheckboxesSelect =
		function () {
			this.setValue(
	...
	OO.ui.CheckboxMultiselectInputWidget.prototype.setValue = function (value) {
		...
		OO.ui.CheckboxMultiselectInputWidget.super.prototype.setValue.call(
			this,
			value
		);
		if (this.optionsDirty) {
	 ...
	 * @return {string[]} Cleaned up value
	 */
	OO.ui.CheckboxMultiselectInputWidget.prototype.cleanUpValue = function (
		value
	) {
		var i,
			singleValue,
		...
		for (i = 0; i < value.length; i++) {
			singleValue =
				OO.ui.CheckboxMultiselectInputWidget.super.prototype.cleanUpValue.call(
					this,
					value[i]
				);
	 ...
	 * @inheritdoc
	 */
	OO.ui.CheckboxMultiselectInputWidget.prototype.setDisabled = function (
		state
	) {
		this.checkboxMultiselectWidget.setDisabled(state);
		OO.ui.CheckboxMultiselectInputWidget.super.prototype.setDisabled.call(
			this,
			state
		);
		return this;
	 ...
	 * @return {OO.ui.Widget} The widget, for chaining
	 */
	OO.ui.CheckboxMultiselectInputWidget.prototype.setOptions = function (
		options
	) {
		var value = this.getValue();
	 ...
	 * @private
	 */
	OO.ui.CheckboxMultiselectInputWidget.prototype.setOptionsData = function (
		options
	) {
		var widget = this;
		...
		this.checkboxMultiselectWidget.clearItems().addItems(
			options.map(function (opt) {
	 ...
	 * @private
	 */
	OO.ui.CheckboxMultiselectInputWidget.prototype.updateOptionsInterface =
		function () {
	...
	OO.ui.TextInputWidget = function OoUiTextInputWidget(config) {
		...
		OO.ui.mixin.PendingElement.call(
			this,
			$.extend({ $pending: this.$input }, config)
		);
		OO.ui.mixin.LabelElement.call(this, config);
		OO.ui.mixin.FlaggedElement.call(this, config);
		...
		this.on(
			"change",
			OO.ui.debounce(this.onDebouncedChange.bind(this), 250)
		);

		// Initialization
		this.$element
			.addClass(
				"oo-ui-textInputWidget oo-ui-textInputWidget-type-" + this.type
			)
	...
	OO.ui.TextInputWidget.prototype.getInputElement = function (config) {
		if (this.getSaneType(config) === "number") {
			return $("<input>").attr("step", "any").attr("type", "number");
		} else {
	...
	OO.ui.TextInputWidget.prototype.getSaneType = function (config) {
		var allowedTypes = ["text", "password", "email", "url", "number"];
	 ...
	 * @return {OO.ui.Widget} The widget, for chaining
	 */
	OO.ui.TextInputWidget.prototype.setLabelPosition = function (
		labelPosition
	) {
	...
	OO.ui.TextInputWidget.prototype.positionLabel = function () {
		...
		newCss[property] =
			this.$label.outerWidth(true) + (after ? this.scrollWidth : 0);
	...
	OO.ui.SearchInputWidget = function OoUiSearchInputWidget(config) {
		...
		this.$indicator.attr("tabindex", -1).attr("role", "button");
	};
	 ...
	 */
	OO.ui.MultilineTextInputWidget = function OoUiMultilineTextInputWidget(
		config
	) {
		config = $.extend(
			{
	 ...
	 * @inheritdoc
	 */
	OO.ui.MultilineTextInputWidget.static.gatherPreInfuseState = function (
		node,
		config
	) {
		var state =
			OO.ui.MultilineTextInputWidget.super.static.gatherPreInfuseState(
				node,
				config
			);
		if (config.$input) {
	...
	OO.ui.MultilineTextInputWidget.prototype.onElementAttach = function () {
		OO.ui.MultilineTextInputWidget.super.prototype.onElementAttach.call(
			this
		);
		this.adjustSize();
	};
	...
	OO.ui.MultilineTextInputWidget.prototype.updatePosition = function () {
		OO.ui.MultilineTextInputWidget.super.prototype.updatePosition.call(
			this
		);
		this.adjustSize();
	};
	...
	OO.ui.MultilineTextInputWidget.prototype.adjustSize = function (force) {
		...
		if (force || this.$input.val() !== this.valCache) {
			if (this.autosize) {
				...
				newHeight =
					idealHeight > innerHeight
						? idealHeight + (outerHeight - innerHeight)
						: "";
			...
			scrollWidth =
				this.$input[0].offsetWidth - this.$input[0].clientWidth;
			if (scrollWidth !== this.scrollWidth) {
				property =
					this.$element.css("direction") === "rtl" ? "left" : "right";
				// Reset
	 ...
	 * @inheritdoc
	 */
	OO.ui.MultilineTextInputWidget.prototype.restorePreInfuseState = function (
		state
	) {
		OO.ui.MultilineTextInputWidget.super.prototype.restorePreInfuseState.call(
			this,
			state
		);
	...
	OO.ui.ComboBoxInputWidget.prototype.setReadOnly = function () {
		// Parent method
		OO.ui.ComboBoxInputWidget.super.prototype.setReadOnly.apply(
			this,
			arguments
		);
		this.updateControlsDisabled();
	...
	OO.ui.FieldLayout = function OoUiFieldLayout(fieldWidget, config) {
		...
		OO.ui.mixin.TitledElement.call(
			this,
			$.extend({ $titled: this.$label }, config)
		);
	...
	OO.ui.FieldLayout.prototype.isFieldInline = function () {
		...
		return (
			this.getField().$element.prop("tagName").toLowerCase() === "span"
		);
	};
	...
	OO.ui.FieldLayout.prototype.setAlignment = function (value) {
		if (value !== this.align) {
			...
			if (this.align) {
				this.$element.removeClass(
					"oo-ui-fieldLayout-align-" + this.align
				);
			}
	...
	OO.ui.FieldLayout.prototype.updateMessages = function () {
		...
		for (i = 0; i < this.warnings.length; i++) {
			this.$messages.append(
				this.makeMessage("warning", this.warnings[i])
			);
		}
		for (i = 0; i < this.successMessages.length; i++) {
			this.$messages.append(
				this.makeMessage("success", this.successMessages[i])
			);
		}
	 ...
	 * @param {Object} config
	 */
	OO.ui.ActionFieldLayout = function OoUiActionFieldLayout(
		fieldWidget,
		buttonWidget,
		config
	) {
	...
	OO.ui.NumberInputWidget = function OoUiNumberInputWidget(config) {
		...
		// Parent constructor
		OO.ui.NumberInputWidget.super.call(
			this,
			$.extend(config, {
				type: "number",
			})
		...
		if (config.showButtons) {
			this.minusButton = new OO.ui.ButtonWidget(
				$.extend(
					{
						disabled: this.isDisabled(),
						tabIndex: -1,
			...
			this.plusButton = new OO.ui.ButtonWidget(
				$.extend(
					{
						disabled: this.isDisabled(),
						tabIndex: -1,
	 ...
	 *  of this.
	 */
	OO.ui.NumberInputWidget.prototype.setStep = function (
		buttonStep,
		pageStep,
		step
	) {
	...
	OO.ui.NumberInputWidget.prototype.setValue = function (value) {
		...
		return OO.ui.NumberInputWidget.super.prototype.setValue.call(
			this,
			value
		);
	};
	...
	OO.ui.NumberInputWidget.prototype.setDisabled = function (disabled) {
		// Parent method
		OO.ui.NumberInputWidget.super.prototype.setDisabled.call(
			this,
			disabled
		);
		this.updateControlsDisabled();
	...
	OO.ui.NumberInputWidget.prototype.setReadOnly = function () {
		// Parent method
		OO.ui.NumberInputWidget.super.prototype.setReadOnly.apply(
			this,
			arguments
		);
		this.updateControlsDisabled();
	...
	OO.ui.SelectFileInputWidget = function OoUiSelectFileInputWidget(config) {
		...
		this.fieldLayout = new OO.ui.ActionFieldLayout(
			this.info,
			this.selectButton,
			{ align: "top" }
		);
	...
	OO.ui.SelectFileInputWidget.prototype.setValue = function (value) {
		...
		if (value) {
			...
			this.currentFiles = [];
			// Parent method
			return OO.ui.SelectFileInputWidget.super.prototype.setValue.call(
				this,
				""
			);
		}
	...
	OO.ui.SelectFileInputWidget.prototype.setDisabled = function (disabled) {
		// Parent method
		OO.ui.SelectFileInputWidget.super.prototype.setDisabled.call(
			this,
			disabled
		);
