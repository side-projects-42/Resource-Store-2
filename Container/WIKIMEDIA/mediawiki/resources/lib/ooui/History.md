...
- Allow _even more_ widgets to be focussed programatically (Bartosz Dziewoński)
...
- Follow-up Iaa7dffc13: _Actually_ allow `$returnFocusTo` to be `null` (Ed Sanders)
...
- PopupWidget: Make popups able to actually pop _up_, as well as sideways (Roan Kattouw)
...
- [DEPRECATING CHANGE] icons: Rename '_Undo' to 'un_' (James D. Forrester)
...
- ClippableElement: Only call reconsiderScrollbars() if we actually _stopped_ clipping (Roan Kattouw)
