...
(function (global$1) {
	...
	var equiv = (function () {
		...
		function isContainer(val) {
			return (
				["object", "array", "map", "set"].indexOf(objectType(val)) !==
				-1
			);
		}
	...
	function processModule(name, options, executeNow) {
		...
		function setHookFromEnvironment(hooks, environment, name) {
			...
			hooks[name] =
				typeof potentialHook === "function" ? [potentialHook] : [];
  ...
  */
    ...
    */
	...
	function done() {
		...
		if (config.stats.all === 0) {
			if (config.filter && config.filter.length) {
				throw new Error(
					'No tests matched the filter "' + config.filter + '".'
				);
			}

			if (config.module && config.module.length) {
				throw new Error(
					'No tests matched the module "' + config.module + '".'
				);
			}
			...
			if (config.testId && config.testId.length) {
				throw new Error(
					'No tests matched the testId "' + config.testId + '".'
				);
			}
			...
			if (this.steps.length) {
				...
				this.pushFailure(
					"Expected assert.verifySteps() to be called before end of test " +
						("after using assert.step(). Unverified steps: " +
							stepsList),
					this.stack
				);
			}
			...
			return runLoggingCallbacks("testDone", {
				...
				.then(function () {
					if (module.testsRun === numberOfTests(module)) {
						...
						while (
							parent &&
							parent.testsRun === numberOfTests(parent)
						) {
	...
	QUnit.isLocal = !(
		defined.document && window$1.location.protocol !== "file:"
	);
	...
	(function () {
		...
		function toggleClass(elem, name, force) {
			if (
				force ||
				(typeof force === "undefined" && !hasClass(elem, name))
			) {
				addClass(elem, name);
			} else {
		...
		function toolbarChanged() {
			...
			if (
				"hidepassed" === field.name &&
				"replaceState" in window$1.history
			) {
				...
				if (tests) {
					...
					if (field.checked) {
						for (var i = 0; i < length; i++) {
							...
							if (
								test$$1 &&
								test$$1.className.indexOf("pass") > -1
							) {
						...
						try {
							for (
								var _iterator = hiddenTests[Symbol.iterator](),
									_step;
		...
		function moduleListHtml() {
			...
			for (i = 0; i < config.modules.length; i++) {
				if (config.modules[i].name !== "") {
					checked =
						config.moduleId.indexOf(config.modules[i].moduleId) >
						-1;
					html +=
		...
		QUnit.done(function (details) {
			...
			if (abortButton && abortButton.disabled) {
				html =
					"Tests aborted after " + details.runtime + " milliseconds.";

				for (var i = 0; i < tests.children.length; i++) {
					test$$1 = tests.children[i];
					if (
						test$$1.className === "" ||
						test$$1.className === "running"
					) {
		...
		QUnit.log(function (details) {
			...
			message +=
				"<span class='runtime'>@ " + details.runtime + " ms</span>";
