...
describe("POST /page", () => {
	...
	describe("successful operation", () => {
		it("should create a page if it does not exist", async () => {
			...
			assert.match(
				location,
				new RegExp(`^https?://.*/v1/page/${encodedTitle}$`)
			);
			checkEditResponse(title, reqBody, editBody);

			// follow redirect
			const { status: redirStatus, body: redirBody } = await supertest
				.agent(location)
				.get("");
			...
			const { status: sourceStatus, body: sourceBody } = await client.get(
				`/page/${title}`
			);
		...
		it("should create a page with specified model", async () => {
			...
			const { status: editStatus, body: editBody } = await client.post(
				"/page",
				reqBody
			);
			assert.equal(editStatus, 201);
