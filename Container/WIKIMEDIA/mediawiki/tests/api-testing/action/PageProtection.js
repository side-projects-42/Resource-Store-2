...
describe("Test page protection levels and effectiveness", function () {
	...
	const testForExpiry = async (expiry) => {
		...
		const editPage = await wikiUser.actionError(
			"edit",
			{
				title,
				token,
	...
	const protectWithWatchParam = async (
		watchedBefore,
		watchlistParam,
		title
	) => {
	...
	before(async () => {
		...
		const addSysopProtection = await admin.action(
			"protect",
			{
				title: protectedPage,
				token: adminEditToken,
		...
		const addAutoConfirmedProtection = await admin.action(
			"protect",
			{
	...
	it("should allow autoconfirmed user to edit Semi Protected page", async () => {
		await wikiUser.edit(semiProtectedPage, {
	...
	it("should NOT allow anonymous user to edit Protected page", async () => {
		...
		const editPage = await anonymousUser.actionError(
			"edit",
			{
				title: protectedPage,
				token,
	...
	it("protect a page for an infinite time to allow only autoconfirmed users to edit it and only sysops to move it", async () => {
		...
		const movePage = await anonymousUser.actionError(
			"move",
			{
				from: title,
	...
	it("should test that a user has created and edited pages on users watchlist when the watchlist parameter is set to preferences", async () => {
		const list = await protectWithWatchParam(
			"watched",
			"preferences",
			testWatchlist
		);
