...
describe("Diff Compare with Variables", function () {
	...
	it("should edit a page", async () => {
		const editPage = await wikiuser.edit(title, {
			text: "Counting: \n* One",
		});
	...
	it("should compare revisions 1 and 4", async () => {
		const compareRevisions = await wikiuser.action("compare", {
			fromrev: variables.revision1,
			torev: variables.revision4,
		});
