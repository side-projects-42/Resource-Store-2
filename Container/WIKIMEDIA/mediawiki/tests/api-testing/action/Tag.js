...
describe("The tag action module", function () {
	...
	before(async () => {
		...
		const recentChange = await bob.getChangeEntry({ rctitle: page });
	...
	it("Adds and removes tags from revision", async () => {
		...
		const { query: queryWithoutTags } = await bob.action(
			"query",
			queryParams
		);
	...
	it("Adds and removes tags from recent change", async () => {
		...
		// Query to make sure tag was saved
		const recentChangeWithTag = await bob.list(
			"recentchanges",
			queryParams
		);
		...
		const recentChangeWithoutTag = await bob.list(
			"recentchanges",
			queryParams
		);
