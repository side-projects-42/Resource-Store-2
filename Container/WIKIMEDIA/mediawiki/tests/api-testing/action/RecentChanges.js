...
describe("Recent Changes", function () {
	...
	it("should create page and get new page recent changes", async () => {
		const edit = await alice.edit(title, {
			text: "Recent changes testing",
			createonly: true,
		});
		const results = await alice.list("recentchanges", {
			rctype: "new",
			rctitle: title,
		});
	...
	it("should edit page and get most recent edit changes", async () => {
		const rev1 = await alice.edit(title, {
			text: "Recent changes testing..R1",
		});
		const results = await alice.list("recentchanges", {
			rctype: "edit",
			rctitle: title,
