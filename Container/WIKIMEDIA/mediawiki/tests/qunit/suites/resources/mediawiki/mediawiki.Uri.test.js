(function () {
	...
	QUnit.test("Constructor( String[, Object ] )", function (assert) {
		...
		assert.strictEqual(
			uri.query.n,
			"1",
			"Simple parameter with overrideKeys:true"
		);
		assert.strictEqual(
			uri.query.m,
			"bar",
			"Last key overrides earlier keys with overrideKeys:true"
		);
		...
		assert.strictEqual(
			uri.query.n,
			"1",
			"Simple parameter with overrideKeys:false"
		);
		assert.strictEqual(
			uri.query.m[0],
			"foo",
			"Order of multi-value parameters with overrideKeys:true"
		);
		assert.strictEqual(
			uri.query.m[1],
			"bar",
		...
		assert.strictEqual(
			uri.query.m.length,
			2,
			"Number of mult-value field is correct"
		);
	...
	QUnit.test("Constructor( Object )", function (assert) {
		...
		assert.strictEqual(
			uri.toString(),
			"http://www.foo.local/this?hi=there#blah",
	...
	QUnit.test("Properties", function (assert) {
		...
		assert.strictEqual(
			uri.toString(),
			"http://en.wiki.local/w/api.php#frag",
			"add a fragment"
		);
		uri.fragment = "café";
		assert.strictEqual(
			uri.toString(),
			"http://en.wiki.local/w/api.php#caf%C3%A9",
		...
		assert.strictEqual(
			uri.toString(),
			"http://fr.wiki.local:8080/w/api.php",
		...
		assert.strictEqual(
			uri.toString(),
			"http://en.wiki.local/w/api.php?foo=bar",
			"add query arguments"
		);
		...
		assert.strictEqual(
			uri.toString(),
			"http://en.wiki.local/w/api.php",
			"delete query arguments"
		);
		...
		assert.ok(
			uri.toString().indexOf("foo=quux") !== -1,
			"extend query arguments"
		);
		// eslint-disable-next-line qunit/no-ok-equality
		assert.notOk(
			uri.toString().indexOf("foo=bar") !== -1,
			"extend query arguments"
		);
		// eslint-disable-next-line qunit/no-ok-equality
		assert.ok(
			uri.toString().indexOf("pif=paf") !== -1,
			"extend query arguments"
		);
	});
	...
	QUnit.test(".getQueryString()", function (assert) {
		...
		uri = new mw.Uri(
			"https://example.com/mw/index.php?title=Sandbox/7&other=Sandbox/7&foo"
		);
		assert.strictEqual(
			uri.getQueryString(),
	...
	QUnit.test("arrayParams", function (assert) {
		...
		uri1 = new mw.Uri("http://example.com/?foo[]=a&foo[]=b&foo[]=c", {
			arrayParams: true,
		});
		uri2 = new mw.Uri("http://example.com/?foo[0]=a&foo[1]=b&foo[2]=c", {
			arrayParams: true,
		});
		uri3 = new mw.Uri("http://example.com/?foo[1]=b&foo[0]=a&foo[]=c", {
			arrayParams: true,
		});
		expectedQ = { foo: ["a", "b", "c"] };
		...
		uriMissing = new mw.Uri("http://example.com/?foo[0]=a&foo[2]=c", {
			arrayParams: true,
		});
		...
		assert.deepEqual(
			uriMissing.getQueryString(),
			expectedMissingS,
			"array query parameters are encoded (missing array item)"
		);

		uriWeird = new mw.Uri(
			"http://example.com/?foo[0]=a&foo[1][1]=b&foo[x]=c",
			{ arrayParams: true }
		);
	...
	QUnit.test(".clone()", function (assert) {
		...
		assert.notEqual(
			original.toString(),
			clone.toString(),
			"Stringified url no longer matches original"
		);
	...
	QUnit.test(".toString() after query manipulation", function (assert) {
		...
		// eslint-disable-next-line qunit/no-ok-equality
		assert.ok(
			uri.toString().indexOf("m=bar") !== -1,
			"toString preserves other values"
		);
		// eslint-disable-next-line qunit/no-ok-equality
		assert.ok(
			uri.toString().indexOf("n=x&n=y&n=z") !== -1,
			"toString parameter includes all values of an array query parameter"
		);
		assert.strictEqual(
			uri.toString().length,
		...
		// eslint-disable-next-line qunit/no-ok-equality
		assert.ok(
			uri.toString().indexOf("m=foo&m=bar") !== -1,
			"toString preserves other values"
		);
		// eslint-disable-next-line qunit/no-ok-equality
		assert.ok(
		...
		assert.strictEqual(
			uri.toString(),
			"http://www.example.com/dir/?m=bar",
			"deletion properties"
		);
	...
	QUnit.test("Advanced URL", function (assert) {
		...
		uri = new mw.Uri(
			"http://auth@www.example.com:81/dir/dir.2/index.htm?q1=0&&test1&test2=value+%28escaped%29#caf%C3%A9"
		);
		...
		assert.strictEqual(
			uri.getAuthority(),
			"auth@www.example.com:81",
			"authority equal to auth@hostport"
		);

		assert.strictEqual(
			uri.getHostPort(),
			"www.example.com:81",
			"hostport equal to host:port"
		);

		queryString = uri.getQueryString();
		// eslint-disable-next-line qunit/no-ok-equality
		assert.ok(
			queryString.indexOf("q1=0") !== -1,
			"query param with numbers"
		);
		// eslint-disable-next-line qunit/no-ok-equality
		assert.ok(
			queryString.indexOf("test1") !== -1,
			"query param with null value is included"
		);
		// eslint-disable-next-line qunit/no-ok-equality
		assert.notOk(
			queryString.indexOf("test1=") !== -1,
			"query param with null value does not generate equals sign"
		);
		// eslint-disable-next-line qunit/no-ok-equality
		assert.ok(
			queryString.indexOf("test2=value+%28escaped%29") !== -1,
			"query param is url escaped"
		);

		relativePath = uri.getRelativePath();
		assert.ok(relativePath.indexOf(uri.path) >= 0, "path in relative path");
		assert.ok(
			relativePath.indexOf(uri.getQueryString()) >= 0,
			"query string in relative path"
		);
		assert.ok(
			relativePath.indexOf(mw.Uri.encode(uri.fragment)) >= 0,
			"escaped fragment in relative path"
		);
	});
	...
	QUnit.test(
		...
		function (assert) {
			var uri = new mw.Uri(
				"http://www.example.com/test@test?x=@uri&y@=uri&z@=@"
			);
	...
	QUnit.test("Handle protocol-relative URLs", function (assert) {
		...
		assert.strictEqual(
			uri.protocol,
			"glork",
			"create protocol-relative URLs with same protocol as document"
		);

		uri = new UriRel("/foo.com");
		assert.strictEqual(
			uri.toString(),
			"glork://en.wiki.local/foo.com",
			"handle absolute paths by supplying protocol and host from document in loose mode"
		);

		uri = new UriRel("http:/foo.com");
		assert.strictEqual(
			uri.toString(),
			"http://en.wiki.local/foo.com",
			"handle absolute paths by supplying host from document in loose mode"
		);
		...
		assert.strictEqual(
			uri.toString(),
			"glork://en.wiki.local/foo.com",
			"handle absolute paths by supplying protocol and host from document in strict mode"
		);
		...
		assert.strictEqual(
			uri.toString(),
			"http://en.wiki.local/foo.com",
			"handle absolute paths by supplying host from document in strict mode"
		);
	});
	...
	QUnit.test("T37658", function (assert) {
		...
		UriClass = mw.UriRelative(
			testProtocol + testServer + "/some/path/index.html"
		);
		uri = new UriClass(testPath);
		href = uri.toString();
		assert.strictEqual(
			href,
			testProtocol + testServer + testPath,
			"Root-relative URL gets host & protocol supplied"
		);

		UriClass = mw.UriRelative(
			testProtocol + testServer + ":" + testPort + "/some/path.php"
		);
		uri = new UriClass(testPath);
		href = uri.toString();
		assert.strictEqual(
			href,
			testProtocol + testServer + ":" + testPort + testPath,
