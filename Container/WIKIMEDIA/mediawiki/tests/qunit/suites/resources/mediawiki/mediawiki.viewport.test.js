(function () {
		...
		QUnit.newMwEnvironment({
			setup: function () {
				this.el = $("<div />")
					...
					.get(0);
				this.sandbox
					.stub(mw.viewport, "makeViewportFromWindow")
	...
	QUnit.test("isElementInViewport", function (assert) {
		...
		assert.ok(
			mw.viewport.isElementInViewport(this.el, viewport),
		...
		assert.ok(
			mw.viewport.isElementInViewport(this.el),
	...
	QUnit.test("isElementInViewport with scrolled page", function (assert) {
		...
		assert.ok(
			mw.viewport.isElementInViewport(el, viewport),
	...
	QUnit.test("isElementCloseToViewport", function (assert) {
		...
		assert.ok(
			mw.viewport.isElementCloseToViewport(this.el, 60, viewport),
			"It should return true when the element is within the given threshold away"
		);
		assert.notOk(
			mw.viewport.isElementCloseToViewport(this.el, 20, viewport),
			"It should return false when the element is further than the given threshold away"
		);
		assert.notOk(
			mw.viewport.isElementCloseToViewport(distantElement),
