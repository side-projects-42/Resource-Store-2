(function () {
	...
	QUnit.test("newFromText", function (assert) {
		...
		sharedCases.invalid.forEach(function (title) {
			assert.strictEqual(mw.Title.newFromText(title), null, title);
		});
	});
	...
	QUnit.test("Transformation", function (assert) {
		...
		assert.strictEqual(
			...
			2,
			"Parse canonical namespace prefix"
		);

		title = new mw.Title("Image:quux pix.jpg");
		assert.strictEqual(
			title.getNamespacePrefix(),
			"File:",
			"Transform alias to canonical namespace"
		);
		...
		assert.strictEqual(
			title.getNamespaceId(),
			2,
			"Case-insensitive namespace prefix"
		);
		...
		assert.strictEqual(
			title.getExtension(),
			"js",
			"Space after an extension is stripped"
		);
	...
	QUnit.test("Namespace detection and conversion", function (assert) {
		...
		assert.strictEqual(
			title.getNamespaceId(),
			6,
			"Default namespace passed is used"
		);
		...
		assert.strictEqual(
			title.getNamespaceId(),
			2,
			"Included namespace prefix overrides the given default"
		);
		...
		assert.strictEqual(
			title.toString(),
			"Penguins:Flightless_yet_cute.jpg"
		);
	...
	QUnit.test("isTalkPage/getTalkPage/getSubjectPage", function (assert) {
		...
		assert.strictEqual(
			title.isTalkPage(),
			false,
			"Non-talk page detected as such"
		);
		assert.strictEqual(
		...
		assert.strictEqual(
			...
			"User talk:Foo",
			"getTalkPage creates correct title"
		);
		assert.strictEqual(
		...
		assert.strictEqual(
			title.getPrefixedText(),
			"User:Foo",
			"getSubjectPage creates correct title"
		);
		...
		assert.strictEqual(
			title.isTalkPage(),
			false,
			"Special page is not a talk page"
		);
		assert.strictEqual(
			title.getTalkPage(),
			null,
			"getTalkPage not valid for this namespace"
		);
		assert.strictEqual(
			title.getSubjectPage().getPrefixedText(),
			"Special:AllPages",
		...
		assert.strictEqual(
			title.getFragment(),
			null,
			"getTalkPage does not copy the fragment"
		);
	});
	...
	QUnit.test("wantSignaturesNamespace", function (assert) {
		...
		assert.strictEqual(
			mw.Title.wantSignaturesNamespace(0),
			false,
			"Main namespace has no signatures"
		);
		assert.strictEqual(
			mw.Title.wantSignaturesNamespace(1),
			true,
			"Talk namespace has signatures"
		);
		assert.strictEqual(
			mw.Title.wantSignaturesNamespace(2),
			false,
			"NS2 has no signatures"
		);
		assert.strictEqual(
			mw.Title.wantSignaturesNamespace(3),
			true,
			"NS3 has signatures"
		);
		...
		assert.strictEqual(
			mw.Title.wantSignaturesNamespace(0),
			true,
			"Main namespace has signatures when explicitly defined"
		);
	});
	...
	QUnit.test("Case-sensivity", function (assert) {
		...
		assert.strictEqual(
			title.toString(),
			"ß",
			"Uppercasing matches PHP behaviour (ß -> ß, not SS)"
		);
		...
		assert.strictEqual(
			title.toString(),
			"ǅ_(digraph)",
			"Uppercasing matches PHP behaviour (ǆ -> ǅ, not Ǆ)"
		);

		// $wgCapitalLinks = false;
		mw.config.set(
			"wgCaseSensitiveNamespaces",
			[0, -2, 1, 4, 5, 6, 7, 10, 11, 12, 13, 14, 15]
		);
	...
	QUnit.test("getExtension", function (assert) {
		...
		extTest(
			"File:Example.longextension",
			"longextension",
			"Extension parsing not limited (T38151)"
		);
		extTest(
			"Example/information.json",
			"json",
			"Extension parsing not restricted from any namespace"
		);
		...
		extTest(
			"Foo.a.",
			null,
			"Page name with dots and ending in a dot does not have an extension"
		);
	...
	QUnit.test("exists", function (assert) {
		...
		assert.strictEqual(
			title.exists(),
			null,
			"Return null with empty existance registry"
		);

		// Basic registry, checks default to boolean
		mw.Title.exist.set(
			["Does_exist", "User_talk:NeilK", "Wikipedia:Sandbox_rules"],
			true
		);
		...
		assert.assertTrue(
			title.exists(),
			"Return true for page titles marked as existing"
		);
		title = new mw.Title("Foobar");
		assert.assertFalse(
			title.exists(),
			"Return false for page titles marked as nonexistent"
		);
	});
	...
	QUnit.test("getUrl", function (assert) {
		...
		assert.strictEqual(
			title.getUrl(),
			"/wiki/Foobar",
			"Basic functionality, getUrl uses mw.util.getUrl"
		);
		assert.strictEqual(
			title.getUrl({ action: "edit" }),
			"/w/index.php?title=Foobar&action=edit",
			"Basic functionality, 'params' parameter"
		);

		title = new mw.Title("John Doe", 3);
		assert.strictEqual(
			title.getUrl(),
			"/wiki/User_talk:John_Doe",
			"Escaping in title and namespace for urls"
		);

		title = new mw.Title("John Cena#And_His_Name_Is", 3);
		assert.strictEqual(
			title.getUrl({ meme: true }),
			"/w/index.php?title=User_talk:John_Cena&meme=true#And_His_Name_Is",
			"title with fragment and query parameter"
		);
	});
	...
	QUnit.test("newFromImg", function (assert) {
		...
		cases.forEach(function (caseItem) {
			...
			if (caseItem.nameText !== undefined) {
				...
				assert.strictEqual(
					title.getPrefixedText(),
					caseItem.prefixedText,
					prefix + "File page title matches original"
				);
				assert.strictEqual(
					title.getNamespaceId(),
					6,
					prefix + "Namespace ID matches File namespace"
				);
			} else {
				assert.strictEqual(
					title,
	...
	QUnit.test("getRelativeText", function (assert) {
		...
		cases.forEach(function (caseItem) {
			...
			assert.strictEqual(
				title.getRelativeText(caseItem.relativeTo),
	...
	QUnit.test("newFromUserInput", function (assert) {
		...
		cases.forEach(function (caseItem) {
			var prefix,
				title = mw.Title.newFromUserInput(
					caseItem.title,
					caseItem.defaultNamespace,
					caseItem.options
				);
			...
			if (caseItem.expected !== undefined) {
				...
				if (caseItem.defaultNamespace === undefined) {
					...
					assert.strictEqual(
						title.toText(),
						caseItem.expected,
						prefix + "Skipping namespace argument"
					);
				}
			} else {
				assert.strictEqual(
	...
	QUnit.test(
		...
		function (assert) {
			...
			assert.strictEqual(
				title,
				null,
				"file name is not accepted for upload"
			);
		}
	);
	...
	QUnit.test("newFromUserInput with misplaced parameter", function (assert) {
		...
		assert.strictEqual(
			title,
			null,
			"misplaced options parameter is ignored"
		);
	});
	...
	QUnit.test(
		...
		function (assert) {
			var title = mw.Title.newFromUserInput("File:No_dot", 0, {
				forUploading: false,
			});
	...
	QUnit.test("newFromFileName", function (assert) {
		...
		if (mw.config.get("wgIllegalFileChars", "").indexOf(":") > -1) {
			...
			cases.push({
				...
				typeOfName:
					"File name with : character not permitted in titles that are replaced",
	...
	QUnit.test("makeTitle for non existent namespace", function (assert) {
		...
		this.sandbox
			.stub(mw.config, "get")
			.withArgs("wgFormattedNamespaces")
			.returns({
				4: "NoTalk",
			})
			.withArgs("wgCaseSensitiveNamespaces")
			.returns([])
			.withArgs("wgNamespaceIds")
			.returns({
				notalk: 4,
			});
