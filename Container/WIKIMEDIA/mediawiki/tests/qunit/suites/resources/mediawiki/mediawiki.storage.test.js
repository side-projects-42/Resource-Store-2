(function () {
	...
	QUnit.test("set/get(Object) with storage support", function (assert) {
		...
		assert.strictEqual(
			mw.storage.setObject("baz", object),
			true,
			"setObject returns true"
		);
		assert.deepEqual(
			mw.storage.getObject("baz"),
			object,
			"Check value gets stored"
		);
		...
		assert.strictEqual(
			mw.storage.getObject("quux"),
			null,
			"Unset values are null"
		);
		assert.strictEqual(
			mw.storage.remove("baz"),
			true,
			"remove returns true"
		);
		assert.strictEqual(
	...
	QUnit.test(
		...
		function (assert) {
			...
			assert.strictEqual(
				mw.storage.setObject("bar", { test: "value" }),
				false
			);
