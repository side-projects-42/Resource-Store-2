(function () {
	...
	QUnit.test("getRGB", function (assert) {
		...
		assert.deepEqual(
			$.colorUtil.getRGB([0, 100, 255]),
			[0, 100, 255],
			"Parse array of rgb values"
		);
		assert.deepEqual(
			$.colorUtil.getRGB("rgb(0,100,255)"),
			[0, 100, 255],
			"Parse simple rgb string"
		);
		assert.deepEqual(
			$.colorUtil.getRGB("rgb(0, 100, 255)"),
			[0, 100, 255],
			"Parse simple rgb string with spaces"
		);
		assert.deepEqual(
			$.colorUtil.getRGB("rgb(0%,20%,40%)"),
			[0, 51, 102],
			"Parse rgb string with percentages"
		);
		assert.deepEqual(
			$.colorUtil.getRGB("rgb(0%, 20%, 40%)"),
			[0, 51, 102],
			"Parse rgb string with percentages and spaces"
		);
		assert.deepEqual(
			$.colorUtil.getRGB("#f2ddee"),
			[242, 221, 238],
			"Hex string: 6 char lowercase"
		);
		assert.deepEqual(
			$.colorUtil.getRGB("#f2DDEE"),
			[242, 221, 238],
			"Hex string: 6 char uppercase"
		);
		assert.deepEqual(
			$.colorUtil.getRGB("#f2DdEe"),
			[242, 221, 238],
			"Hex string: 6 char mixed"
		);
		assert.deepEqual(
			$.colorUtil.getRGB("#eee"),
			[238, 238, 238],
			"Hex string: 3 char lowercase"
		);
		assert.deepEqual(
			$.colorUtil.getRGB("#EEE"),
			[238, 238, 238],
			"Hex string: 3 char uppercase"
		);
		assert.deepEqual(
			$.colorUtil.getRGB("#eEe"),
			[238, 238, 238],
			"Hex string: 3 char mixed"
		);
		assert.deepEqual(
			$.colorUtil.getRGB("rgba(0, 0, 0, 0)"),
			[255, 255, 255],
			"Zero rgba for Safari 3; Transparent (whitespace)"
		);
		...
		assert.deepEqual(
			$.colorUtil.getRGB("lightGreen"),
			[144, 238, 144],
			"Color names (lightGreen)"
		);
		assert.deepEqual(
			$.colorUtil.getRGB("transparent"),
			[255, 255, 255],
			"Color names (transparent)"
		);
		assert.strictEqual(
			$.colorUtil.getRGB("mediaWiki"),
			undefined,
	...
	QUnit.test("hslToRgb", function (assert) {
		...
		assert.deepEqual(
			ret,
			[183, 240, 168],
			"hsl(0.3, 0.7, 0.8): rgb(183, 240, 168)"
		);
	});
	...
	QUnit.test("getColorBrightness", function (assert) {
		...
		assert.strictEqual(
			a,
			"rgb(255,50,50)",
			'Start with named color "red", brighten 10%'
		);
		...
		assert.strictEqual(
			b,
			"rgb(118,29,29)",
			'Start with rgb string "rgb(200,50,50)", darken 20%'
		);
	});
})();
