(function () {
	...
	QUnit.test("get()", function (assert) {
		...
		this.server.respond(function (request) {
			...
			assert.ok(
				/rest.php\/test\/rest\/path\?queryParam=%2Fslash-will-be-encoded%3F$/.test(
					request.url
				),
				"Should have correct request URL"
			);
			assert.strictEqual(
				request.requestHeaders.MyHeader,
				"MyHeaderValue"
			);
	...
	QUnit.test("get() respects ajaxOptions url", function (assert) {
		...
		this.server.respond(function (request) {
			assert.ok(
				/test.php\/test\/rest\/path$/.test(request.url),
				"Should have correct request URL"
			);
			request.respond(200, { "Content-Type": "application/json" }, "{}");
		});
	...
	QUnit.test("post()", function (assert) {
		...
		this.server.respond(function (request) {
			...
			assert.ok(
				/rest.php\/test\/bla\/bla\/bla$/.test(request.url),
