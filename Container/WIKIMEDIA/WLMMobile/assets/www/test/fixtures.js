...
var DUMMY_TEMPLATES = {
  "country-list-template":
    '<% _.each( campaigns, function( campaign ) { %><li><a data-campaign="<%= campaign.code %>"><%= campaign.code %></a></li><% }); %>',
  ...
  "results-page": [
    '<select class="toggle-page"><option value="results-page">list</option>',
    '<option value="map-page-stub">map</option></select>',
  ].join(""),
  ...
  if (options && options.data && options.data.prop === "imageinfo") {
    ...
    if (options.data && options.data.action === "adminlevels") {
      ...
      if (admintree === "") {
        ...
        d.resolve({
          admin_levels: [{ name: "[[Alameda, California|Alameda]]" }],
        });
      } else if (admintree === "US|US-CA|[[Foo County, California]]") {
        d.resolve({ admin_levels: [{ name: "[[Foo, California|Foo]]" }] });
      } else if (
        admintree ===
        "US|US-CA|[[Foo County, California]]|[[Foo, California|Foo]]"
      ) {
        d.resolve({ admin_levels: [] });
      } else if (
        admintree ===
        "US|US-CA|[[Alameda, California|Alameda]]|[[Alameda, California|Alameda]]"
      ) {
        d.resolve({ admin_levels: [] });
      } else if (admintree === "US|US-CA|Los Angeles County, California") {
        d.resolve({ admin_levels: [{ name: "ok" }] });
      } else if (admintree === "US|US-CA|Stump Town") {
        // bottom of treee
        d.resolve({ admin_levels: [] });
      }
    } else if (options.data.action === "search") {
      if (
        options.data.sradm2 === "San Francisco" &&
        options.data.sradm1 === "US-CA" &&
        options.data.sradm0 === "US"
      ) {
