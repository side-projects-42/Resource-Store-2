(function () {
  ...
  test("back button on geolookup-page", function () {
    ...
    strictEqual(
      page,
      "welcome-page",
      "back button should escape location page"
    );
  });
  ...
  test("Skip login page on a back button press when user is authenticated", function () {
    ...
    strictEqual(
      page,
      "detail-page",
      "when logged in login-page should not be in the history"
    );
  });
  ...
  test("toggling view type", function () {
    ...
    app.showPage("results-page");
    strictEqual(
      $("#results-page .toggle-page").val(),
      "results-page",
      "check map view remains selected"
    );
  });
  ...
  test("back button (select by campaign)", function () {
    ...
    strictEqual(
      prevPage,
      "map-page",
      "last page is country page (map not cached)"
    );
    strictEqual(prevPage2, "results-page", "page before is results");
    strictEqual(
      prevPage3,
      "country-page",
      "page before is the country as the user has already been back to the map"
    );
  });
  ...
  test("resolveImageThumbnail", function () {
    var url1 =
      "http://upload.wikimedia.org/wikipedia/test/8/88/%28taken_on_25Jul2012_16hrs38mins14secs%29.jpeg";
    var url1r =
      "http://upload.wikimedia.org/wikipedia/test/thumb/8/88/%28taken_on_25Jul2012_16hrs38mins14secs%29.jpeg/180px-%28taken_on_25Jul2012_16hrs38mins14secs%29.jpeg";
    var url2 =
      "http://upload.wikimedia.org/wikipedia/commons/c/cc/Junonia_hierta_by_kadavoor.JPG";
    var url2r =
      "http://upload.wikimedia.org/wikipedia/commons/thumb/c/cc/Junonia_hierta_by_kadavoor.JPG/180px-Junonia_hierta_by_kadavoor.JPG";
  ...
  test("drill down back behaviour (bug 39354)", function () {
    ...
    strictEqual(
      page4,
      "results-page",
      "check page name is results as we reached the bottom level"
    );
    page5 = app.goBack(); // go back
    strictEqual(
      page5,
      "campaign-page/US/US-CA",
      "check page name after back press"
    );
  ...
  test("drilling down with non-unique names (arthurs bug)", function () {
    ...
    strictEqual(
      page4,
      "campaign-page/US/US-CA/%5B%5BAlameda%2C%20California%7CAlameda%5D%5D"
    );
  ...
  test("drilling down weirdness related to back button (arthurs 2nd bug)", function () {
    ...
    strictEqual(
      page4,
      "campaign-page/US/US-CA/%5B%5BFoo%20County%2C%20California%5D%5D"
    );
