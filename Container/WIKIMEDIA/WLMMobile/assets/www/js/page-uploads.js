...
define(["jquery"], function () {
  ...
  function showUploads() {
    ...
    db.requestUploads(db.UPLOAD_COMPLETE).done(function (uploads) {
      ...
      if (uploads.length) {
        ...
        var uploadsTemplate = templates.getTemplate(
          "upload-list-item-template"
        );
        var uploadCompleteTemplate = templates.getTemplate(
          "upload-completed-item-detail-template"
        );
        $.each(uploads, function (i, upload) {
          ...
          photo.data.fileUrl =
            WLMConfig.WIKI_BASE +
            "File:" +
            encodeURIComponent(photo.data.fileTitle);
          $uploadItem.find("a").click(function () {
            $("#completed-upload-detail").html(
        ...
        var emptyUploadTemplate = templates.getTemplate(
          "upload-list-empty-template"
        );
  ...
  function showIncompleteUploads() {
    ...
    db.requestUploads(db.UPLOAD_INCOMPLETE).done(function (uploads) {
      ...
      if (uploads.length) {
        ...
        var uploadsTemplate = templates.getTemplate(
          "upload-incomplete-list-item-template"
        );
        var uploadIncompleteTemplate = templates.getTemplate(
          "upload-incomplete-item-detail-template"
        );
        var thumbQueue = [];
        $.each(uploads, function (i, upload) {
          ...
          $uploadItem
            ...
            .click(function () {
              var $checked = $(
                "#incomplete-uploads-page .monuments-list input[type=checkbox]:checked"
              );
              if ($checked.length > 0) {
          ...
          $uploadItem.click(function () {
            $("#incomplete-upload-detail")
              .html(
                uploadIncompleteTemplate({
                  upload: upload,
                  monument: monument,
                  photo: photo,
                })
        ...
        var emptyUploadTemplate = templates.getTemplate(
          "upload-incomplete-list-empty-template"
        );
        $list.html(emptyUploadTemplate()).localize();
      }
    });
  ...
  function setupIncompleteUploads() {
    $("#select-all").click(function () {
      var $items = $(
        "#incomplete-uploads-page .monuments-list input[type=checkbox]"
      );
      if ($items.length > 0) {
    ...
    $("#delete-all").click(function () {
      ...
      if (
        window.confirm(
          mw.message("delete-selected-prompt", queue.length).plain()
        )
      ) {
        // Delete in sequence!
        function iter() {
        ...
        if (queue.length > 0) {
          ...
          photo
            .uploadTo(
              api,
              comment,
              templates.getTemplate("upload-photo-description", true)
            )
            .done(function (imageinfo) {
            ...
            .progress(app.uploadProgressHandler)
            .fail(function (data) {
