...
define(["jquery"], function ($) {
  ...
  function formatUploadDescription(photo, template, username) {
    var monument = photo.data.monument,
      campaignConfig = photo.data.campaignConfig;
    ...
    var descData = {
      ...
      cats: campaignConfig.defaultCategories.concat(
        campaignConfig.autoCategories
      ),
      date: dateYMD(),
      monument: monument,
  ...
  Photo.prototype.uploadTo = function (mwApi, comment, template) {
    ...
    mwApi
      .startUpload(data.contentURL, data.fileTitle)
      .done(function (fileKey, token) {
        d.notify("in-progress");
        mwApi
          .finishUpload(fileKey, data.fileTitle, comment, fileContent, token)
