...
require([
  ...
  function addToHistory(page) {
    ...
    if (!blacklisted && pageHistory[pageHistory.length - 1] !== page) {
      // avoid adding the same page twice
  ...
  function showPage(pageName, deferred) {
    ...
    if (monuments && pageName === "results-page") {
      ...
      heading = subPage
        ? mw.msg("choose-region") +
          " (" +
          translatePageName(decodeURIComponent(subPage)) +
          ")"
        : mw.msg("choose-country");
      $page.find("h3").text(heading);
      $("#filter-campaign").attr(
        "placeholder",
        subPage
          ? mw.msg("search-region-placeholder")
          : mw.msg("search-country-placeholder")
      );
    }
  ...
  function showMonumentDetail(monument) {
    ...
    var imageFetcher = commonsApi.getImageFetcher(
    ...
    if (monument.articleLink) {
      $("#monument-detail")
        .find(".monument-article")
        .html(buildMonumentLink(monument));
                  ...
                  alert(
                    mw
                      .message(
                        ...
                        monument.monument_article.replace(/_/g, " ")
                      )
                      .plain()
                  );
  ...
  function getPosition(callback, errback) {
    ...
    navigator.geolocation.getCurrentPosition(
      function (pos) {
        ...
        if (getCurrentPage() === "locationlookup-page") {
          // check user didn't escape page
          showPage(page);
          callback(pos);
        }
      },
      function (err) {
        if (getCurrentPage() === "locationlookup-page") {
          // check user didn't escape page
          displayError(
            mw.msg("geolocating-failed-heading"),
            mw.msg("geolocating-failed-text")
          );
        }
        if (errback) {
          errback();
  ...
  function showMonumentsList(monuments) {
    ...
    function infiniteScroll() {
      ...
      if (footer && monuments.next) {
        ...
        if (infiniteScrollAjax) {
          ...
          infiniteScrollAjax = monuments
            .next()
            .done(function (newMonuments) {
    ...
    $("#monuments-sort")
      .html(templates.getTemplate("monument-list-heading")())
      .localize();

    function sort() {
      window.setTimeout(function () {
        // use timeout for smoother experience
        showMonumentsList($("#results").data("monuments"));
      }, 0);
    }

    $("#monuments-sort button")
      .click(function () {
        $("#results").empty();
    ...
    if (userLocation) {
      ...
      $.each(monuments, function () {
        this.distance = calculateDistance(userLocation.coords, {
          latitude: this.lat,
          longitude: this.lon,
    ...
    $.each(monuments.sort(sortAlgorithm), function (i, monument) {
      ...
      if (monument.distance) {
        msg =
          monument.distance > 200
            ? mw.msg("monument-distance-tofar")
            : mw.msg("monument-distance-km", this.distance);
        $('<div class="distance" />').text(msg).appendTo($("a", $monumentItem));
      }
  ...
  function initMap() {
    ...
    function ping(ev) {
      ...
      searchTimeout = window.setTimeout(function () {
        lastRequest = monuments
          .getInBoundingBox(nw.lng, se.lat, se.lng, nw.lat)
  ...
  function uploadErrorHandler(data) {
    ...
    switch (code) {
      ...
      case "verification-error":
        if (
          data.error.details &&
          data.error.details[0] === "filetype-mime-mismatch"
        ) {
  ...
  function showPhotoConfirmation(fileUrl) {
    ...
    var uploadConfirmTemplate = templates.getTemplate(
      "upload-confirm-template"
    );
    ...
    $("#upload-confirm")
      .html(uploadConfirmTemplate({ monument: curMonument, fileUrl: fileUrl }))
      .localize();
    ...
    function continueUpload() {
      ...
      photo
        .uploadTo(api, comment, PHOTO_TEMPLATE)
        .done(function (imageinfo) {
          $("#upload-latest-page img").attr(
            "src",
            resolveImageThumbnail(imageinfo.url)
          );
          $("#upload-latest-page .share a").attr(
            "href",
      ...
      api
        ...
        .done(function (status) {
          ...
          if (status === "Success" && noPageChange) {
            ...
            $("#settings-user-name").html(
              mw.msg("settings-user-name", username)
            );
            ...
            switch (status) {
              ...
              case "Blocked": // User is blocked
                errMsg = mw.msg(
                  ...
                  WLMConfig.BLOCKING_POLICY.replace("$1", lang)
                );
                break;
  ...
  function translateLevelsForMonument(monument) {
    ...
    return admintree
      .getLeaves(levels, CURRENT_LANGUAGE, /* translate */ true)
      .pipe(function (tree) {
        var names = [];
  ...
  function listCampaigns(campaignTree) {
    ...
    function listMonuments(tree) {
      ...
      var d = monuments.getForAdminLevel(tree).done(function (monuments) {
        showMonumentsList(monuments);
      });
    ...
    d.done(function (campaigns) {
      if (getCurrentPage() !== pageName) {
        // the user pressed back so abort any further changes
        ...
        var countriesListTemplate = templates.getTemplate(
          "country-list-template"
        );
        $clist.empty().html(countriesListTemplate({ campaigns: campaigns }));
  ...
  function init() {
    $("#version").text(
      WLMConfig.VERSION_NUMBER + " (" + WLMConfig.VERSION_NAME + ")"
    );
    var timeout,
      name,
      lang = prefs.get("uiLanguage");
      ...
      navigator.camera.getPicture(
        ...
        function (msg) {
          ...
          if (msg == "Cannot read this image type.") {
            ...
            displayError(
              mw.msg("cannot-load-photo"),
              mw.msg("cannot-load-photo-text")
            );
          }
          // Do nothing.
        },
    ...
    $("#login-create-account-msg a").attr(
      "href",
      WLMConfig.SIGNUP_PAGE.replace("$1", lang)
    );
