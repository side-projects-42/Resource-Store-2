describe("Chart.helpers.config", function () {
  ...
  describe("_createResolver", function () {
    ...
    it("should support common object methods", function () {
      ...
      expect(
        Object.prototype.hasOwnProperty.call(resolver, "option2")
      ).toBeTrue();

      expect(
        Object.prototype.hasOwnProperty.call(resolver, "option1")
      ).toBeFalse();
      expect(
        Object.prototype.hasOwnProperty.call(resolver, "getter")
      ).toBeFalse();
      expect(
        Object.prototype.hasOwnProperty.call(resolver, "nonexistent")
      ).toBeFalse();

      expect(Object.keys(resolver)).toEqual(["option2"]);
      expect(Object.getOwnPropertyNames(resolver)).toEqual([
        "option2",
        "option1",
      ]);
    ...
    describe("_fallback", function () {
      ...
      it("should support _fallback as function", function () {
        const descriptors = {
          _fallback: (prop, value) =>
            prop === "hover" && value.shouldFall && "interaction",
        };
        const defaults = {
          interaction: {
            mode: "test",
      ...
      it("should fallback through multiple routes (animations)", function () {
        ...
        const show = _createResolver([
          options,
          defaults.transitions.show,
          defaults,
          descriptors,
        ]);
        ...
        expect(
          Object.getOwnPropertyNames(show.animations).filter((k) =>
            Chart.helpers.isObject(show.animations[k])
          )
        ).toEqual(["colors", "duration", "numbers"]);
        ...
        expect(
          Object.getOwnPropertyNames(def.animations).filter((k) =>
            Chart.helpers.isObject(show.animations[k])
          )
        ).toEqual(["colors", "duration", "numbers"]);
      });
  ...
  describe("_attachContext", function () {
    ...
    it("same resolver should be usable with multiple contexts", function () {
      ...
      const options = {
        animation: (ctx) =>
          ctx.index === 0 ? { duration: 1000 } : { duration: 500 },
      };
      const resolver = _createResolver([options, defaults]);
    ...
    it("should fall back from object returned from scriptable option", function () {
      ...
      const opts = _attachContext(_createResolver([options, defaults]), {
        contextValue: "test",
      });
    ...
    it("should support overriding options", function () {
      ...
      const opts = _attachContext(_createResolver([options]), {
        index: 2,
        type: "test",
      });
      expect(opts).toEqualOptions({
    ...
    it("should support changing context", function () {
      const opts = _attachContext(
        _createResolver([{ fn: (ctx) => ctx.test }]),
        { test: 1 }
      );
    ...
    it("should support common object methods", function () {
      ...
      expect(
        Object.prototype.hasOwnProperty.call(opts, "nonexistent")
      ).toBeFalse();
    ...
    it("should properly set value to object in array of objects", function () {
      ...
      const resolver = _attachContext(_createResolver([options, defaults]), {
        test: true,
      });
