...
describe("Test the radial linear scale", function () {
  ...
  it("Should use the min and max options", function () {
    ...
    expect(getLabels(chart.scales.r)).toEqual([
      "-1,010",
      "-500",
      "0",
      "500",
  ...
  it("Should build labels using the user supplied callback", function () {
    ...
    expect(chart.scales.r._pointLabels).toEqual([
      "label1",
      "label2",
      "label3",
      "label4",
  ...
  it("should get the correct distance from the center point", function () {
    ...
    expect(
      chart.scales.r.getDistanceFromCenterForValue(chart.scales.r.min)
    ).toBe(0);
    expect(
      chart.scales.r.getDistanceFromCenterForValue(chart.scales.r.max)
    ).toBe(227);
  ...
  it("should get the correct value for a distance from the center point", function () {
    ...
    expect(chart.scales.r.getValueForDistanceFromCenter(0)).toBe(
      chart.scales.r.min
    );
    expect(chart.scales.r.getValueForDistanceFromCenter(227)).toBe(
      chart.scales.r.max
    );
  ...
  it("should correctly get angles for all points", function () {
    ...
    for (var i = 0; i < 5; i++) {
      expect(radToNearestDegree(chart.scales.r.getIndexAngle(i))).toBe(
        15 + slice * i
      );
    }
    ...
    for (var x = 0; x < 5; x++) {
      expect(radToNearestDegree(chart.scales.r.getIndexAngle(x))).toBe(
        slice * x
      );
    }
  });
    ...
    [
      ...
      scale.ctx
        ...
        .forEach(function (x, i) {
          expect(x.args[0])
            .withContext("startAngle: " + expected.startAngle + ", tick: " + i)
            .toBe(expected.textAlign[i]);
        });
