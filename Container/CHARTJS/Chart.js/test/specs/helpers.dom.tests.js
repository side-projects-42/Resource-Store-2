describe("DOM helpers tests", function () {
  ...
  it("should get the maximum size for a node", function () {
    ...
    expect(helpers.getMaximumSize(innerDiv)).toEqual(
      jasmine.objectContaining({ width: 200, height: 300 })
    );
  ...
  it("should get the maximum width of a node that has a max-width style", function () {
    ...
    expect(helpers.getMaximumSize(innerDiv)).toEqual(
      jasmine.objectContaining({ width: 150 })
    );
  ...
  it("should get the maximum height of a node that has a max-height style", function () {
    ...
    expect(helpers.getMaximumSize(innerDiv)).toEqual(
      jasmine.objectContaining({ height: 150 })
    );
  ...
  it("Should get padding of parent as number (pixels) when defined as percent (returns incorrectly in IE11)", function () {
    ...
    // No padding
    expect(helpers.getMaximumSize(canvas)).toEqual(
      jasmine.objectContaining({ width: 300 })
    );

    // test with percentage
    innerDiv.style.padding = "5%";
    expect(helpers.getMaximumSize(canvas)).toEqual(
      jasmine.objectContaining({ width: 270 })
    );
  ...
  describe("getRelativePosition", function () {
      ...
      const chart4 = window.acquireChart(
        {},
        {
          canvas: {
            ...
            width: 200,
            style:
              "width: 400px, height: 400px; padding: 10px; position: absolute; left: 20, top: 20",
          },
        }
      );
    ...
    it("should calculate from clientX/Y as fallback", function () {
      ...
      expect(
        Math.abs(pos.x - Math.round(event.clientX - rect.x))
      ).toBeLessThanOrEqual(1);
      expect(
        Math.abs(pos.y - Math.round(event.clientY - rect.y))
      ).toBeLessThanOrEqual(1);
      ...
      expect(
        Math.abs(
          pos2.x - Math.round(((event.clientX - rect2.x - 10) / 180) * 200)
        )
      ).toBeLessThanOrEqual(1);
      expect(
        Math.abs(
          pos2.y - Math.round(((event.clientY - rect2.y - 10) / 180) * 200)
        )
      ).toBeLessThanOrEqual(1);
      ...
      expect(
        Math.abs(
          pos3.x - Math.round(((event.clientX - rect3.x - 10) / 360) * 400)
        )
      ).toBeLessThanOrEqual(1);
      expect(
        Math.abs(
          pos3.y - Math.round(((event.clientY - rect3.y - 10) / 360) * 400)
        )
      ).toBeLessThanOrEqual(1);
    });
