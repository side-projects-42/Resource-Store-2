...
describe("Chart.helpers.core", function () {
  ...
  describe("each", function () {
    ...
    it("should iterate over object properties", function () {
      ...
      expect(items).toEqual(
        jasmine.objectContaining({ a: "foo", b: "bar", c: 42 })
      );
    });
  ...
  describe("_elementsEqual", function () {
    ...
    it("should return false if arrays are not the same", function () {
      expect(
        helpers._elementsEqual([], [{ datasetIndex: 0, index: 1 }])
      ).toBeFalsy();
      expect(
        helpers._elementsEqual(
          [{ datasetIndex: 0, index: 2 }],
  ...
  describe("merge", function () {
    ...
    it("should recursively overwrite target with source properties", function () {
      expect(helpers.merge({ a: { b: 1 } }, { a: { c: 2 } })).toEqual({
        a: { b: 1, c: 2 },
      });
      expect(helpers.merge({ a: { b: 1 } }, { a: { b: 2 } })).toEqual({
        a: { b: 2 },
      });
      expect(helpers.merge({ a: [1, 2] }, { a: [3, 4] })).toEqual({
        a: [3, 4],
      });
      expect(helpers.merge({ a: 42 }, { a: { b: 0 } })).toEqual({
        a: { b: 0 },
      });
      expect(helpers.merge({ a: 42 }, { a: null })).toEqual({ a: null });
      expect(helpers.merge({ a: 42 }, { a: undefined })).toEqual({
        a: undefined,
      });
    });
    it("should merge multiple sources in the correct order", function () {
      ...
      expect(helpers.merge(t0, [s0, s1, s2])).toEqual({
        a: { b: 5, c: [6, 7], d: 3 },
        e: "foo",
      });
    });
  ...
  describe("mergeIf", function () {
    ...
    it("should return target if not an object", function () {
      ...
      expect(helpers.mergeIf(["foo", "bar"], { a: 42 })).toEqual([
        "foo",
        "bar",
      ]);
    });
    ...
    it("should recursively copy source properties in target only if they do not exist in target", function () {
      expect(helpers.mergeIf({ a: { b: 1 } }, { a: { c: 2 } })).toEqual({
        a: { b: 1, c: 2 },
      });
      expect(helpers.mergeIf({ a: { b: 1 } }, { a: { b: 2 } })).toEqual({
        a: { b: 1 },
      });
      expect(helpers.mergeIf({ a: [1, 2] }, { a: [3, 4] })).toEqual({
        a: [1, 2],
      });
      expect(helpers.mergeIf({ a: 0 }, { a: { b: 2 } })).toEqual({ a: 0 });
      expect(helpers.mergeIf({ a: null }, { a: 42 })).toEqual({ a: null });
      expect(helpers.mergeIf({ a: undefined }, { a: 42 })).toEqual({
        a: undefined,
      });
    });
    it("should merge multiple sources in the correct order", function () {
  ...
  describe("resolveObjectKey", function () {
    ...
    it("should resolve multiple levels", function () {
      ...
      expect(helpers.resolveObjectKey(obj, "child.child.child.level")).toEqual(
        3
      );
    });
    it("should resolve circular reference", function () {
      ...
      expect(
        helpers.resolveObjectKey(root, "child.child.child.child.child.child")
      ).toEqual(child);
