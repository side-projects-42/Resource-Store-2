...
describe("Chart.helpers.canvas", function () {
  ...
  describe("isPointInArea", function () {
    it("should determine if a point is in the area", function () {
      ...
      expect(isPointInArea({ x: 512 + 1e-12, y: 256 + 1e-12 }, area)).toBe(
        true
      );
  ...
  it("should return the width of the longest text in an Array and 2D Array", function () {
    ...
    expect(helpers._longestText(context, font, arrayOfThings1D, {})).toEqual(
      60
    );
    expect(helpers._longestText(context, font, arrayOfThings2D, {})).toEqual(
      80
    );
  ...
  it("compare text with current longest and update", function () {
    ...
    expect(helpers._measureText(context, data, gc, longest, "foobar")).toEqual(
      70
    );
    expect(helpers._measureText(context, data, gc, longest, "foobar_")).toEqual(
      70
    );
    expect(
      helpers._measureText(context, data, gc, longest, "foobar_1")
    ).toEqual(80);
    // We check to make sure we made the right calls to the canvas.
    expect(context.getCalls()).toEqual([
      {
  ...
  describe("renderText", function () {
    ...
    it("should underline the text", function () {
      var context = window.createMockContext();
      var font = { string: "12px arial", lineHeight: 20 };
      helpers.renderText(context, "foo", 0, 0, font, {
        decorationWidth: 3,
        underline: true,
      });
