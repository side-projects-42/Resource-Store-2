...
describe("utils.js", function () {
  describe("toTextLines", function () {
    ...
    it("should flatten children arrays in the correct order", function () {
      expect(toTextLines(["foo", [["bar", "xxx"], "bla"]])).toEqual([
        "foo",
        "bar",
        "xxx",
        "bla",
  ...
  describe("toFontString", function () {
    ...
    it("should return null if size or family are missing", function () {
      expect(
        toFontString({ style: "italic", weight: 300, size: 12 })
      ).toBeNull();
      expect(
        toFontString({ style: "italic", weight: 300, family: "serif" })
      ).toBeNull();
    });
    it("should return the string representation of the given font", function () {
      ...
      ).toBe("italic 300 12px serif");
    });
    it("weigth and style should be optional", function () {
      expect(toFontString({ size: 12, family: "serif" })).toBe("12px serif");
      expect(toFontString({ style: "italic", size: 12, family: "serif" })).toBe(
        "italic 12px serif"
      );
      expect(toFontString({ weight: 300, size: 12, family: "serif" })).toBe(
        "300 12px serif"
      );
    });
  });
  ...
  describe("arrayDiff", function () {
    ...
    it("should return an array of [value, state] with proper state", function () {
      ...
      expect(arrayDiff(a0, a1)).toEqual([
        [11, 1],
        [51, -1],
        [22, -1],
      ]);
      expect(arrayDiff(a1, a0)).toEqual([
        [51, 1],
        [22, 1],
        [11, -1],
      ]);
      expect(arrayDiff(a0, [])).toEqual([
        [42, -1],
        [51, -1],
        [22, -1],
      ]);
