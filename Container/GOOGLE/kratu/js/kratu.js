...
Kratu.prototype.getCalculations_ = function () {
  ...
  var createRankingCalculation = function (direction) {
    return function (entity) {
      ...
      if (!kratu.calculationsRankings_) {
        kratu.calculationsRankings_ = {};
      }
...
Kratu.prototype.render_ = function (options) {
  ...
  if (kratu.newEntities_) {
    // If we haven't seen this data before; Calculate
...
Kratu.prototype.renderReportHeadings_ = function () {
  ...
  for (var i = 0; i < signals.length; i++) {
    ...
    if (signal.headerEventHandlers) {
      kratu.addEventHandlers(signalHeader, signal.headerEventHandlers, {
        signal: signal,
      });
    }
...
Kratu.prototype.calculateEntities_ = function () {
  ...
  for (var eN = 0; eN < entities.length; eN++) {
    ...
    for (var scN = 0; scN < summaryCallbacks.length; scN++) {
      ...
      calculatedEntity[callback.position][callbackTypeName] = callback.callback(
        { sumScore: sumScore, sumWeights: sumWeights }
      );
    }
...
Kratu.prototype.calculateSummaries_ = function () {
  ...
  if (
    ...
    for (i = 0; i < reportDefinition.summaryRows.length; i++) {
      ...
      var className =
        "kratu" + rowDef.type.charAt(0).toUpperCase() + rowDef.type.slice(1);
...
Kratu.prototype.addEventHandlers = function (elm, handlers, args) {
  ...
  for (var eventType in handlers) {
    try {
      elm.addEventListener(eventType, createEventTypeHandler(eventType), false);
    } catch (err) {
...
Kratu.prototype.getReportAs = function (type) {
  ...
  if (type === "html") {
    if (!kratu.getHasRendered()) {
      // TODO Consider doing an anonymous render
...
Kratu.prototype.loadScript = function (url, opt_callback) {
  ...
  url +=
    (url.match(/\?/) ? "&" : "?") + "__kratuTimeStamp=" + new Date().getTime();
...
KratuSignal.prototype.calculateWeight = function (entity, opt_simulation) {
  ...
  if (opt_simulation) {
    ...
    try {
      ...
      throw (
        "Warning! Could not call getData on signal " +
        kratuSignal.key +
        " " +
        err
      );
  ...
  var calculatedWeight =
    ((kratuSignal.weight / 100) * Math.pow(delta, kratuSignal.scaleExponent)) /
