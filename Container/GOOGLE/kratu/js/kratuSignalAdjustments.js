...
KratuSignalAdjustments.prototype.updateMeasures_ = function (signal) {
  ...
  for (var i = signal.range.min; i <= signal.range.max; i++) {
    var annotation = "";
    if (signal.lMax && i == signal.lMax) {
      annotation = "lMax " + signal.lMax;
    }
    if (signal.lMin && i == signal.lMin) {
      annotation = "lMin " + signal.lMin;
    }
    if (signal.hMin && i == signal.hMin) {
      annotation = "hMin " + signal.hMin;
    }
    if (signal.hMax && i == signal.hMax) {
      annotation = "hMax " + signal.hMax;
    }
...
KratuSignalAdjustments.prototype.createSliderObject_ = function (args) {
  ...
  value.textContent = parseInt(args.signal[args.signalKey], 10).toFixed(
    numDecimals
  );
