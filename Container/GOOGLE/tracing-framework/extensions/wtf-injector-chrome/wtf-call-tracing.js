...
(function (global) {
  ...
  function fixupModuleIds(moduleId, unreplacedText) {
    var replacedText = unreplacedText.replace(moduleIdRegex, moduleId << 24);
    return replacedText;
  }
  ...
  function processOrCache(sourceText, moduleId, options, opt_url) {
    ...
    resultText = global["wtfi"]["process"](sourceText, options, opt_url);
    if (ENABLE_CACHING) {
  ...
  function processScript(el, opt_synchronous, opt_baseUrl) {
    ...
    if (el.text || el.innerText) {
      ...
      showActivityIndicator(doc);
      var src = opt_baseUrl
        ? resolveUrl(opt_baseUrl, el.getAttribute("src"))
        : el.src;
  ...
  function injectDocumentWrite(targetWindow, doc) {
    ...
    function processMarkup(markup) {
      ...
      var baseUrl = arguments.callee.caller.caller
        ? arguments.callee.caller.caller["__src"]
        : null;
      if (!baseUrl) {
  ...
  function runSharedInitCode(targetWindow, options) {
    ...
    if (firstBlock) {
      ...
      if (global["wtf"]) {
        ...
        window.alert(
          "The WTF tracing feature cannot be used on " +
    ...
    targetWindow.__wtfd =
      topWindow.__wtfd || new Int32Array(1 << dataMagnitude);
    ...
    targetWindow.__grabTrace = function () {
      ...
      var etitle = window.document.title
        .replace(/\\/g, "\\\\")
        .replace(/\"/g, '\\"');
      var attributes = "[]";
      switch (instrumentationType) {
      ...
      var headerText =
        ...
        "}," +
        '"modules": ' +
        JSON.stringify(targetWindow.__wtfm) +
        "}";
      ...
      return [header, padBytes, contents];
    };
