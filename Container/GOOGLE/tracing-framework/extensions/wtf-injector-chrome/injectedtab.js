...
var InjectedTab = function (extension, tab, pageStatus, pageOptions, port) {
  ...
  if (
    ...
    var tracingEnabled = pageOptions["wtf.trace.provider.chromeDebug.tracing"];
  ...
  if (this.debugger_) {
    this.debuggerTransmitId_ = window.setInterval(
      function () {
...
InjectedTab.prototype.messageReceived_ = function (data, port) {
  ...
  switch (data["command"]) {
    ...
    case "save_settings":
      ...
      options.setPageOptions(this.pageUrl_, JSON.parse(data["content"]));
      if (data["reload"]) {
        this.extension_.reloadTab(this.tabId_, tab.url);
      }
      break;
 ...
 * @private
 */
InjectedTab.prototype.stopChromeTracing_ = function (
  trackerId,
  includeThreads
) {
  var tracer = this.extension_.getTracer();
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      for (var j = 0; j < innerList.length; j++) {
        ...
        switch (e.ph) {
          ...
          case "E":
            filteredData.push([1, thread.zoneId, ts]);
            break;
          case "I":
            filteredData.push([2, thread.zoneId, ts, e.name]);
            break;
          default:
