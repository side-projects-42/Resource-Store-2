...
function prepareDebug() {
  ...
  require("../src/wtf/bootstrap/node").importClosureLibrary(["wtf_js-deps.js"]);
...
exports.util.logEvent = function (it, zone) {
  ...
  var line = exports.util.spaceValues(
    logEventSpacing,
    wtf.util.formatSmallTime(it.getTime()),
    "[",
    zone.getName(),
    "]",
    it.getName(),
...
exports.util.logContextInfo = function (contextInfo) {
  ...
  console.log(exports.util.spaceValues(lineSpacing, "URI:", contextInfo.uri));
  if (contextInfo.title) {
    console.log(
      exports.util.spaceValues(lineSpacing, "Title:", contextInfo.title)
    );
  }
  if (contextInfo.icon && contextInfo.icon.uri) {
    console.log(
      exports.util.spaceValues(lineSpacing, "Icon:", contextInfo.icon.uri)
    );
  }
  if (contextInfo.taskId) {
    console.log(
      exports.util.spaceValues(lineSpacing, "Task ID:", contextInfo.taskId)
    );
  }
  if (contextInfo.args.length) {
    console.log(
      exports.util.spaceValues(lineSpacing, "Args:", contextInfo.args)
    );
  }
  console.log(
    exports.util.spaceValues(lineSpacing, "UA:", contextInfo.userAgent.value)
  );
  console.log(
    exports.util.spaceValues(
      lineSpacing,
      " ",
      "Device:",
      contextInfo.userAgent.device
    )
  );
  console.log(
    exports.util.spaceValues(
