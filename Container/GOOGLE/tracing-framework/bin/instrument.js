...
function transformCode(moduleId, url, sourceCode, argv) {
  ...
  var sharedInitCode =
    "(" +
    function (global, instrumentationType) {
      ...
      switch (instrumentationType) {
        ...
        case "memory":
          ...
          try {
            ...
            window.alert(
              "Launch Chrome with --js-flags=--allow-natives-syntax"
            );
          }
          global.__wtfEnter = function (id) {
            __wtfd[__wtfi++ & dataMask] = id;
...
function processFile(argv, inputPath, opt_outputPath) {
  ...
  if (!opt_outputPath) {
    ...
    if (ext.length) {
      outputPath =
        inputPath.substr(0, inputPath.length - ext.length) +
        ".instrumented" +
        ext;
    } else {
...
function startServer(argv, httpPort, httpsPort, certs) {
  ...
  function handler(req, res) {
    ...
    targetModule
      .get(targetUrl, function (originalRes) {
...
function getHttpsCerts(callback) {
  ...
  function readCerts() {
    var privateKey = fs
      .readFileSync(path.join(certPath, "privatekey.pem"))
      .toString();
    var certificate = fs
      .readFileSync(path.join(certPath, "certificate.pem"))
      .toString();
    callback({
