...
wtf.trace.EventTypeBuilder.prototype.generate = function (context, eventType) {
  ...
  for (var n = 0; n < args.length; n++) {
    ...
    // Track minimum size.
    if (writer.size) {
      minSize += writer.size < 4 ? writer.size + (4 - (writer.size % 4)) : 4;
    }
...
wtf.trace.EventTypeBuilder.WRITE_BOOL_ = {
  ...
  write: function (a, offset) {
    return ["int8Array[(" + offset + ") << 2] = " + a + " ? 1 : 0;"];
  },
};
...
wtf.trace.EventTypeBuilder.WRITE_CHAR_ = {
  ...
  computeSize: null,
  write: function (a, offset) {
    return ["int8Array[(" + offset + ") << 2] = " + a + ".charCodeAt(0);"];
  },
};
...
wtf.trace.EventTypeBuilder.WRITE_WCHAR_ = {
  ...
  computeSize: null,
  write: function (a, offset) {
    return ["int16Array[(" + offset + ") << 1] = " + a + ".charCodeAt(0);"];
  },
};
  ...
  write: function (a, offset) {
    return [
      "int32Array[" +
        ...
        a +
        " !== null ? " +
        "stringTable.addString(stringify(" +
        a +
        ")) : -1;",
    ];
  },
...
wtf.trace.EventTypeBuilder.WRITE_FLOWID_ = {
  ...
  computeSize: null,
  write: function (a, offset) {
    return ["int32Array[" + offset + "] = " + a + " ? " + a + ".getId() : 0;"];
  },
};
...
wtf.trace.EventTypeBuilder.WRITE_TIME32_ = {
  ...
  computeSize: null,
  write: function (a, offset) {
    return ["int32Array[" + offset + "] = " + a + " * 1000;"];
  },
};
