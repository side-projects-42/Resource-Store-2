...
wtf.hud.Overlay = function (session, options, opt_parentElement) {
   ...
   * @private
   */
  this.extensionChannel_ = options.getOptionalBoolean("wtf.injector", false)
    ? wtf.ipc.getWindowMessageChannel(window)
    : null;
  ...
  this.liveGraph_ = new wtf.hud.LiveGraph(
    session,
    options,
    this.getChildElement(goog.getCssName("hudGraph"))
  );
  ...
  this.addButton_(
    ...
    "Clear current data",
    "shift+esc",
    this.clearSnapshotClicked_,
    this
  );
  this.addButton_(
    true,
    "/assets/icons/popout_white.png",
    "Send to UI (middle click for new tab)",
    "f9",
    this.sendSnapshotClicked_,
    this
  );
  this.addButton_(
    true,
...
wtf.hud.Overlay.prototype.reloadOptions_ = function (opt_changedKeys) {
  ...
  var safeReloadKeys = ["wtf.hud.dock"];
...
wtf.hud.Overlay.prototype.sendSnapshotToPage_ = function (
  ...
  var targetIsExtension = goog.string.startsWith(
    endpoint,
    "chrome-extension://"
  );
