 ...
 * @override
 */
wtf.app.nav.TimelinePainter.prototype.repaintInternal = function (ctx, bounds) {
    ...
    function (frame) {
      ...
      if (endTime > pixelStart + pixelStep) {
        ...
        if (gapSize > 33) {
          ...
          ctx.moveTo(
            bounds.left +
              wtf.math.remap(pixelStart, timeLeft, timeRight, 0, bounds.width),
  ...
  ctx.fillRect(
    bounds.left,
    bounds.top + Math.floor(bounds.height - 17 * timeScale),
    bounds.width,
    1
  );
  ctx.fillRect(
    bounds.left,
    bounds.top + Math.floor(bounds.height - 33 * timeScale),
    bounds.width,
    1
  );
