...
wtf.app.query.QueryPanel = function (documentView) {
   ...
   * @private
   */
  this.buttonEls_ = [this.getChildElement(goog.getCssName("saveCsvResults"))];

  var commandManager = wtf.events.getCommandManager();
  commandManager.registerSimpleCommand(
    "query",
    function (source, target, expression) {
...
wtf.app.query.QueryPanel.prototype.createDom = function (dom) {
  return /** @type {!Element} */ (
    goog.soy.renderAsFragment(
      wtf.app.query.querypanel.control,
      undefined,
      undefined,
      dom
    )
...
wtf.app.query.QueryPanel.prototype.issueQuery_ = function (expression) {
  ...
  dom.setTextContent(
    this.infoEl_,
    count + " hits in " + wtf.util.formatSmallTime(query.getDuration())
  );
