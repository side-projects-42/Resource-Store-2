 ...
 * @override
 */
wtf.app.FramePainter.prototype.layoutInternal = function (availableBounds) {
 ...
 * @override
 */
wtf.app.FramePainter.prototype.repaintInternal = function (ctx, bounds) {
  ...
  this.frameList_.forEachIntersecting(
    timeLeft,
    timeRight,
    function (frame) {
 ...
 * @override
 */
wtf.app.FramePainter.prototype.onClickInternal = function (
  x,
  y,
  modifiers,
  bounds
  ...
  if (goog.isArray(hit)) {
    ...
    var timeLeft = frameLeft
      ? frameLeft.getEndTime()
      : this.db_.getFirstEventTime();
    var timeRight = frameRight
      ? frameRight.getTime()
      : this.db_.getLastEventTime();
 ...
 * @override
 */
wtf.app.FramePainter.prototype.getInfoStringInternal = function (x, y, bounds) {
  var hit = this.hitTest_(x, y, bounds);
  if (!hit) {
    return undefined;
  }
  ...
  if (goog.isArray(hit)) {
    ...
    var frame = hit;
    lines.push(
      wtf.util.formatTime(frame.getDuration()) + ": frame #" + frame.getNumber()
    );
  }
  return lines.join("\n");
};
 ...
 * @private
 */
wtf.app.FramePainter.prototype.hitTest_ = function (x, y, bounds) {
