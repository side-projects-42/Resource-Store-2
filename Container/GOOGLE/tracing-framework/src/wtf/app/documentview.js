...
wtf.app.DocumentView = function (parentElement, dom, doc) {
   ...
   * @private
   */
  this.toolbar_ = new wtf.app.Toolbar(
    this,
    this.getChildElement(goog.getCssName("appUiDocumentViewToolbar"))
  );
   ...
   * @private
   */
  this.navbar_ = new wtf.app.nav.Navbar(
    this,
   ...
   * @private
   */
  this.tabbar_ = new wtf.app.Tabbar(
    this,
    this.getChildElement(goog.getCssName("appUiDocumentViewInner"))
  );
   ...
   * @private
   */
  this.statusbar_ = new wtf.app.Statusbar(
    this,
    this.getChildElement(goog.getCssName("appUiDocumentViewStatusbar"))
  );
  ...
  this.tabbar_.addPanel(new wtf.app.EmptyTabPanel(this, "console", "Console"));
...
wtf.app.DocumentView.prototype.setupCommands_ = function () {
  ...
  commandManager.registerSimpleCommand(
    "save_local_trace",
    this.saveLocalTrace_,
    this
  );
  commandManager.registerSimpleCommand(
    "save_drive_trace",
    this.saveDriveTrace_,
    this
  );
  ...
  commandManager.registerSimpleCommand(
    ...
    function () {
      ...
      new wtf.app.HealthDialog(db, this.healthInfo_, body, this.getDom());
    },
    this
  );
  ...
  commandManager.registerSimpleCommand(
    "select_range",
    function (source, target, startTime, endTime) {
  ...
  commandManager.registerSimpleCommand(
    "goto_range",
    function (source, target, timeStart, timeEnd, opt_immediate) {
      var pad = (timeEnd - timeStart) * 0.05;
      view.setVisibleRange(timeStart - pad, timeEnd + pad, opt_immediate);
  ...
  commandManager.registerSimpleCommand(
    "goto_mark",
    function (source, target, mark, opt_immediate) {
      // Go to mark event.
  ...
  commandManager.registerSimpleCommand(
    "goto_frame",
    function (source, target, frameOrNumber, opt_immediate) {
      var frame = null;
...
wtf.app.DocumentView.prototype.layoutInternal = function () {
  ...
  this.navbar_.setSplitterLimits(
    navbarMinHeight,
    Math.min(navbarMaxHeight, currentSize.height)
  );
...
wtf.app.DocumentView.prototype.saveLocalTrace_ = function () {
  ...
  function sendXhrDownload(filename, contentType, xhr) {
    ...
    xhr.onload = function () {
      platform.writeBinaryFile(filename, xhr.response, contentType);
    };
