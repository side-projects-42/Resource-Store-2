 ...
 */
wtf.replay.graphics.Program_test = suite(
  "wtf.replay.graphics.Program",
  function () {
    ...
    setup(function () {
      ...
      assert.isTrue(
        context.getShaderParameter(testVertexShader, goog.webgl.COMPILE_STATUS)
      );
      ...
      assert.isTrue(
        context.getProgramParameter(testProgram, goog.webgl.LINK_STATUS)
      );
    });
    ...
    test("#createVariantProgram", function () {
      ...
      var customVertexSource =
        ...
        "  gl_Position = uPMatrix * uMVMatrix * vec4(aVertexPosition, 1.0);" +
        "}";
      var customFragmentSource =
        "precision mediump float;" +
        "uniform vec4 customColor;" +
        "void main(void) { " +
        "gl_FragColor = customColor;" +
        "}";
      program.createVariantProgram(
        "custom",
        customVertexSource,
      ...
      assert.isTrue(
        context.getProgramParameter(
          customVariantProgram,
          goog.webgl.LINK_STATUS
        )
      );

      // Confirm that the uniform 'customColor' is present.
      var uniformLocation = context.getUniformLocation(
        customVariantProgram,
        "customColor"
      );
    ...
    test("#drawWithVariant", function () {
      ...
      var uniformValue = context.getUniform(
        customVariantProgram,
