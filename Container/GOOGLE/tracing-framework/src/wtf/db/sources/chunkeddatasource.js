...
wtf.db.sources.ChunkedDataSource.prototype.chunkReceived_ = function (chunk) {
  ...
  switch (chunk.getType()) {
    ...
    case wtf.io.cff.ChunkType.EVENT_DATA:
      var eventDataChunk = /** @type {!wtf.io.cff.chunks.EventDataChunk} */ (
        chunk
      );
 ...
 * @private
 */
wtf.db.sources.ChunkedDataSource.prototype.processFileHeaderChunk_ = function (
  chunk
) {
...
wtf.db.sources.ChunkedDataSource.prototype.setupBinaryDispatchTable_ =
  function () {
    ...
    this.binaryDispatch_["wtf.event#define"] = function (eventType, args) {
      ...
      var newEventType = eventTypeTable.defineType(
        new wtf.db.EventType(
          args["name"],
          args["eventClass"],
          args["flags"],
          argList
    ...
    this.binaryDispatch_["wtf.timeRange#begin"] = this.binaryDispatch_[
      "wtf.timeRange#end"
 ...
 * @private
 */
wtf.db.sources.ChunkedDataSource.prototype.processJsonEventBuffer_ = function (
  part
) {
  // TODO(benvanik): get resources.
