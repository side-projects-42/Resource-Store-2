...
var ChannelPlate = (function channelPlateModule() {
  ...
  Base.prototype = {
    ...
    set onmessage(onMessage) {
      ...
      if (this.port.onMessage) {
        ...
        // W3c, implicitly calls start()
        this.port.onmessage = repackage;
      }
      if (this.port.plate !== this) console.error("mismatched plates");
    },
  ...
  function Talker(eventWindow, onMessage) {
    ...
    eventWindow.postMessage(
      ["ChannelPlate", window.location.href],
      this.targetOrigin,
      [this.channel.port2]
    );
  ...
  function Listener(clientWebOriginOrURL, onConnect) {
    ...
    var targetOrigin =
      getWebOrigin(clientWebOriginOrURL) ||
      getWebOrigin(window.location.toString());

    function onChannelPlate(event) {
      if (DEBUG) {
        console.log(
          "ChannelPlate.Listener.onChannelPlate for targetOrigin " +
            targetOrigin
        );
      }
      ...
      if (DEBUG) {
        console.log(
          "ChannelPlate.Listener.onChannelPlate CONNECT " + window.location.href
        );
      }
    }
    ...
    if (DEBUG) {
      console.log(
        "ChannelPlate.Listener start listening for ChannelPlate connect in " +
  ...
  function Parent(existingChildIframe, srcURLToAssign, onConnect) {
    function filterOnConnect(rawPort, url) {
      if (url.indexOf(srcURLToAssign) !== -1) onConnect(rawPort, url);
    }
  ...
  function DevtoolsTalker(onMessage) {
    var name = encodeURIComponent(window.location.href).replace(
      /[!'()*]/g,
      "_"
    );
