...
function getChromeExtensionPipe() {
  ...
  var appEnd = {
    ...
    attach: function (callback, errback) {
      if (!chrome || !chrome.extension) {
        throw new Error(
          "Must be loaded into an iframe using a chrome extension url"
        );
      }
      var request = {
      ...
      chrome.extension.sendRequest(
        request,
        this.onAttach.bind(this, callback, errback)
      );
    },
    ...
    onAttach: function (callback, errback, response) {
      ...
      if (!response.name) {
        console.error(
          "crx2App the extension must send .name in response",
          response
        );
        return;
      }
  ...
  return {
    // these functions are bound to appEnd, not the return object
