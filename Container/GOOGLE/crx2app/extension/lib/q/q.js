...
*/ // browsers
  ...
  var shim = function (object, name, shim) {
    if (!object[name]) object[name] = shim;
    return object[name];
  };
   ...
   * @param {Function} task
   */
  exports.enqueue = exports.nextTick = nextTick; // XXX enqueue deprecated
  ...
  function Promise(descriptor, fallback, valueOf) {
    ...
    if (valueOf) promise.valueOf = valueOf;
  ...
  function isRejected(object) {
    ...
    if (object === undefined || object === nil) return false;
        ...
        viewInfo: function () {
          ...
          while (on) {
            Object.getOwnPropertyNames(on).forEach(function (name) {
              if (!properties[name]) properties[name] = typeof on[name];
            });
   ...
   * without a rejection.
   */
  exports.master = exports.def = def;
  function def(object) {
    return Promise(
      {
        isDef: function () {},
  ...
  function when(value, fulfilled, rejected) {
    ...
    nextTick(function () {
      ref(value).promiseSend(
        "when",
        function (value) {
          if (done) return;
          done = true;
          deferred.resolve(
            ref(value).promiseSend("when", _fulfilled, _rejected)
          );
        },
        function (reason) {
          if (done) return;
  ...
  function send(object, op) {
    ...
    object = ref(object);
    nextTick(function () {
      object.promiseSend.apply(object, [op, deferred.resolve].concat(args));
    });
    return deferred.promise;
  }
  ...
  function all(promises) {
    return when(promises, function (promises) {
      ...
      var values = [];
      if (countDown === 0) return ref(values);
      var deferred = defer();
      reduce.call(
        promises,
        function (undefined, promise, index) {
          when(
            ...
            function (answer) {
              values[index] = answer;
              if (--countDown === 0) deferred.resolve(values);
            },
            deferred.reject
          );
        },
  ...
  /**
   */
  exports.spy = exports.fin = fin; // XXX spy deprecated
  ...
  function end(promise) {
    when(promise, undefined, function (error) {
      ...
      nextTick(function () {
        ...
        if (error.stack && error.toString) {
          // probably an Error
          chainedError = new Error(error.toString());
   ...
   */
  for (var name in exports) ref[name] = exports[name];
  module.exports = ref;
  return ref;
});
