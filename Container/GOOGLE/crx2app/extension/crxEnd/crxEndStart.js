  ...
  return function onDebuggerWindowCreated(onClickInfo, debuggeeTab, win) {
    ...
    crxEnd.checkDebuggerOrigin(
      ...
      function onValidOrigin(validOrigin) {
        ...
        var windowsAdapter = crxEnd.getWindowsAdaptersByOrigin(
          validOrigin,
          debuggerTab
        );
        if (!windowsAdapter) {
          // then this origin has not been seen
          windowsAdapter = crxEnd.createWindowsAdapter(
            validOrigin,
            debuggerTab
          );
        } else {
        ...
        var url =
          ...
          "&" +
          "url=" +
          encodeURIComponent(onClickInfo.pageUrl);
        if (extraURLParams) url += extraURLParams;
...
function debuggerOpener(site, callback, extraURLParams) {
  ...
  //**********
  return halfWidthWindowOpener(
    fakeBlankURL,
    allowSiteWhenDebuggerOpens(site, callback, extraURLParams)
  );
}
