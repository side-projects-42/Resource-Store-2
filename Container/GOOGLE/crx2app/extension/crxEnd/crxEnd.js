...
function makeCxrEnd(config, chrome) {
  var crxEnd = {
    ...
    createOrReuseWindowsAdapter: function (validOrigin, debuggerTab) {
      var windowsAdapter = this.getWindowsAdaptersByOrigin(
        validOrigin,
        debuggerTab
      );
      if (!windowsAdapter) {
        // then this validOrigin has not been seen
        ...
        if (this.isOriginAllowed(origin)) {
          ...
          onInvalid(
            "Web Origin Not Allowed " +
              origin +
              ", Check chrome://extensions options"
          );
        }
      } else {
    ...
    onRequest: function (request, sender, sendResponse) {
      ...
      if (sender.tab && request.name === getChromeExtensionPipe.NAME) {
        this.checkDebuggerOrigin(
          sender.tab.url,
          function onValid(validOrigin) {
            var windowsAdapter = this.createOrReuseWindowsAdapter(
              validOrigin,
              sender.tab
            );
    ...
    onConnect: function (port) {
      ...
      if (windowsAdapter) {
        ...
        console.error(
          "crx2app/crxEnd: no windowsAdapter for port.name: " + port.name
        );
      }
    },
    ...
    onMessage: function (windowsAdapter, jsonObj) {
      if (debugMessages) {
        console.log(
          "crx2app/crxEnd: onMessage " + jsonObj.target + "." + jsonObj.method,
          jsonObj
        );
      }
      ...
      if (target) {
        if (target.api.indexOf(jsonObj.method) > -1) {
          ...
          if (jsonObj.params instanceof Array) {
            if (typeof jsonObj.serial === "number") {
              windowsAdapter.stageChromeCall(
                method,
                target,
                [jsonObj.serial].concat(jsonObj.params)
              );
            } else {
              windowsAdapter.postError(
                "serial '" +
                  jsonObj.serial +
                  "' is not a number; " +
                  jsonObj.target +
        ...
        windowsAdapter.postError(
          "target '" +
            jsonObj.target +
            "' is not one of " +
            Object.keys(this.chromeAdapters),
          jsonObj
        );
      }
    },
