  ...
  WindowsAdapter.prototype = {
    ...
    chromeWrappers: {
      create: function (serial, createData) {
        ...
        chrome.windows.create(
          cleanCreateData,
          this.onCreated.bind(this, serial)
        );
      },
    ...
    removeTab: function (tabId) {
      ...
      if (index > -1) {
        ...
        if (debugAdapters) {
          console.info(
            "windowsAdapter attempt to remove " + tabId + ", unknown tab"
          );
        }
      }
    },
    ...
    onCreated: function (serial, win) {
      ...
      console.assert(
        !win.tabs || win.tabs.length === 1,
        "A newly created chrome.Window should have at most one tab"
      );

      if (typeof serial === "number") {
        // then we created the window, track it
    ...
    _connect: function () {
      if (debugConnection) {
        console.log(
          "WindowsAdapter " + this.name + " connect " + this.debuggerOrigin
        );
      }
    },
