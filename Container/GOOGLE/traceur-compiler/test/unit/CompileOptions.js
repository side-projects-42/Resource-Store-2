...
suite("options", function () {
  ...
  test("CommandOptions fromString", function () {
    ...
    assert.isTrue(
      CommandOptions.fromString("--blockBinding=true").blockBinding
    );
    ...
    assert.equal(
      CommandOptions.fromString("--modules=false").modules,
      "bootstrap"
    );
    assert.equal(
      CommandOptions.fromString("--referrer=traceur@0.0.1").referrer,
      "traceur@0.0.1"
    );

    assert.equal(
      CommandOptions.fromString("--source-maps=inline").sourceMaps,
      "inline"
    );
    assert.equal(
      CommandOptions.fromString(" --source-maps  --blockBinding").sourceMaps,
      "file"
    );
    assert.equal(
      CommandOptions.fromString("--source-maps=memory").sourceMaps,
      "memory"
    );
  });
