...
suite("api.js", function () {
  ...
  test("api compile script function declaration", function () {
    var result = api.compile("function foo() {};", { modules: false });
    assert(result.length > 0);
  });

  test("api compile script function expression", function () {
    var result = api.compile("var foo = function() {};", { modules: false });
    assert(result.length > 0);
  });
  ...
  test("api compile require", function () {
    ...
    assert.equal(
      result.indexOf(
        '$traceurRuntime.registerModule("a.js", [], function(require) {'
      ),
      0,
