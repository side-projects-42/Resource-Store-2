...
export class NodeTraceurTestRunner extends TraceurTestRunner {
  ...
  getOptions() {
    ...
    let commandLine = new Command(process.argv[0] + " " + process.argv[1]);
    ...
    commandLine
      .command("only <file> [files...]")
      .description("only test these [files] ")
      .action((file, files) => {
        commandLine.patterns = [file];
        if (files) commandLine.patterns = commandLine.patterns.concat(files);
      });
