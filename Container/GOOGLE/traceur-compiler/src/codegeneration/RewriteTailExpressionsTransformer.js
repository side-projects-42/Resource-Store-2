...
import { AND, OR } from "../syntax/TokenType.js";
...
function createCall(tree, operand, thisArg, importRuntimeTransformer) {
  ...
  return new CallExpression(
    tree.location,
    continuation,
    new ArgumentList(argList ? argList.location : null, [
      operand,
      thisArg,
...
export class RewriteTailExpressionsTransformer extends ParseTreeTransformer {
  ...
  transformMemberExpressionCall_(tree, operand) {
    ...
    let assignment;
    if (
      object.type === IDENTIFIER_EXPRESSION ||
  ...
  static transform(bodyTransformer, tree) {
    return new RewriteTailExpressionsTransformer(bodyTransformer).transformAny(
      tree
    );
  }
}
