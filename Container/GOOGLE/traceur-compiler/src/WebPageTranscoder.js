...
export const scriptSelector =
  'script[type="module"],script[type="text/traceur"]';

export class WebPageTranscoder {
  constructor(url = document.location.href, traceurOptions = new Options()) {
    this.url = url;
    this.numPending_ = 0;
  ...
  asyncLoad_(url, fncOfContent, onScriptsReady) {
    ...
    webLoader.load(
      ...
      (error) => {
        console.error(
          "WebPageTranscoder FAILED to load " + url,
          error.stack || error
        );
      }
    );
  ...
  addFileFromScriptElement(scriptElement, name, content) {
    ...
    else loadingResult = System.script(content, nameInfo);
