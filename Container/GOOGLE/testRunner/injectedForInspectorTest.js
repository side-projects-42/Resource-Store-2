...
function injectedForInspectorTest(testURL, testParentURL, jsonSignalTokens) {
  (function () {
    ...
    if (window.location.href !== testURL) {
      ...
      window.testRunner = {
        // impl API called by test suite running in the iframe
        dumpAsText: function () {},
        ...
        evaluateInWebInspector: function (callId, script) {
          ...
          if (!window.sentActualHeader) {
            ...
            var actualHeader =
              'InspectorTest.addResult(unescape("' +
              escape(document.body.innerText + "\n") +
              '"))';
            window.parent.postMessage(
              ["evaluateForTest", 99, actualHeader],
              "*"
            );
          }
        },
      ...
      window.initialize_monkeyPatchInspectorTest = function () {
        InspectorTest.Output = {
          testComplete: function () {
            WebInspector.consoleTrue.log(
              SignalTokens.COMPLETE,
    ...
    function monkeyPatch() {
      ...
      WebInspector.evaluateForTestInFrontend = function (callId, script) {
        ...
        function invokeMethod() {
          try {
            script =
              script + "//@ sourceURL=evaluateInWebInspector" + callId + ".js";
            ...
            WebInspector.consoleTrue.error(
              "WebInspector eval error:" + e,
              e.stack
            );
          }
        }
      ...
      WebInspector.navigatePage = function (callback) {
        ...
        var webInspectorRequiredThis = {
          onNavigated: function (event) {
            ...
            if (url === testURL) {
              ...
              WebInspector.consoleTrue.log(
                "testRunner repeat window.location to  " + testURL
              );
            }
          },
        };
        ...
        evaluateInConsole('window.location="' + testURL + '"', "test");
        WebInspector.consoleTrue.log(
          "testRunner window.location to  " + testURL
        );
      };
    }
    ...
    window.addEventListener("load", function checkForInspector() {
      ...
      console.log(
        "testRunner found load event in " +
          path +
          " and it " +
          (matchTestParentURL ? "matches" : " does NOT match")
      );

      if (window.WebInspector) {
        // this part only runs in one frame and on load
        console.log(
          "testRunner found WebInspector event in " + window.location
        );
        monkeyPatch();
        if (matchTestParentURL) {
          // then we are testing devtools and we need to wait longer
