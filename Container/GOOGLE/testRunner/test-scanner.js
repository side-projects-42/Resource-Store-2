...
function xhrGET(url, callback, errback) {
  if (!this.proxy) {
    this.proxy = new RemoteMethodCall.Requestor(
      XHRInBackground,
...
LayoutTests.forEach(function (layoutTest) {
  layoutTest.folders.forEach(function (folder) {
    scanFolder(
      layoutTest.baseURL,
      folder,
      layoutTest.testParentURL,
      layoutTest.extension
    );
  });
});
  ...
  xhrGET(
    ...
    function onload(html) {
      ...
      for (var i = 0; i < links.length; ++i) {
        ...
        testCaseURL +=
          indexLayoutTests !== -1 ? href.substr(indexLayoutTests) : href;
...
if (DEBUG) {
  setInterval(function () {
    ...
    console.log(
      Object.keys(unfetchedByURL).forEach(function (url) {
...
function fetchExpectations(path, testParentURL, extension) {
  ...
  function filter(expectations) {
    ...
    for (var i = 0; i < expectationLines.length; ++i) {
      if (
        !expectationLines[i].indexOf("ALERT: ") ||
