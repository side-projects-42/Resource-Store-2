...
var RemoteMethodCall = (function () {
  ...
  function Responder(serverMethods, rawPort) {
    ...
    this.onMessage = function (message) {
      ...
      if (
        method in this.serverMethods &&
        typeof this.serverMethods[method] === "function"
      ) {
    ...
    this.onException = function (postId, method, args) {
      this.channelPlate.postMessage(
        [postId, method, ERROR, "Exception"].concat(args)
      );
    };

    this.accept = function (port) {
      this.channelPlate.accept.apply(this.channelPlate, [
        port,
        this.onMessage.bind(this),
      ]);
    };

    this.channelPlate = new ChannelPlate.Base(
      rawPort,
      this.onMessage.bind(this)
    );
  }
  ...
  Requestor.prototype = {
    ...
    request: function (method, args, onResponse, onError) {
      this.responseHandlers[++this.postId] = {
        method: method,
      ...
      if (DEBUG) {
        console.log(
          "Requestor " + this.instance + " sent " + this.postId + ": " + method,
          args
        );
      }
    },
    ...
    _onMessage: function (event) {
      ...
      if (!responseHandler) {
        console.error(
          "Requestor " +
            this.instance +
            " _onMessage failed, no responseHandler for postId " +
            postId,
          this.responseHandlers
        );
        return;
      ...
      if (method !== responseHandler.method) {
        console.error(
          "Requestor " +
            this.instance +
            " protocol error, remote method does not match local method"
        );
        return;
      }
      var args = payloadArray;
      ...
      try {
        if (callback && status === RESPONSE) {
          ...
          if (errback) errback.apply(this, args);
          else
            console.error(
              "RemoteMethodCall.Requestor._onMessage ERROR (no errback) ",
              args
            );
        }
        if (DEBUG) {
          console.log(
            "RemoteMethodCall.Requestor._onMessage " +
              postId +
              ": " +
              method +
              ": " +
    ...
    _createProxy: function (serverMethods) {
      var proxy = {};
      Object.keys(serverMethods).forEach(
        function (method) {
