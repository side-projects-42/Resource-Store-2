...
var TestRunnerPanel = {
  ...
  addDebugFlag: function (flag) {
    ...
    if (!exists) {
      ...
      if (flag.selected == "true") this.selectedFlags.push(flag);
    }
  },
...
TestRunnerPanel.loadTests = function loadTests() {
  ...
  TestRunnerPanel.treeOutline = new TreeOutline(
...
TestRunnerPanel.addTest = function (model) {
  ...
  if (this.isStateRan()) this.stateLoaded();
};
...
TestRunnerPanel.runNextTest = function runNextTest(lastResult) {
  ...
  if (!this.isStateInterrupted()) {
    ...
    if (test) {
      this.currentTestRunner = new TestRunner(
        test.model,
        test.view,
        runNextTest.bind(this)
      );
...
TestView.prototype = {
  ...
  onTreeElementSelect: function () {
    ...
    var baseChars =
      this._testModel.expectedURL.indexOf(baseEndSentinel) +
      baseEndSentinel.length;
    if (baseChars > 0)
    ...
    for (var idx = 0; idx < opcodes.length; idx++) {
      ...
      for (var i = 0; i < rowCount; i++) {
        if (change === "delete" || (change === "replace" && b < be)) {
          var lineNumber = b++;
          this._treeElement.appendChild(
            new TreeElement("- [" + lineNumber + "] " + baseLines[lineNumber])
          );
        }
        ...
        if (change === "insert" || (change === "replace" && n < ne)) {
          var lineNumber = n++;
          this._treeElement.appendChild(
            new TreeElement("+ [" + lineNumber + "] " + newLines[lineNumber])
          );
        }
  ...
  restyle: function (status) {
    this._treeElement.title =
      this._testModel.expectedURL + ": " + status.toUpperCase();
  ...
  update: function (actual) {
    ...
    if (!actual) {
      ...
      actual === this._testModel.expected ||
      actual === this._testModel.expected + "\n"
    ) {
...
TestRunner.commands = {
  _metaServer: function () {
    if (!this.backgroundProxy) {
      this.backgroundProxy = new RemoteMethodCall.Requestor(
        BackgroundServerAPI,
        ChannelPlate.DevtoolsTalker
      ).serverProxy();
    }
  ...
  screenshot: function (resultNumber, surround, callback) {
    this._metaServer().screenshot(
      chrome.devtools.inspectedWindow.tabId,
      function (dataURL) {
        var result = surround.replace("screenshot", dataURL);
        TestRunnerPanel.currentTestRunner.actual =
          TestRunnerPanel.currentTestRunner.actual.replace(
            "commandResult " + resultNumber,
            result
          );
        console.log(
          "screenshot for " + resultNumber + " with surround " + surround,
  ...
  output: function (url, callback) {
    ...
    chrome.extension.sendMessage(
      "jmacddndcaceecmiinjnmkfmccipdphp",
      request,
      function maybeSaved(response) {
...
TestRunner.prototype = {
  ...
  _listenForResults: function () {
    chrome.experimental.devtools.console.onMessageAdded.addListener(
  ...
  _onMessageAdded: function (message) {
    ...
    if (mark !== -1) {
      ...
      var actualLine =
        message.text.substring(mark + SignalTokens.RESULT.length) + "\n";
  ...
  _reloadWithTestScripts: function (debug) {
    ...
    if (debug) {
      argsAsString +=
        ", '" +
        JSON.stringify(
          TestRunnerPanel.selectedForDebug().concat(["DebugLogger"])
        ) +
        "'";
    }
    var reloadOptions = {
      ignoreCache: true,
      injectedScript:
        "(" + runInEveryDebuggeeFrame + "(" + argsAsString + ")" + ")",
    };
  ...
  notifyDone: function (actual) {
    ...
    chrome.experimental.devtools.console.onMessageAdded.removeListener(
      this._onMessageAdded
    );
...
function onMessageFromTestScanner(event) {
  ...
  if (method === "test") {
    ...
    if (reFilter) {
      if (!reFilter.test(model.expectedURL)) return;
    }
