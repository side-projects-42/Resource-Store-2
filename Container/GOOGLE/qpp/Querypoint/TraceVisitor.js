...
(function () {
  ...
  Querypoint.TraceVisitor.prototype = {
    visitTrace: function (tree, traceData) {
      ...
      functionDefinitionOffsetKeys.forEach(
        function (functionDefinitionOffsetKey) {
          var functionDefinitionTree;
          if (functionDefinitionOffsetKey === "file") {
            functionDefinitionTree = tree;
          } else {
            var functionDefinitionOffset = parseInt(
              functionDefinitionOffsetKey,
              10
            );
            functionDefinitionTree = this._project
              .treeFinder()
              .byOffset(tree, functionDefinitionOffset);
          }
    ...
    visitFunctionTraced: function (functionTree, activations) {
      ...
      activations.forEach(
        function (activation, index) {
          ...
          this.visitActivationTraced(
            functionTree,
            activation,
            activationInThisTurn
          );
        }.bind(this)
      );
    },
    ...
    _findInTree: function (tree, id) {
      ...
      function byOffsetKey(offset, range, tree) {
        ...
        if (loc) {
          ...
          if (startOffset <= offset && offset <= endOffset) {
            if (tokenRange === range) return 0;
            else
              return Math.max(
                Math.abs(endOffset - offset),
                Math.abs(startOffset - offset)
              );
          } else {
            return -1;
          }
      ...
      return this._project
        .treeFinder()
        .findByDistanceFunction(tree, byOffsetKey.bind(this, offset, range));
    },

    visitActivationTraced: function (
      functionTree,
      activation,
      activationCount
    ) {
      var tracedExpressionIds = Object.keys(activation);
      tracedExpressionIds.forEach(
        function (id) {
          if (id === "turn") return;
          ...
          if (debug) {
            var treeAsString =
              traceur.outputgeneration.TreeWriter.write(expressionTree);
            var treeLoc =
              expressionTree.location.start.offset +
              "-" +
              expressionTree.location.end.offset;
            console.log(
              "TreeHangerTraceVisitor.visitActivationTraced " +
                id +
                " @" +
                treeLoc +
                ": " +
    ...
    visitExpressionsTraced: function (
      expressionTree,
      turn,
      activationCount,
      trace
  ...
  Querypoint.TreeHangerTraceVisitor.prototype = Object.create(
    Querypoint.TraceVisitor.prototype
  );

  Querypoint.TreeHangerTraceVisitor.prototype.isDuplicate = function (
    trace,
    traces
  ) {
    var exists = traces.some(function (existingTrace) {
      if (
        ...
        (trace.functionOffset &&
          trace.functionOffset !== existingTrace.functionOffset)
      )
        return false;
  ...
  Querypoint.TreeHangerTraceVisitor.prototype.appendUnique = function (
    trace,
    traces
  ) {
  ...
  Querypoint.TreeHangerTraceVisitor.prototype.visitExpressionsTraced =
    function (expressionTree, turn, activationCount, trace) {
      if (expressionTree.location) {
        ...
        var traces = (expressionTree.location.traces =
          expressionTree.location.traces || []);
  ...
  Querypoint.LineModelTraceVisitor.prototype = {
    ...
    visitActivationTraced: function (functionTree, activation) {
      ...
      if (functionDefinitionOffset < 0)
        throw new Error(
          "LineModelTraceVisitor.visitActivationTraced invalid functionTree offset " +
            functionDefinitionOffset
        );
      var line = this._sourceFile.lineNumberTable.getLine(
        functionDefinitionOffset
      );
      this.tracesByLine[line] = this.tracesByLine[line] || [];
      this.tracesByLine[line].push({
        offset: functionDefinitionOffset,
        activation: activation,
      });
      Querypoint.TraceVisitor.prototype.visitActivationTraced.call(
        this,
        functionTree,
        activation
      );
    ...
    visitExpressionsTraced: function (expressionTree, turn, index, trace) {
      ...
      if (offset < 0)
        throw new Error(
          "LineModelTraceVisitor.visitExpressionsTraced invalid offset " +
            offset
        );
