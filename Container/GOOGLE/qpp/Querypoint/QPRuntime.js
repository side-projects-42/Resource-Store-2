...
(function () {
  ...
  function define__qp(debug_in_page, useAsync) {
    ...
    function fireLoad() {
      try {
        ...
        if (handlers) {
          ...
          if (debug_in_page) {
            console.log(
              "qp| debug fireLoad complete, fired " +
                handlers.length +
                " handlers"
            );
          }
    ...
    function fireLoadAfterRealLoad() {
      if (debug_in_page)
        console.log(
          "qp| debug fireLoadAfterRealLoad fireLoadTrigger " + !!fireLoadTrigger
        );
      if (fireLoadTrigger) return fireLoadTrigger.call();
      else fireLoadTrigger = fireLoad;
    }
    ...
    function grabLoadEvent(useAsync) {
      window.addEventListener("load", function grabbedLoadEvent(event) {
        if (useAsync) {
          ...
          if (debug_in_page)
            console.log(
              "qp| debug  grabbedLoadEvent; beforeArtificalLoadEvent: " +
                beforeArtificalLoadEvent +
                " loaded: " +
                !!window.__qp.loadEvent
            );
      ...
      window.addEventListener("DOMContentLoaded", function (event) {
        ...
        if (debug_in_page)
          console.log(
            "qp| debug  grabLoadEvent DOMContentLoaded event, stored"
          );
      });
    }
    ...
    function appendFileInfoFromPreamble(entryPointFunction, startInfo) {
      if (entryPointFunction.__qp) {
        ...
        var transcodedFunctionString =
          _FunctionPrototypeToString.call(entryPointFunction);
        var preamble = reTranscodedFunctionPreamble.exec(
          transcodedFunctionString + ""
        );
        if (preamble) {
    ...
    function appendEntryPointInfo(startInfo, entryPointFunction, args) {
      if (typeof entryPointFunction === "string") {
        ...
        throw new Error(
          "QPRuntime.startTurn: illegal entryPointFunction ",
          entryPointFunction
        );
      }
      return startInfo;
    ...
    function appendElementInfo(startInfo, eventObject) {
      ...
      startInfo.eventType =
        eventObject.type || eventObject.name || eventObject.constructor.name;
    ...
    function wrapEntryPoint(
      entryPointFunction,
      registrationTurnNumber,
      registrationInfo
    ) {
      if (!entryPointFunction) return function noop() {};
    ...
    function recordEntryPoints() {
      ...
      window.__qp.intercepts.Node = {
        prototype: { addEventListener: window.Node.prototype.addEventListener },
      };
    }
    ...
    function interceptEntryPoints() {
      window.addEventListener = function (type, listener, useCapture) {
        ...
        if (useAsync && beforeArtificalLoadEvent) {
          ...
          if (type !== "load")
            console.error(
              "qp| debug interceptEntryPoints not a load event, we fail!",
              type
            );
        } else {
          // We've already passed the artifical load event, so don't delay the add call
          window.__qp.intercepts.addEventListener.call(
            this,
            type,
            wrapped,
            useCapture
      ...
      window.Node.prototype.addEventListener = function (
        ...
        console.log(
          "qp| addEventListener " +
            type +
            " " +
            getSelectorUniqueToElement(this)
        );
        var info = {
          type: "addEventListener",
          thisObject: getSelectorUniqueToElement(this),
        };
        window.__qp.intercepts.Node.prototype.addEventListener.call(
          this,
          type,
          wrapEntryPoint(listener, window.__qp.turn, info),
          useCapture
        );
      };
      ...
      window.setTimeout = function (stringFunction, millisecond) {
        if (typeof stringFunction !== "function")
          throw new Error(
            "QPRuntime: Non-function arguments to setTimeout not implemented. For good reason ;-)"
          );
    ...
    // For lastChange
    function reducePropertyChangesToTracedObject(
      propertyKey,
      tracedObjectIndex
    ) {
      if (debug_in_page) {
        if (window.__qp.propertyChanges[propertyKey])
          console.log(
            "qp| debug reducePropertyChangesToTracedObject starts with " +
              window.__qp.propertyChanges[propertyKey].length
          );
        else
          console.error(
            "qp| debug reducePropertyChangesToTracedObject propertyChanges not Initialized for " +
              propertyKey
          );
      }
      ...
      if (!changes || !changes.length) {
        if (debug_in_page)
          console.warn(
            "qp| debug reducePropertyChangesToTracedObject No changes for " +
              propertyKey +
              " at " +
              tracedObjectIndex
          );
      ...
      if (!changes.objectTraced) {
        if (debug_in_page)
          console.error(
            "qp| debug reducePropertyChangesToTracedObject no objectTraced for " +
              propertyKey +
              " at " +
              tracedObjectIndex
          );
      ...
      var rawTracepoints = changes.reduce(function (ours, change) {
        if (debug_in_page)
          console.log(
            "qp| debug reducePropertyChangesToTracedObject %o =?= %o",
            change.obj,
            object,
            change
          );
      ...
      if (debug_in_page)
        console.log(
          "qp| debug reducePropertyChangesToTracedObject ends with " +
            rawTracepoints.length
        );
      return {
        turn: this.turn,
        tracepoints: extractTracepoints(rawTracepoints),
      };
    }
    ...
    function setTracedPropertyObject(object, propertyKey, tracedObjectIndex) {
      ...
      window.__qp.propertyChanges[propertyKey].objectTraced =
        window.__qp.propertyChanges[propertyKey].objectTraced || {};
      window.__qp.propertyChanges[propertyKey].objectTraced[tracedObjectIndex] =
        object;
      if (debug_in_page)
        console.log(
          "qp| debug setTracedPropertyObject: %o setting " + propertyKey,
          object,
          window.__qp.propertyChanges[propertyKey].objectTraced
        );
    }
    ...
    function setTracedElement(selector, propertyKey, tracedObjectIndex) {
      ...
      if (debug_in_page)
        console.log(
          "qp| debug QPRuntime.setTracedElement " +
            selector +
            " key " +
            propertyKey +
            " index " +
    ...
    function initializeHiddenGlobalState() {
      window.__qp = {
        ...
        setTraced: function (propertyName) {
          // called by eval
        ...
        reducePropertyChangesToTracedObject:
          reducePropertyChangesToTracedObject, // changes limited to object
    ...
    console.log(
      "qp| reload " +
        window.__qp_reloads +
        " ----------------------- Querypoint Runtime Initialized ---------------------------------"
    );

    if (debug_in_page)
      console.log(
        "qp| debug runtime initialized: window.__qp: %o",
        window.__qp
      );

    endTurn(
      startTurn("_pageNavigation", [
        { type: window.__qp_reloads ? "reload" : "openWebPage" },
      ])
    ); // define turn zero, it appears to create script tag outer functions.
  }
  ...
  Querypoint.QPRuntime = {
    ...
    runtimeSource: function (numberOfReloads) {
      if (debug)
        console.log(
          "qp| debug QPRuntime creating runtime for load #" + numberOfReloads
        );
      var fncs = this.runtime.map(function (fnc) {
        return (
          ...
          !!debug +
          "," +
          !!Querypoint.QPPreprocessor.useAsyncPreprocessor +
          "));\n"
        );
      });
