...
(function () {
  ...
  Querypoint.AllExpressionsQuery.ifAvailableFor = function (tree, project) {
    if (!!tree.location) {
      var query = Querypoint.AllExpressionsQuery.prototype.getQueryOnTree(
        tree,
        Querypoint.AllExpressionsQuery
      );
  ...
  Querypoint.AllExpressionsQuery.prototype = {
    ...
    getQueryOnTree: function (tree, queryConstructor) {
      return Querypoint.AllExpressionsQuery.filesTraced[
        tree.location.start.source.name
      ];
    },
    ...
    matches: function (ctor, queryData) {
      return (
        ctor === Querypoint.AllExpressionsQuery &&
    ...
    extractTracepoints: function (fileViewModel, onTracepoint) {
      ...
      function onEval(traceData, isException) {
        if (debug)
          console.log(
            "AllExpressionsQuery.extractTracepoints from " + fileName,
            traceData
          );
