CodeMirror.showHint = function (cm, getHints, options) {
  ...
  function startHinting() {
    ...
    if (options.async) getHints(cm, showHints, options);
    else return showHints(getHints(cm, options));
  }
  ...
  function showHints(data) {
    ...
    for (var i = 0; i < completions.length; ++i) {
      ...
      if (completion.className != null)
        className = completion.className + " " + className;
      elt.className = className;
      if (completion.render) completion.render(elt, data, completion);
      else
        elt.appendChild(
          document.createTextNode(completion.displayText || getText(completion))
        );
      elt.hintId = i;
    }
    var pos = cm.cursorCoords(
      options.alignWithWord !== false ? data.from : null
    );
    var left = pos.left,
      top = pos.bottom,
      below = true;
    ...
    function changeActive(i) {
      ...
      if (node.offsetTop < hints.scrollTop)
        ...
        hints.scrollTop + hints.clientHeight
      )
        hints.scrollTop =
          node.offsetTop + node.offsetHeight - hints.clientHeight + 3;
      CodeMirror.signal(data, "select", completions[selectedHint], node);
    }

    function screenAmount() {
      return (
        Math.floor(hints.clientHeight / hints.firstChild.offsetHeight) || 1
      );
    }

    var keyMap = buildKeyMap(options, {
      moveFocus: function (n) {
        changeActive(selectedHint + n);
      },
      setFocus: function (n) {
        changeActive(n);
      },
