...
(function () {
  ...
  var defaultKeymap = [
    ...
    { keys: ["|"], type: "motion", motion: "moveToColumn", motionArgs: {} },
    // Operators
  ...
  var Vim = function () {
    ...
    var validMarks = upperCaseAlphabet
      .concat(lowerCaseAlphabet)
      .concat(numbers)
      .concat(["<", ">"]);
    var validRegisters = upperCaseAlphabet
      .concat(lowerCaseAlphabet)
      .concat(numbers)
      .concat('-"'.split(""));
      ...
      evalInput: function (cm, vim) {
        ...
        if (motion) {
          ...
          if (vim.visualMode) {
            ...
            if (
              cursorIsBefore(selectionStart, selectionEnd) &&
              ...
              cursorIsBefore(selectionEnd, selectionStart) &&
              (cursorEqual(selectionStart, curEnd) ||
        ...
        if (operator) {
          ...
          var linewise =
            motionArgs.linewise || (vim.visualMode && vim.visualLine);
          if (linewise) {
    ...
    var operators = {
      change: function (cm, operatorArgs, vim, curStart, curEnd) {
        ...
        if (operatorArgs.linewise) {
          ...
          // TODO: Fix the above.
          curStart.ch = findFirstNonWhiteSpaceCharacter(
            cm.getLine(curStart.line)
          );
      ...
      delete: function (cm, operatorArgs, vim, curStart, curEnd) {
        ...
        if (
          operatorArgs.linewise &&
          curEnd.line > cm.lastLine() &&
          curStart.line > cm.firstLine()
        ) {
          curStart.line--;
      ...
      toggleVisualMode: function (cm, actionArgs, vim) {
        ...
        if (!vim.visualMode) {
          ...
          if (!vim.visualLine && actionArgs.linewise) {
            ...
            vim.visualLine = true;
            curStart.ch = cursorIsBefore(curStart, curEnd)
              ? 0
              : lineLength(cm, curStart.line);
            curEnd.ch = cursorIsBefore(curStart, curEnd)
              ? lineLength(cm, curEnd.line)
              : 0;
            cm.setSelection(curStart, curEnd);
        ...
        updateMark(
          ...
          vim,
          "<",
          cursorIsBefore(curStart, curEnd) ? curStart : curEnd
        );
        updateMark(
          cm,
          vim,
          ">",
          cursorIsBefore(curStart, curEnd) ? curEnd : curStart
        );
      },
    ...
    function regexLastIndexOf(string, pattern, startIndex) {
      for (var i = !startIndex ? string.length : startIndex; i >= 0; --i) {
      ...
      sort: function (cm, params) {
        ...
        function parseArgs() {
          if (params.argString) {
            ...
            if (!args.eatSpace()) {
              throw "invalid arguments " + args.match(/.*/)[0];
            }
    ...
    function parseRegisterToKeyBuffer(macroModeState, registerName) {
      var match, key;
      var register =
        getVimGlobalState().registerController.getRegister(registerName);
    ...
    function parseKeyBufferToRegister(registerName, keyBuffer) {
      ...
      getVimGlobalState().registerController.setRegisterText(
        registerName,
        text
      );
    }
