...
function processArguments(argv) {
  ...
  for (var i = 2; i < argv.length; i++) {
    ...
    if (option) {
      if (arg === "--out") interpretMode = false;
...
if (out) {
  ...
  if (isSingleFileCompile) compileToSingleFile(out, includes, flags.sourceMaps);
  else compileToDirectory(out, includes, flags.sourceMaps);
} else {
