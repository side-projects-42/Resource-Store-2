(function () {
  ...
  function toArray(nodeList) {
    var ary = [];
    for (var i = 0; i < nodeList.length; i++) ary.push(nodeList[i]);
    return ary;
  }
  ...
  function editorViewModelTests(a, tests) {
    ...
    a.PatientSelector.whenSelectorAll(".qp-highlight", "", function (elts) {
      var highlights = mapSelector(
        ".CodeMirror-code .qp-highlight",
        function (elt) {
          return elt.textContent;
        }
      ).join("");
      if ('"hello world"' === highlights) tests.highlight = true;
    });
    tests.clearHighlight = false;
    a.PatientEvents.mouseToSelector("mouseout", "body", "", function () {
      if (
        document.querySelectorAll(".CodeMirror-code .qp-highlight").length === 0
      )
    ...
    a.PatientSelector.whenSelectorAll(".qp-decorate", "", function (elts) {
      var decorations = mapSelector(
        ".CodeMirror-code .qp-decorate",
        function (elt) {
          return elt.textContent;
        }
      ).join("");
      if ("hellosplits" === decorations) tests.decorate = true;

      tests.clearDecorations = false;
      editorViewModel.clearDecorations();
      if (
        document.querySelectorAll(".CodeMirror-code .qp-decorate").length === 0
      )
  ...
  function loadListViewModelTests(a, tests) {
    ...
    console.assert(
      loadListViewModel.showLoad() === loadListViewModel.lastLoad()
    );
  ...
  a.beginAsynchronousOperations(
    function () {
      var failing = [];
      Object.keys(tests).reduce(function (failing, test) {
        if (!tests[test]) failing.push(test);
        return failing;
      }, failing);
      if (!failing.length) {
    ...
    500,
    function (result) {
      if (result) console.log("editorViewModelTests " + result);
    }
  );
})();
