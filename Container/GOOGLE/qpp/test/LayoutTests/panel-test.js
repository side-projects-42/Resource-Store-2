...
var DevtoolsWindowTestAPI = new RemoteMethodCall.Requestor(
  PatientSelector,
  ChannelPlate.ChildIframe
).serverProxy();
...
function openQuerypointPanel(then) {
  DevtoolsWindowTestAPI.clickSelector(
    "div.toolbar-label",
    "Querypoint",
    function () {
      ExtensionPanelTestAPI.whenSelectorAll(
...
function openSourceFile(fileName, then) {
  DevtoolsWindowTestAPI.clickSelector(
    "div.filtered-item-list-dialog-item",
    fileName,
    then
  );
}
