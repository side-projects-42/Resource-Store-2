...
window.PatientSelector = (function () {
  ...
  var PatientSelector = {
    ...
    mouseToSelector: function (type, selector, textToMatch, callback) {
      ...
      PatientSelector.whenSelectorAll(
        ...
        function () {
          ...
          if (debug)
            console.log(
              "....PatientSelector." + type + "Selector hit ",
    ...
    mouseOverSelector: function (selector, textToMatch, callback) {
      PatientSelector.mouseToSelector(
        "mouseover",
        selector,
        textToMatch,
        callback
    ...
    keySelector: function (selector, textToMatch, keyDescriptor, callback) {
      ...
      PatientSelector.whenSelectorAll(
        ...
        function () {
          ...
          if (debug)
            console.log(
              "....PatientSelector.keySelector hit ",
              PatientSelector.hits[0]
            );
        }.bind(PatientSelector)
      );
    ...
    _moveLineNumber: function (lineNumber, visibleSourceLines, selector, text) {
      while (lineNumber < visibleSourceLines.length) {
        ...
        if (hits.length) {
          if (debug)
            console.log(
              "....PatientSelector.selectTokenInSource " +
                selector +
                "&" +
                text +
                " match " +
      ...
      if (lineNumber === visibleSourceLines.length - 1) {
        console.error(
          "...PatientSelector.selectTokenInSource no source line matchs " +
            selector +
            " & " +
            text,
          visibleSourceLines
    ...
    _fireMouseMove: function (tokenElt, callback) {
      ...
      if (debug)
        console.log(
          "....PatientSelector._fireMouseMove mousemove(" +
            xy.x +
            "," +
            xy.y +
            ") sent to %o",
    ...
    selectTokenInSource: function (editorTokens, callback) {
      ...
      function next(lineNumber) {
        ...
        if (editorTokens.length) {
          lineNumber = PatientSelector._moveLineNumber(
            lineNumber,
            visibleSourceLines,
            selector,
            text
          ...
          if (debug)
            console.log(
              "....PatientSelector.selectTokenInSource seeking pre ancestor of ",
              PatientSelector.hits
            );
          PatientSelector.ancestor("pre", function () {
            if (!PatientSelector.hits.length) {
              console.error(
                "....PatientSelector.selectTokenInSource ancestor selection failed",
                PatientSelector
              );
              callback();
            }
    ...
    evaluateInPage: function (expr, callback) {
      function checkException(result, isException) {
        if (isException) {
          console.error(
            "....PatientSelector.evaluateInPage(" + expr + ") exception",
            isException
          );
        } else {
          callback(result);
    ...
    evaluate: function (expr, callback) {
      try {
        ...
        console.error(
          "....PatientSelector.evaluate(" + expr + ") exception " + exc,
          exc
        );
      }
    },
    ...
    createProxy: function (port, iframeURL) {
      ...
      function onMessage(message) {
        ...
        if (handler) {
          ...
          console.error(
            "....PatientSelector.createProxy.onMessage no handler for " +
              postId +
              " in " +
              doc(),
            message
      ...
      var proxy = (PatientSelector.proxies[iframeURL] = new ChannelPlate.Base(
        port,
        onMessage.bind(PatientSelector)
      ));
      if (debug)
    ...
    _postToProxy: function (proxy) {
      ...
      if (handler && !handler.sent) {
        ...
        if (debug)
          console.log(
            "....PatientSelector.proxyTo.postMessage sent: " +
              handler.sent +
              " to " +
              handler.url,
            handler.args
    ...
    proxyTo: function (url, proxied, callback, errback) {
      ...
      PatientSelector.proxyHandlers[postId] = {
        url: url,
        onResponse: callback,
        onError: errback,
      ...
      var proxy;
      Object.keys(PatientSelector.proxies).some(
        function (iframeURL) {
