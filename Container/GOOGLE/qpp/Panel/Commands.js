...
(function () {
  ...
  QuerypointPanel.Commands = function (panel) {
    ...
    this.selectFile = function () {
      ...
      var names = {};
      panel.project.getSourceFiles().forEach(
        function (sourceFile) {
          names[sourceFile.name] = sourceFile;
          uriItems.appendItem(
            "open: " + sourceFile.name,
            panel.openSourceFileView.bind(panel, sourceFile)
          );
        }.bind(panel)
      );
      panel.project.page.resources.forEach(
        function (resource, index) {
          if (!names.hasOwnProperty(resource.url))
            uriItems.appendItem(
              "open: " + resource.url,
              panel.openResourceView.bind(panel, resource)
            );
        }.bind(panel)
      );
      uriItems.selectItem();
