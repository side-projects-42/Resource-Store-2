...
(function () {
      ...
      .computed(
        function () {
          this.fileViewModels().forEach(function (fileViewModel) {
            var editor =
              fileViewModel.editorViewModel &&
              fileViewModel.editorViewModel.editor();
            if (editor) editor.setSize();
          });
        }.bind(this)
      )
      .extend({ throttle: 1 }); // delay to prevent sizing on N+1 views
  ...
  QuerypointPanel.FileChainViewModel.prototype = {
    openChainedFileView: function (url, fromFileView) {
      ...
      var editorViewModel;
      if (!fromFileView) {
        // no link in the current chain contains this URL, start new chain
        ...
        // we are opening a new link in an existing chain
        var fromFileViewModel =
          this.fileViewModels()[fromFileView.getAttribute("chainIndex")];
    ...
    */
    openURL: function (url) {
      ...
      if (sourceFile) {
        ...
        var fileViewModel;
        var foundResource = this._panel.project.page.resources.some(
          function (resource) {
            if (resource.url)
              fileViewModel =
                QuerypointPanel.FileViewModel.openResourceView(resource);
          }.bind(this)
        );
        if (!fileViewModel)
          fileViewModel = QuerypointPanel.FileViewModel.openErrorMessage(
    ...
    openResourceView: function (resource) {
      if (/\.js$/.test(resource.url)) {
        resource.getContent(
          function (content) {
            var file = this._panel.project.addFileFromContent(
              resource.url,
              content
            );
    ...
    openSourceFileView: function (sourceFile) {
      var fileViewModel =
        QuerypointPanel.FileViewModel.openSourceFileView(sourceFile);
      this._createFileChain(fileViewModel);
    },
