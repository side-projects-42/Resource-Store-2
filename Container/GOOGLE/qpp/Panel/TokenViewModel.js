...
(function () {
  ...
  QuerypointPanel.TokenViewModel = function (
    querypointViewModel,
    editorViewModel,
    panel
  ) {
    ...
    this.followingTokens = ko.computed(
      function () {
    ...
    this.eventedTokenTree = ko.computed(
      function () {
        var tokenEvent = this.tokenEvent();
        if (!tokenEvent || !this._querypointViewModel.sourceFile()) return;

        var line = this.tokenEvent().start.line;
        var offsetOfLine = this._querypointViewModel
          .sourceFile()
          .lineNumberTable.offsetOfLine(line);
        var tokenOffset = offsetOfLine + tokenEvent.start.column;
        var eventedTokenTree = this._querypointViewModel.project
          .treeFinder()
          .byOffset(this._querypointViewModel.treeRoot(), tokenOffset);
        if (debug && eventedTokenTree) {
          ...
          var tokenLog =
            ...
            tokenOffset +
            "-" +
            (offsetOfLine + tokenEvent.end.column);
          var treeEnd = eventedTokenTree.location.end.offset;
          var treeStart = eventedTokenTree.location.start.offset;
          var treeLog =
            eventedTokenTree.type +
            "@" +
            (treeEnd - 1) +
            "_" +
            (treeEnd - treeStart);
          var varIdLog = traces
            ? " varId " + eventedTokenTree.location.varId
            : "";
          console.log(
            "tokenEvent " + tokenLog + " " + treeLog + varIdLog,
            traces ? traces : ""
          );
        } else if (debug) {
          console.warn(
            "No tree at offset " + tokenOffset + " for token " + tokenLog
          );
        }
    ...
    this.currentLocation = ko.computed(
      function () {
        ...
        if (tree) return tree.location;
      }.bind(this)
    );

    this.currentLocationId = ko.computed(
      function () {
      ...
      .computed(
        function () {
          ...
          scopesView.push({
            scopeDeclaration:
              '<div class="fileScope">' +
              tree.location.start.source.name +
              "</div>",
          });
  ...
  QuerypointPanel.TokenViewModel.prototype = {
    ...
    initialize: function () {
      ...
      this.currentExpression = ko
        .computed(
          function () {
            ...
            if (clone) {
              ...
              clone.style.left =
                this.expressionViewLeftOffset(
                  this.pxToNumber(box.style.left) +
                    this.pxToNumber(box.style.width)
                ) + "px";
        ...
        .extend({ throttle: 1 }); // let both location and editor update
    },
    ...
    _cloneEditorLineByLocation: function (location) {
      ...
      this._expressionFontSize =
        window.getComputedStyle(traceViewedLine).fontSize;
