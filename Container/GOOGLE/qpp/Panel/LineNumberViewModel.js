...
(function () {
  QuerypointPanel.LineNumberViewModel = function (
    querypointViewModel,
    editorViewModel,
    panel
  ) {
    ...
    this._editor = ko.computed(
      function () {
        if (
          this._editorListenedTo &&
          this._editorListenedTo.hasListener("onViewportChange")
        ) {
        ...
        if (this._editorListenedTo) {
          this._editorListenedTo.addListener(
            "onViewportChange",
    ...
    this._lineModel = ko
      .computed(
        function () {
          ...
          var tracesByLine = [];
          if (traceData) this._visitTraceData(traceData, tracesByLine);
          this._visitTracepoints(
            this._querypointViewModel.tracepoints(),
            tracesByLine
          );
          return tracesByLine;
        }.bind(this)
    ...
    this.updateLineNumberHighlights = ko
      .computed(
        function () {
          ...
          for (
            var line = viewportRange.start;
            line < viewportRange.end;
            line++, i_viewport++
          ) {
  ...
  QuerypointPanel.LineNumberViewModel.prototype = {
    ...
    _visitTraceData: function (traceData, tracesByLine) {
      var visitor = this._querypointViewModel.project.lineModelTraceVisitor(
        this._querypointViewModel.sourceFile()
      );
