...
(function () {
  ...
  var messagePrototype = {
    tooltip: function () {
      totalLogs++;
      if (debug) console.log("Message.tooltip: total logs : " + totalLogs);
  ...
  QuerypointPanel.Log = {
    ...
    connect: function () {
      QuerypointPanel.Console.onMessageAdded.addListener(
        this._onMessageAddedHandler
      );
    },
    ...
    disconnect: function () {
      QuerypointPanel.Console.onMessageAdded.removeListener(
        this._onMessageAddedHandler
      );
    },
    ...
    _onStartTurn: function (segments, messageSource) {
      ...
      if (this._turnInProgress.registrationTurnNumber)
        this._turnInProgress.registrationTurn =
          this.currentLoad.turns()[this._turnInProgress.registrationTurnNumber];
      else if (this._turnInProgress.turnNumber !== 1)
        console.error(
          "No registrationTurnNumber for turn " +
            this._turnInProgress.turnNumber,
          this._turnInProgress
        );
    ...
    _parse: function (messageSource) {
      ...
      if (mark === 0) {
        ...
        switch (keyword) {
          case "loadEvent":
            this._onLoadEvent(segments);
            break;
          case "reload":
            this._onReload(segments);
            break;
          case "startTurn":
            this._onStartTurn(segments, messageSource);
            break;
          case "endTurn":
            this._onEndTurn(segments);
            break;
          case "script":
          ...
          case "debug":
            break;
          case "setTimeout":
            this._onSetTimeout(segments);
            break;
          case "addEventListener":
            this._onAddEventListener(segments);
            break;
          case "replayComplete":
            this._onReplayComplete();
            break;
          default:
            console.error("Log._parse: unknown keyword: " + messageSource.text);
            break;
        }
      } else {
        // not a qp message
        if (!this.currentLoad) {
          console.error(
            "Log: message before load: " + messageSource.text,
            messageSource
          );
        } else {
          ...
          if (started && started === this.currentLoad.turnEnded())
            console.error(
              "QPRuntime error: No turn for message after turn %o",
              this._turnInProgress.turnNumber
            );
        }
      }
    ...
    _reformatMessage: function (messageSource) {
      ...
      this._turnScrubber.updateTurnIndicator(
        this._turnInProgress,
        messageSource.severity
      );
      if (debug) {
        console.log(
          "QuerypointPanel.Log._reformat messages.length " +
            this._turnInProgress.messages().length
        );
      }
    },
  };
