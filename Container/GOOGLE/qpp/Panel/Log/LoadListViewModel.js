...
(function () {
  ...
  QuerypointPanel.LoadModel = function (loadNumber) {
    ...
    this.currentTurn = ko.computed(
      function () {
        if (this.turns().length) return this.turns()[this.turnStarted() - 1];
      }.bind(this)
    );

    this.turnStarted = ko.computed(
      function () {
  ...
  QuerypointPanel.LoadListViewModel = {
    initialize: function () {
      ...
      this.lastLoad = ko.computed(
        function () {
          var last = this.loadViewModels().length - 1;
          if (debug)
            console.log("LoadListViewModel.lastLoad " + last + " loads");
      ...
      this.loadStartedNumber = ko.computed(
        function () {
      ...
      this.showLoadNumber = ko.computed(
        function () {
          return this.showLoad().loadNumber || "-";
        }.bind(this)
      );

      this.currentLoadIsSelected = ko.computed(
        function () {
          return this.showLoad().loadNumber == this.loadStartedNumber();
        }.bind(this)
      );

      this.isPastLoad = ko.computed(
        function () {
          return (
            this.loadStartedNumber() &&
            this.showLoad().loadNumber != this.loadStartedNumber()
          );
        }.bind(this)
      );
    ...
    selectLoadModel_: function (loadModel) {
      ...
      var loadElement = document.querySelector(
        'div.loadNumber[load="' + loadModel.loadNumber + '"]'
      );
