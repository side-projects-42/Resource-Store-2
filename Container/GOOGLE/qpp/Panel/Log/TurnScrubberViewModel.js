...
(function () {
  ...
  QuerypointPanel.TurnScrubberViewModel = {
    initialize: function (project, tracequeries, sessionViewModel) {
      ...
      this.recorder = QuerypointPanel.Recorder.initialize(
        sessionViewModel.loadListViewModel,
        this
      );
      ...
      this._setMessageWidth = function (width) {
        ...
        if (width < 3) turnIndicatorRule.style.borderLeftWidth = "0px";
        else turnIndicatorRule.style.borderLeftWidth = "2px";

        if (width > 10) turnIndicatorRule.style.width = "10px";
        else turnIndicatorRule.style.width = width + "px";
      };

      this.updateOnLoadSelection = ko.computed(
        function (load) {
          // TODO MDV
          this.recorder.stopIfRecording();
          var load = this.loadListViewModel.showLoad();
          var currentLoadIsSelected =
            this.loadListViewModel.currentLoadIsSelected();
      ...
      this.updateTurnIndicator = function (turn, severity) {
        if (this._currentIndicator.turn !== turn) {
          ...
          var currentIndex = this._severities.indexOf(
            this._currentIndicator.severity
          );
      ...
      this.updateSize = function () {
        ...
        var joinMessages =
          self.preIndicators().length +
          self.recordedIndicators().length +
          self.postIndicators().length;
        ...
        if (joinMessages < this.availableViewWidth) {
          ...
          if (self.indicators !== self.preIndicators)
            self._scaleMessages(self.preIndicators);
          if (self.indicators !== self.recordedIndicators)
            self._scaleMessages(self.recordedIndicators);
      ...
      this._scaleMessages = function (indicators) {
        ...
        for (var i = 0; i < len / 2; i++) {
          ...
          if (
            indicators()[i * 2].severity === "warn" ||
            indicators()[i * 2 + 1].severity === "warn"
          )
            indicator.severity = "warn";
          if (
            indicators()[i * 2].severity === "error" ||
            indicators()[i * 2 + 1].severity === "error"
          )
            indicator.severity = "error";
          if (indicators()[i * 2].severity === "turn")
          ...
          if (indicators()[i * 2 + 1].severity === "turn") {
            // If a indicator is a Turn then we'll take it as more relevant than previos indicators
            indicator.severity = "turn";
      ...
      this._clearIndicators = function () {
        // TODO vs resetMessages
        this.preIndicators([]);
        this.postIndicators([]);
    ...
    collapseTurns: function (node, event) {
      ...
      setTimeout(function () {
        document.querySelector(".turnScrubberView").style.display = "block";
      }, 1);
    },
    ...
    turnInfo: function (indicator) {
      ...
      QuerypointPanel.TurnScrubberViewModel.turnViewModel.showTurn(
        indicator.turn.turnNumber
      );
      var dropDown = document.querySelector(".turnView");
