...
StandaloneTestRunner.prototype = {
  ...
  notifyDone: function () {
    ...
    function allLoaded(event) {
      scriptsLoaded += 1;
      if (scriptsLoaded === 2) viewerLoaded();
    }
  ...
  evaluateInWebInspector: function (callId, script) {
    var message = {
      command: "evaluateInWebInspector",
      callId: callId,
      script: script,
    };
    if (this._isLoaded) this._postMessages([message]);
    else this._pendingMessages.push(message);
  },
...
if (!window.testRunner) window.testRunner = new StandaloneTestRunner();
