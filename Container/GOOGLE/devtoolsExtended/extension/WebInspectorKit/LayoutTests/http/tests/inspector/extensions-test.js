...
var initialize_ExtensionsTest = function () {
  ...
  function onEvaluate(message, port) {
    var reply = WebInspector.extensionServer._dispatchCallback.bind(
    ...
    try {
      ...
      InspectorTest.addResult(
        "Exception while running: " + message.expression + "\n" + (e.stack || e)
      );
      InspectorTest.completeTest();
    }
  }

  WebInspector.extensionServer._registerHandler(
  ...
  InspectorTest.showPanel = function (panelId) {
    if (panelId === "extension")
      panelId =
        WebInspector.inspectorView._panelOrder[
          WebInspector.inspectorView._panelOrder.length - 1
        ];
...
function extension_showPanel(panelId, callback) {
  evaluateOnFrontend(
    "InspectorTest.showPanel(unescape('" + escape(panelId) + "')); reply();",
    callback
  );
}
