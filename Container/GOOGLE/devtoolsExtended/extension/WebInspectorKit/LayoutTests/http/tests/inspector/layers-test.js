function initialize_LayerTreeTests() {
  ...
  InspectorTest.labelForLayer = function (layer) {
    ...
    if (height <= 200 && width <= 200) label += " " + height + "x" + width;
  ...
  InspectorTest.dumpLayerTree = function (prefix, root) {
    ...
    if (!root) {
      ...
      if (!root) {
        InspectorTest.addResult(
          "No layer root, perhaps not in the composited mode! "
        );
    ...
    root
      .children()
      .forEach(
        InspectorTest.dumpLayerTree.bind(InspectorTest, prefix + "    ")
      );
  };

  InspectorTest.dumpLayerTreeOutline = function (prefix, root) {
    if (!prefix) prefix = "";
    if (!root) root = WebInspector.panel("layers")._layerTree._treeOutline;
    if (root.representedObject)
      InspectorTest.addResult(
        prefix + InspectorTest.labelForLayer(root.representedObject)
      );
    root.children.forEach(
      InspectorTest.dumpLayerTreeOutline.bind(InspectorTest, prefix + "    ")
    );
  };
  ...
  InspectorTest.dumpLayers3DView = function (prefix, root) {
    ...
    if (!root)
      root =
        WebInspector.panel("layers")._layers3DView._rotatingContainerElement;
    if (root.__layer)
      InspectorTest.addResult(
        prefix + InspectorTest.labelForLayer(root.__layer)
      );
    for (
  ...
  InspectorTest.evaluateAndRunWhenTreeChanges = function (
    expression,
    callback
  ) {
    function eventHandler() {
      InspectorTest._layerTreeModel.removeEventListener(
        WebInspector.LayerTreeModel.Events.LayerTreeChanged,
        eventHandler
      );
      callback();
    }
    InspectorTest._layerTreeModel.addEventListener(
      WebInspector.LayerTreeModel.Events.LayerTreeChanged,
      eventHandler
    );
  ...
  InspectorTest.findLayerByNodeIdAttribute = function (nodeIdAttribute) {
    ...
    function testLayer(layer) {
      if (!layer.nodeId()) return false;
      var node = WebInspector.domAgent.nodeForId(layer.nodeId());
      if (!node || node.getAttribute("id") !== nodeIdAttribute) return false;
      result = layer;
      return true;
    }
