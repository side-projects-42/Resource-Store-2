var initialize_IndexedDBTest = function () {
  InspectorTest.dumpIndexedDBTree = function () {
    ...
    var indexedDBTreeElement =
      WebInspector.panels.resources.indexedDBListTreeElement;
    ...
    for (var i = 0; i < indexedDBTreeElement.children.length; ++i) {
      ...
      for (var j = 0; j < databaseTreeElement.children.length; ++j) {
        ...
        InspectorTest.addResult(
          "        Object store: " + objectStoreTreeElement.titleText
        );
        ...
        for (var j = 0; j < objectStoreTreeElement.children.length; ++j) {
          ...
          InspectorTest.addResult(
            "            Index: " + indexTreeElement.titleText
          );
        }
      }
    }
  ...
  InspectorTest.evaluateWithCallback = function (
    frameId,
    methodName,
    parameters,
    callback
    ...
    var parametersString =
      'dispatchCallback.bind(this, "' + callbackIdPrefix + callbackId + '")';
  ...
  InspectorTest.createDatabase = function (frameId, databaseName, callback) {
    InspectorTest.evaluateWithCallback(
      frameId,
      "createDatabase",
      [databaseName],
      callback
  ...
  InspectorTest.createObjectStore = function (
    frameId,
    databaseName,
    objectStoreName,
    keyPath,
    ...
    InspectorTest.evaluateWithCallback(
      frameId,
      "createObjectStore",
      [databaseName, objectStoreName, keyPath, autoIncrement],
      callback
    );
  };

  InspectorTest.deleteObjectStore = function (
    frameId,
    databaseName,
    objectStoreName,
    callback
  ...
  InspectorTest.createObjectStoreIndex = function (
    frameId,
    databaseName,
    objectStoreName,
  ...
  InspectorTest.deleteObjectStoreIndex = function (
    frameId,
    databaseName,
    objectStoreName,
    objectStoreIndexName,
    ...
    InspectorTest.evaluateWithCallback(
      frameId,
      "deleteObjectStoreIndex",
      [databaseName, objectStoreName, objectStoreIndexName],
      callback
    );
  };

  InspectorTest.addIDBValue = function (
    frameId,
    databaseName,
    objectStoreName,
    value,
    ...
    InspectorTest.evaluateWithCallback(
      frameId,
      "addIDBValue",
      [databaseName, objectStoreName, value, key],
      callback
    );
  };
};
...
function doWithReadWriteTransaction(
  databaseName,
  objectStoreName,
  callback,
  commitCallback
...
function createObjectStoreIndex(
  callback,
  databaseName,
  objectStoreName,
  objectStoreIndexName,
...
function deleteObjectStoreIndex(
  callback,
  databaseName,
  objectStoreName,
  objectStoreIndexName
...
function addIDBValue(callback, databaseName, objectStoreName, value, key) {
  ...
  function withTransactionCallback(objectStore, commitCallback) {
    var request;
    if (key) request = objectStore.add(value, key);
    else request = objectStore.add(value);
    request.onerror = onIndexedDBError;
