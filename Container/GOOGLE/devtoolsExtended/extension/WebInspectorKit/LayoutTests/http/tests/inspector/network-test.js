...
var initialize_NetworkTest = function () {
    ...
    function nextStep(result) {
      if (!result) {
        InspectorTest.addResult(
          "This test can not be run as window.internals is not available."
        );
  ...
  InspectorTest.makeSimpleXHRWithPayload = function (
    method,
    url,
    async,
    payload,
  ...
  InspectorTest.makeXHR = function (
    method,
    url,
    async,
    user,
    ...
    function innerCallback(msg) {
      if (msg._messageText.indexOf("XHR loaded") !== -1) callback();
      else InspectorTest.addConsoleSniffer(innerCallback);
    }
...
function resetInspectorResourcesData() {
  if (!window.internals) return false;
