var initialize_DebuggerTest = function () {
  InspectorTest.startDebuggerTest = function (callback, quiet) {
    if (quiet !== undefined) InspectorTest._quiet = quiet;
    WebInspector.showPanel("sources");

    if (WebInspector.debuggerModel.debuggerEnabled()) startTest();
    else {
    ...
    function startTest() {
      ...
      InspectorTest.addSniffer(
        WebInspector.debuggerModel,
        "_pausedScript",
        InspectorTest._pausedScript,
        true
      );
      InspectorTest.addSniffer(
        WebInspector.debuggerModel,
        "_resumedScript",
        InspectorTest._resumedScript,
        true
      );
  ...
  InspectorTest.finishDebuggerTest = function (callback) {
    ...
    function disableDebugger() {
      if (!WebInspector.debuggerModel.debuggerEnabled()) completeTest();
      else {
        InspectorTest.addSniffer(
          WebInspector.debuggerModel,
          "_debuggerWasDisabled",
          debuggerDisabled
        );
  ...
  InspectorTest.completeDebuggerTest = function () {
    InspectorTest.finishDebuggerTest(
  ...
  InspectorTest.waitUntilPaused = function (callback) {
    ...
    else InspectorTest._waitUntilPausedCallback = callback;
  };

  InspectorTest.waitUntilResumed = function (callback) {
    callback = InspectorTest.safeWrap(callback);

    if (!InspectorTest._pausedScriptArguments) callback();
    else InspectorTest._waitUntilResumedCallback = callback;
  };
  ...
  InspectorTest._pausedScript = function (
    callFrames,
    reason,
    auxData,
    breakpointIds
  ...
  InspectorTest.showUISourceCode = function (uiSourceCode, callback) {
    ...
    if (sourceFrame.loaded) callback(sourceFrame);
    else
      InspectorTest.addSniffer(
        sourceFrame,
        "onTextEditorContentLoaded",
        callback && callback.bind(null, sourceFrame)
      );
  };
  ...
  InspectorTest.showScriptSource = function (scriptName, callback) {
    ...
    InspectorTest.addSniffer(
      ...
      "_addUISourceCode",
      InspectorTest.showScriptSource.bind(InspectorTest, scriptName, callback)
    );
  };
  ...
  InspectorTest.dumpScriptsNavigator = function (navigator, prefix) {
    ...
    InspectorTest.addResult(
      prefix + "Dumping ScriptsNavigator 'Content scripts' tab:"
    );
    dumpNavigatorTreeOutline(
      prefix,
      navigator._contentScriptsView._scriptsTree
    );
  ...
  InspectorTest.setBreakpoint = function (
    sourceFrame,
    lineNumber,
    condition,
    enabled
  ...
  InspectorTest.removeBreakpoint = function (sourceFrame, lineNumber) {
    sourceFrame._breakpointManager
      .findBreakpoint(sourceFrame._uiSourceCode, lineNumber)
      .remove();
  };
  ...
  InspectorTest._expandProperty = function (
    parentTreeElement,
    path,
    pathIndex,
    callback
    ...
    var propertyTreeElement = InspectorTest._findChildPropertyTreeElement(
      parentTreeElement,
      name
    );
    if (!propertyTreeElement) {
      InspectorTest.addResult(
        "Failed to expand property: " + path.slice(0, pathIndex).join(".")
      );
      InspectorTest.completeDebuggerTest();
      return;
    }
    propertyTreeElement.expand();
    InspectorTest.runAfterPendingDispatches(
      InspectorTest._expandProperty.bind(
        InspectorTest,
        propertyTreeElement,
        path,
        pathIndex,
  ...
  InspectorTest._findChildPropertyTreeElement = function (parent, childName) {
    ...
    for (var i = 0; i < children.length; i++) {
      ...
      if (property.name === childName) return treeElement;
    }
  };
  ...
  InspectorTest.queryScripts = function (filter) {
    ...
    for (var scriptId in WebInspector.debuggerModel._scripts) {
      ...
      if (!filter || filter(script)) scripts.push(script);
    }
    return scripts;
  };

  InspectorTest.createScriptMock = function (
    url,
    startLine,
    startColumn,
    isContentScript,
    ...
    var endColumn =
      lineCount === 1
        ...
        : source.length - source.lineEndings()[lineCount - 2];
    var hasSourceURL =
      !!source.match(/\/\/#\ssourceURL=\s*(\S*?)\s*$/m) ||
      !!source.match(/\/\/@\ssourceURL=\s*(\S*?)\s*$/m);
    ...
    script.requestContent = function (callback) {
      callback(source, false, "text/javascript");
    };
  ...
  InspectorTest.checkRawLocation = function (
    script,
    lineNumber,
    columnNumber,
    location
    ...
    InspectorTest.assertEquals(
      ...
      "Incorrect columnNumber"
    );
  };

  InspectorTest.checkUILocation = function (
    uiSourceCode,
    lineNumber,
    columnNumber,
    location
    ...
    InspectorTest.assertEquals(
      ...
      "Incorrect uiSourceCode, expected '" +
        (uiSourceCode ? uiSourceCode.originURL() : null) +
        "'," +
        " but got '" +
    ...
    InspectorTest.assertEquals(
      columnNumber,
      location.columnNumber,
      "Incorrect columnNumber, expected '" +
        columnNumber +
        "', but got '" +
        location.columnNumber +
        "'"
