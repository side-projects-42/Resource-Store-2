...
var initialize_ApplicationCacheTest = function () {
  InspectorTest.createAndNavigateIFrame = function (url, callback) {
    ...
    InspectorTest.evaluateInPage(
      "createAndNavigateIFrame(unescape('" + escape(url) + "'))"
    );
  ...
  InspectorTest.removeIFrame = function (frameId, callback) {
    var frame = WebInspector.resourceTreeModel.frameForId(frameId);
    InspectorTest.evaluateInPage(
      "removeIFrame(unescape('" + escape(frame.name) + "'))"
    );
    InspectorTest.addSniffer(
      WebInspector.ResourceTreeModel.prototype,
      "_frameDetached",
  ...
  InspectorTest.swapFrameCache = function (frameId) {
    var frame = WebInspector.resourceTreeModel.frameForId(frameId);
    InspectorTest.evaluateInPage(
      "swapFrameCache(unescape('" + escape(frame.name) + "'))"
    );
  };
  ...
  InspectorTest.dumpApplicationCacheTree = function () {
    ...
    var applicationCacheTreeElement =
      WebInspector.panels.resources.applicationCacheListTreeElement;
    ...
    for (var i = 0; i < applicationCacheTreeElement.children.length; ++i) {
      ...
      InspectorTest.addResult(
        "    Manifest URL: " + manifestTreeElement.manifestURL
      );
      ...
      for (var j = 0; j < manifestTreeElement.children.length; ++j) {
        ...
        InspectorTest.addResult(
          "        Frame: " + frameTreeElement.displayName
        );
      }
    }
  };
  ...
  InspectorTest.frameIdToString = function (frameId) {
    if (!InspectorTest.framesByFrameId) InspectorTest.framesByFrameId = {};
    var frame = WebInspector.resourceTreeModel.frameForId(frameId);
    if (!frame) frame = InspectorTest.framesByFrameId[frameId];
  ...
  InspectorTest.applicationCacheStatusToString = function (status) {
    ...
    return (
      statusInformation[status] || statusInformation[applicationCache.UNCACHED]
    );
  };
  ...
  InspectorTest.dumpApplicationCacheModel = function () {
    ...
    var frameIds = [];
    for (var frameId in model._manifestURLsByFrame) frameIds.push(frameId);

    function compareFunc(a, b) {
      return InspectorTest.frameIdToString(a).localeCompare(
        InspectorTest.frameIdToString(b)
      );
    }
    ...
    for (var i = 0; i < frameIds.length; ++i) {
      ...
      InspectorTest.addResult(
        "    Frame: " + InspectorTest.frameIdToString(frameId)
      );
      InspectorTest.addResult("        manifest url: " + manifestURL);
      InspectorTest.addResult(
        "        status:       " +
          InspectorTest.applicationCacheStatusToString(status)
      );
    }
  };

  InspectorTest.waitForFrameManifestURLAndStatus = function (
    frameId,
    manifestURL,
    status,
    callback
  ) {
    var frameManifestStatus =
      WebInspector.panels.resources._applicationCacheModel.frameManifestStatus(
        frameId
      );
    var frameManifestURL =
      WebInspector.panels.resources._applicationCacheModel.frameManifestURL(
        frameId
      );
    if (
      frameManifestStatus === status &&
      frameManifestURL.indexOf(manifestURL) !== -1
    ) {
      callback();
      return;
    }

    var handler = InspectorTest.waitForFrameManifestURLAndStatus.bind(
      this,
      frameId,
      manifestURL,
      status,
  ...
  InspectorTest.startApplicationCacheStatusesRecording = function () {
    ...
    function addRecord(frameId, manifestURL, status) {
      ...
      if (
        ...
        InspectorTest.awaitedFrameStatusEventsCount[frameId]
      ) {
  ...
  InspectorTest.ensureFrameStatusEventsReceived = function (
    frameId,
    count,
    callback
  ) {
