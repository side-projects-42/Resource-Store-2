var initialize_InspectorTest = function () {
  ...
  function consoleOutputHook(messageType) {
    InspectorTest.addResult(
      messageType + ": " + Array.prototype.slice.call(arguments, 1)
    );
  }
  ...
  InspectorTest.Output = {
    // override in window.initialize_yourName
    testComplete: function () {
      RuntimeAgent.evaluate(
        "didEvaluateForTestInFrontend(" +
          InspectorTest.completeTestCallId +
          ', "")',
        "test"
      );
    },
  ...
  InspectorTest.evaluateInConsole = function (code, callback) {
    ...
    InspectorTest.addConsoleSniffer(function (commandResult) {
  ...
  InspectorTest.evaluateInPageWithTimeout = function (code) {
    InspectorTest.evaluateInPage(
      "setTimeout(unescape('" + escape(code) + "'))"
    );
  };

  InspectorTest.addResult = function (text) {
    results.push(text);
    if (resultsSynchronized) InspectorTest.Output.addResult(text);
    else {
  ...
  function onError(event) {
    ...
    InspectorTest.addResult(
      "Uncaught exception in inspector front-end: " +
        event.message +
        " [" +
        event.filename +
        ":" +
  ...
  InspectorTest.addObject = function (
    object,
    customFormatters,
    prefix,
    firstLinePrefix
  ...
  InspectorTest.addArray = function (
    array,
    customFormatters,
    prefix,
    firstLinePrefix
  ...
  InspectorTest.dump = function (
    ...
    if (prefixWithName && prefixWithName.length > 80) {
      InspectorTest.addResult(
        prefixWithName + "was skipped due to prefix length limit"
      );
      return;
    }
    if (value === null) InspectorTest.addResult(prefixWithName + "null");
    ...
    else InspectorTest.addResult(prefixWithName + value);
  };

  InspectorTest.assertGreaterOrEqual = function (a, b, message) {
    if (a < b)
      InspectorTest.addResult(
        "FAILED: " + (message ? message + ": " : "") + a + " < " + b
      );
  };
  ...
  InspectorTest.hardReloadPage = function (
    callback,
    scriptToEvaluateOnLoad,
    scriptPreprocessor
  ) {
    InspectorTest._innerReloadPage(
      true,
      callback,
      scriptToEvaluateOnLoad,
      scriptPreprocessor
  ...
  InspectorTest._innerReloadPage = function (
    hardReload,
    callback,
    scriptToEvaluateOnLoad,
    scriptPreprocessor
    ...
    if (WebInspector.panels.network) WebInspector.panels.network._reset();
  ...
  InspectorTest.createKeyEvent = function (
    keyIdentifier,
    ctrlKey,
    altKey,
    shiftKey,
  ...
  InspectorTest.assertEquals = function (expected, found, message) {
    ...
    var error;
    if (message) error = "Failure (" + message + "):";
    else error = "Failure:";
    throw new Error(
  ...
  InspectorTest.safeWrap = function (func, onexception) {
    function result() {
      ...
      try {
        ...
        if (onexception) InspectorTest.safeWrap(onexception)();
        else InspectorTest.completeTest();
      }
    }
    return result;
  };

  InspectorTest.addSniffer = function (
    receiver,
    methodName,
    override,
    opt_sticky
    ...
    receiver[methodName] = function (var_args) {
      try {
        ...
        if (!opt_sticky) receiver[methodName] = original;
      }
  ...
  InspectorTest.override = function (
    receiver,
    methodName,
    override,
    opt_sticky
  ...
  InspectorTest.textContentWithLineBreaks = function (node) {
    ...
    while ((currentNode = currentNode.traverseNextNode(node))) {
      ...
      else if (currentNode.nodeName === "LI") buffer += "\n    ";
...
function runAfterIframeIsLoaded() {
  ...
  function step() {
    if (!window.iframeLoaded) setTimeout(step, 100);
    else runTest();
  }
  setTimeout(step, 100);
}
...
function runTest(enableWatchDogWhileDebugging) {
  ...
  function runTestInFrontend(testFunction, completeTestCallId) {
    if (InspectorTest.completeTestCallId) return;
...
function didEvaluateForTestInFrontend(callId) {
  if (callId !== completeTestCallId) return;
...
function closeInspectorAndNotifyDone() {
  if (window._watchDogTimer) clearTimeout(window._watchDogTimer);
