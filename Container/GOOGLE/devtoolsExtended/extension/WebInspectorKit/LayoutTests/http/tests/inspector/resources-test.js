var initialize_ResourceTest = function () {
  ...
  InspectorTest.HARPropertyFormattersWithSize = JSON.parse(
    JSON.stringify(InspectorTest.HARPropertyFormatters)
  );
  ...
  InspectorTest.runAfterResourcesAreFinished = function (
    resourceURLs,
    callback
  ) {
    ...
    function checkResources() {
      for (url in resourceURLsMap) {
        ...
        if (resource) delete resourceURLsMap[url];
      }
  ...
  InspectorTest.showResource = function (resourceURL, callback) {
    ...
    function showResourceCallback() {
      ...
      var sourceFrame =
        WebInspector.panels.resources._resourceViewForResource(resource);
      if (sourceFrame.loaded) callbackWrapper(sourceFrame);
      else
        InspectorTest.addSniffer(
          sourceFrame,
          "onTextEditorContentLoaded",
          callbackWrapper.bind(null, sourceFrame)
        );
    }
    InspectorTest.runAfterResourcesAreFinished(
      [resourceURL],
