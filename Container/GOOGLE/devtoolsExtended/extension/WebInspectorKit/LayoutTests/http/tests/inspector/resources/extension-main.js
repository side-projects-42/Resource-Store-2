function trimURL(url) {
  if (!url) return;
  if (/^data:/.test(url)) return url.replace(/,.*$/, "...");
  return url.replace(/.*\//, ".../");
}
...
function dumpObject(object, nondeterministicProps, prefix, firstLinePrefix) {
  ...
  for (var i = 0; i < propertyNames.length; ++i) {
    ...
    if (nondeterministicProps && prop in nondeterministicProps) {
      var value =
        nondeterministicProps[prop] === "url"
          ? trimURL(propValue)
          : "<" + typeof propValue + ">";
    ...
    else output(prefixWithName + propValue);
  }
  output(prefix + "}");
}
...
function dumpArray(result) {
  if (result instanceof Array) {
    for (var i = 0; i < result.length; ++i) output(result[i]);
  } else output(result);
}

function evaluateOnFrontend(expression, callback) {
  window._extensionServerForTests.sendRequest(
    { command: "evaluateForTestInFrontEnd", expression: expression },
    callback
  );
}
...
function output(message) {
  evaluateOnFrontend(
    "InspectorTest.addResult(unescape('" + escape(message) + "'));"
  );
}
...
function onError(event) {
  ...
  output(
    "Uncaught exception in extension context: " +
      event.message +
      " [" +
      event.filename +
      ":" +
