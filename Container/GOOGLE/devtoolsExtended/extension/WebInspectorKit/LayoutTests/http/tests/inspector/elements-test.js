var initialize_ElementTest = function () {
    ...
    function processChildren(node) {
      try {
        ...
        var children = (node.children() || [])
          .concat(node.shadowRoots())
          .concat(Object.values(node.pseudoElements() || {}));
  ...
  InspectorTest.selectNodeWithId = function (idValue, callback) {
    ...
    function onNodeFound(node) {
      if (node) WebInspector._updateFocusedNode(node.id);
      callback(node);
    }
  ...
  InspectorTest.waitForStyles = function (idValue, callback, requireRebuild) {
    ...
    (function sniff(node, rebuild) {
      if (
        (rebuild || !requireRebuild) &&
        node &&
        node.getAttribute("id") === idValue
      ) {
        callback();
        return;
      }
      InspectorTest.addSniffer(
        WebInspector.StylesSidebarPane.prototype,
        "_nodeStylesUpdatedForTest",
        sniff
      );
    })(null);
  };
  ...
  InspectorTest.selectNodeAndWaitForStylesWithComputed = function (
    idValue,
    callback
  ) {
    callback = InspectorTest.safeWrap(callback);
  ...
  InspectorTest.dumpSelectedElementStyles = function (
    ...
    function extractText(element) {
      ...
      var name = anchor.uiSourceCode
        ? anchor.uiSourceCode.name()
        : new WebInspector.ParsedURL(anchor.href).lastPathComponent;
    ...
    function buildMarkedSelectors(element) {
      ...
      for (var node = element.firstChild; node; node = node.nextSibling) {
        if (
          ...
          node.className === "selector-matches"
        )
    ...
    var styleSections =
      WebInspector.panels.elements.sidebarPanes.styles.sections;
    for (var pseudoId in styleSections) {
      ...
      for (var i = 0; i < sections.length; ++i) {
        ...
        if (section.computedStyle && excludeComputed) continue;
        if (section.rule && excludeMatched) continue;
        if (
          section.element.previousSibling &&
          section.element.previousSibling.className === "sidebar-separator"
        )
          InspectorTest.addResult(
            "======== " +
              section.element.previousSibling.textContent +
              " ========"
          );
        InspectorTest.addResult(
          (section.expanded ? "[expanded] " : "[collapsed] ") +
            (section.noAffect ? "[no-affect] " : "")
        );
        var chainEntries = section.titleElement.children;
        for (var j = 0; j < chainEntries.length; ++j) {
          var chainEntry = chainEntries[j];
          var entryLine = includeSelectorGroupMarks
            ? buildMarkedSelectors(chainEntry.children[1])
            : chainEntry.children[1].textContent;
        ...
        section.expand();
        InspectorTest.dumpStyleTreeOutline(
          section.propertiesTreeOutline,
          omitLonghands ? 1 : 2
        );
        InspectorTest.addResult("");
      }
  ...
  InspectorTest.expandAndDumpSelectedElementEventListeners = function (
    callback
  ) {
  ...
  InspectorTest.expandSelectedElementEventListenersSubsections = function (
    callback
  ) {
    var eventListenerSections =
      WebInspector.panels.elements.sidebarPanes.eventListeners.sections;
  ...
  InspectorTest.expandSelectedElementEventListenersEventBars = function (
    callback
  ) {
    var eventListenerSections =
      WebInspector.panels.elements.sidebarPanes.eventListeners.sections;
  ...
  InspectorTest.dumpSelectedElementEventListeners = function (callback) {
    function formatSourceNameProperty(value) {
      return (
        "[clipped-for-test]/" +
        value.replace(/(.*?\/)LayoutTests/, "LayoutTests")
      );
    }
  ...
  InspectorTest.dumpObjectPropertySection = function (section, formatters) {
    ...
    InspectorTest.addResult(
      expandedSubstring +
        " " +
        section.titleElement.textContent +
        " " +
        section.subtitleAsTextForTest
    );
    if (!section.propertiesForTest) return;
    ...
    for (var i = 0; i < section.propertiesForTest.length; ++i) {
      ...
      if (key in formatters) value = formatters[key](value);
  ...
  InspectorTest.dumpObjectPropertySectionDeep = function (section) {
    ...
    function dumpTreeElementRecursively(treeElement, prefix) {
      if ("nameElement" in treeElement)
        InspectorTest.addResult(
          prefix +
            domNodeToString(treeElement.nameElement) +
            " => " +
            domNodeToString(treeElement.valueElement)
        );
      else InspectorTest.addResult(prefix + treeElement.title);
  ...
  InspectorTest.getElementStylePropertyTreeItem = function (propertyName) {
    var styleSections =
      WebInspector.panels.elements.sidebarPanes.styles.sections[0];
    var elementStyleSection = styleSections[1];
    return InspectorTest.getFirstPropertyTreeItemForSection(
      elementStyleSection,
      propertyName
    );
  };
  ...
  InspectorTest.getMatchedStylePropertyTreeItem = function (propertyName) {
    ...
    for (var i = 1; i < styleSections.length; ++i) {
      var treeItem = InspectorTest.getFirstPropertyTreeItemForSection(
        styleSections[i],
        propertyName
      );
  ...
  InspectorTest.getFirstPropertyTreeItemForSection = function (
    section,
    propertyName
  ) {
    ...
    for (var i = 0; i < outline.children.length; ++i) {
      ...
      if (treeItem.name === propertyName) return treeItem;
    }
    return null;
  };
  ...
  InspectorTest.dumpStyleTreeItem = function (treeItem, prefix, depth) {
    ...
    // potential diffs.
    if (
      !treeItem.listItemElement.textContent.indexOf("width") ||
      !treeItem.listItemElement.textContent.indexOf("height")
    )
      return;

    if (treeItem.listItemElement.hasStyleClass("inherited")) return;
    var typePrefix = "";
    if (
      treeItem.listItemElement.hasStyleClass("overloaded") ||
      treeItem.listItemElement.hasStyleClass("inactive") ||
      treeItem.listItemElement.hasStyleClass("not-parsed-ok")
    )
    ...
    if (textData) textContent += textData.getAttribute("data-uncopyable");
  ...
  InspectorTest.dumpElementsTree = function (rootNode, depth, resultsArray) {
    ...
    function dumpMap(name, map) {
      var result = [];
      for (var id in map) result.push(id + "=" + map[id]);
      if (!result.length) return "";
      return name + ":[" + result.join(",") + "]";
    }

    function userPropertyDataDump(treeItem) {
      if (treeItem._elementCloseTag) return "";
      ...
      if (node) {
        ...
        var dump = dumpMap(
          "descendantUserAttributeCounters",
          node._descendantUserPropertyCounters
        );
        if (dump) {
          if (userProperties) userProperties += ", ";
          userProperties += dump;
        }
        if (userProperties) userProperties = " [" + userProperties + "]";
      }
    ...
    function print(treeItem, prefix, depth) {
      if (treeItem.listItemElement) {
        ...
        var value =
          prefix +
          expander +
          beautify(treeItem.listItemElement) +
          userProperties;
        if (resultsArray) resultsArray.push(value);
        else InspectorTest.addResult(value);
      }
      ...
      for (var i = 0; depth && children && i < children.length; ++i) {
        ...
        else print(children[i], prefix, depth);
      }
    }
    ...
    print(
      rootNode ? treeOutline.findTreeElement(rootNode) : treeOutline,
      "",
      depth || 10000
    );
  };
  ...
  InspectorTest.expandElementsTree = function (callback) {
    ...
    WebInspector.showPanel("elements");
    InspectorTest.findNode(function () {
      return false;
    }, onAllNodesAvailable);
  };

  InspectorTest.dumpDOMAgentTree = function (node) {
    if (!WebInspector.domAgent._document) return;

    function dump(node, prefix) {
      InspectorTest.addResult(prefix + node.nodeName());
      prefix = prefix + "    ";

      if (node.templateContent()) dump(node.templateContent(), prefix);
      var shadowRoots = node.shadowRoots();
      for (var i = 0; i < shadowRoots.length; ++i) dump(shadowRoots[i], prefix);
      var children = node.children();
      for (var i = 0; children && i < children.length; ++i)
  ...
  InspectorTest.generateUndoTest = function (testBody) {
    function result(next) {
      var testNode = InspectorTest.expandedNodeWithId(
        /function\s([^(]*)/.exec(testBody)[1]
      );
  ...
  InspectorTest.dumpRule = function (rule, currentIndent) {
    function selectorRange() {
      if (!rule.selectorList || !rule.selectorList.range) return "";
    ...
    if (rule.type === "media") {
      InspectorTest.addResult(
        currentIndent + "@media " + rule.mediaText + " {"
      );
    ...
    if (rule.type === "import") {
      InspectorTest.addResult(
        currentIndent +
          "@import: header=" +
          InspectorTest.rangeText(rule.headerRange) +
          ", body=" +
          InspectorTest.rangeText(rule.bodyRange)
      );
      return;
    }
      ...
      if (rule.type === "page")
        ...
        InspectorTest.addResult(
          currentIndent +
            ...
            rule.type +
            " " +
            (rule.selectorList.text ? rule.selectorList.text + " " : "") +
            "{"
        );
      InspectorTest.dumpStyle(rule.style, currentIndent + indent);
      InspectorTest.addResult(currentIndent + "}");
      ...
      if (property.status !== "disabled")
        InspectorTest.addResult(
          currentIndent +
            ...
            property.value +
            "'" +
            (property.priority === "important" ? " is-important" : "") +
        ...
        InspectorTest.addResult(
          currentIndent + "[text='" + property.text + "'] " + property.status
        );
    }
  };
};
