var initialize_Timeline = function () {
  ...
  InspectorTest.startTimeline = function (callback) {
    ...
    WebInspector.timelineManager.addEventListener(
      ...
      InspectorTest._addTimelineEvent
    );
  };
  ...
  InspectorTest.performActionsAndPrint = function (
    actions,
    typeName,
    includeTimeStamps
  ) {
  ...
  InspectorTest.printTimelineRecords = function (typeName, formatter) {
    InspectorTest.innerPrintTimelineRecords(
      InspectorTest._timelineRecords,
      typeName,
      formatter
    );
  ...
  InspectorTest.printTimestampRecords = function (typeName, formatter) {
    InspectorTest.innerPrintTimelineRecords(
      WebInspector.panels.timeline._presentationModel
        .eventDividerRecords()
        .select("_record"),
      typeName,
      formatter
  ...
  InspectorTest.innerPrintTimelineRecords = function (
    records,
    typeName,
    formatter
  ) {
    for (var i = 0; i < records.length; ++i) {
      if (
        typeName &&
        records[i].type === WebInspector.TimelineModel.RecordType[typeName]
      )
        InspectorTest.printTimelineRecordProperties(records[i]);
      if (formatter) formatter(records[i]);
    }
  };
  ...
  InspectorTest.dumpTimelineRecord = function (record, level) {
    ...
    var prefix = "";
    var suffix = "";
    for (var i = 0; i < level; ++i) prefix = "----" + prefix;
    if (level > 0) prefix = prefix + "> ";
    if (record.type === WebInspector.TimelineModel.RecordType.TimeStamp) {
      suffix = " : " + record.data.message;
    }
    InspectorTest.addResult(
      prefix + InspectorTest._timelineAgentTypeToString(record.type) + suffix
    );
  ...
  InspectorTest.printTimelineRecordProperties = function (record) {
    InspectorTest.addResult(
      InspectorTest._timelineAgentTypeToString(record.type) + " Properties:"
    );
