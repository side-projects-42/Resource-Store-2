var initialize_FileSystemTest = function () {
  ...
  InspectorTest.createDirectory = function (path, callback) {
    InspectorTest.evaluateInPage(
      'createDirectory(escape("' +
        escape(path) +
        '"), ' +
        InspectorTest.registerCallback(callback) +
        ")"
    );
  };
  ...
  InspectorTest.clearFileSystem = function (callback) {
    InspectorTest.evaluateInPage(
      "clearFileSystem(" + InspectorTest.registerCallback(callback) + ")"
    );
  };

  InspectorTest.dumpFileSystemRootRequestResult = function (
    errorCode,
    backendRootEntry
  ) {
    ...
    else InspectorTest.addResult("backendRootEntry: (null)");
    InspectorTest.addResult("");
  };

  InspectorTest.dumpDirectoryContentRequestResult = function (
    errorCode,
    entries
  ) {
    InspectorTest.addResult("errorCode: " + errorCode);
  ...
  InspectorTest.dumpMetadataRequestResult = function (errorCode, metadata) {
    ...
    if (metadata) {
      ...
      InspectorTest.addResult(
        "  modificationTime: " +
          ("modificationTime" in metadata ? "(exists)" : "(null)")
      );
      InspectorTest.addResult(
        "  size: " + ("size" in metadata ? metadata.size : "(null)")
      );
    } else {
  ...
  InspectorTest.dumpFileContentRequestResult = function (
    errorCode,
    content,
    charset
  ) {
    ...
    if (content) InspectorTest.addResult('content: "' + content + '"');
    else InspectorTest.addResult("content: (null)");
...
function dispatchCallback() {
  ...
  testRunner.evaluateInWebInspector(
    999,
    'InspectorTest.dispatchCallback(unescape("' + escape(args) + '"))'
  );
}
...
function writeFile(path, content, callback) {
  ...
  function didGetWriter(writer) {
    ...
    writer.onwrite = function () {
      if (writer.readyState === writer.DONE) callback();
    };
  }
}
...
function clearFileSystem(callback) {
  webkitResolveLocalFileSystemURL(
    "filesystem:" + location.origin + "/temporary/",
    didGetRoot,
    onError
  );
  ...
  function didGetRoot(root) {
    ...
    function didReadEntries(newEntries) {
      ...
      for (var i = 0; i < newEntries.length; ++i) entries.push(newEntries[i]);
      reader.readEntries(didReadEntries);
    }
    ...
    function removeAll() {
      ...
      if (entry.isDirectory) entry.removeRecursively(removeAll);
      else entry.remove(removeAll);
    }
  }
