var initialize_IsolatedFileSystemTest = function () {
  InspectorTest.createIsolatedFileSystemManager = function (
    workspace,
    fileSystemMapping
  ) {
    var manager = new MockIsolatedFileSystemManager();
    manager.fileSystemWorkspaceProvider =
      new WebInspector.FileSystemWorkspaceProvider(manager, workspace);
  ...
  MockIsolatedFileSystem.prototype = {
    ...
    requestMetadata: function (path, callback) {
      ...
      callback(
        new Date(this._modificationTimestamps[path]),
        this._files[path].length
      );
    },

    setFileContent: function (path, newContent, callback) {
      this._files[path] = newContent;
      this._modificationTimestamps[path] =
        (this._modificationTimestamps[path] ||
          this.originalTimestamp - this.modifiationTimestampDelta) +
    ...
    _innerRequestFilesRecursive: function () {
      ...
      for (var i = 0; i < files.length; ++i) {
        ...
        for (var j = 0; j < files[i].length; ++j) {
          if (files[i][j] === "/") {
            if (
              this._manager.fileSystemMapping.isFileExcluded(
                this._path,
                files[i].substr(0, j + 1)
              )
            )
              isExcluded = true;
          }
  ...
  MockIsolatedFileSystemManager.prototype = {
    addMockFileSystem: function (path, skipAddFileSystem) {
      ...
      if (!skipAddFileSystem) this.fileSystemMapping.addFileSystem(path);
