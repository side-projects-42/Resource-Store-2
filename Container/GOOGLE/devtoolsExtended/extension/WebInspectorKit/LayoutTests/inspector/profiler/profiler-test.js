var initialize_ProfilerTest = function () {
  InspectorTest.startProfilerTest = function (callback) {
    ...
    InspectorTest.addSniffer(
      ...
      "_addProfileHeader",
      InspectorTest._profileHeaderAdded,
      true
    );
    InspectorTest.addSniffer(
      WebInspector.CPUProfileView.prototype,
      "refresh",
      InspectorTest._profileViewRefresh,
      true
    );
  ...
  InspectorTest.waitUntilProfileViewIsShown = function (title, callback) {
    ...
    if (
      profilesPanel.visibleView &&
      profilesPanel.visibleView.profile &&
      profilesPanel.visibleView.profile.title === title
    )
      callback(profilesPanel.visibleView);
    else
      InspectorTest._waitUntilProfileViewIsShownCallback = {
        title: title,
        callback: callback,
      };
  };
  ...
  InspectorTest._profileViewRefresh = function () {
    ...
    if (
      InspectorTest._waitUntilProfileViewIsShownCallback &&
      InspectorTest._waitUntilProfileViewIsShownCallback.title ===
        this.profile.title
    ) {
