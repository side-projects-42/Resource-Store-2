...
var initialize_ExtensionsAuditsTest = function () {
  InspectorTest.startExtensionAudits = function (callback) {
    ...
    var extensionCategories = document.evaluate(
      "label[starts-with(.,'Extension ')]/input[@type='checkbox']",
      WebInspector.panels.audits._launcherView._categoriesElement,
      null,
    ...
    InspectorTest.addSniffer(
      WebInspector.panels.audits,
      "auditFinishedCallback",
      onAuditsDone,
      true
    );
  ...
  InspectorTest.dumpAuditProgress = function () {
    ...
    InspectorTest.addResult(
      "Progress: " + Math.round((100 * progress.value) / progress.max) + "%"
    );
  };
};
