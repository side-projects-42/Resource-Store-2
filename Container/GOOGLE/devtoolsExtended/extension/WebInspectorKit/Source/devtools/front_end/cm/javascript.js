...
CodeMirror.defineMode("javascript", function (config, parserConfig) {
  ...
  function maybeoperatorNoComma(type, value, me) {
    ...
    if (type == "[")
      return cont(pushlex("]"), expression, expect("]"), poplex, me);
  }
  ...
  function maybeelse(indent) {
    return function (type, value) {
      if (type == "keyword b" && value == "else") {
        cx.state.lexical = new JSLexical(
          indent,
          0,
          "form",
          null,
  ...
  return {
    ...
    indent: function (state, textAfter) {
      ...
      if (parserConfig.statementIndent != null) {
        if (type == ")" && lexical.prev && lexical.prev.type == "stat")
          lexical = lexical.prev;
        if (lexical.type == "stat")
          return lexical.indented + parserConfig.statementIndent;
      }
