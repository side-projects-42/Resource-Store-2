  ...
  this._dataGrid = new WebInspector.DataGrid(
    [
      ...
      },
      {
        ...
        sortable: false,
        alig: WebInspector.DataGrid.Align.Right,
        weight: 5,
      },
...
WebInspector.ResourceWebSocketFrameView.prototype = {
  appendFrame: function (frame) {
    ...
    var row = {
      data: "",
      length:
        typeof payload.payloadData === "undefined"
          ? payload.errorMessage.length.toString()
          : payload.payloadData.length.toString(),
    ...
    if (payload.errorMessage) {
      ...
      payload.opcode ==
      WebInspector.ResourceWebSocketFrameView.OpCodes.TextFrame
    ) {
      if (payload.sent) rowClass = "outcoming";
      ...
      switch (payload.opcode) {
        ...
        case WebInspector.ResourceWebSocketFrameView.OpCodes.BinaryFrame:
          ...
          break;
        case WebInspector.ResourceWebSocketFrameView.OpCodes
      ...
      row.data = WebInspector.UIString(
        "%s (Opcode %d%s)",
        opcodeMeaning,
        payload.opcode,
        payload.mask ? ", mask" : ""
  ...
  _onContextMenu: function (contextMenu, node) {
    contextMenu.appendItem(
      WebInspector.UIString(
        WebInspector.useLowerCaseMenuTitles() ? "Copy message" : "Copy Message"
      ),
      this._copyMessage.bind(this, node.data)
    );
  },
