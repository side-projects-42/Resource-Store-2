  ...
  chromeExtensionPipe.createFrom = function (crx2appBase) {
    ...
    var proxyEnd = {
      ...
      attach: function (callback) {
        ...
        if (debug) {
          console.log(
            "proxyChromePipe awaiting introduction from " + iframeDomain
          );
        }
      },
      ...
      fromExtnToApp: function (event) {
        if (event.origin === iframeDomain) {
          if (debug) {
            console.log(
              "crx2app.proxyChromePipe.fromExtnToApp in " + window.location,
              event
            );
          }
      ...
      fromAppToExtn: function (msgObj) {
        if (!this.listeners.length) {
          console.warning(
            "crx2app.appEnd.proxyChromePipe.fromAppToExtn: sending but not listening"
          );
        }
        if (debug) {
    ...
    return {
      // these functions are bound to proxyEnd, not the return object
