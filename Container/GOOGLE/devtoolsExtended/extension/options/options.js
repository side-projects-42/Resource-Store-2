...
function extractOptionsFromUI(options) {
  ...
  for (var i = 0; i < extensionInfosRows.length; i++) {
    ...
    var enabled =
      extensionInfosRows[i].querySelector(".extensionInfo-enable")
        .textContent == "yes";
    var startPage = extensionInfosRows[i].querySelector(
      ".extensionInfo-startPage"
    ).value;
...
var DevtoolsExtendedOptions = new ExtensionOptions(
  optionsKey,
  defaultExtensions,
  extractOptionsFromUI
);
...
function showNoExtensions(noshow) {
  ...
  if (noshow) noExtensions.classList.remove("hidden");
  else noExtensions.classList.add("hidden");
}
...
function addExtensionInfosRowClickHandler(row) {
  row.addEventListener("click", function (event) {
    ...
    if (event.target.classList.contains("remove")) removeRow(event);
    else if (
...
function addListeners() {
  ...
  addExtensionInfosRow.addEventListener(
    ...
    function (event) {
      edit(
        addExtensionInfosRowClickHandler(
