...
(function () {
  ...
  var atopwiURL =
    "chrome-extension://ggimboaoffjaeoblofehalflljohnfbl/atopwi/atopwi.html";
  ...
  var websocketJSONURL =
    "http://localhost:" + options.remoteDebugPort + "/json";
  ...
  function updateSync() {
    ...
    chrome.storage.sync.set(sync, function () {
      ...
      console.log(
        debuggerOrDebuggee + " sent remoteDebug in " + (sent - sending) + "ms",
        sync
      );
    });
  }
  ...
  function openDebugger(tab) {
    function findDevtoolsExtendedInJSON(entry) {
      ...
      if (entry.url === tab.url) {
        openRemoteDevtools(
          getWebSocketURLFromJSONEntry(entry),
          function onPopup(win) {
    ...
    if ("remoteDebug" in changes) {
      ...
      if (debuggerOrDebuggee === "debugger") {
        chrome.storage.sync.get("remoteDebug", function onStorage(items) {
          console.log(
            "storage.sync.get remoteDebug: %o, debuggers: %o",
            items.remoteDebug,
            debuggers
          );
      ...
      if (tabId in debuggers) {
        if (remoteDebug.focused === tabId) {
          chrome.windows.update(
            ...
            function () {
              console.log(
                "syncDebuggers tried to focus the debugger matching" + tabId
              );
            }
          );
        }
...
*/
  function openFromSync(tab) {
    ...
    chrome.storage.sync.get("remoteDebug", function onStorage(items) {
      ...
      if (!tab) {
        console.error(
          "Focused tab %s not among remote tabs %o",
          tabId,
          items.remoteDebug.tabs
        );
        return;
      }
      openDebugger(tab);
