  ...
  DebuggerAdapter.prototype = {
    ...
    chromeWrappers: {
      attach: function (serial, debuggee, version, callback) {
        ...
        chrome.debugger.attach(
          debuggee,
          version,
          this.onAttach.bind(this, serial, debuggee)
        );
      },
      ...
      sendCommand: function (serial, debuggee, method, params) {
        ...
        var commandResponse = function (response) {
          if (!chrome.extension.lastError) {
            if (debug) {
              console.log(
                serial + " crxEnd/DebuggerAdapter.commandResponse " + method,
                response
              );
            }
          } else {
            console.error(
              "sendCommand " + method + " FAILS " + chrome.extension.lastError,
        ...
        if (debug) {
          console.log(
            serial + " crxEnd/DebuggerAdapter.sendCommand " + method,
            params
          );
        }
    ...
    onAttach: function (serial, debuggee) {
      if (this.noErrorPosted({ serial: serial })) {
        if (debug) {
          console.log(
            serial + ":  crx2app.DebuggerAdapter.onAttach: " + debuggee.tabId
          );
        }

        this.addListeners();

        if (DebuggerAdapter.cleanUpDebuggees[debuggee.tabId]) {
          // then we reloaded the debugger
          window.clearTimeout(DebuggerAdapter.cleanUpDebuggees[debuggee.tabId]);
          if (debug) {
            var index = this.debuggeeTabIds.indexOf(debuggee.tabId);
            console.info(
              "crxEnd.DebuggerAdapter.onAttach cleared the cleanup of " +
                debuggee.tabId +
                " index: " +
                index
            );
