...
wtf.trace.providers.WebWorkerProvider = function (traceManager, options) {
  ...
  this.injecting_ = options.getBoolean(
    "wtf.trace.provider.webworker.inject",
    false
  );
...
wtf.trace.providers.WebWorkerProvider.prototype.injectMessagePort_ =
  function () {
    ...
    var startEvent = wtf.trace.events.createScope("MessagePort#start()");
    var originalStart = proto.start;
    this.injectFunction(proto, "start", function start() {
      var result;
      try {
    ...
    var closeEvent = wtf.trace.events.createScope("MessagePort#close()");
    var originalClose = proto.close;
    this.injectFunction(proto, "close", function close() {
      var result;
      try {
    ...
    var descriptor = wtf.trace.eventtarget.createDescriptor(
      "MessagePort",
      wtf.data.webidl.getAllEvents("MessagePort")
    );
  ...
  function () {
    ...
    function createInjectionShim(scriptUrl, workerId) {
      ...
      if (goog.string.startsWith(scriptUrl, "blob:")) {
        var xhr = new (goog.global["XMLHttpRequest"]["raw"] ||
          XMLHttpRequest)();
    ...
    for (var n = 0; n < eventInfos.length; n++) {
      ...
      Object.defineProperty(ProxyWorker.prototype, "on" + eventInfo.name, {
        configurable: false,
        enumerable: false,
