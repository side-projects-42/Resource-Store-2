...
wtf.replay.graphics.ui.ArgumentsDialog = function (it, dom) {
  goog.base(
    ...
    /** @type {!Element} */ (dom.getDocument().body),
    dom
  );
...
wtf.replay.graphics.ui.ArgumentsDialog.prototype.createDom = function (dom) {
  return /** @type {!Element} */ (
    goog.soy.renderAsFragment(
      wtf.replay.graphics.ui.argumentsdialog.control,
      null,
      dom
    )
  );
...
wtf.replay.graphics.ui.ArgumentsDialog.prototype.createForm_ = function () {
  ...
  var formTitle = /** @type {Element} */ (
    this.getChildElement(goog.getCssName("title"))
  );
  domHelper.setTextContent(
    formTitle,
    "Editing " + this.eventIterator_.getName()
  );

  // Listen for form submission events.
  var form = /** @type {HTMLFormElement} */ (
    this.getChildElement(goog.getCssName("form"))
  );
  var saveButton = this.getChildElement(goog.getCssName("saveArgumentsButton"));
 ...
 * @private
 */
wtf.replay.graphics.ui.ArgumentsDialog.prototype.updateArguments_ = function (
  newArgumentMap
) {
...
wtf.replay.graphics.ui.ArgumentsDialog.prototype.createLabelInputPair_ =
  function (key, value) {
    var domHelper = this.getDom();
    var labelInputPair = /** @type {!Element} */ (
      goog.soy.renderAsFragment(
