...
wtf.replay.graphics.ui.EventNavigatorTableSource.prototype.paintRowRange =
  function (ctx, bounds, scrollBounds, rowOffset, rowHeight, first, last) {
    ...
    var createContextTypeId = this.eventList_.getEventTypeId(
      "wtf.webgl#createContext"
    );
    var setContextTypeId = this.eventList_.getEventTypeId(
    ...
    for (var n = first; n <= last && !it.done(); n++, y += rowHeight) {
      ...
      if ((currentRow == -1 && !n) || currentRow == n - 1) {
        ...
        if (
          n &&
          filter &&
          this.matchedEventTypeIds_[it.getTypeId()] &&
          ...
          playback.isDrawCall(it) &&
          skipCalls &&
          skipCalls.isEventSkipped(it)
        ) {
          ctx.fillStyle = colors.SKIPPED_BACKGROUND;
        } else {
          ctx.fillStyle =
            n % 2 ? colors.ODD_ROW_BACKGROUND : colors.EVEN_ROW_BACKGROUND;
        }
      }
      ...
      if (n) {
        if (it.isScope() || it.isInstance()) {
          ...
          if (contextChangingEvents.length) {
            ...
            if (
              nextEntry < contextChangingEvents.length &&
          ...
          var contextHandleText =
            currentContextHandle != -1 ? "" + currentContextHandle : "-";
 ...
 * @override
 */
wtf.replay.graphics.ui.EventNavigatorTableSource.prototype.onClick = function (
  row,
  x,
  modifiers,
  bounds
...
wtf.replay.graphics.ui.EventNavigatorTableSource.prototype.handleEditClick_ =
  function (rowIndex) {
    ...
    var alterArgumentsDialog = new wtf.replay.graphics.ui.ArgumentsDialog(
      it,
      this.domHelper_
    );
    ...
    alterArgumentsDialog.addListener(
      ...
      goog.partial(this.markArgsAltered_, eventId),
      this
    );
    alterArgumentsDialog.addListener(
      wtf.replay.graphics.ui.ArgumentsDialog.EventType.ARGUMENTS_RESET,
      goog.partial(this.markArgsReset_, eventId),
      this
    );
  };
...
wtf.replay.graphics.ui.EventNavigatorTableSource.prototype.setSearchValue =
  function (value) {
    if (!value || !value.length) {
      ...
      this.matchedEventTypeIds_ = this.filter_.getMatchedEventTypes(
        this.eventList_.eventTypeTable
      );
    }
