...
wtf.replay.graphics.OverdrawSurface = function (gl, width, height, opt_args) {
   ...
   * @private
   */
  this.thresholdColors_ =
    (opt_args && /** @type {Array.<string>} */ (opt_args["thresholdColors"])) ||
    this.getDefaultColors_();
...
goog.inherits(
  wtf.replay.graphics.OverdrawSurface,
...
wtf.replay.graphics.OverdrawSurface.prototype.initialize = function () {
  ...
  for (var i = 0; i < this.thresholdColors_.length; i++) {
    ...
    thresholdFragmentSource +=
      ...
      " ) {" +
      "  outputColor = " +
      this.thresholdColors_[i] +
      ";" +
      "}";
  }
  thresholdFragmentSource += "" + "  gl_FragColor = vec4(outputColor);" + "}";
  ...
  var quadVertexSource =
    "attribute vec2 aVertexPosition;" +
    "void main(void) {" +
    "  gl_Position = vec4(aVertexPosition, 0.0, 1.0);" +
    "}";
  var quadFragmentSource =
    "precision mediump float;" +
    "void main(void) {" +
    "  gl_FragColor = " +
    this.getThresholdDrawColor() +
    ";" +
    "}";
  ...
  goog.asserts.assert(
    gl.getProgramParameter(quadProgram, goog.webgl.LINK_STATUS),
    "OverdrawSurface quad program did not link."
  );
