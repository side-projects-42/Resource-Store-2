...
wtf.tests.EventBuffers_test = suite("wtf.tests.EventBuffers", function () {
  ...
  test("basic", function () {
    ...
    assert.arraysEqual(
      wtf.io.BufferView.getUsedBytes(bufferView),
  ...
  test("capacity", function () {
    ...
    var ev = setupTestEvent("capacity(uint8[] ui)");
    ev.write(new Uint8Array([1, 2, 3, 4]), 1234567, bufferView);
    assert.arraysEqual(
      wtf.io.BufferView.getUsedBytes(bufferView),
  ...
  test("mixed", function () {
    ...
    assert.arraysEqual(
      wtf.io.BufferView.getUsedBytes(bufferView),
      [
        1, 0, 0, 0, 88, 255, 149, 73, 222, 0, 0, 0, 2, 0, 0, 0, 96, 121, 254,
        255, 192, 242, 252, 255, 151, 208, 0, 0, 0, 0, 0, 0, 3, 0, 0, 0, 12,
        212, 22, 0,
      ]
    );
    wtf.io.BufferView.setOffset(bufferView, 8);
  ...
  test("primitiveArgInt8", function () {
    var bufferView = wtf.io.BufferView.createEmpty(1024);
    var ev = setupTestEvent("primitiveArgInt8(int8 i, uint8 ui, uint8 ui_o)");
  ...
  test("primitiveArgInt8Array", function () {
    ...
    assert.arraysEqual(
      wtf.io.BufferView.getUsedBytes(bufferView),
      [
        1, 0, 0, 0, 88, 255, 149, 73, 4, 0, 0, 0, 255, 254, 253, 252, 3, 0, 0,
        0, 200, 201, 202, 0, 255, 255, 255, 255,
      ]
    );
    wtf.io.BufferView.setOffset(bufferView, 8);
    assert.deepEqual(ev.read(bufferView), {
  ...
  test("primitiveArgInt16", function () {
    ...
    assert.arraysEqual(
      wtf.io.BufferView.getUsedBytes(bufferView),
      [
        1, 0, 0, 0, 88, 255, 149, 73, 189, 224, 0, 0, 2, 83, 0, 0, 239, 190, 0,
        0,
      ]
    );
    wtf.io.BufferView.setOffset(bufferView, 8);
  ...
  test("primitiveArgInt16Array", function () {
    ...
    assert.arraysEqual(
      wtf.io.BufferView.getUsedBytes(bufferView),
      [
        1, 0, 0, 0, 88, 255, 149, 73, 4, 0, 0, 0, 24, 252, 48, 248, 72, 244, 96,
        240, 3, 0, 0, 0, 32, 78, 132, 78, 232, 78, 0, 0, 255, 255, 255, 255,
      ]
    );
    wtf.io.BufferView.setOffset(bufferView, 8);
    assert.deepEqual(ev.read(bufferView), {
  ...
  test("primitiveArgInt32", function () {
    var bufferView = wtf.io.BufferView.createEmpty(1024);
    var ev = setupTestEvent("primitiveArgInt32(int32 i, uint32 ui)");
  ...
  test("primitiveArgInt32Array", function () {
    ...
    assert.arraysEqual(
      wtf.io.BufferView.getUsedBytes(bufferView),
      [
        1, 0, 0, 0, 88, 255, 149, 73, 4, 0, 0, 0, 96, 121, 254, 255, 192, 242,
        252, 255, 32, 108, 251, 255, 128, 229, 249, 255, 3, 0, 0, 0, 128, 132,
        30, 0, 144, 171, 30, 0, 160, 210, 30, 0, 255, 255, 255, 255,
      ]
    );
    wtf.io.BufferView.setOffset(bufferView, 8);
    assert.deepEqual(ev.read(bufferView), {
  ...
  test("primitiveArgFloat32", function () {
    var bufferView = wtf.io.BufferView.createEmpty(1024);
    var ev = setupTestEvent("primitiveArgFloat32(float32 f)");
  ...
  test("primitiveArgFloat32Array", function () {
    ...
    assert.arraysEqual(
      wtf.io.BufferView.getUsedBytes(bufferView),
      [
        1, 0, 0, 0, 88, 255, 149, 73, 4, 0, 0, 0, 117, 147, 200, 191, 186, 73,
        36, 192, 186, 73, 100, 192, 221, 36, 146, 192, 255, 255, 255, 255,
      ]
    );
    wtf.io.BufferView.setOffset(bufferView, 8);
    var args = ev.read(bufferView);
  ...
  test("primitiveArgString", function () {
    ...
    assert.arraysEqual(
      wtf.io.BufferView.getUsedBytes(bufferView),
      [
        1, 0, 0, 0, 88, 255, 149, 73, 0, 0, 0, 0, 254, 255, 255, 255, 255, 255,
        255, 255,
      ]
    );
    wtf.io.BufferView.setOffset(bufferView, 8);
  ...
  test("primitiveArgChar", function () {
    ...
    assert.arraysEqual(
      wtf.io.BufferView.getUsedBytes(bufferView),
      [
        1, 0, 0, 0, 88, 255, 149, 73, 97, 0, 0, 0, 4, 0, 0, 0, 97, 98, 99, 100,
  ...
  test("primitiveArgWchar", function () {
    ...
    assert.arraysEqual(
      wtf.io.BufferView.getUsedBytes(bufferView),
      [
        1, 0, 0, 0, 88, 255, 149, 73, 66, 32, 0, 0, 4, 0, 0, 0, 241, 33, 242,
        33, 186, 33, 187, 33, 0, 0, 0, 0, 255, 255, 255, 255,
      ]
    );
    wtf.io.BufferView.setOffset(bufferView, 8);
    assert.deepEqual(ev.read(bufferView), {
  ...
  test("primitiveArgAny", function () {
    var bufferView = wtf.io.BufferView.createEmpty(1024);
    var ev = setupTestEvent("primitiveArgAny(any a, any a_empty, any a_null)");
