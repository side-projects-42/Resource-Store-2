...
wtf.db.EventIterator.prototype.seek = function (index) {
  ...
  var i = this.indirectionTable_
    ? this.indirectionTable_[this.index_]
    : this.index_;
 ...
 */
wtf.db.EventIterator.prototype.getArgumentString = function (
  opt_argumentNames
) {
  var type = this.getType();
  var s = [];
...
wtf.db.EventIterator.prototype.getChildFlows_ = function () {
  ...
  for (; !it.done(); it.next()) {
    ...
    if (type == branchId || type == extendId || type == terminateId) {
      ret = ret || [];
...
goog.exportProperty(
  wtf.db.EventIterator.prototype,
  "getCount",
