...
wtf.ui.Control.prototype.setupDropDownButton = function (
  ...
  var buttonWidth =
    goog.style.getSize(this.getChildElement(buttonStyle)).width - 2;
  menu.attach(
  ...
  for (var n = 0; n < items.length; n++) {
    menu.addChild(
      new goog.ui.MenuItem(items[n].name, items[n].command, this.dom_),
      true
    );
  }
  menu.render();
...
wtf.ui.Control.prototype.toggleInputEvents_ = function (value) {
  ...
  eh.listen(canvas, goog.events.EventType.MOUSEMOVE, function (e) {
    var x = e.offsetX;
    var y = e.offsetY;
  ...
  eh.listen(canvas, goog.events.EventType.MOUSEUP, function (e) {
    var x = e.offsetX;
    var y = e.offsetY;
    var modifiers = getEventModifiers(e);
    if (delta < 4) {
  ...
  eh.listen(canvas, goog.events.EventType.MOUSEOUT, function (e) {
    lastX = lastY = delta = 0;
    if (this.tooltip_) {
      this.tooltip_.hide();
    }
...
wtf.ui.Control.prototype.layout = function () {
  ...
  if (this.paintContext_) {
    ...
    wtf.util.canvas.reshape(canvas, ctx, currentSize.width, currentSize.height);
  }
  ...
  if (this.paintContext_ && this.scrollPaintContext_) {
    ...
    if (newDesiredSize.height != currentCanvasSize.height) {
      ...
      // Resize the canvas.
      wtf.util.canvas.reshape(canvas, ctx, currentCanvasSize.width, newHeight);
