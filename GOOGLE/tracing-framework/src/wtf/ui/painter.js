...
wtf.ui.Painter.prototype.layout = function (availableBounds, opt_extents) {
  ...
  remainingBounds.width = Math.max(0, availableBounds.width - newBounds.left);
  remainingBounds.height = Math.max(0, availableBounds.height - newBounds.top);
  ...
  if (opt_extents) {
    if (desiredBounds.width != INFINITE_SIZE) {
      opt_extents.width = Math.max(
        opt_extents.width,
        desiredBounds.left + desiredBounds.width
      );
    }
    if (desiredBounds.height != INFINITE_SIZE) {
      opt_extents.height = Math.max(
        opt_extents.height,
        desiredBounds.top + desiredBounds.height
      );
    }
  }
...
wtf.ui.Painter.prototype.recursiveRepaint_ = function () {
  ...
  // Skip if too small.
  if (this.drawBounds_.width <= 0 || this.drawBounds_.height <= 0) {
    return;
  }
...
wtf.ui.Painter.prototype.drawLabel = function (label, opt_y, opt_height) {
  ...
  if (height > nameHeight) {
    ...
    ctx.fillRect(
      bounds.left,
      bounds.top + y + (height + nameHeight) / 2 - nameHeight,
      textSize.width + 8,
      nameHeight + 2
    );
    ...
    ctx.fillText(
      label,
      bounds.left + 4,
      bounds.top + y + (height + nameHeight) / 2 - 1
    );
  }
  ctx.globalAlpha = 1;
};
...
wtf.ui.Painter.prototype.onClickInternal = function (x, y, modifiers, bounds) {
  return this.defaultClickHandler_
    ? this.defaultClickHandler_(x, y, modifiers, bounds)
    : undefined;
};
