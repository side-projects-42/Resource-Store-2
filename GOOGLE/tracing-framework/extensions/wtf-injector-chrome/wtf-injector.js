...
(function () {
  ...
  function main() {
    ...
    if (options["__instrumented__"]) {
      ...
      if (isTop) {
        ...
        xhr.open(
          "GET",
          chrome.extension.getURL("third_party/falafel.js"),
          false
        );
        xhr.send();
        ...
        var blob = new Blob([falafelSource, processSource], {
      ...
      if (isTop) {
        var traceScriptUrl = chrome.extension.getURL(
          "wtf_trace_web_js_compiled.js"
        );
        injectScriptFunction(
          function (traceScriptUrl) {
      ...
      if (isTop) {
        ...
        injectScriptFunction(startTracing, [addons]);
      }
    }
  ...
  function fetchOptions() {
    ...
    var blobUrl =
      "blob:" + chrome.extension.getURL(optionsUuid).replace(":", "%3A");
    var headerUrl =
