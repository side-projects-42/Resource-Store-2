...
(function (global) {
   ...
   */
  global["wtfi"]["process"] = function (sourceText, options, opt_url) {
    ...
    function isFunctionBlock(node) {
      ...
      // get foo() {} / set foo() {}
      var isProperty =
        (parent && parent.parent && parent.parent.kind == "get") ||
        ...
        if (node.type == "FunctionDeclaration") {
          ...
          if (tryCatchMode) {
            ...
            node.update(
              [
                "{",
                "var __wtfId=__WTFMID__|" +
                  fnId +
                  ",__wtfRet;__wtfEnter(__wtfId);",
                node.source(),
                "__wtfExit(__wtfId);",
                "}",
              ].join("")
  ...
  if (!global.document) {
    global.onmessage = function (e) {
      var data = e.data;
      var result = global.wtfi.process(data.sourceText, data.options, data.url);
      postMessage({
