...
function onLoad() {
  function resetProject(url) {
    ...
    if (model.qpPanel) model.qpPanel.disconnect();

    model.qpPanel = new view.window.QuerypointPanel.Panel(
      view.panel,
      view.window,
      model.project
    );
  ...
  function onNavigated(url) {
    if (!view.window)
      // Then our panel was never opened.
      return;
    var QPRuntimeInstalled = model.project && model.project.qpRuntimeInstalled;
    if (!model.project || model.project.url !== url) {
      if (model.qpPanel) model.qpPanel.save();
      resetProject(url);
    } else {
      // Same url, but maybe the user reloaded the page without us.
      model.project.onReload(model.qpPanel.pageWasReloaded.bind(model.qpPanel));
    }
    if (
      QPRuntimeInstalled &&
      model.project &&
      !model.project.qpRuntimeInstalling
    )
  ...
  chrome.devtools.panels.create(
    "Querypoint",
    "Panel/QuerypointIcon.png",
    "Panel/QuerypointPanel.html",
    function (panel) {
      view.panel = panel;
      var helpButton = panel.createStatusBarButton(
        "Panel/QuerypointHelpIcon.png",
        "Querypoint Panel Help",
        false
      );
      ...
      panel.onHidden.addListener(function () {
        if (model.qpPanel) model.qpPanel.onHidden();
      });
    }
  );
}
