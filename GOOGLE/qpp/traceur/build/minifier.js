...
function mergedCompilerMsgLists(list1, list2) {
  ...
  function lessThan(e1, e2) {
    if (e1.lineno < e2.lineno) return true;
  ...
  var i1 = 0,
    i2 = 0;
  while (true) {
    if (i1 >= list1.length) return list.concat(list2.slice(i2));
    if (i2 >= list2.length) return list.concat(list1.slice(i1));
    if (lessThan(list1[i1], list2[i2])) list.push(list1[i1++]);
    else list.push(list2[i2++]);
  }
}
...
var req = http.request(opt, function (res) {
  ...
  res.on("end", function () {
    ...
    if (json.errors || json.warnings)
      mergedCompilerMsgLists(json.errors, json.warnings).forEach(function (x) {
        printCompilerMsg(x, outfileName);
      });

    if (json.errors) process.exit(1);
