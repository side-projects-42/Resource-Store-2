...
CodeMirror.defineExtension("commentRange", function (isComment, from, to) {
  ...
  if (isComment) {
    ...
    this.replaceRange(
      curMode.commentStart + this.getRange(from, to) + curMode.commentEnd,
      from,
      to
    );
...
CodeMirror.defineExtension("autoFormatRange", function (from, to) {
  ...
  // mode's syntax
  var res = this.getModeExt().autoFormatLineBreaks(
    this.getValue(),
    absStart,
    absEnd
  );
    ...
    if (nonBreakableBlocks != null) {
      ...
      for (var i = 0; i < nonBreakableBlocks.length; i++) {
        if (nonBreakableBlocks[i].start > curPos) {
          ...
          res += text
            .substring(curPos, nonBreakableBlocks[i].start)
...
CodeMirror.modeExtensions["xml"] = {
  ...
  autoFormatLineBreaks: function (text, startPos, endPos) {
    ...
    var reProcessedPortion = new RegExp(
      "(^\\s*?<|^[^<]*?)(.+)(>\\s*?$|[^>]*?$)"
    );
...
CodeMirror.modeExtensions["htmlmixed"] = {
  ...
  autoFormatLineBreaks: function (text, startPos, endPos) {
    ...
    if (modeInfos.length > 1) {
      ...
      // Single-mode text
      res = modeInfos[0].modeExt.autoFormatLineBreaks(
