...
window.CodeMirror = (function () {
    ...
    for (var i = 0, cur; i < measure.length; ++i)
      if ((cur = measure[i])) {
        ...
        if (/\bCodeMirror-widget\b/.test(cur.className) && cur.getClientRects) {
          var rects = cur.getClientRects(),
            rLeft = rects[0],
            rRight = rects[rects.length - 1];
          if (rects.length == 1) {
            size = rLeft;
          } else {
            var vCatLeft = categorizeVSpan(
              rLeft.top - outer.top,
              rLeft.bottom - outer.top
            );
            var vCatRight = categorizeVSpan(
              rRight.top - outer.top,
              rRight.bottom - outer.top
            );
            data[i] = {
        ...
        var vCat = categorizeVSpan(
          size.top - outer.top,
          size.bottom - outer.top
        );
    ...
    for (;;) {
      ...
      if (
        ...
        (found.ch > mergedPos.from.ch ||
          (found.ch == mergedPos.from.ch && found.after))
      )
  ...
  function registerEventHandlers(cm) {
    ...
    else on(d.scroller, "dblclick", e_preventDefault);
