(function () {
    ...
    function scan(line, lineNo, start) {
      ...
      for (; pos != end; pos += d) {
        ...
        if (
          re.test(ch) &&
          (!checkTokenStyles ||
            cm.getTokenAt(Pos(lineNo, pos + 1)).type == style)
        ) {
  ...
  function matchBrackets(cm, autoclear) {
    ...
    if (
      !found ||
      cm.getLine(found.from.line).length > maxHighlightLen ||
    ...
    var two =
      found.to &&
      cm.markText(found.to, Pos(found.to.line, found.to.ch + 1), {
        className: style,
      });
