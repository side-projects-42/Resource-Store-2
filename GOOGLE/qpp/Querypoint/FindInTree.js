...
(function () {
  ...
  Querypoint.FindInTree.findByDistanceFunction = function (tree, fncOfTree) {
    ...
    if (debug)
      console.log(
        "FindInTree closest " + visitor._closest,
        traceur.outputgeneration.TreeWriter.write(visitor.getMatchedTree())
      );
  ...
  Querypoint.FindInTree.byOffset = function (tree, offset) {
    ...
    return Querypoint.FindInTree.findByDistanceFunction(
      tree,
      byOffset.bind(this, offset)
    );
  };
  ...
  Querypoint.FindInTree.prototype = {
    ...
    visitAny: function (tree) {
      ...
      if (tree && tree.location && !tree.doNotTransform && !tree.doNotTrace) {
        ...
        if (debug) {
          var range = tree.location
            ? tree.location.start.offset + "-" + tree.location.end.offset
            : "no-location";
          console.log(
            "FindInTree " +
              distance +
