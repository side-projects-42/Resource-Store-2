...
(function () {
  ...
  RemoteWebPageProject.xhrFromBackground = new RemoteMethodCall.Requestor(
    XHRInBackground,
    ChannelPlate.DevtoolsTalker
  ).serverProxy();

  RemoteWebPageProject.prototype = Object.create(
    traceur.WebPageProject.prototype
  );
  ...
  //
  RemoteWebPageProject.prototype.loadResource = function (
    url,
    fncOfContentOrNull
  ) {
  ...
  RemoteWebPageProject.prototype.getPageScripts = function (callback) {
    ...
    function onScripts(remoteScripts) {
      // runs here
      this.remoteScripts = remoteScripts;
      if (debug)
        console.log(
          "RemoteWebPageProject found " + this.remoteScripts.length + " scripts"
        );
      callback();
    }

    chrome.devtools.inspectedWindow.eval(
      this.evalStringify(getScripts, []),
      onScripts.bind(this)
    );
  };
  ...
  RemoteWebPageProject.prototype.putPageScripts = function (scripts, callback) {
    function putScripts(scripts) {
      ...
      scripts.forEach(function (script) {
        ...
        try {
          var turn = window.__qp.startTurn("ScriptBody", [
            { name: script.name },
          ]);
    ...
    return chrome.devtools.inspectedWindow.eval(
      this.evalStringify(putScripts, scripts),
      callback
    );
  };
