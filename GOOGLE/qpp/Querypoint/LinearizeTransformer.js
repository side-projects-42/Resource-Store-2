...
(function () {
  ...
  LinearizeTransformer.prototype = {
    ...
    transformAny: function (tree) {
      var output_tree = ParseTreeTransformer.prototype.transformAny.call(
        this,
        tree
      );
      if (output_tree) {
    ...
    insertVariableFor: function (tree, traceId) {
      var linearExpression =
        Querypoint.InsertVariableForExpressionTransformer.prototype.insertVariableFor.call(
          this,
          tree,
          traceId
        );
    ...
    transformWhileStatement: function (tree) {
      ...
      var ifClause = new Block(tree.body.location, [
        this.wrapInLabels(
      ...
      var ifStatement = new IfStatement(null, condition, ifClause, elseBreak);
    ...
    transformForInStatement: function (tree) {
      ...
      if (
        ...
        tree = new ForInStatement(tree.location, initializer, collection, body);
      }
      var labels = this.getLabels(tree);
      var breakIdentifier = this.getBreakLabel();
      this.popIterationLabels();
    ...
    _transformBinaryAssignmentOperator: function (tree) {
      ...
      var tmp = this.insertVariableFor(
        right,
        this.generateIdentifier(tree.left)
      );
    ...
    _insertIfAroundExpression: function (condition, tree) {
      ...
      this.insertions.push(ifStatement);
    ...
    transformBinaryOperator: function (tree) {
      ...
      if (operator.isAssignmentOperator()) {
        ...
        return this._transformShortCircuitOperator(
          tree,
          operator.type === TokenType.OR
        );
      } else {
    ...
    transformBreakStatement: function (tree) {
      if (tree.name) {
        // labeled break ok as is
        return tree;
      } else {
        // else unlabeled break
        return new BreakStatement(
          tree.location,
          createIdentifierToken(this.getBreakLabel())
        );
      }
    },
    ...
    _transformCallExpressionOperand: function (tree) {
      if (tree.type === ParseTreeType.MEMBER_EXPRESSION) {
        ...
        tree = new MemberLookupExpression(
          tree.location,
          operand,
          memberExpression
        );
    ...
    transformPostfixExpression: function (tree) {
      ...
      // ++tmp3
      var postOperationExpr = this._binaryExpressionEquivalentToUnary(
        tree,
        preOperationValue
      );
      ...
      // tmp1[tmp2] = tmp4
      var assignment = new BinaryOperator(
        tree.location,
        operand,
        createOperatorToken(TokenType.EQUAL),
        postOperationTemp
      );

      // inserts tmp1[tmp2] = ++tmp3;
      this.insertions.push(new ExpressionStatement(tree.location, assignment));
    ...
    _transformUnaryAssignmentExpression: function (tree) {
      ...
      else throw new Error("Not a unary assignment expression");
      ...
      // obj.prop = tmp2
      var assignmentExpr = new BinaryOperator(
        tree.location,
        operand,
        createOperatorToken(TokenType.EQUAL),
        postOperationTemp
      ...
      var assignmentStatement = this.insertions.push(
        new ExpressionStatement(tree.location, assignmentExpr)
      );
    ...
    _transformTypeofExpression: function (tree) {
      ...
      // typeof expr
      var conditionLeft = new UnaryExpression(
        tree.location,
        tree.operator,
        operand
      );
        ...
        function (declaration) {
          ...
          this.insertions.push(
            new VariableStatement(
              ...
              new VariableDeclarationList(declaration.location, TokenType.VAR, [
                declaration,
              ])
            )
          );
