...
(function () {
  ...
  Querypoint.ElementChangeQuery.ifAvailableFor = function (
    project,
    selector,
    functionURL
  ) {
    ...
    var tree = project.find(
      functionInfo.filename,
      functionInfo.startOffset - 1
    );
    if (tree)
  ...
  Querypoint.ElementChangeQuery.prototype = {
    ...
    activate: function (queryIndex) {
      ...
      this._setTracedElementTransformer =
        new Querypoint.SetTracedElementTransformer(transformData);
    ...
    matches: function (ctor, queryData) {
      return (
        ctor === Querypoint.ElementChangeQuery &&
    ...
    runtimeSource: function () {
      var src = "";
      this._setTracedElementTransformer
        .runtimeInitializationStatements()
    ...
    extractTracepoints: function (
      ...
      function onEval(result, isException) {
        if (
          ...
          query._lastLoadEvaluated ===
            sessionViewModel.loadListViewModel.loadStartedNumber()
        )
          return;
        query._lastEvaluated = result.turn;
        query._lastLoadEvaluated =
        ...
        if (
          !isException &&
          result.tracepoints &&
          result.tracepoints instanceof Array
        ) {
          ...
          console.error(
            "ValueChangeQuery extractTracepoints eval failed",
            isException,
            result
          );
        }
      ...
      var previousTurn =
        sessionViewModel.loadListViewModel.lastLoad().turnStarted() - 1;
      var thisLoad = sessionViewModel.loadListViewModel.loadStartedNumber();

      if (
        !this._lastEvaluated ||
        this._lastLoadEvaluated !== thisLoad ||
        this._lastEvaluated === previousTurn
      ) {
        var tracedObjectIndex = query._queryIndex;
        query._properties.forEach(function (property) {
          var expr =
            'window.__qp.reducePropertyChangesToTracedObject("' +
            property +
            '",' +
            tracedObjectIndex +
            ")";
