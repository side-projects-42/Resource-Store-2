  ...
  var QPPreprocessor = {
    ...
    loadInIFrame: function (src) {
      ...
      function transcoderTest() {
        console.log(
          transcoder(
            'console.log("XXXXXXXXXXXXXXXXXXXXX success XXXXXXXXXXXXXXXXXXXXX");',
            "foo"
          )
        );
      }
      ...
      iframe.setAttribute(
        "src",
        toHTMLDataURL(Querypoint.toBase64DataURI(src))
      );
    ...
    source: function (descriptors, onSource) {
      ...
      else this._combineSources(descriptors, this.scripts, onSource);
    },
    ...
    _combineSources: function (descriptors, scripts, onSource) {
      this._scriptSource(scripts, function (concatentatedScripts) {
        ...
        wrapper += "  }\n";
        wrapper +=
          "return Querypoint.transformAndGenerate(input, '" + json + "');\n";
        wrapper += "}\n";
      ...
      function loadOne(scriptURL) {
        XHR.asyncLoadText(
          ...
          function errback(error) {
            console.error(
              "QPPreprocessor._scriptSource FAILED on " +
                scriptURL +
                " with " +
                error
            );
