...
(function () {
  ...
  var useAsyncPreprocessor = DebugLogger.register(
    "useAsyncPreprocessor",
    function (flag) {
      return (debug = typeof flag === "boolean" ? flag : debug);
    }
  );
  ...
  function QPProject(url, loads) {
    ...
    if (useAsyncPreprocessor)
      console.warn(
        "QPProject ctor using " +
          (Querypoint.QPPreprocessor.useAsyncPreprocessor ? "async" : "sync") +
          " preprocessor for " +
          url
      );
  }
  ...
  QPProject.prototype = {
    ...
    compile: function (onAllCompiled) {
      ...
      if (this.remoteScripts && this.remoteScripts.length)
        this.addFilesFromScriptElements(
          this.remoteScripts,
          onScriptsReady.bind(this)
        );
      else onAllCompiled(this.parseTrees_);
    },

    transformDescriptions: function () {
      var transformDescriptions = this.querypoints.tracequeries.reduce(
        function (descriptions, tq) {
    ...
    generateSourceFromTrees: function (treeObjectMap) {
      if (!treeObjectMap) return [];

      return treeObjectMap.keys().map(
        function (file) {
          ...
          file.generatedSource = this._fileCompiler.generateSourceFromTree(
            tree,
    ...
    startRuntime: function () {
      ...
      chrome.devtools.inspectedWindow.eval(
        this.evalStringify(startRuntime, []),
        onRuntimeStarted
      );
    },
    ...
    isGeneratedFile: function (name) {
      return Object.keys(this.sourceFiles_).some(
        function (key) {
    ...
    getTreeByName: function (name) {
      ...
      if (sourceFile) return this.getParseTree(sourceFile);
    },

    find: function (name, offset) {
      var tree = this.getTreeByName(name);
      if (tree) return this.treeFinder().byOffset(tree, offset);
    },
    ...
    reload: function () {
      ...
      var onNavigated = function (url) {
        ...
        if (url !== this.url) {
          console.error(
            "QPProject reload failed: url mismatch: " + url + "!==" + this.url
          );
        }
