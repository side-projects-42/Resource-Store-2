...
(function () {
  ...
  var QPFunctionPreambleTransformer =
    (Querypoint.QPFunctionPreambleTransformer = function () {
  ...
  QPFunctionPreambleTransformer.prototype = {
    ...
    _encodedSource: function (location) {
      return escape(
        location.start.source.contents.substring(
    ...
    _createInitializationStatements: function (tree) {
      var statements = [];
      this.functionLocations.forEach(
        function (location) {
          // window.__qp.functions["<file_name>"]["<functionId>"] = [];
          statements.push(
    ...
    _preparePreamble: function (tree) {
      ...
      this.__qp_functionToStringStatement =
        this._create__qp_functionToStringStatement(tree.location);
    },

    _createPreambleStatements: function (tree) {
      // tree here is function body
      var var__qp_functionStatement = this._create__qp_functionStatement(
        tree.location
      );
      var if__qp_functionCallStatement = this._createIfRedirectStatement(
        tree.location
      );
      return [
        var__qp_functionStatement,
        this.__qp_functionToStringStatement,
        if__qp_functionCallStatement,
      ];
    },
    ...
    transformFunctionBody: function (tree) {
      ...
      var preamble = this._createPreambleStatements(tree).map(
        Querypoint.markDoNot
      );
    ...
    transformProgram: function (tree) {
      ...
      var prefix = [this._createFileNameStatement(tree)].concat(
        this._createInitializationStatements(tree)
      );
