...
(function () {
  ...
  function toBase64DataURI(src) {
    ...
    return (
      "data:application/javascript;base64," + Querypoint.Base64.encode(src)
    );
  }
  ...
  function transformAndGenerate(input, descriptorsJSON) {
    ...
    var name =
      input.name || "eval_" + Math.random().toString().split(".")[1] + ".js";
    ...
    var generatedSource = fileCompiler.generateSourceFromTree(
      tree,
      name,
      descriptors
    );
    ...
    if (input.name) {
      ...
      var turnIndicator =
        "window.__qp._theTurn = window.__qp.startTurn('ScriptBody', [" +
