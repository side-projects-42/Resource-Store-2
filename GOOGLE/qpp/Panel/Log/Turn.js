...
(function () {
  ...
  QuerypointPanel.AddEventListenerRegistration = function (
    eventType,
    targetFnc,
    bubbles
  ) {
  ...
  QuerypointPanel.Turn = function (loadNumber, runtimeData) {
    Object.keys(runtimeData).forEach(
      function (key) {
    ...
    if (debug)
      console.log(
        "Turn " + runtimeData.turnNumber + " runtimeData",
        runtimeData
      );
  ...
  QuerypointPanel.Turn.prototype = {
    onSetTimeout: function (millis, target) {
      this.registeredEntryPoints.push(
        new QuerypointPanel.SetTimeoutRegistration(millis, target)
      );
    },

    onAddEventListener: function (eventType, target) {
      this.registeredEntryPoints.push(
        new QuerypointPanel.AddEventListenerRegistration(eventType, target)
      );
    },
