...
(function () {
    ...
    if (debug)
      console.log(
        "buildCausalChain " +
          turn.turnNumber +
          " caused by " +
          turn.registrationTurnNumber
      );
    if (turn.registrationTurnNumber)
      chain = buildCausalChain(
        allTurns,
        allTurns[turn.registrationTurnNumber - 1]
      );
  ...
  QuerypointPanel.TurnReplayTrigger = function (
    turnsToReplay,
    loadListViewModel,
    onReoccurance
  ) {
    ...
    while (potentialChainHeads.length) {
      ...
      if (
        replayStart >= chainHeadCauseNumber &&
        chainHeadCauseNumber <= replayEnd
      )
        continue;
      else
    ...
    this.currentTurnInReload = ko.computed(
      function () {
        if (this.replayTriggered()) return;
        var reloadModel = loadListViewModel.showLoad();
        if (reloadModel === loadModel) return;
        ...
        if (turns.length) {
          ...
          if (started === ended) {
            // wait until the turn ends before checking it.
  ...
  QuerypointPanel.Recorder = {
    ...
    onEndRecorded: function (event) {
      ...
      switch (state) {
        case "record":
          ...
          break;
        case "off":
          break; // not visible in UI
        case "recorded":
          this._eraseRecording();
          break;
        case "play":
          this._stopPlayback();
          break;
    ...
    stopIfRecording: function () {
      if (this.recordingState() === "record") this.onBeginRecorded();
    },
    ...
    _autoReplay: function () {
      if (debug)
        console.log(
          "Recorder autoReplay " + this._recordedTurns.length + " turns."
        );
    ...
    pageWasReloaded: function (runtimeInstalled, runtimeInstalling) {
      if (this._replayTrigger) this._replayTrigger.replayTriggered(false);
    },
  };
})();
