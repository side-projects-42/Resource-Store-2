  ...
  QuerypointPanel.TurnViewModel = {
    initialize: function (loadListViewModel, project, tracequeries) {
          ...
          if (currentTurnNumber) {
            ...
            if (load.turns) {
              ...
              if (turn) {
                ...
                console.warn(
                  "loadListViewModelViewModel.turn no entry in load.turns() for " +
                    (currentTurnNumber - 1),
                  Object.keys(load.turns())
                );
              }
            } else {
              if (load.loadNumber !== "-") {
                console.warn(
                  "loadListViewModelViewModel.turn no .turns in load",
                  load
                );
              }
            }
      ...
      this.triggeredEvents = ko.computed(function () {
        ...
        if (
          !currentTurn ||
          !currentTurn.firedEvents ||
          currentTurn.firedEvents.length === 0
        )
          return false;
      ...
      this.addedEvents = ko.computed(function () {
        ...
        if (
          !currentTurn ||
          !currentTurn.addedEvents ||
          currentTurn.addedEvents.length === 0
        )
          return false;
      ...
      this.switchTurn = function () {
        QuerypointPanel.loadListViewModelViewModel.turnViewModel.showTurn(
          this.turnNumber
        );
      };

      this.elementQueryProvider = new QuerypointPanel.ElementQueryProvider(
        project
      );
    ...
    createFileURL: function (eventInfo) {
      ...
      return this._project.createFileURL(
        eventInfo.filename,
        startOffset,
        endOffset
      );
    ...
    traceElement: function (turnViewModel) {
      ...
      this.query = this.elementQueryProvider.getQueriesBySelector(
        selector,
        functionURL
      )[0];
        ...
        if (selector === "#document") {
          ...
          DOM.querySelector(root.nodeId, selector, function (error, node) {
            DOM.highlightNode(highlightConfig, node, "", function (error) {
              if (debug && error)
                console.error(
                  "loadListViewModelViewModel.highlightElement FAILED:",
                  error
                );
