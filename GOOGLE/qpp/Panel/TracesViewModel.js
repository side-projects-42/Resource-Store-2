...
(function () {
  ...
  QuerypointPanel.TracesViewModel = function (
    querypointViewModel,
    loadListViewModel,
    tracequeries
  ) {
    ...
    this.currentTreeTrace = ko.computed(
      function () {
    ...
    this.treeTraces = ko
      .computed(
        function () {
          var tracepoints;
          if (
            loadListViewModel.showLoad().loadNumber !==
            loadListViewModel.loadStartedNumber()
          ) {
          ...
          if (tree) {
            ...
            tracepoints.reduce(function (treeTracepoints, tracepoint) {
              if (tracepoint.query.targetTree() === tree)
                treeTracepoints.push(tracepoint);
            ...
            tracequeries().reduce(function (prompts, query) {
              if (query.targetTree() === tree)
                prompts.push(query.tracePrompt());
              return prompts;
            }, prompts);
    ...
    function traceBodySpace() {
      ...
      return (
        hoverDoorTarget.offsetHeight -
        tokenView -
        explainTokenPanel.offsetHeight -
        queryView.offsetHeight
      );
    }
          ...
          if (tree && tracequeries().length) {
            ...
            if (traces && traces.length) {
              var traceViewModels = traces.map(function (trace) {
                return new QuerypointPanel.TraceViewModel(
                  trace,
                  querypointViewModel.project
                );
              });
