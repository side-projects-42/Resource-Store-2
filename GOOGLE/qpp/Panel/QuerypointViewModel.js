...
(function () {
  ...
  QuerypointPanel.QuerypointViewModel = function (
    panel,
    sourceFile,
    editorViewModel
  ) {
    ...
    this.tokenViewModel = new QuerypointPanel.TokenViewModel(
      this,
      this._editorViewModel,
      panel
    ); // wired to editor token
    this.tracesViewModel = new QuerypointPanel.TracesViewModel(
      this,
      panel.sessionViewModel.loadListViewModel,
      panel.tracequeries
    ); // wired to token viewed
    this.queryProvider = new QuerypointPanel.TokenQueryProvider(
      this.tokenViewModel,
      this.project
    ); // wired to token viewed.
    this.queriesViewModel = new QuerypointPanel.QueriesViewModel(
      this.queryProvider,
      panel
    );
    this.lineNumberViewModel = new QuerypointPanel.LineNumberViewModel(
      this,
      editorViewModel,
      panel
    );
    ...
    this.currentLocation = ko.computed(
      function () {
        ...
        if (!this.tracesViewModel.currentTraces()) return tokenViewLocation;

        var traceViewLocation = this.tracesViewModel.currentLocation();
        if (traceViewLocation === tokenViewLocation) return traceViewLocation;
      }.bind(this)
    );

    this._turnSubscription = panel.sessionViewModel.currentTurn.subscribe(
      function (newValue) {
        if (newValue) {
      ...
      .computed(
        function () {
          if (this._hoverDoorTarget && this.treeRoot()) {
            var hoverDoorChannel =
              this._hoverDoorTarget.querySelector(".hoverDoorChannel");
  ...
  QuerypointPanel.QuerypointViewModel.prototype = {
    ...
    update: function (turn) {
      ...
      if (!tracequeries.length) return;

      tracequeries.forEach(
        function (tq) {
          tq.extractTracepoints(
            this,
            function (tracepoint) {
              if (tracepoint) {
