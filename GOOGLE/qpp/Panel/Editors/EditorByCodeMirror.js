  ...
  EditorByCodeMirror.prototype = {
    ...
    setSize: function () {
      ...
      this._resize(
        ...
        this._container.parentElement.offsetHeight - 1
      );
    },
    ...
    insertElement: function (line, column, element, scrollIntoView) {
      var widgetCharCoord = this.editorImpl.charCoords(
        { line: line, ch: column },
        "page"
      );
      var lineCharCoord = this.editorImpl.charCoords(
        { line: line, ch: 0 },
        "page"
      );
    ...
    showRegion: function (startIndex, endIndex, cssClass) {
      ...
      var mark = this.editorImpl.markText(startPos, endPos, {
        className: cssClass,
      });
      return mark;
    },
    ...
    _onViewportChange: function (editor, from, to) {
      if (debug)
        console.log(
          "EditorByCodeMirror onViewportChange" +
            this.name +
            ":" +
            from +
            "," +
      ...
      this.dispatch("onViewportChange", {
        name: this.name,
        start: from,
        end: to,
      });
    ...
    _onMouseOver: function (event) {
      // emit tokenEvent iff things change
      var pos = this.editorImpl.coordsChar({
        top: event.clientY,
        left: event.clientX,
      });
      var posDidChange =
