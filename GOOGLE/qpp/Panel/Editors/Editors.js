  ...
  QuerypointPanel.Editors = {
    initialize: function (buffers, statusBar, commands) {
      ...
      chrome.devtools.inspectedWindow.onResourceContentCommitted.addListener(
        this._onResourceUpdate.bind(this)
      );
    ...
    */
    openURL: function (project, url) {
      ...
      if (sourceFile) {
        ...
        var editorViewModel;
        var foundResource = this._panel.page.resources.some(
          function (resource) {
            if (resource.url) editorViewModel = this.openResourceView(resource);
          }.bind(this)
        );
        if (!editorViewModel)
          editorViewModel = this.openErrorMessage(
            "Found no source file or resource named " + url
          );
    ...
    openResourceView: function (resource) {
      ...
      resource.getContent(
        function (content, encoding) {
          this._createEditor(editorViewModel, resource.url, content);
        }.bind(this)
      );
      return editorViewModel;
    },
    ...
    openSourceFileView: function (sourceFile) {
      var editorViewModel = new QuerypointPanel.EditorViewModel();
      this._createEditor(editorViewModel, sourceFile.name, sourceFile.contents);
      return editorViewModel;
    },

    openErrorMessage: function (content) {
      var editorViewModel = new QuerypointPanel.EditorViewModel();
      this._createEditor(editorViewModel, "error", content);
      return editorViewModel;
    },
    ...
    _createEditor: function (editorViewModel, url, content) {
      ...
      editorViewModel.statusSubscription = editorViewModel.status.subscribe(
        this._updateStatusBar.bind(this, editorViewModel)
      );
    ...
    _updateStatusBar: function (editorViewModel, status) {
      ...
      switch (status) {
        ...
        default:
          console.error(
            "Editors._updateStatusBar unknown editor-status " +
              status +
              " for " +
              editor.getName()
          );
    ...
    _getEditorByName: function (name) {
      ...
      if (index !== -1) return this._editors[index];
    },
    ...
    _onResourceUpdate: function (resource, content) {
      if (this._statusBar.unsavedEditors.indexOf(resource.url) !== -1) {
        ...
        if (editor) editor.resetContent(content);
      }
    },
