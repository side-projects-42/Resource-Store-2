...
(function () {
  ...
  QuerypointPanel.Panel = function (extensionPanel, panel_window, project) {
    ...
    this.sessionViewModel = QuerypointPanel.SessionViewModel.initialize(
      project,
      this.tracequeries
    );
    ...
    $(".panel").live("click", function (jQueryEvent) {
      ...
      if (url) {
        ...
        target.dispatchEvent(
          new CustomEvent("navigatedFrom", { bubbles: true })
        );
  ...
  QuerypointPanel.Panel.prototype = {
    ...
    */
    ...
    _initEditors: function (panelModel) {
      this._statusBar = QuerypointPanel.StatusBar.initialize(
        this,
        this.sessionViewModel
      );
      this._editors = QuerypointPanel.Editors.initialize(
        panelModel.buffers,
        this._statusBar,
        this.commands
      );
      ...
      this._fileChainViewModel = new QuerypointPanel.FileChainViewModel(
        workspaceElement,
        this
      );
    ...
    pageWasReloaded: function (runtimeInstalled, runtimeInstalling) {
      this.sessionViewModel.pageWasReloaded(
        runtimeInstalled,
        runtimeInstalling
      );
