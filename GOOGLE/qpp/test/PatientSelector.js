...
(function (global) {
  ...
  var PatientSelector = {
    ...
    whenSelectorAll: function (selector, textToMatch, callback, errback) {
      ...
      PatientSelector.hits = PatientSelector._querySelectorAll(
        selector,
        textToMatch
      );
      if (PatientSelector.hits.length) {
        ...
        try {
          PatientSelector._setMutationObservers(
            selector,
            textToMatch,
            callback
          );
        } catch (exc) {
          if (errback) errback(exc);
          else
            console.error(
              "....PatientSelector._setMutationObservers FAIL " + exc,
              exc
            );
        }
      }
    ...
    _querySelectorAll: function (selector, opt_textToMatch) {
      ...
      try {
        ...
        console.error(
          "....PatientSelector._querySelectorAll query failed for " +
            selector +
            ": " +
            exc,
          targets
      ...
      if (debug)
        console.log(
          "....PatientSelector._querySelectorAll finds " +
            PatientSelector._selected.length +
            " matches for " +
            selector
        );
    ...
    _setMutationObservers: function (selector, textToMatch, callback) {
      PatientSelector._disconnectOnFind = function () {
        ...
        if (PatientSelector._textChangeObservers) {
          PatientSelector._textChangeObservers.forEach(
            function (observer) {
              observer.disconnect();
              if (debug)
                console.log(
                  "....PatientSelector._setMutationObservers disconnect  " +
                    observer._textToMatch,
                  observer
                );
            }.bind(PatientSelector)
          );
          PatientSelector._totalTextChangeObservers -=
            PatientSelector._textChangeObservers.length;
          if (debug)
            console.log(
              "....PatientSelector._setMutationObservers _totalTextChangeObservers " +
                PatientSelector._totalTextChangeObservers,
              PatientSelector
            );
        }
        ...
        if (debug)
          console.log(
            "....PatientSelector.whenSelectorAll found " +
              PatientSelector.hits.length +
              " for " +
              selector +
              " with text " +
              textToMatch
      ...
      if (PatientSelector._selected.length) {
        if (PatientSelector._textChangeObservers) {
          console.error(
            "....PatientSelector._setMutationObservers unexpected _textChangeObservers ",
            PatientSelector._textChangeObservers
          );
        }
        PatientSelector._textChangeObservers = PatientSelector._selected.map(
          PatientSelector._createTextChangeObserver.bind(
            PatientSelector,
            textToMatch
          )
        );
        if (debug)
          console.log(
            "....PatientSelector._setMutationObservers initial _totalTextChangeObservers " +
              PatientSelector._totalTextChangeObservers,
            PatientSelector
          );
      }
      PatientSelector._addedSelectionObserver = new MutationSummary({
        callback: PatientSelector._whenSelectorHits.bind(
          PatientSelector,
          textToMatch,
          PatientSelector._disconnectOnFind.bind(PatientSelector)
        ),
        queries: [{ element: selector }],
      });
      if (debug) {
        console.log(
          "....PatientSelector.whenSelectorAll waiting for '" +
            selector +
            "' with text " +
            textToMatch +
            " in " +
    ...
    _textSelectorAll: function (nodes, textToMatch) {
      return nodes.reduce(function findTextMatching(nodes, node) {
        if (node.textContent.indexOf(textToMatch) !== -1) nodes.push(node);
        return nodes;
      }, []);
    },
    ...
    _textChanges: function (textToMatch, callback, mutationSummary) {
      if (debug)
        console.log(
          "....PatientSelector._textChanges mutationSummary for " + textToMatch,
          mutationSummary[0]
        );
      var target = mutationSummary[0].target;
      this.hits = this._textSelectorAll([target], textToMatch);
      if (this.hits.length) callback(this.hits);
    },
    ...
    _createTextChangeObserver: function (textToMatch, element) {
      ...
      if (debug)
        console.log(
          "....PatientSelector._createTextChangeObserver _totalTextChangeObservers " +
            PatientSelector._totalTextChangeObservers,
          PatientSelector
        );
      var handler = PatientSelector._textChanges.bind(
        PatientSelector,
        textToMatch,
        PatientSelector._disconnectOnFind.bind(PatientSelector)
      );
    ...
    _whenSelectorHits: function (textToMatch, callback, mutationSummary) {
      if (debug)
        console.log(
          "....PatientSelector._whenSelectorHits mutationSummary for " +
            textToMatch,
          mutationSummary[0]
        );
      var added = mutationSummary[0].added;
      PatientSelector.hits = PatientSelector._textSelectorAll(
        added,
        textToMatch
      );
      if (PatientSelector.hits.length) {
      ...
      added.forEach(
        function (element) {
          PatientSelector._textChangeObservers =
            PatientSelector._textChangeObservers || [];
          PatientSelector._textChangeObservers.push(
            PatientSelector._createTextChangeObserver(textToMatch, element)
          );
        }.bind(PatientSelector)
      );
    },
  ...
  var PatientEvents = {
    ...
    mouseToSelector: function (type, selector, textToMatch, callback) {
      ...
      PatientSelector.whenSelectorAll(
        selector,
        textToMatch,
        function () {
          ...
          if (debug)
            console.log(
              "....PatientEvents." + type + "Selector hit ",
              PatientSelector.hits[0]
            );
        }.bind(PatientEvents)
      );
    },
    ...
    _moveLineNumber: function (lineNumber, visibleSourceLines, selector, text) {
      while (lineNumber < visibleSourceLines.length) {
        ...
        if (hits.length) {
          if (debug)
            console.log(
              "....PatientEvents._moveLineNumber " +
                selector +
                "&" +
                text +
                " match " +
                visibleSourceLines[lineNumber].textContent
            );
          lineNumber++;
          break;
        } else {
      ...
      if (lineNumber === visibleSourceLines.length - 1) {
        console.error(
          "...PatientEvents._moveLineNumber no source line matchs " +
            selector +
            " & " +
            text,
          visibleSourceLines
    ...
    _fireMouseMove: function (tokenElt, callback) {
      function onMouseMove(event) {
        ...
        setTimeout(function () {
          callback(
            target.textContent + " in " + target.parentElement.textContent
          );
        });
      }
      ...
      if (debug)
        console.log(
          "....PatientEvents._fireMouseMove mousemove(" +
            xy.x +
            "," +
            xy.y +
            ") sent to %o",
  ...
  var PatientEditor = {
    ...
    selectTokenInSource: function (editorTokens, callback) {
      if (debug)
        console.log(
          "....PatientEvents.selectTokenInSource(" +
            editorTokens.length +
            " editorTokens) ",
          editorTokens
        );
      var visibleSourceElement =
        PatientSelector._querySelectorAll(".CodeMirror-lines");
      ...
      function next(lineNumber) {
        ...
        if (editorTokens.length) {
          lineNumber = PatientEvents._moveLineNumber(
            lineNumber,
            visibleSourceLines,
            selector,
            text
          );
          if (lineNumber) next(lineNumber);
          else callback("err");
        } else {
          if (debug)
            console.log(
              "....PatientEvents.selectTokenInSource seeking pre ancestor of ",
              PatientSelector.hits
            );
          PatientEvents.ancestor("pre", function () {
            if (!PatientSelector.hits.length) {
              console.error(
                "....PatientEvents.selectTokenInSource ancestor selection failed",
                PatientEvents
              );
              callback();
            }
            // select within the previous hit
            var tokenElts = PatientSelector._querySelectorAll(
              "| " + selector,
              text
            );
  ...
  var PatientRemoteOperations = {
    ...
    evaluateInPage: function (expr, callback) {
      function checkException(result, isException) {
        if (isException) {
          console.error(
            "....PatientRemoteOperations.evaluateInPage(" +
              expr +
              ") exception",
            isException
          );
    ...
    evaluate: function (expr, callback) {
      try {
        ...
        console.error(
          "....PatientRemoteOperations.evaluate(" + expr + ") exception " + exc,
          exc
        );
      }
    },
    ...
    extractText: function (selector, callback) {
      var text = PatientSelector._querySelectorAll(selector)
        .map(function (node) {
    ...
    getBoundingClientRect: function (selector, textToMatch, callback) {
      var rects = PatientSelector._querySelectorAll(selector, textToMatch).map(
        function (node) {
    ...
    getStyle: function (selector, textToMatch, callback) {
      var styles = PatientSelector._querySelectorAll(selector, textToMatch).map(
        function (node) {
    ...
    createProxy: function (port, iframeURL) {
      // onConnect from extension iframe

      function onMessage(message) {
        if (debug)
          console.log(
            "....PatientRemoteOperations.proxyTo.onMessage ",
            message
          );
        var payload = message;
        ...
        if (handler) {
          if (status === "Error") handler.onError(payload);
          else handler.onResponse(payload);
          delete PatientSelector.proxyHandlers[postId];
        } else {
          console.error(
            "....PatientRemoteOperations.createProxy.onMessage no handler for " +
              postId +
              " in " +
              doc(),
            message
      ...
      var proxy = (PatientRemoteOperations.proxies[iframeURL] =
        new ChannelPlate.Base(port, onMessage.bind(PatientRemoteOperations)));
      if (debug)
        console.log("....PatientRemoteOperations.createProxy for " + iframeURL);
    ...
    _postToProxy: function (proxy) {
      var handler =
        PatientRemoteOperations.proxyHandlers[PatientRemoteOperations.postId];
      if (handler && !handler.sent) {
        handler.sent = proxy.postMessage(handler.args);
        if (debug)
          console.log(
            "....PatientRemoteOperations.proxyTo.postMessage sent: " +
              handler.sent +
              " to " +
              handler.url,
            handler.args
    ...
    proxyTo: function (url, proxied, callback, errback) {
      if (debug)
        console.log("....PatientRemoteOperations.proxyTo " + url, proxied);
      var postId = ++PatientRemoteOperations.postId;
      PatientRemoteOperations.proxyHandlers[postId] = {
        url: url,
        onResponse: callback,
        onError: errback,
        args: [postId].concat(proxied),
      };

      var proxy;
      Object.keys(PatientRemoteOperations.proxies).some(
        function (iframeURL) {
      ...
      if (proxy) {
        ...
        if (debug)
          console.log("....PatientRemoteOperations.proxyTo waiting for " + url);
      }
    },
  };
