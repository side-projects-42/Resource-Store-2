  ...
  var Vim = function () {
    ...
    function beforeSelectionChange(cm, obj) {
      ...
      if (head.ch && head.ch == cm.doc.getLine(head.line).length) {
        ...
        obj.update([
          { anchor: cursorEqual(head, anchor) ? pos : anchor, head: pos },
        ]);
      }
    }
      ...
      evalInput: function (cm, vim) {
        ...
        if (motion) {
          ...
          if (vim.visualMode) {
            ...
            selectionEnd = curEnd;
            selectionStart =
              motionResult instanceof Array ? curStart : selectionStart;
            if (vim.visualLine) {
        ...
        if (!vim.visualMode) {
          ...
          if (vim.visualLine) {
            ...
            curEnd = clipCursorToContent(
              cm,
              Pos(curStart.line + repeat - 1, lineLength(cm, curStart.line)),
              true /** includeLineBreak */
            );
          } else {
            curEnd = clipCursorToContent(
              cm,
              Pos(curStart.line, curStart.ch + repeat),
              true /** includeLineBreak */
            );
          }
      ...
      unmap: function (lhs, ctx) {
        ...
        if (lhs != ":" && lhs.charAt(0) == ":") {
          ...
          for (var i = 0; i < defaultKeymap.length; i++) {
            if (
              arrayEquals(keys, defaultKeymap[i].keys) &&
        ...
        while (text) {
          ...
          if (vim.insertMode) {
            repeatInsertModeChanges(
              cm,
              register.insertModeChanges[imc++].changes,
              1
            );
            exitInsertMode(cm);
          }
