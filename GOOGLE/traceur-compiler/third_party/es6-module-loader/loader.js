...
(function (__global) {
  (function () {
              ...
              loadPromises.push(
                requestLoad(loader, request, load.name, load.address)
                  ...
                  .then(function (depLoad) {
                    console.assert(
                      !load.dependencies.some(function (dep) {
    ...
    function buildLinkageGroups(load, loads, groups, loader) {
      ...
      if (indexOf.call(groups[load.groupIndex], load) != -1) return;
      ...
      for (var i = 0, l = loads.length; i < l; i++) {
        ...
        for (var j = 0; j < load.dependencies.length; j++) {
          if (loadDep.name == load.dependencies[j].value) {
            ...
            console.assert(
              loadDep.status == "loaded",
              "Load in linkSet not loaded!"
            );

            // if it is a group transition, the index of the dependency has gone up
            // otherwise it is the same as the parent
            var loadDepGroupIndex =
              load.groupIndex + (loadDep.kind != load.kind);

            // the group index of an entry is always the maximum
            if (
              loadDep.groupIndex === undefined ||
              loadDep.groupIndex < loadDepGroupIndex
            ) {
              // if already in a group, remove from the old group
              if (loadDep.groupIndex) {
                groups[loadDep.groupIndex].splice(
                  indexOf.call(groups[loadDep.groupIndex], loadDep),
                  1
                );
    ...
    function link(linkSet) {
      ...
      var curGroupDeclarative =
        (startingLoad.kind == "declarative") == groups.length % 2;
      ...
      var registryEntry = load.declare.call(
        ...
        function (name, value) {
          ...
          if (typeof name == "object") {
            for (var p in name) moduleObj[p] = name[p];
          }
      ...
      for (var i = 0, l = load.dependencies.length; i < l; i++) {
        ...
        if (!loader.modules[depName]) {
          ...
          for (var j = 0; j < loads.length; j++) {
            if (loads[j].name != depName) continue;
        ...
        if (module.setters[i]) module.setters[i](depModule.module);
      }
    ...
    function ensureEvaluated(module, seen, loader) {
      if (module.evaluated || !module.dependencies) return;
      ...
      if (module.failed) return new Error("Module failed execution.");

      if (module.evaluated) return;

      module.evaluated = true;
      err = doExecute(module);
      if (err) module.failed = true;

      // spec variation
      // we don't create a new module here because it was created and ammended
      // we just disable further extensions instead
      if (Object.preventExtensions) Object.preventExtensions(module.module);
    ...
    Loader.prototype = {
      ...
      delete: function (name) {
        return this._loader.modules[name]
          ? delete this._loader.modules[name]
          : false;
      },
      ...
      import: function (name, options) {
        ...
        return Promise.resolve(
          ...
          return (
            ...
            createImportPromise(
              name,
              loadModule(loader, name, options || {}).then(function (load) {
      ...
      load: function (name, options) {
        ...
        return (
          importPromises[name] ||
          createImportPromise(name, loadModule(this._loader, name, {}))
        );
      },
      // 26.3.3.11
      ...
      newModule: function (obj) {
        ...
        if (Object.preventExtensions) Object.preventExtensions(m);
