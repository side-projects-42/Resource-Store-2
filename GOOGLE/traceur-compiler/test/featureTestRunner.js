...
function cloneTest(name, url) {
  function doTest(source) {
    ...
    if (reporter.hadError()) {
      assert.fail(
        "cloneTest Error compiling " + name + ".\n" + reporter.errorsAsString()
      );
      return;
    }
    ...
    if (reporter.hadError()) {
      assert.fail(
        "Error compiling generated code for " +
          name +
          ".\n" +
  ...
  test(name, (done) => {
    let load = {
      metadata: {},
      normalizedName: ModuleStore.normalize(url),
    };
    load.address = System.locate(load);
    moduleLoader.fetch({ address: url }).then(
      (data) => {
        doTest(data);
        done();
      },
