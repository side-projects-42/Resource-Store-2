...
require.extensions[".js"] = function (module, path) {
  if (!/\/node_modules\//.test(path)) {
    ...
    var compiled = traceurAPI.compile(
      content,
      {
        modules: "commonjs",
        importRuntime: true,
    ...
    if (needsWrapper(path)) {
      var header =
        'var assert = require("chai").assert, test = require("mocha").test;' +
        'test("' +
        path +
        '", function(){';
      var footer = "});";
...
var blackList = ["/Modules/ModuleName.js"];
...
function shouldSkip(path, content) {
  ...
  return (
    /\/\/ (Error:|Options:|Skip.|Async.)/.test(content) ||
    /\.script\.js$/.test(path)
  );
}
