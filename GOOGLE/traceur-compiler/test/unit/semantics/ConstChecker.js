...
suite("ConstChecker.js", function () {
  function makeTest(name, code, expectedErrors, mode) {
    test(name, function () {
      ...
      var parser = new Parser(
        new SourceFile("SOURCE", code),
        reporter,
        options
      );
      var tree =
        mode === "script" ? parser.parseScript() : parser.parseModule();
  ...
  makeTest("const prefix", "const x = 1; ++x;", [
    "SOURCE:1:16: x is read-only",
  ]);
  makeTest("const prefix", "const x = 1; --x;", [
    "SOURCE:1:16: x is read-only",
  ]);

  makeTest("const postfix", "const x = 1; x++;", [
    "SOURCE:1:14: x is read-only",
  ]);
  makeTest("const postfix", "const x = 1; x--;", [
    "SOURCE:1:14: x is read-only",
  ...
  makeTest("function params", "const x = 1;\n" + "function f(y = (x = 2)) {}", [
  ...
  makeTest("class declaration", "var m;\n" + "class C { m() { m = 2; } }", []);
  makeTest(
    "class declaration",
    "class C {\n" + "  m() {\n" + "    C = 2;\n" + "  }\n" + "}",
  ...
  makeTest("class expression", "var m;\n" + "(class C { m() { m = 2; } })", []);
  makeTest(
    "class expression",
    "(class C {\n" + "  m() {\n" + "    C = 2;\n" + "  }\n" + "})",
    ["SOURCE:3:5: C is read-only"]
  ...
  makeTest("Duplicate declarations", "let x;\n" + "{\n" + "  var x;\n" + "}", [
    "SOURCE:3:7: Duplicate declaration, x",
  ]);

  makeTest(
    "let bound function declaration",
    "const x = 1;\n" + "{\n" + "  function x() {}\n" + "}",
    []
  );
  ...
  makeTest("Array comprehension", "let x;\n" + "[for (x of []) x]", []);
  makeTest(
    "Array comprehension",
    "let x;\n" + "[for (x of []) for (x of []) x]",
    []
  );
  makeTest("Array comprehension", "const x = 1;\n" + "[for (x of []) x]", []);
  makeTest(
    "Array comprehension",
    "const x = 1;\n" + "[for (y of (x = [])) x]",
  ...
  makeTest("Generator comprehension", "let x;\n" + "(for (x of []) x)", []);
  makeTest(
