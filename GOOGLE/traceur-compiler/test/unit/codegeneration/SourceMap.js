...
suite("SourceMap.js", function () {
  ...
  var scriptCompilerLowRes = new Compiler({
    sourceMaps: "file",
    script: true,
  ...
  test("relativeToSource", function () {
    assert.equal(
      relativePath("@foo", "/w/t/out/"),
      "@foo",
      "@ names are unchanged"
    );

    assert.equal(
      relativePath("/w/t/src/foo.js", "/w/t/out/"),
      "../src/foo.js",
      "relative to sourceRoot in /out"
    );

    assert.equal(
      relativePath("/w/t/src/bar/foo.js", "/w/t/out/"),
      "../src/bar/foo.js",
      "deeper left side"
    );

    assert.equal(
      relativePath("/w/t/src/bar/foo.js", "/w/t/out/baz/"),
      "../../src/bar/foo.js",
      "deeper both side"
    );

    assert.equal(
      relativePath("/w/t/src/foo.js", "/w/t/src/"),
      "foo.js",
      "same directory  "
    );
    ...
    assert.equal(
      relativePath("/w/t/t/w/c/d/s/js/greeting.js", "/w/t/t/w/c/d/d/js/"),
      "../../s/js/greeting.js",
      "src/js bug"
    );
  });
  ...
  test("SourceMap", function () {
    ...
    generatedLines.forEach(function (line, index) {
      assert.equal(
        line.length - 1,
        expectedFilledColumnsZeroThrough[index],
  ...
  test("SourceMap with low resolution option", function () {
    ...
    var consumer = new SourceMapConsumer(
      scriptCompilerLowRes.getSourceMap(filename)
    );
