...
suite("context test", function () {
  ...
  teardown(function () {
    ...
    if (tempMapName && fs.existsSync(tempMapName)) fs.unlinkSync(tempMapName);
    tempMapName = null;
  });
  ...
  function executeFileWithRuntime(fileName, options, debug) {
    ...
    return loader.script(source, { metadata: metadata }).then(function () {
      ...
      if (debug) context.debugGenerated = true;

      try {
        vm.runInNewContext(runtime + output, context, fileName);
      } catch (ex) {
        console.error(
          "Executing (bin/traceur-runtime.js + transcoded " +
            fileName +
            ") failed",
          ex
        );
  ...
  test("class", function (done) {
    ...
    executeFileWithRuntime(fileName)
      .then(function (value) {
  ...
  test("generator", function (done) {
    ...
    executeFileWithRuntime(fileName, options)
      .then(function (value) {
  ...
  test("compiled modules", function (done) {
    ...
    exec(
      executable + " --out " + tempFileName + " -- " + inputFilename,
  ...
  test("compiled modules with --source-maps and sourceroot", function (done) {
    ...
    var commandLine =
      executable +
      " --source-maps --source-root=true --out " +
      tempFileName +
      " -- " +
      inputFilename;
    exec(commandLine, function (error, stdout, stderr) {
      ...
      assert(foundInput, "the inputFilename is one of the sourcemap sources");
      done();
    });
  });
  ...
  test("compiled modules with --source-maps=inline and sourceroot", function (done) {
    ...
    var commandLine =
      executable +
      " --source-maps=inline --source-root=true --out " +
      tempFileName +
      " -- " +
      inputFilename;
    exec(commandLine, function (error, stdout, stderr) {
      ...
      var transcoded = fs.readFileSync(tempFileName, "utf-8");
      var m =
        /\/\/#\s*sourceMappingURL=data:application\/json;base64,(.*)/.exec(
          transcoded
        );
      var b64string = m[1];
      ...
      var foundInput = map.sources.some(function (name) {
        return (
          inputFilename === forwardSlash(path.resolve(actualSourceRoot, name))
        );
      });
      assert(foundInput, "the inputFilename is one of the sourcemap sources");
      done();
  ...
  test("sourceMappingURL from --source-maps=true and deep directories", function (done) {
    ...
    var cmd =
      executable +
      " --source-maps " +
      "--source-root=false --out ./dist/js/bundle.js " +
      inputFilename;
    var child = exec(cmd, function (error, stdout, stderr) {
      ...
      var inputRelativeToOutput = path
        .relative(path.dirname(tempFileName), absoluteInputFilename)
        .replace(/\\/g, "/");
      var foundInput = map.sources.some(function (name) {
      ...
      assert(
        foundInput,
        "the inputFilename " +
          inputRelativeToOutput +
          " is one of the sourcemap sources " +
          map.sources.join(",")
      );
      done();
    });
  ...
  test("compiled modules sourcemaps=memory", function (done) {
    ...
    var inputFilename = resolve(
      "test/unit/runtime/resources/CallsThrowsError.js"
    );
    var executable = "node " + resolve("src/node/command.js");
  ...
  test("compiled modules inline with outputLanguage=es6", function (done) {
    ...
    exec(
      executable +
        " --out " +
        tempFileName +
        " --modules=inline --outputLanguage=es6 -- " +
        inputFilename,
      function (error, stdout, stderr) {
        assert.isNull(error);
        executeFileWithRuntime(tempFileName)
  ...
  test("working dir doesn't change when recursive compiling", function (done) {
    var recursiveCompile =
      require("../../../src/node/recursiveModuleCompile").recursiveModuleCompileToSingleFile;
    tempFileName = resolve(uuid.v4() + ".js");
    var inputFilename = resolve("test/unit/node/resources/import-x.js");
    var rootSources = [
      {
  ...
  test("script option per file", function (done) {
    ...
    exec(
      executable + " --out " + tempFileName + " --script " + inputFilename,
      function (error, stdout, stderr) {
        assert.isNull(error);
  ...
  test("./traceur can mix require() and import", function (done) {
    var cmd =
      "node ./traceur --require -- ./test/unit/node/resources/testForRequireAndImport.js";
    exec(cmd, function (error, stdout, stderr) {
      assert.isNull(error);
      assert.equal(
        "we have path and x=x and aNodeExport=intoTraceur\n",
        stdout
      );
      done();
    });
  ...
  test("./traceur --source-maps can report errors on the correct lines", function (done) {
    this.timeout(4000); // On travis this test takes more than 2s
    var cmd =
      "node ./traceur --source-maps=memory ./test/unit/node/resources/testErrorForSourceMaps.js";
    exec(cmd, function (error, stdout, stderr) {
      var m = /Test error on line ([0-9]*)/.exec(error);
      assert(m && m[1], "The evaluation should fail with the thrown error");
      assert.notEqual(
        error.toString().indexOf(":" + m[1] + ":"),
        -1,
  ...
  test("compile module dir option AMD", function (done) {
    ...
    var cmd =
      executable + " --dir " + inputDir + " " + outDir + " --modules=amd";
    exec(cmd, function (error, stdout, stderr) {
  ...
  test("compile module dir option CommonJS", function (done) {
    ...
    exec(
      executable + " --dir " + inputDir + " " + outDir + " --modules=commonjs",
      function (error, stdout, stderr) {
        if (error) return done(error);
        checkFile(outDir, goldenDir, "file.js");
        checkFile(outDir, goldenDir, "dep.js");
  ...
  test("compile module dir option CommonJS with source-maps", function (done) {
    ...
    exec(cmd, function (error, stdout, stderr) {
      if (error) return done(error);
      var fileMapContents = fs.readFileSync(
        path.resolve(outDir, "file.js.map")
      );
  ...
  test("tval uses argv", function (done) {
    var cmd =
      "node tval test/unit/node/resources/test_tval.js --arg1 --arg2 arg3";
    exec(cmd, function (error, stdout, stderr) {
      assert.isNull(error);
