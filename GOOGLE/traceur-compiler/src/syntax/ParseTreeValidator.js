...
export class ParseTreeValidator extends ParseTreeVisitor {
  ...
  visitBinaryExpression(tree) {
    switch (tree.operator.type) {
      ...
      case BAR_EQUAL:
        ...
        this.check_(
          tree.right.isAssignmentExpression(),
          tree.right,
      ...
      case STAR_STAR:
        this.check_(
          tree.left.isAssignmentExpression(),
          tree.left,
          "assignment expression expected"
        );
        this.check_(
  ...
  visitConditionalExpression(tree) {
    this.checkVisit_(
      tree.condition.isAssignmentExpression(),
      tree.condition,
      "expression expected"
    );
    this.checkVisit_(
  ...
  visitNamedExport(tree) {
    ...
    this.checkVisit_(
      specifierType === EXPORT_SPECIFIER ||
  ...
  visitImportDeclaration(tree) {
    if (tree.importClause !== null) {
      this.check_(
        tree.importClause.type === NAME_SPACE_IMPORT ||
  ...
  visitImportClausePair(tree) {
    ...
    this.check_(
      tree.second.type === NAME_SPACE_IMPORT ||
  ...
  visitJsxElement(tree) {
    ...
    for (let i = 0; i < tree.attributes.length; i++) {
      ...
      this.checkVisit_(
        attr.type === JSX_ATTRIBUTE || attr.type === JSX_SPREAD_ATTRIBUTE,
        attr,
  ...
  visitObjectPatternField(tree) {
    this.checkPropertyName_(tree.name);
    this.checkVisit_(
      tree.element.type === ASSIGNMENT_ELEMENT ||
  ...
  visitPostfixExpression(tree) {
    this.checkVisit_(
      tree.operand.isAssignmentExpression(),
      tree.operand,
      "assignment expression expected"
    );
  }
  ...
  visitScript(tree) {
    for (let i = 0; i < tree.scriptItemList.length; i++) {
      ...
      this.checkVisit_(
        scriptItemList.isScriptElement(),
        scriptItemList,
  ...
  visitPropertyNameAssignment(tree) {
    this.checkPropertyName_(tree.name);
    this.checkVisit_(
      tree.value.isAssignmentExpression(),
      tree.value,
      "assignment expression expected"
    );
  }
  ...
  visitSpreadExpression(tree) {
    this.checkVisit_(
      tree.expression.isAssignmentExpression(),
      tree.expression,
      "assignment expression expected"
    );
  }
  ...
  visitTypeName(tree) {
    this.checkVisit_(
      tree.moduleName === null || tree.moduleName.type === TYPE_NAME,
      tree.moduleName,
  ...
  visitTypeParameter(tree) {
    this.check_(
      tree.identifierToken.type === IDENTIFIER,
      tree,
      "Type parameter must be an identifier token"
    );
    if (tree.extendsType) {
      this.checkVisit_(
        tree.extendsType.isType(),
        tree.extendsType,
  ...
  visitVariableDeclaration(tree) {
    ...
    if (tree.initializer !== null) {
      this.checkVisit_(
        tree.initializer.isAssignmentExpression(),
        tree.initializer,
        "assignment expression expected"
      );
    }
