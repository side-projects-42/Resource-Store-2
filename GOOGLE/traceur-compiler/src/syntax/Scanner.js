...
import { idContinueTable, idStartTable } from "./unicode-tables.js";
...
export function isIdentifierPart(code) {
  return code < 128
    ? isIdentifierPartArray[code]
    : inTable(idStartTable, code) ||
        inTable(idContinueTable, code) ||
        code === 8204 ||
        code === 8205; // <ZWNJ>, <ZWJ>
}
...
function skipRegularExpressionBackslashSequence() {
  ...
  if (isLineTerminator(currentCharCode) || isAtEnd()) {
    reportError(
      "New line not allowed in regular expression literal",
      beginIndex,
      index
    );
    return false;
...
function scanPostZero(beginIndex) {
  switch (currentCharCode) {
    ...
    case 66: // B
    case 98: // b
      if (!options.numericLiterals) break;
    ...
    case 111: // o
      ...
      if (!isOctalDigit(currentCharCode)) {
        reportError(
          "Octal Integer Literal must contain at least one digit",
          beginIndex
        );
      }
      skipOctalDigits();
