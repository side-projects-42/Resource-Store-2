...
define(["crx2app/rpc/ChromeProxy", "atopwi/appendFrame"], function (
  ChromeProxy,
  appendFrame
) {
  var debug = true;
  ...
  Debuggee.prototype = {
    attachToParent: function () {
      ...
      this.portToAtopwi = new ChannelPlate.ChildIframe(
        function (message) {
    ...
    attachToChrome: function () {
      ...
      this.chromeConnection.attach(
        function onConnectedToChrome() {
          ...
          this.chrome = ChromeProxy.new(this.chromeConnection, {
            windows: {},
            tabs: {
              onRemoved: function () {
    ...
    open: function (debuggeeSpec) {
      ...
      if (debug) {
        console.log(
          "Debuggee parsed debuggeeSpec %o and got %o",
          debuggeeSpec,
          this
        );
      }
    ...
    rerouteMessages: function () {
      ...
      this.inspectorWindow.InspectorFrontendHost.sendMessageToBackend =
        function () {
    ...
    loadExtensions: function () {
      var optionsString = window.localStorage.getItem(
        "DevtoolsExtended.options"
      );
      if (debug) console.log("loadExtensions", optionsString);
      ...
      if (options.extensionInfos && options.extensionInfos.length) {
        var infos = options.extensionInfos.map(
          function (info) {
    ...
    navigateToURL: function (inspectorReady) {
      if (this.url) {
        // then we started in a new tab, navigate
        if (debug) {
    ...
    sendMessageObject: function (messageObject) {
      if (debug) {
        ...
        from += messageObject.requestId
          ? messageObject.requestId + " from extension "
          : "";
      ...
      function handleSendCommandResponse(id, data) {
        ...
        if (debug) {
          var msg =
            id + " atopwi response to sendCommand " + messageObject.method;
          var obj = {
    ...
    listenForTestRunner: function () {
      chrome.extension.onMessage.addListener(
        function (message, sender, sendResponse) {
          ...
          if (method) {
            sendResponse(
              method.apply(this.testRunnerResponder, message.arguments)
            );
          } else {
            console.error(
              "testRunnerResponder: no such method " +
                message.method +
                " from TestRunner ",
              message
            );
    ...
    loadCompleted: function () {
      if (this.obeyTestRunner) {
        if (debug)
          console.log(
            "loadCompleted, sending runTest with window.InspectorTest: ",
            window.InspectorTest
          );
        chrome.extension.sendMessage(
          { to: "testPage", method: "runTest", arguments: [] },
