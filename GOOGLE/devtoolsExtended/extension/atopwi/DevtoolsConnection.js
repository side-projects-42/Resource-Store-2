...
define(["appendFrame"], function (appendFrame) {
  var debug = true;
  ...
  var DevtoolsConnection = {
    ...
    showWaiting: function () {
      window.document.querySelector(".devtoolsBackendURL").textContent =
        this.debuggee.devtoolsURL;
      this._timeoutIndex = setTimeout(this._reportTimeout.bind(this), 2000);
    },

    _reportTimeout: function () {
      window.document.querySelector(".splashWaitingMessage").textContent =
        " No response!";
    },

    showInspectorIframe: function () {
      clearTimeout(this._timeoutIndex);
      var inspectorElt = window.document.querySelector(
    ...
    listenDebuggee: function (childFrame) {
      ...
      ChannelPlate.Parent(
        ...
        function (rawPort) {
          ...
          this.portToDevtools = new ChannelPlate.Base(
            rawPort,
    ...
    onMessage: function (message) {
      if (debug) {
        console.log(
          "atopwi puts debuggee %o and hears %o",
          this.debuggee,
          message
        );
      }
