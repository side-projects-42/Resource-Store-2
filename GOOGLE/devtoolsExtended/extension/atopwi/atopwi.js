...
var DevtoolsExtendedBase =
  "chrome-extension://ggimboaoffjaeoblofehalflljohnfbl";
...
function URLOptions() {
  ...
  var defaultPreset =
    localStorage.getItem("DevtoolsExtended.frontendURL") ||
    window.DevtoolsExtendedBase + "/WebInspectorKit/Source/devtools/front_end";
  preset(defaultPreset);

  document.querySelector(".crxFrontEnd").addEventListener("click", function () {
    preset(
      window.DevtoolsExtendedBase +
        "/WebInspectorKit/Source/devtools/front_end/"
    );
  });
  document
    .querySelector(".testingFrontEnd")
    .addEventListener("click", function () {
      preset(
        "http://localhost:9696/WebInspectorKit/Source/devtools/front_end/"
      );
    });
...
URLOptions.prototype = {
  ...
  useCrx2app: function () {
    return (
      this.params.hasOwnProperty("url") || this.params.hasOwnProperty("tabId")
    );
  },
