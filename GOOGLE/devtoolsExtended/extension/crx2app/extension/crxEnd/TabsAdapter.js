...
var makeTabsAdapter = function (chrome, PostSource) {
  function TabsAdapter(windowsAdapter) {
    ...
    Object.keys(portDelegate).forEach(
      function (key) {
  ...
  TabsAdapter.prototype = {
    chromeWrappers: {
      create: function (serial, createProperties) {
        var cleanCreateProperties =
          this._cleanseCreateProperties(createProperties);
    ...
    onCreated: function (chromeTab) {
      ...
      var tabAdapter = this;
      this.windowsAdapter.barrier(
        chromeTab.windowId,
        arguments,
        function (windowId, index) {
          // |this| is windowsAdapter inside of barrier()
          this.addTab(chromeTab.id);
          this.postMessage({
            source: tabAdapter.getPath(),
            method: "onCreated",
            params: { tab: chromeTab },
          });
    ...
    warnAttached: function (tabId) {
      chrome.pageAction.setTitle({
        tabId: tabId,
        title: "This tab controlled by " + this.windowsAdapter.debuggerOrigin,
      });
    ...
    onUpdated: function (tabId, changeInfo, tab) {
      this.barrier(tabId, arguments, function (tabId, changeInfo, tab) {
        ...
        this.postMessage({
          source: this.getPath(),
          method: "onUpdated",
          params: { tabId: tabId, changeInfo: changeInfo, tab: tab },
        });
      });
    },
    ...
    onRemoved: function (tabId, removeInfo) {
      this.barrier(tabId, arguments, function (tabId, removeInfo, index) {
        ...
        this.postMessage({
          source: this.getPath(),
          method: "onRemoved",
          params: { tabId: tabId, removeInfo: removeInfo },
        });
      });
    },
    ...
    onPageActionClicked: function () {
      ...
      chrome.windows.update(
        this.windowsAdapter.debuggerTab.windowId,
        updateInfo,
        function (win) {
          if (chrome.extension.lastError) {
            console.error(
              "crx2app onPageActionClicked ERROR ",
              chrome.extension.lastError
            );
          } else {
            if (debugWarnings)
              console.log(
                "crx2app onPageActionClicked update window complete",
                win
              );
          }
        }
      ...
      chrome.tabs.update(
        this.windowsAdapter.debuggerTab.id,
        updateProperties,
        function (tab) {
          if (chrome.extension.lastError) {
            console.error(
    ...
    _cleanseCreateProperties: function (createProperties) {
      console.assert(
        typeof createProperties.id === "number",
        "The createProperties.id must be a number"
      );
      var chromeWindowId =
        this.windowsAdapter.chromeWindowIds[createProperties.id];
      if (chromeWindowId) {
        return createProperties;
      }
      var msg =
        "The createProperties.id " +
        createProperties.id +
        " is not a valid chrome.window id";
      this.postError(msg);
    },
