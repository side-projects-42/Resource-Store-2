...
function buildContextMenuItem(title, url, onDebuggerWindowCreated) {
  ...
  function getClickHandler() {
    return function (info, tab) {
      ...
      createData.width =
        insureNumber(width) || Math.floor(window.screen.availWidth / 2) - 2;

      var height = parseInt(window.localStorage.getItem("windowHeight"), 10);
      createData.height = Math.max(
      ...
      createData.left =
        insureNumber(left) ||
        window.screen.availLeft + (window.screen.availWidth - createData.width);

      // Open Purple in a new window.
      chrome.windows.create(
        createData,
        onDebuggerWindowCreated.bind(null, info, tab)
      );
    };
  }
