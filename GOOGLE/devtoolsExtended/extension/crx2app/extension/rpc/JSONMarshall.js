...
define(["crx2app/rpc/Cause"], function (Cause) {
  ...
  function makeSendCommand(channel, target, method, debuggee) {
    ...
    return function sendCommand() {
      // arguments here will depend upon method
  ...
  function makeRemoteDebugSendCommand(
    chromeProxy,
    name,
    domainMethod,
    debuggee
    ...
    return function () {
      return chromeProxy.debugger.sendCommand(
        debuggee,
        domainMethod,
        arguments
      );
  ...
  JSONMarshall.recvResponse = function (data) {
    ...
    if (data && data.serial) {
      ...
      if (method === "onError") {
        console.error(
          "JSONMarshal.recvResponse ERROR " + data.source + ":" + data.params[0]
        );
      } else {
        if (method) {
          ...
          if (this.jsonHandlers.hasOwnProperty(objectKey)) {
            ...
            if (handler) {
              ...
              if (object.jsonObjectHandler) {
                ...
                console.warn(
                  "JSONMarshal.recvResponse dropped data, no handler for " +
                    method,
                  data
                );
              }
            ...
            console.warn(
              "JSONMarshal.recvResponse dropped data, no handlers for object " +
                objectKey,
              data
            );
          }
          ...
          console.error(
            "JSONMarshal.recvResponse dropped data, no .serial and  .method ",
            data
          );
        }
      }
  ...
  JSONMarshall.build2LevelCommands = function (
    iface,
    impl,
    chromeProxy,
    debuggee
    ...
    domains.forEach(function buildSend(domain) {
      if (iface[domain].commands) {
        ...
        methods.forEach(function buildMethod(method) {
          // each RHS is a function returning a promise
          impl[domain][method] = makeRemoteDebugSendCommand(
            chromeProxy,
            iface.version,
            domain + "." + method,
            debuggee
