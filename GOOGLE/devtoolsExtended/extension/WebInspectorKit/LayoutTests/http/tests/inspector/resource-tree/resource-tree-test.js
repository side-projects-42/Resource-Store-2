var initialize_ResourceTreeTest = function () {
  InspectorTest.dumpResources = function (formatter) {
    ...
    function formatterWrapper(resource) {
      ...
      else results.push({ resource: resource, text: resource.url });
    }
  ...
  InspectorTest.dumpResourcesURLMap = function () {
    ...
    function collect(resource) {
      results.push({
        url: resource.url,
        resource: WebInspector.resourceTreeModel.resourceForURL(resource.url),
      });
    }

    function comparator(result1, result2) {
      if (result1.url > result2.url) return 1;
    ...
    for (var i = 0; i < results.length; ++i)
      InspectorTest.addResult(
        results[i].url + " == " + results[i].resource.url
      );
  };
  ...
  InspectorTest.dumpResourcesTree = function () {
    function dump(treeItem, prefix) {
      ...
      if (typeof treeItem._resetBubble === "function") treeItem._resetBubble();
