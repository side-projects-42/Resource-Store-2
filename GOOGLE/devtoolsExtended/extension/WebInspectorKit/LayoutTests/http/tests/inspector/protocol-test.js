var initialize_ProtocolTest = function () {
  InspectorTest.filterProps = function (object, nondeterministicProps) {
    ...
    else if (typeof object === "number") object = "<number>";
  ...
  InspectorTest._dumpEvent = function () {
    ...
    InspectorTest.addResult(
      "event " + InspectorTest._agentName + "." + eventName
    );
    InspectorTest.addObject(
      InspectorTest._lastReceivedMessage,
      InspectorTest._customFormatters
    );
    InspectorTest.addResult("");
  ...
  InspectorTest._dumpCallArguments = function (callArguments) {
    ...
    InspectorTest.addResult(
      agentName + "." + functionName + "(" + expression + ")"
    );
    InspectorTest.addResult("");
  };
  ...
  InspectorTest.runProtocolTestSuite = function (
    agentName,
    testSuite,
    ...
    this._agentCoverage = {};
    for (var key in agent) this._agentCoverage[key] = "not checked";
    ...
    for (var eventName in domainMessagesHandler) {
      if (typeof domainMessagesHandler[eventName] !== "function") continue;
      this._agentCoverage[eventName] = "not checked";
      domainMessagesHandler[eventName] = InspectorTest._dumpEvent.bind(
        domainMessagesHandler,
        eventName,
        domainMessagesHandler[eventName]
      );
    }
    ...
    InspectorFrontendHost.sendMessageToBackend = function (message) {
      ...
      InspectorTest._originalSendMessageToBackend.apply(InspectorFrontendHost, [
        message,
      ]);
    };
