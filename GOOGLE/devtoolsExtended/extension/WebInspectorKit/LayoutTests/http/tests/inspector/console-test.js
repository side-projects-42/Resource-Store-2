var initialize_ConsoleTest = function () {
  ...
  InspectorTest.dumpConsoleMessages = function (
    printOriginatingCommand,
    dumpClassNames
  ) {
    var result = [];
    var visibleMessagesIndices =
      WebInspector.consoleView._visibleMessagesIndices;
  ...
  InspectorTest.dumpConsoleMessagesWithStyles = function (sortMessages) {
    ...
    for (var i = 0; i < indices.length; ++i) {
      var element =
        WebInspector.console.messages[indices[i]].toMessageElement();
      var messageText = InspectorTest.prepareConsoleMessageText(element);
      InspectorTest.addResult(messageText);
      var spans = element.querySelectorAll(
        ".console-message-text > span > span"
      );
      for (var j = 0; j < spans.length; j++)
        InspectorTest.addResult(
          "Styled text #" +
            j +
            ": " +
            (spans[j].style.cssText || "NO STYLES DEFINED")
        );
    }
  };
  ...
  InspectorTest.dumpConsoleMessagesWithClasses = function (sortMessages) {
    ...
    if (sortMessages) result.sort();
    for (var i = 0; i < result.length; ++i) InspectorTest.addResult(result[i]);
  };
  ...
  InspectorTest.expandConsoleMessages = function () {
    ...
    for (var i = 0; i < indices.length; ++i) {
      ...
      var node = element;
      while (node) {
        if (node.treeElementForTest) node.treeElementForTest.expand();
        node = node.traverseNextNode(element);
      }
    }
  };
  ...
  InspectorTest.checkConsoleMessagesDontHaveParameters = function () {
    ...
    for (var i = 0; i < indices.length; ++i) {
      ...
      InspectorTest.addResult(
        "Message: " +
          WebInspector.displayNameForURL(m.url) +
          ":" +
          m.line +
          " " +
          m.message
      ...
      if ("_parameters" in m) {
        if (m._parameters)
          InspectorTest.addResult(
            "FAILED: message parameters list is not empty: " + m._parameters
          );
        else
          InspectorTest.addResult(
            "SUCCESS: message parameters list is empty. "
          );
      } else {
        InspectorTest.addResult(
          "FAILED: didn't find _parameters field in the message."
        );
      }
    }
  };
