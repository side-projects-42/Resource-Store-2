var initialize_HeapSnapshotTest = function () {
  InspectorTest.createJSHeapSnapshotMockObject = function () {
    return {
      ...
      _containmentEdges: [
        2,
        6,
        3, //  0: shortcut 'a' to node 'A'
        1,
        7,
        6, //  3: property 'b' to node 'B'
        0,
        1,
        6, //  6: element '1' to node 'B'
        1,
        8,
        9, //  9: property 'ac' to node 'C'
        1,
        9,
        9, // 12: property 'bc' to node 'C'
        1,
        10,
        12, // 15: property 'bd' to node 'D'
        1,
        11,
        15,
      ], // 18: property 'ce' to node 'E'
  ...
  InspectorTest.createHeapSnapshotMockRaw = function () {
    ...
    return {
      ...
      edges: [
        ...
        2,
        6,
        7, // shortcut 'a' to node 'A'
        1,
        7,
        14, // property 'b' to node 'B'

        // A node edges
        0,
        1,
        14, // element 1 to node 'B'
        1,
        8,
        21, // property 'ac' to node 'C'

        // B node edges
        1,
        9,
        21, // property 'bc' to node 'C'
        1,
        10,
        28, // property 'bd' to node 'D'

        // C node edges
        1,
        11,
        35,
      ], // property 'ce' to node 'E'
  ...
  InspectorTest.createHeapSnapshotMock = function () {
    return InspectorTest._postprocessHeapSnapshotMock(
      InspectorTest.createHeapSnapshotMockRaw()
    );
  };
  ...
  InspectorTest.createHeapSnapshotMockWithDOM = function () {
    return InspectorTest._postprocessHeapSnapshotMock({
      ...
      nodes: [
        ...
        //    |----->F--->G       M
        //
        /* (root) */ 2, 0, 1, 4, /* Window */ 1, 11, 2, 2, /* Window */ 1, 11,
        3, 3, /* E */ 2, 5, 4, 0, /* F */ 2, 6, 5, 1, /* A */ 1, 1, 6, 0,
        /* B */ 1, 2, 7, 1, /* D */ 1, 4, 8, 2, /* H */ 1, 8, 9, 0, /* G */ 1,
        7, 10, 0, /* C */ 1, 3, 11, 0, /* N */ 1, 10, 12, 0, /* M */ 1, 9, 13,
        0,
      ],
      edges: [
        /* from (root) */ 0, 1, 4, 0, 2, 8, 0, 3, 12, 0, 4, 16,
        /* from Window */ 0, 1, 20, 0, 2, 24, /* from Window */ 0, 1, 24, 0, 2,
        28, 1, 3, 32, /* from F */ 0, 1, 36, /* from B */ 0, 1, 40,
  ...
  InspectorTest.startProfilerTest = function (callback) {
    ...
    InspectorTest._panelReset = InspectorTest.override(
      WebInspector.panels.profiles,
      "_reset",
      function () {},
      true
    );
    InspectorTest.addSniffer(
      ...
      "show",
      InspectorTest._snapshotViewShown,
      true
    );

    // Reduce the number of populated nodes to speed up testing.
    WebInspector.HeapSnapshotContainmentDataGrid.prototype.defaultPopulateCount =
      function () {
        return 10;
      };
    WebInspector.HeapSnapshotConstructorsDataGrid.prototype.defaultPopulateCount =
      function () {
        return 10;
      };
  ...
  InspectorTest.runHeapSnapshotTestSuite = function (testSuite) {
    ...
    function runner() {
      ...
      InspectorTest.addResult("");
      InspectorTest.addResult(
        "Running: " + /function\s([^(]*)/.exec(nextTest)[1]
      );
  ...
  InspectorTest.checkArrayIsSorted = function (contents, sortType, sortOrder) {
    ...
    function extractField(data, field) {
      ...
      if (!data[field])
        InspectorTest.addResult(
          "No " + field + " field in " + JSON.stringify(data)
        );
      return data[field];
    }
    ...
    var extractor = {
      text: function (data) {
        return extractField(data, "value");
      },
  ...
  InspectorTest.clickColumn = function (column, callback) {
    ...
    function sortingComplete() {
      InspectorTest._currentGrid().removeEventListener(
        "sorting complete",
        sortingComplete,
        this
      );
      InspectorTest.assertEquals(
        column.identifier,
        this._currentGrid().sortColumnIdentifier(),
        "unexpected sorting"
      );
    ...
    InspectorTest._currentGrid().addEventListener(
      "sorting complete",
      sortingComplete,
      this
    );
  ...
  InspectorTest.clickRowAndGetRetainers = function (row, callback) {
    ...
    var rootNode =
      InspectorTest.currentProfileView().retainmentDataGrid.rootNode();
  ...
  InspectorTest.columnContents = function (column, row) {
    ...
    for (
      ...
      node;
      node = node.traverseNextNode(true, parent, true)
    ) {
  ...
  InspectorTest.countDataRows = function (row, filter) {
    ...
    for (
      ...
      node;
      node = node.traverseNextNode(true, row, true)
    ) {
      if (filter(node)) ++result;
    }
    return result;
  };
  ...
  InspectorTest.HeapNode.prototype = {
    linkNode: function (node, type, nameOrIndex) {
      ...
      if (!node._builder) node._setBuilder(this._builder);

      if (nameOrIndex === undefined) nameOrIndex = this._edgesCount;
      ++this._edgesCount;

      if (nameOrIndex in this._edges)
        throw new Error(
          "Can't add edge with the same nameOrIndex. nameOrIndex: " +
            nameOrIndex +
            " oldNodeName: " +
            this._edges[nameOrIndex]._name +
            " newNodeName: " +
            node._name
        );
      this._edges[nameOrIndex] = new InspectorTest.HeapEdge(
        node,
        type,
        nameOrIndex
      );
    ...
    _setBuilder: function (builder) {
      if (this._builder) throw new Error("node reusing is prohibited");
    ...
    _serialize: function (rawSnapshot) {
      ...
      for (var i in this._edges) this._edges[i]._serialize(rawSnapshot);
    },
  };
  ...
  InspectorTest.HeapEdge.prototype = {
    _serialize: function (rawSnapshot) {
      if (!this._targetNode._builder)
        throw new Error(
          "Inconsistent state of node: " + this._name + " no builder assigned"
        );
      var builder = this._targetNode._builder;
      rawSnapshot.edges.push(builder.lookupEdgeType(this._type));
      rawSnapshot.edges.push(
        typeof this._nameOrIndex === "string"
          ? builder.lookupOrAddString(this._nameOrIndex)
          : this._nameOrIndex
      );
      rawSnapshot.edges.push(
        this._targetNode._ordinal * builder.nodeFieldsCount
      ); // index
    },
  };
  ...
  InspectorTest.HeapSnapshotBuilder.prototype = {
    generateSnapshot: function () {
      ...
      rawSnapshot.snapshot.edge_count =
        rawSnapshot.edges.length / meta.edge_fields.length;
      rawSnapshot.snapshot.node_count =
        rawSnapshot.nodes.length / meta.node_fields.length;
    ...
    lookupOrAddString: function (string) {
      if (string in this._string2id) return this._string2id[string];
  ...
  InspectorTest.createHeapSnapshot = function (instanceCount, firstId) {
    ...
    for (var i = 0; i < instanceCount; ++i) {
      ...
      var nodeB = new InspectorTest.HeapNode(
        "B",
        sizeOfB,
        undefined,
        firstId++
      ...
      var nodeA = new InspectorTest.HeapNode(
        "A",
        sizeOfA,
        undefined,
        firstId++
  ...
  InspectorTest.findButtonsNode = function (row, startNode) {
    var result = 0;
    for (
      var node = startNode || row.children[0];
      node;
      node = node.traverseNextNode(true, row, true)
    ) {
      if (!node.selectable && node.showNext) return node;
    }
    return null;
  };
  ...
  InspectorTest.findRow = function (columnIdentifier, matcher, parent) {
    ...
    for (
      ...
      if (matcher(node.data[columnIdentifier])) return node;
    }
    return null;
  };
  ...
  InspectorTest.findRow2 = function (matcher, parent) {
    ...
    for (
      ...
      if (matcher(node.data)) return node;
    }
    return null;
  };
  ...
  InspectorTest.takeAndOpenSnapshot = function (generator, callback) {
    ...
    var profileType = WebInspector.panels.profiles.getProfileType(
      WebInspector.HeapSnapshotProfileType.TypeId
    );
    var profile = profileType.createProfile({
      title:
        WebInspector.ProfilesPanelDescriptor.UserInitiatedProfileName +
        "." +
        uid,
      uid: uid,
