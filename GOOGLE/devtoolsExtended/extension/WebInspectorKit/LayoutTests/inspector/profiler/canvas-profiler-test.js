var initialize_CanvasWebGLProfilerTest = function () {
  InspectorTest.enableCanvasAgent = function (callback) {
    ...
    function canvasAgentEnabled(error) {
      if (!error) InspectorTest.safeWrap(callback)();
      else {
  ...
  InspectorTest.dumpTraceLogCall = function (call, indent, filter) {
    ...
    var args = (call.arguments || []).map(
      InspectorTest.callArgumentDescription
    );
    var properties = [
      "{Call}",
      show("functionName") && call.functionName
        ? 'functionName:"' + call.functionName + '"'
        : "",
      show("arguments") && call.arguments
        ? "arguments:[" + args.join(",") + "]"
        : "",
      show("result") && call.result
        ? "result:" + InspectorTest.callArgumentDescription(call.result)
        : "",
      show("property") && call.property
        ? 'property:"' + call.property + '"'
        : "",
      show("value") && call.value
        ? "value:" + InspectorTest.callArgumentDescription(call.value)
        : "",
      show("isDrawingCall") && call.isDrawingCall ? "isDrawingCall:true" : "",
      show("isFrameEndCall") && call.isFrameEndCall
        ? "isFrameEndCall:true"
        : "",
  ...
  InspectorTest.dumpResourceStateDescriptor = function (descriptor, indent) {
    ...
    if (descriptor.values) {
      ...
      if (descriptor.isArray) name += "[" + descriptor.values.length + "]";
      ...
      for (var i = 0, n = descriptors.length; i < n; ++i)
        InspectorTest.dumpResourceStateDescriptor(
          descriptors[i],
          indent + "  "
        );
    } else
      InspectorTest.addResult(
        indent +
          descriptor.name +
          ": " +
          InspectorTest.callArgumentDescription(descriptor.value)
      );
  };
