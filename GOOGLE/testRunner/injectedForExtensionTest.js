...
function injectedForExtensionTest(
  testURL,
  testParentURL,
  jsonSignalTokens,
  selectedForDebug
  ...
  if (!matchDevtoolsURL && !matchTestParentURL && !matchTestURL) return;

  var debugFlags = (window.DebugLogger = selectedForDebug
    ? JSON.parse(selectedForDebug)
    : []);
  ...
  if (debug) {
    ...
    console.log(
      "injectedForExtensionTest testParentURL " + testParentURL + " vs " + path
    );
    console.log("injectedForExtensionTest debugFlags " + debug, debugFlags);
  }

  if (matchTestURL) return;
  ...
  if (matchDevtoolsURL) {
    ...
    function startTestCaseServer() {
      ...
      ChannelPlate.Listener(testURL, function (rawPort, iframeURL) {
        testCaseServer = new RemoteMethodCall.Responder(
          PatientSelector,
          rawPort
        );
        if (debug)
          console.log(
            "injectedForExtensionTest startTestCaseServer in " +
      ...
      if (debug)
        console.log(
          "injectedForExtensionTest startTestCaseServer completed, appended iframe " +
            testURL
        );
    }
      ...
      extensionInfos.forEach(function (info) {
        if (info.startPage.indexOf(testParentURL) !== -1) {
          ChannelPlate.Listener(info.startPage, function (rawPort, iframeURL) {
            ...
            if (debug)
              console.log(
                "injectedForExtensionTest.startProxyServers in " +
                  window.location.href +
          ...
          if (debug)
            console.log(
              "injectedForExtensionTest.startProxyServers listening for " +
                info.startPage
            );
        }
      });
    ...
    if (debug)
      console.log(
        "injectedForExtensionTest found devtoolsURL, waiting for extensionsRegistering " +
          window.location.href
      );
    ...
    function startResponder() {
      ...
      window._testRunnerService.responder = new RemoteMethodCall.Responder(
        PatientSelector
      );
      window._testRunnerService.channelPlate = new ChannelPlate.ChildIframe(
        window._testRunnerService.responder.onMessage
      );
      window._testRunnerService.responder.accept(
        window._testRunnerService.channelPlate.port
      );
      window.removeEventListener("load", startResponder);
      if (debug)
        console.log(
          "injectedForExtensionTest.startListener in extension URL " +
            window.location.href
        );
    }
